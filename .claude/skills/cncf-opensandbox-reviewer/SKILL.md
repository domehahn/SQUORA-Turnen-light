...
---
# Cncf Opensandbox Reviewer

## Purpose

Review and operate OpenSandbox using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenSandbox code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenSandbox requires evidence-backed assessment.
...
- First-party source: https://github.com/alibaba/OpenSandbox
...
- [ ] Trace OpenSandbox inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenSandbox defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenSandbox least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenSandbox resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenSandbox release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenSandbox dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenSandbox identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenSandbox change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenSandbox documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenSandbox architecture and dependency summary with trust boundaries and persistent state.
- Include OpenSandbox validation commands or tests that are safe for the stated environment.
...
- OpenSandbox identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenSandbox failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenSandbox is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenSandbox as the basis for migration or production guidance.
