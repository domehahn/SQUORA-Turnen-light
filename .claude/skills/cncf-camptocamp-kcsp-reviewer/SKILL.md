---
name: "cncf-camptocamp-kcsp-reviewer"
description: "Review Camptocamp (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. As part of its Infrastructure services, Camptocamp provides Kubernetes consulting and training, with a focus on OpenShift installation and integration."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Camptocamp Kcsp Reviewer

## Purpose

Review and operate Camptocamp (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Camptocamp (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Camptocamp (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.camptocamp.com/container_orchestration
...
- [ ] Trace Camptocamp (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Camptocamp (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Camptocamp (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Camptocamp (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Camptocamp (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Camptocamp (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Camptocamp (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Camptocamp (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Camptocamp (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Camptocamp (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Camptocamp (KCSP) validation commands or tests that are safe for the stated environment.
...
- Camptocamp (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Camptocamp (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Camptocamp (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Camptocamp (KCSP) as the basis for migration or production guidance.
