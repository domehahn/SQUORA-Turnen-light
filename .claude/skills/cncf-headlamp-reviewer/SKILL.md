---
name: "cncf-headlamp-reviewer"
description: "Review Headlamp using its official documentation and repository in the Observability and Analysis / Observability category. Extensible open source multi-cluster Kubernetes user interface"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Headlamp Reviewer

## Purpose

Review and operate Headlamp using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Headlamp code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Headlamp requires evidence-backed assessment.
...
- First-party source: https://github.com/kubernetes-sigs/headlamp
...
- [ ] Trace Headlamp inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Headlamp defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Headlamp least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Headlamp resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Headlamp release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Headlamp dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Headlamp identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Headlamp change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Headlamp documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Headlamp architecture and dependency summary with trust boundaries and persistent state.
- Include Headlamp validation commands or tests that are safe for the stated environment.
...
- Headlamp identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Headlamp failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Headlamp is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Headlamp as the basis for migration or production guidance.
