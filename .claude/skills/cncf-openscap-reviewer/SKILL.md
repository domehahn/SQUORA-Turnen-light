---
name: "cncf-openscap-reviewer"
description: "Review OpenSCAP using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Openscap Reviewer

## Purpose

Review and operate OpenSCAP using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenSCAP code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenSCAP requires evidence-backed assessment.
...
- First-party source: https://github.com/OpenSCAP/openscap
...
- [ ] Trace OpenSCAP inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenSCAP defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenSCAP least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenSCAP resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenSCAP release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenSCAP dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenSCAP identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenSCAP change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenSCAP documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenSCAP architecture and dependency summary with trust boundaries and persistent state.
- Include OpenSCAP validation commands or tests that are safe for the stated environment.
...
- OpenSCAP identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenSCAP failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenSCAP is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenSCAP as the basis for migration or production guidance.
