---
name: "cncf-pravega-reviewer"
description: "Review Pravega using its official documentation and repository in the App Definition and Development / Streaming & Messaging category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Pravega Reviewer

## Purpose

Review and operate Pravega using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Pravega code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Pravega requires evidence-backed assessment.
...
- First-party source: https://github.com/pravega/pravega
...
- [ ] Trace Pravega inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Pravega defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Pravega least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Pravega resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Pravega release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Pravega dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Pravega identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Pravega change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Pravega documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Pravega architecture and dependency summary with trust boundaries and persistent state.
- Include Pravega validation commands or tests that are safe for the stated environment.
...
- Pravega identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Pravega failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Pravega is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Pravega as the basis for migration or production guidance.
