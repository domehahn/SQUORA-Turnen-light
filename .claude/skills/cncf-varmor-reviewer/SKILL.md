...
---
# Cncf Varmor Reviewer

## Purpose

Review and operate vArmor using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- vArmor code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving vArmor requires evidence-backed assessment.
...
- First-party source: https://github.com/bytedance/vArmor
...
- [ ] Trace vArmor inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check vArmor defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check vArmor least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check vArmor resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check vArmor release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check vArmor dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If vArmor identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a vArmor change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- vArmor documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a vArmor architecture and dependency summary with trust boundaries and persistent state.
- Include vArmor validation commands or tests that are safe for the stated environment.
...
- vArmor identity, version, configuration, dependencies, and deployment assumptions are explicit.
- vArmor failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that vArmor is secure, supported, compatible, or suitable.
- Using an unpinned latest version of vArmor as the basis for migration or production guidance.
