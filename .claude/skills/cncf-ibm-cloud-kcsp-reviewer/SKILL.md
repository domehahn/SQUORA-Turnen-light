...
---
# Cncf Ibm Cloud Kcsp Reviewer

## Purpose

Review and operate IBM Cloud (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- IBM Cloud (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving IBM Cloud (KCSP) requires evidence-backed assessment.
...
- [ ] Trace IBM Cloud (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check IBM Cloud (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check IBM Cloud (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check IBM Cloud (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check IBM Cloud (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check IBM Cloud (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If IBM Cloud (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a IBM Cloud (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- IBM Cloud (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a IBM Cloud (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include IBM Cloud (KCSP) validation commands or tests that are safe for the stated environment.
...
- IBM Cloud (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- IBM Cloud (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that IBM Cloud (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of IBM Cloud (KCSP) as the basis for migration or production guidance.
