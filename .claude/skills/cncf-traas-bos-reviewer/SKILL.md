...
---
# Cncf Traas Bos Reviewer

## Purpose

Review and operate TRaaS BOS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- TRaaS BOS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving TRaaS BOS requires evidence-backed assessment.
...
- First-party source: https://antchain.antgroup.com/products/biosp
...
- [ ] Trace TRaaS BOS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check TRaaS BOS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check TRaaS BOS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check TRaaS BOS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check TRaaS BOS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check TRaaS BOS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If TRaaS BOS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a TRaaS BOS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- TRaaS BOS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a TRaaS BOS architecture and dependency summary with trust boundaries and persistent state.
- Include TRaaS BOS validation commands or tests that are safe for the stated environment.
...
- TRaaS BOS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- TRaaS BOS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that TRaaS BOS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of TRaaS BOS as the basis for migration or production guidance.
