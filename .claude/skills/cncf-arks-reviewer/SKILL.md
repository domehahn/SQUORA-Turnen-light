---
name: "cncf-arks-reviewer"
description: "Review Arks using its official documentation and repository in the Inference / Framework category. Arks is an end-to-end framework for managing LLM-based applications within Kubernetes cluster. It provides a robust and extensible infrastructure tailored for deploying, orchestrating, and scaling LLM inference workloads in cloud-native environments."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Arks using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Arks code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Arks requires evidence-backed assessment.
...
- CNCF classification: Inference / Framework
- CNCF project maturity field: not specified
- First-party source: https://github.com/scitix/arks
...
- [ ] Trace Arks inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Arks defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Arks least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Arks resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Arks release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Arks dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Arks identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Arks change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Arks documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Arks architecture and dependency summary with trust boundaries and persistent state.
- Include Arks validation commands or tests that are safe for the stated environment.
...
- Arks identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Arks failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Arks is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Arks as the basis for migration or production guidance.
