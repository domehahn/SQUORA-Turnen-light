---
name: "cncf-unsloth-reviewer"
description: "Review Unsloth using its official documentation and repository in the Training / Post Training category. Unsloth Studio is a web UI for training and running open models like Gemma 4, Qwen3.5, DeepSeek, gpt-oss locally."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Unsloth Reviewer

## Purpose

Review and operate Unsloth using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Unsloth code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Unsloth requires evidence-backed assessment.
...
- First-party source: https://github.com/unslothai/unsloth
...
- [ ] Trace Unsloth inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Unsloth defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Unsloth least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Unsloth resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Unsloth release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Unsloth dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Unsloth identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Unsloth change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Unsloth documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Unsloth architecture and dependency summary with trust boundaries and persistent state.
- Include Unsloth validation commands or tests that are safe for the stated environment.
...
- Unsloth identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Unsloth failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Unsloth is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Unsloth as the basis for migration or production guidance.
