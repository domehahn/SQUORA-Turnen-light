---
name: "cncf-layotto-reviewer"
description: "Review Layotto using its official documentation and repository in the Serverless / Framework category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Layotto Reviewer

## Purpose

Review and operate Layotto using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Layotto code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Layotto requires evidence-backed assessment.
...
- First-party source: https://github.com/mosn/layotto
...
- [ ] Trace Layotto inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Layotto defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Layotto least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Layotto resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Layotto release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Layotto dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Layotto identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Layotto change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Layotto documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Layotto architecture and dependency summary with trust boundaries and persistent state.
- Include Layotto validation commands or tests that are safe for the stated environment.
...
- Layotto identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Layotto failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Layotto is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Layotto as the basis for migration or production guidance.
