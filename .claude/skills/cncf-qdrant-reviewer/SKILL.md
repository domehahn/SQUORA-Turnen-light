---
name: "cncf-qdrant-reviewer"
description: "Review Qdrant using its official documentation and repository in the App Definition and Development / Database category. Vector Database for the next generation of AI applications."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Qdrant Reviewer

## Purpose

Review and operate Qdrant using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Qdrant code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Qdrant requires evidence-backed assessment.
...
- First-party source: https://github.com/qdrant/qdrant
...
- [ ] Trace Qdrant inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Qdrant defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Qdrant least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Qdrant resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Qdrant release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Qdrant dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Qdrant identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Qdrant change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Qdrant documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Qdrant architecture and dependency summary with trust boundaries and persistent state.
- Include Qdrant validation commands or tests that are safe for the stated environment.
...
- Qdrant identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Qdrant failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Qdrant is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Qdrant as the basis for migration or production guidance.
