---
name: "cncf-athenz-reviewer"
description: "Review Athenz using its official documentation and repository in the Provisioning / Key Management category. Open source platform for X.509 certificate based service authentication and fine grained access control in dynamic infrastructures"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Athenz Reviewer

## Purpose

Review and operate Athenz using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Athenz code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Athenz requires evidence-backed assessment.
...
- First-party source: https://github.com/AthenZ/athenz
...
- [ ] Trace Athenz inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Athenz defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Athenz least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Athenz resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Athenz release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Athenz dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Athenz identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Athenz change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Athenz documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Athenz architecture and dependency summary with trust boundaries and persistent state.
- Include Athenz validation commands or tests that are safe for the stated environment.
...
- Athenz identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Athenz failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Athenz is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Athenz as the basis for migration or production guidance.
