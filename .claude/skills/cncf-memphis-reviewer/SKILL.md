---
name: "cncf-memphis-reviewer"
description: "Review Memphis using its official documentation and repository in the App Definition and Development / Streaming & Messaging category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Memphis Reviewer

## Purpose

Review and operate Memphis using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Memphis code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Memphis requires evidence-backed assessment.
...
- First-party source: https://github.com/memphisdev/memphis
...
- [ ] Trace Memphis inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Memphis defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Memphis least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Memphis resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Memphis release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Memphis dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Memphis identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Memphis change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Memphis documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Memphis architecture and dependency summary with trust boundaries and persistent state.
- Include Memphis validation commands or tests that are safe for the stated environment.
...
- Memphis identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Memphis failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Memphis is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Memphis as the basis for migration or production guidance.
