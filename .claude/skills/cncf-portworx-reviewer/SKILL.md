---
name: "cncf-portworx-reviewer"
description: "Review Portworx using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Portworx Reviewer

## Purpose

Review and operate Portworx using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Portworx code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Portworx requires evidence-backed assessment.
...
- [ ] Trace Portworx inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Portworx defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Portworx least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Portworx resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Portworx release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Portworx dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Portworx identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Portworx change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Portworx documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Portworx architecture and dependency summary with trust boundaries and persistent state.
- Include Portworx validation commands or tests that are safe for the stated environment.
...
- Portworx identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Portworx failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Portworx is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Portworx as the basis for migration or production guidance.
