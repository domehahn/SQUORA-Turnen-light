...
---
# Cncf Carvel Reviewer

## Purpose

Review and operate Carvel using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Carvel code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Carvel requires evidence-backed assessment.
...
- First-party source: https://github.com/carvel-dev/ytt
...
- [ ] Trace Carvel inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Carvel defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Carvel least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Carvel resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Carvel release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Carvel dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Carvel identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Carvel change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Carvel documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Carvel architecture and dependency summary with trust boundaries and persistent state.
- Include Carvel validation commands or tests that are safe for the stated environment.
...
- Carvel identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Carvel failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Carvel is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Carvel as the basis for migration or production guidance.
