---
name: "cncf-scylladb-reviewer"
description: "Review ScyllaDB using its official documentation and repository in the Data / Data Architecture category. NoSQL data store using the seastar framework, compatible with Apache Cassandra."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Scylladb Reviewer

## Purpose

Review and operate ScyllaDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ScyllaDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ScyllaDB requires evidence-backed assessment.
...
- First-party source: https://github.com/scylladb/scylladb
...
- [ ] Trace ScyllaDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ScyllaDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ScyllaDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ScyllaDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ScyllaDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ScyllaDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ScyllaDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ScyllaDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ScyllaDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ScyllaDB architecture and dependency summary with trust boundaries and persistent state.
- Include ScyllaDB validation commands or tests that are safe for the stated environment.
...
- ScyllaDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ScyllaDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ScyllaDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ScyllaDB as the basis for migration or production guidance.
