---
name: "cncf-trulens-743c42bc-reviewer"
description: "Review Trulens using its official documentation and repository in the AI Native Infra / Observability category. TruLens is a powerful open source library for evaluating and tracking large language model-based applications."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Trulens using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Trulens code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Trulens requires evidence-backed assessment.
...
- First-party source: https://github.com/truera/trulens
...
- [ ] Trace Trulens inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Trulens defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Trulens least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Trulens resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Trulens release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Trulens dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Trulens identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Trulens change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Trulens documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Trulens architecture and dependency summary with trust boundaries and persistent state.
- Include Trulens validation commands or tests that are safe for the stated environment.
...
- Trulens identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Trulens failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Trulens is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Trulens as the basis for migration or production guidance.
