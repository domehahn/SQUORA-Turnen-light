---
name: "cncf-open-vswitch-reviewer"
description: "Review Open vSwitch using its official documentation and repository in the Runtime / Cloud Native Network category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Open Vswitch Reviewer

## Purpose

Review and operate Open vSwitch using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Open vSwitch code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Open vSwitch requires evidence-backed assessment.
...
- First-party source: https://github.com/openvswitch/ovs
...
- [ ] Trace Open vSwitch inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Open vSwitch defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Open vSwitch least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Open vSwitch resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Open vSwitch release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Open vSwitch dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Open vSwitch identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Open vSwitch change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Open vSwitch documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Open vSwitch architecture and dependency summary with trust boundaries and persistent state.
- Include Open vSwitch validation commands or tests that are safe for the stated environment.
...
- Open vSwitch identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Open vSwitch failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Open vSwitch is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Open vSwitch as the basis for migration or production guidance.
