---
name: "cncf-tencent-kubernetes-engine-tke-reviewer"
...
---
# Cncf Tencent Kubernetes Engine Tke Reviewer

## Purpose

Review and operate Tencent Kubernetes Engine (TKE) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Tencent Kubernetes Engine (TKE) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Tencent Kubernetes Engine (TKE) requires evidence-backed assessment.
...
- First-party source: https://intl.cloud.tencent.com/products/tke
...
- [ ] Trace Tencent Kubernetes Engine (TKE) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Tencent Kubernetes Engine (TKE) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Tencent Kubernetes Engine (TKE) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Tencent Kubernetes Engine (TKE) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Tencent Kubernetes Engine (TKE) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Tencent Kubernetes Engine (TKE) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Tencent Kubernetes Engine (TKE) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Tencent Kubernetes Engine (TKE) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Tencent Kubernetes Engine (TKE) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Tencent Kubernetes Engine (TKE) architecture and dependency summary with trust boundaries and persistent state.
- Include Tencent Kubernetes Engine (TKE) validation commands or tests that are safe for the stated environment.
...
- Tencent Kubernetes Engine (TKE) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Tencent Kubernetes Engine (TKE) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Tencent Kubernetes Engine (TKE) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Tencent Kubernetes Engine (TKE) as the basis for migration or production guidance.
