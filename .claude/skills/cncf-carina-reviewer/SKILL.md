---
name: "cncf-carina-reviewer"
description: "Review Carina using its official documentation and repository in the Runtime / Cloud Native Storage category. Carina: an high performance and ops-free local storage for kubernetes"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Carina Reviewer

## Purpose

Review and operate Carina using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Carina code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Carina requires evidence-backed assessment.
...
- First-party source: https://github.com/carina-io/carina
...
- [ ] Trace Carina inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Carina defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Carina least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Carina resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Carina release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Carina dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Carina identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Carina change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Carina documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Carina architecture and dependency summary with trust boundaries and persistent state.
- Include Carina validation commands or tests that are safe for the stated environment.
...
- Carina identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Carina failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Carina is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Carina as the basis for migration or production guidance.
