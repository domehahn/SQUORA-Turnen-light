---
name: "cncf-tensor-security-reviewer"
description: "Review Tensor Security using its official documentation and repository in the Provisioning / Security & Compliance category. Tensor Security is a cloud-native security company focused on providing AI-based and full-stack cloud-native security solutions."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Tensor Security Reviewer

## Purpose

Review and operate Tensor Security using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Tensor Security code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Tensor Security requires evidence-backed assessment.
...
- First-party source: https://www.tensorsecurity.cn
...
- [ ] Trace Tensor Security inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Tensor Security defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Tensor Security least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Tensor Security resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Tensor Security release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Tensor Security dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Tensor Security identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Tensor Security change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Tensor Security documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Tensor Security architecture and dependency summary with trust boundaries and persistent state.
- Include Tensor Security validation commands or tests that are safe for the stated environment.
...
- Tensor Security identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Tensor Security failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Tensor Security is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Tensor Security as the basis for migration or production guidance.
