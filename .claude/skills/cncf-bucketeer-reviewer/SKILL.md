---
name: "cncf-bucketeer-reviewer"
description: "Review Bucketeer using its official documentation and repository in the Observability and Analysis / Feature Flagging category. A feature flag management platform created to help teams make better decisions, reduce deployment lead time, and release risk through feature flags."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Bucketeer Reviewer

## Purpose

Review and operate Bucketeer using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Bucketeer code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Bucketeer requires evidence-backed assessment.
...
- CNCF classification: Observability and Analysis / Feature Flagging
- CNCF project maturity field: not specified
- First-party source: https://github.com/bucketeer-io/bucketeer
...
- [ ] Trace Bucketeer inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Bucketeer defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Bucketeer least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Bucketeer resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Bucketeer release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Bucketeer dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Bucketeer identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Bucketeer change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Bucketeer documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Bucketeer architecture and dependency summary with trust boundaries and persistent state.
- Include Bucketeer validation commands or tests that are safe for the stated environment.
...
- Bucketeer identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Bucketeer failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Bucketeer is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Bucketeer as the basis for migration or production guidance.
