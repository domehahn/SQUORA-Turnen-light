---
name: "cncf-druid-reviewer"
description: "Review Druid using its official documentation and repository in the App Definition and Development / Database category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Druid using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Druid code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Druid requires evidence-backed assessment.
...
- [ ] Trace Druid inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Druid defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Druid least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Druid resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Druid release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Druid dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Druid identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Druid change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Druid documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Druid architecture and dependency summary with trust boundaries and persistent state.
- Include Druid validation commands or tests that are safe for the stated environment.
...
- Druid identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Druid failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Druid is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Druid as the basis for migration or production guidance.
