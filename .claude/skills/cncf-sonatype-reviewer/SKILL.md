---
name: "cncf-sonatype-reviewer"
description: "Review Sonatype using its official documentation and repository in the Provisioning / Security & Compliance category. Sonatype is the developer-friendly full-spectrum software supply, chain management platform helps organizations and software developers. One of the Sonatype products includes the open source software repository manager Sonatype Nexus Repository."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Sonatype Reviewer

## Purpose

Review and operate Sonatype using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Sonatype code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Sonatype requires evidence-backed assessment.
...
- First-party source: https://github.com/sonatype/nexus-public
...
- [ ] Trace Sonatype inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Sonatype defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Sonatype least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Sonatype resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Sonatype release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Sonatype dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Sonatype identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Sonatype change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Sonatype documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Sonatype architecture and dependency summary with trust boundaries and persistent state.
- Include Sonatype validation commands or tests that are safe for the stated environment.
...
- Sonatype identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Sonatype failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Sonatype is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Sonatype as the basis for migration or production guidance.
