---
name: "cncf-sentinel-reviewer"
description: "Review Sentinel using its official documentation and repository in the Orchestration & Management / Service Proxy category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Sentinel Reviewer

## Purpose

Review and operate Sentinel using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Sentinel code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Sentinel requires evidence-backed assessment.
...
- First-party source: https://github.com/alibaba/Sentinel
...
- [ ] Trace Sentinel inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Sentinel defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Sentinel least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Sentinel resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Sentinel release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Sentinel dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Sentinel identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Sentinel change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Sentinel documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Sentinel architecture and dependency summary with trust boundaries and persistent state.
- Include Sentinel validation commands or tests that are safe for the stated environment.
...
- Sentinel identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Sentinel failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Sentinel is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Sentinel as the basis for migration or production guidance.
