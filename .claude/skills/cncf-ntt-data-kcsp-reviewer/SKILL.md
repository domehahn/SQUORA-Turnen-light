...
---
# Cncf Ntt Data Kcsp Reviewer

## Purpose

Review and operate NTT DATA (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- NTT DATA (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving NTT DATA (KCSP) requires evidence-backed assessment.
...
- First-party source: https://us.nttdata.com/en/services/cloud-and-it-infrastructure-services
...
- [ ] Trace NTT DATA (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check NTT DATA (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check NTT DATA (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check NTT DATA (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check NTT DATA (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check NTT DATA (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If NTT DATA (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a NTT DATA (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- NTT DATA (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a NTT DATA (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include NTT DATA (KCSP) validation commands or tests that are safe for the stated environment.
...
- NTT DATA (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- NTT DATA (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that NTT DATA (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of NTT DATA (KCSP) as the basis for migration or production guidance.
