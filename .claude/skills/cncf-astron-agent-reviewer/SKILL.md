---
name: "cncf-astron-agent-reviewer"
description: "Review Astron Agent using its official documentation and repository in the AI Agent / Workflow Orchestration category. Enterprise-grade, commercial-friendly agentic workflow platform for building next-generation SuperAgents with multi-agent orchestration capabilities."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Astron Agent Reviewer

## Purpose

Review and operate Astron Agent using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Astron Agent code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Astron Agent requires evidence-backed assessment.
...
- CNCF classification: AI Agent / Workflow Orchestration
- CNCF project maturity field: not specified
- First-party source: https://github.com/iflytek/astron-agent
...
- [ ] Trace Astron Agent inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Astron Agent defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Astron Agent least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Astron Agent resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Astron Agent release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Astron Agent dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Astron Agent identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Astron Agent change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Astron Agent documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Astron Agent architecture and dependency summary with trust boundaries and persistent state.
- Include Astron Agent validation commands or tests that are safe for the stated environment.
...
- Astron Agent identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Astron Agent failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Astron Agent is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Astron Agent as the basis for migration or production guidance.
