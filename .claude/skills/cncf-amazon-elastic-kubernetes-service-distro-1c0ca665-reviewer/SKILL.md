...
---
# Cncf Amazon Elastic Kubernetes Service Distro 1c0ca665 Reviewer

## Purpose

Review and operate Amazon Elastic Kubernetes Service Distro (Amazon EKS-D) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Amazon Elastic Kubernetes Service Distro (Amazon EKS-D) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Amazon Elastic Kubernetes Service Distro (Amazon EKS-D) requires evidence-backed assessment.
...
- CNCF classification: Platform / Certified Kubernetes - Distribution
- CNCF project maturity field: not specified
- First-party source: https://github.com/aws/eks-distro
...
- [ ] Trace Amazon Elastic Kubernetes Service Distro (Amazon EKS-D) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Amazon Elastic Kubernetes Service Distro (Amazon EKS-D) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Amazon Elastic Kubernetes Service Distro (Amazon EKS-D) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Amazon Elastic Kubernetes Service Distro (Amazon EKS-D) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Amazon Elastic Kubernetes Service Distro (Amazon EKS-D) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Amazon Elastic Kubernetes Service Distro (Amazon EKS-D) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Amazon Elastic Kubernetes Service Distro (Amazon EKS-D) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Amazon Elastic Kubernetes Service Distro (Amazon EKS-D) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Amazon Elastic Kubernetes Service Distro (Amazon EKS-D) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Amazon Elastic Kubernetes Service Distro (Amazon EKS-D) architecture and dependency summary with trust boundaries and persistent state.
- Include Amazon Elastic Kubernetes Service Distro (Amazon EKS-D) validation commands or tests that are safe for the stated environment.
...
- Amazon Elastic Kubernetes Service Distro (Amazon EKS-D) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Amazon Elastic Kubernetes Service Distro (Amazon EKS-D) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Amazon Elastic Kubernetes Service Distro (Amazon EKS-D) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Amazon Elastic Kubernetes Service Distro (Amazon EKS-D) as the basis for migration or production guidance.
