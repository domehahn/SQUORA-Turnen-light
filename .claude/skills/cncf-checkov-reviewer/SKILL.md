---
name: "cncf-checkov-reviewer"
description: "Review Checkov using its official documentation and repository in the Provisioning / Security & Compliance category. Checkov scans cloud infrastructure configurations to find misconfigurations before they are deployed. Checkov manages and analyzes infrastructure as code (IaC) scan results across platforms such as Terraform, CloudFormation, Kubernetes, Helm, ARM Templates and Serverless framework."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Checkov Reviewer

## Purpose

Review and operate Checkov using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Checkov code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Checkov requires evidence-backed assessment.
...
- First-party source: https://github.com/bridgecrewio/checkov
...
- [ ] Trace Checkov inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Checkov defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Checkov least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Checkov resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Checkov release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Checkov dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Checkov identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Checkov change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Checkov documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Checkov architecture and dependency summary with trust boundaries and persistent state.
- Include Checkov validation commands or tests that are safe for the stated environment.
...
- Checkov identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Checkov failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Checkov is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Checkov as the basis for migration or production guidance.
