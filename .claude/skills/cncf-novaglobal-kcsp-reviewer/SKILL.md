---
name: "cncf-novaglobal-kcsp-reviewer"
description: "Review NovaGlobal (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Our commitment to excellence is reflected in a comprehensive suite of professional services tailored for organizations seeking to adopt Kubernetes seamlessly into their computing systems."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Novaglobal Kcsp Reviewer

## Purpose

Review and operate NovaGlobal (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- NovaGlobal (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving NovaGlobal (KCSP) requires evidence-backed assessment.
...
- First-party source: https://novaglobal.com.sg/index.php/kubernetes/
...
- [ ] Trace NovaGlobal (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check NovaGlobal (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check NovaGlobal (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check NovaGlobal (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check NovaGlobal (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check NovaGlobal (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If NovaGlobal (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a NovaGlobal (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- NovaGlobal (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a NovaGlobal (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include NovaGlobal (KCSP) validation commands or tests that are safe for the stated environment.
...
- NovaGlobal (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- NovaGlobal (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that NovaGlobal (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of NovaGlobal (KCSP) as the basis for migration or production guidance.
