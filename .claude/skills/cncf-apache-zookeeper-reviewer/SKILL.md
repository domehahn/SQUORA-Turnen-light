---
name: "cncf-apache-zookeeper-reviewer"
description: "Review Apache Zookeeper using its official documentation and repository in the Orchestration & Management / Coordination & Service Discovery category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Apache Zookeeper Reviewer

## Purpose

Review and operate Apache Zookeeper using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Apache Zookeeper code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Apache Zookeeper requires evidence-backed assessment.
...
- CNCF classification: Orchestration & Management / Coordination & Service Discovery
- CNCF project maturity field: not specified
- First-party source: https://github.com/apache/zookeeper
...
- [ ] Trace Apache Zookeeper inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Apache Zookeeper defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Apache Zookeeper least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Apache Zookeeper resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Apache Zookeeper release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Apache Zookeeper dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Apache Zookeeper identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Apache Zookeeper change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Apache Zookeeper documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Apache Zookeeper architecture and dependency summary with trust boundaries and persistent state.
- Include Apache Zookeeper validation commands or tests that are safe for the stated environment.
...
- Apache Zookeeper identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Apache Zookeeper failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Apache Zookeeper is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Apache Zookeeper as the basis for migration or production guidance.
