---
name: "cncf-logicmonitor-reviewer"
description: "Review LogicMonitor using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Logicmonitor Reviewer

## Purpose

Review and operate LogicMonitor using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- LogicMonitor code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving LogicMonitor requires evidence-backed assessment.
...
- First-party source: https://www.logicmonitor.com/
...
- [ ] Trace LogicMonitor inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check LogicMonitor defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check LogicMonitor least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check LogicMonitor resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check LogicMonitor release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check LogicMonitor dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If LogicMonitor identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a LogicMonitor change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- LogicMonitor documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a LogicMonitor architecture and dependency summary with trust boundaries and persistent state.
- Include LogicMonitor validation commands or tests that are safe for the stated environment.
...
- LogicMonitor identity, version, configuration, dependencies, and deployment assumptions are explicit.
- LogicMonitor failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that LogicMonitor is secure, supported, compatible, or suitable.
- Using an unpinned latest version of LogicMonitor as the basis for migration or production guidance.
