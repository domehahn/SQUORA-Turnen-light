...
description: "Review code vulnerabilities such as injection, path traversal, SSRF, XSS, deserialization, crypto misuse, and race conditions."
version: "1.0.0"
since: "2026-08-27"
...
1. Identify the relevant secure code review artifacts, owners, systems, environments, and review boundary.
2. Compare the available artifacts against expected signals such as tainted input, authorization check, fuzz case, unit test, static-analysis warning, exploitability note.
...
- [ ] Confirm the review boundary covers the right secure code review systems, teams, and environments.
- [ ] Inventory and inspect the current source diff.
- [ ] Check whether input parser is current, approved, versioned, and owned.
- [ ] Verify that file access path has test, ticket, log, or approval support.
- [ ] Look for injection and record concrete repository or process evidence.
- [ ] Look for path traversal and identify affected assets, services, or stakeholders.
- [ ] Look for SSRF and classify the operational or audit impact.
- [ ] Use tainted input to validate that the control or practice is operating.
- [ ] Use authorization check to confirm ownership, timing, and reproducibility.
- [ ] Check exception, risk-acceptance, and expiry handling for secure code review.
...
- If source diff is missing for a critical service, raise at least a high-severity readiness gap.
...
- Missing or stale secure code review artifact.
...
- Critical: a gap in secure code review creates immediate outage, data-loss, privilege, regulatory-reporting, or irreversible business risk.
- High: source diff is missing, unowned, untested, or unauditable for a critical service or material change.
- Medium: input parser exists but is stale, incomplete, inconsistently enforced, or weakly evidenced.
...
- Findings ordered by severity with affected secure code review artifacts and evidence references.
- Coverage note for reviewed artifacts: source diff, input parser, file access path, HTTP client, template rendering, crypto use.
- Risk note covering relevant themes: injection, path traversal, SSRF, XSS, unsafe deserialization, race condition.
- Evidence request list using expected signals: tainted input, authorization check, fuzz case, unit test, static-analysis warning, exploitability note.
- Deliverables or updates needed: secure code findings, exploitability assessment, fix guidance, negative-test recommendations, residual risk notes.
- Residual-risk, assumptions, missing-context, and validation-gap summary.

## Acceptance Criteria

- Relevant secure code review artifacts are identified, current, owned, and versioned where applicable.
