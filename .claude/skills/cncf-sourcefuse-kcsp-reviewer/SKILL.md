---
name: "cncf-sourcefuse-kcsp-reviewer"
description: "Review SourceFuse (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. SourceFuse delivers end-to-end Kubernetes services - from architecture consulting to production-grade managed operations - for enterprises in healthcare, fintech, and SaaS."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Sourcefuse Kcsp Reviewer

## Purpose

Review and operate SourceFuse (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SourceFuse (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SourceFuse (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.sourcefuse.com/kcsp-partner
...
- [ ] Trace SourceFuse (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SourceFuse (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SourceFuse (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SourceFuse (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SourceFuse (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SourceFuse (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SourceFuse (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SourceFuse (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SourceFuse (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SourceFuse (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include SourceFuse (KCSP) validation commands or tests that are safe for the stated environment.
...
- SourceFuse (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SourceFuse (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SourceFuse (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SourceFuse (KCSP) as the basis for migration or production guidance.
