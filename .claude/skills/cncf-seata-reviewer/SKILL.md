---
name: "cncf-seata-reviewer"
description: "Review Seata using its official documentation and repository in the App Definition and Development / Database category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Seata using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Seata code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Seata requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/incubator-seata
...
- [ ] Trace Seata inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Seata defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Seata least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Seata resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Seata release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Seata dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Seata identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Seata change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Seata documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Seata architecture and dependency summary with trust boundaries and persistent state.
- Include Seata validation commands or tests that are safe for the stated environment.
...
- Seata identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Seata failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Seata is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Seata as the basis for migration or production guidance.
