---
name: "cncf-chronosphere-reviewer"
description: "Review Chronosphere using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Chronosphere Reviewer

## Purpose

Review and operate Chronosphere using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Chronosphere code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Chronosphere requires evidence-backed assessment.
...
- [ ] Trace Chronosphere inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Chronosphere defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Chronosphere least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Chronosphere resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Chronosphere release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Chronosphere dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Chronosphere identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Chronosphere change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Chronosphere documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Chronosphere architecture and dependency summary with trust boundaries and persistent state.
- Include Chronosphere validation commands or tests that are safe for the stated environment.
...
- Chronosphere identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Chronosphere failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Chronosphere is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Chronosphere as the basis for migration or production guidance.
