---
name: "cncf-amazon-elastic-container-service-ecs-reviewer"
...
---
# Cncf Amazon Elastic Container Service Ecs Reviewer

## Purpose

Review and operate Amazon Elastic Container Service (ECS) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Amazon Elastic Container Service (ECS) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Amazon Elastic Container Service (ECS) requires evidence-backed assessment.
...
- First-party source: https://aws.amazon.com/ecs/
...
- [ ] Trace Amazon Elastic Container Service (ECS) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Amazon Elastic Container Service (ECS) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Amazon Elastic Container Service (ECS) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Amazon Elastic Container Service (ECS) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Amazon Elastic Container Service (ECS) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Amazon Elastic Container Service (ECS) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Amazon Elastic Container Service (ECS) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Amazon Elastic Container Service (ECS) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Amazon Elastic Container Service (ECS) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Amazon Elastic Container Service (ECS) architecture and dependency summary with trust boundaries and persistent state.
- Include Amazon Elastic Container Service (ECS) validation commands or tests that are safe for the stated environment.
...
- Amazon Elastic Container Service (ECS) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Amazon Elastic Container Service (ECS) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Amazon Elastic Container Service (ECS) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Amazon Elastic Container Service (ECS) as the basis for migration or production guidance.
