---
name: "cncf-kubefleet-reviewer"
description: "Review KubeFleet using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. A multi-cluster solution that enables users to effectively manage their applications running in multiple Kubernetes clusters."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kubefleet Reviewer

## Purpose

Review and operate KubeFleet using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeFleet code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeFleet requires evidence-backed assessment.
...
- First-party source: https://github.com/kubefleet-dev/kubefleet
...
- [ ] Trace KubeFleet inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeFleet defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeFleet least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeFleet resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeFleet release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeFleet dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeFleet identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeFleet change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeFleet documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeFleet architecture and dependency summary with trust boundaries and persistent state.
- Include KubeFleet validation commands or tests that are safe for the stated environment.
...
- KubeFleet identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeFleet failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeFleet is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeFleet as the basis for migration or production guidance.
