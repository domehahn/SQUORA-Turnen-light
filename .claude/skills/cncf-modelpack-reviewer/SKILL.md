---
name: "cncf-modelpack-reviewer"
description: "Review ModelPack using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category. The project establishes open standards for packaging, distributing and running AI artifacts in the cloud-native environment."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Modelpack Reviewer

## Purpose

Review and operate ModelPack using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ModelPack code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ModelPack requires evidence-backed assessment.
...
- First-party source: https://github.com/modelpack/model-spec
...
- [ ] Trace ModelPack inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ModelPack defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ModelPack least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ModelPack resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ModelPack release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ModelPack dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ModelPack identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ModelPack change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ModelPack documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ModelPack architecture and dependency summary with trust boundaries and persistent state.
- Include ModelPack validation commands or tests that are safe for the stated environment.
...
- ModelPack identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ModelPack failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ModelPack is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ModelPack as the basis for migration or production guidance.
