---
name: "cncf-drivescale-reviewer"
description: "Review DriveScale using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Drivescale Reviewer

## Purpose

Review and operate DriveScale using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DriveScale code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DriveScale requires evidence-backed assessment.
...
- First-party source: https://www.linkedin.com/company/drivescale-inc-/
...
- [ ] Trace DriveScale inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DriveScale defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DriveScale least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DriveScale resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DriveScale release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DriveScale dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DriveScale identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DriveScale change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DriveScale documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DriveScale architecture and dependency summary with trust boundaries and persistent state.
- Include DriveScale validation commands or tests that are safe for the stated environment.
...
- DriveScale identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DriveScale failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DriveScale is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DriveScale as the basis for migration or production guidance.
