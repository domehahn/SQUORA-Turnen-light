---
name: "cncf-mem0-reviewer"
description: "Review Mem0 using its official documentation and repository in the AI Agent / State and Memory category. Universal memory layer for AI Agents"
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Mem0 using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Mem0 code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Mem0 requires evidence-backed assessment.
...
- CNCF classification: AI Agent / State and Memory
- CNCF project maturity field: not specified
- First-party source: https://github.com/mem0ai/mem0
...
- [ ] Trace Mem0 inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Mem0 defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Mem0 least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Mem0 resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Mem0 release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Mem0 dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Mem0 identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Mem0 change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Mem0 documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Mem0 architecture and dependency summary with trust boundaries and persistent state.
- Include Mem0 validation commands or tests that are safe for the stated environment.
...
- Mem0 identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Mem0 failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Mem0 is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Mem0 as the basis for migration or production guidance.
