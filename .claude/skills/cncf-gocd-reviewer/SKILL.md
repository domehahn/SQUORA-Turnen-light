---
name: "cncf-gocd-reviewer"
description: "Review GoCD using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate GoCD using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- GoCD code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving GoCD requires evidence-backed assessment.
...
- First-party source: https://github.com/gocd/gocd
...
- [ ] Trace GoCD inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check GoCD defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check GoCD least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check GoCD resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check GoCD release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check GoCD dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If GoCD identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a GoCD change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- GoCD documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a GoCD architecture and dependency summary with trust boundaries and persistent state.
- Include GoCD validation commands or tests that are safe for the stated environment.
...
- GoCD identity, version, configuration, dependencies, and deployment assumptions are explicit.
- GoCD failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that GoCD is secure, supported, compatible, or suitable.
- Using an unpinned latest version of GoCD as the basis for migration or production guidance.
