---
name: "cncf-twilio-functions-reviewer"
description: "Review Twilio Functions using its official documentation and repository in the Serverless / Hosted Platform category. A serverless environment to build and run your Twilio code so you can get to production faster."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Twilio Functions Reviewer

## Purpose

Review and operate Twilio Functions using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Twilio Functions code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Twilio Functions requires evidence-backed assessment.
...
- First-party source: https://www.twilio.com/docs/serverless/functions-assets/functions
...
- [ ] Trace Twilio Functions inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Twilio Functions defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Twilio Functions least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Twilio Functions resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Twilio Functions release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Twilio Functions dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Twilio Functions identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Twilio Functions change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Twilio Functions documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Twilio Functions architecture and dependency summary with trust boundaries and persistent state.
- Include Twilio Functions validation commands or tests that are safe for the stated environment.
...
- Twilio Functions identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Twilio Functions failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Twilio Functions is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Twilio Functions as the basis for migration or production guidance.
