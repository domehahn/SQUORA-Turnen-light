---
name: "cncf-core-24-7-kcsp-reviewer"
description: "Review Core 24/7 (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. We specialize in Kubernetes-based infrastructure design, deployment, and ongoing management. Our certified engineers deliver DevOps-as-a-Service, ensuring 24×7×365 production reliability and developer assistance."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Core 24 7 Kcsp Reviewer

## Purpose

Review and operate Core 24/7 (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Core 24/7 (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Core 24/7 (KCSP) requires evidence-backed assessment.
...
- First-party source: https://core247.kz/kubernetes
...
- [ ] Trace Core 24/7 (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Core 24/7 (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Core 24/7 (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Core 24/7 (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Core 24/7 (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Core 24/7 (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Core 24/7 (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Core 24/7 (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Core 24/7 (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Core 24/7 (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Core 24/7 (KCSP) validation commands or tests that are safe for the stated environment.
...
- Core 24/7 (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Core 24/7 (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Core 24/7 (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Core 24/7 (KCSP) as the basis for migration or production guidance.
