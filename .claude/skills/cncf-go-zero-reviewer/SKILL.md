...
---
# Cncf Go Zero Reviewer

## Purpose

Review and operate go-zero using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- go-zero code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving go-zero requires evidence-backed assessment.
...
- First-party source: https://github.com/zeromicro/go-zero
...
- [ ] Trace go-zero inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check go-zero defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check go-zero least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check go-zero resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check go-zero release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check go-zero dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If go-zero identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a go-zero change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- go-zero documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a go-zero architecture and dependency summary with trust boundaries and persistent state.
- Include go-zero validation commands or tests that are safe for the stated environment.
...
- go-zero identity, version, configuration, dependencies, and deployment assumptions are explicit.
- go-zero failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that go-zero is secure, supported, compatible, or suitable.
- Using an unpinned latest version of go-zero as the basis for migration or production guidance.
