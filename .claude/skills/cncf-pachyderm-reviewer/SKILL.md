---
name: "cncf-pachyderm-reviewer"
description: "Review Pachyderm using its official documentation and repository in the App Definition and Development / Streaming & Messaging category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Pachyderm Reviewer

## Purpose

Review and operate Pachyderm using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Pachyderm code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Pachyderm requires evidence-backed assessment.
...
- First-party source: https://github.com/pachyderm/pachyderm
...
- [ ] Trace Pachyderm inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Pachyderm defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Pachyderm least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Pachyderm resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Pachyderm release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Pachyderm dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Pachyderm identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Pachyderm change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Pachyderm documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Pachyderm architecture and dependency summary with trust boundaries and persistent state.
- Include Pachyderm validation commands or tests that are safe for the stated environment.
...
- Pachyderm identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Pachyderm failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Pachyderm is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Pachyderm as the basis for migration or production guidance.
