---
name: "cncf-kapitan-reviewer"
description: "Review Kapitan using its official documentation and repository in the Provisioning / Automation & Configuration category. A configuration management system for platform engineering and other things"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kapitan Reviewer

## Purpose

Review and operate Kapitan using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kapitan code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kapitan requires evidence-backed assessment.
...
- First-party source: https://github.com/kapicorp/kapitan
...
- [ ] Trace Kapitan inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kapitan defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kapitan least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kapitan resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kapitan release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kapitan dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kapitan identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kapitan change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kapitan documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kapitan architecture and dependency summary with trust boundaries and persistent state.
- Include Kapitan validation commands or tests that are safe for the stated environment.
...
- Kapitan identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kapitan failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kapitan is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kapitan as the basis for migration or production guidance.
