---
name: "cncf-t-systems-kcsp-reviewer"
description: "Review T-Systems (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Our team of CNCF certified Kubernetes experts have extensive experience working with a wide range of industries and can help you design and implement a solution that meets your specific needs."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf T Systems Kcsp Reviewer

## Purpose

Review and operate T-Systems (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- T-Systems (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving T-Systems (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.t-systems.com/de/en/cloud-services/cloud-application-services/managed-container-stack
...
- [ ] Trace T-Systems (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check T-Systems (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check T-Systems (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check T-Systems (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check T-Systems (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check T-Systems (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If T-Systems (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a T-Systems (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- T-Systems (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a T-Systems (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include T-Systems (KCSP) validation commands or tests that are safe for the stated environment.
...
- T-Systems (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- T-Systems (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that T-Systems (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of T-Systems (KCSP) as the basis for migration or production guidance.
