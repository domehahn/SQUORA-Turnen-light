---
name: "cncf-platform-sh-reviewer"
description: "Review Platform.sh using its official documentation and repository in the Platform / PaaS/Container Service category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Platform Sh Reviewer

## Purpose

Review and operate Platform.sh using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Platform.sh code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Platform.sh requires evidence-backed assessment.
...
- First-party source: https://platform.sh/
...
- [ ] Trace Platform.sh inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Platform.sh defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Platform.sh least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Platform.sh resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Platform.sh release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Platform.sh dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Platform.sh identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Platform.sh change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Platform.sh documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Platform.sh architecture and dependency summary with trust boundaries and persistent state.
- Include Platform.sh validation commands or tests that are safe for the stated environment.
...
- Platform.sh identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Platform.sh failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Platform.sh is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Platform.sh as the basis for migration or production guidance.
