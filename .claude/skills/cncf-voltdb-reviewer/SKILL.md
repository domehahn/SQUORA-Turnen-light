---
name: "cncf-voltdb-reviewer"
description: "Review VoltDB using its official documentation and repository in the App Definition and Development / Database category. VoltDB is a high-velocity decisioning engine, powering real-time applications that must react in milliseconds."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Voltdb Reviewer

## Purpose

Review and operate VoltDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- VoltDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving VoltDB requires evidence-backed assessment.
...
- First-party source: https://www.voltactivedata.com
...
- [ ] Trace VoltDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check VoltDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check VoltDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check VoltDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check VoltDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check VoltDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If VoltDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a VoltDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- VoltDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a VoltDB architecture and dependency summary with trust boundaries and persistent state.
- Include VoltDB validation commands or tests that are safe for the stated environment.
...
- VoltDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- VoltDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that VoltDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of VoltDB as the basis for migration or production guidance.
