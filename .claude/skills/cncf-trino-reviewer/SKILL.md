---
name: "cncf-trino-reviewer"
description: "Review Trino using its official documentation and repository in the Data / Data Architecture category. The distributed SQL query engine for big data, formerly known as PrestoSQL."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Trino using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Trino code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Trino requires evidence-backed assessment.
...
- First-party source: https://github.com/trinodb/trino
...
- [ ] Trace Trino inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Trino defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Trino least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Trino resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Trino release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Trino dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Trino identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Trino change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Trino documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Trino architecture and dependency summary with trust boundaries and persistent state.
- Include Trino validation commands or tests that are safe for the stated environment.
...
- Trino identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Trino failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Trino is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Trino as the basis for migration or production guidance.
