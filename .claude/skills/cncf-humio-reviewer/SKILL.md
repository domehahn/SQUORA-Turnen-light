---
name: "cncf-humio-reviewer"
description: "Review Humio using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Humio using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Humio code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Humio requires evidence-backed assessment.
...
- First-party source: https://www.humio.com/
...
- [ ] Trace Humio inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Humio defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Humio least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Humio resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Humio release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Humio dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Humio identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Humio change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Humio documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Humio architecture and dependency summary with trust boundaries and persistent state.
- Include Humio validation commands or tests that are safe for the stated environment.
...
- Humio identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Humio failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Humio is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Humio as the basis for migration or production guidance.
