---
name: "cncf-bentoml-reviewer"
description: "Review BentoML using its official documentation and repository in the AI Native Infra / Continuous Integration and Delivery category. Build Production-Grade AI Applications"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Bentoml Reviewer

## Purpose

Review and operate BentoML using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- BentoML code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving BentoML requires evidence-backed assessment.
...
- First-party source: https://github.com/bentoml/BentoML
...
- [ ] Trace BentoML inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check BentoML defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check BentoML least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check BentoML resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check BentoML release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check BentoML dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If BentoML identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a BentoML change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- BentoML documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a BentoML architecture and dependency summary with trust boundaries and persistent state.
- Include BentoML validation commands or tests that are safe for the stated environment.
...
- BentoML identity, version, configuration, dependencies, and deployment assumptions are explicit.
- BentoML failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that BentoML is secure, supported, compatible, or suitable.
- Using an unpinned latest version of BentoML as the basis for migration or production guidance.
