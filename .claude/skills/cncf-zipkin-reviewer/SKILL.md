---
name: "cncf-zipkin-reviewer"
description: "Review Zipkin using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Zipkin Reviewer

## Purpose

Review and operate Zipkin using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Zipkin code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Zipkin requires evidence-backed assessment.
...
- First-party source: https://github.com/openzipkin/zipkin
...
- [ ] Trace Zipkin inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Zipkin defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Zipkin least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Zipkin resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Zipkin release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Zipkin dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Zipkin identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Zipkin change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Zipkin documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Zipkin architecture and dependency summary with trust boundaries and persistent state.
- Include Zipkin validation commands or tests that are safe for the stated environment.
...
- Zipkin identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Zipkin failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Zipkin is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Zipkin as the basis for migration or production guidance.
