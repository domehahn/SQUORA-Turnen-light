---
name: "cncf-deployhub-reviewer"
description: "Review DeployHub using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Deployhub Reviewer

## Purpose

Review and operate DeployHub using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DeployHub code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DeployHub requires evidence-backed assessment.
...
- First-party source: https://www.deployhub.com/
...
- [ ] Trace DeployHub inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DeployHub defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DeployHub least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DeployHub resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DeployHub release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DeployHub dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DeployHub identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DeployHub change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DeployHub documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DeployHub architecture and dependency summary with trust boundaries and persistent state.
- Include DeployHub validation commands or tests that are safe for the stated environment.
...
- DeployHub identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DeployHub failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DeployHub is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DeployHub as the basis for migration or production guidance.
