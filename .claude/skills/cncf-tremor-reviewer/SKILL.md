---
name: "cncf-tremor-reviewer"
description: "Review Tremor using its official documentation and repository in the App Definition and Development / Streaming & Messaging category. An early-stage event processing system for unstructured data with rich support for structural pattern-matching, filtering and transformation"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Tremor Reviewer

## Purpose

Review and operate Tremor using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Tremor code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Tremor requires evidence-backed assessment.
...
- First-party source: https://github.com/tremor-rs/tremor-runtime
...
- [ ] Trace Tremor inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Tremor defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Tremor least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Tremor resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Tremor release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Tremor dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Tremor identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Tremor change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Tremor documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Tremor architecture and dependency summary with trust boundaries and persistent state.
- Include Tremor validation commands or tests that are safe for the stated environment.
...
- Tremor identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Tremor failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Tremor is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Tremor as the basis for migration or production guidance.
