...
---
# Cncf Citrix Adc Formerly Netscaler Adc Reviewer

## Purpose

Review and operate Citrix ADC (formerly NetScaler ADC) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Citrix ADC (formerly NetScaler ADC) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Citrix ADC (formerly NetScaler ADC) requires evidence-backed assessment.
...
- CNCF classification: Orchestration & Management / Service Proxy
- CNCF project maturity field: not specified
- First-party source: https://www.citrix.com/products/citrix-adc/
...
- [ ] Trace Citrix ADC (formerly NetScaler ADC) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Citrix ADC (formerly NetScaler ADC) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Citrix ADC (formerly NetScaler ADC) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Citrix ADC (formerly NetScaler ADC) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Citrix ADC (formerly NetScaler ADC) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Citrix ADC (formerly NetScaler ADC) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Citrix ADC (formerly NetScaler ADC) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Citrix ADC (formerly NetScaler ADC) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Citrix ADC (formerly NetScaler ADC) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Citrix ADC (formerly NetScaler ADC) architecture and dependency summary with trust boundaries and persistent state.
- Include Citrix ADC (formerly NetScaler ADC) validation commands or tests that are safe for the stated environment.
...
- Citrix ADC (formerly NetScaler ADC) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Citrix ADC (formerly NetScaler ADC) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Citrix ADC (formerly NetScaler ADC) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Citrix ADC (formerly NetScaler ADC) as the basis for migration or production guidance.
