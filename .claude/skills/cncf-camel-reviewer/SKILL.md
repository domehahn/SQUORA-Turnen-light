---
name: "cncf-camel-reviewer"
description: "Review CAMEL using its official documentation and repository in the AI Agent / Agent Framework category. The first and the best multi-agent framework. Finding the Scaling Law of Agents."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate CAMEL using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CAMEL code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CAMEL requires evidence-backed assessment.
...
- First-party source: https://github.com/camel-ai/camel
...
- [ ] Trace CAMEL inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CAMEL defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CAMEL least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CAMEL resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CAMEL release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CAMEL dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CAMEL identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CAMEL change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CAMEL documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CAMEL architecture and dependency summary with trust boundaries and persistent state.
- Include CAMEL validation commands or tests that are safe for the stated environment.
...
- CAMEL identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CAMEL failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CAMEL is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CAMEL as the basis for migration or production guidance.
