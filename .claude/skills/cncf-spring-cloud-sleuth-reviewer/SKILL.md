---
name: "cncf-spring-cloud-sleuth-reviewer"
...
Review and operate Spring Cloud Sleuth using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Spring Cloud Sleuth code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Spring Cloud Sleuth requires evidence-backed assessment.
...
- First-party source: https://github.com/spring-cloud/spring-cloud-sleuth
...
- [ ] Trace Spring Cloud Sleuth inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Spring Cloud Sleuth defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Spring Cloud Sleuth least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Spring Cloud Sleuth resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Spring Cloud Sleuth release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Spring Cloud Sleuth dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Spring Cloud Sleuth identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Spring Cloud Sleuth change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Spring Cloud Sleuth documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Spring Cloud Sleuth architecture and dependency summary with trust boundaries and persistent state.
- Include Spring Cloud Sleuth validation commands or tests that are safe for the stated environment.
...
- Spring Cloud Sleuth identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Spring Cloud Sleuth failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Spring Cloud Sleuth is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Spring Cloud Sleuth as the basis for migration or production guidance.
