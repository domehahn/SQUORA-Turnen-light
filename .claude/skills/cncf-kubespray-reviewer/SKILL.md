---
name: "cncf-kubespray-reviewer"
description: "Review Kubespray using its official documentation and repository in the Platform / Certified Kubernetes - Installer category. Deploy a Production Ready Kubernetes Cluster"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kubespray Reviewer

## Purpose

Review and operate Kubespray using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kubespray code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kubespray requires evidence-backed assessment.
...
- First-party source: https://github.com/kubernetes-sigs/kubespray
...
- [ ] Trace Kubespray inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kubespray defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kubespray least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kubespray resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kubespray release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kubespray dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kubespray identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kubespray change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kubespray documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kubespray architecture and dependency summary with trust boundaries and persistent state.
- Include Kubespray validation commands or tests that are safe for the stated environment.
...
- Kubespray identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kubespray failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kubespray is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kubespray as the basis for migration or production guidance.
