---
name: "cncf-bocloud-kcntp-reviewer"
description: "Review BoCloud (KCNTP) using its official documentation and repository in the Special / Kubernetes and Cloud Native Training Partner category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Bocloud Kcntp Reviewer

## Purpose

Review and operate BoCloud (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- BoCloud (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving BoCloud (KCNTP) requires evidence-backed assessment.
...
- First-party source: https://www.bocloud.com.cn/train/?id=92
...
- [ ] Trace BoCloud (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check BoCloud (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check BoCloud (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check BoCloud (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check BoCloud (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check BoCloud (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If BoCloud (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a BoCloud (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- BoCloud (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a BoCloud (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include BoCloud (KCNTP) validation commands or tests that are safe for the stated environment.
...
- BoCloud (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- BoCloud (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that BoCloud (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of BoCloud (KCNTP) as the basis for migration or production guidance.
