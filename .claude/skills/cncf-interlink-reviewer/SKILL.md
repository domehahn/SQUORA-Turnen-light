---
name: "cncf-interlink-reviewer"
description: "Review Interlink using its official documentation and repository in the Runtime / Container Runtime category. InterLink aims to provide an abstraction for the execution of a Kubernetes pod on any remote resource capable of managing a Container execution lifecycle thanks to the Virtual Kubelet interface. It allows you to extend your cloud environment anywhere by running Kubernetes workloads on various infrastructures, creating a seamless cloud-native experience across diverse environments."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Interlink Reviewer

## Purpose

Review and operate Interlink using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Interlink code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Interlink requires evidence-backed assessment.
...
- First-party source: https://github.com/interlink-hq/interLink
...
- [ ] Trace Interlink inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Interlink defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Interlink least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Interlink resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Interlink release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Interlink dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Interlink identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Interlink change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Interlink documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Interlink architecture and dependency summary with trust boundaries and persistent state.
- Include Interlink validation commands or tests that are safe for the stated environment.
...
- Interlink identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Interlink failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Interlink is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Interlink as the basis for migration or production guidance.
