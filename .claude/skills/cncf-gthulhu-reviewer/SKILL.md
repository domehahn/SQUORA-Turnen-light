---
name: "cncf-gthulhu-reviewer"
description: "Review Gthulhu using its official documentation and repository in the Provisioning / Automation & Configuration category. Gthulhu optimizes cloud-native workloads using the Linux Scheduler Extension for different application scenarios."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Gthulhu Reviewer

## Purpose

Review and operate Gthulhu using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Gthulhu code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Gthulhu requires evidence-backed assessment.
...
- First-party source: https://github.com/Gthulhu/Gthulhu
...
- [ ] Trace Gthulhu inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Gthulhu defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Gthulhu least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Gthulhu resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Gthulhu release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Gthulhu dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Gthulhu identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Gthulhu change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Gthulhu documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Gthulhu architecture and dependency summary with trust boundaries and persistent state.
- Include Gthulhu validation commands or tests that are safe for the stated environment.
...
- Gthulhu identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Gthulhu failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Gthulhu is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Gthulhu as the basis for migration or production guidance.
