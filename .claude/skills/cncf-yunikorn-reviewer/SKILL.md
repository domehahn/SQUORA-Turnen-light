---
name: "cncf-yunikorn-reviewer"
description: "Review Yunikorn using its official documentation and repository in the AI Native Infra / Orchestration and Scheduling category. Unleash the power of resource scheduling for running Batch, Data & ML on Kubernetes!"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Yunikorn Reviewer

## Purpose

Review and operate Yunikorn using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Yunikorn code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Yunikorn requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/yunikorn-core/
...
- [ ] Trace Yunikorn inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Yunikorn defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Yunikorn least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Yunikorn resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Yunikorn release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Yunikorn dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Yunikorn identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Yunikorn change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Yunikorn documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Yunikorn architecture and dependency summary with trust boundaries and persistent state.
- Include Yunikorn validation commands or tests that are safe for the stated environment.
...
- Yunikorn identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Yunikorn failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Yunikorn is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Yunikorn as the basis for migration or production guidance.
