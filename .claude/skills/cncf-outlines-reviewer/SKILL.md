---
name: "cncf-outlines-reviewer"
description: "Review Outlines using its official documentation and repository in the AI Agent / Structured Output category. Structured Outputs"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Outlines Reviewer

## Purpose

Review and operate Outlines using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Outlines code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Outlines requires evidence-backed assessment.
...
- First-party source: https://github.com/dottxt-ai/outlines
...
- [ ] Trace Outlines inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Outlines defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Outlines least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Outlines resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Outlines release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Outlines dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Outlines identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Outlines change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Outlines documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Outlines architecture and dependency summary with trust boundaries and persistent state.
- Include Outlines validation commands or tests that are safe for the stated environment.
...
- Outlines identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Outlines failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Outlines is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Outlines as the basis for migration or production guidance.
