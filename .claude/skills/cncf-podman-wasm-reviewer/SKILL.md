---
name: "cncf-podman-wasm-reviewer"
description: "Review podman (Wasm) using its official documentation and repository in the Wasm / Orchestration & Management category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Podman Wasm Reviewer

## Purpose

Review and operate podman (Wasm) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- podman (Wasm) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving podman (Wasm) requires evidence-backed assessment.
...
- First-party source: https://github.com/containers/podman
...
- [ ] Trace podman (Wasm) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check podman (Wasm) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check podman (Wasm) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check podman (Wasm) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check podman (Wasm) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check podman (Wasm) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If podman (Wasm) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a podman (Wasm) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- podman (Wasm) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a podman (Wasm) architecture and dependency summary with trust boundaries and persistent state.
- Include podman (Wasm) validation commands or tests that are safe for the stated environment.
...
- podman (Wasm) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- podman (Wasm) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that podman (Wasm) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of podman (Wasm) as the basis for migration or production guidance.
