---
name: "cncf-keylime-reviewer"
description: "Review Keylime using its official documentation and repository in the Provisioning / Security & Compliance category. Bootstrap & Maintain Trust on the Edge / Cloud and IoT."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Keylime Reviewer

## Purpose

Review and operate Keylime using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Keylime code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Keylime requires evidence-backed assessment.
...
- First-party source: https://github.com/keylime/keylime
...
- [ ] Trace Keylime inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Keylime defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Keylime least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Keylime resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Keylime release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Keylime dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Keylime identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Keylime change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Keylime documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Keylime architecture and dependency summary with trust boundaries and persistent state.
- Include Keylime validation commands or tests that are safe for the stated environment.
...
- Keylime identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Keylime failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Keylime is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Keylime as the basis for migration or production guidance.
