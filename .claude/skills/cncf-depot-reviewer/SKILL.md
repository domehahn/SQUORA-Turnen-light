---
name: "cncf-depot-reviewer"
description: "Review Depot using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Depot using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Depot code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Depot requires evidence-backed assessment.
...
- [ ] Trace Depot inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Depot defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Depot least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Depot resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Depot release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Depot dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Depot identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Depot change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Depot documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Depot architecture and dependency summary with trust boundaries and persistent state.
- Include Depot validation commands or tests that are safe for the stated environment.
...
- Depot identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Depot failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Depot is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Depot as the basis for migration or production guidance.
