...
Review and operate itopia using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- itopia code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving itopia requires evidence-backed assessment.
...
- First-party source: https://itopia.com/itopia-spaces/
...
- [ ] Trace itopia inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check itopia defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check itopia least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check itopia resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check itopia release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check itopia dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If itopia identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a itopia change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- itopia documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a itopia architecture and dependency summary with trust boundaries and persistent state.
- Include itopia validation commands or tests that are safe for the stated environment.
...
- itopia identity, version, configuration, dependencies, and deployment assumptions are explicit.
- itopia failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that itopia is secure, supported, compatible, or suitable.
- Using an unpinned latest version of itopia as the basis for migration or production guidance.
