---
name: "cncf-hyperopt-reviewer"
description: "Review Hyperopt using its official documentation and repository in the Training / Post Training category. Distributed Asynchronous Hyper-parameter Optimization"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Hyperopt Reviewer

## Purpose

Review and operate Hyperopt using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Hyperopt code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Hyperopt requires evidence-backed assessment.
...
- First-party source: https://github.com/hyperopt/hyperopt/
...
- [ ] Trace Hyperopt inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Hyperopt defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Hyperopt least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Hyperopt resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Hyperopt release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Hyperopt dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Hyperopt identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Hyperopt change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Hyperopt documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Hyperopt architecture and dependency summary with trust boundaries and persistent state.
- Include Hyperopt validation commands or tests that are safe for the stated environment.
...
- Hyperopt identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Hyperopt failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Hyperopt is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Hyperopt as the basis for migration or production guidance.
