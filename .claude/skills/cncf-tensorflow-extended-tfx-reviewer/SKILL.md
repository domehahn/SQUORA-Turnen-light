---
name: "cncf-tensorflow-extended-tfx-reviewer"
description: "Review TensorFlow Extended (TFX) using its official documentation and repository in the AI Native Infra / Continuous Integration and Delivery category. TFX is an end-to-end platform for deploying production ML pipelines"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Tensorflow Extended Tfx Reviewer

## Purpose

Review and operate TensorFlow Extended (TFX) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- TensorFlow Extended (TFX) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving TensorFlow Extended (TFX) requires evidence-backed assessment.
...
- CNCF classification: AI Native Infra / Continuous Integration and Delivery
- CNCF project maturity field: not specified
- First-party source: https://github.com/tensorflow/tfx
...
- [ ] Trace TensorFlow Extended (TFX) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check TensorFlow Extended (TFX) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check TensorFlow Extended (TFX) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check TensorFlow Extended (TFX) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check TensorFlow Extended (TFX) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check TensorFlow Extended (TFX) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If TensorFlow Extended (TFX) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a TensorFlow Extended (TFX) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- TensorFlow Extended (TFX) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a TensorFlow Extended (TFX) architecture and dependency summary with trust boundaries and persistent state.
- Include TensorFlow Extended (TFX) validation commands or tests that are safe for the stated environment.
...
- TensorFlow Extended (TFX) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- TensorFlow Extended (TFX) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that TensorFlow Extended (TFX) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of TensorFlow Extended (TFX) as the basis for migration or production guidance.
