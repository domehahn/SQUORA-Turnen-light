...
description: "Review Python code for typing, packaging, async behavior, resource safety, tests, security, and maintainability."
version: "1.0.0"
since: "2026-08-27"
...
- Source code, configuration, dependencies, tests, deployment artifacts, or runtime behavior in Python engineering change.
...
- type annotations, packaging, asyncio, context managers, pytest, dependency hygiene
...
- [ ] Trace Python engineering inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Python engineering defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Python engineering least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Python engineering resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Python engineering release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
...
- If Python engineering identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Python engineering change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Python engineering documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Python engineering architecture and dependency summary with trust boundaries and persistent state.
- Include Python engineering validation commands or tests that are safe for the stated environment.
...
- Python engineering identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Python engineering failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Python engineering is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Python engineering as the basis for migration or production guidance.
