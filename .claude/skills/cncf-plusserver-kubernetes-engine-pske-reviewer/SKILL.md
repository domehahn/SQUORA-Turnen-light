...
---
# Cncf Plusserver Kubernetes Engine Pske Reviewer

## Purpose

Review and operate plusserver Kubernetes Engine (PSKE) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- plusserver Kubernetes Engine (PSKE) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving plusserver Kubernetes Engine (PSKE) requires evidence-backed assessment.
...
- First-party source: https://www.plusserver.com/en/product/managed-kubernetes/
...
- [ ] Trace plusserver Kubernetes Engine (PSKE) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check plusserver Kubernetes Engine (PSKE) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check plusserver Kubernetes Engine (PSKE) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check plusserver Kubernetes Engine (PSKE) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check plusserver Kubernetes Engine (PSKE) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check plusserver Kubernetes Engine (PSKE) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If plusserver Kubernetes Engine (PSKE) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a plusserver Kubernetes Engine (PSKE) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- plusserver Kubernetes Engine (PSKE) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a plusserver Kubernetes Engine (PSKE) architecture and dependency summary with trust boundaries and persistent state.
- Include plusserver Kubernetes Engine (PSKE) validation commands or tests that are safe for the stated environment.
...
- plusserver Kubernetes Engine (PSKE) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- plusserver Kubernetes Engine (PSKE) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that plusserver Kubernetes Engine (PSKE) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of plusserver Kubernetes Engine (PSKE) as the basis for migration or production guidance.
