...
---
# Cncf Portworx By Pure Storage Kcsp Reviewer

## Purpose

Review and operate Portworx by Pure Storage (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Portworx by Pure Storage (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Portworx by Pure Storage (KCSP) requires evidence-backed assessment.
...
- First-party source: https://portworx.com/kubernetes-certified-service-provider
...
- [ ] Trace Portworx by Pure Storage (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Portworx by Pure Storage (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Portworx by Pure Storage (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Portworx by Pure Storage (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Portworx by Pure Storage (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Portworx by Pure Storage (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Portworx by Pure Storage (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Portworx by Pure Storage (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Portworx by Pure Storage (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Portworx by Pure Storage (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Portworx by Pure Storage (KCSP) validation commands or tests that are safe for the stated environment.
...
- Portworx by Pure Storage (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Portworx by Pure Storage (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Portworx by Pure Storage (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Portworx by Pure Storage (KCSP) as the basis for migration or production guidance.
