---
name: "cncf-agenda-kcsp-reviewer"
description: "Review Agenda (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. We provide expert consulting, implementation, and support services for Kubernetes to help organizations streamline operations and  accelerate innovation."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Agenda Kcsp Reviewer

## Purpose

Review and operate Agenda (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Agenda (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Agenda (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.agenda.si/it-storitve/vzdrzevanje-in-upravljanje/kubernetes/
...
- [ ] Trace Agenda (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Agenda (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Agenda (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Agenda (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Agenda (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Agenda (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Agenda (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Agenda (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Agenda (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Agenda (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Agenda (KCSP) validation commands or tests that are safe for the stated environment.
...
- Agenda (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Agenda (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Agenda (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Agenda (KCSP) as the basis for migration or production guidance.
