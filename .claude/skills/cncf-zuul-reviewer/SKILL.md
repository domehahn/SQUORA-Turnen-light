---
name: "cncf-zuul-reviewer"
description: "Review Zuul using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category. Zuul is a system that drives continuous integration, delivery, and deployment with a focus on project gating and interrelated projects."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Zuul using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Zuul code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Zuul requires evidence-backed assessment.
...
- First-party source: https://opendev.org/zuul/zuul
...
- [ ] Trace Zuul inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Zuul defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Zuul least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Zuul resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Zuul release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Zuul dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Zuul identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Zuul change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Zuul documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Zuul architecture and dependency summary with trust boundaries and persistent state.
- Include Zuul validation commands or tests that are safe for the stated environment.
...
- Zuul identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Zuul failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Zuul is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Zuul as the basis for migration or production guidance.
