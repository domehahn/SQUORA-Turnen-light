...
---
# Cncf Monokle Reviewer

## Purpose

Review and operate Monokle using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Monokle code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Monokle requires evidence-backed assessment.
...
- First-party source: https://github.com/kubeshop/monokle
...
- [ ] Trace Monokle inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Monokle defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Monokle least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Monokle resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Monokle release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Monokle dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Monokle identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Monokle change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Monokle documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Monokle architecture and dependency summary with trust boundaries and persistent state.
- Include Monokle validation commands or tests that are safe for the stated environment.
...
- Monokle identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Monokle failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Monokle is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Monokle as the basis for migration or production guidance.
