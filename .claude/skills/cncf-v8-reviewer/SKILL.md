---
name: "cncf-v8-reviewer"
description: "Review V8 using its official documentation and repository in the Wasm / Runtimes category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate V8 using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- V8 code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving V8 requires evidence-backed assessment.
...
- First-party source: https://github.com/v8/v8
...
- [ ] Trace V8 inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check V8 defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check V8 least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check V8 resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check V8 release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check V8 dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If V8 identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a V8 change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- V8 documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a V8 architecture and dependency summary with trust boundaries and persistent state.
- Include V8 validation commands or tests that are safe for the stated environment.
...
- V8 identity, version, configuration, dependencies, and deployment assumptions are explicit.
- V8 failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that V8 is secure, supported, compatible, or suitable.
- Using an unpinned latest version of V8 as the basis for migration or production guidance.
