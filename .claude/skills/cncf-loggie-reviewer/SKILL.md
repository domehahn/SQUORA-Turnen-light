---
name: "cncf-loggie-reviewer"
description: "Review Loggie using its official documentation and repository in the Observability and Analysis / Observability category. Loggie is a lightweight, high-performance, cloud-native log collection agent and aggregator based on Golang."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Loggie Reviewer

## Purpose

Review and operate Loggie using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Loggie code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Loggie requires evidence-backed assessment.
...
- First-party source: https://github.com/loggie-io/loggie
...
- [ ] Trace Loggie inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Loggie defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Loggie least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Loggie resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Loggie release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Loggie dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Loggie identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Loggie change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Loggie documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Loggie architecture and dependency summary with trust boundaries and persistent state.
- Include Loggie validation commands or tests that are safe for the stated environment.
...
- Loggie identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Loggie failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Loggie is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Loggie as the basis for migration or production guidance.
