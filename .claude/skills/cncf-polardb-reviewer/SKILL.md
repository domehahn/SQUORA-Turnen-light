---
name: "cncf-polardb-reviewer"
description: "Review PolarDB using its official documentation and repository in the App Definition and Development / Database category. PolarDB is a cloud native SQL Database."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Polardb Reviewer

## Purpose

Review and operate PolarDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- PolarDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving PolarDB requires evidence-backed assessment.
...
- First-party source: https://github.com/polardb/polardbx-sql
...
- [ ] Trace PolarDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check PolarDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check PolarDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check PolarDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check PolarDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check PolarDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If PolarDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a PolarDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- PolarDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a PolarDB architecture and dependency summary with trust boundaries and persistent state.
- Include PolarDB validation commands or tests that are safe for the stated environment.
...
- PolarDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- PolarDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that PolarDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of PolarDB as the basis for migration or production guidance.
