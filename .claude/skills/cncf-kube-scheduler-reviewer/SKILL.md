---
name: "cncf-kube-scheduler-reviewer"
description: "Review Kube-scheduler using its official documentation and repository in the Wasm / Embedded Functions category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kube Scheduler Reviewer

## Purpose

Review and operate Kube-scheduler using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kube-scheduler code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kube-scheduler requires evidence-backed assessment.
...
- First-party source: https://github.com/kubernetes-sigs/scheduler-plugins
...
- [ ] Trace Kube-scheduler inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kube-scheduler defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kube-scheduler least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kube-scheduler resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kube-scheduler release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kube-scheduler dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kube-scheduler identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kube-scheduler change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kube-scheduler documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kube-scheduler architecture and dependency summary with trust boundaries and persistent state.
- Include Kube-scheduler validation commands or tests that are safe for the stated environment.
...
- Kube-scheduler identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kube-scheduler failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kube-scheduler is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kube-scheduler as the basis for migration or production guidance.
