---
name: "cncf-kubermatic-kubeone-reviewer"
description: "Review Kubermatic KubeOne using its official documentation and repository in the Platform / Certified Kubernetes - Installer category. Lifecycle management tool for Highly-Available Kubernetes clusters on any infrastructure"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kubermatic Kubeone Reviewer

## Purpose

Review and operate Kubermatic KubeOne using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kubermatic KubeOne code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kubermatic KubeOne requires evidence-backed assessment.
...
- First-party source: https://github.com/kubermatic/kubeone
...
- [ ] Trace Kubermatic KubeOne inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kubermatic KubeOne defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kubermatic KubeOne least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kubermatic KubeOne resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kubermatic KubeOne release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kubermatic KubeOne dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kubermatic KubeOne identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kubermatic KubeOne change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kubermatic KubeOne documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kubermatic KubeOne architecture and dependency summary with trust boundaries and persistent state.
- Include Kubermatic KubeOne validation commands or tests that are safe for the stated environment.
...
- Kubermatic KubeOne identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kubermatic KubeOne failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kubermatic KubeOne is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kubermatic KubeOne as the basis for migration or production guidance.
