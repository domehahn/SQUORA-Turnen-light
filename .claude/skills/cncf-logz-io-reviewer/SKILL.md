---
name: "cncf-logz-io-reviewer"
description: "Review Logz.io using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Logz Io Reviewer

## Purpose

Review and operate Logz.io using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Logz.io code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Logz.io requires evidence-backed assessment.
...
- First-party source: https://logz.io/
...
- [ ] Trace Logz.io inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Logz.io defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Logz.io least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Logz.io resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Logz.io release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Logz.io dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Logz.io identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Logz.io change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Logz.io documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Logz.io architecture and dependency summary with trust boundaries and persistent state.
- Include Logz.io validation commands or tests that are safe for the stated environment.
...
- Logz.io identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Logz.io failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Logz.io is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Logz.io as the basis for migration or production guidance.
