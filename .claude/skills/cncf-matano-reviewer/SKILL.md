---
name: "cncf-matano-reviewer"
description: "Review Matano using its official documentation and repository in the Provisioning / Security & Compliance category. Open source security lake platform for AWS"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Matano Reviewer

## Purpose

Review and operate Matano using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Matano code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Matano requires evidence-backed assessment.
...
- First-party source: https://github.com/matanolabs/matano
...
- [ ] Trace Matano inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Matano defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Matano least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Matano resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Matano release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Matano dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Matano identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Matano change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Matano documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Matano architecture and dependency summary with trust boundaries and persistent state.
- Include Matano validation commands or tests that are safe for the stated environment.
...
- Matano identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Matano failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Matano is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Matano as the basis for migration or production guidance.
