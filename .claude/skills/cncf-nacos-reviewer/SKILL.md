---
name: "cncf-nacos-reviewer"
description: "Review Nacos using its official documentation and repository in the Orchestration & Management / Coordination & Service Discovery category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Nacos using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Nacos code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Nacos requires evidence-backed assessment.
...
- First-party source: https://github.com/alibaba/nacos
...
- [ ] Trace Nacos inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Nacos defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Nacos least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Nacos resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Nacos release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Nacos dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Nacos identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Nacos change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Nacos documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Nacos architecture and dependency summary with trust boundaries and persistent state.
- Include Nacos validation commands or tests that are safe for the stated environment.
...
- Nacos identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Nacos failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Nacos is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Nacos as the basis for migration or production guidance.
