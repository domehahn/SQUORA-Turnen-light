---
name: "cncf-apisix-reviewer"
description: "Review APISIX using its official documentation and repository in the Orchestration & Management / API Gateway category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Apisix Reviewer

## Purpose

Review and operate APISIX using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- APISIX code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving APISIX requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/apisix
...
- [ ] Trace APISIX inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check APISIX defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check APISIX least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check APISIX resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check APISIX release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check APISIX dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If APISIX identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a APISIX change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- APISIX documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a APISIX architecture and dependency summary with trust boundaries and persistent state.
- Include APISIX validation commands or tests that are safe for the stated environment.
...
- APISIX identity, version, configuration, dependencies, and deployment assumptions are explicit.
- APISIX failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that APISIX is secure, supported, compatible, or suitable.
- Using an unpinned latest version of APISIX as the basis for migration or production guidance.
