---
name: "cncf-de-novo-kcsp-reviewer"
description: "Review De Novo (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. De Novo, Ukraine's leading cloud provider since 2012, offers a revamped service portfolio, including classic IaaS and advanced VMware Tanzu Kubernetes Grid-based private and public Kubernetes orchestration platforms. Our professional services cover consulting, training, PoC, onboarding, migration, and commercial-grade support."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf De Novo Kcsp Reviewer

## Purpose

Review and operate De Novo (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- De Novo (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving De Novo (KCSP) requires evidence-backed assessment.
...
- First-party source: https://denovo.ua/en/cloud-for-developers
...
- [ ] Trace De Novo (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check De Novo (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check De Novo (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check De Novo (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check De Novo (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check De Novo (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If De Novo (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a De Novo (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- De Novo (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a De Novo (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include De Novo (KCSP) validation commands or tests that are safe for the stated environment.
...
- De Novo (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- De Novo (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that De Novo (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of De Novo (KCSP) as the basis for migration or production guidance.
