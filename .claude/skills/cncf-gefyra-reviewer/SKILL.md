...
---
# Cncf Gefyra Reviewer

## Purpose

Review and operate Gefyra using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Gefyra code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Gefyra requires evidence-backed assessment.
...
- First-party source: https://github.com/gefyrahq/gefyra
...
- [ ] Trace Gefyra inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Gefyra defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Gefyra least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Gefyra resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Gefyra release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Gefyra dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Gefyra identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Gefyra change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Gefyra documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Gefyra architecture and dependency summary with trust boundaries and persistent state.
- Include Gefyra validation commands or tests that are safe for the stated environment.
...
- Gefyra identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Gefyra failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Gefyra is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Gefyra as the basis for migration or production guidance.
