---
name: "cncf-bub-reviewer"
description: "Review Bub using its official documentation and repository in the Provisioning / Automation & Configuration category. A common shape for agents that live alongside people."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Bub using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Bub code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Bub requires evidence-backed assessment.
...
- First-party source: https://github.com/bubbuild/bub
...
- [ ] Trace Bub inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Bub defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Bub least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Bub resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Bub release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Bub dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Bub identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Bub change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Bub documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Bub architecture and dependency summary with trust boundaries and persistent state.
- Include Bub validation commands or tests that are safe for the stated environment.
...
- Bub identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Bub failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Bub is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Bub as the basis for migration or production guidance.
