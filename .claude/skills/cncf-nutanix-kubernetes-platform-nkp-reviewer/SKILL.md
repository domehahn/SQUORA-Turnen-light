...
---
# Cncf Nutanix Kubernetes Platform Nkp Reviewer

## Purpose

Review and operate Nutanix Kubernetes Platform (NKP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Nutanix Kubernetes Platform (NKP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Nutanix Kubernetes Platform (NKP) requires evidence-backed assessment.
...
- First-party source: https://www.nutanix.com/products/kubernetes-management-platform
...
- [ ] Trace Nutanix Kubernetes Platform (NKP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Nutanix Kubernetes Platform (NKP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Nutanix Kubernetes Platform (NKP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Nutanix Kubernetes Platform (NKP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Nutanix Kubernetes Platform (NKP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Nutanix Kubernetes Platform (NKP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Nutanix Kubernetes Platform (NKP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Nutanix Kubernetes Platform (NKP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Nutanix Kubernetes Platform (NKP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Nutanix Kubernetes Platform (NKP) architecture and dependency summary with trust boundaries and persistent state.
- Include Nutanix Kubernetes Platform (NKP) validation commands or tests that are safe for the stated environment.
...
- Nutanix Kubernetes Platform (NKP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Nutanix Kubernetes Platform (NKP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Nutanix Kubernetes Platform (NKP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Nutanix Kubernetes Platform (NKP) as the basis for migration or production guidance.
