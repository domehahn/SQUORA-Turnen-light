---
name: "cncf-tigera-reviewer"
description: "Review Tigera using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Tigera Reviewer

## Purpose

Review and operate Tigera using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Tigera code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Tigera requires evidence-backed assessment.
...
- [ ] Trace Tigera inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Tigera defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Tigera least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Tigera resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Tigera release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Tigera dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Tigera identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Tigera change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Tigera documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Tigera architecture and dependency summary with trust boundaries and persistent state.
- Include Tigera validation commands or tests that are safe for the stated environment.
...
- Tigera identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Tigera failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Tigera is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Tigera as the basis for migration or production guidance.
