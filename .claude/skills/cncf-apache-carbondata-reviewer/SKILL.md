---
name: "cncf-apache-carbondata-reviewer"
description: "Review Apache CarbonData using its official documentation and repository in the App Definition and Development / Database category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Apache Carbondata Reviewer

## Purpose

Review and operate Apache CarbonData using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Apache CarbonData code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Apache CarbonData requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/carbondata
...
- [ ] Trace Apache CarbonData inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Apache CarbonData defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Apache CarbonData least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Apache CarbonData resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Apache CarbonData release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Apache CarbonData dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Apache CarbonData identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Apache CarbonData change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Apache CarbonData documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Apache CarbonData architecture and dependency summary with trust boundaries and persistent state.
- Include Apache CarbonData validation commands or tests that are safe for the stated environment.
...
- Apache CarbonData identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Apache CarbonData failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Apache CarbonData is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Apache CarbonData as the basis for migration or production guidance.
