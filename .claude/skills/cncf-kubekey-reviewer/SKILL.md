...
---
# Cncf Kubekey Reviewer

## Purpose

Review and operate Kubekey using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kubekey code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kubekey requires evidence-backed assessment.
...
- First-party source: https://github.com/kubesphere/kubekey
...
- [ ] Trace Kubekey inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kubekey defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kubekey least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kubekey resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kubekey release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kubekey dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kubekey identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kubekey change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kubekey documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kubekey architecture and dependency summary with trust boundaries and persistent state.
- Include Kubekey validation commands or tests that are safe for the stated environment.
...
- Kubekey identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kubekey failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kubekey is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kubekey as the basis for migration or production guidance.
