---
name: "cncf-ferymon-cloud-reviewer"
description: "Review Ferymon Cloud using its official documentation and repository in the Wasm / Hosted Platforms category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Ferymon Cloud Reviewer

## Purpose

Review and operate Ferymon Cloud using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Ferymon Cloud code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Ferymon Cloud requires evidence-backed assessment.
...
- First-party source: https://www.fermyon.com/cloud
...
- [ ] Trace Ferymon Cloud inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Ferymon Cloud defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Ferymon Cloud least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Ferymon Cloud resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Ferymon Cloud release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Ferymon Cloud dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Ferymon Cloud identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Ferymon Cloud change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Ferymon Cloud documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Ferymon Cloud architecture and dependency summary with trust boundaries and persistent state.
- Include Ferymon Cloud validation commands or tests that are safe for the stated environment.
...
- Ferymon Cloud identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Ferymon Cloud failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Ferymon Cloud is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Ferymon Cloud as the basis for migration or production guidance.
