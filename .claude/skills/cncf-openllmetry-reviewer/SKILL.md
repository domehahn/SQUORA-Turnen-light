---
name: "cncf-openllmetry-reviewer"
description: "Review OpenLLMetry using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Openllmetry Reviewer

## Purpose

Review and operate OpenLLMetry using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenLLMetry code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenLLMetry requires evidence-backed assessment.
...
- First-party source: https://github.com/traceloop/openllmetry
...
- [ ] Trace OpenLLMetry inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenLLMetry defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenLLMetry least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenLLMetry resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenLLMetry release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenLLMetry dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenLLMetry identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenLLMetry change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenLLMetry documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenLLMetry architecture and dependency summary with trust boundaries and persistent state.
- Include OpenLLMetry validation commands or tests that are safe for the stated environment.
...
- OpenLLMetry identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenLLMetry failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenLLMetry is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenLLMetry as the basis for migration or production guidance.
