---
name: "cncf-chaterm-reviewer"
description: "Review Chaterm using its official documentation and repository in the Provisioning / Automation & Configuration category. AI terminal and SSH Client for EC2, Database and Kubernetes."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Chaterm Reviewer

## Purpose

Review and operate Chaterm using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Chaterm code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Chaterm requires evidence-backed assessment.
...
- First-party source: https://github.com/chaterm/Chaterm
...
- [ ] Trace Chaterm inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Chaterm defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Chaterm least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Chaterm resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Chaterm release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Chaterm dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Chaterm identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Chaterm change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Chaterm documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Chaterm architecture and dependency summary with trust boundaries and persistent state.
- Include Chaterm validation commands or tests that are safe for the stated environment.
...
- Chaterm identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Chaterm failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Chaterm is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Chaterm as the basis for migration or production guidance.
