---
name: "cncf-aviatrix-reviewer"
description: "Review Aviatrix using its official documentation and repository in the Runtime / Cloud Native Network category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Aviatrix Reviewer

## Purpose

Review and operate Aviatrix using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Aviatrix code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Aviatrix requires evidence-backed assessment.
...
- First-party source: https://www.aviatrix.com/
...
- [ ] Trace Aviatrix inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Aviatrix defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Aviatrix least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Aviatrix resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Aviatrix release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Aviatrix dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Aviatrix identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Aviatrix change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Aviatrix documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Aviatrix architecture and dependency summary with trust boundaries and persistent state.
- Include Aviatrix validation commands or tests that are safe for the stated environment.
...
- Aviatrix identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Aviatrix failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Aviatrix is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Aviatrix as the basis for migration or production guidance.
