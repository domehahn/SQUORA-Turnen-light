---
name: "cncf-jetstack-kcsp-reviewer"
description: "Review Jetstack (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Jetstack is an organisation focused entirely on Kubernetes. They will help you to get the most out of Kubernetes through expert professional services and open source tooling. Get in touch, and accelerate your project."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Jetstack Kcsp Reviewer

## Purpose

Review and operate Jetstack (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Jetstack (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Jetstack (KCSP) requires evidence-backed assessment.
...
- [ ] Trace Jetstack (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Jetstack (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Jetstack (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Jetstack (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Jetstack (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Jetstack (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Jetstack (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Jetstack (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Jetstack (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Jetstack (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Jetstack (KCSP) validation commands or tests that are safe for the stated environment.
...
- Jetstack (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Jetstack (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Jetstack (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Jetstack (KCSP) as the basis for migration or production guidance.
