---
name: "cncf-devfile-reviewer"
description: "Review Devfile using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category. An open standard defining containerized development environments that enables developer tools to simplify and accelerate workflows"
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Devfile using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Devfile code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Devfile requires evidence-backed assessment.
...
- First-party source: https://github.com/devfile/api
...
- [ ] Trace Devfile inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Devfile defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Devfile least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Devfile resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Devfile release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Devfile dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Devfile identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Devfile change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Devfile documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Devfile architecture and dependency summary with trust boundaries and persistent state.
- Include Devfile validation commands or tests that are safe for the stated environment.
...
- Devfile identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Devfile failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Devfile is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Devfile as the basis for migration or production guidance.
