---
name: "cncf-mariadb-reviewer"
description: "Review MariaDB using its official documentation and repository in the App Definition and Development / Database category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Mariadb Reviewer

## Purpose

Review and operate MariaDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- MariaDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving MariaDB requires evidence-backed assessment.
...
- First-party source: https://github.com/MariaDB/server
...
- [ ] Trace MariaDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check MariaDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check MariaDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check MariaDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check MariaDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check MariaDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If MariaDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a MariaDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- MariaDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a MariaDB architecture and dependency summary with trust boundaries and persistent state.
- Include MariaDB validation commands or tests that are safe for the stated environment.
...
- MariaDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- MariaDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that MariaDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of MariaDB as the basis for migration or production guidance.
