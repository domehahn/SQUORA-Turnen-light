---
name: "cncf-telepresence-reviewer"
description: "Review Telepresence using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category. Local development against a remote Kubernetes or OpenShift cluster"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Telepresence Reviewer

## Purpose

Review and operate Telepresence using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Telepresence code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Telepresence requires evidence-backed assessment.
...
- First-party source: https://github.com/telepresenceio/telepresence
...
- [ ] Trace Telepresence inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Telepresence defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Telepresence least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Telepresence resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Telepresence release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Telepresence dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Telepresence identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Telepresence change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Telepresence documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Telepresence architecture and dependency summary with trust boundaries and persistent state.
- Include Telepresence validation commands or tests that are safe for the stated environment.
...
- Telepresence identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Telepresence failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Telepresence is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Telepresence as the basis for migration or production guidance.
