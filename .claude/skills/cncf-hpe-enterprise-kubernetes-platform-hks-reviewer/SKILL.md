---
name: "cncf-hpe-enterprise-kubernetes-platform-hks-reviewer"
description: "Review HPE Enterprise Kubernetes Platform (HKS) using its official documentation and repository in the Platform / Certified Kubernetes - Distribution category. HKS delivers an enterprise-grade Kubernetes platform to provision and manage container workload across your hybrid cloud environments."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate HPE Enterprise Kubernetes Platform (HKS) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- HPE Enterprise Kubernetes Platform (HKS) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving HPE Enterprise Kubernetes Platform (HKS) requires evidence-backed assessment.
...
- First-party source: https://www.hpe.com/us/en/morpheus-enterprise-software.html
...
- [ ] Trace HPE Enterprise Kubernetes Platform (HKS) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check HPE Enterprise Kubernetes Platform (HKS) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check HPE Enterprise Kubernetes Platform (HKS) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check HPE Enterprise Kubernetes Platform (HKS) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check HPE Enterprise Kubernetes Platform (HKS) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check HPE Enterprise Kubernetes Platform (HKS) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If HPE Enterprise Kubernetes Platform (HKS) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a HPE Enterprise Kubernetes Platform (HKS) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- HPE Enterprise Kubernetes Platform (HKS) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a HPE Enterprise Kubernetes Platform (HKS) architecture and dependency summary with trust boundaries and persistent state.
- Include HPE Enterprise Kubernetes Platform (HKS) validation commands or tests that are safe for the stated environment.
...
- HPE Enterprise Kubernetes Platform (HKS) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- HPE Enterprise Kubernetes Platform (HKS) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that HPE Enterprise Kubernetes Platform (HKS) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of HPE Enterprise Kubernetes Platform (HKS) as the basis for migration or production guidance.
