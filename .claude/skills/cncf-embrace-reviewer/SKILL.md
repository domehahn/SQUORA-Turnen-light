---
name: "cncf-embrace-reviewer"
description: "Review Embrace using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Embrace Reviewer

## Purpose

Review and operate Embrace using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Embrace code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Embrace requires evidence-backed assessment.
...
- First-party source: https://github.com/embrace-io/embrace-apple-sdk
...
- [ ] Trace Embrace inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Embrace defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Embrace least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Embrace resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Embrace release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Embrace dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Embrace identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Embrace change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Embrace documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Embrace architecture and dependency summary with trust boundaries and persistent state.
- Include Embrace validation commands or tests that are safe for the stated environment.
...
- Embrace identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Embrace failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Embrace is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Embrace as the basis for migration or production guidance.
