---
name: "cncf-pulsar-reviewer"
description: "Review Pulsar using its official documentation and repository in the App Definition and Development / Streaming & Messaging category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Pulsar Reviewer

## Purpose

Review and operate Pulsar using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Pulsar code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Pulsar requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/pulsar
...
- [ ] Trace Pulsar inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Pulsar defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Pulsar least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Pulsar resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Pulsar release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Pulsar dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Pulsar identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Pulsar change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Pulsar documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Pulsar architecture and dependency summary with trust boundaries and persistent state.
- Include Pulsar validation commands or tests that are safe for the stated environment.
...
- Pulsar identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Pulsar failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Pulsar is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Pulsar as the basis for migration or production guidance.
