---
name: "cncf-score-reviewer"
description: "Review Score using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category. Score is an open-source workload specification designed to simplify development for cloud-native developers."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Score using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Score code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Score requires evidence-backed assessment.
...
- First-party source: https://github.com/score-spec/spec
...
- [ ] Trace Score inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Score defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Score least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Score resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Score release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Score dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Score identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Score change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Score documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Score architecture and dependency summary with trust boundaries and persistent state.
- Include Score validation commands or tests that are safe for the stated environment.
...
- Score identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Score failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Score is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Score as the basis for migration or production guidance.
