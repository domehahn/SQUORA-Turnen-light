---
name: "cncf-wasi-logging-reviewer"
description: "Review wasi-logging using its official documentation and repository in the Wasm / Debugging & Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Wasi Logging Reviewer

## Purpose

Review and operate wasi-logging using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- wasi-logging code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving wasi-logging requires evidence-backed assessment.
...
- CNCF classification: Wasm / Debugging & Observability
- CNCF project maturity field: not specified
- First-party source: https://github.com/WebAssembly/wasi-logging
...
- [ ] Trace wasi-logging inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check wasi-logging defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check wasi-logging least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check wasi-logging resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check wasi-logging release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check wasi-logging dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If wasi-logging identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a wasi-logging change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- wasi-logging documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a wasi-logging architecture and dependency summary with trust boundaries and persistent state.
- Include wasi-logging validation commands or tests that are safe for the stated environment.
...
- wasi-logging identity, version, configuration, dependencies, and deployment assumptions are explicit.
- wasi-logging failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that wasi-logging is secure, supported, compatible, or suitable.
- Using an unpinned latest version of wasi-logging as the basis for migration or production guidance.
