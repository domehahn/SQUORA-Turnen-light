---
name: "cncf-kubeorbit-reviewer"
description: "Review KubeOrbit using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kubeorbit Reviewer

## Purpose

Review and operate KubeOrbit using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeOrbit code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeOrbit requires evidence-backed assessment.
...
- First-party source: https://github.com/teamcode-inc/kubeorbit
...
- [ ] Trace KubeOrbit inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeOrbit defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeOrbit least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeOrbit resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeOrbit release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeOrbit dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeOrbit identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeOrbit change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeOrbit documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeOrbit architecture and dependency summary with trust boundaries and persistent state.
- Include KubeOrbit validation commands or tests that are safe for the stated environment.
...
- KubeOrbit identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeOrbit failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeOrbit is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeOrbit as the basis for migration or production guidance.
