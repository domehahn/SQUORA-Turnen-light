...
---
# Cncf Cocktail Io Kcsp Reviewer

## Purpose

Review and operate Cocktail.io (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Cocktail.io (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Cocktail.io (KCSP) requires evidence-backed assessment.
...
- [ ] Trace Cocktail.io (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Cocktail.io (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Cocktail.io (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Cocktail.io (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Cocktail.io (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Cocktail.io (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Cocktail.io (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Cocktail.io (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Cocktail.io (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Cocktail.io (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Cocktail.io (KCSP) validation commands or tests that are safe for the stated environment.
...
- Cocktail.io (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Cocktail.io (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Cocktail.io (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Cocktail.io (KCSP) as the basis for migration or production guidance.
