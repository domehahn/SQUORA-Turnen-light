---
name: "cncf-sparta-reviewer"
description: "Review Sparta using its official documentation and repository in the Serverless / Framework category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Sparta Reviewer

## Purpose

Review and operate Sparta using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Sparta code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Sparta requires evidence-backed assessment.
...
- First-party source: https://github.com/mweagle/Sparta
...
- [ ] Trace Sparta inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Sparta defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Sparta least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Sparta resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Sparta release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Sparta dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Sparta identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Sparta change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Sparta documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Sparta architecture and dependency summary with trust boundaries and persistent state.
- Include Sparta validation commands or tests that are safe for the stated environment.
...
- Sparta identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Sparta failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Sparta is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Sparta as the basis for migration or production guidance.
