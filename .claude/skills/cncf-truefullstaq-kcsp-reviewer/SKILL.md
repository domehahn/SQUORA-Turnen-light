...
---
# Cncf Truefullstaq Kcsp Reviewer

## Purpose

Review and operate TrueFullstaq (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- TrueFullstaq (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving TrueFullstaq (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.truefullstaq.com/en/services/cloud-solutions/managed-kubernetes
...
- [ ] Trace TrueFullstaq (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check TrueFullstaq (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check TrueFullstaq (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check TrueFullstaq (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check TrueFullstaq (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check TrueFullstaq (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If TrueFullstaq (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a TrueFullstaq (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- TrueFullstaq (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a TrueFullstaq (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include TrueFullstaq (KCSP) validation commands or tests that are safe for the stated environment.
...
- TrueFullstaq (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- TrueFullstaq (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that TrueFullstaq (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of TrueFullstaq (KCSP) as the basis for migration or production guidance.
