---
name: "cncf-strimzi-reviewer"
description: "Review Strimzi using its official documentation and repository in the App Definition and Development / Streaming & Messaging category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Strimzi Reviewer

## Purpose

Review and operate Strimzi using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Strimzi code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Strimzi requires evidence-backed assessment.
...
- First-party source: https://github.com/strimzi/strimzi-kafka-operator
...
- [ ] Trace Strimzi inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Strimzi defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Strimzi least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Strimzi resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Strimzi release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Strimzi dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Strimzi identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Strimzi change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Strimzi documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Strimzi architecture and dependency summary with trust boundaries and persistent state.
- Include Strimzi validation commands or tests that are safe for the stated environment.
...
- Strimzi identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Strimzi failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Strimzi is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Strimzi as the basis for migration or production guidance.
