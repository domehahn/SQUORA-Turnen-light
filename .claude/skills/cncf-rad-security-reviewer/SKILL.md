---
name: "cncf-rad-security-reviewer"
description: "Review RAD Security using its official documentation and repository in the Provisioning / Security & Compliance category. RAD Security maps a broad set of cluster components across the Kubernetes lifecycle  using a real-time graph, cutting noise in half through contextualized risks, highest impact remediations and Kubernetes-first  incident response."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Rad Security Reviewer

## Purpose

Review and operate RAD Security using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- RAD Security code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving RAD Security requires evidence-backed assessment.
...
- [ ] Trace RAD Security inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check RAD Security defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check RAD Security least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check RAD Security resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check RAD Security release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check RAD Security dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If RAD Security identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a RAD Security change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- RAD Security documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a RAD Security architecture and dependency summary with trust boundaries and persistent state.
- Include RAD Security validation commands or tests that are safe for the stated environment.
...
- RAD Security identity, version, configuration, dependencies, and deployment assumptions are explicit.
- RAD Security failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that RAD Security is secure, supported, compatible, or suitable.
- Using an unpinned latest version of RAD Security as the basis for migration or production guidance.
