...
---
# Cncf Google Stackdriver Reviewer

## Purpose

Review and operate Google Stackdriver using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Google Stackdriver code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Google Stackdriver requires evidence-backed assessment.
...
- First-party source: https://cloud.google.com/products/operations
...
- [ ] Trace Google Stackdriver inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Google Stackdriver defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Google Stackdriver least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Google Stackdriver resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Google Stackdriver release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Google Stackdriver dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Google Stackdriver identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Google Stackdriver change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Google Stackdriver documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Google Stackdriver architecture and dependency summary with trust boundaries and persistent state.
- Include Google Stackdriver validation commands or tests that are safe for the stated environment.
...
- Google Stackdriver identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Google Stackdriver failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Google Stackdriver is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Google Stackdriver as the basis for migration or production guidance.
