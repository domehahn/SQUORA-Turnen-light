---
name: "cncf-antrea-reviewer"
description: "Review Antrea using its official documentation and repository in the Runtime / Cloud Native Network category. Kubernetes networking based on Open vSwitch"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Antrea Reviewer

## Purpose

Review and operate Antrea using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Antrea code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Antrea requires evidence-backed assessment.
...
- First-party source: https://github.com/antrea-io/antrea
...
- [ ] Trace Antrea inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Antrea defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Antrea least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Antrea resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Antrea release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Antrea dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Antrea identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Antrea change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Antrea documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Antrea architecture and dependency summary with trust boundaries and persistent state.
- Include Antrea validation commands or tests that are safe for the stated environment.
...
- Antrea identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Antrea failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Antrea is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Antrea as the basis for migration or production guidance.
