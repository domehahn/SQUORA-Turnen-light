---
name: "cncf-b1-systems-kcsp-reviewer"
description: "Review B1 Systems (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. B1 Systems GmbH is a worldwide operating provider of Kubernetes consulting, training, managed service & support located in Germany."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf B1 Systems Kcsp Reviewer

## Purpose

Review and operate B1 Systems (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- B1 Systems (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving B1 Systems (KCSP) requires evidence-backed assessment.
...
- First-party source: https://b1-systems.de/loesungen/kubernetes/
...
- [ ] Trace B1 Systems (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check B1 Systems (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check B1 Systems (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check B1 Systems (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check B1 Systems (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check B1 Systems (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If B1 Systems (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a B1 Systems (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- B1 Systems (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a B1 Systems (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include B1 Systems (KCSP) validation commands or tests that are safe for the stated environment.
...
- B1 Systems (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- B1 Systems (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that B1 Systems (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of B1 Systems (KCSP) as the basis for migration or production guidance.
