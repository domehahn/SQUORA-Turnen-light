---
name: "cncf-keda-reviewer"
description: "Review KEDA using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate KEDA using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KEDA code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KEDA requires evidence-backed assessment.
...
- First-party source: https://github.com/kedacore/keda
...
- [ ] Trace KEDA inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KEDA defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KEDA least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KEDA resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KEDA release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KEDA dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KEDA identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KEDA change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KEDA documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KEDA architecture and dependency summary with trust boundaries and persistent state.
- Include KEDA validation commands or tests that are safe for the stated environment.
...
- KEDA identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KEDA failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KEDA is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KEDA as the basis for migration or production guidance.
