---
name: "cncf-stackgenie-kcsp-reviewer"
description: "Review stackgenie (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. As a Kubernetes Certified Service Provider, StackGenie offers Kubernetes support, consulting and professional services for businesses and organisations wanting to migrate to cloud technologies, or improve their existing cloud infrastructure."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Stackgenie Kcsp Reviewer

## Purpose

Review and operate stackgenie (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- stackgenie (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving stackgenie (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.stackgenie.io/kubernetes/
...
- [ ] Trace stackgenie (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check stackgenie (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check stackgenie (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check stackgenie (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check stackgenie (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check stackgenie (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If stackgenie (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a stackgenie (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- stackgenie (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a stackgenie (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include stackgenie (KCSP) validation commands or tests that are safe for the stated environment.
...
- stackgenie (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- stackgenie (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that stackgenie (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of stackgenie (KCSP) as the basis for migration or production guidance.
