---
name: "cncf-jaeger-reviewer"
description: "Review Jaeger using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Jaeger Reviewer

## Purpose

Review and operate Jaeger using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Jaeger code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Jaeger requires evidence-backed assessment.
...
- First-party source: https://github.com/jaegertracing/jaeger
...
- [ ] Trace Jaeger inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Jaeger defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Jaeger least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Jaeger resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Jaeger release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Jaeger dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Jaeger identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Jaeger change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Jaeger documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Jaeger architecture and dependency summary with trust boundaries and persistent state.
- Include Jaeger validation commands or tests that are safe for the stated environment.
...
- Jaeger identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Jaeger failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Jaeger is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Jaeger as the basis for migration or production guidance.
