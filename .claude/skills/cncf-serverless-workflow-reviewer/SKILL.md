---
name: "cncf-serverless-workflow-reviewer"
description: "Review Serverless Workflow using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category. Standards-based DSL and open-source dev tools and runtimes are at the heart of the Serverless Workflow project"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Serverless Workflow Reviewer

## Purpose

Review and operate Serverless Workflow using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Serverless Workflow code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Serverless Workflow requires evidence-backed assessment.
...
- First-party source: https://github.com/serverlessworkflow/specification
...
- [ ] Trace Serverless Workflow inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Serverless Workflow defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Serverless Workflow least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Serverless Workflow resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Serverless Workflow release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Serverless Workflow dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Serverless Workflow identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Serverless Workflow change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Serverless Workflow documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Serverless Workflow architecture and dependency summary with trust boundaries and persistent state.
- Include Serverless Workflow validation commands or tests that are safe for the stated environment.
...
- Serverless Workflow identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Serverless Workflow failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Serverless Workflow is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Serverless Workflow as the basis for migration or production guidance.
