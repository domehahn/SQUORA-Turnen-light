---
name: "cncf-easystack-kcsp-reviewer"
description: "Review EasyStack (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Easystack Kcsp Reviewer

## Purpose

Review and operate EasyStack (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- EasyStack (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving EasyStack (KCSP) requires evidence-backed assessment.
...
- [ ] Trace EasyStack (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check EasyStack (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check EasyStack (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check EasyStack (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check EasyStack (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check EasyStack (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If EasyStack (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a EasyStack (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- EasyStack (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a EasyStack (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include EasyStack (KCSP) validation commands or tests that are safe for the stated environment.
...
- EasyStack (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- EasyStack (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that EasyStack (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of EasyStack (KCSP) as the basis for migration or production guidance.
