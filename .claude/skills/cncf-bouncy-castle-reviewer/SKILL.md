---
name: "cncf-bouncy-castle-reviewer"
description: "Review Bouncy Castle using its official documentation and repository in the Provisioning / Security & Compliance category. Bouncy Castle is one of the most widely used FIPS-certified open-source cryptographic APIs for Java and C#, allowing developers to easily integrate  PKI security into their applications."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Bouncy Castle Reviewer

## Purpose

Review and operate Bouncy Castle using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Bouncy Castle code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Bouncy Castle requires evidence-backed assessment.
...
- First-party source: https://github.com/bcgit/bc-java
...
- [ ] Trace Bouncy Castle inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Bouncy Castle defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Bouncy Castle least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Bouncy Castle resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Bouncy Castle release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Bouncy Castle dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Bouncy Castle identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Bouncy Castle change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Bouncy Castle documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Bouncy Castle architecture and dependency summary with trust boundaries and persistent state.
- Include Bouncy Castle validation commands or tests that are safe for the stated environment.
...
- Bouncy Castle identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Bouncy Castle failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Bouncy Castle is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Bouncy Castle as the basis for migration or production guidance.
