---
name: "cncf-stratovirt-reviewer"
description: "Review StratoVirt using its official documentation and repository in the Runtime / Container Runtime category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Stratovirt Reviewer

## Purpose

Review and operate StratoVirt using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- StratoVirt code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving StratoVirt requires evidence-backed assessment.
...
- First-party source: https://github.com/openeuler-mirror/stratovirt
...
- [ ] Trace StratoVirt inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check StratoVirt defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check StratoVirt least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check StratoVirt resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check StratoVirt release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check StratoVirt dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If StratoVirt identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a StratoVirt change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- StratoVirt documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a StratoVirt architecture and dependency summary with trust boundaries and persistent state.
- Include StratoVirt validation commands or tests that are safe for the stated environment.
...
- StratoVirt identity, version, configuration, dependencies, and deployment assumptions are explicit.
- StratoVirt failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that StratoVirt is secure, supported, compatible, or suitable.
- Using an unpinned latest version of StratoVirt as the basis for migration or production guidance.
