---
name: "cncf-google-persistent-disk-reviewer"
description: "Review Google Persistent Disk using its official documentation and repository in the Runtime / Cloud Native Storage category. Google Persistent Disk is durable and high performance block storage for the Google Cloud Platform. Persistent Disk provides SSD and HDD storage which can be attached to instances running in either Google Compute Engine or Google Kubernetes Engine. Storage volumes can be transparently resized, quickly backed up, and offer the ability to support simultaneous readers."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Google Persistent Disk Reviewer

## Purpose

Review and operate Google Persistent Disk using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Google Persistent Disk code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Google Persistent Disk requires evidence-backed assessment.
...
- First-party source: https://cloud.google.com/persistent-disk
...
- [ ] Trace Google Persistent Disk inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Google Persistent Disk defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Google Persistent Disk least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Google Persistent Disk resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Google Persistent Disk release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Google Persistent Disk dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Google Persistent Disk identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Google Persistent Disk change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Google Persistent Disk documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Google Persistent Disk architecture and dependency summary with trust boundaries and persistent state.
- Include Google Persistent Disk validation commands or tests that are safe for the stated environment.
...
- Google Persistent Disk identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Google Persistent Disk failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Google Persistent Disk is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Google Persistent Disk as the basis for migration or production guidance.
