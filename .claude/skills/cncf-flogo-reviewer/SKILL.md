---
name: "cncf-flogo-reviewer"
description: "Review Flogo using its official documentation and repository in the Serverless / Framework category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Flogo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Flogo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Flogo requires evidence-backed assessment.
...
- First-party source: https://github.com/TIBCOSoftware/flogo
...
- [ ] Trace Flogo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Flogo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Flogo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Flogo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Flogo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Flogo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Flogo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Flogo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Flogo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Flogo architecture and dependency summary with trust boundaries and persistent state.
- Include Flogo validation commands or tests that are safe for the stated environment.
...
- Flogo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Flogo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Flogo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Flogo as the basis for migration or production guidance.
