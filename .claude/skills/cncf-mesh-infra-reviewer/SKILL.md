...
---
# Cncf Mesh Infra Reviewer

## Purpose

Review and operate Mesh Infra using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Mesh Infra code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Mesh Infra requires evidence-backed assessment.
...
- First-party source: https://meshinfra.net
...
- [ ] Trace Mesh Infra inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Mesh Infra defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Mesh Infra least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Mesh Infra resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Mesh Infra release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Mesh Infra dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Mesh Infra identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Mesh Infra change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Mesh Infra documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Mesh Infra architecture and dependency summary with trust boundaries and persistent state.
- Include Mesh Infra validation commands or tests that are safe for the stated environment.
...
- Mesh Infra identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Mesh Infra failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Mesh Infra is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Mesh Infra as the basis for migration or production guidance.
