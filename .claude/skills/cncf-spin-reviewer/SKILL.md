---
name: "cncf-spin-reviewer"
description: "Review Spin using its official documentation and repository in the Wasm / Application Frameworks category. Spin is a framework for building and deploying serverless applications in WebAssembly."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Spin using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Spin code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Spin requires evidence-backed assessment.
...
- First-party source: https://github.com/spinframework/spin
...
- [ ] Trace Spin inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Spin defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Spin least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Spin resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Spin release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Spin dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Spin identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Spin change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Spin documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Spin architecture and dependency summary with trust boundaries and persistent state.
- Include Spin validation commands or tests that are safe for the stated environment.
...
- Spin identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Spin failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Spin is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Spin as the basis for migration or production guidance.
