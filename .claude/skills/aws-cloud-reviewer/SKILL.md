---
name: "aws-cloud-reviewer"
description: "Review AWS accounts, IAM, networking, compute, storage, databases, observability, security, cost, and resilience."
version: "1.0.0"
since: "2026-08-27"
...
- Source code, configuration, dependencies, tests, deployment artifacts, or runtime behavior in AWS cloud architecture change.
...
- Organizations and accounts, IAM, VPC, compute, data services, CloudTrail and CloudWatch
...
- [ ] Trace AWS cloud architecture inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check AWS cloud architecture defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check AWS cloud architecture least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check AWS cloud architecture resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check AWS cloud architecture release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
...
- If AWS cloud architecture identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a AWS cloud architecture change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- AWS cloud architecture documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a AWS cloud architecture architecture and dependency summary with trust boundaries and persistent state.
- Include AWS cloud architecture validation commands or tests that are safe for the stated environment.
...
- AWS cloud architecture identity, version, configuration, dependencies, and deployment assumptions are explicit.
- AWS cloud architecture failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that AWS cloud architecture is secure, supported, compatible, or suitable.
- Using an unpinned latest version of AWS cloud architecture as the basis for migration or production guidance.
