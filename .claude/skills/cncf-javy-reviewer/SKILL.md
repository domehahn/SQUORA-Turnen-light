---
name: "cncf-javy-reviewer"
description: "Review Javy using its official documentation and repository in the Wasm / Languages category. Scripting languages that support Wasm"
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Javy using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Javy code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Javy requires evidence-backed assessment.
...
- First-party source: https://github.com/bytecodealliance/javy
...
- [ ] Trace Javy inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Javy defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Javy least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Javy resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Javy release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Javy dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Javy identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Javy change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Javy documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Javy architecture and dependency summary with trust boundaries and persistent state.
- Include Javy validation commands or tests that are safe for the stated environment.
...
- Javy identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Javy failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Javy is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Javy as the basis for migration or production guidance.
