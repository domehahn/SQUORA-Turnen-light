---
name: "cncf-chalice-reviewer"
description: "Review Chalice using its official documentation and repository in the Serverless / Framework category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Chalice Reviewer

## Purpose

Review and operate Chalice using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Chalice code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Chalice requires evidence-backed assessment.
...
- First-party source: https://github.com/aws/chalice
...
- [ ] Trace Chalice inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Chalice defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Chalice least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Chalice resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Chalice release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Chalice dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Chalice identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Chalice change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Chalice documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Chalice architecture and dependency summary with trust boundaries and persistent state.
- Include Chalice validation commands or tests that are safe for the stated environment.
...
- Chalice identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Chalice failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Chalice is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Chalice as the basis for migration or production guidance.
