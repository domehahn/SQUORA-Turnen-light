---
name: "cncf-redeploy-kcsp-reviewer"
description: "Review Redeploy (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. We know Azure AKS."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Redeploy Kcsp Reviewer

## Purpose

Review and operate Redeploy (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Redeploy (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Redeploy (KCSP) requires evidence-backed assessment.
...
- First-party source: https://redeploy.com/services/
...
- [ ] Trace Redeploy (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Redeploy (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Redeploy (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Redeploy (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Redeploy (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Redeploy (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Redeploy (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Redeploy (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Redeploy (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Redeploy (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Redeploy (KCSP) validation commands or tests that are safe for the stated environment.
...
- Redeploy (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Redeploy (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Redeploy (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Redeploy (KCSP) as the basis for migration or production guidance.
