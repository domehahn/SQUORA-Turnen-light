---
name: "cncf-ory-hydra-reviewer"
description: "Review ORY Hydra using its official documentation and repository in the Provisioning / Key Management category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Ory Hydra Reviewer

## Purpose

Review and operate ORY Hydra using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ORY Hydra code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ORY Hydra requires evidence-backed assessment.
...
- First-party source: https://github.com/ory/hydra
...
- [ ] Trace ORY Hydra inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ORY Hydra defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ORY Hydra least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ORY Hydra resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ORY Hydra release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ORY Hydra dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ORY Hydra identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ORY Hydra change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ORY Hydra documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ORY Hydra architecture and dependency summary with trust boundaries and persistent state.
- Include ORY Hydra validation commands or tests that are safe for the stated environment.
...
- ORY Hydra identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ORY Hydra failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ORY Hydra is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ORY Hydra as the basis for migration or production guidance.
