---
name: "cncf-gocrane-reviewer"
description: "Review gocrane using its official documentation and repository in the Observability and Analysis / Continuous Optimization category. Crane (FinOps Crane) is an opensource project which manages cloud resource on Kubernetes stack, it is inspired by FinOps concepts."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Gocrane Reviewer

## Purpose

Review and operate gocrane using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- gocrane code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving gocrane requires evidence-backed assessment.
...
- First-party source: https://github.com/gocrane/crane
...
- [ ] Trace gocrane inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check gocrane defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check gocrane least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check gocrane resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check gocrane release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check gocrane dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If gocrane identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a gocrane change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- gocrane documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a gocrane architecture and dependency summary with trust boundaries and persistent state.
- Include gocrane validation commands or tests that are safe for the stated environment.
...
- gocrane identity, version, configuration, dependencies, and deployment assumptions are explicit.
- gocrane failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that gocrane is secure, supported, compatible, or suitable.
- Using an unpinned latest version of gocrane as the basis for migration or production guidance.
