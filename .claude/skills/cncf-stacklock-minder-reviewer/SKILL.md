---
name: "cncf-stacklock-minder-reviewer"
description: "Review Stacklock Minder using its official documentation and repository in the AI Native Infra / Governance, Policy and Security category. Software Supply Chain Security Platform"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Stacklock Minder Reviewer

## Purpose

Review and operate Stacklock Minder using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Stacklock Minder code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Stacklock Minder requires evidence-backed assessment.
...
- First-party source: https://github.com/stacklok/minder
...
- [ ] Trace Stacklock Minder inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Stacklock Minder defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Stacklock Minder least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Stacklock Minder resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Stacklock Minder release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Stacklock Minder dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Stacklock Minder identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Stacklock Minder change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Stacklock Minder documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Stacklock Minder architecture and dependency summary with trust boundaries and persistent state.
- Include Stacklock Minder validation commands or tests that are safe for the stated environment.
...
- Stacklock Minder identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Stacklock Minder failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Stacklock Minder is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Stacklock Minder as the basis for migration or production guidance.
