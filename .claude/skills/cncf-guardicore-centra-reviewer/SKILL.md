---
name: "cncf-guardicore-centra-reviewer"
description: "Review Guardicore Centra using its official documentation and repository in the Runtime / Cloud Native Network category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Guardicore Centra Reviewer

## Purpose

Review and operate Guardicore Centra using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Guardicore Centra code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Guardicore Centra requires evidence-backed assessment.
...
- First-party source: https://www.guardicore.com/cyber-security-platform
...
- [ ] Trace Guardicore Centra inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Guardicore Centra defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Guardicore Centra least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Guardicore Centra resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Guardicore Centra release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Guardicore Centra dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Guardicore Centra identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Guardicore Centra change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Guardicore Centra documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Guardicore Centra architecture and dependency summary with trust boundaries and persistent state.
- Include Guardicore Centra validation commands or tests that are safe for the stated environment.
...
- Guardicore Centra identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Guardicore Centra failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Guardicore Centra is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Guardicore Centra as the basis for migration or production guidance.
