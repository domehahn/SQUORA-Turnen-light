---
name: "cncf-threat-stack-reviewer"
description: "Review Threat Stack using its official documentation and repository in the Serverless / Security category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Threat Stack Reviewer

## Purpose

Review and operate Threat Stack using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Threat Stack code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Threat Stack requires evidence-backed assessment.
...
- First-party source: https://www.threatstack.com/
...
- [ ] Trace Threat Stack inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Threat Stack defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Threat Stack least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Threat Stack resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Threat Stack release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Threat Stack dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Threat Stack identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Threat Stack change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Threat Stack documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Threat Stack architecture and dependency summary with trust boundaries and persistent state.
- Include Threat Stack validation commands or tests that are safe for the stated environment.
...
- Threat Stack identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Threat Stack failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Threat Stack is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Threat Stack as the basis for migration or production guidance.
