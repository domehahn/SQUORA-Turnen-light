---
name: "cncf-glasnostic-reviewer"
description: "Review Glasnostic using its official documentation and repository in the Orchestration & Management / Service Mesh category. Glasnostic makes modern cloud applications resilient by shaping how systems interact, automatically and in real-time."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Glasnostic Reviewer

## Purpose

Review and operate Glasnostic using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Glasnostic code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Glasnostic requires evidence-backed assessment.
...
- First-party source: https://www.linkedin.com/company/glasnostic/
...
- [ ] Trace Glasnostic inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Glasnostic defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Glasnostic least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Glasnostic resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Glasnostic release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Glasnostic dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Glasnostic identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Glasnostic change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Glasnostic documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Glasnostic architecture and dependency summary with trust boundaries and persistent state.
- Include Glasnostic validation commands or tests that are safe for the stated environment.
...
- Glasnostic identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Glasnostic failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Glasnostic is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Glasnostic as the basis for migration or production guidance.
