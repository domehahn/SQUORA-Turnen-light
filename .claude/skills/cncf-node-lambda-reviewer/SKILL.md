---
name: "cncf-node-lambda-reviewer"
description: "Review Node Lambda using its official documentation and repository in the Serverless / Tools category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Node Lambda Reviewer

## Purpose

Review and operate Node Lambda using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Node Lambda code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Node Lambda requires evidence-backed assessment.
...
- First-party source: https://github.com/motdotla/node-lambda
...
- [ ] Trace Node Lambda inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Node Lambda defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Node Lambda least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Node Lambda resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Node Lambda release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Node Lambda dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Node Lambda identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Node Lambda change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Node Lambda documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Node Lambda architecture and dependency summary with trust boundaries and persistent state.
- Include Node Lambda validation commands or tests that are safe for the stated environment.
...
- Node Lambda identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Node Lambda failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Node Lambda is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Node Lambda as the basis for migration or production guidance.
