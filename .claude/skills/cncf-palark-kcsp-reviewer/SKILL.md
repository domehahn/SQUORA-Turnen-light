---
name: "cncf-palark-kcsp-reviewer"
description: "Review Palark (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Palark is a European provider of DevOps and SRE services. We design, implement, and support robust Kubernetes-based infrastructure and efficient CI/CD for cloud native apps."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Palark Kcsp Reviewer

## Purpose

Review and operate Palark (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Palark (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Palark (KCSP) requires evidence-backed assessment.
...
- First-party source: https://palark.com/services/kubernetes-support/
...
- [ ] Trace Palark (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Palark (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Palark (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Palark (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Palark (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Palark (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Palark (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Palark (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Palark (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Palark (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Palark (KCSP) validation commands or tests that are safe for the stated environment.
...
- Palark (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Palark (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Palark (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Palark (KCSP) as the basis for migration or production guidance.
