---
name: "cncf-nebulagraph-reviewer"
description: "Review NebulaGraph using its official documentation and repository in the App Definition and Development / Database category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Nebulagraph Reviewer

## Purpose

Review and operate NebulaGraph using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- NebulaGraph code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving NebulaGraph requires evidence-backed assessment.
...
- First-party source: https://github.com/vesoft-inc/nebula
...
- [ ] Trace NebulaGraph inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check NebulaGraph defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check NebulaGraph least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check NebulaGraph resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check NebulaGraph release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check NebulaGraph dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If NebulaGraph identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a NebulaGraph change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- NebulaGraph documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a NebulaGraph architecture and dependency summary with trust boundaries and persistent state.
- Include NebulaGraph validation commands or tests that are safe for the stated environment.
...
- NebulaGraph identity, version, configuration, dependencies, and deployment assumptions are explicit.
- NebulaGraph failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that NebulaGraph is secure, supported, compatible, or suitable.
- Using an unpinned latest version of NebulaGraph as the basis for migration or production guidance.
