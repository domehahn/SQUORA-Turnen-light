---
name: "cncf-wasm-pack-reviewer"
description: "Review wasm-pack using its official documentation and repository in the Wasm / Tooling category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Wasm Pack Reviewer

## Purpose

Review and operate wasm-pack using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- wasm-pack code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving wasm-pack requires evidence-backed assessment.
...
- First-party source: https://github.com/rustwasm/wasm-pack
...
- [ ] Trace wasm-pack inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check wasm-pack defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check wasm-pack least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check wasm-pack resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check wasm-pack release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check wasm-pack dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If wasm-pack identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a wasm-pack change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- wasm-pack documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a wasm-pack architecture and dependency summary with trust boundaries and persistent state.
- Include wasm-pack validation commands or tests that are safe for the stated environment.
...
- wasm-pack identity, version, configuration, dependencies, and deployment assumptions are explicit.
- wasm-pack failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that wasm-pack is secure, supported, compatible, or suitable.
- Using an unpinned latest version of wasm-pack as the basis for migration or production guidance.
