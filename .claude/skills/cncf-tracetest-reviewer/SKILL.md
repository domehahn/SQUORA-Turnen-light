...
---
# Cncf Tracetest Reviewer

## Purpose

Review and operate Tracetest using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Tracetest code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Tracetest requires evidence-backed assessment.
...
- First-party source: https://github.com/kubeshop/tracetest
...
- [ ] Trace Tracetest inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Tracetest defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Tracetest least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Tracetest resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Tracetest release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Tracetest dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Tracetest identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Tracetest change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Tracetest documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Tracetest architecture and dependency summary with trust boundaries and persistent state.
- Include Tracetest validation commands or tests that are safe for the stated environment.
...
- Tracetest identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Tracetest failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Tracetest is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Tracetest as the basis for migration or production guidance.
