...
---
# Cncf Aws Cloud Map Reviewer

## Purpose

Review and operate AWS Cloud Map using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- AWS Cloud Map code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving AWS Cloud Map requires evidence-backed assessment.
...
- First-party source: https://aws.amazon.com/cloud-map/
...
- [ ] Trace AWS Cloud Map inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check AWS Cloud Map defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check AWS Cloud Map least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check AWS Cloud Map resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check AWS Cloud Map release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check AWS Cloud Map dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If AWS Cloud Map identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a AWS Cloud Map change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- AWS Cloud Map documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a AWS Cloud Map architecture and dependency summary with trust boundaries and persistent state.
- Include AWS Cloud Map validation commands or tests that are safe for the stated environment.
...
- AWS Cloud Map identity, version, configuration, dependencies, and deployment assumptions are explicit.
- AWS Cloud Map failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that AWS Cloud Map is secure, supported, compatible, or suitable.
- Using an unpinned latest version of AWS Cloud Map as the basis for migration or production guidance.
