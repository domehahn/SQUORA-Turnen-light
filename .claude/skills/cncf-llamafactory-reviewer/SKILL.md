---
name: "cncf-llamafactory-reviewer"
description: "Review LlamaFactory using its official documentation and repository in the Training / Post Training category. Unified Efficient Fine-Tuning of 100+ LLMs & VLMs"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Llamafactory Reviewer

## Purpose

Review and operate LlamaFactory using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- LlamaFactory code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving LlamaFactory requires evidence-backed assessment.
...
- First-party source: https://github.com/hiyouga/LlamaFactory
...
- [ ] Trace LlamaFactory inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check LlamaFactory defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check LlamaFactory least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check LlamaFactory resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check LlamaFactory release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check LlamaFactory dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If LlamaFactory identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a LlamaFactory change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- LlamaFactory documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a LlamaFactory architecture and dependency summary with trust boundaries and persistent state.
- Include LlamaFactory validation commands or tests that are safe for the stated environment.
...
- LlamaFactory identity, version, configuration, dependencies, and deployment assumptions are explicit.
- LlamaFactory failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that LlamaFactory is secure, supported, compatible, or suitable.
- Using an unpinned latest version of LlamaFactory as the basis for migration or production guidance.
