---
name: "cncf-kubero-reviewer"
description: "Review Kubero using its official documentation and repository in the Platform / PaaS/Container Service category. Kubero is a developer friendly selfservice platform for Kubernetes."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kubero Reviewer

## Purpose

Review and operate Kubero using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kubero code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kubero requires evidence-backed assessment.
...
- First-party source: https://github.com/kubero-dev/kubero
...
- [ ] Trace Kubero inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kubero defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kubero least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kubero resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kubero release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kubero dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kubero identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kubero change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kubero documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kubero architecture and dependency summary with trust boundaries and persistent state.
- Include Kubero validation commands or tests that are safe for the stated environment.
...
- Kubero identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kubero failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kubero is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kubero as the basis for migration or production guidance.
