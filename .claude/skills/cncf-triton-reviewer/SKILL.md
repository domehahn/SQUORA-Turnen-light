---
name: "cncf-triton-reviewer"
description: "Review Triton using its official documentation and repository in the AI Native Infra / Accelerator and SuperPod category. Triton is a language and compiler for parallel programming."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Triton Reviewer

## Purpose

Review and operate Triton using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Triton code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Triton requires evidence-backed assessment.
...
- CNCF classification: AI Native Infra / Accelerator and SuperPod
- CNCF project maturity field: not specified
- First-party source: https://github.com/triton-lang/triton
...
- [ ] Trace Triton inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Triton defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Triton least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Triton resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Triton release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Triton dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Triton identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Triton change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Triton documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Triton architecture and dependency summary with trust boundaries and persistent state.
- Include Triton validation commands or tests that are safe for the stated environment.
...
- Triton identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Triton failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Triton is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Triton as the basis for migration or production guidance.
