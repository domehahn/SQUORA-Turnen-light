---
name: "cncf-chaos-toolkit-reviewer"
description: "Review Chaos Toolkit using its official documentation and repository in the Observability and Analysis / Chaos Engineering category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Chaos Toolkit Reviewer

## Purpose

Review and operate Chaos Toolkit using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Chaos Toolkit code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Chaos Toolkit requires evidence-backed assessment.
...
- First-party source: https://github.com/chaostoolkit/chaostoolkit
...
- [ ] Trace Chaos Toolkit inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Chaos Toolkit defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Chaos Toolkit least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Chaos Toolkit resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Chaos Toolkit release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Chaos Toolkit dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Chaos Toolkit identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Chaos Toolkit change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Chaos Toolkit documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Chaos Toolkit architecture and dependency summary with trust boundaries and persistent state.
- Include Chaos Toolkit validation commands or tests that are safe for the stated environment.
...
- Chaos Toolkit identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Chaos Toolkit failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Chaos Toolkit is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Chaos Toolkit as the basis for migration or production guidance.
