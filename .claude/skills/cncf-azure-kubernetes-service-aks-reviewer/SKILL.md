...
---
# Cncf Azure Kubernetes Service Aks Reviewer

## Purpose

Review and operate Azure Kubernetes Service (AKS) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Azure Kubernetes Service (AKS) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Azure Kubernetes Service (AKS) requires evidence-backed assessment.
...
- First-party source: https://azure.microsoft.com/en-us/services/kubernetes-service/
...
- [ ] Trace Azure Kubernetes Service (AKS) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Azure Kubernetes Service (AKS) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Azure Kubernetes Service (AKS) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Azure Kubernetes Service (AKS) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Azure Kubernetes Service (AKS) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Azure Kubernetes Service (AKS) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Azure Kubernetes Service (AKS) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Azure Kubernetes Service (AKS) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Azure Kubernetes Service (AKS) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Azure Kubernetes Service (AKS) architecture and dependency summary with trust boundaries and persistent state.
- Include Azure Kubernetes Service (AKS) validation commands or tests that are safe for the stated environment.
...
- Azure Kubernetes Service (AKS) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Azure Kubernetes Service (AKS) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Azure Kubernetes Service (AKS) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Azure Kubernetes Service (AKS) as the basis for migration or production guidance.
