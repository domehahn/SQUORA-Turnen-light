---
name: "cncf-hango-reviewer"
description: "Review Hango using its official documentation and repository in the Orchestration & Management / API Gateway category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Hango using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Hango code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Hango requires evidence-backed assessment.
...
- First-party source: https://github.com/hango-io/hango-gateway
...
- [ ] Trace Hango inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Hango defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Hango least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Hango resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Hango release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Hango dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Hango identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Hango change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Hango documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Hango architecture and dependency summary with trust boundaries and persistent state.
- Include Hango validation commands or tests that are safe for the stated environment.
...
- Hango identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Hango failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Hango is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Hango as the basis for migration or production guidance.
