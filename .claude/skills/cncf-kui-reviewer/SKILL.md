---
name: "cncf-kui-reviewer"
description: "Review Kui using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Kui using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kui code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kui requires evidence-backed assessment.
...
- First-party source: https://github.com/kubernetes-sigs/kui
...
- [ ] Trace Kui inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kui defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kui least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kui resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kui release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kui dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kui identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kui change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kui documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kui architecture and dependency summary with trust boundaries and persistent state.
- Include Kui validation commands or tests that are safe for the stated environment.
...
- Kui identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kui failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kui is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kui as the basis for migration or production guidance.
