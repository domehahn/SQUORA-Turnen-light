---
name: "cncf-krakend-reviewer"
description: "Review KrakenD using its official documentation and repository in the Orchestration & Management / API Gateway category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Krakend Reviewer

## Purpose

Review and operate KrakenD using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KrakenD code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KrakenD requires evidence-backed assessment.
...
- First-party source: https://github.com/luraproject/lura
...
- [ ] Trace KrakenD inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KrakenD defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KrakenD least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KrakenD resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KrakenD release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KrakenD dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KrakenD identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KrakenD change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KrakenD documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KrakenD architecture and dependency summary with trust boundaries and persistent state.
- Include KrakenD validation commands or tests that are safe for the stated environment.
...
- KrakenD identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KrakenD failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KrakenD is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KrakenD as the basis for migration or production guidance.
