---
name: "cncf-rbg-reviewer"
description: "Review RBG using its official documentation and repository in the Inference / Framework category. Kubernetes API for orchestrating distributed, stateful AI inference workloads with multi-role collaboration and built-in service discovery."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate RBG using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- RBG code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving RBG requires evidence-backed assessment.
...
- First-party source: https://github.com/sgl-project/rbg
...
- [ ] Trace RBG inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check RBG defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check RBG least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check RBG resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check RBG release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check RBG dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If RBG identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a RBG change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- RBG documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a RBG architecture and dependency summary with trust boundaries and persistent state.
- Include RBG validation commands or tests that are safe for the stated environment.
...
- RBG identity, version, configuration, dependencies, and deployment assumptions are explicit.
- RBG failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that RBG is secure, supported, compatible, or suitable.
- Using an unpinned latest version of RBG as the basis for migration or production guidance.
