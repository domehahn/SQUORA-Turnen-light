...
Review and operate ngrok using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ngrok code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ngrok requires evidence-backed assessment.
...
- First-party source: https://github.com/ngrok/ngrok-go
...
- [ ] Trace ngrok inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ngrok defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ngrok least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ngrok resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ngrok release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ngrok dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ngrok identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ngrok change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ngrok documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ngrok architecture and dependency summary with trust boundaries and persistent state.
- Include ngrok validation commands or tests that are safe for the stated environment.
...
- ngrok identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ngrok failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ngrok is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ngrok as the basis for migration or production guidance.
