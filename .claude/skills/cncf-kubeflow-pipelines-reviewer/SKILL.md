---
name: "cncf-kubeflow-pipelines-reviewer"
description: "Review Kubeflow Pipelines using its official documentation and repository in the AI Native Infra / Continuous Integration and Delivery category. Machine Learning Pipelines for Kubeflow"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kubeflow Pipelines Reviewer

## Purpose

Review and operate Kubeflow Pipelines using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kubeflow Pipelines code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kubeflow Pipelines requires evidence-backed assessment.
...
- First-party source: https://github.com/kubeflow/pipelines
...
- [ ] Trace Kubeflow Pipelines inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kubeflow Pipelines defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kubeflow Pipelines least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kubeflow Pipelines resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kubeflow Pipelines release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kubeflow Pipelines dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kubeflow Pipelines identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kubeflow Pipelines change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kubeflow Pipelines documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kubeflow Pipelines architecture and dependency summary with trust boundaries and persistent state.
- Include Kubeflow Pipelines validation commands or tests that are safe for the stated environment.
...
- Kubeflow Pipelines identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kubeflow Pipelines failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kubeflow Pipelines is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kubeflow Pipelines as the basis for migration or production guidance.
