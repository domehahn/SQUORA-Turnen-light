---
name: "cncf-copa-reviewer"
description: "Review Copa using its official documentation and repository in the Provisioning / Security & Compliance category. CLI tool for directly patching container image vulnerabilities"
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Copa using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Copa code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Copa requires evidence-backed assessment.
...
- First-party source: https://github.com/project-copacetic/copacetic
...
- [ ] Trace Copa inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Copa defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Copa least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Copa resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Copa release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Copa dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Copa identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Copa change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Copa documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Copa architecture and dependency summary with trust boundaries and persistent state.
- Include Copa validation commands or tests that are safe for the stated environment.
...
- Copa identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Copa failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Copa is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Copa as the basis for migration or production guidance.
