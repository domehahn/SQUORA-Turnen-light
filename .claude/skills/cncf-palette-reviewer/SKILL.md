---
name: "cncf-palette-reviewer"
description: "Review Palette using its official documentation and repository in the Platform / Certified Kubernetes - Distribution category. Keep control of your Kubernetes infrastructure stack with Spectro Cloud’s as-a-Service experience for public clouds, private clouds, and bare metal environments"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Palette Reviewer

## Purpose

Review and operate Palette using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Palette code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Palette requires evidence-backed assessment.
...
- First-party source: https://spectrocloud.com
...
- [ ] Trace Palette inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Palette defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Palette least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Palette resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Palette release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Palette dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Palette identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Palette change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Palette documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Palette architecture and dependency summary with trust boundaries and persistent state.
- Include Palette validation commands or tests that are safe for the stated environment.
...
- Palette identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Palette failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Palette is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Palette as the basis for migration or production guidance.
