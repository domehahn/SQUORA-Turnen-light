...
---
# Cncf Huawei Functionstage Reviewer

## Purpose

Review and operate Huawei FunctionStage using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Huawei FunctionStage code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Huawei FunctionStage requires evidence-backed assessment.
...
- [ ] Trace Huawei FunctionStage inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Huawei FunctionStage defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Huawei FunctionStage least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Huawei FunctionStage resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Huawei FunctionStage release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Huawei FunctionStage dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Huawei FunctionStage identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Huawei FunctionStage change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Huawei FunctionStage documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Huawei FunctionStage architecture and dependency summary with trust boundaries and persistent state.
- Include Huawei FunctionStage validation commands or tests that are safe for the stated environment.
...
- Huawei FunctionStage identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Huawei FunctionStage failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Huawei FunctionStage is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Huawei FunctionStage as the basis for migration or production guidance.
