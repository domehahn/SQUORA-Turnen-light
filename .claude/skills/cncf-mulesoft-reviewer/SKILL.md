---
name: "cncf-mulesoft-reviewer"
description: "Review MuleSoft using its official documentation and repository in the Orchestration & Management / API Gateway category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Mulesoft Reviewer

## Purpose

Review and operate MuleSoft using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- MuleSoft code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving MuleSoft requires evidence-backed assessment.
...
- First-party source: https://github.com/mulesoft/mule
...
- [ ] Trace MuleSoft inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check MuleSoft defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check MuleSoft least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check MuleSoft resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check MuleSoft release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check MuleSoft dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If MuleSoft identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a MuleSoft change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- MuleSoft documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a MuleSoft architecture and dependency summary with trust boundaries and persistent state.
- Include MuleSoft validation commands or tests that are safe for the stated environment.
...
- MuleSoft identity, version, configuration, dependencies, and deployment assumptions are explicit.
- MuleSoft failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that MuleSoft is secure, supported, compatible, or suitable.
- Using an unpinned latest version of MuleSoft as the basis for migration or production guidance.
