...
Review and operate Pipy using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Pipy code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Pipy requires evidence-backed assessment.
...
- First-party source: https://github.com/flomesh-io/pipy
...
- [ ] Trace Pipy inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Pipy defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Pipy least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Pipy resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Pipy release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Pipy dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Pipy identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Pipy change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Pipy documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Pipy architecture and dependency summary with trust boundaries and persistent state.
- Include Pipy validation commands or tests that are safe for the stated environment.
...
- Pipy identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Pipy failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Pipy is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Pipy as the basis for migration or production guidance.
