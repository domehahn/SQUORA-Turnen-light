...
Review and operate KubeDiagrams using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeDiagrams code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeDiagrams requires evidence-backed assessment.
...
- First-party source: https://github.com/philippemerle/KubeDiagrams
...
- [ ] Trace KubeDiagrams inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeDiagrams defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeDiagrams least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeDiagrams resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeDiagrams release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeDiagrams dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeDiagrams identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeDiagrams change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeDiagrams documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeDiagrams architecture and dependency summary with trust boundaries and persistent state.
- Include KubeDiagrams validation commands or tests that are safe for the stated environment.
...
- KubeDiagrams identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeDiagrams failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeDiagrams is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeDiagrams as the basis for migration or production guidance.
