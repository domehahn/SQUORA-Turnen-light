---
name: "cncf-docker-swarm-reviewer"
description: "Review Docker Swarm using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Docker Swarm Reviewer

## Purpose

Review and operate Docker Swarm using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Docker Swarm code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Docker Swarm requires evidence-backed assessment.
...
- First-party source: https://github.com/moby/swarmkit
...
- [ ] Trace Docker Swarm inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Docker Swarm defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Docker Swarm least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Docker Swarm resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Docker Swarm release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Docker Swarm dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Docker Swarm identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Docker Swarm change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Docker Swarm documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Docker Swarm architecture and dependency summary with trust boundaries and persistent state.
- Include Docker Swarm validation commands or tests that are safe for the stated environment.
...
- Docker Swarm identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Docker Swarm failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Docker Swarm is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Docker Swarm as the basis for migration or production guidance.
