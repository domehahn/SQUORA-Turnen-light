---
name: "cncf-sva-kcsp-reviewer"
description: "Review SVA (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. SVA services include consulting, implementing and optimizing Kubernetes platforms on all layers and to share our knowledge with our customers and the community."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Sva Kcsp Reviewer

## Purpose

Review and operate SVA (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SVA (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SVA (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.sva.de/de/partner/cloud-native-computing-foundation-cncf
...
- [ ] Trace SVA (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SVA (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SVA (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SVA (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SVA (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SVA (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SVA (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SVA (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SVA (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SVA (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include SVA (KCSP) validation commands or tests that are safe for the stated environment.
...
- SVA (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SVA (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SVA (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SVA (KCSP) as the basis for migration or production guidance.
