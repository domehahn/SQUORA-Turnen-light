---
name: "cncf-microk8s-reviewer"
description: "Review MicroK8s using its official documentation and repository in the Platform / Certified Kubernetes - Distribution category. An easy to install, single node local Kubernetes."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Microk8s Reviewer

## Purpose

Review and operate MicroK8s using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- MicroK8s code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving MicroK8s requires evidence-backed assessment.
...
- First-party source: https://github.com/canonical/microk8s
...
- [ ] Trace MicroK8s inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check MicroK8s defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check MicroK8s least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check MicroK8s resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check MicroK8s release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check MicroK8s dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If MicroK8s identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a MicroK8s change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- MicroK8s documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a MicroK8s architecture and dependency summary with trust boundaries and persistent state.
- Include MicroK8s validation commands or tests that are safe for the stated environment.
...
- MicroK8s identity, version, configuration, dependencies, and deployment assumptions are explicit.
- MicroK8s failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that MicroK8s is secure, supported, compatible, or suitable.
- Using an unpinned latest version of MicroK8s as the basis for migration or production guidance.
