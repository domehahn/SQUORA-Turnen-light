---
name: "cncf-kapeta-reviewer"
description: "Review Kapeta using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category. Kapeta accelerates and automates the entire software development lifecycle"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kapeta Reviewer

## Purpose

Review and operate Kapeta using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kapeta code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kapeta requires evidence-backed assessment.
...
- First-party source: https://github.com/kapetacom/kapeta
...
- [ ] Trace Kapeta inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kapeta defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kapeta least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kapeta resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kapeta release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kapeta dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kapeta identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kapeta change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kapeta documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kapeta architecture and dependency summary with trust boundaries and persistent state.
- Include Kapeta validation commands or tests that are safe for the stated environment.
...
- Kapeta identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kapeta failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kapeta is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kapeta as the basis for migration or production guidance.
