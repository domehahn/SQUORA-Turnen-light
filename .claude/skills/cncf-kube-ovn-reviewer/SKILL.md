---
name: "cncf-kube-ovn-reviewer"
description: "Review Kube-OVN using its official documentation and repository in the Runtime / Cloud Native Network category. A Kubernetes Network Fabric for Enterprises that is Rich in Functions and Easy in Operations"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kube Ovn Reviewer

## Purpose

Review and operate Kube-OVN using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kube-OVN code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kube-OVN requires evidence-backed assessment.
...
- First-party source: https://github.com/kubeovn/kube-ovn
...
- [ ] Trace Kube-OVN inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kube-OVN defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kube-OVN least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kube-OVN resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kube-OVN release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kube-OVN dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kube-OVN identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kube-OVN change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kube-OVN documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kube-OVN architecture and dependency summary with trust boundaries and persistent state.
- Include Kube-OVN validation commands or tests that are safe for the stated environment.
...
- Kube-OVN identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kube-OVN failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kube-OVN is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kube-OVN as the basis for migration or production guidance.
