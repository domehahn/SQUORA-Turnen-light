---
name: "cncf-postgresql-reviewer"
description: "Review PostgreSQL using its official documentation and repository in the App Definition and Development / Database category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Postgresql Reviewer

## Purpose

Review and operate PostgreSQL using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- PostgreSQL code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving PostgreSQL requires evidence-backed assessment.
...
- First-party source: https://github.com/postgres/postgres
...
- [ ] Trace PostgreSQL inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check PostgreSQL defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check PostgreSQL least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check PostgreSQL resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check PostgreSQL release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check PostgreSQL dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If PostgreSQL identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a PostgreSQL change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- PostgreSQL documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a PostgreSQL architecture and dependency summary with trust boundaries and persistent state.
- Include PostgreSQL validation commands or tests that are safe for the stated environment.
...
- PostgreSQL identity, version, configuration, dependencies, and deployment assumptions are explicit.
- PostgreSQL failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that PostgreSQL is secure, supported, compatible, or suitable.
- Using an unpinned latest version of PostgreSQL as the basis for migration or production guidance.
