---
name: "cncf-karpenter-reviewer"
description: "Review Karpenter using its official documentation and repository in the Observability and Analysis / Continuous Optimization category. Karpenter is a Kubernetes Node Autoscaler built for flexibility, performance, and simplicity."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Karpenter Reviewer

## Purpose

Review and operate Karpenter using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Karpenter code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Karpenter requires evidence-backed assessment.
...
- First-party source: https://github.com/kubernetes-sigs/karpenter
...
- [ ] Trace Karpenter inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Karpenter defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Karpenter least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Karpenter resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Karpenter release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Karpenter dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Karpenter identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Karpenter change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Karpenter documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Karpenter architecture and dependency summary with trust boundaries and persistent state.
- Include Karpenter validation commands or tests that are safe for the stated environment.
...
- Karpenter identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Karpenter failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Karpenter is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Karpenter as the basis for migration or production guidance.
