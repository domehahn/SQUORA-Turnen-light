---
name: "cncf-aqua-security-kcsp-reviewer"
description: "Review Aqua Security (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Aqua Security helps enterprises secure their cloud native applications from development to production, whether they run using containers, serverless, or virtual machines"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Aqua Security Kcsp Reviewer

## Purpose

Review and operate Aqua Security (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Aqua Security (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Aqua Security (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.aquasec.com/aqua-cloud-native-security-platform
...
- [ ] Trace Aqua Security (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Aqua Security (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Aqua Security (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Aqua Security (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Aqua Security (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Aqua Security (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Aqua Security (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Aqua Security (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Aqua Security (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Aqua Security (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Aqua Security (KCSP) validation commands or tests that are safe for the stated environment.
...
- Aqua Security (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Aqua Security (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Aqua Security (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Aqua Security (KCSP) as the basis for migration or production guidance.
