...
---
# Cncf Grafana Mimir Reviewer

## Purpose

Review and operate Grafana Mimir using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Grafana Mimir code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Grafana Mimir requires evidence-backed assessment.
...
- First-party source: https://github.com/grafana/mimir
...
- [ ] Trace Grafana Mimir inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Grafana Mimir defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Grafana Mimir least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Grafana Mimir resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Grafana Mimir release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Grafana Mimir dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Grafana Mimir identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Grafana Mimir change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Grafana Mimir documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Grafana Mimir architecture and dependency summary with trust boundaries and persistent state.
- Include Grafana Mimir validation commands or tests that are safe for the stated environment.
...
- Grafana Mimir identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Grafana Mimir failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Grafana Mimir is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Grafana Mimir as the basis for migration or production guidance.
