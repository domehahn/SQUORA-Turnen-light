---
name: "cncf-bucket-reviewer"
description: "Review Bucket using its official documentation and repository in the Observability and Analysis / Feature Flagging category. Feature flagging purpose-built for B2B SaaS products"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Bucket Reviewer

## Purpose

Review and operate Bucket using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Bucket code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Bucket requires evidence-backed assessment.
...
- First-party source: https://bucket.co
...
- [ ] Trace Bucket inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Bucket defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Bucket least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Bucket resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Bucket release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Bucket dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Bucket identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Bucket change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Bucket documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Bucket architecture and dependency summary with trust boundaries and persistent state.
- Include Bucket validation commands or tests that are safe for the stated environment.
...
- Bucket identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Bucket failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Bucket is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Bucket as the basis for migration or production guidance.
