---
name: "cncf-huatuo-reviewer"
description: "Review HUATUO using its official documentation and repository in the Observability and Analysis / Observability category. Kernel-wide Insight, Instant Observability, AutoTracing, Continuous Profiling for cloud-native and AI infrastructure using eBPF."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Huatuo Reviewer

## Purpose

Review and operate HUATUO using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- HUATUO code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving HUATUO requires evidence-backed assessment.
...
- First-party source: https://github.com/ccfos/huatuo
...
- [ ] Trace HUATUO inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check HUATUO defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check HUATUO least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check HUATUO resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check HUATUO release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check HUATUO dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If HUATUO identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a HUATUO change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- HUATUO documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a HUATUO architecture and dependency summary with trust boundaries and persistent state.
- Include HUATUO validation commands or tests that are safe for the stated environment.
...
- HUATUO identity, version, configuration, dependencies, and deployment assumptions are explicit.
- HUATUO failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that HUATUO is secure, supported, compatible, or suitable.
- Using an unpinned latest version of HUATUO as the basis for migration or production guidance.
