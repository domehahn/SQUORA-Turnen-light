---
name: "cncf-causely-reviewer"
description: "Review Causely using its official documentation and repository in the Observability and Analysis / Observability category. Causely's Causal Reasoning Platform continuously analyzes observed anomalies to automatically pinpoint root causes and trigger immediate remediation actions."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Causely Reviewer

## Purpose

Review and operate Causely using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Causely code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Causely requires evidence-backed assessment.
...
- [ ] Trace Causely inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Causely defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Causely least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Causely resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Causely release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Causely dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Causely identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Causely change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Causely documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Causely architecture and dependency summary with trust boundaries and persistent state.
- Include Causely validation commands or tests that are safe for the stated environment.
...
- Causely identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Causely failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Causely is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Causely as the basis for migration or production guidance.
