...
---
# Cncf Previder Kcsp Reviewer

## Purpose

Review and operate Previder (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Previder (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Previder (KCSP) requires evidence-backed assessment.
...
- First-party source: https://previder.nl/kubernetes-ondersteuning-op-maat
...
- [ ] Trace Previder (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Previder (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Previder (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Previder (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Previder (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Previder (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Previder (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Previder (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Previder (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Previder (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Previder (KCSP) validation commands or tests that are safe for the stated environment.
...
- Previder (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Previder (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Previder (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Previder (KCSP) as the basis for migration or production guidance.
