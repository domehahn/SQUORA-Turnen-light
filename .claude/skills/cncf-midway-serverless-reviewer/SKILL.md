---
name: "cncf-midway-serverless-reviewer"
description: "Review Midway Serverless using its official documentation and repository in the Serverless / Framework category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Midway Serverless Reviewer

## Purpose

Review and operate Midway Serverless using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Midway Serverless code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Midway Serverless requires evidence-backed assessment.
...
- First-party source: https://github.com/midwayjs/midway
...
- [ ] Trace Midway Serverless inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Midway Serverless defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Midway Serverless least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Midway Serverless resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Midway Serverless release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Midway Serverless dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Midway Serverless identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Midway Serverless change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Midway Serverless documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Midway Serverless architecture and dependency summary with trust boundaries and persistent state.
- Include Midway Serverless validation commands or tests that are safe for the stated environment.
...
- Midway Serverless identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Midway Serverless failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Midway Serverless is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Midway Serverless as the basis for migration or production guidance.
