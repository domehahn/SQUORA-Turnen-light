---
name: "cncf-ambient-it-kcntp-reviewer"
description: "Review Ambient IT (KCNTP) using its official documentation and repository in the Special / Kubernetes and Cloud Native Training Partner category. Ambient IT is a French training organization specializing in IT and digital professions."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Ambient It Kcntp Reviewer

## Purpose

Review and operate Ambient IT (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Ambient IT (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Ambient IT (KCNTP) requires evidence-backed assessment.
...
- First-party source: https://www.ambient-it.net/formation/formation-kubernetes/
...
- [ ] Trace Ambient IT (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Ambient IT (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Ambient IT (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Ambient IT (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Ambient IT (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Ambient IT (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Ambient IT (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Ambient IT (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Ambient IT (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Ambient IT (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include Ambient IT (KCNTP) validation commands or tests that are safe for the stated environment.
...
- Ambient IT (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Ambient IT (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Ambient IT (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Ambient IT (KCNTP) as the basis for migration or production guidance.
