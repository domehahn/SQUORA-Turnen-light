---
name: "cncf-thundra-reviewer"
description: "Review Thundra using its official documentation and repository in the Serverless / Tools category. Thundra helps answer your toughest questions about the health of your Serverless infrastructure by giving you invaluable insights into your AWS Lambda functions."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Thundra Reviewer

## Purpose

Review and operate Thundra using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Thundra code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Thundra requires evidence-backed assessment.
...
- First-party source: https://www.thundra.io/
...
- [ ] Trace Thundra inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Thundra defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Thundra least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Thundra resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Thundra release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Thundra dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Thundra identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Thundra change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Thundra documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Thundra architecture and dependency summary with trust boundaries and persistent state.
- Include Thundra validation commands or tests that are safe for the stated environment.
...
- Thundra identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Thundra failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Thundra is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Thundra as the basis for migration or production guidance.
