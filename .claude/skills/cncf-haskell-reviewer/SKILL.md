---
name: "cncf-haskell-reviewer"
description: "Review Haskell using its official documentation and repository in the Wasm / Languages category. Purely functional language with wasm backend"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Haskell Reviewer

## Purpose

Review and operate Haskell using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Haskell code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Haskell requires evidence-backed assessment.
...
- First-party source: https://gitlab.haskell.org/ghc/ghc
...
- [ ] Trace Haskell inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Haskell defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Haskell least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Haskell resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Haskell release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Haskell dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Haskell identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Haskell change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Haskell documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Haskell architecture and dependency summary with trust boundaries and persistent state.
- Include Haskell validation commands or tests that are safe for the stated environment.
...
- Haskell identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Haskell failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Haskell is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Haskell as the basis for migration or production guidance.
