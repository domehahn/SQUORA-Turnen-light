---
name: "cncf-quobyte-reviewer"
description: "Review Quobyte using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Quobyte Reviewer

## Purpose

Review and operate Quobyte using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Quobyte code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Quobyte requires evidence-backed assessment.
...
- First-party source: https://www.quobyte.com/
...
- [ ] Trace Quobyte inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Quobyte defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Quobyte least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Quobyte resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Quobyte release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Quobyte dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Quobyte identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Quobyte change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Quobyte documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Quobyte architecture and dependency summary with trust boundaries and persistent state.
- Include Quobyte validation commands or tests that are safe for the stated environment.
...
- Quobyte identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Quobyte failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Quobyte is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Quobyte as the basis for migration or production guidance.
