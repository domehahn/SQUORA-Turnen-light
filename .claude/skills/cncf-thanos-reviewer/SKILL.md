---
name: "cncf-thanos-reviewer"
description: "Review Thanos using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Thanos Reviewer

## Purpose

Review and operate Thanos using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Thanos code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Thanos requires evidence-backed assessment.
...
- First-party source: https://github.com/thanos-io/thanos
...
- [ ] Trace Thanos inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Thanos defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Thanos least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Thanos resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Thanos release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Thanos dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Thanos identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Thanos change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Thanos documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Thanos architecture and dependency summary with trust boundaries and persistent state.
- Include Thanos validation commands or tests that are safe for the stated environment.
...
- Thanos identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Thanos failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Thanos is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Thanos as the basis for migration or production guidance.
