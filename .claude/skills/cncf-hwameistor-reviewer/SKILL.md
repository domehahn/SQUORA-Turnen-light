---
name: "cncf-hwameistor-reviewer"
description: "Review HwameiStor using its official documentation and repository in the Runtime / Cloud Native Storage category. Hwameistor is an HA local storage system for cloud-native stateful workloads"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Hwameistor Reviewer

## Purpose

Review and operate HwameiStor using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- HwameiStor code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving HwameiStor requires evidence-backed assessment.
...
- First-party source: https://github.com/hwameistor/hwameistor
...
- [ ] Trace HwameiStor inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check HwameiStor defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check HwameiStor least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check HwameiStor resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check HwameiStor release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check HwameiStor dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If HwameiStor identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a HwameiStor change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- HwameiStor documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a HwameiStor architecture and dependency summary with trust boundaries and persistent state.
- Include HwameiStor validation commands or tests that are safe for the stated environment.
...
- HwameiStor identity, version, configuration, dependencies, and deployment assumptions are explicit.
- HwameiStor failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that HwameiStor is secure, supported, compatible, or suitable.
- Using an unpinned latest version of HwameiStor as the basis for migration or production guidance.
