---
name: "cncf-endee-reviewer"
description: "Review Endee using its official documentation and repository in the App Definition and Development / Database category. High-performance open-source vector database for AI search, RAG, semantic search, and hybrid retrieval."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Endee using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Endee code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Endee requires evidence-backed assessment.
...
- First-party source: https://github.com/endee-io/endee
...
- [ ] Trace Endee inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Endee defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Endee least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Endee resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Endee release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Endee dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Endee identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Endee change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Endee documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Endee architecture and dependency summary with trust boundaries and persistent state.
- Include Endee validation commands or tests that are safe for the stated environment.
...
- Endee identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Endee failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Endee is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Endee as the basis for migration or production guidance.
