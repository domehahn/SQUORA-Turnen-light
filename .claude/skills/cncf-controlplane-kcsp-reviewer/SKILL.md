---
name: "cncf-controlplane-kcsp-reviewer"
description: "Review ControlPlane (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. ControlPlane is a cloud native security consultancy specialising in high-compliance development and security operations for financial and government organisations, third party code risk management, and end-to-end supply chain security."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Controlplane Kcsp Reviewer

## Purpose

Review and operate ControlPlane (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ControlPlane (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ControlPlane (KCSP) requires evidence-backed assessment.
...
- [ ] Trace ControlPlane (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ControlPlane (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ControlPlane (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ControlPlane (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ControlPlane (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ControlPlane (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ControlPlane (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ControlPlane (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ControlPlane (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ControlPlane (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include ControlPlane (KCSP) validation commands or tests that are safe for the stated environment.
...
- ControlPlane (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ControlPlane (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ControlPlane (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ControlPlane (KCSP) as the basis for migration or production guidance.
