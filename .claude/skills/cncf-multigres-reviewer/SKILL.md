---
name: "cncf-multigres-reviewer"
description: "Review Multigres using its official documentation and repository in the App Definition and Development / Database category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Multigres Reviewer

## Purpose

Review and operate Multigres using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Multigres code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Multigres requires evidence-backed assessment.
...
- First-party source: https://github.com/multigres/multigres
...
- [ ] Trace Multigres inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Multigres defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Multigres least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Multigres resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Multigres release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Multigres dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Multigres identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Multigres change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Multigres documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Multigres architecture and dependency summary with trust boundaries and persistent state.
- Include Multigres validation commands or tests that are safe for the stated environment.
...
- Multigres identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Multigres failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Multigres is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Multigres as the basis for migration or production guidance.
