---
name: "cncf-cargo-reviewer"
description: "Review cargo using its official documentation and repository in the Wasm / Tooling category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate cargo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- cargo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving cargo requires evidence-backed assessment.
...
- First-party source: https://github.com/rust-lang/cargo
...
- [ ] Trace cargo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check cargo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check cargo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check cargo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check cargo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check cargo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If cargo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a cargo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- cargo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a cargo architecture and dependency summary with trust boundaries and persistent state.
- Include cargo validation commands or tests that are safe for the stated environment.
...
- cargo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- cargo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that cargo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of cargo as the basis for migration or production guidance.
