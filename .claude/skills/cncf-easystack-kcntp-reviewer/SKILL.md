---
name: "cncf-easystack-kcntp-reviewer"
...
Review and operate EasyStack (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- EasyStack (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving EasyStack (KCNTP) requires evidence-backed assessment.
...
- [ ] Trace EasyStack (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check EasyStack (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check EasyStack (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check EasyStack (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check EasyStack (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check EasyStack (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If EasyStack (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a EasyStack (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- EasyStack (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a EasyStack (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include EasyStack (KCNTP) validation commands or tests that are safe for the stated environment.
...
- EasyStack (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- EasyStack (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that EasyStack (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of EasyStack (KCNTP) as the basis for migration or production guidance.
