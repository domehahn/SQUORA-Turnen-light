---
name: "cncf-docker-wasm-reviewer"
description: "Review Docker (Wasm) using its official documentation and repository in the Wasm / Orchestration & Management category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Docker Wasm Reviewer

## Purpose

Review and operate Docker (Wasm) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Docker (Wasm) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Docker (Wasm) requires evidence-backed assessment.
...
- [ ] Trace Docker (Wasm) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Docker (Wasm) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Docker (Wasm) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Docker (Wasm) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Docker (Wasm) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Docker (Wasm) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Docker (Wasm) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Docker (Wasm) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Docker (Wasm) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Docker (Wasm) architecture and dependency summary with trust boundaries and persistent state.
- Include Docker (Wasm) validation commands or tests that are safe for the stated environment.
...
- Docker (Wasm) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Docker (Wasm) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Docker (Wasm) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Docker (Wasm) as the basis for migration or production guidance.
