...
Review and operate Cedar using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Cedar code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Cedar requires evidence-backed assessment.
...
- First-party source: https://github.com/cedar-policy/cedar
...
- [ ] Trace Cedar inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Cedar defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Cedar least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Cedar resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Cedar release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Cedar dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Cedar identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Cedar change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Cedar documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Cedar architecture and dependency summary with trust boundaries and persistent state.
- Include Cedar validation commands or tests that are safe for the stated environment.
...
- Cedar identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Cedar failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Cedar is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Cedar as the basis for migration or production guidance.
