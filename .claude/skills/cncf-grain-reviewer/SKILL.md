---
name: "cncf-grain-reviewer"
description: "Review Grain using its official documentation and repository in the Wasm / Languages category. WASM focused languages"
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Grain using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Grain code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Grain requires evidence-backed assessment.
...
- First-party source: https://github.com/grain-lang/grain
...
- [ ] Trace Grain inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Grain defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Grain least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Grain resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Grain release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Grain dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Grain identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Grain change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Grain documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Grain architecture and dependency summary with trust boundaries and persistent state.
- Include Grain validation commands or tests that are safe for the stated environment.
...
- Grain identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Grain failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Grain is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Grain as the basis for migration or production guidance.
