---
name: "cncf-granulate-reviewer"
description: "Review Granulate using its official documentation and repository in the Observability and Analysis / Continuous Optimization category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Granulate Reviewer

## Purpose

Review and operate Granulate using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Granulate code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Granulate requires evidence-backed assessment.
...
- First-party source: https://www.granulate.io/
...
- [ ] Trace Granulate inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Granulate defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Granulate least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Granulate resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Granulate release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Granulate dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Granulate identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Granulate change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Granulate documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Granulate architecture and dependency summary with trust boundaries and persistent state.
- Include Granulate validation commands or tests that are safe for the stated environment.
...
- Granulate identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Granulate failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Granulate is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Granulate as the basis for migration or production guidance.
