...
1. Identify the relevant ICT third-party risk artifacts, owners, systems, environments, and review boundary.
2. Compare the available artifacts against expected signals such as contract clause, provider assessment, service criticality, suboutsourcing notice, exit-test result, register update.
...
- Risk themes: unlisted subcontractor, weak exit clause, concentration exposure, missing data location, untracked outsourcing dependency, contract evidence gap.
- Evidence signals: contract clause, provider assessment, service criticality, suboutsourcing notice, exit-test result, register update.
...
- [ ] Confirm the review boundary covers the right ICT third-party risk systems, teams, and environments.
- [ ] Inventory and inspect the current provider inventory.
- [ ] Check whether cloud contract is current, approved, versioned, and owned.
- [ ] Verify that SaaS data-flow has test, ticket, log, or approval support.
- [ ] Look for unlisted subcontractor and record concrete repository or process evidence.
- [ ] Look for weak exit clause and identify affected assets, services, or stakeholders.
- [ ] Look for concentration exposure and classify the operational or audit impact.
- [ ] Use contract clause to validate that the control or practice is operating.
- [ ] Use provider assessment to confirm ownership, timing, and reproducibility.
- [ ] Check exception, risk-acceptance, and expiry handling for ICT third-party risk.
...
- If provider inventory is missing for a critical service, raise at least a high-severity readiness gap.
- If provider assessment cannot be tied to an owner and approval, treat the outcome as unauditable until corrected.
- If subcontractor list is present but expired or untested, require validation before accepting residual risk.
...
- Missing or stale ICT third-party risk artifact.
...
- Critical: a gap in ICT third-party risk creates immediate outage, data-loss, privilege, regulatory-reporting, or irreversible business risk.
- High: provider inventory is missing, unowned, untested, or unauditable for a critical service or material change.
- Medium: cloud contract exists but is stale, incomplete, inconsistently enforced, or weakly evidenced.
...
- Findings ordered by severity with affected ICT third-party risk artifacts and evidence references.
- Coverage note for reviewed artifacts: provider inventory, cloud contract, SaaS data-flow, subcontractor list, exit plan, information register fields.
- Risk note covering relevant themes: unlisted subcontractor, weak exit clause, concentration exposure, missing data location, untracked outsourcing dependency, contract evidence gap.
- Evidence request list using expected signals: contract clause, provider assessment, service criticality, suboutsourcing notice, exit-test result, register update.
- Deliverables or updates needed: third-party risk report, DORA register gap list, exit readiness findings, concentration risk summary, contract remediation items.
- Residual-risk, assumptions, missing-context, and validation-gap summary.

## Acceptance Criteria

- Relevant ICT third-party risk artifacts are identified, current, owned, and versioned where applicable.
