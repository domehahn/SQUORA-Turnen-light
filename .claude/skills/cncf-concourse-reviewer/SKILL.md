---
name: "cncf-concourse-reviewer"
description: "Review Concourse using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Concourse Reviewer

## Purpose

Review and operate Concourse using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Concourse code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Concourse requires evidence-backed assessment.
...
- First-party source: https://github.com/concourse/concourse
...
- [ ] Trace Concourse inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Concourse defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Concourse least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Concourse resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Concourse release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Concourse dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Concourse identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Concourse change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Concourse documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Concourse architecture and dependency summary with trust boundaries and persistent state.
- Include Concourse validation commands or tests that are safe for the stated environment.
...
- Concourse identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Concourse failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Concourse is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Concourse as the basis for migration or production guidance.
