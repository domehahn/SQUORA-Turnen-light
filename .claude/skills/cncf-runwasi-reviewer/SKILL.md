---
name: "cncf-runwasi-reviewer"
description: "Review runwasi using its official documentation and repository in the Wasm / Orchestration & Management category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Runwasi Reviewer

## Purpose

Review and operate runwasi using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- runwasi code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving runwasi requires evidence-backed assessment.
...
- First-party source: https://github.com/containerd/runwasi
...
- [ ] Trace runwasi inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check runwasi defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check runwasi least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check runwasi resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check runwasi release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check runwasi dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If runwasi identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a runwasi change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- runwasi documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a runwasi architecture and dependency summary with trust boundaries and persistent state.
- Include runwasi validation commands or tests that are safe for the stated environment.
...
- runwasi identity, version, configuration, dependencies, and deployment assumptions are explicit.
- runwasi failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that runwasi is secure, supported, compatible, or suitable.
- Using an unpinned latest version of runwasi as the basis for migration or production guidance.
