---
name: "cncf-heroku-reviewer"
description: "Review Heroku using its official documentation and repository in the Platform / PaaS/Container Service category. Developers, teams, and businesses of all sizes use Heroku to deploy, manage, and scale apps."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Heroku Reviewer

## Purpose

Review and operate Heroku using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Heroku code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Heroku requires evidence-backed assessment.
...
- [ ] Trace Heroku inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Heroku defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Heroku least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Heroku resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Heroku release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Heroku dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Heroku identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Heroku change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Heroku documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Heroku architecture and dependency summary with trust boundaries and persistent state.
- Include Heroku validation commands or tests that are safe for the stated environment.
...
- Heroku identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Heroku failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Heroku is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Heroku as the basis for migration or production guidance.
