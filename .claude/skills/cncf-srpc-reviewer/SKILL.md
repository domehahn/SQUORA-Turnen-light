---
name: "cncf-srpc-reviewer"
description: "Review SRPC using its official documentation and repository in the Orchestration & Management / Remote Procedure Call category. High performance, low latency, lightweight enterprise-level RPC system, which supports Baidu bRPC, Tencent tRPC, thrift protocols."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate SRPC using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SRPC code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SRPC requires evidence-backed assessment.
...
- First-party source: https://github.com/sogou/srpc
...
- [ ] Trace SRPC inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SRPC defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SRPC least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SRPC resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SRPC release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SRPC dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SRPC identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SRPC change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SRPC documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SRPC architecture and dependency summary with trust boundaries and persistent state.
- Include SRPC validation commands or tests that are safe for the stated environment.
...
- SRPC identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SRPC failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SRPC is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SRPC as the basis for migration or production guidance.
