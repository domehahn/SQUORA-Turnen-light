---
name: "cncf-sofatracer-reviewer"
description: "Review SOFATracer using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Sofatracer Reviewer

## Purpose

Review and operate SOFATracer using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SOFATracer code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SOFATracer requires evidence-backed assessment.
...
- First-party source: https://github.com/sofastack/sofa-tracer
...
- [ ] Trace SOFATracer inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SOFATracer defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SOFATracer least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SOFATracer resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SOFATracer release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SOFATracer dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SOFATracer identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SOFATracer change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SOFATracer documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SOFATracer architecture and dependency summary with trust boundaries and persistent state.
- Include SOFATracer validation commands or tests that are safe for the stated environment.
...
- SOFATracer identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SOFATracer failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SOFATracer is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SOFATracer as the basis for migration or production guidance.
