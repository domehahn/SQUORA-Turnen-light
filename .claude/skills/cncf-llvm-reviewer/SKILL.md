---
name: "cncf-llvm-reviewer"
description: "Review llvm using its official documentation and repository in the Wasm / Tooling category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate llvm using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- llvm code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving llvm requires evidence-backed assessment.
...
- First-party source: https://github.com/llvm/llvm-project
...
- [ ] Trace llvm inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check llvm defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check llvm least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check llvm resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check llvm release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check llvm dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If llvm identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a llvm change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- llvm documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a llvm architecture and dependency summary with trust boundaries and persistent state.
- Include llvm validation commands or tests that are safe for the stated environment.
...
- llvm identity, version, configuration, dependencies, and deployment assumptions are explicit.
- llvm failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that llvm is secure, supported, compatible, or suitable.
- Using an unpinned latest version of llvm as the basis for migration or production guidance.
