---
name: "cncf-argo-reviewer"
description: "Review Argo using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category. Kubernetes-native tools to run workflows, manage clusters, and do GitOps right."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Argo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Argo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Argo requires evidence-backed assessment.
...
- First-party source: https://github.com/argoproj/argo-cd
...
- [ ] Trace Argo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Argo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Argo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Argo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Argo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Argo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Argo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Argo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Argo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Argo architecture and dependency summary with trust boundaries and persistent state.
- Include Argo validation commands or tests that are safe for the stated environment.
...
- Argo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Argo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Argo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Argo as the basis for migration or production guidance.
