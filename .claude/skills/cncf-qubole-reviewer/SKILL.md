---
name: "cncf-qubole-reviewer"
description: "Review Qubole using its official documentation and repository in the App Definition and Development / Database category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Qubole Reviewer

## Purpose

Review and operate Qubole using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Qubole code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Qubole requires evidence-backed assessment.
...
- First-party source: https://www.qubole.com/
...
- [ ] Trace Qubole inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Qubole defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Qubole least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Qubole resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Qubole release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Qubole dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Qubole identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Qubole change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Qubole documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Qubole architecture and dependency summary with trust boundaries and persistent state.
- Include Qubole validation commands or tests that are safe for the stated environment.
...
- Qubole identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Qubole failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Qubole is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Qubole as the basis for migration or production guidance.
