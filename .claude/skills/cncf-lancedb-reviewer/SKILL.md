---
name: "cncf-lancedb-reviewer"
description: "Review LanceDB using its official documentation and repository in the AI Agent / Vector Database category. Developer-friendly OSS embedded retrieval library for multimodal AI. Search More; Manage Less."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Lancedb Reviewer

## Purpose

Review and operate LanceDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- LanceDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving LanceDB requires evidence-backed assessment.
...
- CNCF classification: AI Agent / Vector Database
- CNCF project maturity field: not specified
- First-party source: https://github.com/lancedb/lancedb
...
- [ ] Trace LanceDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check LanceDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check LanceDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check LanceDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check LanceDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check LanceDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If LanceDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a LanceDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- LanceDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a LanceDB architecture and dependency summary with trust boundaries and persistent state.
- Include LanceDB validation commands or tests that are safe for the stated environment.
...
- LanceDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- LanceDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that LanceDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of LanceDB as the basis for migration or production guidance.
