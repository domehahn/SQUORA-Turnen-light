---
name: "cncf-neuvector-reviewer"
description: "Review NeuVector using its official documentation and repository in the Provisioning / Security & Compliance category. NeuVector Full Lifecycle Container Security Platform delivers the only cloud-native security with uncompromising end-to-end protection from DevOps vulnerability protection to automated run-time security, and featuring a true Layer 7 container firewall."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Neuvector Reviewer

## Purpose

Review and operate NeuVector using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- NeuVector code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving NeuVector requires evidence-backed assessment.
...
- First-party source: https://github.com/neuvector/neuvector
...
- [ ] Trace NeuVector inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check NeuVector defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check NeuVector least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check NeuVector resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check NeuVector release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check NeuVector dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If NeuVector identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a NeuVector change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- NeuVector documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a NeuVector architecture and dependency summary with trust boundaries and persistent state.
- Include NeuVector validation commands or tests that are safe for the stated environment.
...
- NeuVector identity, version, configuration, dependencies, and deployment assumptions are explicit.
- NeuVector failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that NeuVector is secure, supported, compatible, or suitable.
- Using an unpinned latest version of NeuVector as the basis for migration or production guidance.
