---
name: "cncf-kiali-reviewer"
description: "Review Kiali using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Kiali using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kiali code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kiali requires evidence-backed assessment.
...
- First-party source: https://github.com/kiali/kiali
...
- [ ] Trace Kiali inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kiali defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kiali least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kiali resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kiali release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kiali dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kiali identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kiali change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kiali documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kiali architecture and dependency summary with trust boundaries and persistent state.
- Include Kiali validation commands or tests that are safe for the stated environment.
...
- Kiali identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kiali failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kiali is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kiali as the basis for migration or production guidance.
