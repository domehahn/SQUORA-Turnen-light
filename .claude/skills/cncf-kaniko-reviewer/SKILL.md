---
name: "cncf-kaniko-reviewer"
description: "Review kaniko using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kaniko Reviewer

## Purpose

Review and operate kaniko using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- kaniko code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving kaniko requires evidence-backed assessment.
...
- First-party source: https://github.com/chainguard-dev/kaniko
...
- [ ] Trace kaniko inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check kaniko defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check kaniko least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check kaniko resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check kaniko release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check kaniko dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If kaniko identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a kaniko change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- kaniko documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a kaniko architecture and dependency summary with trust boundaries and persistent state.
- Include kaniko validation commands or tests that are safe for the stated environment.
...
- kaniko identity, version, configuration, dependencies, and deployment assumptions are explicit.
- kaniko failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that kaniko is secure, supported, compatible, or suitable.
- Using an unpinned latest version of kaniko as the basis for migration or production guidance.
