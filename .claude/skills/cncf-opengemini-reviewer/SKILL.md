...
---
# Cncf Opengemini Reviewer

## Purpose

Review and operate openGemini using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- openGemini code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving openGemini requires evidence-backed assessment.
...
- First-party source: https://github.com/openGemini/openGemini
...
- [ ] Trace openGemini inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check openGemini defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check openGemini least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check openGemini resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check openGemini release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check openGemini dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If openGemini identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a openGemini change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- openGemini documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a openGemini architecture and dependency summary with trust boundaries and persistent state.
- Include openGemini validation commands or tests that are safe for the stated environment.
...
- openGemini identity, version, configuration, dependencies, and deployment assumptions are explicit.
- openGemini failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that openGemini is secure, supported, compatible, or suitable.
- Using an unpinned latest version of openGemini as the basis for migration or production guidance.
