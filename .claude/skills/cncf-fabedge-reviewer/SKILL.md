---
name: "cncf-fabedge-reviewer"
description: "Review FabEdge using its official documentation and repository in the Runtime / Cloud Native Network category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Fabedge Reviewer

## Purpose

Review and operate FabEdge using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- FabEdge code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving FabEdge requires evidence-backed assessment.
...
- First-party source: https://github.com/FabEdge/fabedge
...
- [ ] Trace FabEdge inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check FabEdge defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check FabEdge least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check FabEdge resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check FabEdge release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check FabEdge dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If FabEdge identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a FabEdge change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- FabEdge documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a FabEdge architecture and dependency summary with trust boundaries and persistent state.
- Include FabEdge validation commands or tests that are safe for the stated environment.
...
- FabEdge identity, version, configuration, dependencies, and deployment assumptions are explicit.
- FabEdge failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that FabEdge is secure, supported, compatible, or suitable.
- Using an unpinned latest version of FabEdge as the basis for migration or production guidance.
