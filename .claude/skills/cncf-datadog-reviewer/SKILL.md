---
name: "cncf-datadog-reviewer"
description: "Review Datadog using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Datadog Reviewer

## Purpose

Review and operate Datadog using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Datadog code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Datadog requires evidence-backed assessment.
...
- [ ] Trace Datadog inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Datadog defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Datadog least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Datadog resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Datadog release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Datadog dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Datadog identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Datadog change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Datadog documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Datadog architecture and dependency summary with trust boundaries and persistent state.
- Include Datadog validation commands or tests that are safe for the stated environment.
...
- Datadog identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Datadog failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Datadog is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Datadog as the basis for migration or production guidance.
