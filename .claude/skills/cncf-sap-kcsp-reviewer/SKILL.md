---
name: "cncf-sap-kcsp-reviewer"
description: "Review SAP (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Sap Kcsp Reviewer

## Purpose

Review and operate SAP (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SAP (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SAP (KCSP) requires evidence-backed assessment.
...
- [ ] Trace SAP (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SAP (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SAP (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SAP (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SAP (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SAP (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SAP (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SAP (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SAP (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SAP (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include SAP (KCSP) validation commands or tests that are safe for the stated environment.
...
- SAP (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SAP (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SAP (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SAP (KCSP) as the basis for migration or production guidance.
