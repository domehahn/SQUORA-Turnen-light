---
name: "cncf-gitness-reviewer"
description: "Review Gitness using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Gitness Reviewer

## Purpose

Review and operate Gitness using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Gitness code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Gitness requires evidence-backed assessment.
...
- First-party source: https://github.com/harness/gitness
...
- [ ] Trace Gitness inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Gitness defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Gitness least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Gitness resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Gitness release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Gitness dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Gitness identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Gitness change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Gitness documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Gitness architecture and dependency summary with trust boundaries and persistent state.
- Include Gitness validation commands or tests that are safe for the stated environment.
...
- Gitness identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Gitness failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Gitness is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Gitness as the basis for migration or production guidance.
