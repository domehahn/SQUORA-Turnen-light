---
name: "cncf-datacore-puls8-reviewer"
description: "Review DataCore Puls8 using its official documentation and repository in the Runtime / Cloud Native Storage category. DataCore Puls8 gives Kubernetes the enterprise-grade storage it needs – fast, resilient, and fully container-native. It ensures stateful workloads stay persistent, protected, and production-ready across any environment."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Datacore Puls8 Reviewer

## Purpose

Review and operate DataCore Puls8 using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DataCore Puls8 code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DataCore Puls8 requires evidence-backed assessment.
...
- First-party source: https://www.datacore.com/products/puls8/
...
- [ ] Trace DataCore Puls8 inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DataCore Puls8 defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DataCore Puls8 least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DataCore Puls8 resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DataCore Puls8 release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DataCore Puls8 dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DataCore Puls8 identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DataCore Puls8 change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DataCore Puls8 documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DataCore Puls8 architecture and dependency summary with trust boundaries and persistent state.
- Include DataCore Puls8 validation commands or tests that are safe for the stated environment.
...
- DataCore Puls8 identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DataCore Puls8 failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DataCore Puls8 is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DataCore Puls8 as the basis for migration or production guidance.
