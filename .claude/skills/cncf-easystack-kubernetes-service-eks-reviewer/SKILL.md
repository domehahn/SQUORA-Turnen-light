...
---
# Cncf Easystack Kubernetes Service Eks Reviewer

## Purpose

Review and operate EasyStack Kubernetes Service (EKS) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- EasyStack Kubernetes Service (EKS) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving EasyStack Kubernetes Service (EKS) requires evidence-backed assessment.
...
- First-party source: https://www.easystack.cn/eks
...
- [ ] Trace EasyStack Kubernetes Service (EKS) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check EasyStack Kubernetes Service (EKS) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check EasyStack Kubernetes Service (EKS) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check EasyStack Kubernetes Service (EKS) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check EasyStack Kubernetes Service (EKS) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check EasyStack Kubernetes Service (EKS) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If EasyStack Kubernetes Service (EKS) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a EasyStack Kubernetes Service (EKS) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- EasyStack Kubernetes Service (EKS) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a EasyStack Kubernetes Service (EKS) architecture and dependency summary with trust boundaries and persistent state.
- Include EasyStack Kubernetes Service (EKS) validation commands or tests that are safe for the stated environment.
...
- EasyStack Kubernetes Service (EKS) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- EasyStack Kubernetes Service (EKS) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that EasyStack Kubernetes Service (EKS) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of EasyStack Kubernetes Service (EKS) as the basis for migration or production guidance.
