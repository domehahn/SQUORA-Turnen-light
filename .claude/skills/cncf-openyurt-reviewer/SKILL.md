---
name: "cncf-openyurt-reviewer"
description: "Review OpenYurt using its official documentation and repository in the Provisioning / Automation & Configuration category. An open platform that extends upstream Kubernetes to Edge."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Openyurt Reviewer

## Purpose

Review and operate OpenYurt using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenYurt code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenYurt requires evidence-backed assessment.
...
- First-party source: https://github.com/openyurtio/openyurt
...
- [ ] Trace OpenYurt inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenYurt defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenYurt least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenYurt resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenYurt release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenYurt dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenYurt identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenYurt change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenYurt documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenYurt architecture and dependency summary with trust boundaries and persistent state.
- Include OpenYurt validation commands or tests that are safe for the stated environment.
...
- OpenYurt identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenYurt failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenYurt is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenYurt as the basis for migration or production guidance.
