---
name: "cncf-bigchaindb-reviewer"
description: "Review BigchainDB using its official documentation and repository in the App Definition and Development / Database category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Bigchaindb Reviewer

## Purpose

Review and operate BigchainDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- BigchainDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving BigchainDB requires evidence-backed assessment.
...
- First-party source: https://github.com/bigchaindb/bigchaindb
...
- [ ] Trace BigchainDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check BigchainDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check BigchainDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check BigchainDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check BigchainDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check BigchainDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If BigchainDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a BigchainDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- BigchainDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a BigchainDB architecture and dependency summary with trust boundaries and persistent state.
- Include BigchainDB validation commands or tests that are safe for the stated environment.
...
- BigchainDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- BigchainDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that BigchainDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of BigchainDB as the basis for migration or production guidance.
