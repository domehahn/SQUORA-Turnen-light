---
name: "cncf-foreman-reviewer"
description: "Review Foreman using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Foreman Reviewer

## Purpose

Review and operate Foreman using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Foreman code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Foreman requires evidence-backed assessment.
...
- First-party source: https://github.com/theforeman/foreman
...
- [ ] Trace Foreman inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Foreman defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Foreman least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Foreman resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Foreman release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Foreman dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Foreman identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Foreman change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Foreman documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Foreman architecture and dependency summary with trust boundaries and persistent state.
- Include Foreman validation commands or tests that are safe for the stated environment.
...
- Foreman identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Foreman failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Foreman is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Foreman as the basis for migration or production guidance.
