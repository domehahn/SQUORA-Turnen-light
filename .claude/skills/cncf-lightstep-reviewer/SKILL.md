---
name: "cncf-lightstep-reviewer"
description: "Review LightStep using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Lightstep Reviewer

## Purpose

Review and operate LightStep using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- LightStep code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving LightStep requires evidence-backed assessment.
...
- First-party source: https://lightstep.com/
...
- [ ] Trace LightStep inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check LightStep defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check LightStep least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check LightStep resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check LightStep release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check LightStep dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If LightStep identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a LightStep change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- LightStep documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a LightStep architecture and dependency summary with trust boundaries and persistent state.
- Include LightStep validation commands or tests that are safe for the stated environment.
...
- LightStep identity, version, configuration, dependencies, and deployment assumptions are explicit.
- LightStep failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that LightStep is secure, supported, compatible, or suitable.
- Using an unpinned latest version of LightStep as the basis for migration or production guidance.
