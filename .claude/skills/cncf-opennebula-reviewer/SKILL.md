---
name: "cncf-opennebula-reviewer"
description: "Review OpenNebula using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Opennebula Reviewer

## Purpose

Review and operate OpenNebula using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenNebula code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenNebula requires evidence-backed assessment.
...
- First-party source: https://github.com/OpenNebula/one
...
- [ ] Trace OpenNebula inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenNebula defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenNebula least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenNebula resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenNebula release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenNebula dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenNebula identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenNebula change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenNebula documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenNebula architecture and dependency summary with trust boundaries and persistent state.
- Include OpenNebula validation commands or tests that are safe for the stated environment.
...
- OpenNebula identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenNebula failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenNebula is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenNebula as the basis for migration or production guidance.
