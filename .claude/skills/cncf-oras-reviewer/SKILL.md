---
name: "cncf-oras-reviewer"
description: "Review ORAS using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category. Multi-language OCI Registry SDKs and CLI"
version: "1.0.0"
since: "2026-08-27"
...
Review and operate ORAS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ORAS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ORAS requires evidence-backed assessment.
...
- First-party source: https://github.com/oras-project/oras
...
- [ ] Trace ORAS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ORAS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ORAS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ORAS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ORAS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ORAS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ORAS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ORAS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ORAS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ORAS architecture and dependency summary with trust boundaries and persistent state.
- Include ORAS validation commands or tests that are safe for the stated environment.
...
- ORAS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ORAS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ORAS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ORAS as the basis for migration or production guidance.
