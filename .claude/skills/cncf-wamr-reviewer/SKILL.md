---
name: "cncf-wamr-reviewer"
description: "Review WAMR using its official documentation and repository in the Wasm / Runtimes category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate WAMR using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- WAMR code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving WAMR requires evidence-backed assessment.
...
- First-party source: https://github.com/bytecodealliance/wasm-micro-runtime
...
- [ ] Trace WAMR inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check WAMR defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check WAMR least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check WAMR resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check WAMR release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check WAMR dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If WAMR identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a WAMR change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- WAMR documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a WAMR architecture and dependency summary with trust boundaries and persistent state.
- Include WAMR validation commands or tests that are safe for the stated environment.
...
- WAMR identity, version, configuration, dependencies, and deployment assumptions are explicit.
- WAMR failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that WAMR is secure, supported, compatible, or suitable.
- Using an unpinned latest version of WAMR as the basis for migration or production guidance.
