---
name: "cncf-marimo-reviewer"
description: "Review marimo using its official documentation and repository in the Data / Data Science category. A reactive notebook for Python — run reproducible experiments, execute as a script, deploy as an app, and version with git."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Marimo Reviewer

## Purpose

Review and operate marimo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- marimo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving marimo requires evidence-backed assessment.
...
- First-party source: https://github.com/marimo-team/marimo
...
- [ ] Trace marimo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check marimo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check marimo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check marimo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check marimo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check marimo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If marimo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a marimo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- marimo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a marimo architecture and dependency summary with trust boundaries and persistent state.
- Include marimo validation commands or tests that are safe for the stated environment.
...
- marimo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- marimo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that marimo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of marimo as the basis for migration or production guidance.
