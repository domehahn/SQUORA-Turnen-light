---
name: "cncf-kube-rs-reviewer"
description: "Review kube-rs using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. kube-rs is the core Rust ecosystem for building applications against Kubernetes"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kube Rs Reviewer

## Purpose

Review and operate kube-rs using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- kube-rs code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving kube-rs requires evidence-backed assessment.
...
- First-party source: https://github.com/kube-rs/kube
...
- [ ] Trace kube-rs inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check kube-rs defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check kube-rs least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check kube-rs resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check kube-rs release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check kube-rs dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If kube-rs identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a kube-rs change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- kube-rs documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a kube-rs architecture and dependency summary with trust boundaries and persistent state.
- Include kube-rs validation commands or tests that are safe for the stated environment.
...
- kube-rs identity, version, configuration, dependencies, and deployment assumptions are explicit.
- kube-rs failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that kube-rs is secure, supported, compatible, or suitable.
- Using an unpinned latest version of kube-rs as the basis for migration or production guidance.
