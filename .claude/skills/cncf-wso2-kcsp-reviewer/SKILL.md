---
name: "cncf-wso2-kcsp-reviewer"
description: "Review WSO2 (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Wso2 Kcsp Reviewer

## Purpose

Review and operate WSO2 (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- WSO2 (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving WSO2 (KCSP) requires evidence-backed assessment.
...
- First-party source: https://wso2.com/strategic-consulting/cloud-native-strategy/
...
- [ ] Trace WSO2 (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check WSO2 (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check WSO2 (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check WSO2 (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check WSO2 (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check WSO2 (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If WSO2 (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a WSO2 (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- WSO2 (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a WSO2 (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include WSO2 (KCSP) validation commands or tests that are safe for the stated environment.
...
- WSO2 (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- WSO2 (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that WSO2 (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of WSO2 (KCSP) as the basis for migration or production guidance.
