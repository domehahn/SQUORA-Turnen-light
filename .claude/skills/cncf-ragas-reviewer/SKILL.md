---
name: "cncf-ragas-reviewer"
description: "Review Ragas using its official documentation and repository in the AI Agent / Evaluation category. Supercharge Your LLM Application Evaluations"
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Ragas using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Ragas code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Ragas requires evidence-backed assessment.
...
- First-party source: https://github.com/vibrantlabsai/ragas
...
- [ ] Trace Ragas inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Ragas defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Ragas least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Ragas resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Ragas release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Ragas dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Ragas identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Ragas change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Ragas documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Ragas architecture and dependency summary with trust boundaries and persistent state.
- Include Ragas validation commands or tests that are safe for the stated environment.
...
- Ragas identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Ragas failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Ragas is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Ragas as the basis for migration or production guidance.
