---
name: "cncf-bloombase-reviewer"
description: "Review Bloombase using its official documentation and repository in the Provisioning / Security & Compliance category. Bloombase is an intelligent storage firewall company providing AI-accelerated security and PQC encryption of high-bandwidth, low-latency data cloud from edge computing, physical/virtual datacenters, through HCI and CDI, to the cloud."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Bloombase Reviewer

## Purpose

Review and operate Bloombase using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Bloombase code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Bloombase requires evidence-backed assessment.
...
- First-party source: https://www.bloombase.com
...
- [ ] Trace Bloombase inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Bloombase defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Bloombase least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Bloombase resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Bloombase release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Bloombase dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Bloombase identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Bloombase change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Bloombase documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Bloombase architecture and dependency summary with trust boundaries and persistent state.
- Include Bloombase validation commands or tests that are safe for the stated environment.
...
- Bloombase identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Bloombase failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Bloombase is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Bloombase as the basis for migration or production guidance.
