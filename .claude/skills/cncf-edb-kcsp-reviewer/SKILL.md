...
---
# Cncf Edb Kcsp Reviewer

## Purpose

Review and operate EDB (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- EDB (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving EDB (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.enterprisedb.com/products/edb-postgres-cloudnativepg
...
- [ ] Trace EDB (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check EDB (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check EDB (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check EDB (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check EDB (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check EDB (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If EDB (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a EDB (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- EDB (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a EDB (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include EDB (KCSP) validation commands or tests that are safe for the stated environment.
...
- EDB (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- EDB (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that EDB (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of EDB (KCSP) as the basis for migration or production guidance.
