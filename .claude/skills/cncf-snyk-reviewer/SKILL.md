---
name: "cncf-snyk-reviewer"
description: "Review Snyk using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Snyk using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Snyk code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Snyk requires evidence-backed assessment.
...
- [ ] Trace Snyk inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Snyk defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Snyk least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Snyk resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Snyk release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Snyk dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Snyk identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Snyk change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Snyk documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Snyk architecture and dependency summary with trust boundaries and persistent state.
- Include Snyk validation commands or tests that are safe for the stated environment.
...
- Snyk identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Snyk failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Snyk is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Snyk as the basis for migration or production guidance.
