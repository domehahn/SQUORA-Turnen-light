---
name: "cncf-kuberhealthy-reviewer"
description: "Review Kuberhealthy using its official documentation and repository in the Observability and Analysis / Observability category. A Kubernetes operator for running synthetic checks as pods. Works great with Prometheus!"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kuberhealthy Reviewer

## Purpose

Review and operate Kuberhealthy using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kuberhealthy code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kuberhealthy requires evidence-backed assessment.
...
- First-party source: https://github.com/kuberhealthy/kuberhealthy
...
- [ ] Trace Kuberhealthy inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kuberhealthy defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kuberhealthy least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kuberhealthy resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kuberhealthy release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kuberhealthy dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kuberhealthy identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kuberhealthy change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kuberhealthy documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kuberhealthy architecture and dependency summary with trust boundaries and persistent state.
- Include Kuberhealthy validation commands or tests that are safe for the stated environment.
...
- Kuberhealthy identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kuberhealthy failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kuberhealthy is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kuberhealthy as the basis for migration or production guidance.
