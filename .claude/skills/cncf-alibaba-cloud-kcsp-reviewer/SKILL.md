---
name: "cncf-alibaba-cloud-kcsp-reviewer"
description: "Review Alibaba Cloud (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Alibaba Cloud Container Service for Kubernetes is a fully-managed service compatible with Kubernetes to help users focus on their  applications rather than managing container infrastructure."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Alibaba Cloud Kcsp Reviewer

## Purpose

Review and operate Alibaba Cloud (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Alibaba Cloud (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Alibaba Cloud (KCSP) requires evidence-backed assessment.
...
- [ ] Trace Alibaba Cloud (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Alibaba Cloud (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Alibaba Cloud (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Alibaba Cloud (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Alibaba Cloud (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Alibaba Cloud (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Alibaba Cloud (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Alibaba Cloud (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Alibaba Cloud (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Alibaba Cloud (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Alibaba Cloud (KCSP) validation commands or tests that are safe for the stated environment.
...
- Alibaba Cloud (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Alibaba Cloud (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Alibaba Cloud (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Alibaba Cloud (KCSP) as the basis for migration or production guidance.
