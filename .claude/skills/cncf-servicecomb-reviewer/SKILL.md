---
name: "cncf-servicecomb-reviewer"
description: "Review ServiceComb using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Servicecomb Reviewer

## Purpose

Review and operate ServiceComb using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ServiceComb code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ServiceComb requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/servicecomb-java-chassis
...
- [ ] Trace ServiceComb inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ServiceComb defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ServiceComb least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ServiceComb resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ServiceComb release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ServiceComb dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ServiceComb identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ServiceComb change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ServiceComb documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ServiceComb architecture and dependency summary with trust boundaries and persistent state.
- Include ServiceComb validation commands or tests that are safe for the stated environment.
...
- ServiceComb identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ServiceComb failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ServiceComb is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ServiceComb as the basis for migration or production guidance.
