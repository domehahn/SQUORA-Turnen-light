...
---
# Cncf Spectro Cloud Kcsp Reviewer

## Purpose

Review and operate Spectro Cloud (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Spectro Cloud (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Spectro Cloud (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.spectrocloud.com/services/
...
- [ ] Trace Spectro Cloud (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Spectro Cloud (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Spectro Cloud (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Spectro Cloud (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Spectro Cloud (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Spectro Cloud (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Spectro Cloud (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Spectro Cloud (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Spectro Cloud (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Spectro Cloud (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Spectro Cloud (KCSP) validation commands or tests that are safe for the stated environment.
...
- Spectro Cloud (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Spectro Cloud (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Spectro Cloud (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Spectro Cloud (KCSP) as the basis for migration or production guidance.
