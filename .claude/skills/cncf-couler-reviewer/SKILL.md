---
name: "cncf-couler-reviewer"
description: "Review Couler using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Couler Reviewer

## Purpose

Review and operate Couler using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Couler code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Couler requires evidence-backed assessment.
...
- First-party source: https://github.com/couler-proj/couler
...
- [ ] Trace Couler inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Couler defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Couler least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Couler resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Couler release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Couler dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Couler identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Couler change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Couler documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Couler architecture and dependency summary with trust boundaries and persistent state.
- Include Couler validation commands or tests that are safe for the stated environment.
...
- Couler identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Couler failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Couler is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Couler as the basis for migration or production guidance.
