---
name: "cncf-koordinator-reviewer"
description: "Review Koordinator using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. QoS based scheduling system for hybrid orchestration workloads on Kubernetes, bringing workloads the best layout and status."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Koordinator Reviewer

## Purpose

Review and operate Koordinator using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Koordinator code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Koordinator requires evidence-backed assessment.
...
- First-party source: https://github.com/koordinator-sh/koordinator
...
- [ ] Trace Koordinator inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Koordinator defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Koordinator least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Koordinator resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Koordinator release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Koordinator dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Koordinator identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Koordinator change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Koordinator documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Koordinator architecture and dependency summary with trust boundaries and persistent state.
- Include Koordinator validation commands or tests that are safe for the stated environment.
...
- Koordinator identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Koordinator failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Koordinator is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Koordinator as the basis for migration or production guidance.
