---
name: "cncf-observe-sdk-reviewer"
description: "Review Observe SDK using its official documentation and repository in the Wasm / Debugging & Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Observe Sdk Reviewer

## Purpose

Review and operate Observe SDK using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Observe SDK code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Observe SDK requires evidence-backed assessment.
...
- First-party source: https://github.com/dylibso/observe-sdk
...
- [ ] Trace Observe SDK inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Observe SDK defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Observe SDK least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Observe SDK resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Observe SDK release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Observe SDK dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Observe SDK identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Observe SDK change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Observe SDK documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Observe SDK architecture and dependency summary with trust boundaries and persistent state.
- Include Observe SDK validation commands or tests that are safe for the stated environment.
...
- Observe SDK identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Observe SDK failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Observe SDK is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Observe SDK as the basis for migration or production guidance.
