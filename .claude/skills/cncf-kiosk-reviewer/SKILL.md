---
name: "cncf-kiosk-reviewer"
description: "Review kiosk using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate kiosk using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- kiosk code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving kiosk requires evidence-backed assessment.
...
- First-party source: https://github.com/loft-sh/kiosk
...
- [ ] Trace kiosk inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check kiosk defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check kiosk least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check kiosk resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check kiosk release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check kiosk dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If kiosk identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a kiosk change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- kiosk documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a kiosk architecture and dependency summary with trust boundaries and persistent state.
- Include kiosk validation commands or tests that are safe for the stated environment.
...
- kiosk identity, version, configuration, dependencies, and deployment assumptions are explicit.
- kiosk failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that kiosk is secure, supported, compatible, or suitable.
- Using an unpinned latest version of kiosk as the basis for migration or production guidance.
