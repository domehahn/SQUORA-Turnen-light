---
name: "cncf-metatype-reviewer"
description: "Review Metatype using its official documentation and repository in the Wasm / Application Frameworks category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Metatype Reviewer

## Purpose

Review and operate Metatype using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Metatype code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Metatype requires evidence-backed assessment.
...
- First-party source: https://github.com/metatypedev/metatype
...
- [ ] Trace Metatype inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Metatype defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Metatype least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Metatype resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Metatype release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Metatype dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Metatype identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Metatype change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Metatype documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Metatype architecture and dependency summary with trust boundaries and persistent state.
- Include Metatype validation commands or tests that are safe for the stated environment.
...
- Metatype identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Metatype failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Metatype is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Metatype as the basis for migration or production guidance.
