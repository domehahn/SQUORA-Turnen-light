---
name: "cncf-wa-lang-reviewer"
description: "Review Wa-lang using its official documentation and repository in the Wasm / Languages category. Design for WebAssembly"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Wa Lang Reviewer

## Purpose

Review and operate Wa-lang using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Wa-lang code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Wa-lang requires evidence-backed assessment.
...
- First-party source: https://github.com/wa-lang/wa
...
- [ ] Trace Wa-lang inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Wa-lang defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Wa-lang least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Wa-lang resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Wa-lang release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Wa-lang dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Wa-lang identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Wa-lang change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Wa-lang documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Wa-lang architecture and dependency summary with trust boundaries and persistent state.
- Include Wa-lang validation commands or tests that are safe for the stated environment.
...
- Wa-lang identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Wa-lang failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Wa-lang is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Wa-lang as the basis for migration or production guidance.
