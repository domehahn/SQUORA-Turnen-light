---
name: "cncf-kubevirt-reviewer"
description: "Review KubeVirt using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category. Kubernetes Virtualization API and runtime in order to define and manage virtual machines"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kubevirt Reviewer

## Purpose

Review and operate KubeVirt using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeVirt code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeVirt requires evidence-backed assessment.
...
- First-party source: https://github.com/kubevirt/kubevirt
...
- [ ] Trace KubeVirt inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeVirt defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeVirt least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeVirt resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeVirt release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeVirt dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeVirt identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeVirt change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeVirt documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeVirt architecture and dependency summary with trust boundaries and persistent state.
- Include KubeVirt validation commands or tests that are safe for the stated environment.
...
- KubeVirt identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeVirt failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeVirt is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeVirt as the basis for migration or production guidance.
