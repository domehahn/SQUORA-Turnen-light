...
Review and operate Nirmata Cloud Native Policy Manager using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Nirmata Cloud Native Policy Manager code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Nirmata Cloud Native Policy Manager requires evidence-backed assessment.
...
- [ ] Trace Nirmata Cloud Native Policy Manager inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Nirmata Cloud Native Policy Manager defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Nirmata Cloud Native Policy Manager least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Nirmata Cloud Native Policy Manager resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Nirmata Cloud Native Policy Manager release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Nirmata Cloud Native Policy Manager dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Nirmata Cloud Native Policy Manager identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Nirmata Cloud Native Policy Manager change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Nirmata Cloud Native Policy Manager documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Nirmata Cloud Native Policy Manager architecture and dependency summary with trust boundaries and persistent state.
- Include Nirmata Cloud Native Policy Manager validation commands or tests that are safe for the stated environment.
...
- Nirmata Cloud Native Policy Manager identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Nirmata Cloud Native Policy Manager failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Nirmata Cloud Native Policy Manager is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Nirmata Cloud Native Policy Manager as the basis for migration or production guidance.
