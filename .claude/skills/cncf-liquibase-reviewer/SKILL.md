---
name: "cncf-liquibase-reviewer"
description: "Review Liquibase using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category. Liquibase helps release software faster by bringing DevOps to the database"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Liquibase Reviewer

## Purpose

Review and operate Liquibase using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Liquibase code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Liquibase requires evidence-backed assessment.
...
- First-party source: https://github.com/liquibase/liquibase
...
- [ ] Trace Liquibase inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Liquibase defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Liquibase least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Liquibase resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Liquibase release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Liquibase dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Liquibase identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Liquibase change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Liquibase documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Liquibase architecture and dependency summary with trust boundaries and persistent state.
- Include Liquibase validation commands or tests that are safe for the stated environment.
...
- Liquibase identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Liquibase failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Liquibase is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Liquibase as the basis for migration or production guidance.
