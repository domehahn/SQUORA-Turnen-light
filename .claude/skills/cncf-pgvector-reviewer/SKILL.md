---
name: "cncf-pgvector-reviewer"
description: "Review pgvector using its official documentation and repository in the AI Agent / Vector Database category. Open-source vector similarity search for Postgres"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Pgvector Reviewer

## Purpose

Review and operate pgvector using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- pgvector code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving pgvector requires evidence-backed assessment.
...
- First-party source: https://github.com/pgvector/pgvector
...
- [ ] Trace pgvector inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check pgvector defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check pgvector least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check pgvector resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check pgvector release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check pgvector dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If pgvector identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a pgvector change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- pgvector documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a pgvector architecture and dependency summary with trust boundaries and persistent state.
- Include pgvector validation commands or tests that are safe for the stated environment.
...
- pgvector identity, version, configuration, dependencies, and deployment assumptions are explicit.
- pgvector failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that pgvector is secure, supported, compatible, or suitable.
- Using an unpinned latest version of pgvector as the basis for migration or production guidance.
