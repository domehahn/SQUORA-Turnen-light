---
name: "cncf-kyma-reviewer"
description: "Review Kyma using its official documentation and repository in the Platform / PaaS/Container Service category. Kyma is the opinionated set of Kubernetes based modular building blocks that includes the necessary capabilities to develop and run enterprise-grade cloud-native applications."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Kyma using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kyma code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kyma requires evidence-backed assessment.
...
- First-party source: https://github.com/kyma-project/kyma
...
- [ ] Trace Kyma inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kyma defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kyma least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kyma resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kyma release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kyma dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kyma identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kyma change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kyma documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kyma architecture and dependency summary with trust boundaries and persistent state.
- Include Kyma validation commands or tests that are safe for the stated environment.
...
- Kyma identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kyma failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kyma is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kyma as the basis for migration or production guidance.
