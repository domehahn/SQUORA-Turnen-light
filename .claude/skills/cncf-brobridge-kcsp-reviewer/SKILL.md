---
name: "cncf-brobridge-kcsp-reviewer"
description: "Review Brobridge (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Brobridge provides professional Kubernetes support and microservice-relevant components such as workflow engine, API gateway, queueing system, and orchestration engine to assist your business to move to microservice."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Brobridge Kcsp Reviewer

## Purpose

Review and operate Brobridge (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Brobridge (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Brobridge (KCSP) requires evidence-backed assessment.
...
- [ ] Trace Brobridge (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Brobridge (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Brobridge (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Brobridge (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Brobridge (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Brobridge (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Brobridge (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Brobridge (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Brobridge (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Brobridge (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Brobridge (KCSP) validation commands or tests that are safe for the stated environment.
...
- Brobridge (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Brobridge (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Brobridge (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Brobridge (KCSP) as the basis for migration or production guidance.
