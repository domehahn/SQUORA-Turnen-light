...
---
# Cncf Dg I Kubernetes Platform Reviewer

## Purpose

Review and operate DG-i Kubernetes Platform using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DG-i Kubernetes Platform code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DG-i Kubernetes Platform requires evidence-backed assessment.
...
- First-party source: https://www.dg-i.net/hosting/kubernetes/
...
- [ ] Trace DG-i Kubernetes Platform inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DG-i Kubernetes Platform defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DG-i Kubernetes Platform least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DG-i Kubernetes Platform resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DG-i Kubernetes Platform release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DG-i Kubernetes Platform dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DG-i Kubernetes Platform identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DG-i Kubernetes Platform change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DG-i Kubernetes Platform documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DG-i Kubernetes Platform architecture and dependency summary with trust boundaries and persistent state.
- Include DG-i Kubernetes Platform validation commands or tests that are safe for the stated environment.
...
- DG-i Kubernetes Platform identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DG-i Kubernetes Platform failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DG-i Kubernetes Platform is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DG-i Kubernetes Platform as the basis for migration or production guidance.
