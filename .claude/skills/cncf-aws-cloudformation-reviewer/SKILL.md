---
name: "cncf-aws-cloudformation-reviewer"
description: "Review AWS CloudFormation using its official documentation and repository in the Provisioning / Automation & Configuration category. AWS CloudFormation provides a common language for you to describe and provision all the infrastructure resources in your cloud environment."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Aws Cloudformation Reviewer

## Purpose

Review and operate AWS CloudFormation using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- AWS CloudFormation code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving AWS CloudFormation requires evidence-backed assessment.
...
- First-party source: https://aws.amazon.com/cloudformation/
...
- [ ] Trace AWS CloudFormation inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check AWS CloudFormation defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check AWS CloudFormation least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check AWS CloudFormation resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check AWS CloudFormation release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check AWS CloudFormation dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If AWS CloudFormation identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a AWS CloudFormation change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- AWS CloudFormation documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a AWS CloudFormation architecture and dependency summary with trust boundaries and persistent state.
- Include AWS CloudFormation validation commands or tests that are safe for the stated environment.
...
- AWS CloudFormation identity, version, configuration, dependencies, and deployment assumptions are explicit.
- AWS CloudFormation failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that AWS CloudFormation is secure, supported, compatible, or suitable.
- Using an unpinned latest version of AWS CloudFormation as the basis for migration or production guidance.
