---
name: "cncf-container-storage-interface-csi-reviewer"
description: "Review Container Storage Interface (CSI) using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Container Storage Interface Csi Reviewer

## Purpose

Review and operate Container Storage Interface (CSI) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Container Storage Interface (CSI) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Container Storage Interface (CSI) requires evidence-backed assessment.
...
- First-party source: https://github.com/container-storage-interface/spec
...
- [ ] Trace Container Storage Interface (CSI) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Container Storage Interface (CSI) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Container Storage Interface (CSI) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Container Storage Interface (CSI) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Container Storage Interface (CSI) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Container Storage Interface (CSI) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Container Storage Interface (CSI) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Container Storage Interface (CSI) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Container Storage Interface (CSI) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Container Storage Interface (CSI) architecture and dependency summary with trust boundaries and persistent state.
- Include Container Storage Interface (CSI) validation commands or tests that are safe for the stated environment.
...
- Container Storage Interface (CSI) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Container Storage Interface (CSI) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Container Storage Interface (CSI) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Container Storage Interface (CSI) as the basis for migration or production guidance.
