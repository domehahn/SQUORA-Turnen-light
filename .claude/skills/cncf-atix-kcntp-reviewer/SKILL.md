---
name: "cncf-atix-kcntp-reviewer"
description: "Review ATIX (KCNTP) using its official documentation and repository in the Special / Kubernetes and Cloud Native Training Partner category. ATIX plans, builds & runs Kubernetes clusters. We provide operational support and teach companies how to profit from using Kubernetes."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Atix Kcntp Reviewer

## Purpose

Review and operate ATIX (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ATIX (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ATIX (KCNTP) requires evidence-backed assessment.
...
- First-party source: https://atix.de/en/trainings/kubernetes-training/
...
- [ ] Trace ATIX (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ATIX (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ATIX (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ATIX (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ATIX (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ATIX (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ATIX (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ATIX (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ATIX (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ATIX (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include ATIX (KCNTP) validation commands or tests that are safe for the stated environment.
...
- ATIX (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ATIX (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ATIX (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ATIX (KCNTP) as the basis for migration or production guidance.
