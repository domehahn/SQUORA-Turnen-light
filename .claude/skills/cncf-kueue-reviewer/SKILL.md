---
name: "cncf-kueue-reviewer"
description: "Review Kueue using its official documentation and repository in the AI Native Infra / Orchestration and Scheduling category. Cloud-native job queueing system for batch, HPC, AI/ML, and similar applications in a Kubernetes cluster."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Kueue using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kueue code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kueue requires evidence-backed assessment.
...
- First-party source: https://github.com/kubernetes-sigs/kueue
...
- [ ] Trace Kueue inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kueue defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kueue least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kueue resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kueue release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kueue dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kueue identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kueue change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kueue documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kueue architecture and dependency summary with trust boundaries and persistent state.
- Include Kueue validation commands or tests that are safe for the stated environment.
...
- Kueue identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kueue failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kueue is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kueue as the basis for migration or production guidance.
