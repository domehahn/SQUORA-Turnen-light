---
name: "cncf-nuage-networks-reviewer"
description: "Review Nuage Networks using its official documentation and repository in the Runtime / Cloud Native Network category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Nuage Networks Reviewer

## Purpose

Review and operate Nuage Networks using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Nuage Networks code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Nuage Networks requires evidence-backed assessment.
...
- First-party source: https://www.nuagenetworks.net/
...
- [ ] Trace Nuage Networks inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Nuage Networks defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Nuage Networks least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Nuage Networks resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Nuage Networks release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Nuage Networks dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Nuage Networks identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Nuage Networks change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Nuage Networks documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Nuage Networks architecture and dependency summary with trust boundaries and persistent state.
- Include Nuage Networks validation commands or tests that are safe for the stated environment.
...
- Nuage Networks identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Nuage Networks failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Nuage Networks is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Nuage Networks as the basis for migration or production guidance.
