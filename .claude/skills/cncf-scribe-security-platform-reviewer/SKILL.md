---
name: "cncf-scribe-security-platform-reviewer"
description: "Review Scribe Security Platform using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Scribe Security Platform Reviewer

## Purpose

Review and operate Scribe Security Platform using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Scribe Security Platform code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Scribe Security Platform requires evidence-backed assessment.
...
- First-party source: https://scribesecurity.com
...
- [ ] Trace Scribe Security Platform inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Scribe Security Platform defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Scribe Security Platform least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Scribe Security Platform resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Scribe Security Platform release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Scribe Security Platform dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Scribe Security Platform identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Scribe Security Platform change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Scribe Security Platform documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Scribe Security Platform architecture and dependency summary with trust boundaries and persistent state.
- Include Scribe Security Platform validation commands or tests that are safe for the stated environment.
...
- Scribe Security Platform identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Scribe Security Platform failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Scribe Security Platform is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Scribe Security Platform as the basis for migration or production guidance.
