---
name: "cncf-spot-io-reviewer"
description: "Review Spot.io using its official documentation and repository in the Observability and Analysis / Continuous Optimization category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Spot Io Reviewer

## Purpose

Review and operate Spot.io using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Spot.io code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Spot.io requires evidence-backed assessment.
...
- First-party source: https://spot.io/
...
- [ ] Trace Spot.io inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Spot.io defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Spot.io least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Spot.io resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Spot.io release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Spot.io dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Spot.io identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Spot.io change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Spot.io documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Spot.io architecture and dependency summary with trust boundaries and persistent state.
- Include Spot.io validation commands or tests that are safe for the stated environment.
...
- Spot.io identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Spot.io failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Spot.io is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Spot.io as the basis for migration or production guidance.
