---
name: "cncf-cdk-for-kubernetes-cdk8s-reviewer"
description: "Review CDK for Kubernetes (CDK8s) using its official documentation and repository in the Provisioning / Automation & Configuration category. CDK8s lets you define Kubernetes apps and components using familiar programming languages and object-oriented APIs."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Cdk For Kubernetes Cdk8s Reviewer

## Purpose

Review and operate CDK for Kubernetes (CDK8s) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CDK for Kubernetes (CDK8s) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CDK for Kubernetes (CDK8s) requires evidence-backed assessment.
...
- CNCF classification: Provisioning / Automation & Configuration
- CNCF project maturity field: sandbox
- First-party source: https://github.com/cdk8s-team/cdk8s
...
- [ ] Trace CDK for Kubernetes (CDK8s) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CDK for Kubernetes (CDK8s) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CDK for Kubernetes (CDK8s) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CDK for Kubernetes (CDK8s) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CDK for Kubernetes (CDK8s) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CDK for Kubernetes (CDK8s) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CDK for Kubernetes (CDK8s) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CDK for Kubernetes (CDK8s) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CDK for Kubernetes (CDK8s) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CDK for Kubernetes (CDK8s) architecture and dependency summary with trust boundaries and persistent state.
- Include CDK for Kubernetes (CDK8s) validation commands or tests that are safe for the stated environment.
...
- CDK for Kubernetes (CDK8s) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CDK for Kubernetes (CDK8s) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CDK for Kubernetes (CDK8s) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CDK for Kubernetes (CDK8s) as the basis for migration or production guidance.
