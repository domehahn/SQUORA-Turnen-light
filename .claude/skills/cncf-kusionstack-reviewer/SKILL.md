---
name: "cncf-kusionstack-reviewer"
description: "Review KusionStack using its official documentation and repository in the Provisioning / Automation & Configuration category. Declarative Intent Driven Platform Orchestrator for Internal Developer Platform (IDP)"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kusionstack Reviewer

## Purpose

Review and operate KusionStack using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KusionStack code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KusionStack requires evidence-backed assessment.
...
- First-party source: https://github.com/KusionStack/kusion
...
- [ ] Trace KusionStack inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KusionStack defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KusionStack least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KusionStack resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KusionStack release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KusionStack dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KusionStack identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KusionStack change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KusionStack documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KusionStack architecture and dependency summary with trust boundaries and persistent state.
- Include KusionStack validation commands or tests that are safe for the stated environment.
...
- KusionStack identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KusionStack failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KusionStack is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KusionStack as the basis for migration or production guidance.
