---
name: "cncf-zettaset-reviewer"
description: "Review Zettaset using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Zettaset Reviewer

## Purpose

Review and operate Zettaset using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Zettaset code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Zettaset requires evidence-backed assessment.
...
- First-party source: https://www.zettaset.com/
...
- [ ] Trace Zettaset inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Zettaset defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Zettaset least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Zettaset resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Zettaset release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Zettaset dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Zettaset identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Zettaset change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Zettaset documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Zettaset architecture and dependency summary with trust boundaries and persistent state.
- Include Zettaset validation commands or tests that are safe for the stated environment.
...
- Zettaset identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Zettaset failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Zettaset is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Zettaset as the basis for migration or production guidance.
