---
name: "cncf-langgraph-reviewer"
description: "Review LangGraph using its official documentation and repository in the AI Agent / Agent Framework category. Build resilient language agents as graphs."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Langgraph Reviewer

## Purpose

Review and operate LangGraph using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- LangGraph code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving LangGraph requires evidence-backed assessment.
...
- First-party source: https://github.com/langchain-ai/langgraph
...
- [ ] Trace LangGraph inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check LangGraph defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check LangGraph least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check LangGraph resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check LangGraph release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check LangGraph dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If LangGraph identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a LangGraph change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- LangGraph documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a LangGraph architecture and dependency summary with trust boundaries and persistent state.
- Include LangGraph validation commands or tests that are safe for the stated environment.
...
- LangGraph identity, version, configuration, dependencies, and deployment assumptions are explicit.
- LangGraph failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that LangGraph is secure, supported, compatible, or suitable.
- Using an unpinned latest version of LangGraph as the basis for migration or production guidance.
