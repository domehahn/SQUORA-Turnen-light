---
name: "cncf-eos-reviewer"
description: "Review EOS using its official documentation and repository in the Wasm / Decentralized Platforms category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate EOS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- EOS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving EOS requires evidence-backed assessment.
...
- First-party source: https://eos.io/
...
- [ ] Trace EOS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check EOS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check EOS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check EOS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check EOS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check EOS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If EOS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a EOS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- EOS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a EOS architecture and dependency summary with trust boundaries and persistent state.
- Include EOS validation commands or tests that are safe for the stated environment.
...
- EOS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- EOS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that EOS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of EOS as the basis for migration or production guidance.
