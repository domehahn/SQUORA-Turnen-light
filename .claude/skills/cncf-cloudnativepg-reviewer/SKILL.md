---
name: "cncf-cloudnativepg-reviewer"
description: "Review CloudNativePG using its official documentation and repository in the App Definition and Development / Database category. The most popular Kubernetes Operator for PostgreSQL."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Cloudnativepg Reviewer

## Purpose

Review and operate CloudNativePG using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CloudNativePG code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CloudNativePG requires evidence-backed assessment.
...
- First-party source: https://github.com/cloudnative-pg/cloudnative-pg
...
- [ ] Trace CloudNativePG inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CloudNativePG defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CloudNativePG least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CloudNativePG resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CloudNativePG release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CloudNativePG dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CloudNativePG identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CloudNativePG change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CloudNativePG documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CloudNativePG architecture and dependency summary with trust boundaries and persistent state.
- Include CloudNativePG validation commands or tests that are safe for the stated environment.
...
- CloudNativePG identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CloudNativePG failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CloudNativePG is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CloudNativePG as the basis for migration or production guidance.
