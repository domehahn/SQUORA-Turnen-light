...
---
# Cncf Edge Delta Reviewer

## Purpose

Review and operate Edge Delta using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Edge Delta code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Edge Delta requires evidence-backed assessment.
...
- First-party source: https://edgedelta.com/
...
- [ ] Trace Edge Delta inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Edge Delta defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Edge Delta least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Edge Delta resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Edge Delta release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Edge Delta dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Edge Delta identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Edge Delta change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Edge Delta documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Edge Delta architecture and dependency summary with trust boundaries and persistent state.
- Include Edge Delta validation commands or tests that are safe for the stated environment.
...
- Edge Delta identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Edge Delta failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Edge Delta is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Edge Delta as the basis for migration or production guidance.
