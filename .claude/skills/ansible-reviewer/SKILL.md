...
description: "Review Ansible inventories, roles, playbooks, idempotency, secrets, privilege escalation, testing, and rollout safety."
version: "1.0.0"
since: "2026-08-27"
...
- Source code, configuration, dependencies, tests, deployment artifacts, or runtime behavior in Ansible automation change.
...
- inventories, roles, playbooks, idempotency, Ansible Vault, Molecule
...
- [ ] Trace Ansible automation inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Ansible automation defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Ansible automation least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Ansible automation resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Ansible automation release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
...
- If Ansible automation identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Ansible automation change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Ansible automation documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Ansible automation architecture and dependency summary with trust boundaries and persistent state.
- Include Ansible automation validation commands or tests that are safe for the stated environment.
...
- Ansible automation identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Ansible automation failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Ansible automation is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Ansible automation as the basis for migration or production guidance.
