---
name: "cncf-oteemo-kcsp-reviewer"
description: "Review Oteemo (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Oteemo is a digital product engineering firm delivering Agentic AI solutions through human-centered design. Our services include operationalization of Kubernetes platforms, application container migrations and development on cloud native architectures."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Oteemo Kcsp Reviewer

## Purpose

Review and operate Oteemo (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Oteemo (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Oteemo (KCSP) requires evidence-backed assessment.
...
- [ ] Trace Oteemo (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Oteemo (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Oteemo (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Oteemo (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Oteemo (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Oteemo (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Oteemo (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Oteemo (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Oteemo (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Oteemo (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Oteemo (KCSP) validation commands or tests that are safe for the stated environment.
...
- Oteemo (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Oteemo (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Oteemo (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Oteemo (KCSP) as the basis for migration or production guidance.
