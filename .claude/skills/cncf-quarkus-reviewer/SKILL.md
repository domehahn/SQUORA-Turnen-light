...
---
# Cncf Quarkus Reviewer

## Purpose

Review and operate Quarkus using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Quarkus code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Quarkus requires evidence-backed assessment.
...
- First-party source: https://github.com/quarkusio/quarkus
...
- [ ] Trace Quarkus inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Quarkus defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Quarkus least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Quarkus resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Quarkus release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Quarkus dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Quarkus identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Quarkus change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Quarkus documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Quarkus architecture and dependency summary with trust boundaries and persistent state.
- Include Quarkus validation commands or tests that are safe for the stated environment.
...
- Quarkus identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Quarkus failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Quarkus is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Quarkus as the basis for migration or production guidance.
