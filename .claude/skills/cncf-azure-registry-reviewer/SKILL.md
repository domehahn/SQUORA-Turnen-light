...
---
# Cncf Azure Registry Reviewer

## Purpose

Review and operate Azure Registry using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Azure Registry code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Azure Registry requires evidence-backed assessment.
...
- First-party source: https://azure.microsoft.com/en-us/services/container-registry/
...
- [ ] Trace Azure Registry inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Azure Registry defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Azure Registry least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Azure Registry resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Azure Registry release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Azure Registry dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Azure Registry identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Azure Registry change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Azure Registry documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Azure Registry architecture and dependency summary with trust boundaries and persistent state.
- Include Azure Registry validation commands or tests that are safe for the stated environment.
...
- Azure Registry identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Azure Registry failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Azure Registry is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Azure Registry as the basis for migration or production guidance.
