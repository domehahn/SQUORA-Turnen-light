---
name: "cncf-kubemq-reviewer"
description: "Review KubeMQ using its official documentation and repository in the App Definition and Development / Streaming & Messaging category. KubeMQ is a Kubernetes Message Queue Broker"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kubemq Reviewer

## Purpose

Review and operate KubeMQ using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeMQ code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeMQ requires evidence-backed assessment.
...
- First-party source: https://github.com/kubemq-io/kubemq-community
...
- [ ] Trace KubeMQ inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeMQ defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeMQ least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeMQ resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeMQ release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeMQ dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeMQ identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeMQ change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeMQ documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeMQ architecture and dependency summary with trust boundaries and persistent state.
- Include KubeMQ validation commands or tests that are safe for the stated environment.
...
- KubeMQ identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeMQ failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeMQ is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeMQ as the basis for migration or production guidance.
