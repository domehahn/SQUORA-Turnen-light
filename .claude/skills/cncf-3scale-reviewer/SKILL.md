---
name: "cncf-3scale-reviewer"
description: "Review 3Scale using its official documentation and repository in the Orchestration & Management / API Gateway category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf 3scale Reviewer

## Purpose

Review and operate 3Scale using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- 3Scale code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving 3Scale requires evidence-backed assessment.
...
- First-party source: https://github.com/3scale/apicast
...
- [ ] Trace 3Scale inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check 3Scale defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check 3Scale least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check 3Scale resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check 3Scale release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check 3Scale dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If 3Scale identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a 3Scale change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- 3Scale documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a 3Scale architecture and dependency summary with trust boundaries and persistent state.
- Include 3Scale validation commands or tests that are safe for the stated environment.
...
- 3Scale identity, version, configuration, dependencies, and deployment assumptions are explicit.
- 3Scale failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that 3Scale is secure, supported, compatible, or suitable.
- Using an unpinned latest version of 3Scale as the basis for migration or production guidance.
