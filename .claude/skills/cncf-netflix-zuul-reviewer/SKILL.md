---
name: "cncf-netflix-zuul-reviewer"
description: "Review Netflix Zuul using its official documentation and repository in the Orchestration & Management / Service Proxy category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Netflix Zuul Reviewer

## Purpose

Review and operate Netflix Zuul using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Netflix Zuul code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Netflix Zuul requires evidence-backed assessment.
...
- First-party source: https://github.com/Netflix/zuul
...
- [ ] Trace Netflix Zuul inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Netflix Zuul defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Netflix Zuul least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Netflix Zuul resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Netflix Zuul release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Netflix Zuul dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Netflix Zuul identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Netflix Zuul change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Netflix Zuul documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Netflix Zuul architecture and dependency summary with trust boundaries and persistent state.
- Include Netflix Zuul validation commands or tests that are safe for the stated environment.
...
- Netflix Zuul identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Netflix Zuul failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Netflix Zuul is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Netflix Zuul as the basis for migration or production guidance.
