...
---
# Cncf Volcano Engine Apmplus Reviewer

## Purpose

Review and operate Volcano Engine APMPlus using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Volcano Engine APMPlus code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Volcano Engine APMPlus requires evidence-backed assessment.
...
- First-party source: https://www.volcengine.com/product/apmplus
...
- [ ] Trace Volcano Engine APMPlus inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Volcano Engine APMPlus defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Volcano Engine APMPlus least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Volcano Engine APMPlus resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Volcano Engine APMPlus release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Volcano Engine APMPlus dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Volcano Engine APMPlus identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Volcano Engine APMPlus change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Volcano Engine APMPlus documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Volcano Engine APMPlus architecture and dependency summary with trust boundaries and persistent state.
- Include Volcano Engine APMPlus validation commands or tests that are safe for the stated environment.
...
- Volcano Engine APMPlus identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Volcano Engine APMPlus failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Volcano Engine APMPlus is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Volcano Engine APMPlus as the basis for migration or production guidance.
