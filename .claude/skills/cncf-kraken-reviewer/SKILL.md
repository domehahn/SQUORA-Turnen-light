---
name: "cncf-kraken-reviewer"
description: "Review Kraken using its official documentation and repository in the Provisioning / Container Registry category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kraken Reviewer

## Purpose

Review and operate Kraken using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kraken code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kraken requires evidence-backed assessment.
...
- First-party source: https://github.com/uber/kraken
...
- [ ] Trace Kraken inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kraken defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kraken least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kraken resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kraken release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kraken dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kraken identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kraken change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kraken documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kraken architecture and dependency summary with trust boundaries and persistent state.
- Include Kraken validation commands or tests that are safe for the stated environment.
...
- Kraken identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kraken failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kraken is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kraken as the basis for migration or production guidance.
