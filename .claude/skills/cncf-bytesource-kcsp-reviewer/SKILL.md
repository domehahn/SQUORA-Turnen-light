---
name: "cncf-bytesource-kcsp-reviewer"
description: "Review ByteSource (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. One Stop Shop Professional Services for everything Kubernetes"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Bytesource Kcsp Reviewer

## Purpose

Review and operate ByteSource (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ByteSource (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ByteSource (KCSP) requires evidence-backed assessment.
...
- First-party source: https://bytesource.net/en/kubernetes/
...
- [ ] Trace ByteSource (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ByteSource (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ByteSource (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ByteSource (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ByteSource (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ByteSource (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ByteSource (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ByteSource (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ByteSource (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ByteSource (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include ByteSource (KCSP) validation commands or tests that are safe for the stated environment.
...
- ByteSource (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ByteSource (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ByteSource (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ByteSource (KCSP) as the basis for migration or production guidance.
