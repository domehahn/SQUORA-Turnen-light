...
---
# Cncf Google Cloud Build Reviewer

## Purpose

Review and operate Google Cloud Build using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Google Cloud Build code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Google Cloud Build requires evidence-backed assessment.
...
- First-party source: https://cloud.google.com/build
...
- [ ] Trace Google Cloud Build inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Google Cloud Build defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Google Cloud Build least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Google Cloud Build resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Google Cloud Build release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Google Cloud Build dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Google Cloud Build identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Google Cloud Build change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Google Cloud Build documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Google Cloud Build architecture and dependency summary with trust boundaries and persistent state.
- Include Google Cloud Build validation commands or tests that are safe for the stated environment.
...
- Google Cloud Build identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Google Cloud Build failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Google Cloud Build is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Google Cloud Build as the basis for migration or production guidance.
