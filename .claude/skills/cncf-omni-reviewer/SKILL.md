---
name: "cncf-omni-reviewer"
description: "Review Omni using its official documentation and repository in the Provisioning / Automation & Configuration category. Omni is a Kubernetes orchestrator for bare metal and edge environments. It manages full OS and Kubernetes lifecycles with Talos Linux and remote management with wireguard communication."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Omni using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Omni code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Omni requires evidence-backed assessment.
...
- First-party source: https://github.com/siderolabs/omni
...
- [ ] Trace Omni inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Omni defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Omni least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Omni resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Omni release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Omni dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Omni identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Omni change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Omni documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Omni architecture and dependency summary with trust boundaries and persistent state.
- Include Omni validation commands or tests that are safe for the stated environment.
...
- Omni identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Omni failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Omni is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Omni as the basis for migration or production guidance.
