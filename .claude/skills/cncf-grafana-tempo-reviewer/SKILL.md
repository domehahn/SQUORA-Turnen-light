...
---
# Cncf Grafana Tempo Reviewer

## Purpose

Review and operate Grafana Tempo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Grafana Tempo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Grafana Tempo requires evidence-backed assessment.
...
- First-party source: https://github.com/grafana/tempo
...
- [ ] Trace Grafana Tempo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Grafana Tempo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Grafana Tempo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Grafana Tempo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Grafana Tempo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Grafana Tempo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Grafana Tempo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Grafana Tempo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Grafana Tempo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Grafana Tempo architecture and dependency summary with trust boundaries and persistent state.
- Include Grafana Tempo validation commands or tests that are safe for the stated environment.
...
- Grafana Tempo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Grafana Tempo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Grafana Tempo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Grafana Tempo as the basis for migration or production guidance.
