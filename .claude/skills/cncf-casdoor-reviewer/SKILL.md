---
name: "cncf-casdoor-reviewer"
description: "Review Casdoor using its official documentation and repository in the Provisioning / Security & Compliance category. Casdoor is an open-source UI-first identity access management (IAM) / Single-Sign-On (SSO) platform with web UI supporting OAuth 2.0, OIDC, SAML and CAS."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Casdoor Reviewer

## Purpose

Review and operate Casdoor using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Casdoor code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Casdoor requires evidence-backed assessment.
...
- First-party source: https://github.com/casdoor/casdoor
...
- [ ] Trace Casdoor inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Casdoor defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Casdoor least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Casdoor resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Casdoor release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Casdoor dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Casdoor identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Casdoor change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Casdoor documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Casdoor architecture and dependency summary with trust boundaries and persistent state.
- Include Casdoor validation commands or tests that are safe for the stated environment.
...
- Casdoor identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Casdoor failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Casdoor is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Casdoor as the basis for migration or production guidance.
