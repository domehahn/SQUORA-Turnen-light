---
name: "cncf-stunner-reviewer"
description: "Review STUNner using its official documentation and repository in the Orchestration & Management / Service Proxy category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Stunner Reviewer

## Purpose

Review and operate STUNner using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- STUNner code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving STUNner requires evidence-backed assessment.
...
- First-party source: https://github.com/l7mp/stunner
...
- [ ] Trace STUNner inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check STUNner defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check STUNner least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check STUNner resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check STUNner release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check STUNner dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If STUNner identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a STUNner change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- STUNner documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a STUNner architecture and dependency summary with trust boundaries and persistent state.
- Include STUNner validation commands or tests that are safe for the stated environment.
...
- STUNner identity, version, configuration, dependencies, and deployment assumptions are explicit.
- STUNner failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that STUNner is secure, supported, compatible, or suitable.
- Using an unpinned latest version of STUNner as the basis for migration or production guidance.
