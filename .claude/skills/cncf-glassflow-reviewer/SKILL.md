---
name: "cncf-glassflow-reviewer"
description: "Review GlassFlow using its official documentation and repository in the App Definition and Development / Streaming & Messaging category. Open-source streaming ETL for ClickHouse with deduplication, joins, and transforms over Kafka and other sources."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Glassflow Reviewer

## Purpose

Review and operate GlassFlow using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- GlassFlow code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving GlassFlow requires evidence-backed assessment.
...
- First-party source: https://github.com/glassflow/clickhouse-etl
...
- [ ] Trace GlassFlow inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check GlassFlow defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check GlassFlow least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check GlassFlow resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check GlassFlow release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check GlassFlow dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If GlassFlow identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a GlassFlow change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- GlassFlow documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a GlassFlow architecture and dependency summary with trust boundaries and persistent state.
- Include GlassFlow validation commands or tests that are safe for the stated environment.
...
- GlassFlow identity, version, configuration, dependencies, and deployment assumptions are explicit.
- GlassFlow failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that GlassFlow is secure, supported, compatible, or suitable.
- Using an unpinned latest version of GlassFlow as the basis for migration or production guidance.
