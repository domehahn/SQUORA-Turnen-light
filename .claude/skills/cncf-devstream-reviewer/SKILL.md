---
name: "cncf-devstream-reviewer"
description: "Review DevStream using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Devstream Reviewer

## Purpose

Review and operate DevStream using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DevStream code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DevStream requires evidence-backed assessment.
...
- First-party source: https://github.com/devstream-io/devstream
...
- [ ] Trace DevStream inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DevStream defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DevStream least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DevStream resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DevStream release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DevStream dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DevStream identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DevStream change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DevStream documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DevStream architecture and dependency summary with trust boundaries and persistent state.
- Include DevStream validation commands or tests that are safe for the stated environment.
...
- DevStream identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DevStream failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DevStream is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DevStream as the basis for migration or production guidance.
