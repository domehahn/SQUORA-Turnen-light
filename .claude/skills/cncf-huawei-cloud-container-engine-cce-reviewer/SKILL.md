...
---
# Cncf Huawei Cloud Container Engine Cce Reviewer

## Purpose

Review and operate Huawei Cloud Container Engine (CCE) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Huawei Cloud Container Engine (CCE) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Huawei Cloud Container Engine (CCE) requires evidence-backed assessment.
...
- [ ] Trace Huawei Cloud Container Engine (CCE) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Huawei Cloud Container Engine (CCE) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Huawei Cloud Container Engine (CCE) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Huawei Cloud Container Engine (CCE) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Huawei Cloud Container Engine (CCE) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Huawei Cloud Container Engine (CCE) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Huawei Cloud Container Engine (CCE) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Huawei Cloud Container Engine (CCE) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Huawei Cloud Container Engine (CCE) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Huawei Cloud Container Engine (CCE) architecture and dependency summary with trust boundaries and persistent state.
- Include Huawei Cloud Container Engine (CCE) validation commands or tests that are safe for the stated environment.
...
- Huawei Cloud Container Engine (CCE) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Huawei Cloud Container Engine (CCE) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Huawei Cloud Container Engine (CCE) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Huawei Cloud Container Engine (CCE) as the basis for migration or production guidance.
