---
name: "cncf-llama-cpp-reviewer"
description: "Review llama.cpp using its official documentation and repository in the Inference / Runtime category. LLM inference in C/C++"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Llama Cpp Reviewer

## Purpose

Review and operate llama.cpp using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- llama.cpp code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving llama.cpp requires evidence-backed assessment.
...
- First-party source: https://github.com/ggml-org/llama.cpp
...
- [ ] Trace llama.cpp inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check llama.cpp defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check llama.cpp least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check llama.cpp resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check llama.cpp release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check llama.cpp dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If llama.cpp identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a llama.cpp change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- llama.cpp documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a llama.cpp architecture and dependency summary with trust boundaries and persistent state.
- Include llama.cpp validation commands or tests that are safe for the stated environment.
...
- llama.cpp identity, version, configuration, dependencies, and deployment assumptions are explicit.
- llama.cpp failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that llama.cpp is secure, supported, compatible, or suitable.
- Using an unpinned latest version of llama.cpp as the basis for migration or production guidance.
