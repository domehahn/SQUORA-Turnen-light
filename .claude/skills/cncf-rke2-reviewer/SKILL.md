---
name: "cncf-rke2-reviewer"
description: "Review RKE2 using its official documentation and repository in the Platform / Certified Kubernetes - Distribution category. A Kubernetes distribution focused on enabling Federal government compliance-based use cases."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate RKE2 using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- RKE2 code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving RKE2 requires evidence-backed assessment.
...
- First-party source: https://github.com/rancher/rke2
...
- [ ] Trace RKE2 inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check RKE2 defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check RKE2 least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check RKE2 resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check RKE2 release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check RKE2 dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If RKE2 identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a RKE2 change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- RKE2 documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a RKE2 architecture and dependency summary with trust boundaries and persistent state.
- Include RKE2 validation commands or tests that are safe for the stated environment.
...
- RKE2 identity, version, configuration, dependencies, and deployment assumptions are explicit.
- RKE2 failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that RKE2 is secure, supported, compatible, or suitable.
- Using an unpinned latest version of RKE2 as the basis for migration or production guidance.
