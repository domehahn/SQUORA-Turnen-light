---
name: "cncf-cloud-custodian-reviewer"
description: "Review Cloud Custodian using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Cloud Custodian Reviewer

## Purpose

Review and operate Cloud Custodian using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Cloud Custodian code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Cloud Custodian requires evidence-backed assessment.
...
- First-party source: https://github.com/cloud-custodian/cloud-custodian
...
- [ ] Trace Cloud Custodian inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Cloud Custodian defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Cloud Custodian least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Cloud Custodian resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Cloud Custodian release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Cloud Custodian dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Cloud Custodian identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Cloud Custodian change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Cloud Custodian documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Cloud Custodian architecture and dependency summary with trust boundaries and persistent state.
- Include Cloud Custodian validation commands or tests that are safe for the stated environment.
...
- Cloud Custodian identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Cloud Custodian failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Cloud Custodian is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Cloud Custodian as the basis for migration or production guidance.
