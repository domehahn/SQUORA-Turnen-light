---
name: "cncf-filecoin-reviewer"
description: "Review Filecoin using its official documentation and repository in the Wasm / Decentralized Platforms category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Filecoin Reviewer

## Purpose

Review and operate Filecoin using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Filecoin code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Filecoin requires evidence-backed assessment.
...
- First-party source: https://github.com/filecoin-project/ref-fvm
...
- [ ] Trace Filecoin inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Filecoin defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Filecoin least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Filecoin resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Filecoin release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Filecoin dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Filecoin identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Filecoin change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Filecoin documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Filecoin architecture and dependency summary with trust boundaries and persistent state.
- Include Filecoin validation commands or tests that are safe for the stated environment.
...
- Filecoin identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Filecoin failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Filecoin is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Filecoin as the basis for migration or production guidance.
