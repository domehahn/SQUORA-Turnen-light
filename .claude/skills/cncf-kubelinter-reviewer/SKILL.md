---
name: "cncf-kubelinter-reviewer"
description: "Review KubeLinter using its official documentation and repository in the Provisioning / Security & Compliance category. KubeLinter analyzes Kubernetes YAML files and Helm charts, and checks them against a variety of best practices, with a focus on production readiness and security."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kubelinter Reviewer

## Purpose

Review and operate KubeLinter using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeLinter code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeLinter requires evidence-backed assessment.
...
- First-party source: https://github.com/stackrox/kube-linter
...
- [ ] Trace KubeLinter inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeLinter defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeLinter least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeLinter resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeLinter release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeLinter dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeLinter identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeLinter change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeLinter documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeLinter architecture and dependency summary with trust boundaries and persistent state.
- Include KubeLinter validation commands or tests that are safe for the stated environment.
...
- KubeLinter identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeLinter failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeLinter is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeLinter as the basis for migration or production guidance.
