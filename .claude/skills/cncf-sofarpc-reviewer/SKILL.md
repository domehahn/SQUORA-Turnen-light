---
name: "cncf-sofarpc-reviewer"
description: "Review SOFARPC using its official documentation and repository in the Orchestration & Management / Remote Procedure Call category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Sofarpc Reviewer

## Purpose

Review and operate SOFARPC using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SOFARPC code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SOFARPC requires evidence-backed assessment.
...
- First-party source: https://github.com/sofastack/sofa-rpc
...
- [ ] Trace SOFARPC inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SOFARPC defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SOFARPC least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SOFARPC resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SOFARPC release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SOFARPC dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SOFARPC identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SOFARPC change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SOFARPC documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SOFARPC architecture and dependency summary with trust boundaries and persistent state.
- Include SOFARPC validation commands or tests that are safe for the stated environment.
...
- SOFARPC identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SOFARPC failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SOFARPC is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SOFARPC as the basis for migration or production guidance.
