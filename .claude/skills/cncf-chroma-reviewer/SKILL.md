---
name: "cncf-chroma-reviewer"
description: "Review Chroma using its official documentation and repository in the AI Agent / Vector Database category. The AI-native open-source embedding database."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Chroma Reviewer

## Purpose

Review and operate Chroma using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Chroma code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Chroma requires evidence-backed assessment.
...
- First-party source: https://github.com/chroma-core/chroma
...
- [ ] Trace Chroma inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Chroma defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Chroma least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Chroma resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Chroma release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Chroma dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Chroma identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Chroma change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Chroma documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Chroma architecture and dependency summary with trust boundaries and persistent state.
- Include Chroma validation commands or tests that are safe for the stated environment.
...
- Chroma identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Chroma failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Chroma is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Chroma as the basis for migration or production guidance.
