---
name: "cncf-terasky-kcntp-reviewer"
...
Review and operate TeraSky (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- TeraSky (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving TeraSky (KCNTP) requires evidence-backed assessment.
...
- First-party source: https://www.terasky.com/kubernetes-academy/
...
- [ ] Trace TeraSky (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check TeraSky (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check TeraSky (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check TeraSky (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check TeraSky (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check TeraSky (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If TeraSky (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a TeraSky (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- TeraSky (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a TeraSky (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include TeraSky (KCNTP) validation commands or tests that are safe for the stated environment.
...
- TeraSky (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- TeraSky (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that TeraSky (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of TeraSky (KCNTP) as the basis for migration or production guidance.
