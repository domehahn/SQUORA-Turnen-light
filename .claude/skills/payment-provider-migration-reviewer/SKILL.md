---
name: "payment-provider-migration-reviewer"
description: "Review payment-provider migrations for parity, token portability, dual processing, routing, reconciliation, rollback, and decommissioning."
version: "1.0.0"
since: "2026-08-27"
...
Plan and review payment-provider migrations with capability parity, token portability, dual processing, routing, reconciliation, rollback, customer impact, and decommissioning controls. Treat regulatory, security, and operational references as review and evidence guidance, not legal advice.
...
- payment provider migration decisions, controls, or operating practices need independent review.
- A change affects payment provider migration artifacts such as provider capability matrix, token migration plan, dual-processing design, traffic-routing rule, rollback plan, provider decommission checklist.
- The user needs evidence-oriented findings for risks such as non-portable payment token, double charge during cutover, capability regression, settlement reconciliation gap, unsafe big-bang migration, premature provider shutdown.
- Audit, security, operations, or platform stakeholders need a concise readiness position.
- Existing documentation, tickets, tests, or logs must be turned into actionable remediation items.

## Operating model

1. Identify the relevant payment provider migration artifacts, owners, systems, environments, and review boundary.
2. Compare the available artifacts against expected signals such as sandbox parity test, shadow or canary result, token migration evidence, route allocation metric, dual-ledger reconciliation, rollback exercise.
...
- Primary artifacts: provider capability matrix, token migration plan, dual-processing design, traffic-routing rule, rollback plan, provider decommission checklist.
- Risk themes: non-portable payment token, double charge during cutover, capability regression, settlement reconciliation gap, unsafe big-bang migration, premature provider shutdown.
- Evidence signals: sandbox parity test, shadow or canary result, token migration evidence, route allocation metric, dual-ledger reconciliation, rollback exercise.
...
- [ ] Confirm the review boundary covers the right payment provider migration systems, teams, and environments.
- [ ] Inventory and inspect the current provider capability matrix.
- [ ] Check whether token migration plan is current, approved, versioned, and owned.
- [ ] Verify that dual-processing design has test, ticket, log, or approval support.
- [ ] Look for non-portable payment token and record concrete repository or process evidence.
- [ ] Look for double charge during cutover and identify affected assets, services, or stakeholders.
- [ ] Look for capability regression and classify the operational or audit impact.
- [ ] Use sandbox parity test to validate that the control or practice is operating.
- [ ] Use shadow or canary result to confirm ownership, timing, and reproducibility.
- [ ] Check exception, risk-acceptance, and expiry handling for payment provider migration.
...
- If provider capability matrix is missing for a critical service, raise at least a high-severity readiness gap.
- If shadow or canary result cannot be tied to an owner and approval, treat the outcome as unauditable until corrected.
- If traffic-routing rule is present but expired or untested, require validation before accepting residual risk.
...
- Missing or stale payment provider migration artifact.
...
- Findings ordered by severity with affected payment provider migration artifacts and evidence references.
- Coverage note for reviewed artifacts: provider capability matrix, token migration plan, dual-processing design, traffic-routing rule, rollback plan, provider decommission checklist.
- Risk note covering relevant themes: non-portable payment token, double charge during cutover, capability regression, settlement reconciliation gap, unsafe big-bang migration, premature provider shutdown.
- Evidence request list using expected signals: sandbox parity test, shadow or canary result, token migration evidence, route allocation metric, dual-ledger reconciliation, rollback exercise.
- Deliverables or updates needed: provider migration plan, capability gap matrix, cutover and rollback runbook, dual-processing controls, decommission readiness decision.
...
- Relevant payment provider migration artifacts are identified, current, owned, and versioned where applicable.
