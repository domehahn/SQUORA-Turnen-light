---
name: "cncf-bootc-reviewer"
description: "Review bootc using its official documentation and repository in the Runtime / Container Runtime category. The bootc provides transactional, in-place operating system images and updates using OCI/Docker container images. This project applies the Docker container layering model to bootable host systems, using standard OCI/Docker containers as a transport and delivery format for base operating system updates."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate bootc using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- bootc code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving bootc requires evidence-backed assessment.
...
- First-party source: https://github.com/bootc-dev/bootc
...
- [ ] Trace bootc inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check bootc defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check bootc least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check bootc resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check bootc release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check bootc dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If bootc identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a bootc change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- bootc documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a bootc architecture and dependency summary with trust boundaries and persistent state.
- Include bootc validation commands or tests that are safe for the stated environment.
...
- bootc identity, version, configuration, dependencies, and deployment assumptions are explicit.
- bootc failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that bootc is secure, supported, compatible, or suitable.
- Using an unpinned latest version of bootc as the basis for migration or production guidance.
