---
name: "cncf-woodpecker-ci-reviewer"
description: "Review Woodpecker CI using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Woodpecker Ci Reviewer

## Purpose

Review and operate Woodpecker CI using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Woodpecker CI code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Woodpecker CI requires evidence-backed assessment.
...
- First-party source: https://github.com/woodpecker-ci/woodpecker
...
- [ ] Trace Woodpecker CI inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Woodpecker CI defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Woodpecker CI least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Woodpecker CI resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Woodpecker CI release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Woodpecker CI dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Woodpecker CI identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Woodpecker CI change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Woodpecker CI documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Woodpecker CI architecture and dependency summary with trust boundaries and persistent state.
- Include Woodpecker CI validation commands or tests that are safe for the stated environment.
...
- Woodpecker CI identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Woodpecker CI failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Woodpecker CI is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Woodpecker CI as the basis for migration or production guidance.
