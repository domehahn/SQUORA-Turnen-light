---
name: "cncf-grafeas-reviewer"
description: "Review Grafeas using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Grafeas Reviewer

## Purpose

Review and operate Grafeas using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Grafeas code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Grafeas requires evidence-backed assessment.
...
- First-party source: https://github.com/grafeas/grafeas
...
- [ ] Trace Grafeas inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Grafeas defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Grafeas least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Grafeas resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Grafeas release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Grafeas dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Grafeas identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Grafeas change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Grafeas documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Grafeas architecture and dependency summary with trust boundaries and persistent state.
- Include Grafeas validation commands or tests that are safe for the stated environment.
...
- Grafeas identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Grafeas failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Grafeas is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Grafeas as the basis for migration or production guidance.
