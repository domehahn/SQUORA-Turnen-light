---
name: "cncf-elastic-apm-reviewer"
description: "Review Elastic APM using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Elastic Apm Reviewer

## Purpose

Review and operate Elastic APM using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Elastic APM code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Elastic APM requires evidence-backed assessment.
...
- First-party source: https://github.com/elastic/apm-server
...
- [ ] Trace Elastic APM inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Elastic APM defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Elastic APM least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Elastic APM resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Elastic APM release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Elastic APM dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Elastic APM identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Elastic APM change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Elastic APM documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Elastic APM architecture and dependency summary with trust boundaries and persistent state.
- Include Elastic APM validation commands or tests that are safe for the stated environment.
...
- Elastic APM identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Elastic APM failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Elastic APM is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Elastic APM as the basis for migration or production guidance.
