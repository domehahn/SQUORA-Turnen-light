---
name: "cncf-tinkerbell-reviewer"
description: "Review Tinkerbell using its official documentation and repository in the Provisioning / Automation & Configuration category. Bare metal provisioning engine, supporting network and ISO booting, BMC interactions, metadata service, and workflow engine."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Tinkerbell Reviewer

## Purpose

Review and operate Tinkerbell using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Tinkerbell code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Tinkerbell requires evidence-backed assessment.
...
- First-party source: https://github.com/tinkerbell/tinkerbell
...
- [ ] Trace Tinkerbell inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Tinkerbell defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Tinkerbell least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Tinkerbell resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Tinkerbell release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Tinkerbell dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Tinkerbell identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Tinkerbell change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Tinkerbell documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Tinkerbell architecture and dependency summary with trust boundaries and persistent state.
- Include Tinkerbell validation commands or tests that are safe for the stated environment.
...
- Tinkerbell identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Tinkerbell failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Tinkerbell is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Tinkerbell as the basis for migration or production guidance.
