---
name: "cncf-foundationdb-reviewer"
...
Review and operate FoundationDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- FoundationDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving FoundationDB requires evidence-backed assessment.
...
- First-party source: https://github.com/apple/foundationdb
...
- [ ] Trace FoundationDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check FoundationDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check FoundationDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check FoundationDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check FoundationDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check FoundationDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If FoundationDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a FoundationDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- FoundationDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a FoundationDB architecture and dependency summary with trust boundaries and persistent state.
- Include FoundationDB validation commands or tests that are safe for the stated environment.
...
- FoundationDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- FoundationDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that FoundationDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of FoundationDB as the basis for migration or production guidance.
