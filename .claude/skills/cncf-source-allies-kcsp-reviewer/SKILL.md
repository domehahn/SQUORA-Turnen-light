...
---
# Cncf Source Allies Kcsp Reviewer

## Purpose

Review and operate Source Allies (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Source Allies (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Source Allies (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.sourceallies.com/our-expertise/cloud-devops/kubernetes/
...
- [ ] Trace Source Allies (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Source Allies (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Source Allies (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Source Allies (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Source Allies (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Source Allies (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Source Allies (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Source Allies (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Source Allies (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Source Allies (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Source Allies (KCSP) validation commands or tests that are safe for the stated environment.
...
- Source Allies (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Source Allies (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Source Allies (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Source Allies (KCSP) as the basis for migration or production guidance.
