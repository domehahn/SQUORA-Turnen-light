---
name: "cncf-kruize-reviewer"
description: "Review Kruize using its official documentation and repository in the Observability and Analysis / Continuous Optimization category. Kruize analyzes your Kubernetes workload metrics and automatically generates right-sizing recommendations for CPU, memory, and GPU resources — reducing costs and improving performance without manual tuning."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kruize Reviewer

## Purpose

Review and operate Kruize using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kruize code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kruize requires evidence-backed assessment.
...
- First-party source: https://github.com/kruize/autotune
...
- [ ] Trace Kruize inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kruize defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kruize least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kruize resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kruize release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kruize dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kruize identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kruize change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kruize documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kruize architecture and dependency summary with trust boundaries and persistent state.
- Include Kruize validation commands or tests that are safe for the stated environment.
...
- Kruize identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kruize failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kruize is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kruize as the basis for migration or production guidance.
