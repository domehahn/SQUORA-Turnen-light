---
name: "cncf-launchdarkly-reviewer"
description: "Review LaunchDarkly using its official documentation and repository in the Observability and Analysis / Feature Flagging category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Launchdarkly Reviewer

## Purpose

Review and operate LaunchDarkly using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- LaunchDarkly code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving LaunchDarkly requires evidence-backed assessment.
...
- First-party source: https://launchdarkly.com
...
- [ ] Trace LaunchDarkly inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check LaunchDarkly defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check LaunchDarkly least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check LaunchDarkly resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check LaunchDarkly release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check LaunchDarkly dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If LaunchDarkly identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a LaunchDarkly change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- LaunchDarkly documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a LaunchDarkly architecture and dependency summary with trust boundaries and persistent state.
- Include LaunchDarkly validation commands or tests that are safe for the stated environment.
...
- LaunchDarkly identity, version, configuration, dependencies, and deployment assumptions are explicit.
- LaunchDarkly failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that LaunchDarkly is secure, supported, compatible, or suitable.
- Using an unpinned latest version of LaunchDarkly as the basis for migration or production guidance.
