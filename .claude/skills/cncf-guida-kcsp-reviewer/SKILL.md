---
name: "cncf-guida-kcsp-reviewer"
description: "Review Guida (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Guida builds and operates cloud native platforms based on Kubernetes so that software developers can focus on building innovative software."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Guida Kcsp Reviewer

## Purpose

Review and operate Guida (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Guida (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Guida (KCSP) requires evidence-backed assessment.
...
- [ ] Trace Guida (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Guida (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Guida (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Guida (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Guida (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Guida (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Guida (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Guida (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Guida (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Guida (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Guida (KCSP) validation commands or tests that are safe for the stated environment.
...
- Guida (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Guida (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Guida (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Guida (KCSP) as the basis for migration or production guidance.
