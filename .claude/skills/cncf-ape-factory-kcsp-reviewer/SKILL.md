---
name: "cncf-ape-factory-kcsp-reviewer"
description: "Review ape factory (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. As Kubernetes Experts, we design, build and operate your Kubernetes cluster in the public cloud, on-prem, or multi/hybrid cloud environments  tailored to your needs."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Ape Factory Kcsp Reviewer

## Purpose

Review and operate ape factory (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ape factory (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ape factory (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.apefactory.com/en/technologies/kubernetes
...
- [ ] Trace ape factory (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ape factory (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ape factory (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ape factory (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ape factory (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ape factory (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ape factory (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ape factory (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ape factory (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ape factory (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include ape factory (KCSP) validation commands or tests that are safe for the stated environment.
...
- ape factory (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ape factory (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ape factory (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ape factory (KCSP) as the basis for migration or production guidance.
