---
name: "cncf-appsignal-reviewer"
description: "Review AppSignal using its official documentation and repository in the Observability and Analysis / Observability category. AppSignal is a powerful APM that works out-of-the-box for Elixir, Ruby, and Node.js."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Appsignal Reviewer

## Purpose

Review and operate AppSignal using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- AppSignal code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving AppSignal requires evidence-backed assessment.
...
- First-party source: https://www.appsignal.com/
...
- [ ] Trace AppSignal inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check AppSignal defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check AppSignal least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check AppSignal resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check AppSignal release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check AppSignal dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If AppSignal identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a AppSignal change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- AppSignal documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a AppSignal architecture and dependency summary with trust boundaries and persistent state.
- Include AppSignal validation commands or tests that are safe for the stated environment.
...
- AppSignal identity, version, configuration, dependencies, and deployment assumptions are explicit.
- AppSignal failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that AppSignal is secure, supported, compatible, or suitable.
- Using an unpinned latest version of AppSignal as the basis for migration or production guidance.
