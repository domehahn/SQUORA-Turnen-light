---
name: "cncf-kubeblocks-by-apecloud-reviewer"
description: "Review KubeBlocks by ApeCloud using its official documentation and repository in the App Definition and Development / Database category. KubeBlocks is an open-source Kubernetes operator that manages relational, NoSQL, vector, and streaming databases on the public cloud or on-premise."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kubeblocks By Apecloud Reviewer

## Purpose

Review and operate KubeBlocks by ApeCloud using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeBlocks by ApeCloud code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeBlocks by ApeCloud requires evidence-backed assessment.
...
- First-party source: https://github.com/apecloud/kubeblocks
...
- [ ] Trace KubeBlocks by ApeCloud inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeBlocks by ApeCloud defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeBlocks by ApeCloud least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeBlocks by ApeCloud resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeBlocks by ApeCloud release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeBlocks by ApeCloud dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeBlocks by ApeCloud identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeBlocks by ApeCloud change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeBlocks by ApeCloud documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeBlocks by ApeCloud architecture and dependency summary with trust boundaries and persistent state.
- Include KubeBlocks by ApeCloud validation commands or tests that are safe for the stated environment.
...
- KubeBlocks by ApeCloud identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeBlocks by ApeCloud failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeBlocks by ApeCloud is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeBlocks by ApeCloud as the basis for migration or production guidance.
