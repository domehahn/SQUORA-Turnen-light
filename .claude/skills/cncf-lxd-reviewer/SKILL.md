---
name: "cncf-lxd-reviewer"
description: "Review lxd using its official documentation and repository in the Runtime / Container Runtime category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate lxd using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- lxd code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving lxd requires evidence-backed assessment.
...
- First-party source: https://github.com/lxc/lxd
...
- [ ] Trace lxd inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check lxd defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check lxd least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check lxd resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check lxd release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check lxd dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If lxd identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a lxd change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- lxd documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a lxd architecture and dependency summary with trust boundaries and persistent state.
- Include lxd validation commands or tests that are safe for the stated environment.
...
- lxd identity, version, configuration, dependencies, and deployment assumptions are explicit.
- lxd failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that lxd is secure, supported, compatible, or suitable.
- Using an unpinned latest version of lxd as the basis for migration or production guidance.
