---
name: "cncf-wasmer-reviewer"
description: "Review Wasmer using its official documentation and repository in the Wasm / Runtimes category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Wasmer Reviewer

## Purpose

Review and operate Wasmer using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Wasmer code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Wasmer requires evidence-backed assessment.
...
- First-party source: https://github.com/wasmerio/wasmer
...
- [ ] Trace Wasmer inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Wasmer defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Wasmer least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Wasmer resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Wasmer release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Wasmer dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Wasmer identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Wasmer change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Wasmer documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Wasmer architecture and dependency summary with trust boundaries and persistent state.
- Include Wasmer validation commands or tests that are safe for the stated environment.
...
- Wasmer identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Wasmer failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Wasmer is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Wasmer as the basis for migration or production guidance.
