---
name: "cncf-juicefs-reviewer"
description: "Review JuiceFS using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Juicefs Reviewer

## Purpose

Review and operate JuiceFS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- JuiceFS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving JuiceFS requires evidence-backed assessment.
...
- First-party source: https://github.com/juicedata/juicefs
...
- [ ] Trace JuiceFS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check JuiceFS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check JuiceFS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check JuiceFS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check JuiceFS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check JuiceFS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If JuiceFS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a JuiceFS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- JuiceFS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a JuiceFS architecture and dependency summary with trust boundaries and persistent state.
- Include JuiceFS validation commands or tests that are safe for the stated environment.
...
- JuiceFS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- JuiceFS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that JuiceFS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of JuiceFS as the basis for migration or production guidance.
