---
name: "cncf-kubeservice-stack-reviewer"
description: "Review KubeService Stack using its official documentation and repository in the Platform / Certified Kubernetes - Distribution category. KubeService Stack = Kubernetes + Custom Service Build a Kubernetes/KubeEdge enterprise-level peripheral ecosystem"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kubeservice Stack Reviewer

## Purpose

Review and operate KubeService Stack using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeService Stack code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeService Stack requires evidence-backed assessment.
...
- First-party source: https://stack.kubeservice.cn/
...
- [ ] Trace KubeService Stack inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeService Stack defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeService Stack least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeService Stack resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeService Stack release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeService Stack dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeService Stack identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeService Stack change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeService Stack documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeService Stack architecture and dependency summary with trust boundaries and persistent state.
- Include KubeService Stack validation commands or tests that are safe for the stated environment.
...
- KubeService Stack identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeService Stack failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeService Stack is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeService Stack as the basis for migration or production guidance.
