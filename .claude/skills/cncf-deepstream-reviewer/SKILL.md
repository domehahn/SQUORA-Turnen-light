---
name: "cncf-deepstream-reviewer"
description: "Review deepstream using its official documentation and repository in the App Definition and Development / Streaming & Messaging category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Deepstream Reviewer

## Purpose

Review and operate deepstream using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- deepstream code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving deepstream requires evidence-backed assessment.
...
- First-party source: https://github.com/deepstreamIO/deepstream.io
...
- [ ] Trace deepstream inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check deepstream defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check deepstream least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check deepstream resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check deepstream release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check deepstream dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If deepstream identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a deepstream change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- deepstream documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a deepstream architecture and dependency summary with trust boundaries and persistent state.
- Include deepstream validation commands or tests that are safe for the stated environment.
...
- deepstream identity, version, configuration, dependencies, and deployment assumptions are explicit.
- deepstream failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that deepstream is secure, supported, compatible, or suitable.
- Using an unpinned latest version of deepstream as the basis for migration or production guidance.
