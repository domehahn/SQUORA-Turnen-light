---
name: "cncf-easegress-reviewer"
description: "Review Easegress using its official documentation and repository in the Orchestration & Management / API Gateway category. A Cloud Native traffic orchestration system"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Easegress Reviewer

## Purpose

Review and operate Easegress using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Easegress code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Easegress requires evidence-backed assessment.
...
- First-party source: https://github.com/easegress-io/easegress
...
- [ ] Trace Easegress inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Easegress defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Easegress least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Easegress resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Easegress release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Easegress dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Easegress identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Easegress change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Easegress documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Easegress architecture and dependency summary with trust boundaries and persistent state.
- Include Easegress validation commands or tests that are safe for the stated environment.
...
- Easegress identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Easegress failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Easegress is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Easegress as the basis for migration or production guidance.
