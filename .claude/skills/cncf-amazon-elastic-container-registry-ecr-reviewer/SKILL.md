---
name: "cncf-amazon-elastic-container-registry-ecr-reviewer"
...
---
# Cncf Amazon Elastic Container Registry Ecr Reviewer

## Purpose

Review and operate Amazon Elastic Container Registry (ECR) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Amazon Elastic Container Registry (ECR) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Amazon Elastic Container Registry (ECR) requires evidence-backed assessment.
...
- First-party source: https://aws.amazon.com/ecr/
...
- [ ] Trace Amazon Elastic Container Registry (ECR) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Amazon Elastic Container Registry (ECR) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Amazon Elastic Container Registry (ECR) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Amazon Elastic Container Registry (ECR) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Amazon Elastic Container Registry (ECR) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Amazon Elastic Container Registry (ECR) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Amazon Elastic Container Registry (ECR) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Amazon Elastic Container Registry (ECR) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Amazon Elastic Container Registry (ECR) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Amazon Elastic Container Registry (ECR) architecture and dependency summary with trust boundaries and persistent state.
- Include Amazon Elastic Container Registry (ECR) validation commands or tests that are safe for the stated environment.
...
- Amazon Elastic Container Registry (ECR) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Amazon Elastic Container Registry (ECR) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Amazon Elastic Container Registry (ECR) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Amazon Elastic Container Registry (ECR) as the basis for migration or production guidance.
