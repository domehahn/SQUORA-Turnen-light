---
name: "cncf-istio-wasm-reviewer"
description: "Review Istio (Wasm) using its official documentation and repository in the Wasm / Embedded Functions category. Simplify observability, traffic management, security, and policy with the Istio service mesh."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Istio Wasm Reviewer

## Purpose

Review and operate Istio (Wasm) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Istio (Wasm) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Istio (Wasm) requires evidence-backed assessment.
...
- First-party source: https://github.com/istio/istio
...
- [ ] Trace Istio (Wasm) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Istio (Wasm) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Istio (Wasm) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Istio (Wasm) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Istio (Wasm) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Istio (Wasm) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Istio (Wasm) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Istio (Wasm) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Istio (Wasm) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Istio (Wasm) architecture and dependency summary with trust boundaries and persistent state.
- Include Istio (Wasm) validation commands or tests that are safe for the stated environment.
...
- Istio (Wasm) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Istio (Wasm) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Istio (Wasm) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Istio (Wasm) as the basis for migration or production guidance.
