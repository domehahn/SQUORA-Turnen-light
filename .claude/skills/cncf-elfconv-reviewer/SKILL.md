---
name: "cncf-elfconv-reviewer"
description: "Review elfconv using its official documentation and repository in the Wasm / Tooling category. An AOT binary translator that converts Linux/ELF binaries to WebAssembly"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Elfconv Reviewer

## Purpose

Review and operate elfconv using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- elfconv code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving elfconv requires evidence-backed assessment.
...
- First-party source: https://github.com/yomaytk/elfconv
...
- [ ] Trace elfconv inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check elfconv defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check elfconv least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check elfconv resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check elfconv release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check elfconv dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If elfconv identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a elfconv change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- elfconv documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a elfconv architecture and dependency summary with trust boundaries and persistent state.
- Include elfconv validation commands or tests that are safe for the stated environment.
...
- elfconv identity, version, configuration, dependencies, and deployment assumptions are explicit.
- elfconv failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that elfconv is secure, supported, compatible, or suitable.
- Using an unpinned latest version of elfconv as the basis for migration or production guidance.
