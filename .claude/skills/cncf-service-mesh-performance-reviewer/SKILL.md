---
name: "cncf-service-mesh-performance-reviewer"
description: "Review Service Mesh Performance using its official documentation and repository in the Orchestration & Management / Service Mesh category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Service Mesh Performance Reviewer

## Purpose

Review and operate Service Mesh Performance using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Service Mesh Performance code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Service Mesh Performance requires evidence-backed assessment.
...
- First-party source: https://github.com/service-mesh-performance/service-mesh-performance
...
- [ ] Trace Service Mesh Performance inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Service Mesh Performance defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Service Mesh Performance least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Service Mesh Performance resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Service Mesh Performance release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Service Mesh Performance dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Service Mesh Performance identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Service Mesh Performance change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Service Mesh Performance documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Service Mesh Performance architecture and dependency summary with trust boundaries and persistent state.
- Include Service Mesh Performance validation commands or tests that are safe for the stated environment.
...
- Service Mesh Performance identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Service Mesh Performance failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Service Mesh Performance is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Service Mesh Performance as the basis for migration or production guidance.
