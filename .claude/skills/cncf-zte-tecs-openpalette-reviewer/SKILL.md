---
name: "cncf-zte-tecs-openpalette-reviewer"
description: "Review ZTE TECS OpenPalette using its official documentation and repository in the Platform / Certified Kubernetes - Hosted category. The TECS OpenPalette platform is a unified cloud platform oriented to the ICT realm."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Zte Tecs Openpalette Reviewer

## Purpose

Review and operate ZTE TECS OpenPalette using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ZTE TECS OpenPalette code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ZTE TECS OpenPalette requires evidence-backed assessment.
...
- First-party source: https://sdnfv.zte.com.cn/en
...
- [ ] Trace ZTE TECS OpenPalette inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ZTE TECS OpenPalette defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ZTE TECS OpenPalette least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ZTE TECS OpenPalette resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ZTE TECS OpenPalette release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ZTE TECS OpenPalette dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ZTE TECS OpenPalette identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ZTE TECS OpenPalette change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ZTE TECS OpenPalette documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ZTE TECS OpenPalette architecture and dependency summary with trust boundaries and persistent state.
- Include ZTE TECS OpenPalette validation commands or tests that are safe for the stated environment.
...
- ZTE TECS OpenPalette identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ZTE TECS OpenPalette failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ZTE TECS OpenPalette is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ZTE TECS OpenPalette as the basis for migration or production guidance.
