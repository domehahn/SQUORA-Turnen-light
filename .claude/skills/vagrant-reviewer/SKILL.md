---
name: "vagrant-reviewer"
description: "Review Vagrant environments, providers, networking, provisioning, shared folders, reproducibility, and isolation."
version: "1.0.0"
since: "2026-08-27"
...
- Source code, configuration, dependencies, tests, deployment artifacts, or runtime behavior in Vagrant development environments change.
...
- Vagrantfile, providers, networking, provisioners, shared folders, base boxes
...
- [ ] Trace Vagrant development environments inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Vagrant development environments defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Vagrant development environments least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Vagrant development environments resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Vagrant development environments release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
...
- If Vagrant development environments identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Vagrant development environments change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Vagrant development environments documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Vagrant development environments architecture and dependency summary with trust boundaries and persistent state.
- Include Vagrant development environments validation commands or tests that are safe for the stated environment.
...
- Vagrant development environments identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Vagrant development environments failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Vagrant development environments is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Vagrant development environments as the basis for migration or production guidance.
