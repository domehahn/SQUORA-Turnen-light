---
name: "cncf-tyk-reviewer"
description: "Review Tyk using its official documentation and repository in the Orchestration & Management / API Gateway category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Tyk using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Tyk code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Tyk requires evidence-backed assessment.
...
- First-party source: https://github.com/TykTechnologies/tyk
...
- [ ] Trace Tyk inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Tyk defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Tyk least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Tyk resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Tyk release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Tyk dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Tyk identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Tyk change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Tyk documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Tyk architecture and dependency summary with trust boundaries and persistent state.
- Include Tyk validation commands or tests that are safe for the stated environment.
...
- Tyk identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Tyk failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Tyk is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Tyk as the basis for migration or production guidance.
