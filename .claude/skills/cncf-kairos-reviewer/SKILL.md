---
name: "cncf-kairos-reviewer"
description: "Review Kairos using its official documentation and repository in the Provisioning / Automation & Configuration category. The immutable Linux meta-distribution for edge Kubernetes"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kairos Reviewer

## Purpose

Review and operate Kairos using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kairos code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kairos requires evidence-backed assessment.
...
- First-party source: https://github.com/kairos-io/kairos
...
- [ ] Trace Kairos inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kairos defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kairos least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kairos resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kairos release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kairos dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kairos identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kairos change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kairos documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kairos architecture and dependency summary with trust boundaries and persistent state.
- Include Kairos validation commands or tests that are safe for the stated environment.
...
- Kairos identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kairos failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kairos is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kairos as the basis for migration or production guidance.
