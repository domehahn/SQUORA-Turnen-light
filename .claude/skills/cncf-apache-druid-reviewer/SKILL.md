---
name: "cncf-apache-druid-reviewer"
description: "Review Apache Druid using its official documentation and repository in the Data / Data Architecture category. A high performance real-time analytics database."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Apache Druid Reviewer

## Purpose

Review and operate Apache Druid using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Apache Druid code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Apache Druid requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/druid
...
- [ ] Trace Apache Druid inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Apache Druid defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Apache Druid least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Apache Druid resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Apache Druid release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Apache Druid dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Apache Druid identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Apache Druid change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Apache Druid documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Apache Druid architecture and dependency summary with trust boundaries and persistent state.
- Include Apache Druid validation commands or tests that are safe for the stated environment.
...
- Apache Druid identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Apache Druid failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Apache Druid is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Apache Druid as the basis for migration or production guidance.
