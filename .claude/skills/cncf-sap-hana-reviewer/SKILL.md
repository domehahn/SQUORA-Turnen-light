---
name: "cncf-sap-hana-reviewer"
description: "Review SAP HANA using its official documentation and repository in the App Definition and Development / Database category. SAP HANA is an in-memory, column-oriented, relational database management system"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Sap Hana Reviewer

## Purpose

Review and operate SAP HANA using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SAP HANA code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SAP HANA requires evidence-backed assessment.
...
- First-party source: https://community.sap.com/topics/hana
...
- [ ] Trace SAP HANA inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SAP HANA defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SAP HANA least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SAP HANA resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SAP HANA release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SAP HANA dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SAP HANA identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SAP HANA change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SAP HANA documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SAP HANA architecture and dependency summary with trust boundaries and persistent state.
- Include SAP HANA validation commands or tests that are safe for the stated environment.
...
- SAP HANA identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SAP HANA failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SAP HANA is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SAP HANA as the basis for migration or production guidance.
