---
name: "cncf-taikun-cloudworks-wasm-reviewer"
description: "Review Taikun CloudWorks (Wasm) using its official documentation and repository in the Wasm / Hosted Platforms category. Taikun is a hybrid cloud management orchestration platform."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Taikun Cloudworks Wasm Reviewer

## Purpose

Review and operate Taikun CloudWorks (Wasm) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Taikun CloudWorks (Wasm) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Taikun CloudWorks (Wasm) requires evidence-backed assessment.
...
- First-party source: https://taikun.cloud
...
- [ ] Trace Taikun CloudWorks (Wasm) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Taikun CloudWorks (Wasm) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Taikun CloudWorks (Wasm) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Taikun CloudWorks (Wasm) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Taikun CloudWorks (Wasm) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Taikun CloudWorks (Wasm) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Taikun CloudWorks (Wasm) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Taikun CloudWorks (Wasm) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Taikun CloudWorks (Wasm) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Taikun CloudWorks (Wasm) architecture and dependency summary with trust boundaries and persistent state.
- Include Taikun CloudWorks (Wasm) validation commands or tests that are safe for the stated environment.
...
- Taikun CloudWorks (Wasm) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Taikun CloudWorks (Wasm) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Taikun CloudWorks (Wasm) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Taikun CloudWorks (Wasm) as the basis for migration or production guidance.
