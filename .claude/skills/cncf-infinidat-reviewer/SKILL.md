---
name: "cncf-infinidat-reviewer"
description: "Review INFINIDAT using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Infinidat Reviewer

## Purpose

Review and operate INFINIDAT using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- INFINIDAT code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving INFINIDAT requires evidence-backed assessment.
...
- First-party source: https://www.infinidat.com/en
...
- [ ] Trace INFINIDAT inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check INFINIDAT defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check INFINIDAT least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check INFINIDAT resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check INFINIDAT release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check INFINIDAT dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If INFINIDAT identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a INFINIDAT change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- INFINIDAT documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a INFINIDAT architecture and dependency summary with trust boundaries and persistent state.
- Include INFINIDAT validation commands or tests that are safe for the stated environment.
...
- INFINIDAT identity, version, configuration, dependencies, and deployment assumptions are explicit.
- INFINIDAT failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that INFINIDAT is secure, supported, compatible, or suitable.
- Using an unpinned latest version of INFINIDAT as the basis for migration or production guidance.
