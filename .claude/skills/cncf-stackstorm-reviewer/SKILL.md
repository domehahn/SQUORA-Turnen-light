---
name: "cncf-stackstorm-reviewer"
description: "Review StackStorm using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Stackstorm Reviewer

## Purpose

Review and operate StackStorm using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- StackStorm code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving StackStorm requires evidence-backed assessment.
...
- First-party source: https://github.com/stackstorm/st2
...
- [ ] Trace StackStorm inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check StackStorm defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check StackStorm least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check StackStorm resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check StackStorm release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check StackStorm dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If StackStorm identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a StackStorm change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- StackStorm documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a StackStorm architecture and dependency summary with trust boundaries and persistent state.
- Include StackStorm validation commands or tests that are safe for the stated environment.
...
- StackStorm identity, version, configuration, dependencies, and deployment assumptions are explicit.
- StackStorm failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that StackStorm is secure, supported, compatible, or suitable.
- Using an unpinned latest version of StackStorm as the basis for migration or production guidance.
