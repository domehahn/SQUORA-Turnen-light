---
name: "cncf-megatron-lm-reviewer"
description: "Review Megatron-LM using its official documentation and repository in the Training / Distributed Training category. GPU optimized techniques for training transformer models at-scale"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Megatron Lm Reviewer

## Purpose

Review and operate Megatron-LM using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Megatron-LM code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Megatron-LM requires evidence-backed assessment.
...
- First-party source: https://github.com/NVIDIA/Megatron-LM
...
- [ ] Trace Megatron-LM inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Megatron-LM defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Megatron-LM least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Megatron-LM resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Megatron-LM release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Megatron-LM dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Megatron-LM identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Megatron-LM change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Megatron-LM documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Megatron-LM architecture and dependency summary with trust boundaries and persistent state.
- Include Megatron-LM validation commands or tests that are safe for the stated environment.
...
- Megatron-LM identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Megatron-LM failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Megatron-LM is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Megatron-LM as the basis for migration or production guidance.
