---
name: "cncf-beam-reviewer"
description: "Review Beam using its official documentation and repository in the App Definition and Development / Streaming & Messaging category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Beam using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Beam code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Beam requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/beam
...
- [ ] Trace Beam inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Beam defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Beam least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Beam resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Beam release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Beam dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Beam identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Beam change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Beam documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Beam architecture and dependency summary with trust boundaries and persistent state.
- Include Beam validation commands or tests that are safe for the stated environment.
...
- Beam identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Beam failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Beam is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Beam as the basis for migration or production guidance.
