---
name: "cncf-azure-pipelines-reviewer"
description: "Review Azure Pipelines using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category. Continuously build, test, and deploy to any platform and cloud"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Azure Pipelines Reviewer

## Purpose

Review and operate Azure Pipelines using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Azure Pipelines code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Azure Pipelines requires evidence-backed assessment.
...
- First-party source: https://azure.microsoft.com/en-us/services/devops/pipelines/
...
- [ ] Trace Azure Pipelines inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Azure Pipelines defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Azure Pipelines least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Azure Pipelines resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Azure Pipelines release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Azure Pipelines dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Azure Pipelines identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Azure Pipelines change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Azure Pipelines documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Azure Pipelines architecture and dependency summary with trust boundaries and persistent state.
- Include Azure Pipelines validation commands or tests that are safe for the stated environment.
...
- Azure Pipelines identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Azure Pipelines failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Azure Pipelines is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Azure Pipelines as the basis for migration or production guidance.
