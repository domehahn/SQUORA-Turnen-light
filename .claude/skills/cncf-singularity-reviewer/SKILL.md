---
name: "cncf-singularity-reviewer"
description: "Review Singularity using its official documentation and repository in the Runtime / Container Runtime category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Singularity Reviewer

## Purpose

Review and operate Singularity using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Singularity code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Singularity requires evidence-backed assessment.
...
- First-party source: https://github.com/apptainer/singularity
...
- [ ] Trace Singularity inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Singularity defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Singularity least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Singularity resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Singularity release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Singularity dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Singularity identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Singularity change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Singularity documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Singularity architecture and dependency summary with trust boundaries and persistent state.
- Include Singularity validation commands or tests that are safe for the stated environment.
...
- Singularity identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Singularity failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Singularity is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Singularity as the basis for migration or production guidance.
