...
Review and operate Keptn using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Keptn code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Keptn requires evidence-backed assessment.
...
- First-party source: https://github.com/keptn/lifecycle-toolkit
...
- [ ] Trace Keptn inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Keptn defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Keptn least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Keptn resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Keptn release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Keptn dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Keptn identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Keptn change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Keptn documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Keptn architecture and dependency summary with trust boundaries and persistent state.
- Include Keptn validation commands or tests that are safe for the stated environment.
...
- Keptn identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Keptn failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Keptn is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Keptn as the basis for migration or production guidance.
