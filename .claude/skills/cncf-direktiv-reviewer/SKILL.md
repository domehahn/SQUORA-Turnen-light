---
name: "cncf-direktiv-reviewer"
description: "Review Direktiv using its official documentation and repository in the Serverless / Installable Platform category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Direktiv Reviewer

## Purpose

Review and operate Direktiv using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Direktiv code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Direktiv requires evidence-backed assessment.
...
- First-party source: https://github.com/direktiv/direktiv
...
- [ ] Trace Direktiv inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Direktiv defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Direktiv least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Direktiv resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Direktiv release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Direktiv dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Direktiv identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Direktiv change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Direktiv documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Direktiv architecture and dependency summary with trust boundaries and persistent state.
- Include Direktiv validation commands or tests that are safe for the stated environment.
...
- Direktiv identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Direktiv failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Direktiv is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Direktiv as the basis for migration or production guidance.
