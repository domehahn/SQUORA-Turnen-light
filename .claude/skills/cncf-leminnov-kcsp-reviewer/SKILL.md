---
name: "cncf-leminnov-kcsp-reviewer"
description: "Review Leminnov (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Leminnov helps companies design, deploy, and manage resilient Kubernetes infrastructures tailored to their business needs. From cloud native transformation to automated CI/CD pipelines, we enable scalable, secure, and efficient operations."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Leminnov Kcsp Reviewer

## Purpose

Review and operate Leminnov (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Leminnov (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Leminnov (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.leminnov.com/cloud_native/
...
- [ ] Trace Leminnov (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Leminnov (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Leminnov (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Leminnov (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Leminnov (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Leminnov (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Leminnov (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Leminnov (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Leminnov (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Leminnov (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Leminnov (KCSP) validation commands or tests that are safe for the stated environment.
...
- Leminnov (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Leminnov (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Leminnov (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Leminnov (KCSP) as the basis for migration or production guidance.
