---
name: "cncf-pionative-kcsp-reviewer"
description: "Review Pionative (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Pionative helps companies succeed with their Kubernetes journey by providing enterprise-level support."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Pionative Kcsp Reviewer

## Purpose

Review and operate Pionative (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Pionative (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Pionative (KCSP) requires evidence-backed assessment.
...
- First-party source: https://pionative.com/how-we-work/
...
- [ ] Trace Pionative (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Pionative (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Pionative (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Pionative (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Pionative (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Pionative (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Pionative (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Pionative (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Pionative (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Pionative (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Pionative (KCSP) validation commands or tests that are safe for the stated environment.
...
- Pionative (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Pionative (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Pionative (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Pionative (KCSP) as the basis for migration or production guidance.
