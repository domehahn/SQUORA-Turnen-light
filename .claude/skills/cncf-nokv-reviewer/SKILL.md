---
name: "cncf-nokv-reviewer"
description: "Review NoKV using its official documentation and repository in the Runtime / Cloud Native Storage category. AI-native distributed filesystem."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate NoKV using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- NoKV code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving NoKV requires evidence-backed assessment.
...
- First-party source: https://github.com/NoKV-Lab/NoKV
...
- [ ] Trace NoKV inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check NoKV defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check NoKV least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check NoKV resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check NoKV release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check NoKV dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If NoKV identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a NoKV change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- NoKV documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a NoKV architecture and dependency summary with trust boundaries and persistent state.
- Include NoKV validation commands or tests that are safe for the stated environment.
...
- NoKV identity, version, configuration, dependencies, and deployment assumptions are explicit.
- NoKV failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that NoKV is secure, supported, compatible, or suitable.
- Using an unpinned latest version of NoKV as the basis for migration or production guidance.
