---
name: "cncf-soda-foundation-reviewer"
description: "Review Soda Foundation using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Soda Foundation Reviewer

## Purpose

Review and operate Soda Foundation using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Soda Foundation code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Soda Foundation requires evidence-backed assessment.
...
- First-party source: https://github.com/sodafoundation/api
...
- [ ] Trace Soda Foundation inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Soda Foundation defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Soda Foundation least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Soda Foundation resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Soda Foundation release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Soda Foundation dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Soda Foundation identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Soda Foundation change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Soda Foundation documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Soda Foundation architecture and dependency summary with trust boundaries and persistent state.
- Include Soda Foundation validation commands or tests that are safe for the stated environment.
...
- Soda Foundation identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Soda Foundation failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Soda Foundation is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Soda Foundation as the basis for migration or production guidance.
