---
name: "cncf-doris-reviewer"
description: "Review Doris using its official documentation and repository in the App Definition and Development / Database category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Doris using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Doris code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Doris requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/doris
...
- [ ] Trace Doris inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Doris defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Doris least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Doris resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Doris release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Doris dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Doris identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Doris change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Doris documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Doris architecture and dependency summary with trust boundaries and persistent state.
- Include Doris validation commands or tests that are safe for the stated environment.
...
- Doris identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Doris failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Doris is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Doris as the basis for migration or production guidance.
