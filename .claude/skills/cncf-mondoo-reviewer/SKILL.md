---
name: "cncf-mondoo-reviewer"
description: "Review Mondoo using its official documentation and repository in the Provisioning / Security & Compliance category. Cloud native infrastructure security for your entire fleet"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Mondoo Reviewer

## Purpose

Review and operate Mondoo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Mondoo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Mondoo requires evidence-backed assessment.
...
- First-party source: https://mondoo.com
...
- [ ] Trace Mondoo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Mondoo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Mondoo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Mondoo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Mondoo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Mondoo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Mondoo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Mondoo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Mondoo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Mondoo architecture and dependency summary with trust boundaries and persistent state.
- Include Mondoo validation commands or tests that are safe for the stated environment.
...
- Mondoo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Mondoo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Mondoo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Mondoo as the basis for migration or production guidance.
