---
name: "cncf-composio-reviewer"
description: "Review Composio using its official documentation and repository in the AI Agent / Agent Tool category. Composio powers 1000+ toolkits, tool search, context management, authentication, and a sandboxed workbench to help you build AI agents that turn intent into action."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Composio Reviewer

## Purpose

Review and operate Composio using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Composio code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Composio requires evidence-backed assessment.
...
- CNCF classification: AI Agent / Agent Tool
- CNCF project maturity field: not specified
- First-party source: https://github.com/ComposioHQ/composio
...
- [ ] Trace Composio inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Composio defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Composio least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Composio resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Composio release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Composio dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Composio identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Composio change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Composio documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Composio architecture and dependency summary with trust boundaries and persistent state.
- Include Composio validation commands or tests that are safe for the stated environment.
...
- Composio identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Composio failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Composio is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Composio as the basis for migration or production guidance.
