---
name: "cncf-nutanix-kcsp-reviewer"
description: "Review Nutanix (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Nutanix provides comprehensive professional services, support, and training for Kubernetes."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Nutanix Kcsp Reviewer

## Purpose

Review and operate Nutanix (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Nutanix (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Nutanix (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.nutanix.com/support-services/professional-services/cloud-native
...
- [ ] Trace Nutanix (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Nutanix (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Nutanix (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Nutanix (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Nutanix (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Nutanix (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Nutanix (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Nutanix (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Nutanix (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Nutanix (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Nutanix (KCSP) validation commands or tests that are safe for the stated environment.
...
- Nutanix (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Nutanix (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Nutanix (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Nutanix (KCSP) as the basis for migration or production guidance.
