---
name: "cncf-inspur-kcntp-reviewer"
description: "Review INSPUR (KCNTP) using its official documentation and repository in the Special / Kubernetes and Cloud Native Training Partner category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Inspur Kcntp Reviewer

## Purpose

Review and operate INSPUR (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- INSPUR (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving INSPUR (KCNTP) requires evidence-backed assessment.
...
- [ ] Trace INSPUR (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check INSPUR (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check INSPUR (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check INSPUR (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check INSPUR (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check INSPUR (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If INSPUR (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a INSPUR (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- INSPUR (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a INSPUR (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include INSPUR (KCNTP) validation commands or tests that are safe for the stated environment.
...
- INSPUR (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- INSPUR (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that INSPUR (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of INSPUR (KCNTP) as the basis for migration or production guidance.
