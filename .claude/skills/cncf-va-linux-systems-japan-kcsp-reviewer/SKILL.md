---
name: "cncf-va-linux-systems-japan-kcsp-reviewer"
description: "Review VA Linux Systems Japan (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. VA Linux provides the following services for Kubernetes: Consulting, Development, Failure Analysis & Support."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Va Linux Systems Japan Kcsp Reviewer

## Purpose

Review and operate VA Linux Systems Japan (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- VA Linux Systems Japan (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving VA Linux Systems Japan (KCSP) requires evidence-backed assessment.
...
- [ ] Trace VA Linux Systems Japan (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check VA Linux Systems Japan (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check VA Linux Systems Japan (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check VA Linux Systems Japan (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check VA Linux Systems Japan (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check VA Linux Systems Japan (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If VA Linux Systems Japan (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a VA Linux Systems Japan (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- VA Linux Systems Japan (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a VA Linux Systems Japan (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include VA Linux Systems Japan (KCSP) validation commands or tests that are safe for the stated environment.
...
- VA Linux Systems Japan (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- VA Linux Systems Japan (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that VA Linux Systems Japan (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of VA Linux Systems Japan (KCSP) as the basis for migration or production guidance.
