...
---
# Cncf Infracost Reviewer

## Purpose

Review and operate Infracost using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Infracost code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Infracost requires evidence-backed assessment.
...
- First-party source: https://github.com/infracost/infracost
...
- [ ] Trace Infracost inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Infracost defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Infracost least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Infracost resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Infracost release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Infracost dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Infracost identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Infracost change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Infracost documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Infracost architecture and dependency summary with trust boundaries and persistent state.
- Include Infracost validation commands or tests that are safe for the stated environment.
...
- Infracost identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Infracost failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Infracost is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Infracost as the basis for migration or production guidance.
