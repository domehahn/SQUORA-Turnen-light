---
name: "cncf-fd-io-reviewer"
description: "Review FD.io using its official documentation and repository in the Runtime / Cloud Native Network category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate FD.io using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- FD.io code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving FD.io requires evidence-backed assessment.
...
- First-party source: https://github.com/FDio/vpp
...
- [ ] Trace FD.io inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check FD.io defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check FD.io least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check FD.io resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check FD.io release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check FD.io dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If FD.io identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a FD.io change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- FD.io documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a FD.io architecture and dependency summary with trust boundaries and persistent state.
- Include FD.io validation commands or tests that are safe for the stated environment.
...
- FD.io identity, version, configuration, dependencies, and deployment assumptions are explicit.
- FD.io failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that FD.io is secure, supported, compatible, or suitable.
- Using an unpinned latest version of FD.io as the basis for migration or production guidance.
