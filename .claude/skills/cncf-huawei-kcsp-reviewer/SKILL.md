---
name: "cncf-huawei-kcsp-reviewer"
description: "Review Huawei (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. HUAWEI CLOUD provides customers with stable, reliable, secure, and sustainably growing cloud services. It helps large enterprises address challenges in cloud transformation and enables them to take better advantages of potential business opportunities. It also helps small- and medium-sized enterprises expand their business growth and rise to challenges."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Huawei Kcsp Reviewer

## Purpose

Review and operate Huawei (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Huawei (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Huawei (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.huaweicloud.com/intl/en-us/product/cce.html
...
- [ ] Trace Huawei (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Huawei (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Huawei (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Huawei (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Huawei (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Huawei (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Huawei (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Huawei (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Huawei (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Huawei (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Huawei (KCSP) validation commands or tests that are safe for the stated environment.
...
- Huawei (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Huawei (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Huawei (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Huawei (KCSP) as the basis for migration or production guidance.
