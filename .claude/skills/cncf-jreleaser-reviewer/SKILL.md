---
name: "cncf-jreleaser-reviewer"
description: "Review JReleaser using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Jreleaser Reviewer

## Purpose

Review and operate JReleaser using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- JReleaser code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving JReleaser requires evidence-backed assessment.
...
- First-party source: https://github.com/jreleaser/jreleaser
...
- [ ] Trace JReleaser inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check JReleaser defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check JReleaser least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check JReleaser resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check JReleaser release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check JReleaser dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If JReleaser identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a JReleaser change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- JReleaser documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a JReleaser architecture and dependency summary with trust boundaries and persistent state.
- Include JReleaser validation commands or tests that are safe for the stated environment.
...
- JReleaser identity, version, configuration, dependencies, and deployment assumptions are explicit.
- JReleaser failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that JReleaser is secure, supported, compatible, or suitable.
- Using an unpinned latest version of JReleaser as the basis for migration or production guidance.
