...
---
# Cncf Nmstate Reviewer

## Purpose

Review and operate NMstate using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- NMstate code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving NMstate requires evidence-backed assessment.
...
- First-party source: https://github.com/nmstate/nmstate
...
- [ ] Trace NMstate inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check NMstate defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check NMstate least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check NMstate resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check NMstate release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check NMstate dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If NMstate identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a NMstate change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- NMstate documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a NMstate architecture and dependency summary with trust boundaries and persistent state.
- Include NMstate validation commands or tests that are safe for the stated environment.
...
- NMstate identity, version, configuration, dependencies, and deployment assumptions are explicit.
- NMstate failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that NMstate is secure, supported, compatible, or suitable.
- Using an unpinned latest version of NMstate as the basis for migration or production guidance.
