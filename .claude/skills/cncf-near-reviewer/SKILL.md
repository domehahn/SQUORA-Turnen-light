---
name: "cncf-near-reviewer"
description: "Review NEAR using its official documentation and repository in the Wasm / Decentralized Platforms category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate NEAR using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- NEAR code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving NEAR requires evidence-backed assessment.
...
- First-party source: https://github.com/near/nearcore
...
- [ ] Trace NEAR inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check NEAR defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check NEAR least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check NEAR resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check NEAR release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check NEAR dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If NEAR identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a NEAR change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- NEAR documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a NEAR architecture and dependency summary with trust boundaries and persistent state.
- Include NEAR validation commands or tests that are safe for the stated environment.
...
- NEAR identity, version, configuration, dependencies, and deployment assumptions are explicit.
- NEAR failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that NEAR is secure, supported, compatible, or suitable.
- Using an unpinned latest version of NEAR as the basis for migration or production guidance.
