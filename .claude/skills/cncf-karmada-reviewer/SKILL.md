---
name: "cncf-karmada-reviewer"
description: "Review Karmada using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. Open, Multi-Cloud, Multi-Cluster Kubernetes Orchestration"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Karmada Reviewer

## Purpose

Review and operate Karmada using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Karmada code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Karmada requires evidence-backed assessment.
...
- First-party source: https://github.com/karmada-io/karmada
...
- [ ] Trace Karmada inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Karmada defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Karmada least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Karmada resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Karmada release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Karmada dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Karmada identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Karmada change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Karmada documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Karmada architecture and dependency summary with trust boundaries and persistent state.
- Include Karmada validation commands or tests that are safe for the stated environment.
...
- Karmada identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Karmada failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Karmada is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Karmada as the basis for migration or production guidance.
