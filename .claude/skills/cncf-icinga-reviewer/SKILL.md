---
name: "cncf-icinga-reviewer"
description: "Review Icinga using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Icinga Reviewer

## Purpose

Review and operate Icinga using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Icinga code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Icinga requires evidence-backed assessment.
...
- First-party source: https://github.com/Icinga/icinga2
...
- [ ] Trace Icinga inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Icinga defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Icinga least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Icinga resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Icinga release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Icinga dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Icinga identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Icinga change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Icinga documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Icinga architecture and dependency summary with trust boundaries and persistent state.
- Include Icinga validation commands or tests that are safe for the stated environment.
...
- Icinga identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Icinga failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Icinga is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Icinga as the basis for migration or production guidance.
