---
name: "cncf-kwasm-reviewer"
description: "Review Kwasm using its official documentation and repository in the Wasm / Orchestration & Management category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Kwasm using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kwasm code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kwasm requires evidence-backed assessment.
...
- First-party source: https://github.com/KWasm/kwasm-operator
...
- [ ] Trace Kwasm inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kwasm defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kwasm least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kwasm resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kwasm release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kwasm dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kwasm identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kwasm change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kwasm documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kwasm architecture and dependency summary with trust boundaries and persistent state.
- Include Kwasm validation commands or tests that are safe for the stated environment.
...
- Kwasm identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kwasm failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kwasm is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kwasm as the basis for migration or production guidance.
