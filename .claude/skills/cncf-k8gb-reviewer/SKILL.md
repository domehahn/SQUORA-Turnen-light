---
name: "cncf-k8gb-reviewer"
description: "Review k8gb using its official documentation and repository in the Orchestration & Management / Coordination & Service Discovery category. A cloud native Kubernetes Global Balancer"
version: "1.0.0"
since: "2026-08-27"
...
Review and operate k8gb using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- k8gb code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving k8gb requires evidence-backed assessment.
...
- First-party source: https://github.com/k8gb-io/k8gb
...
- [ ] Trace k8gb inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check k8gb defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check k8gb least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check k8gb resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check k8gb release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check k8gb dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If k8gb identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a k8gb change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- k8gb documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a k8gb architecture and dependency summary with trust boundaries and persistent state.
- Include k8gb validation commands or tests that are safe for the stated environment.
...
- k8gb identity, version, configuration, dependencies, and deployment assumptions are explicit.
- k8gb failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that k8gb is secure, supported, compatible, or suitable.
- Using an unpinned latest version of k8gb as the basis for migration or production guidance.
