---
name: "cncf-kuberay-reviewer"
description: "Review Kuberay using its official documentation and repository in the AI Native Infra / Orchestration and Scheduling category. A toolkit to run Ray applications on Kubernetes"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kuberay Reviewer

## Purpose

Review and operate Kuberay using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kuberay code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kuberay requires evidence-backed assessment.
...
- First-party source: https://github.com/ray-project/kuberay
...
- [ ] Trace Kuberay inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kuberay defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kuberay least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kuberay resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kuberay release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kuberay dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kuberay identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kuberay change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kuberay documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kuberay architecture and dependency summary with trust boundaries and persistent state.
- Include Kuberay validation commands or tests that are safe for the stated environment.
...
- Kuberay identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kuberay failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kuberay is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kuberay as the basis for migration or production guidance.
