...
---
# Cncf Opentofu Reviewer

## Purpose

Review and operate OpenTofu using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenTofu code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenTofu requires evidence-backed assessment.
...
- First-party source: https://github.com/opentofu/opentofu
...
- [ ] Trace OpenTofu inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenTofu defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenTofu least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenTofu resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenTofu release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenTofu dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenTofu identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenTofu change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenTofu documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenTofu architecture and dependency summary with trust boundaries and persistent state.
- Include OpenTofu validation commands or tests that are safe for the stated environment.
...
- OpenTofu identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenTofu failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenTofu is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenTofu as the basis for migration or production guidance.
