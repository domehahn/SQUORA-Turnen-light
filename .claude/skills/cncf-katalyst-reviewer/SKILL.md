---
name: "cncf-katalyst-reviewer"
description: "Review Katalyst using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. Katalyst is a QoS-based resource management system for workload colocation on kubernetes"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Katalyst Reviewer

## Purpose

Review and operate Katalyst using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Katalyst code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Katalyst requires evidence-backed assessment.
...
- First-party source: https://github.com/kubewharf/katalyst-core
...
- [ ] Trace Katalyst inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Katalyst defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Katalyst least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Katalyst resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Katalyst release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Katalyst dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Katalyst identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Katalyst change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Katalyst documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Katalyst architecture and dependency summary with trust boundaries and persistent state.
- Include Katalyst validation commands or tests that are safe for the stated environment.
...
- Katalyst identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Katalyst failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Katalyst is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Katalyst as the basis for migration or production guidance.
