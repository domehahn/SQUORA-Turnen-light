---
name: "cncf-wasmedge-runtime-reviewer"
description: "Review WasmEdge Runtime using its official documentation and repository in the Runtime / Container Runtime category. WasmEdge is a lightweight, high-performance, and extensible WebAssembly runtime for cloud native, edge, and decentralized applications. It powers serverless apps, embedded functions, microservices, smart contracts, and IoT devices"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Wasmedge Runtime Reviewer

## Purpose

Review and operate WasmEdge Runtime using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- WasmEdge Runtime code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving WasmEdge Runtime requires evidence-backed assessment.
...
- First-party source: https://github.com/WasmEdge/WasmEdge
...
- [ ] Trace WasmEdge Runtime inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check WasmEdge Runtime defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check WasmEdge Runtime least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check WasmEdge Runtime resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check WasmEdge Runtime release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check WasmEdge Runtime dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If WasmEdge Runtime identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a WasmEdge Runtime change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- WasmEdge Runtime documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a WasmEdge Runtime architecture and dependency summary with trust boundaries and persistent state.
- Include WasmEdge Runtime validation commands or tests that are safe for the stated environment.
...
- WasmEdge Runtime identity, version, configuration, dependencies, and deployment assumptions are explicit.
- WasmEdge Runtime failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that WasmEdge Runtime is secure, supported, compatible, or suitable.
- Using an unpinned latest version of WasmEdge Runtime as the basis for migration or production guidance.
