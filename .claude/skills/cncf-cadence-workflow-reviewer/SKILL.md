---
name: "cncf-cadence-workflow-reviewer"
description: "Review Cadence Workflow using its official documentation and repository in the Provisioning / Automation & Configuration category. Cadence is a distributed, scalable, durable, and highly available fault-oblivious stateful code platform."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Cadence Workflow Reviewer

## Purpose

Review and operate Cadence Workflow using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Cadence Workflow code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Cadence Workflow requires evidence-backed assessment.
...
- First-party source: https://github.com/cadence-workflow/cadence
...
- [ ] Trace Cadence Workflow inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Cadence Workflow defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Cadence Workflow least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Cadence Workflow resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Cadence Workflow release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Cadence Workflow dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Cadence Workflow identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Cadence Workflow change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Cadence Workflow documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Cadence Workflow architecture and dependency summary with trust boundaries and persistent state.
- Include Cadence Workflow validation commands or tests that are safe for the stated environment.
...
- Cadence Workflow identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Cadence Workflow failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Cadence Workflow is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Cadence Workflow as the basis for migration or production guidance.
