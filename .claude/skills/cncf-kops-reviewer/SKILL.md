---
name: "cncf-kops-reviewer"
description: "Review kOps using its official documentation and repository in the Platform / Certified Kubernetes - Installer category. kOps is open-source, community-maintained tooling for production-grade Kubernetes cluster installation, management and upgrades."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate kOps using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- kOps code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving kOps requires evidence-backed assessment.
...
- First-party source: https://github.com/kubernetes/kops
...
- [ ] Trace kOps inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check kOps defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check kOps least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check kOps resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check kOps release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check kOps dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If kOps identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a kOps change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- kOps documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a kOps architecture and dependency summary with trust boundaries and persistent state.
- Include kOps validation commands or tests that are safe for the stated environment.
...
- kOps identity, version, configuration, dependencies, and deployment assumptions are explicit.
- kOps failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that kOps is secure, supported, compatible, or suitable.
- Using an unpinned latest version of kOps as the basis for migration or production guidance.
