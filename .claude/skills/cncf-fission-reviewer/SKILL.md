---
name: "cncf-fission-reviewer"
description: "Review Fission using its official documentation and repository in the Serverless / Installable Platform category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Fission Reviewer

## Purpose

Review and operate Fission using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Fission code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Fission requires evidence-backed assessment.
...
- First-party source: https://github.com/fission/fission
...
- [ ] Trace Fission inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Fission defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Fission least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Fission resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Fission release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Fission dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Fission identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Fission change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Fission documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Fission architecture and dependency summary with trust boundaries and persistent state.
- Include Fission validation commands or tests that are safe for the stated environment.
...
- Fission identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Fission failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Fission is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Fission as the basis for migration or production guidance.
