---
name: "cncf-containerized-routing-protocol-daemon-reviewer"
description: "Review Containerized Routing Protocol Daemon using its official documentation and repository in the Special / Certified CNFs category. The Junos® containerized routing protocol daemon (cRPD) offers deployment-hardened feature-rich routing functionality in a container for cloud-native deployments."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Containerized Routing Protocol Daemon Reviewer

## Purpose

Review and operate Containerized Routing Protocol Daemon using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Containerized Routing Protocol Daemon code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Containerized Routing Protocol Daemon requires evidence-backed assessment.
...
- First-party source: http://www.juniper.net/
...
- [ ] Trace Containerized Routing Protocol Daemon inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Containerized Routing Protocol Daemon defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Containerized Routing Protocol Daemon least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Containerized Routing Protocol Daemon resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Containerized Routing Protocol Daemon release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Containerized Routing Protocol Daemon dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Containerized Routing Protocol Daemon identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Containerized Routing Protocol Daemon change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Containerized Routing Protocol Daemon documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Containerized Routing Protocol Daemon architecture and dependency summary with trust boundaries and persistent state.
- Include Containerized Routing Protocol Daemon validation commands or tests that are safe for the stated environment.
...
- Containerized Routing Protocol Daemon identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Containerized Routing Protocol Daemon failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Containerized Routing Protocol Daemon is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Containerized Routing Protocol Daemon as the basis for migration or production guidance.
