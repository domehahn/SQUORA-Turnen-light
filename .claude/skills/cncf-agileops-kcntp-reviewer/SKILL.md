---
name: "cncf-agileops-kcntp-reviewer"
description: "Review AgileOps (KCNTP) using its official documentation and repository in the Special / Kubernetes and Cloud Native Training Partner category. AgileOps is a recognized Kubernetes Certified Service Provider (KCSP) and Kubernetes and Cloud Native Training Partner (KCNTP) in Vietnam. Our certified instructors bring real-world, hands-on experience to deliver world-class Kubernetes and DevSecOps training programs."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Agileops Kcntp Reviewer

## Purpose

Review and operate AgileOps (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- AgileOps (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving AgileOps (KCNTP) requires evidence-backed assessment.
...
- First-party source: https://agileops.vn/en/services/kubernetes
...
- [ ] Trace AgileOps (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check AgileOps (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check AgileOps (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check AgileOps (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check AgileOps (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check AgileOps (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If AgileOps (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a AgileOps (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- AgileOps (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a AgileOps (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include AgileOps (KCNTP) validation commands or tests that are safe for the stated environment.
...
- AgileOps (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- AgileOps (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that AgileOps (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of AgileOps (KCNTP) as the basis for migration or production guidance.
