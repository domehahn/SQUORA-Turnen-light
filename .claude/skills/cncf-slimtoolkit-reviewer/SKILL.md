---
name: "cncf-slimtoolkit-reviewer"
description: "Review SlimToolkit using its official documentation and repository in the Provisioning / Security & Compliance category. Inspect, Optimize and Debug Your Containers"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Slimtoolkit Reviewer

## Purpose

Review and operate SlimToolkit using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SlimToolkit code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SlimToolkit requires evidence-backed assessment.
...
- First-party source: https://github.com/slimtoolkit/slim
...
- [ ] Trace SlimToolkit inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SlimToolkit defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SlimToolkit least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SlimToolkit resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SlimToolkit release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SlimToolkit dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SlimToolkit identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SlimToolkit change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SlimToolkit documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SlimToolkit architecture and dependency summary with trust boundaries and persistent state.
- Include SlimToolkit validation commands or tests that are safe for the stated environment.
...
- SlimToolkit identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SlimToolkit failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SlimToolkit is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SlimToolkit as the basis for migration or production guidance.
