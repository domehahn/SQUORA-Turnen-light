...
---
# Cncf Oracle Cloud Native Environment Reviewer

## Purpose

Review and operate Oracle Cloud Native Environment using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Oracle Cloud Native Environment code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Oracle Cloud Native Environment requires evidence-backed assessment.
...
- First-party source: https://github.com/oracle-cne/ocne
...
- [ ] Trace Oracle Cloud Native Environment inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Oracle Cloud Native Environment defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Oracle Cloud Native Environment least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Oracle Cloud Native Environment resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Oracle Cloud Native Environment release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Oracle Cloud Native Environment dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Oracle Cloud Native Environment identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Oracle Cloud Native Environment change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Oracle Cloud Native Environment documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Oracle Cloud Native Environment architecture and dependency summary with trust boundaries and persistent state.
- Include Oracle Cloud Native Environment validation commands or tests that are safe for the stated environment.
...
- Oracle Cloud Native Environment identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Oracle Cloud Native Environment failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Oracle Cloud Native Environment is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Oracle Cloud Native Environment as the basis for migration or production guidance.
