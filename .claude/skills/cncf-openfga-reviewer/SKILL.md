---
name: "cncf-openfga-reviewer"
description: "Review OpenFGA using its official documentation and repository in the Provisioning / Security & Compliance category. OpenFGA is a high performance and flexible authorization/permission system built for developers and inspired by Google Zanzibar"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Openfga Reviewer

## Purpose

Review and operate OpenFGA using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenFGA code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenFGA requires evidence-backed assessment.
...
- First-party source: https://github.com/openfga/openfga
...
- [ ] Trace OpenFGA inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenFGA defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenFGA least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenFGA resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenFGA release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenFGA dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenFGA identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenFGA change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenFGA documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenFGA architecture and dependency summary with trust boundaries and persistent state.
- Include OpenFGA validation commands or tests that are safe for the stated environment.
...
- OpenFGA identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenFGA failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenFGA is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenFGA as the basis for migration or production guidance.
