---
name: "cncf-aeraki-mesh-reviewer"
description: "Review Aeraki Mesh using its official documentation and repository in the Orchestration & Management / Service Mesh category. Aeraki Mesh allows you to manage any layer-7 traffic in a service mesh"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Aeraki Mesh Reviewer

## Purpose

Review and operate Aeraki Mesh using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Aeraki Mesh code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Aeraki Mesh requires evidence-backed assessment.
...
- First-party source: https://github.com/aeraki-mesh/aeraki
...
- [ ] Trace Aeraki Mesh inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Aeraki Mesh defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Aeraki Mesh least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Aeraki Mesh resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Aeraki Mesh release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Aeraki Mesh dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Aeraki Mesh identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Aeraki Mesh change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Aeraki Mesh documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Aeraki Mesh architecture and dependency summary with trust boundaries and persistent state.
- Include Aeraki Mesh validation commands or tests that are safe for the stated environment.
...
- Aeraki Mesh identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Aeraki Mesh failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Aeraki Mesh is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Aeraki Mesh as the basis for migration or production guidance.
