...
---
# Cncf Databuff Reviewer

## Purpose

Review and operate DataBuff using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DataBuff code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DataBuff requires evidence-backed assessment.
...
- First-party source: https://github.com/databufflabs/databuff
...
- [ ] Trace DataBuff inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DataBuff defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DataBuff least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DataBuff resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DataBuff release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DataBuff dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DataBuff identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DataBuff change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DataBuff documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DataBuff architecture and dependency summary with trust boundaries and persistent state.
- Include DataBuff validation commands or tests that are safe for the stated environment.
...
- DataBuff identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DataBuff failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DataBuff is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DataBuff as the basis for migration or production guidance.
