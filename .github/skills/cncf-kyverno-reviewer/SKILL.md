---
name: "cncf-kyverno-reviewer"
description: "Review Kyverno using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kyverno Reviewer

## Purpose

Review and operate Kyverno using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kyverno code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kyverno requires evidence-backed assessment.
...
- First-party source: https://github.com/kyverno/kyverno
...
- [ ] Trace Kyverno inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kyverno defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kyverno least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kyverno resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kyverno release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kyverno dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kyverno identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kyverno change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kyverno documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kyverno architecture and dependency summary with trust boundaries and persistent state.
- Include Kyverno validation commands or tests that are safe for the stated environment.
...
- Kyverno identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kyverno failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kyverno is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kyverno as the basis for migration or production guidance.
