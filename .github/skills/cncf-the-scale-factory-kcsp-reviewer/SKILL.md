---
name: "cncf-the-scale-factory-kcsp-reviewer"
description: "Review The Scale Factory (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. We help technology teams deliver great cloud native solutions on AWS."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf The Scale Factory Kcsp Reviewer

## Purpose

Review and operate The Scale Factory (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- The Scale Factory (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving The Scale Factory (KCSP) requires evidence-backed assessment.
...
- [ ] Trace The Scale Factory (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check The Scale Factory (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check The Scale Factory (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check The Scale Factory (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check The Scale Factory (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check The Scale Factory (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If The Scale Factory (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a The Scale Factory (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- The Scale Factory (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a The Scale Factory (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include The Scale Factory (KCSP) validation commands or tests that are safe for the stated environment.
...
- The Scale Factory (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- The Scale Factory (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that The Scale Factory (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of The Scale Factory (KCSP) as the basis for migration or production guidance.
