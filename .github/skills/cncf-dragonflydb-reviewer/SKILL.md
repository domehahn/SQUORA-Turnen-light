---
name: "cncf-dragonflydb-reviewer"
...
Review and operate DragonflyDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DragonflyDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DragonflyDB requires evidence-backed assessment.
...
- First-party source: https://github.com/dragonflydb/dragonfly
...
- [ ] Trace DragonflyDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DragonflyDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DragonflyDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DragonflyDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DragonflyDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DragonflyDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DragonflyDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DragonflyDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DragonflyDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DragonflyDB architecture and dependency summary with trust boundaries and persistent state.
- Include DragonflyDB validation commands or tests that are safe for the stated environment.
...
- DragonflyDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DragonflyDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DragonflyDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DragonflyDB as the basis for migration or production guidance.
