---
name: "cncf-webiny-reviewer"
description: "Review Webiny using its official documentation and repository in the Serverless / Framework category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Webiny Reviewer

## Purpose

Review and operate Webiny using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Webiny code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Webiny requires evidence-backed assessment.
...
- First-party source: https://github.com/webiny/webiny-js
...
- [ ] Trace Webiny inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Webiny defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Webiny least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Webiny resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Webiny release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Webiny dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Webiny identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Webiny change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Webiny documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Webiny architecture and dependency summary with trust boundaries and persistent state.
- Include Webiny validation commands or tests that are safe for the stated environment.
...
- Webiny identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Webiny failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Webiny is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Webiny as the basis for migration or production guidance.
