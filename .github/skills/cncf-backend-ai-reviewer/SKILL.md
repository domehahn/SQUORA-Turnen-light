...
---
# Cncf Backend Ai Reviewer

## Purpose

Review and operate Backend.AI using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Backend.AI code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Backend.AI requires evidence-backed assessment.
...
- First-party source: https://github.com/lablup/backend.ai
...
- [ ] Trace Backend.AI inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Backend.AI defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Backend.AI least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Backend.AI resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Backend.AI release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Backend.AI dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Backend.AI identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Backend.AI change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Backend.AI documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Backend.AI architecture and dependency summary with trust boundaries and persistent state.
- Include Backend.AI validation commands or tests that are safe for the stated environment.
...
- Backend.AI identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Backend.AI failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Backend.AI is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Backend.AI as the basis for migration or production guidance.
