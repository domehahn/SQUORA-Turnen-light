---
name: "cncf-apolicy-reviewer"
description: "Review Apolicy using its official documentation and repository in the Provisioning / Security & Compliance category. Apolicy fuses security and compliance into your cloud native development pipeline. Using policy-as-code,  we automate risk, policy and remediation from source to production"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Apolicy Reviewer

## Purpose

Review and operate Apolicy using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Apolicy code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Apolicy requires evidence-backed assessment.
...
- First-party source: https://www.apolicy.io/
...
- [ ] Trace Apolicy inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Apolicy defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Apolicy least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Apolicy resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Apolicy release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Apolicy dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Apolicy identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Apolicy change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Apolicy documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Apolicy architecture and dependency summary with trust boundaries and persistent state.
- Include Apolicy validation commands or tests that are safe for the stated environment.
...
- Apolicy identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Apolicy failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Apolicy is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Apolicy as the basis for migration or production guidance.
