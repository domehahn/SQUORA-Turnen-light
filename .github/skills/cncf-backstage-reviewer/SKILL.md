---
name: "cncf-backstage-reviewer"
description: "Review Backstage using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Backstage Reviewer

## Purpose

Review and operate Backstage using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Backstage code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Backstage requires evidence-backed assessment.
...
- First-party source: https://github.com/backstage/backstage
...
- [ ] Trace Backstage inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Backstage defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Backstage least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Backstage resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Backstage release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Backstage dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Backstage identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Backstage change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Backstage documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Backstage architecture and dependency summary with trust boundaries and persistent state.
- Include Backstage validation commands or tests that are safe for the stated environment.
...
- Backstage identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Backstage failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Backstage is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Backstage as the basis for migration or production guidance.
