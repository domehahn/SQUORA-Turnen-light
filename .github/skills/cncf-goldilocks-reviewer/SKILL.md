---
name: "cncf-goldilocks-reviewer"
description: "Review Goldilocks using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Goldilocks Reviewer

## Purpose

Review and operate Goldilocks using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Goldilocks code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Goldilocks requires evidence-backed assessment.
...
- First-party source: https://github.com/FairwindsOps/goldilocks
...
- [ ] Trace Goldilocks inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Goldilocks defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Goldilocks least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Goldilocks resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Goldilocks release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Goldilocks dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Goldilocks identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Goldilocks change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Goldilocks documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Goldilocks architecture and dependency summary with trust boundaries and persistent state.
- Include Goldilocks validation commands or tests that are safe for the stated environment.
...
- Goldilocks identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Goldilocks failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Goldilocks is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Goldilocks as the basis for migration or production guidance.
