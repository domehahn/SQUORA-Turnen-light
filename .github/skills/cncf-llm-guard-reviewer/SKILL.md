---
name: "cncf-llm-guard-reviewer"
description: "Review LLM Guard using its official documentation and repository in the AI Agent / Guardrail category. The Security Toolkit for LLM Interactions"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Llm Guard Reviewer

## Purpose

Review and operate LLM Guard using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- LLM Guard code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving LLM Guard requires evidence-backed assessment.
...
- First-party source: https://github.com/protectai/llm-guard
...
- [ ] Trace LLM Guard inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check LLM Guard defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check LLM Guard least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check LLM Guard resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check LLM Guard release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check LLM Guard dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If LLM Guard identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a LLM Guard change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- LLM Guard documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a LLM Guard architecture and dependency summary with trust boundaries and persistent state.
- Include LLM Guard validation commands or tests that are safe for the stated environment.
...
- LLM Guard identity, version, configuration, dependencies, and deployment assumptions are explicit.
- LLM Guard failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that LLM Guard is secure, supported, compatible, or suitable.
- Using an unpinned latest version of LLM Guard as the basis for migration or production guidance.
