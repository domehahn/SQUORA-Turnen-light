---
name: "cncf-platform9-kcsp-reviewer"
description: "Review Platform9 (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Platform9 Managed Kubernetes is SaaS-managed, infrastructure-agnostic, and works across public clouds, on-premises servers, and VMware infrastructure."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Platform9 Kcsp Reviewer

## Purpose

Review and operate Platform9 (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Platform9 (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Platform9 (KCSP) requires evidence-backed assessment.
...
- [ ] Trace Platform9 (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Platform9 (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Platform9 (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Platform9 (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Platform9 (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Platform9 (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Platform9 (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Platform9 (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Platform9 (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Platform9 (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Platform9 (KCSP) validation commands or tests that are safe for the stated environment.
...
- Platform9 (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Platform9 (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Platform9 (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Platform9 (KCSP) as the basis for migration or production guidance.
