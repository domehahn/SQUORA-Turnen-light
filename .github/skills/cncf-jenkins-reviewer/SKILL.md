---
name: "cncf-jenkins-reviewer"
description: "Review Jenkins using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Jenkins Reviewer

## Purpose

Review and operate Jenkins using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Jenkins code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Jenkins requires evidence-backed assessment.
...
- First-party source: https://github.com/jenkinsci/jenkins
...
- [ ] Trace Jenkins inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Jenkins defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Jenkins least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Jenkins resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Jenkins release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Jenkins dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Jenkins identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Jenkins change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Jenkins documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Jenkins architecture and dependency summary with trust boundaries and persistent state.
- Include Jenkins validation commands or tests that are safe for the stated environment.
...
- Jenkins identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Jenkins failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Jenkins is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Jenkins as the basis for migration or production guidance.
