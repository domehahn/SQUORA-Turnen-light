---
name: "cncf-fonio-reviewer"
description: "Review Fonio using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Fonio using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Fonio code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Fonio requires evidence-backed assessment.
...
- First-party source: https://github.com/foniod/foniod
...
- [ ] Trace Fonio inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Fonio defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Fonio least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Fonio resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Fonio release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Fonio dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Fonio identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Fonio change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Fonio documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Fonio architecture and dependency summary with trust boundaries and persistent state.
- Include Fonio validation commands or tests that are safe for the stated environment.
...
- Fonio identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Fonio failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Fonio is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Fonio as the basis for migration or production guidance.
