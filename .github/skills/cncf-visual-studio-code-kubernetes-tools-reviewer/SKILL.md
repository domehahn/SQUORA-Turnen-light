...
---
# Cncf Visual Studio Code Kubernetes Tools Reviewer

## Purpose

Review and operate Visual Studio Code Kubernetes Tools using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Visual Studio Code Kubernetes Tools code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Visual Studio Code Kubernetes Tools requires evidence-backed assessment.
...
- First-party source: https://github.com/vscode-kubernetes-tools/vscode-kubernetes-tools
...
- [ ] Trace Visual Studio Code Kubernetes Tools inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Visual Studio Code Kubernetes Tools defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Visual Studio Code Kubernetes Tools least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Visual Studio Code Kubernetes Tools resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Visual Studio Code Kubernetes Tools release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Visual Studio Code Kubernetes Tools dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Visual Studio Code Kubernetes Tools identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Visual Studio Code Kubernetes Tools change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Visual Studio Code Kubernetes Tools documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Visual Studio Code Kubernetes Tools architecture and dependency summary with trust boundaries and persistent state.
- Include Visual Studio Code Kubernetes Tools validation commands or tests that are safe for the stated environment.
...
- Visual Studio Code Kubernetes Tools identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Visual Studio Code Kubernetes Tools failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Visual Studio Code Kubernetes Tools is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Visual Studio Code Kubernetes Tools as the basis for migration or production guidance.
