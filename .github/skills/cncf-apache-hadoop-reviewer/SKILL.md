---
name: "cncf-apache-hadoop-reviewer"
description: "Review Apache Hadoop using its official documentation and repository in the App Definition and Development / Database category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Apache Hadoop Reviewer

## Purpose

Review and operate Apache Hadoop using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Apache Hadoop code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Apache Hadoop requires evidence-backed assessment.
...
- [ ] Trace Apache Hadoop inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Apache Hadoop defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Apache Hadoop least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Apache Hadoop resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Apache Hadoop release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Apache Hadoop dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Apache Hadoop identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Apache Hadoop change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Apache Hadoop documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Apache Hadoop architecture and dependency summary with trust boundaries and persistent state.
- Include Apache Hadoop validation commands or tests that are safe for the stated environment.
...
- Apache Hadoop identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Apache Hadoop failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Apache Hadoop is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Apache Hadoop as the basis for migration or production guidance.
