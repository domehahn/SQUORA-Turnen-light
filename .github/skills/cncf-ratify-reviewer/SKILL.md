---
name: "cncf-ratify-reviewer"
description: "Review Ratify using its official documentation and repository in the Provisioning / Security & Compliance category. A verification engine on Kubernetes which enables verification of artifact security metadata and admits for deployment only those that comply with policies you create."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Ratify Reviewer

## Purpose

Review and operate Ratify using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Ratify code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Ratify requires evidence-backed assessment.
...
- First-party source: https://github.com/ratify-project/ratify
...
- [ ] Trace Ratify inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Ratify defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Ratify least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Ratify resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Ratify release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Ratify dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Ratify identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Ratify change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Ratify documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Ratify architecture and dependency summary with trust boundaries and persistent state.
- Include Ratify validation commands or tests that are safe for the stated environment.
...
- Ratify identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Ratify failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Ratify is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Ratify as the basis for migration or production guidance.
