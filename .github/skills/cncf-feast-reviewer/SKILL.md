---
name: "cncf-feast-reviewer"
description: "Review FEAST using its official documentation and repository in the Data / Data Architecture category. Feature Store to manage machine learning features."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate FEAST using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- FEAST code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving FEAST requires evidence-backed assessment.
...
- First-party source: https://github.com/feast-dev/feast
...
- [ ] Trace FEAST inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check FEAST defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check FEAST least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check FEAST resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check FEAST release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check FEAST dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If FEAST identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a FEAST change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- FEAST documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a FEAST architecture and dependency summary with trust boundaries and persistent state.
- Include FEAST validation commands or tests that are safe for the stated environment.
...
- FEAST identity, version, configuration, dependencies, and deployment assumptions are explicit.
- FEAST failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that FEAST is secure, supported, compatible, or suitable.
- Using an unpinned latest version of FEAST as the basis for migration or production guidance.
