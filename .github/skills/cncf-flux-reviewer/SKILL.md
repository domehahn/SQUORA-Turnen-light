---
name: "cncf-flux-reviewer"
description: "Review Flux using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Flux using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Flux code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Flux requires evidence-backed assessment.
...
- First-party source: https://github.com/fluxcd/flux2
...
- [ ] Trace Flux inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Flux defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Flux least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Flux resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Flux release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Flux dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Flux identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Flux change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Flux documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Flux architecture and dependency summary with trust boundaries and persistent state.
- Include Flux validation commands or tests that are safe for the stated environment.
...
- Flux identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Flux failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Flux is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Flux as the basis for migration or production guidance.
