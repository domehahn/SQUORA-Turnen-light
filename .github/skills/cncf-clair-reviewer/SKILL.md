---
name: "cncf-clair-reviewer"
description: "Review Clair using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Clair using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Clair code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Clair requires evidence-backed assessment.
...
- First-party source: https://github.com/quay/clair
...
- [ ] Trace Clair inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Clair defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Clair least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Clair resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Clair release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Clair dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Clair identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Clair change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Clair documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Clair architecture and dependency summary with trust boundaries and persistent state.
- Include Clair validation commands or tests that are safe for the stated environment.
...
- Clair identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Clair failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Clair is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Clair as the basis for migration or production guidance.
