---
name: "cncf-vitess-reviewer"
description: "Review Vitess using its official documentation and repository in the App Definition and Development / Database category. MySQL-compatible, horizontally scalable, cloud-native database solution."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Vitess Reviewer

## Purpose

Review and operate Vitess using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Vitess code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Vitess requires evidence-backed assessment.
...
- First-party source: https://github.com/vitessio/vitess
...
- [ ] Trace Vitess inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Vitess defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Vitess least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Vitess resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Vitess release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Vitess dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Vitess identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Vitess change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Vitess documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Vitess architecture and dependency summary with trust boundaries and persistent state.
- Include Vitess validation commands or tests that are safe for the stated environment.
...
- Vitess identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Vitess failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Vitess is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Vitess as the basis for migration or production guidance.
