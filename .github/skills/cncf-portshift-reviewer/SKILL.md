---
name: "cncf-portshift-reviewer"
description: "Review Portshift using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Portshift Reviewer

## Purpose

Review and operate Portshift using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Portshift code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Portshift requires evidence-backed assessment.
...
- First-party source: https://github.com/Portshift
...
- [ ] Trace Portshift inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Portshift defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Portshift least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Portshift resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Portshift release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Portshift dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Portshift identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Portshift change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Portshift documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Portshift architecture and dependency summary with trust boundaries and persistent state.
- Include Portshift validation commands or tests that are safe for the stated environment.
...
- Portshift identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Portshift failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Portshift is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Portshift as the basis for migration or production guidance.
