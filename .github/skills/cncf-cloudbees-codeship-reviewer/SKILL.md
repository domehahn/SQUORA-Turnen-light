---
name: "cncf-cloudbees-codeship-reviewer"
description: "Review Cloudbees Codeship using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category. Ship faster with CI/CD as a Service"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Cloudbees Codeship Reviewer

## Purpose

Review and operate Cloudbees Codeship using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Cloudbees Codeship code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Cloudbees Codeship requires evidence-backed assessment.
...
- CNCF classification: App Definition and Development / Continuous Integration & Delivery
- CNCF project maturity field: not specified
- First-party source: https://www.cloudbees.com/products/codeship/overview
...
- [ ] Trace Cloudbees Codeship inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Cloudbees Codeship defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Cloudbees Codeship least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Cloudbees Codeship resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Cloudbees Codeship release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Cloudbees Codeship dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Cloudbees Codeship identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Cloudbees Codeship change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Cloudbees Codeship documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Cloudbees Codeship architecture and dependency summary with trust boundaries and persistent state.
- Include Cloudbees Codeship validation commands or tests that are safe for the stated environment.
...
- Cloudbees Codeship identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Cloudbees Codeship failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Cloudbees Codeship is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Cloudbees Codeship as the basis for migration or production guidance.
