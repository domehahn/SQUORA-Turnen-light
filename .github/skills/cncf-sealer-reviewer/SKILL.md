---
name: "cncf-sealer-reviewer"
description: "Review sealer using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Sealer Reviewer

## Purpose

Review and operate sealer using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- sealer code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving sealer requires evidence-backed assessment.
...
- First-party source: https://github.com/sealerio/sealer
...
- [ ] Trace sealer inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check sealer defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check sealer least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check sealer resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check sealer release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check sealer dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If sealer identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a sealer change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- sealer documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a sealer architecture and dependency summary with trust boundaries and persistent state.
- Include sealer validation commands or tests that are safe for the stated environment.
...
- sealer identity, version, configuration, dependencies, and deployment assumptions are explicit.
- sealer failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that sealer is secure, supported, compatible, or suitable.
- Using an unpinned latest version of sealer as the basis for migration or production guidance.
