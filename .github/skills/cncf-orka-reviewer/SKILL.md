---
name: "cncf-orka-reviewer"
description: "Review Orka using its official documentation and repository in the Platform / Certified Kubernetes - Hosted category. Orka is a platform that allows Orchestration via Kubernetes on Apple. It allows MacOS on genuine Apple hardware to be used in Kubernetes."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Orka using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Orka code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Orka requires evidence-backed assessment.
...
- First-party source: https://www.macstadium.com/orka
...
- [ ] Trace Orka inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Orka defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Orka least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Orka resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Orka release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Orka dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Orka identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Orka change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Orka documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Orka architecture and dependency summary with trust boundaries and persistent state.
- Include Orka validation commands or tests that are safe for the stated environment.
...
- Orka identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Orka failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Orka is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Orka as the basis for migration or production guidance.
