---
name: "cncf-llama-2-reviewer"
description: "Review Llama 2 using its official documentation and repository in the Wasm / AI/Machine Learning category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Llama 2 Reviewer

## Purpose

Review and operate Llama 2 using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Llama 2 code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Llama 2 requires evidence-backed assessment.
...
- First-party source: https://ai.meta.com/llama/
...
- [ ] Trace Llama 2 inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Llama 2 defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Llama 2 least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Llama 2 resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Llama 2 release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Llama 2 dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Llama 2 identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Llama 2 change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Llama 2 documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Llama 2 architecture and dependency summary with trust boundaries and persistent state.
- Include Llama 2 validation commands or tests that are safe for the stated environment.
...
- Llama 2 identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Llama 2 failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Llama 2 is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Llama 2 as the basis for migration or production guidance.
