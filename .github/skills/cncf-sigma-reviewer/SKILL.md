---
name: "cncf-sigma-reviewer"
description: "Review Sigma using its official documentation and repository in the Serverless / Tools category. Sigma is a serverless application developer tool; a cloud IDE, which helps you rapidly build, test and deploy serverless applications."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Sigma using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Sigma code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Sigma requires evidence-backed assessment.
...
- First-party source: https://slappforge.github.io/sigma/
...
- [ ] Trace Sigma inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Sigma defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Sigma least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Sigma resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Sigma release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Sigma dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Sigma identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Sigma change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Sigma documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Sigma architecture and dependency summary with trust boundaries and persistent state.
- Include Sigma validation commands or tests that are safe for the stated environment.
...
- Sigma identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Sigma failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Sigma is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Sigma as the basis for migration or production guidance.
