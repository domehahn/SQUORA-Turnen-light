---
name: "cncf-firecracker-reviewer"
description: "Review Firecracker using its official documentation and repository in the Runtime / Container Runtime category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Firecracker Reviewer

## Purpose

Review and operate Firecracker using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Firecracker code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Firecracker requires evidence-backed assessment.
...
- First-party source: https://github.com/firecracker-microvm/firecracker
...
- [ ] Trace Firecracker inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Firecracker defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Firecracker least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Firecracker resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Firecracker release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Firecracker dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Firecracker identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Firecracker change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Firecracker documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Firecracker architecture and dependency summary with trust boundaries and persistent state.
- Include Firecracker validation commands or tests that are safe for the stated environment.
...
- Firecracker identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Firecracker failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Firecracker is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Firecracker as the basis for migration or production guidance.
