---
name: "cncf-manageiq-reviewer"
description: "Review ManageIQ using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Manageiq Reviewer

## Purpose

Review and operate ManageIQ using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ManageIQ code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ManageIQ requires evidence-backed assessment.
...
- First-party source: https://github.com/ManageIQ/manageiq
...
- [ ] Trace ManageIQ inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ManageIQ defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ManageIQ least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ManageIQ resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ManageIQ release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ManageIQ dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ManageIQ identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ManageIQ change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ManageIQ documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ManageIQ architecture and dependency summary with trust boundaries and persistent state.
- Include ManageIQ validation commands or tests that are safe for the stated environment.
...
- ManageIQ identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ManageIQ failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ManageIQ is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ManageIQ as the basis for migration or production guidance.
