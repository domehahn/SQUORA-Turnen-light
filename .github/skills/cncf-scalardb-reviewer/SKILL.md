---
name: "cncf-scalardb-reviewer"
description: "Review ScalarDB using its official documentation and repository in the App Definition and Development / Database category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Scalardb Reviewer

## Purpose

Review and operate ScalarDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ScalarDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ScalarDB requires evidence-backed assessment.
...
- First-party source: https://github.com/scalar-labs/scalardb
...
- [ ] Trace ScalarDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ScalarDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ScalarDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ScalarDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ScalarDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ScalarDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ScalarDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ScalarDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ScalarDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ScalarDB architecture and dependency summary with trust boundaries and persistent state.
- Include ScalarDB validation commands or tests that are safe for the stated environment.
...
- ScalarDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ScalarDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ScalarDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ScalarDB as the basis for migration or production guidance.
