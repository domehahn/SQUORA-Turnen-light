---
name: "cncf-oteemo-kcntp-reviewer"
description: "Review Oteemo (KCNTP) using its official documentation and repository in the Special / Kubernetes and Cloud Native Training Partner category. Oteemo is a technology solutions provider specializing in helping enterprises with cloud native transformations. Our services include operationalization of Kubernetes platforms, application container migrations and mentoring of client staff on cloud native technologies."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Oteemo Kcntp Reviewer

## Purpose

Review and operate Oteemo (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Oteemo (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Oteemo (KCNTP) requires evidence-backed assessment.
...
- [ ] Trace Oteemo (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Oteemo (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Oteemo (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Oteemo (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Oteemo (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Oteemo (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Oteemo (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Oteemo (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Oteemo (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Oteemo (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include Oteemo (KCNTP) validation commands or tests that are safe for the stated environment.
...
- Oteemo (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Oteemo (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Oteemo (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Oteemo (KCNTP) as the basis for migration or production guidance.
