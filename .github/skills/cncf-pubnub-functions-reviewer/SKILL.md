---
name: "cncf-pubnub-functions-reviewer"
description: "Review PubNub Functions using its official documentation and repository in the Serverless / Hosted Platform category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Pubnub Functions Reviewer

## Purpose

Review and operate PubNub Functions using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- PubNub Functions code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving PubNub Functions requires evidence-backed assessment.
...
- First-party source: https://www.pubnub.com/products/functions/
...
- [ ] Trace PubNub Functions inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check PubNub Functions defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check PubNub Functions least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check PubNub Functions resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check PubNub Functions release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check PubNub Functions dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If PubNub Functions identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a PubNub Functions change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- PubNub Functions documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a PubNub Functions architecture and dependency summary with trust boundaries and persistent state.
- Include PubNub Functions validation commands or tests that are safe for the stated environment.
...
- PubNub Functions identity, version, configuration, dependencies, and deployment assumptions are explicit.
- PubNub Functions failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that PubNub Functions is secure, supported, compatible, or suitable.
- Using an unpinned latest version of PubNub Functions as the basis for migration or production guidance.
