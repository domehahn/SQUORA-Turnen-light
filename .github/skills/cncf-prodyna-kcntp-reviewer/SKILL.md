---
name: "cncf-prodyna-kcntp-reviewer"
description: "Review PRODYNA (KCNTP) using its official documentation and repository in the Special / Kubernetes and Cloud Native Training Partner category. PRODYNA designs, implements, and operates Kubernetes-based custom applications for mid- to large size enterprises in all of Europe."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Prodyna Kcntp Reviewer

## Purpose

Review and operate PRODYNA (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- PRODYNA (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving PRODYNA (KCNTP) requires evidence-backed assessment.
...
- First-party source: https://www.prodyna.com/expertises/it-training-certifications
...
- [ ] Trace PRODYNA (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check PRODYNA (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check PRODYNA (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check PRODYNA (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check PRODYNA (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check PRODYNA (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If PRODYNA (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a PRODYNA (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- PRODYNA (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a PRODYNA (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include PRODYNA (KCNTP) validation commands or tests that are safe for the stated environment.
...
- PRODYNA (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- PRODYNA (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that PRODYNA (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of PRODYNA (KCNTP) as the basis for migration or production guidance.
