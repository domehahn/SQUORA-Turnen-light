---
name: "cncf-huawei-reviewer"
description: "Review Huawei using its official documentation and repository in the Runtime / Cloud Native Storage category. Ubiquitous Storage Unleashes Data Power"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Huawei Reviewer

## Purpose

Review and operate Huawei using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Huawei code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Huawei requires evidence-backed assessment.
...
- First-party source: https://e.huawei.com/en/products/storage
...
- [ ] Trace Huawei inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Huawei defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Huawei least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Huawei resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Huawei release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Huawei dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Huawei identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Huawei change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Huawei documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Huawei architecture and dependency summary with trust boundaries and persistent state.
- Include Huawei validation commands or tests that are safe for the stated environment.
...
- Huawei identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Huawei failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Huawei is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Huawei as the basis for migration or production guidance.
