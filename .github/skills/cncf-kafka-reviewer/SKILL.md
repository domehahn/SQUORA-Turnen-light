---
name: "cncf-kafka-reviewer"
description: "Review Kafka using its official documentation and repository in the App Definition and Development / Streaming & Messaging category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Kafka using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kafka code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kafka requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/kafka
...
- [ ] Trace Kafka inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kafka defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kafka least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kafka resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kafka release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kafka dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kafka identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kafka change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kafka documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kafka architecture and dependency summary with trust boundaries and persistent state.
- Include Kafka validation commands or tests that are safe for the stated environment.
...
- Kafka identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kafka failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kafka is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kafka as the basis for migration or production guidance.
