---
name: "cncf-stormforge-reviewer"
description: "Review StormForge using its official documentation and repository in the Observability and Analysis / Continuous Optimization category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Stormforge Reviewer

## Purpose

Review and operate StormForge using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- StormForge code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving StormForge requires evidence-backed assessment.
...
- First-party source: https://stormforge.io/
...
- [ ] Trace StormForge inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check StormForge defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check StormForge least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check StormForge resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check StormForge release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check StormForge dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If StormForge identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a StormForge change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- StormForge documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a StormForge architecture and dependency summary with trust boundaries and persistent state.
- Include StormForge validation commands or tests that are safe for the stated environment.
...
- StormForge identity, version, configuration, dependencies, and deployment assumptions are explicit.
- StormForge failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that StormForge is secure, supported, compatible, or suitable.
- Using an unpinned latest version of StormForge as the basis for migration or production guidance.
