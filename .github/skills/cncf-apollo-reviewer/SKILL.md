---
name: "cncf-apollo-reviewer"
description: "Review Apollo using its official documentation and repository in the Provisioning / Automation & Configuration category. Apollo is a reliable open-source configuration management system"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Apollo Reviewer

## Purpose

Review and operate Apollo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Apollo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Apollo requires evidence-backed assessment.
...
- First-party source: https://github.com/apolloconfig/apollo
...
- [ ] Trace Apollo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Apollo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Apollo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Apollo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Apollo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Apollo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Apollo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Apollo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Apollo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Apollo architecture and dependency summary with trust boundaries and persistent state.
- Include Apollo validation commands or tests that are safe for the stated environment.
...
- Apollo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Apollo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Apollo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Apollo as the basis for migration or production guidance.
