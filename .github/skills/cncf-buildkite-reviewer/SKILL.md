---
name: "cncf-buildkite-reviewer"
description: "Review Buildkite using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category. Buildkite is a platform for running fast, secure, and scalable continuous integration pipelines on your own infrastructure."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Buildkite Reviewer

## Purpose

Review and operate Buildkite using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Buildkite code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Buildkite requires evidence-backed assessment.
...
- First-party source: https://buildkite.com/
...
- [ ] Trace Buildkite inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Buildkite defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Buildkite least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Buildkite resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Buildkite release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Buildkite dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Buildkite identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Buildkite change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Buildkite documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Buildkite architecture and dependency summary with trust boundaries and persistent state.
- Include Buildkite validation commands or tests that are safe for the stated environment.
...
- Buildkite identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Buildkite failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Buildkite is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Buildkite as the basis for migration or production guidance.
