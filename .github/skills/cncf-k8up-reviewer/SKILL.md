---
name: "cncf-k8up-reviewer"
description: "Review K8up using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate K8up using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- K8up code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving K8up requires evidence-backed assessment.
...
- First-party source: https://github.com/k8up-io/k8up
...
- [ ] Trace K8up inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check K8up defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check K8up least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check K8up resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check K8up release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check K8up dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If K8up identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a K8up change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- K8up documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a K8up architecture and dependency summary with trust boundaries and persistent state.
- Include K8up validation commands or tests that are safe for the stated environment.
...
- K8up identity, version, configuration, dependencies, and deployment assumptions are explicit.
- K8up failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that K8up is secure, supported, compatible, or suitable.
- Using an unpinned latest version of K8up as the basis for migration or production guidance.
