---
name: "cncf-quai-network-reviewer"
description: "Review Quai Network using its official documentation and repository in the Wasm / Decentralized Platforms category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Quai Network Reviewer

## Purpose

Review and operate Quai Network using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Quai Network code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Quai Network requires evidence-backed assessment.
...
- CNCF classification: Wasm / Decentralized Platforms
- CNCF project maturity field: not specified
- First-party source: https://github.com/dominant-strategies/go-quai
...
- [ ] Trace Quai Network inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Quai Network defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Quai Network least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Quai Network resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Quai Network release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Quai Network dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Quai Network identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Quai Network change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Quai Network documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Quai Network architecture and dependency summary with trust boundaries and persistent state.
- Include Quai Network validation commands or tests that are safe for the stated environment.
...
- Quai Network identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Quai Network failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Quai Network is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Quai Network as the basis for migration or production guidance.
