---
name: "cncf-opea-reviewer"
description: "Review OPEA using its official documentation and repository in the AI Native Infra / Governance, Policy and Security category. OPEA is an ecosystem orchestration framework to integrate performant GenAI technologies & workflows leading to quicker GenAI adoption and business value."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate OPEA using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OPEA code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OPEA requires evidence-backed assessment.
...
- First-party source: https://github.com/opea-project
...
- [ ] Trace OPEA inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OPEA defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OPEA least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OPEA resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OPEA release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OPEA dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OPEA identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OPEA change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OPEA documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OPEA architecture and dependency summary with trust boundaries and persistent state.
- Include OPEA validation commands or tests that are safe for the stated environment.
...
- OPEA identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OPEA failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OPEA is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OPEA as the basis for migration or production guidance.
