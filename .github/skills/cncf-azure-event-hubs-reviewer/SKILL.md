---
name: "cncf-azure-event-hubs-reviewer"
description: "Review Azure Event Hubs using its official documentation and repository in the App Definition and Development / Streaming & Messaging category. Event Hubs is a fully managed, real-time data ingestion service that’s simple, trusted, and scalable."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Azure Event Hubs Reviewer

## Purpose

Review and operate Azure Event Hubs using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Azure Event Hubs code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Azure Event Hubs requires evidence-backed assessment.
...
- First-party source: https://azure.microsoft.com/en-us/services/event-hubs/
...
- [ ] Trace Azure Event Hubs inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Azure Event Hubs defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Azure Event Hubs least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Azure Event Hubs resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Azure Event Hubs release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Azure Event Hubs dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Azure Event Hubs identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Azure Event Hubs change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Azure Event Hubs documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Azure Event Hubs architecture and dependency summary with trust boundaries and persistent state.
- Include Azure Event Hubs validation commands or tests that are safe for the stated environment.
...
- Azure Event Hubs identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Azure Event Hubs failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Azure Event Hubs is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Azure Event Hubs as the basis for migration or production guidance.
