---
name: "cncf-dragonfly-reviewer"
description: "Review Dragonfly using its official documentation and repository in the Provisioning / Container Registry category. Delivers efficient, stable, and secure data distribution and acceleration powered by P2P technology, with an optional content‑addressable filesystem that accelerates OCI container launch."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Dragonfly Reviewer

## Purpose

Review and operate Dragonfly using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Dragonfly code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Dragonfly requires evidence-backed assessment.
...
- First-party source: https://github.com/dragonflyoss/dragonfly
...
- [ ] Trace Dragonfly inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Dragonfly defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Dragonfly least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Dragonfly resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Dragonfly release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Dragonfly dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Dragonfly identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Dragonfly change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Dragonfly documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Dragonfly architecture and dependency summary with trust boundaries and persistent state.
- Include Dragonfly validation commands or tests that are safe for the stated environment.
...
- Dragonfly identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Dragonfly failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Dragonfly is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Dragonfly as the basis for migration or production guidance.
