---
name: "cncf-ydb-reviewer"
description: "Review YDB using its official documentation and repository in the App Definition and Development / Database category. YDB is an open-source Distributed SQL Database that combines high availability and scalability with strong consistency and ACID transactions."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate YDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- YDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving YDB requires evidence-backed assessment.
...
- First-party source: https://github.com/ydb-platform/ydb
...
- [ ] Trace YDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check YDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check YDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check YDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check YDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check YDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If YDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a YDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- YDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a YDB architecture and dependency summary with trust boundaries and persistent state.
- Include YDB validation commands or tests that are safe for the stated environment.
...
- YDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- YDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that YDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of YDB as the basis for migration or production guidance.
