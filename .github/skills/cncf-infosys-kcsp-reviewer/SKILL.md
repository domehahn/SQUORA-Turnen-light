...
---
# Cncf Infosys Kcsp Reviewer

## Purpose

Review and operate Infosys (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Infosys (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Infosys (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.infosys.com/services/application-modernization/offerings/cloud-migration/cloud-migration-overview.html
...
- [ ] Trace Infosys (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Infosys (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Infosys (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Infosys (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Infosys (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Infosys (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Infosys (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Infosys (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Infosys (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Infosys (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Infosys (KCSP) validation commands or tests that are safe for the stated environment.
...
- Infosys (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Infosys (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Infosys (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Infosys (KCSP) as the basis for migration or production guidance.
