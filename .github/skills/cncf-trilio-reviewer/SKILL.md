---
name: "cncf-trilio-reviewer"
description: "Review Trilio using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Trilio Reviewer

## Purpose

Review and operate Trilio using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Trilio code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Trilio requires evidence-backed assessment.
...
- First-party source: https://www.trilio.io/
...
- [ ] Trace Trilio inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Trilio defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Trilio least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Trilio resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Trilio release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Trilio dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Trilio identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Trilio change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Trilio documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Trilio architecture and dependency summary with trust boundaries and persistent state.
- Include Trilio validation commands or tests that are safe for the stated environment.
...
- Trilio identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Trilio failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Trilio is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Trilio as the basis for migration or production guidance.
