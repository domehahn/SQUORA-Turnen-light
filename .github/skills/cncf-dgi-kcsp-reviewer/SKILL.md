...
---
# Cncf Dgi Kcsp Reviewer

## Purpose

Review and operate DGi (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DGi (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DGi (KCSP) requires evidence-backed assessment.
...
- [ ] Trace DGi (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DGi (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DGi (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DGi (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DGi (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DGi (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DGi (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DGi (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DGi (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DGi (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include DGi (KCSP) validation commands or tests that are safe for the stated environment.
...
- DGi (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DGi (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DGi (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DGi (KCSP) as the basis for migration or production guidance.
