---
name: "cncf-tikv-reviewer"
description: "Review TiKV using its official documentation and repository in the App Definition and Development / Database category. A distributed transactional key-value database. Based on the design of Google Spanner and HBase, but simpler to manage and without dependencies on any distributed filesystem"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate TiKV using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- TiKV code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving TiKV requires evidence-backed assessment.
...
- First-party source: https://github.com/tikv/tikv
...
- [ ] Trace TiKV inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check TiKV defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check TiKV least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check TiKV resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check TiKV release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check TiKV dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If TiKV identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a TiKV change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- TiKV documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a TiKV architecture and dependency summary with trust boundaries and persistent state.
- Include TiKV validation commands or tests that are safe for the stated environment.
...
- TiKV identity, version, configuration, dependencies, and deployment assumptions are explicit.
- TiKV failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that TiKV is secure, supported, compatible, or suitable.
- Using an unpinned latest version of TiKV as the basis for migration or production guidance.
