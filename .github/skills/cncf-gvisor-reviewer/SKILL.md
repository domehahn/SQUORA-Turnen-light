---
name: "cncf-gvisor-reviewer"
description: "Review gVisor using its official documentation and repository in the Runtime / Container Runtime category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Gvisor Reviewer

## Purpose

Review and operate gVisor using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- gVisor code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving gVisor requires evidence-backed assessment.
...
- First-party source: https://github.com/google/gvisor
...
- [ ] Trace gVisor inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check gVisor defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check gVisor least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check gVisor resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check gVisor release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check gVisor dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If gVisor identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a gVisor change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- gVisor documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a gVisor architecture and dependency summary with trust boundaries and persistent state.
- Include gVisor validation commands or tests that are safe for the stated environment.
...
- gVisor identity, version, configuration, dependencies, and deployment assumptions are explicit.
- gVisor failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that gVisor is secure, supported, compatible, or suitable.
- Using an unpinned latest version of gVisor as the basis for migration or production guidance.
