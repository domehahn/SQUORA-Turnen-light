---
name: "cncf-cloudsmith-reviewer"
description: "Review Cloudsmith using its official documentation and repository in the Provisioning / Container Registry category. A single source of truth for every artifact and container. Cloudsmith is a powerful, cloud native, enterprise-grade artifact management solution."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Cloudsmith Reviewer

## Purpose

Review and operate Cloudsmith using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Cloudsmith code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Cloudsmith requires evidence-backed assessment.
...
- First-party source: https://cloudsmith.com/
...
- [ ] Trace Cloudsmith inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Cloudsmith defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Cloudsmith least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Cloudsmith resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Cloudsmith release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Cloudsmith dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Cloudsmith identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Cloudsmith change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Cloudsmith documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Cloudsmith architecture and dependency summary with trust boundaries and persistent state.
- Include Cloudsmith validation commands or tests that are safe for the stated environment.
...
- Cloudsmith identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Cloudsmith failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Cloudsmith is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Cloudsmith as the basis for migration or production guidance.
