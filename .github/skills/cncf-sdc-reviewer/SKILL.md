---
name: "cncf-sdc-reviewer"
description: "Review SDC using its official documentation and repository in the Provisioning / Automation & Configuration category. SDC (schema Driven Configuration) exposes any schema based configuration as Kubernetes resources."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate SDC using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SDC code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SDC requires evidence-backed assessment.
...
- First-party source: https://github.com/sdcio/docs
...
- [ ] Trace SDC inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SDC defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SDC least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SDC resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SDC release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SDC dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SDC identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SDC change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SDC documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SDC architecture and dependency summary with trust boundaries and persistent state.
- Include SDC validation commands or tests that are safe for the stated environment.
...
- SDC identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SDC failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SDC is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SDC as the basis for migration or production guidance.
