...
---
# Cncf Okestro Kcsp Reviewer

## Purpose

Review and operate OKESTRO (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OKESTRO (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OKESTRO (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.okestro.com/tech/dx-consulting/
...
- [ ] Trace OKESTRO (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OKESTRO (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OKESTRO (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OKESTRO (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OKESTRO (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OKESTRO (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OKESTRO (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OKESTRO (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OKESTRO (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OKESTRO (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include OKESTRO (KCSP) validation commands or tests that are safe for the stated environment.
...
- OKESTRO (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OKESTRO (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OKESTRO (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OKESTRO (KCSP) as the basis for migration or production guidance.
