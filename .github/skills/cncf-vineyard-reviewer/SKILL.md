---
name: "cncf-vineyard-reviewer"
description: "Review Vineyard using its official documentation and repository in the Runtime / Cloud Native Storage category. Vineyard (v6d) is an in-memory immutable data manager."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Vineyard Reviewer

## Purpose

Review and operate Vineyard using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Vineyard code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Vineyard requires evidence-backed assessment.
...
- First-party source: https://github.com/v6d-io/v6d
...
- [ ] Trace Vineyard inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Vineyard defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Vineyard least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Vineyard resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Vineyard release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Vineyard dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Vineyard identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Vineyard change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Vineyard documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Vineyard architecture and dependency summary with trust boundaries and persistent state.
- Include Vineyard validation commands or tests that are safe for the stated environment.
...
- Vineyard identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Vineyard failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Vineyard is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Vineyard as the basis for migration or production guidance.
