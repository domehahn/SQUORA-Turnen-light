---
name: "cncf-airlock-reviewer"
description: "Review Airlock using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Airlock Reviewer

## Purpose

Review and operate Airlock using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Airlock code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Airlock requires evidence-backed assessment.
...
- First-party source: https://www.airlock.com/
...
- [ ] Trace Airlock inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Airlock defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Airlock least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Airlock resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Airlock release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Airlock dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Airlock identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Airlock change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Airlock documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Airlock architecture and dependency summary with trust boundaries and persistent state.
- Include Airlock validation commands or tests that are safe for the stated environment.
...
- Airlock identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Airlock failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Airlock is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Airlock as the basis for migration or production guidance.
