---
name: "cncf-cartographer-reviewer"
description: "Review Cartographer using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category. Cartographer is a Kubernetes-native Choreographer providing higher modularity and scalability for the software supply chain."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Cartographer Reviewer

## Purpose

Review and operate Cartographer using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Cartographer code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Cartographer requires evidence-backed assessment.
...
- First-party source: https://github.com/vmware-tanzu/cartographer
...
- [ ] Trace Cartographer inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Cartographer defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Cartographer least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Cartographer resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Cartographer release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Cartographer dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Cartographer identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Cartographer change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Cartographer documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Cartographer architecture and dependency summary with trust boundaries and persistent state.
- Include Cartographer validation commands or tests that are safe for the stated environment.
...
- Cartographer identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Cartographer failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Cartographer is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Cartographer as the basis for migration or production guidance.
