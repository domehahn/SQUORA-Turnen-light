---
name: "cncf-hubble-reviewer"
description: "Review Hubble using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Hubble Reviewer

## Purpose

Review and operate Hubble using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Hubble code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Hubble requires evidence-backed assessment.
...
- First-party source: https://github.com/cilium/hubble
...
- [ ] Trace Hubble inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Hubble defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Hubble least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Hubble resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Hubble release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Hubble dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Hubble identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Hubble change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Hubble documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Hubble architecture and dependency summary with trust boundaries and persistent state.
- Include Hubble validation commands or tests that are safe for the stated environment.
...
- Hubble identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Hubble failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Hubble is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Hubble as the basis for migration or production guidance.
