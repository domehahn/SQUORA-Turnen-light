---
name: "cncf-singlestore-wasm-reviewer"
description: "Review SingleStore (Wasm) using its official documentation and repository in the Wasm / Embedded Functions category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Singlestore Wasm Reviewer

## Purpose

Review and operate SingleStore (Wasm) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SingleStore (Wasm) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SingleStore (Wasm) requires evidence-backed assessment.
...
- CNCF classification: Wasm / Embedded Functions
- CNCF project maturity field: not specified
- First-party source: https://www.singlestore.com/
...
- [ ] Trace SingleStore (Wasm) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SingleStore (Wasm) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SingleStore (Wasm) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SingleStore (Wasm) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SingleStore (Wasm) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SingleStore (Wasm) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SingleStore (Wasm) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SingleStore (Wasm) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SingleStore (Wasm) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SingleStore (Wasm) architecture and dependency summary with trust boundaries and persistent state.
- Include SingleStore (Wasm) validation commands or tests that are safe for the stated environment.
...
- SingleStore (Wasm) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SingleStore (Wasm) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SingleStore (Wasm) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SingleStore (Wasm) as the basis for migration or production guidance.
