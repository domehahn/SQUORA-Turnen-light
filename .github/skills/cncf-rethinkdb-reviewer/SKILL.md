---
name: "cncf-rethinkdb-reviewer"
description: "Review RethinkDB using its official documentation and repository in the App Definition and Development / Database category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Rethinkdb Reviewer

## Purpose

Review and operate RethinkDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- RethinkDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving RethinkDB requires evidence-backed assessment.
...
- First-party source: https://github.com/rethinkdb/rethinkdb
...
- [ ] Trace RethinkDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check RethinkDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check RethinkDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check RethinkDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check RethinkDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check RethinkDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If RethinkDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a RethinkDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- RethinkDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a RethinkDB architecture and dependency summary with trust boundaries and persistent state.
- Include RethinkDB validation commands or tests that are safe for the stated environment.
...
- RethinkDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- RethinkDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that RethinkDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of RethinkDB as the basis for migration or production guidance.
