---
name: "cncf-membrane-reviewer"
description: "Review Membrane using its official documentation and repository in the Orchestration & Management / API Gateway category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Membrane Reviewer

## Purpose

Review and operate Membrane using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Membrane code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Membrane requires evidence-backed assessment.
...
- First-party source: https://github.com/membrane/api-gateway
...
- [ ] Trace Membrane inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Membrane defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Membrane least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Membrane resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Membrane release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Membrane dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Membrane identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Membrane change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Membrane documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Membrane architecture and dependency summary with trust boundaries and persistent state.
- Include Membrane validation commands or tests that are safe for the stated environment.
...
- Membrane identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Membrane failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Membrane is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Membrane as the basis for migration or production guidance.
