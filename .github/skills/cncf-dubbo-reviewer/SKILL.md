---
name: "cncf-dubbo-reviewer"
description: "Review Dubbo using its official documentation and repository in the Orchestration & Management / Remote Procedure Call category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Dubbo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Dubbo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Dubbo requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/dubbo
...
- [ ] Trace Dubbo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Dubbo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Dubbo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Dubbo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Dubbo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Dubbo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Dubbo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Dubbo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Dubbo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Dubbo architecture and dependency summary with trust boundaries and persistent state.
- Include Dubbo validation commands or tests that are safe for the stated environment.
...
- Dubbo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Dubbo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Dubbo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Dubbo as the basis for migration or production guidance.
