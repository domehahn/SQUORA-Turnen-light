---
name: "cncf-linkerd-reviewer"
description: "Review Linkerd using its official documentation and repository in the Orchestration & Management / Service Mesh category. Ultra light, ultra simple, ultra powerful. Linkerd adds security, observability, and reliability to Kubernetes, without the complexity."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Linkerd Reviewer

## Purpose

Review and operate Linkerd using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Linkerd code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Linkerd requires evidence-backed assessment.
...
- First-party source: https://github.com/linkerd/linkerd2
...
- [ ] Trace Linkerd inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Linkerd defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Linkerd least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Linkerd resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Linkerd release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Linkerd dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Linkerd identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Linkerd change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Linkerd documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Linkerd architecture and dependency summary with trust boundaries and persistent state.
- Include Linkerd validation commands or tests that are safe for the stated environment.
...
- Linkerd identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Linkerd failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Linkerd is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Linkerd as the basis for migration or production guidance.
