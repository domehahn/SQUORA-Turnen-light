---
name: "cncf-tars-reviewer"
description: "Review TARS using its official documentation and repository in the Orchestration & Management / Remote Procedure Call category. TARS is a high-performance RPC framework that provides an integrated solution of microservice governance."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate TARS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- TARS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving TARS requires evidence-backed assessment.
...
- First-party source: https://github.com/tarsCloud/tars
...
- [ ] Trace TARS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check TARS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check TARS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check TARS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check TARS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check TARS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If TARS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a TARS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- TARS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a TARS architecture and dependency summary with trust boundaries and persistent state.
- Include TARS validation commands or tests that are safe for the stated environment.
...
- TARS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- TARS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that TARS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of TARS as the basis for migration or production guidance.
