---
name: "cncf-shopify-reviewer"
description: "Review Shopify using its official documentation and repository in the Wasm / Embedded Functions category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Shopify Reviewer

## Purpose

Review and operate Shopify using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Shopify code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Shopify requires evidence-backed assessment.
...
- [ ] Trace Shopify inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Shopify defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Shopify least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Shopify resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Shopify release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Shopify dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Shopify identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Shopify change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Shopify documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Shopify architecture and dependency summary with trust boundaries and persistent state.
- Include Shopify validation commands or tests that are safe for the stated environment.
...
- Shopify identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Shopify failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Shopify is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Shopify as the basis for migration or production guidance.
