---
name: "cncf-devtron-reviewer"
description: "Review Devtron using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category. Open source Software delivery workflow for Kubernetes"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Devtron Reviewer

## Purpose

Review and operate Devtron using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Devtron code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Devtron requires evidence-backed assessment.
...
- First-party source: https://github.com/devtron-labs/devtron
...
- [ ] Trace Devtron inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Devtron defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Devtron least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Devtron resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Devtron release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Devtron dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Devtron identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Devtron change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Devtron documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Devtron architecture and dependency summary with trust boundaries and persistent state.
- Include Devtron validation commands or tests that are safe for the stated environment.
...
- Devtron identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Devtron failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Devtron is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Devtron as the basis for migration or production guidance.
