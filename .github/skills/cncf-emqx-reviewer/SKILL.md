---
name: "cncf-emqx-reviewer"
description: "Review EMQX using its official documentation and repository in the App Definition and Development / Streaming & Messaging category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate EMQX using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- EMQX code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving EMQX requires evidence-backed assessment.
...
- First-party source: https://github.com/emqx/emqx
...
- [ ] Trace EMQX inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check EMQX defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check EMQX least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check EMQX resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check EMQX release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check EMQX dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If EMQX identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a EMQX change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- EMQX documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a EMQX architecture and dependency summary with trust boundaries and persistent state.
- Include EMQX validation commands or tests that are safe for the stated environment.
...
- EMQX identity, version, configuration, dependencies, and deployment assumptions are explicit.
- EMQX failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that EMQX is secure, supported, compatible, or suitable.
- Using an unpinned latest version of EMQX as the basis for migration or production guidance.
