---
name: "cncf-kube-green-reviewer"
description: "Review kube-green using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. kube-green is a simple k8s addon that automatically shuts down (some of) your resources when you don't need them."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kube Green Reviewer

## Purpose

Review and operate kube-green using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- kube-green code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving kube-green requires evidence-backed assessment.
...
- First-party source: https://github.com/kube-green/kube-green
...
- [ ] Trace kube-green inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check kube-green defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check kube-green least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check kube-green resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check kube-green release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check kube-green dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If kube-green identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a kube-green change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- kube-green documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a kube-green architecture and dependency summary with trust boundaries and persistent state.
- Include kube-green validation commands or tests that are safe for the stated environment.
...
- kube-green identity, version, configuration, dependencies, and deployment assumptions are explicit.
- kube-green failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that kube-green is secure, supported, compatible, or suitable.
- Using an unpinned latest version of kube-green as the basis for migration or production guidance.
