---
name: "cncf-unryo-reviewer"
description: "Review unryo using its official documentation and repository in the Observability and Analysis / Observability category. Observability and cross-layer topology correlation on top of CNCF ecosystem: Prometheus, Influx, Kubernetes, Elastic, networks and 5G."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate unryo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- unryo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving unryo requires evidence-backed assessment.
...
- First-party source: https://www.unryo.com/
...
- [ ] Trace unryo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check unryo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check unryo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check unryo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check unryo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check unryo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If unryo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a unryo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- unryo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a unryo architecture and dependency summary with trust boundaries and persistent state.
- Include unryo validation commands or tests that are safe for the stated environment.
...
- unryo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- unryo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that unryo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of unryo as the basis for migration or production guidance.
