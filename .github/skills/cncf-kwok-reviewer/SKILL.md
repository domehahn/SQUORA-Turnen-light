---
name: "cncf-kwok-reviewer"
description: "Review kwok using its official documentation and repository in the Platform / Certified Kubernetes - Installer category. Kubernetes WithOut Kubelet - Simulates thousands of Nodes and Clusters."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate kwok using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- kwok code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving kwok requires evidence-backed assessment.
...
- First-party source: https://github.com/kubernetes-sigs/kwok
...
- [ ] Trace kwok inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check kwok defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check kwok least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check kwok resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check kwok release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check kwok dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If kwok identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a kwok change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- kwok documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a kwok architecture and dependency summary with trust boundaries and persistent state.
- Include kwok validation commands or tests that are safe for the stated environment.
...
- kwok identity, version, configuration, dependencies, and deployment assumptions are explicit.
- kwok failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that kwok is secure, supported, compatible, or suitable.
- Using an unpinned latest version of kwok as the basis for migration or production guidance.
