---
name: "cncf-warg-reviewer"
description: "Review warg using its official documentation and repository in the Wasm / Packaging, Registries & Application Delivery category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate warg using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- warg code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving warg requires evidence-backed assessment.
...
- First-party source: https://github.com/bytecodealliance/registry
...
- [ ] Trace warg inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check warg defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check warg least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check warg resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check warg release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check warg dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If warg identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a warg change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- warg documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a warg architecture and dependency summary with trust boundaries and persistent state.
- Include warg validation commands or tests that are safe for the stated environment.
...
- warg identity, version, configuration, dependencies, and deployment assumptions are explicit.
- warg failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that warg is secure, supported, compatible, or suitable.
- Using an unpinned latest version of warg as the basis for migration or production guidance.
