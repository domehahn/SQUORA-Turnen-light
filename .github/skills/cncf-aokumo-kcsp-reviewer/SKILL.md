...
---
# Cncf Aokumo Kcsp Reviewer

## Purpose

Review and operate Aokumo (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Aokumo (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Aokumo (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.aokumo.io/services/kubernetes-professional-services
...
- [ ] Trace Aokumo (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Aokumo (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Aokumo (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Aokumo (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Aokumo (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Aokumo (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Aokumo (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Aokumo (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Aokumo (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Aokumo (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Aokumo (KCSP) validation commands or tests that are safe for the stated environment.
...
- Aokumo (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Aokumo (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Aokumo (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Aokumo (KCSP) as the basis for migration or production guidance.
