---
name: "cncf-singlestore-reviewer"
description: "Review SingleStore using its official documentation and repository in the App Definition and Development / Database category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Singlestore Reviewer

## Purpose

Review and operate SingleStore using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SingleStore code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SingleStore requires evidence-backed assessment.
...
- [ ] Trace SingleStore inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SingleStore defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SingleStore least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SingleStore resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SingleStore release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SingleStore dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SingleStore identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SingleStore change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SingleStore documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SingleStore architecture and dependency summary with trust boundaries and persistent state.
- Include SingleStore validation commands or tests that are safe for the stated environment.
...
- SingleStore identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SingleStore failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SingleStore is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SingleStore as the basis for migration or production guidance.
