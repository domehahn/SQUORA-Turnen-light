---
name: "cncf-grafana-reviewer"
description: "Review Grafana using its official documentation and repository in the Observability and Analysis / Observability category. Grafana allows you to query, visualize, alert on and understand your metrics no matter where they are stored. Create, explore, and share beautiful dashboards with your team and foster a data driven culture."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Grafana Reviewer

## Purpose

Review and operate Grafana using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Grafana code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Grafana requires evidence-backed assessment.
...
- First-party source: https://github.com/grafana/grafana
...
- [ ] Trace Grafana inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Grafana defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Grafana least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Grafana resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Grafana release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Grafana dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Grafana identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Grafana change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Grafana documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Grafana architecture and dependency summary with trust boundaries and persistent state.
- Include Grafana validation commands or tests that are safe for the stated environment.
...
- Grafana identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Grafana failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Grafana is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Grafana as the basis for migration or production guidance.
