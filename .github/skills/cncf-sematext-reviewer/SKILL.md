---
name: "cncf-sematext-reviewer"
description: "Review Sematext using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Sematext Reviewer

## Purpose

Review and operate Sematext using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Sematext code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Sematext requires evidence-backed assessment.
...
- First-party source: https://sematext.com/
...
- [ ] Trace Sematext inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Sematext defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Sematext least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Sematext resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Sematext release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Sematext dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Sematext identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Sematext change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Sematext documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Sematext architecture and dependency summary with trust boundaries and persistent state.
- Include Sematext validation commands or tests that are safe for the stated environment.
...
- Sematext identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Sematext failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Sematext is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Sematext as the basis for migration or production guidance.
