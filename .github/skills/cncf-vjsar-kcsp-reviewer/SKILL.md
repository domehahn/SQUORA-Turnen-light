---
name: "cncf-vjsar-kcsp-reviewer"
description: "Review VJSAR (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. We deliver expert Kubernetes services, including automated cluster management, zero-downtime deployments, cloud-native migrations, and integrated security, ensuring your infrastructure remains scalable, resilient, and future-ready."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Vjsar Kcsp Reviewer

## Purpose

Review and operate VJSAR (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- VJSAR (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving VJSAR (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.vjsar.com/application-modernization
...
- [ ] Trace VJSAR (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check VJSAR (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check VJSAR (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check VJSAR (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check VJSAR (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check VJSAR (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If VJSAR (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a VJSAR (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- VJSAR (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a VJSAR (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include VJSAR (KCSP) validation commands or tests that are safe for the stated environment.
...
- VJSAR (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- VJSAR (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that VJSAR (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of VJSAR (KCSP) as the basis for migration or production guidance.
