---
name: "cncf-goose-reviewer"
description: "Review Goose using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Goose using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Goose code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Goose requires evidence-backed assessment.
...
- First-party source: https://github.com/block/goose
...
- [ ] Trace Goose inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Goose defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Goose least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Goose resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Goose release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Goose dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Goose identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Goose change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Goose documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Goose architecture and dependency summary with trust boundaries and persistent state.
- Include Goose validation commands or tests that are safe for the stated environment.
...
- Goose identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Goose failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Goose is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Goose as the basis for migration or production guidance.
