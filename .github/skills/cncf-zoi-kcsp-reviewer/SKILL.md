...
---
# Cncf Zoi Kcsp Reviewer

## Purpose

Review and operate Zoi (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Zoi (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Zoi (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.zoi.tech/
...
- [ ] Trace Zoi (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Zoi (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Zoi (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Zoi (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Zoi (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Zoi (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Zoi (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Zoi (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Zoi (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Zoi (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Zoi (KCSP) validation commands or tests that are safe for the stated environment.
...
- Zoi (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Zoi (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Zoi (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Zoi (KCSP) as the basis for migration or production guidance.
