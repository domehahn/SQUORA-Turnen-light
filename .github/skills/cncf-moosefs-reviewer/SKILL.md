---
name: "cncf-moosefs-reviewer"
description: "Review MooseFS using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Moosefs Reviewer

## Purpose

Review and operate MooseFS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- MooseFS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving MooseFS requires evidence-backed assessment.
...
- First-party source: https://github.com/moosefs/moosefs
...
- [ ] Trace MooseFS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check MooseFS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check MooseFS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check MooseFS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check MooseFS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check MooseFS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If MooseFS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a MooseFS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- MooseFS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a MooseFS architecture and dependency summary with trust boundaries and persistent state.
- Include MooseFS validation commands or tests that are safe for the stated environment.
...
- MooseFS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- MooseFS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that MooseFS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of MooseFS as the basis for migration or production guidance.
