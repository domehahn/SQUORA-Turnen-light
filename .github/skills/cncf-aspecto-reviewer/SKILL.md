---
name: "cncf-aspecto-reviewer"
description: "Review Aspecto using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Aspecto Reviewer

## Purpose

Review and operate Aspecto using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Aspecto code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Aspecto requires evidence-backed assessment.
...
- First-party source: https://www.aspecto.io/
...
- [ ] Trace Aspecto inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Aspecto defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Aspecto least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Aspecto resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Aspecto release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Aspecto dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Aspecto identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Aspecto change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Aspecto documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Aspecto architecture and dependency summary with trust boundaries and persistent state.
- Include Aspecto validation commands or tests that are safe for the stated environment.
...
- Aspecto identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Aspecto failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Aspecto is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Aspecto as the basis for migration or production guidance.
