---
name: "cncf-skaffold-reviewer"
description: "Review Skaffold using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Skaffold Reviewer

## Purpose

Review and operate Skaffold using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Skaffold code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Skaffold requires evidence-backed assessment.
...
- First-party source: https://github.com/GoogleContainerTools/skaffold
...
- [ ] Trace Skaffold inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Skaffold defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Skaffold least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Skaffold resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Skaffold release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Skaffold dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Skaffold identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Skaffold change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Skaffold documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Skaffold architecture and dependency summary with trust boundaries and persistent state.
- Include Skaffold validation commands or tests that are safe for the stated environment.
...
- Skaffold identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Skaffold failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Skaffold is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Skaffold as the basis for migration or production guidance.
