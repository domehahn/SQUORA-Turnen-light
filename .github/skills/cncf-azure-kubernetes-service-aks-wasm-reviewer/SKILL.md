---
name: "cncf-azure-kubernetes-service-aks-wasm-reviewer"
description: "Review Azure Kubernetes Service (AKS) (Wasm) using its official documentation and repository in the Wasm / Hosted Platforms category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Azure Kubernetes Service Aks Wasm Reviewer

## Purpose

Review and operate Azure Kubernetes Service (AKS) (Wasm) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Azure Kubernetes Service (AKS) (Wasm) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Azure Kubernetes Service (AKS) (Wasm) requires evidence-backed assessment.
...
- CNCF classification: Wasm / Hosted Platforms
- CNCF project maturity field: not specified
- First-party source: https://learn.microsoft.com/en-us/azure/aks/use-wasi-node-pools
...
- [ ] Trace Azure Kubernetes Service (AKS) (Wasm) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Azure Kubernetes Service (AKS) (Wasm) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Azure Kubernetes Service (AKS) (Wasm) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Azure Kubernetes Service (AKS) (Wasm) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Azure Kubernetes Service (AKS) (Wasm) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Azure Kubernetes Service (AKS) (Wasm) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Azure Kubernetes Service (AKS) (Wasm) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Azure Kubernetes Service (AKS) (Wasm) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Azure Kubernetes Service (AKS) (Wasm) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Azure Kubernetes Service (AKS) (Wasm) architecture and dependency summary with trust boundaries and persistent state.
- Include Azure Kubernetes Service (AKS) (Wasm) validation commands or tests that are safe for the stated environment.
...
- Azure Kubernetes Service (AKS) (Wasm) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Azure Kubernetes Service (AKS) (Wasm) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Azure Kubernetes Service (AKS) (Wasm) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Azure Kubernetes Service (AKS) (Wasm) as the basis for migration or production guidance.
