---
name: "cncf-velero-reviewer"
description: "Review Velero using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Velero Reviewer

## Purpose

Review and operate Velero using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Velero code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Velero requires evidence-backed assessment.
...
- First-party source: https://github.com/velero-io/velero
...
- [ ] Trace Velero inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Velero defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Velero least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Velero resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Velero release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Velero dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Velero identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Velero change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Velero documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Velero architecture and dependency summary with trust boundaries and persistent state.
- Include Velero validation commands or tests that are safe for the stated environment.
...
- Velero identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Velero failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Velero is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Velero as the basis for migration or production guidance.
