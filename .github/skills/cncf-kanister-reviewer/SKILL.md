---
name: "cncf-kanister-reviewer"
description: "Review Kanister using its official documentation and repository in the Runtime / Cloud Native Storage category. An extensible framework for application-level data management on Kubernetes"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kanister Reviewer

## Purpose

Review and operate Kanister using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kanister code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kanister requires evidence-backed assessment.
...
- First-party source: https://github.com/kanisterio/kanister
...
- [ ] Trace Kanister inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kanister defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kanister least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kanister resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kanister release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kanister dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kanister identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kanister change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kanister documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kanister architecture and dependency summary with trust boundaries and persistent state.
- Include Kanister validation commands or tests that are safe for the stated environment.
...
- Kanister identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kanister failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kanister is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kanister as the basis for migration or production guidance.
