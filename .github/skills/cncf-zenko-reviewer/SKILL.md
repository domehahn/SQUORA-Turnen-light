---
name: "cncf-zenko-reviewer"
description: "Review Zenko using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Zenko using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Zenko code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Zenko requires evidence-backed assessment.
...
- First-party source: https://github.com/scality/zenko
...
- [ ] Trace Zenko inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Zenko defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Zenko least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Zenko resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Zenko release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Zenko dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Zenko identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Zenko change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Zenko documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Zenko architecture and dependency summary with trust boundaries and persistent state.
- Include Zenko validation commands or tests that are safe for the stated environment.
...
- Zenko identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Zenko failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Zenko is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Zenko as the basis for migration or production guidance.
