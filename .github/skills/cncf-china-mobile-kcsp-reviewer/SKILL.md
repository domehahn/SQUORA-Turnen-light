---
name: "cncf-china-mobile-kcsp-reviewer"
description: "Review China Mobile (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. China Mobile (Suzhou) Software Technology provides full support and services to domestic companies for their cloud native journeys"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf China Mobile Kcsp Reviewer

## Purpose

Review and operate China Mobile (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- China Mobile (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving China Mobile (KCSP) requires evidence-backed assessment.
...
- First-party source: https://ecloud.10086.cn/home/product-introduction/containerser
...
- [ ] Trace China Mobile (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check China Mobile (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check China Mobile (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check China Mobile (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check China Mobile (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check China Mobile (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If China Mobile (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a China Mobile (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- China Mobile (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a China Mobile (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include China Mobile (KCSP) validation commands or tests that are safe for the stated environment.
...
- China Mobile (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- China Mobile (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that China Mobile (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of China Mobile (KCSP) as the basis for migration or production guidance.
