---
name: "cncf-dachs-it-kcsp-reviewer"
description: "Review DACHS IT (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. DACHS IT GMBH provides Kubernetes consulting, training, managed service & support located in Germany."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Dachs It Kcsp Reviewer

## Purpose

Review and operate DACHS IT (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DACHS IT (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DACHS IT (KCSP) requires evidence-backed assessment.
...
- First-party source: https://dachs-it.de/hands-on-consulting/container-platform-engineering
...
- [ ] Trace DACHS IT (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DACHS IT (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DACHS IT (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DACHS IT (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DACHS IT (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DACHS IT (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DACHS IT (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DACHS IT (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DACHS IT (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DACHS IT (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include DACHS IT (KCSP) validation commands or tests that are safe for the stated environment.
...
- DACHS IT (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DACHS IT (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DACHS IT (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DACHS IT (KCSP) as the basis for migration or production guidance.
