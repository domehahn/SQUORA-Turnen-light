---
name: "cncf-cockroachdb-reviewer"
description: "Review CockroachDB using its official documentation and repository in the App Definition and Development / Database category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Cockroachdb Reviewer

## Purpose

Review and operate CockroachDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CockroachDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CockroachDB requires evidence-backed assessment.
...
- First-party source: https://github.com/cockroachdb/cockroach
...
- [ ] Trace CockroachDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CockroachDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CockroachDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CockroachDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CockroachDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CockroachDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CockroachDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CockroachDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CockroachDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CockroachDB architecture and dependency summary with trust boundaries and persistent state.
- Include CockroachDB validation commands or tests that are safe for the stated environment.
...
- CockroachDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CockroachDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CockroachDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CockroachDB as the basis for migration or production guidance.
