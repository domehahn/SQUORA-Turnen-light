---
name: "cncf-akana-reviewer"
description: "Review Akana using its official documentation and repository in the Orchestration & Management / API Gateway category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Akana using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Akana code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Akana requires evidence-backed assessment.
...
- First-party source: https://www.akana.com/
...
- [ ] Trace Akana inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Akana defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Akana least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Akana resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Akana release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Akana dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Akana identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Akana change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Akana documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Akana architecture and dependency summary with trust boundaries and persistent state.
- Include Akana validation commands or tests that are safe for the stated environment.
...
- Akana identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Akana failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Akana is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Akana as the basis for migration or production guidance.
