...
---
# Cncf Panweidb Reviewer

## Purpose

Review and operate PanWeiDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- PanWeiDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving PanWeiDB requires evidence-backed assessment.
...
- First-party source: https://open.10086.cn/panweidb/home
...
- [ ] Trace PanWeiDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check PanWeiDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check PanWeiDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check PanWeiDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check PanWeiDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check PanWeiDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If PanWeiDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a PanWeiDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- PanWeiDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a PanWeiDB architecture and dependency summary with trust boundaries and persistent state.
- Include PanWeiDB validation commands or tests that are safe for the stated environment.
...
- PanWeiDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- PanWeiDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that PanWeiDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of PanWeiDB as the basis for migration or production guidance.
