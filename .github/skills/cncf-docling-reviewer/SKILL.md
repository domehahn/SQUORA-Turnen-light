---
name: "cncf-docling-reviewer"
description: "Review Docling using its official documentation and repository in the AI Agent / RAG category. Get your documents ready for gen AI"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Docling Reviewer

## Purpose

Review and operate Docling using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Docling code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Docling requires evidence-backed assessment.
...
- First-party source: https://github.com/docling-project/docling
...
- [ ] Trace Docling inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Docling defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Docling least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Docling resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Docling release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Docling dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Docling identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Docling change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Docling documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Docling architecture and dependency summary with trust boundaries and persistent state.
- Include Docling validation commands or tests that are safe for the stated environment.
...
- Docling identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Docling failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Docling is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Docling as the basis for migration or production guidance.
