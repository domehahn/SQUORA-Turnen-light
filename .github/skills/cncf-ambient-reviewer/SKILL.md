---
name: "cncf-ambient-reviewer"
description: "Review Ambient using its official documentation and repository in the Wasm / Application Frameworks category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Ambient Reviewer

## Purpose

Review and operate Ambient using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Ambient code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Ambient requires evidence-backed assessment.
...
- First-party source: https://github.com/AmbientRun/Ambient
...
- [ ] Trace Ambient inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Ambient defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Ambient least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Ambient resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Ambient release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Ambient dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Ambient identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Ambient change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Ambient documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Ambient architecture and dependency summary with trust boundaries and persistent state.
- Include Ambient validation commands or tests that are safe for the stated environment.
...
- Ambient identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Ambient failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Ambient is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Ambient as the basis for migration or production guidance.
