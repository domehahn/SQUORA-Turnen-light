---
name: "helm-reviewer"
description: "Review Helm charts, templates, values, dependencies, hooks, secrets, schema validation, upgrades, and rollbacks."
version: "0.1.0"
since: "2026-08-27"
...
- Source code, configuration, dependencies, tests, deployment artifacts, or runtime behavior in Helm package management change.
...
- Chart.yaml, templates, values schema, dependencies, hooks, upgrade and rollback
...
- [ ] Trace Helm package management inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Helm package management defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Helm package management least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Helm package management resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Helm package management release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
...
- If Helm package management identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Helm package management change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Helm package management documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Helm package management architecture and dependency summary with trust boundaries and persistent state.
- Include Helm package management validation commands or tests that are safe for the stated environment.
...
- Helm package management identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Helm package management failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Helm package management is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Helm package management as the basis for migration or production guidance.
