---
name: "cncf-nemo-guardrails-reviewer"
description: "Review NeMo Guardrails using its official documentation and repository in the AI Agent / Guardrail category. NeMo Guardrails is an open-source toolkit for easily adding programmable guardrails to LLM-based conversational systems."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Nemo Guardrails Reviewer

## Purpose

Review and operate NeMo Guardrails using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- NeMo Guardrails code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving NeMo Guardrails requires evidence-backed assessment.
...
- CNCF classification: AI Agent / Guardrail
- CNCF project maturity field: not specified
- First-party source: https://github.com/NVIDIA-NeMo/Guardrails
...
- [ ] Trace NeMo Guardrails inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check NeMo Guardrails defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check NeMo Guardrails least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check NeMo Guardrails resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check NeMo Guardrails release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check NeMo Guardrails dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If NeMo Guardrails identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a NeMo Guardrails change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- NeMo Guardrails documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a NeMo Guardrails architecture and dependency summary with trust boundaries and persistent state.
- Include NeMo Guardrails validation commands or tests that are safe for the stated environment.
...
- NeMo Guardrails identity, version, configuration, dependencies, and deployment assumptions are explicit.
- NeMo Guardrails failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that NeMo Guardrails is secure, supported, compatible, or suitable.
- Using an unpinned latest version of NeMo Guardrails as the basis for migration or production guidance.
