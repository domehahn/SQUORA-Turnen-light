---
name: "cncf-consul-reviewer"
description: "Review Consul using its official documentation and repository in the Orchestration & Management / Service Mesh category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Consul Reviewer

## Purpose

Review and operate Consul using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Consul code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Consul requires evidence-backed assessment.
...
- First-party source: https://github.com/hashicorp/consul
...
- [ ] Trace Consul inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Consul defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Consul least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Consul resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Consul release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Consul dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Consul identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Consul change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Consul documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Consul architecture and dependency summary with trust boundaries and persistent state.
- Include Consul validation commands or tests that are safe for the stated environment.
...
- Consul identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Consul failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Consul is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Consul as the basis for migration or production guidance.
