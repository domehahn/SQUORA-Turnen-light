---
name: "cncf-kilo-reviewer"
description: "Review Kilo using its official documentation and repository in the Runtime / Cloud Native Network category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Kilo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kilo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kilo requires evidence-backed assessment.
...
- First-party source: https://github.com/squat/kilo
...
- [ ] Trace Kilo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kilo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kilo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kilo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kilo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kilo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kilo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kilo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kilo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kilo architecture and dependency summary with trust boundaries and persistent state.
- Include Kilo validation commands or tests that are safe for the stated environment.
...
- Kilo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kilo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kilo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kilo as the basis for migration or production guidance.
