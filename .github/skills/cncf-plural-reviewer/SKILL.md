---
name: "cncf-plural-reviewer"
description: "Review Plural using its official documentation and repository in the Platform / Certified Kubernetes - Distribution category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Plural Reviewer

## Purpose

Review and operate Plural using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Plural code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Plural requires evidence-backed assessment.
...
- First-party source: https://github.com/pluralsh/plural
...
- [ ] Trace Plural inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Plural defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Plural least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Plural resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Plural release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Plural dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Plural identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Plural change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Plural documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Plural architecture and dependency summary with trust boundaries and persistent state.
- Include Plural validation commands or tests that are safe for the stated environment.
...
- Plural identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Plural failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Plural is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Plural as the basis for migration or production guidance.
