---
name: "cncf-lenovo-kcsp-reviewer"
description: "Review Lenovo (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Based on the experience of enterprise-level IT governance in the dual state of stability and flexibility, Lenovo Cloud Native products provide one-stop Kubernetes services to help customers achieve a balance between development efficiency and operation and maintenance control."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Lenovo Kcsp Reviewer

## Purpose

Review and operate Lenovo (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Lenovo (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Lenovo (KCSP) requires evidence-backed assessment.
...
- First-party source: https://xcloud.lenovo.com.cn/product/detail/product_detail_1/
...
- [ ] Trace Lenovo (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Lenovo (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Lenovo (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Lenovo (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Lenovo (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Lenovo (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Lenovo (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Lenovo (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Lenovo (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Lenovo (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Lenovo (KCSP) validation commands or tests that are safe for the stated environment.
...
- Lenovo (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Lenovo (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Lenovo (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Lenovo (KCSP) as the basis for migration or production guidance.
