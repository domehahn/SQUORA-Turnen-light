...
Review and operate OpenSDN using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenSDN code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenSDN requires evidence-backed assessment.
...
- First-party source: https://github.com/OpenSDN-io
...
- [ ] Trace OpenSDN inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenSDN defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenSDN least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenSDN resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenSDN release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenSDN dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenSDN identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenSDN change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenSDN documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenSDN architecture and dependency summary with trust boundaries and persistent state.
- Include OpenSDN validation commands or tests that are safe for the stated environment.
...
- OpenSDN identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenSDN failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenSDN is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenSDN as the basis for migration or production guidance.
