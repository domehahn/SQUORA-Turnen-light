---
name: "cncf-nocalhost-reviewer"
description: "Review Nocalhost using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Nocalhost Reviewer

## Purpose

Review and operate Nocalhost using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Nocalhost code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Nocalhost requires evidence-backed assessment.
...
- First-party source: https://github.com/nocalhost/nocalhost
...
- [ ] Trace Nocalhost inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Nocalhost defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Nocalhost least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Nocalhost resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Nocalhost release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Nocalhost dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Nocalhost identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Nocalhost change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Nocalhost documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Nocalhost architecture and dependency summary with trust boundaries and persistent state.
- Include Nocalhost validation commands or tests that are safe for the stated environment.
...
- Nocalhost identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Nocalhost failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Nocalhost is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Nocalhost as the basis for migration or production guidance.
