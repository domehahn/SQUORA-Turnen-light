---
name: "cncf-lmql-reviewer"
description: "Review LMQL using its official documentation and repository in the AI Agent / Structured Output category. A language for constraint-guided and efficient LLM programming"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate LMQL using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- LMQL code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving LMQL requires evidence-backed assessment.
...
- First-party source: https://github.com/eth-sri/lmql
...
- [ ] Trace LMQL inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check LMQL defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check LMQL least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check LMQL resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check LMQL release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check LMQL dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If LMQL identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a LMQL change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- LMQL documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a LMQL architecture and dependency summary with trust boundaries and persistent state.
- Include LMQL validation commands or tests that are safe for the stated environment.
...
- LMQL identity, version, configuration, dependencies, and deployment assumptions are explicit.
- LMQL failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that LMQL is secure, supported, compatible, or suitable.
- Using an unpinned latest version of LMQL as the basis for migration or production guidance.
