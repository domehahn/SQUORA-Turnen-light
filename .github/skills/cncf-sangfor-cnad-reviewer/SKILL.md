...
---
# Cncf Sangfor Cnad Reviewer

## Purpose

Review and operate Sangfor CNAD using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Sangfor CNAD code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Sangfor CNAD requires evidence-backed assessment.
...
- First-party source: https://www.sangfor.com.cn/product-and-solution/sangfor-its/5
...
- [ ] Trace Sangfor CNAD inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Sangfor CNAD defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Sangfor CNAD least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Sangfor CNAD resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Sangfor CNAD release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Sangfor CNAD dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Sangfor CNAD identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Sangfor CNAD change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Sangfor CNAD documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Sangfor CNAD architecture and dependency summary with trust boundaries and persistent state.
- Include Sangfor CNAD validation commands or tests that are safe for the stated environment.
...
- Sangfor CNAD identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Sangfor CNAD failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Sangfor CNAD is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Sangfor CNAD as the basis for migration or production guidance.
