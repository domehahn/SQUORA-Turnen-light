...
---
# Cncf Google Kubernetes Engine Gke Reviewer

## Purpose

Review and operate Google Kubernetes Engine (GKE) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Google Kubernetes Engine (GKE) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Google Kubernetes Engine (GKE) requires evidence-backed assessment.
...
- First-party source: https://cloud.google.com/kubernetes-engine/
...
- [ ] Trace Google Kubernetes Engine (GKE) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Google Kubernetes Engine (GKE) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Google Kubernetes Engine (GKE) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Google Kubernetes Engine (GKE) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Google Kubernetes Engine (GKE) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Google Kubernetes Engine (GKE) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Google Kubernetes Engine (GKE) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Google Kubernetes Engine (GKE) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Google Kubernetes Engine (GKE) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Google Kubernetes Engine (GKE) architecture and dependency summary with trust boundaries and persistent state.
- Include Google Kubernetes Engine (GKE) validation commands or tests that are safe for the stated environment.
...
- Google Kubernetes Engine (GKE) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Google Kubernetes Engine (GKE) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Google Kubernetes Engine (GKE) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Google Kubernetes Engine (GKE) as the basis for migration or production guidance.
