---
name: "cncf-xgboost-reviewer"
description: "Review XGBoost using its official documentation and repository in the Data / Data Science category. An optimized distributed gradient boosting library for machine learning."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Xgboost Reviewer

## Purpose

Review and operate XGBoost using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- XGBoost code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving XGBoost requires evidence-backed assessment.
...
- First-party source: https://github.com/dmlc/xgboost
...
- [ ] Trace XGBoost inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check XGBoost defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check XGBoost least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check XGBoost resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check XGBoost release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check XGBoost dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If XGBoost identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a XGBoost change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- XGBoost documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a XGBoost architecture and dependency summary with trust boundaries and persistent state.
- Include XGBoost validation commands or tests that are safe for the stated environment.
...
- XGBoost identity, version, configuration, dependencies, and deployment assumptions are explicit.
- XGBoost failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that XGBoost is secure, supported, compatible, or suitable.
- Using an unpinned latest version of XGBoost as the basis for migration or production guidance.
