---
name: "cncf-ffmpeg-reviewer"
description: "Review FFmpeg using its official documentation and repository in the Wasm / AI/Machine Learning category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Ffmpeg Reviewer

## Purpose

Review and operate FFmpeg using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- FFmpeg code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving FFmpeg requires evidence-backed assessment.
...
- First-party source: https://github.com/FFmpeg/FFmpeg
...
- [ ] Trace FFmpeg inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check FFmpeg defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check FFmpeg least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check FFmpeg resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check FFmpeg release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check FFmpeg dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If FFmpeg identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a FFmpeg change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- FFmpeg documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a FFmpeg architecture and dependency summary with trust boundaries and persistent state.
- Include FFmpeg validation commands or tests that are safe for the stated environment.
...
- FFmpeg identity, version, configuration, dependencies, and deployment assumptions are explicit.
- FFmpeg failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that FFmpeg is secure, supported, compatible, or suitable.
- Using an unpinned latest version of FFmpeg as the basis for migration or production guidance.
