---
name: "cncf-kubeasz-reviewer"
description: "Review Kubeasz using its official documentation and repository in the Platform / Certified Kubernetes - Installer category. Kubeasz is a tool to deploy a Production Ready Kubernetes Cluster with ansible playbooks."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kubeasz Reviewer

## Purpose

Review and operate Kubeasz using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kubeasz code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kubeasz requires evidence-backed assessment.
...
- First-party source: https://github.com/easzlab/kubeasz
...
- [ ] Trace Kubeasz inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kubeasz defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kubeasz least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kubeasz resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kubeasz release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kubeasz dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kubeasz identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kubeasz change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kubeasz documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kubeasz architecture and dependency summary with trust boundaries and persistent state.
- Include Kubeasz validation commands or tests that are safe for the stated environment.
...
- Kubeasz identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kubeasz failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kubeasz is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kubeasz as the basis for migration or production guidance.
