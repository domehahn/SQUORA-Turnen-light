---
name: "cncf-kubeflow-reviewer"
description: "Review Kubeflow using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. Kubeflow is the foundation of tools for AI Platforms on Kubernetes."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kubeflow Reviewer

## Purpose

Review and operate Kubeflow using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kubeflow code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kubeflow requires evidence-backed assessment.
...
- [ ] Trace Kubeflow inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kubeflow defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kubeflow least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kubeflow resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kubeflow release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kubeflow dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kubeflow identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kubeflow change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kubeflow documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kubeflow architecture and dependency summary with trust boundaries and persistent state.
- Include Kubeflow validation commands or tests that are safe for the stated environment.
...
- Kubeflow identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kubeflow failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kubeflow is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kubeflow as the basis for migration or production guidance.
