---
name: "cncf-spinnaker-reviewer"
description: "Review Spinnaker using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Spinnaker Reviewer

## Purpose

Review and operate Spinnaker using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Spinnaker code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Spinnaker requires evidence-backed assessment.
...
- First-party source: https://github.com/spinnaker/spinnaker
...
- [ ] Trace Spinnaker inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Spinnaker defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Spinnaker least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Spinnaker resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Spinnaker release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Spinnaker dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Spinnaker identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Spinnaker change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Spinnaker documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Spinnaker architecture and dependency summary with trust boundaries and persistent state.
- Include Spinnaker validation commands or tests that are safe for the stated environment.
...
- Spinnaker identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Spinnaker failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Spinnaker is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Spinnaker as the basis for migration or production guidance.
