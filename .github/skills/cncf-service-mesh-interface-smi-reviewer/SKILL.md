---
name: "cncf-service-mesh-interface-smi-reviewer"
description: "Review Service Mesh Interface (SMI) using its official documentation and repository in the Orchestration & Management / Service Mesh category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Service Mesh Interface Smi Reviewer

## Purpose

Review and operate Service Mesh Interface (SMI) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Service Mesh Interface (SMI) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Service Mesh Interface (SMI) requires evidence-backed assessment.
...
- CNCF classification: Orchestration & Management / Service Mesh
- CNCF project maturity field: archived
- First-party source: https://github.com/servicemeshinterface/smi-spec
...
- [ ] Trace Service Mesh Interface (SMI) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Service Mesh Interface (SMI) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Service Mesh Interface (SMI) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Service Mesh Interface (SMI) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Service Mesh Interface (SMI) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Service Mesh Interface (SMI) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Service Mesh Interface (SMI) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Service Mesh Interface (SMI) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Service Mesh Interface (SMI) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Service Mesh Interface (SMI) architecture and dependency summary with trust boundaries and persistent state.
- Include Service Mesh Interface (SMI) validation commands or tests that are safe for the stated environment.
...
- Service Mesh Interface (SMI) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Service Mesh Interface (SMI) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Service Mesh Interface (SMI) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Service Mesh Interface (SMI) as the basis for migration or production guidance.
