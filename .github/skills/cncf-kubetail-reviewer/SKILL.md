---
name: "cncf-kubetail-reviewer"
description: "Review Kubetail using its official documentation and repository in the Observability and Analysis / Observability category. Real-time logging dashboard for Kubernetes"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kubetail Reviewer

## Purpose

Review and operate Kubetail using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kubetail code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kubetail requires evidence-backed assessment.
...
- First-party source: https://github.com/kubetail-org/kubetail
...
- [ ] Trace Kubetail inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kubetail defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kubetail least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kubetail resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kubetail release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kubetail dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kubetail identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kubetail change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kubetail documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kubetail architecture and dependency summary with trust boundaries and persistent state.
- Include Kubetail validation commands or tests that are safe for the stated environment.
...
- Kubetail identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kubetail failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kubetail is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kubetail as the basis for migration or production guidance.
