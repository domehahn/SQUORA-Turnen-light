...
---
# Cncf Stackit Kubernetes Engine Reviewer

## Purpose

Review and operate STACKIT Kubernetes Engine using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- STACKIT Kubernetes Engine code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving STACKIT Kubernetes Engine requires evidence-backed assessment.
...
- [ ] Trace STACKIT Kubernetes Engine inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check STACKIT Kubernetes Engine defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check STACKIT Kubernetes Engine least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check STACKIT Kubernetes Engine resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check STACKIT Kubernetes Engine release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check STACKIT Kubernetes Engine dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If STACKIT Kubernetes Engine identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a STACKIT Kubernetes Engine change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- STACKIT Kubernetes Engine documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a STACKIT Kubernetes Engine architecture and dependency summary with trust boundaries and persistent state.
- Include STACKIT Kubernetes Engine validation commands or tests that are safe for the stated environment.
...
- STACKIT Kubernetes Engine identity, version, configuration, dependencies, and deployment assumptions are explicit.
- STACKIT Kubernetes Engine failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that STACKIT Kubernetes Engine is secure, supported, compatible, or suitable.
- Using an unpinned latest version of STACKIT Kubernetes Engine as the basis for migration or production guidance.
