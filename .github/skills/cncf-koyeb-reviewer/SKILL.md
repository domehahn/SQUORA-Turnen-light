...
Review and operate Koyeb using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Koyeb code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Koyeb requires evidence-backed assessment.
...
- First-party source: https://www.koyeb.com
...
- [ ] Trace Koyeb inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Koyeb defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Koyeb least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Koyeb resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Koyeb release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Koyeb dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Koyeb identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Koyeb change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Koyeb documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Koyeb architecture and dependency summary with trust boundaries and persistent state.
- Include Koyeb validation commands or tests that are safe for the stated environment.
...
- Koyeb identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Koyeb failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Koyeb is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Koyeb as the basis for migration or production guidance.
