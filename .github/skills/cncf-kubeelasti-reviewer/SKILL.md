...
---
# Cncf Kubeelasti Reviewer

## Purpose

Review and operate KubeElasti using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeElasti code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeElasti requires evidence-backed assessment.
...
- First-party source: https://github.com/KubeElasti/KubeElasti
...
- [ ] Trace KubeElasti inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeElasti defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeElasti least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeElasti resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeElasti release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeElasti dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeElasti identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeElasti change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeElasti documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeElasti architecture and dependency summary with trust boundaries and persistent state.
- Include KubeElasti validation commands or tests that are safe for the stated environment.
...
- KubeElasti identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeElasti failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeElasti is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeElasti as the basis for migration or production guidance.
