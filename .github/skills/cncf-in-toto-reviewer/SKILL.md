---
name: "cncf-in-toto-reviewer"
description: "Review in-toto using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf In Toto Reviewer

## Purpose

Review and operate in-toto using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- in-toto code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving in-toto requires evidence-backed assessment.
...
- First-party source: https://github.com/in-toto/in-toto
...
- [ ] Trace in-toto inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check in-toto defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check in-toto least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check in-toto resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check in-toto release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check in-toto dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If in-toto identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a in-toto change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- in-toto documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a in-toto architecture and dependency summary with trust boundaries and persistent state.
- Include in-toto validation commands or tests that are safe for the stated environment.
...
- in-toto identity, version, configuration, dependencies, and deployment assumptions are explicit.
- in-toto failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that in-toto is secure, supported, compatible, or suitable.
- Using an unpinned latest version of in-toto as the basis for migration or production guidance.
