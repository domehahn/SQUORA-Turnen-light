---
name: "cncf-okteto-reviewer"
description: "Review Okteto using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Okteto Reviewer

## Purpose

Review and operate Okteto using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Okteto code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Okteto requires evidence-backed assessment.
...
- First-party source: https://github.com/okteto/okteto
...
- [ ] Trace Okteto inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Okteto defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Okteto least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Okteto resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Okteto release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Okteto dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Okteto identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Okteto change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Okteto documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Okteto architecture and dependency summary with trust boundaries and persistent state.
- Include Okteto validation commands or tests that are safe for the stated environment.
...
- Okteto identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Okteto failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Okteto is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Okteto as the basis for migration or production guidance.
