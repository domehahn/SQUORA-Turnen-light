---
name: "cncf-mosn-reviewer"
description: "Review MOSN using its official documentation and repository in the Orchestration & Management / Service Proxy category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate MOSN using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- MOSN code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving MOSN requires evidence-backed assessment.
...
- First-party source: https://github.com/mosn/mosn
...
- [ ] Trace MOSN inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check MOSN defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check MOSN least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check MOSN resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check MOSN release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check MOSN dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If MOSN identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a MOSN change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- MOSN documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a MOSN architecture and dependency summary with trust boundaries and persistent state.
- Include MOSN validation commands or tests that are safe for the stated environment.
...
- MOSN identity, version, configuration, dependencies, and deployment assumptions are explicit.
- MOSN failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that MOSN is secure, supported, compatible, or suitable.
- Using an unpinned latest version of MOSN as the basis for migration or production guidance.
