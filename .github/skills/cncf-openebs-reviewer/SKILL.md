---
name: "cncf-openebs-reviewer"
description: "Review OpenEBS using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Openebs Reviewer

## Purpose

Review and operate OpenEBS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenEBS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenEBS requires evidence-backed assessment.
...
- First-party source: https://github.com/openebs/openebs
...
- [ ] Trace OpenEBS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenEBS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenEBS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenEBS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenEBS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenEBS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenEBS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenEBS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenEBS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenEBS architecture and dependency summary with trust boundaries and persistent state.
- Include OpenEBS validation commands or tests that are safe for the stated environment.
...
- OpenEBS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenEBS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenEBS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenEBS as the basis for migration or production guidance.
