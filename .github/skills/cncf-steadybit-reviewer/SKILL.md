...
Review and operate steadybit using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- steadybit code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving steadybit requires evidence-backed assessment.
...
- First-party source: https://www.steadybit.com/
...
- [ ] Trace steadybit inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check steadybit defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check steadybit least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check steadybit resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check steadybit release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check steadybit dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If steadybit identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a steadybit change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- steadybit documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a steadybit architecture and dependency summary with trust boundaries and persistent state.
- Include steadybit validation commands or tests that are safe for the stated environment.
...
- steadybit identity, version, configuration, dependencies, and deployment assumptions are explicit.
- steadybit failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that steadybit is secure, supported, compatible, or suitable.
- Using an unpinned latest version of steadybit as the basis for migration or production guidance.
