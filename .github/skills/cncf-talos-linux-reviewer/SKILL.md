---
name: "cncf-talos-linux-reviewer"
description: "Review Talos Linux using its official documentation and repository in the Platform / Certified Kubernetes - Distribution category. Talos Linux is a single purpose distribution for Kubernetes. It provides an API driven—no SSH—operating system to deploy vanilla Kubernetes anywhere."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Talos Linux Reviewer

## Purpose

Review and operate Talos Linux using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Talos Linux code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Talos Linux requires evidence-backed assessment.
...
- First-party source: https://talos.dev/
...
- [ ] Trace Talos Linux inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Talos Linux defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Talos Linux least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Talos Linux resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Talos Linux release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Talos Linux dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Talos Linux identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Talos Linux change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Talos Linux documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Talos Linux architecture and dependency summary with trust boundaries and persistent state.
- Include Talos Linux validation commands or tests that are safe for the stated environment.
...
- Talos Linux identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Talos Linux failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Talos Linux is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Talos Linux as the basis for migration or production guidance.
