...
---
# Cncf Kuadrant Reviewer

## Purpose

Review and operate Kuadrant using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kuadrant code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kuadrant requires evidence-backed assessment.
...
- First-party source: https://github.com/kuadrant/kuadrant-operator
...
- [ ] Trace Kuadrant inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kuadrant defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kuadrant least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kuadrant resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kuadrant release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kuadrant dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kuadrant identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kuadrant change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kuadrant documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kuadrant architecture and dependency summary with trust boundaries and persistent state.
- Include Kuadrant validation commands or tests that are safe for the stated environment.
...
- Kuadrant identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kuadrant failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kuadrant is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kuadrant as the basis for migration or production guidance.
