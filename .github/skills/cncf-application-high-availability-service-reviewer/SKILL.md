...
---
# Cncf Application High Availability Service Reviewer

## Purpose

Review and operate Application High Availability Service using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Application High Availability Service code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Application High Availability Service requires evidence-backed assessment.
...
- First-party source: https://www.alibabacloud.com/product/ahas
...
- [ ] Trace Application High Availability Service inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Application High Availability Service defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Application High Availability Service least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Application High Availability Service resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Application High Availability Service release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Application High Availability Service dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Application High Availability Service identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Application High Availability Service change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Application High Availability Service documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Application High Availability Service architecture and dependency summary with trust boundaries and persistent state.
- Include Application High Availability Service validation commands or tests that are safe for the stated environment.
...
- Application High Availability Service identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Application High Availability Service failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Application High Availability Service is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Application High Availability Service as the basis for migration or production guidance.
