---
name: "cncf-suse-kcsp-reviewer"
description: "Review SUSE (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. SUSE is here to help simplify your business modernization journey offering reliable and interoperable cloud native solutions with consulting, training and support services to keep up with the pace of change and your expectations."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Suse Kcsp Reviewer

## Purpose

Review and operate SUSE (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SUSE (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SUSE (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.suse.com/products/suse-rancher/
...
- [ ] Trace SUSE (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SUSE (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SUSE (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SUSE (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SUSE (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SUSE (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SUSE (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SUSE (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SUSE (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SUSE (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include SUSE (KCSP) validation commands or tests that are safe for the stated environment.
...
- SUSE (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SUSE (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SUSE (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SUSE (KCSP) as the basis for migration or production guidance.
