---
name: "cncf-mockserver-reviewer"
description: "Review MockServer using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category. Mocking, debugging proxy and chaos engineering for HTTP, gRPC, GraphQL, LLM, MCP, Kafka, TCP and more."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Mockserver Reviewer

## Purpose

Review and operate MockServer using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- MockServer code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving MockServer requires evidence-backed assessment.
...
- CNCF classification: App Definition and Development / Continuous Integration & Delivery; Observability and Analysis / Chaos Engineering
- CNCF project maturity field: not specified
- First-party source: https://github.com/mock-server/mockserver-monorepo
...
- [ ] Trace MockServer inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check MockServer defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check MockServer least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check MockServer resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check MockServer release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check MockServer dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If MockServer identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a MockServer change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- MockServer documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a MockServer architecture and dependency summary with trust boundaries and persistent state.
- Include MockServer validation commands or tests that are safe for the stated environment.
...
- MockServer identity, version, configuration, dependencies, and deployment assumptions are explicit.
- MockServer failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that MockServer is secure, supported, compatible, or suitable.
- Using an unpinned latest version of MockServer as the basis for migration or production guidance.
