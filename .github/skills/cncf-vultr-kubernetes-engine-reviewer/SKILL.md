...
---
# Cncf Vultr Kubernetes Engine Reviewer

## Purpose

Review and operate Vultr Kubernetes Engine using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Vultr Kubernetes Engine code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Vultr Kubernetes Engine requires evidence-backed assessment.
...
- First-party source: https://www.vultr.com/kubernetes/
...
- [ ] Trace Vultr Kubernetes Engine inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Vultr Kubernetes Engine defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Vultr Kubernetes Engine least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Vultr Kubernetes Engine resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Vultr Kubernetes Engine release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Vultr Kubernetes Engine dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Vultr Kubernetes Engine identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Vultr Kubernetes Engine change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Vultr Kubernetes Engine documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Vultr Kubernetes Engine architecture and dependency summary with trust boundaries and persistent state.
- Include Vultr Kubernetes Engine validation commands or tests that are safe for the stated environment.
...
- Vultr Kubernetes Engine identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Vultr Kubernetes Engine failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Vultr Kubernetes Engine is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Vultr Kubernetes Engine as the basis for migration or production guidance.
