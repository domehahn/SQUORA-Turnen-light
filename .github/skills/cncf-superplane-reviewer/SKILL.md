---
name: "cncf-superplane-reviewer"
description: "Review SuperPlane using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. Open source DevOps control plane for event-driven workflows."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Superplane Reviewer

## Purpose

Review and operate SuperPlane using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SuperPlane code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SuperPlane requires evidence-backed assessment.
...
- First-party source: https://github.com/superplanehq/superplane
...
- [ ] Trace SuperPlane inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SuperPlane defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SuperPlane least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SuperPlane resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SuperPlane release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SuperPlane dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SuperPlane identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SuperPlane change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SuperPlane documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SuperPlane architecture and dependency summary with trust boundaries and persistent state.
- Include SuperPlane validation commands or tests that are safe for the stated environment.
...
- SuperPlane identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SuperPlane failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SuperPlane is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SuperPlane as the basis for migration or production guidance.
