---
name: "cncf-coredge-kcsp-reviewer"
description: "Review Coredge (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Coredge helps to swiftly deploy, manage, and maintain Kubernetes with the help of our comprehensive Kubernetes professional services that accelerates your transformation journey with our innovative solutions."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Coredge Kcsp Reviewer

## Purpose

Review and operate Coredge (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Coredge (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Coredge (KCSP) requires evidence-backed assessment.
...
- First-party source: https://coredge.io/kubernetes-services/
...
- [ ] Trace Coredge (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Coredge (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Coredge (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Coredge (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Coredge (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Coredge (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Coredge (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Coredge (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Coredge (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Coredge (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Coredge (KCSP) validation commands or tests that are safe for the stated environment.
...
- Coredge (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Coredge (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Coredge (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Coredge (KCSP) as the basis for migration or production guidance.
