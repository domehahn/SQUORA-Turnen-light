---
name: "cncf-google-container-registry-reviewer"
description: "Review Google Container Registry using its official documentation and repository in the Provisioning / Container Registry category. Container Registry is a single place for your team to manage Docker images, perform vulnerability analysis, and decide who can access what with fine-grained access control."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Google Container Registry Reviewer

## Purpose

Review and operate Google Container Registry using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Google Container Registry code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Google Container Registry requires evidence-backed assessment.
...
- First-party source: https://cloud.google.com/container-registry/
...
- [ ] Trace Google Container Registry inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Google Container Registry defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Google Container Registry least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Google Container Registry resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Google Container Registry release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Google Container Registry dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Google Container Registry identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Google Container Registry change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Google Container Registry documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Google Container Registry architecture and dependency summary with trust boundaries and persistent state.
- Include Google Container Registry validation commands or tests that are safe for the stated environment.
...
- Google Container Registry identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Google Container Registry failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Google Container Registry is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Google Container Registry as the basis for migration or production guidance.
