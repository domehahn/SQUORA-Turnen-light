...
---
# Cncf Kubermatic Kubernetes Platform Reviewer

## Purpose

Review and operate Kubermatic Kubernetes Platform using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kubermatic Kubernetes Platform code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kubermatic Kubernetes Platform requires evidence-backed assessment.
...
- First-party source: https://github.com/kubermatic/kubermatic
...
- [ ] Trace Kubermatic Kubernetes Platform inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kubermatic Kubernetes Platform defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kubermatic Kubernetes Platform least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kubermatic Kubernetes Platform resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kubermatic Kubernetes Platform release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kubermatic Kubernetes Platform dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kubermatic Kubernetes Platform identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kubermatic Kubernetes Platform change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kubermatic Kubernetes Platform documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kubermatic Kubernetes Platform architecture and dependency summary with trust boundaries and persistent state.
- Include Kubermatic Kubernetes Platform validation commands or tests that are safe for the stated environment.
...
- Kubermatic Kubernetes Platform identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kubermatic Kubernetes Platform failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kubermatic Kubernetes Platform is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kubermatic Kubernetes Platform as the basis for migration or production guidance.
