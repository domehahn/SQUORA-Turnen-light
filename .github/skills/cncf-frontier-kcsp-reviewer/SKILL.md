...
---
# Cncf Frontier Kcsp Reviewer

## Purpose

Review and operate Frontier (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Frontier (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Frontier (KCSP) requires evidence-backed assessment.
...
- First-party source: https://frontierhq.com/kubernetes
...
- [ ] Trace Frontier (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Frontier (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Frontier (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Frontier (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Frontier (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Frontier (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Frontier (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Frontier (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Frontier (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Frontier (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Frontier (KCSP) validation commands or tests that are safe for the stated environment.
...
- Frontier (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Frontier (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Frontier (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Frontier (KCSP) as the basis for migration or production guidance.
