---
name: "cncf-nomad-reviewer"
description: "Review Nomad using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Nomad using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Nomad code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Nomad requires evidence-backed assessment.
...
- First-party source: https://github.com/hashicorp/nomad
...
- [ ] Trace Nomad inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Nomad defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Nomad least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Nomad resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Nomad release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Nomad dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Nomad identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Nomad change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Nomad documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Nomad architecture and dependency summary with trust boundaries and persistent state.
- Include Nomad validation commands or tests that are safe for the stated environment.
...
- Nomad identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Nomad failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Nomad is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Nomad as the basis for migration or production guidance.
