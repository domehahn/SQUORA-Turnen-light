---
name: "cncf-haproxy-reviewer"
description: "Review HAProxy using its official documentation and repository in the Orchestration & Management / Service Proxy category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Haproxy Reviewer

## Purpose

Review and operate HAProxy using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- HAProxy code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving HAProxy requires evidence-backed assessment.
...
- First-party source: https://github.com/haproxy/haproxy
...
- [ ] Trace HAProxy inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check HAProxy defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check HAProxy least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check HAProxy resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check HAProxy release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check HAProxy dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If HAProxy identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a HAProxy change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- HAProxy documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a HAProxy architecture and dependency summary with trust boundaries and persistent state.
- Include HAProxy validation commands or tests that are safe for the stated environment.
...
- HAProxy identity, version, configuration, dependencies, and deployment assumptions are explicit.
- HAProxy failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that HAProxy is secure, supported, compatible, or suitable.
- Using an unpinned latest version of HAProxy as the basis for migration or production guidance.
