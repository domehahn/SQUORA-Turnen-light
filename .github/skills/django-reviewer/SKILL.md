...
description: "Review Django models, migrations, ORM usage, authentication, middleware, settings, tests, and deployment safety."
version: "0.1.0"
since: "2026-08-27"
...
- Source code, configuration, dependencies, tests, deployment artifacts, or runtime behavior in Django applications change.
...
- models and migrations, ORM queries, authentication, middleware, settings, deployment checks
...
- [ ] Trace Django applications inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Django applications defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Django applications least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Django applications resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Django applications release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
...
- If Django applications identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Django applications change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Django applications documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Django applications architecture and dependency summary with trust boundaries and persistent state.
- Include Django applications validation commands or tests that are safe for the stated environment.
...
- Django applications identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Django applications failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Django applications is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Django applications as the basis for migration or production guidance.
