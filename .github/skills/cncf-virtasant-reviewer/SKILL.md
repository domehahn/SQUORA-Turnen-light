...
---
# Cncf Virtasant Reviewer

## Purpose

Review and operate Virtasant using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Virtasant code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Virtasant requires evidence-backed assessment.
...
- First-party source: https://www.virtasant.com/services/cloud-cost-optimization
...
- [ ] Trace Virtasant inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Virtasant defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Virtasant least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Virtasant resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Virtasant release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Virtasant dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Virtasant identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Virtasant change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Virtasant documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Virtasant architecture and dependency summary with trust boundaries and persistent state.
- Include Virtasant validation commands or tests that are safe for the stated environment.
...
- Virtasant identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Virtasant failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Virtasant is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Virtasant as the basis for migration or production guidance.
