---
name: "cncf-neo4j-reviewer"
description: "Review Neo4j using its official documentation and repository in the App Definition and Development / Database category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Neo4j using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Neo4j code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Neo4j requires evidence-backed assessment.
...
- First-party source: https://github.com/neo4j/neo4j
...
- [ ] Trace Neo4j inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Neo4j defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Neo4j least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Neo4j resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Neo4j release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Neo4j dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Neo4j identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Neo4j change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Neo4j documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Neo4j architecture and dependency summary with trust boundaries and persistent state.
- Include Neo4j validation commands or tests that are safe for the stated environment.
...
- Neo4j identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Neo4j failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Neo4j is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Neo4j as the basis for migration or production guidance.
