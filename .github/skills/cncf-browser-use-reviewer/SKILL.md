---
name: "cncf-browser-use-reviewer"
description: "Review Browser Use using its official documentation and repository in the AI Agent / Agent Tool category. Make websites accessible for AI agents. Automate tasks online with ease."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Browser Use Reviewer

## Purpose

Review and operate Browser Use using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Browser Use code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Browser Use requires evidence-backed assessment.
...
- First-party source: https://github.com/browser-use/browser-use
...
- [ ] Trace Browser Use inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Browser Use defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Browser Use least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Browser Use resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Browser Use release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Browser Use dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Browser Use identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Browser Use change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Browser Use documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Browser Use architecture and dependency summary with trust boundaries and persistent state.
- Include Browser Use validation commands or tests that are safe for the stated environment.
...
- Browser Use identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Browser Use failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Browser Use is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Browser Use as the basis for migration or production guidance.
