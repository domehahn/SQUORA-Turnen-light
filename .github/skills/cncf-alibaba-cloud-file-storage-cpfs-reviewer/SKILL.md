...
---
# Cncf Alibaba Cloud File Storage Cpfs Reviewer

## Purpose

Review and operate Alibaba Cloud File Storage CPFS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Alibaba Cloud File Storage CPFS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Alibaba Cloud File Storage CPFS requires evidence-backed assessment.
...
- First-party source: https://www.alibabacloud.com/
...
- [ ] Trace Alibaba Cloud File Storage CPFS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Alibaba Cloud File Storage CPFS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Alibaba Cloud File Storage CPFS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Alibaba Cloud File Storage CPFS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Alibaba Cloud File Storage CPFS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Alibaba Cloud File Storage CPFS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Alibaba Cloud File Storage CPFS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Alibaba Cloud File Storage CPFS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Alibaba Cloud File Storage CPFS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Alibaba Cloud File Storage CPFS architecture and dependency summary with trust boundaries and persistent state.
- Include Alibaba Cloud File Storage CPFS validation commands or tests that are safe for the stated environment.
...
- Alibaba Cloud File Storage CPFS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Alibaba Cloud File Storage CPFS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Alibaba Cloud File Storage CPFS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Alibaba Cloud File Storage CPFS as the basis for migration or production guidance.
