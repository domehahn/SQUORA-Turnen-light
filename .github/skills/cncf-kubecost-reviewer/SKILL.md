---
name: "cncf-kubecost-reviewer"
description: "Review Kubecost using its official documentation and repository in the Observability and Analysis / Continuous Optimization category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kubecost Reviewer

## Purpose

Review and operate Kubecost using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kubecost code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kubecost requires evidence-backed assessment.
...
- First-party source: https://github.com/kubecost/cost-model
...
- [ ] Trace Kubecost inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kubecost defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kubecost least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kubecost resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kubecost release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kubecost dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kubecost identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kubecost change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kubecost documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kubecost architecture and dependency summary with trust boundaries and persistent state.
- Include Kubecost validation commands or tests that are safe for the stated environment.
...
- Kubecost identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kubecost failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kubecost is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kubecost as the basis for migration or production guidance.
