---
name: "cncf-kubeflow-spark-operator-reviewer"
description: "Review Kubeflow Spark Operator using its official documentation and repository in the Data / Data Architecture category. Operator to manage Apache Spark applications on Kubernetes."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kubeflow Spark Operator Reviewer

## Purpose

Review and operate Kubeflow Spark Operator using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kubeflow Spark Operator code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kubeflow Spark Operator requires evidence-backed assessment.
...
- CNCF classification: Data / Data Architecture
- CNCF project maturity field: not specified
- First-party source: https://github.com/kubeflow/spark-operator
...
- [ ] Trace Kubeflow Spark Operator inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kubeflow Spark Operator defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kubeflow Spark Operator least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kubeflow Spark Operator resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kubeflow Spark Operator release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kubeflow Spark Operator dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kubeflow Spark Operator identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kubeflow Spark Operator change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kubeflow Spark Operator documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kubeflow Spark Operator architecture and dependency summary with trust boundaries and persistent state.
- Include Kubeflow Spark Operator validation commands or tests that are safe for the stated environment.
...
- Kubeflow Spark Operator identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kubeflow Spark Operator failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kubeflow Spark Operator is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kubeflow Spark Operator as the basis for migration or production guidance.
