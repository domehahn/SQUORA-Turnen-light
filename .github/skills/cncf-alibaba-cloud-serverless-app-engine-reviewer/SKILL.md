...
---
# Cncf Alibaba Cloud Serverless App Engine Reviewer

## Purpose

Review and operate Alibaba Cloud Serverless App Engine using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Alibaba Cloud Serverless App Engine code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Alibaba Cloud Serverless App Engine requires evidence-backed assessment.
...
- CNCF classification: Serverless / Hosted Platform
- CNCF project maturity field: not specified
- First-party source: https://cn.aliyun.com/product/aliware/sae
...
- [ ] Trace Alibaba Cloud Serverless App Engine inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Alibaba Cloud Serverless App Engine defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Alibaba Cloud Serverless App Engine least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Alibaba Cloud Serverless App Engine resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Alibaba Cloud Serverless App Engine release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Alibaba Cloud Serverless App Engine dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Alibaba Cloud Serverless App Engine identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Alibaba Cloud Serverless App Engine change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Alibaba Cloud Serverless App Engine documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Alibaba Cloud Serverless App Engine architecture and dependency summary with trust boundaries and persistent state.
- Include Alibaba Cloud Serverless App Engine validation commands or tests that are safe for the stated environment.
...
- Alibaba Cloud Serverless App Engine identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Alibaba Cloud Serverless App Engine failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Alibaba Cloud Serverless App Engine is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Alibaba Cloud Serverless App Engine as the basis for migration or production guidance.
