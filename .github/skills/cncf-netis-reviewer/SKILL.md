---
name: "cncf-netis-reviewer"
description: "Review Netis using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Netis using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Netis code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Netis requires evidence-backed assessment.
...
- First-party source: https://github.com/Netis/packet-agent
...
- [ ] Trace Netis inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Netis defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Netis least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Netis resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Netis release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Netis dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Netis identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Netis change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Netis documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Netis architecture and dependency summary with trust boundaries and persistent state.
- Include Netis validation commands or tests that are safe for the stated environment.
...
- Netis identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Netis failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Netis is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Netis as the basis for migration or production guidance.
