---
name: "cncf-net-reviewer"
description: "Review .NET using its official documentation and repository in the Wasm / Languages category. Managed language"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate .NET using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- .NET code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving .NET requires evidence-backed assessment.
...
- First-party source: https://github.com/dotnet/runtime
...
- [ ] Trace .NET inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check .NET defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check .NET least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check .NET resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check .NET release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check .NET dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If .NET identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a .NET change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- .NET documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a .NET architecture and dependency summary with trust boundaries and persistent state.
- Include .NET validation commands or tests that are safe for the stated environment.
...
- .NET identity, version, configuration, dependencies, and deployment assumptions are explicit.
- .NET failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that .NET is secure, supported, compatible, or suitable.
- Using an unpinned latest version of .NET as the basis for migration or production guidance.
