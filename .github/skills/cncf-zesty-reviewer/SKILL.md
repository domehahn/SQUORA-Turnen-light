---
name: "cncf-zesty-reviewer"
description: "Review Zesty using its official documentation and repository in the Observability and Analysis / Continuous Optimization category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Zesty using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Zesty code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Zesty requires evidence-backed assessment.
...
- [ ] Trace Zesty inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Zesty defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Zesty least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Zesty resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Zesty release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Zesty dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Zesty identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Zesty change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Zesty documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Zesty architecture and dependency summary with trust boundaries and persistent state.
- Include Zesty validation commands or tests that are safe for the stated environment.
...
- Zesty identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Zesty failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Zesty is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Zesty as the basis for migration or production guidance.
