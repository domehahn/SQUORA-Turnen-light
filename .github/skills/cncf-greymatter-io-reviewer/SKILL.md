---
name: "cncf-greymatter-io-reviewer"
description: "Review greymatter.io using its official documentation and repository in the Orchestration & Management / Service Mesh category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Greymatter Io Reviewer

## Purpose

Review and operate greymatter.io using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- greymatter.io code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving greymatter.io requires evidence-backed assessment.
...
- First-party source: https://greymatter.io
...
- [ ] Trace greymatter.io inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check greymatter.io defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check greymatter.io least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check greymatter.io resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check greymatter.io release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check greymatter.io dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If greymatter.io identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a greymatter.io change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- greymatter.io documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a greymatter.io architecture and dependency summary with trust boundaries and persistent state.
- Include greymatter.io validation commands or tests that are safe for the stated environment.
...
- greymatter.io identity, version, configuration, dependencies, and deployment assumptions are explicit.
- greymatter.io failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that greymatter.io is secure, supported, compatible, or suitable.
- Using an unpinned latest version of greymatter.io as the basis for migration or production guidance.
