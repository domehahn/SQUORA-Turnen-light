---
name: "cncf-perses-reviewer"
description: "Review Perses using its official documentation and repository in the Observability and Analysis / Observability category. Perses is a dashboard tool to visualize observability data from Prometheus/Thanos/Jaeger."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Perses Reviewer

## Purpose

Review and operate Perses using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Perses code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Perses requires evidence-backed assessment.
...
- First-party source: https://github.com/perses/perses
...
- [ ] Trace Perses inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Perses defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Perses least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Perses resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Perses release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Perses dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Perses identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Perses change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Perses documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Perses architecture and dependency summary with trust boundaries and persistent state.
- Include Perses validation commands or tests that are safe for the stated environment.
...
- Perses identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Perses failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Perses is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Perses as the basis for migration or production guidance.
