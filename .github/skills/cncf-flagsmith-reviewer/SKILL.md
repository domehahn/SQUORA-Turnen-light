---
name: "cncf-flagsmith-reviewer"
description: "Review Flagsmith using its official documentation and repository in the Observability and Analysis / Feature Flagging category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Flagsmith Reviewer

## Purpose

Review and operate Flagsmith using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Flagsmith code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Flagsmith requires evidence-backed assessment.
...
- First-party source: https://github.com/Flagsmith/flagsmith
...
- [ ] Trace Flagsmith inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Flagsmith defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Flagsmith least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Flagsmith resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Flagsmith release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Flagsmith dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Flagsmith identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Flagsmith change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Flagsmith documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Flagsmith architecture and dependency summary with trust boundaries and persistent state.
- Include Flagsmith validation commands or tests that are safe for the stated environment.
...
- Flagsmith identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Flagsmith failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Flagsmith is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Flagsmith as the basis for migration or production guidance.
