---
name: "cncf-zte-kcsp-reviewer"
description: "Review ZTE (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Zte Kcsp Reviewer

## Purpose

Review and operate ZTE (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ZTE (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ZTE (KCSP) requires evidence-backed assessment.
...
- [ ] Trace ZTE (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ZTE (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ZTE (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ZTE (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ZTE (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ZTE (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ZTE (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ZTE (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ZTE (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ZTE (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include ZTE (KCSP) validation commands or tests that are safe for the stated environment.
...
- ZTE (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ZTE (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ZTE (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ZTE (KCSP) as the basis for migration or production guidance.
