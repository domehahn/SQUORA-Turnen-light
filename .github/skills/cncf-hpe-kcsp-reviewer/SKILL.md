---
name: "cncf-hpe-kcsp-reviewer"
description: "Review HPE (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. HPE Advisory & Professional Service provides various services from strategic advisory to design/implementation professional services related to Kubernetes and cloud native computing technologies."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Hpe Kcsp Reviewer

## Purpose

Review and operate HPE (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- HPE (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving HPE (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.hpe.com/us/en/services/consulting/cloud.html
...
- [ ] Trace HPE (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check HPE (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check HPE (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check HPE (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check HPE (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check HPE (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If HPE (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a HPE (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- HPE (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a HPE (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include HPE (KCSP) validation commands or tests that are safe for the stated environment.
...
- HPE (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- HPE (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that HPE (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of HPE (KCSP) as the basis for migration or production guidance.
