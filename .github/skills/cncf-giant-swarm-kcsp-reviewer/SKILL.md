---
name: "cncf-giant-swarm-kcsp-reviewer"
description: "Review Giant Swarm (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Giant Swarm provides fully-managed Kubernetes Clusters in your location of choice, so you can focus on your product."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Giant Swarm Kcsp Reviewer

## Purpose

Review and operate Giant Swarm (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Giant Swarm (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Giant Swarm (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.giantswarm.io/developer-platforms
...
- [ ] Trace Giant Swarm (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Giant Swarm (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Giant Swarm (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Giant Swarm (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Giant Swarm (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Giant Swarm (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Giant Swarm (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Giant Swarm (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Giant Swarm (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Giant Swarm (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Giant Swarm (KCSP) validation commands or tests that are safe for the stated environment.
...
- Giant Swarm (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Giant Swarm (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Giant Swarm (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Giant Swarm (KCSP) as the basis for migration or production guidance.
