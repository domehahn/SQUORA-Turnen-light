---
name: "cncf-mplat-reviewer"
description: "Review mPLAT using its official documentation and repository in the Observability and Analysis / Continuous Optimization category. mPLAT offers cloud-type service management functions necessary for businesses."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate mPLAT using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- mPLAT code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving mPLAT requires evidence-backed assessment.
...
- First-party source: https://senjufamily.nri.com/mplat/index.html
...
- [ ] Trace mPLAT inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check mPLAT defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check mPLAT least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check mPLAT resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check mPLAT release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check mPLAT dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If mPLAT identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a mPLAT change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- mPLAT documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a mPLAT architecture and dependency summary with trust boundaries and persistent state.
- Include mPLAT validation commands or tests that are safe for the stated environment.
...
- mPLAT identity, version, configuration, dependencies, and deployment assumptions are explicit.
- mPLAT failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that mPLAT is secure, supported, compatible, or suitable.
- Using an unpinned latest version of mPLAT as the basis for migration or production guidance.
