---
name: "cncf-redpanda-reviewer"
description: "Review Redpanda using its official documentation and repository in the App Definition and Development / Streaming & Messaging category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Redpanda Reviewer

## Purpose

Review and operate Redpanda using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Redpanda code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Redpanda requires evidence-backed assessment.
...
- First-party source: https://github.com/redpanda-data/redpanda
...
- [ ] Trace Redpanda inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Redpanda defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Redpanda least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Redpanda resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Redpanda release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Redpanda dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Redpanda identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Redpanda change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Redpanda documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Redpanda architecture and dependency summary with trust boundaries and persistent state.
- Include Redpanda validation commands or tests that are safe for the stated environment.
...
- Redpanda identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Redpanda failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Redpanda is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Redpanda as the basis for migration or production guidance.
