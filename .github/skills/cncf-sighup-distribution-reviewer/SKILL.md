...
Review and operate SIGHUP Distribution using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SIGHUP Distribution code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SIGHUP Distribution requires evidence-backed assessment.
...
- First-party source: https://github.com/sighupio/distribution
...
- [ ] Trace SIGHUP Distribution inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SIGHUP Distribution defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SIGHUP Distribution least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SIGHUP Distribution resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SIGHUP Distribution release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SIGHUP Distribution dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SIGHUP Distribution identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SIGHUP Distribution change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SIGHUP Distribution documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SIGHUP Distribution architecture and dependency summary with trust boundaries and persistent state.
- Include SIGHUP Distribution validation commands or tests that are safe for the stated environment.
...
- SIGHUP Distribution identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SIGHUP Distribution failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SIGHUP Distribution is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SIGHUP Distribution as the basis for migration or production guidance.
