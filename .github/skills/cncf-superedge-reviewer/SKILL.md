---
name: "cncf-superedge-reviewer"
description: "Review SuperEdge using its official documentation and repository in the Provisioning / Automation & Configuration category. An edge-native container management system for edge computing"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Superedge Reviewer

## Purpose

Review and operate SuperEdge using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SuperEdge code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SuperEdge requires evidence-backed assessment.
...
- First-party source: https://github.com/superedge/superedge
...
- [ ] Trace SuperEdge inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SuperEdge defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SuperEdge least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SuperEdge resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SuperEdge release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SuperEdge dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SuperEdge identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SuperEdge change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SuperEdge documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SuperEdge architecture and dependency summary with trust boundaries and persistent state.
- Include SuperEdge validation commands or tests that are safe for the stated environment.
...
- SuperEdge identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SuperEdge failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SuperEdge is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SuperEdge as the basis for migration or production guidance.
