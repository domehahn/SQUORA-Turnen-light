---
name: "cncf-signalfx-reviewer"
description: "Review SignalFX using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Signalfx Reviewer

## Purpose

Review and operate SignalFX using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SignalFX code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SignalFX requires evidence-backed assessment.
...
- First-party source: https://www.splunk.com/en_us/devops/signalfx.html
...
- [ ] Trace SignalFX inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SignalFX defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SignalFX least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SignalFX resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SignalFX release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SignalFX dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SignalFX identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SignalFX change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SignalFX documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SignalFX architecture and dependency summary with trust boundaries and persistent state.
- Include SignalFX validation commands or tests that are safe for the stated environment.
...
- SignalFX identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SignalFX failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SignalFX is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SignalFX as the basis for migration or production guidance.
