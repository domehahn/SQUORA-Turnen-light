---
name: "cncf-loxilb-reviewer"
description: "Review LoxiLB using its official documentation and repository in the Orchestration & Management / Service Proxy category. eBPF based cloud-native load-balancer. Powering Kubernetes|Edge|5G|IoT|XaaS Apps."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Loxilb Reviewer

## Purpose

Review and operate LoxiLB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- LoxiLB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving LoxiLB requires evidence-backed assessment.
...
- First-party source: https://github.com/loxilb-io/loxilb
...
- [ ] Trace LoxiLB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check LoxiLB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check LoxiLB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check LoxiLB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check LoxiLB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check LoxiLB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If LoxiLB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a LoxiLB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- LoxiLB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a LoxiLB architecture and dependency summary with trust boundaries and persistent state.
- Include LoxiLB validation commands or tests that are safe for the stated environment.
...
- LoxiLB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- LoxiLB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that LoxiLB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of LoxiLB as the basis for migration or production guidance.
