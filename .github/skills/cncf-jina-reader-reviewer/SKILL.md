---
name: "cncf-jina-reader-reviewer"
description: "Review Jina Reader using its official documentation and repository in the AI Agent / Agent Tool category. Convert any URL to an LLM-friendly input with a simple prefix"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Jina Reader Reviewer

## Purpose

Review and operate Jina Reader using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Jina Reader code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Jina Reader requires evidence-backed assessment.
...
- First-party source: https://github.com/jina-ai/reader
...
- [ ] Trace Jina Reader inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Jina Reader defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Jina Reader least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Jina Reader resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Jina Reader release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Jina Reader dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Jina Reader identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Jina Reader change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Jina Reader documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Jina Reader architecture and dependency summary with trust boundaries and persistent state.
- Include Jina Reader validation commands or tests that are safe for the stated environment.
...
- Jina Reader identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Jina Reader failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Jina Reader is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Jina Reader as the basis for migration or production guidance.
