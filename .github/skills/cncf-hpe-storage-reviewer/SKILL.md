---
name: "cncf-hpe-storage-reviewer"
description: "Review HPE Storage using its official documentation and repository in the Runtime / Cloud Native Storage category. The world’s most intelligent storage, built for our hybrid cloud world to help you unlock the full potential of your data."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Hpe Storage Reviewer

## Purpose

Review and operate HPE Storage using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- HPE Storage code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving HPE Storage requires evidence-backed assessment.
...
- First-party source: https://www.hpe.com/us/en/storage.html
...
- [ ] Trace HPE Storage inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check HPE Storage defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check HPE Storage least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check HPE Storage resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check HPE Storage release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check HPE Storage dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If HPE Storage identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a HPE Storage change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- HPE Storage documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a HPE Storage architecture and dependency summary with trust boundaries and persistent state.
- Include HPE Storage validation commands or tests that are safe for the stated environment.
...
- HPE Storage identity, version, configuration, dependencies, and deployment assumptions are explicit.
- HPE Storage failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that HPE Storage is secure, supported, compatible, or suitable.
- Using an unpinned latest version of HPE Storage as the basis for migration or production guidance.
