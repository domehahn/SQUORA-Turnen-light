---
name: "cncf-flannel-reviewer"
description: "Review Flannel using its official documentation and repository in the Runtime / Cloud Native Network category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Flannel Reviewer

## Purpose

Review and operate Flannel using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Flannel code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Flannel requires evidence-backed assessment.
...
- First-party source: https://github.com/flannel-io/flannel
...
- [ ] Trace Flannel inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Flannel defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Flannel least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Flannel resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Flannel release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Flannel dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Flannel identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Flannel change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Flannel documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Flannel architecture and dependency summary with trust boundaries and persistent state.
- Include Flannel validation commands or tests that are safe for the stated environment.
...
- Flannel identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Flannel failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Flannel is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Flannel as the basis for migration or production guidance.
