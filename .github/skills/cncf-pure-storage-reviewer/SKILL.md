---
name: "cncf-pure-storage-reviewer"
description: "Review Pure Storage using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Pure Storage Reviewer

## Purpose

Review and operate Pure Storage using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Pure Storage code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Pure Storage requires evidence-backed assessment.
...
- First-party source: https://www.purestorage.com/
...
- [ ] Trace Pure Storage inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Pure Storage defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Pure Storage least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Pure Storage resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Pure Storage release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Pure Storage dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Pure Storage identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Pure Storage change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Pure Storage documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Pure Storage architecture and dependency summary with trust boundaries and persistent state.
- Include Pure Storage validation commands or tests that are safe for the stated environment.
...
- Pure Storage identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Pure Storage failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Pure Storage is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Pure Storage as the basis for migration or production guidance.
