---
name: "cncf-mongodb-reviewer"
description: "Review MongoDB using its official documentation and repository in the App Definition and Development / Database category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Mongodb Reviewer

## Purpose

Review and operate MongoDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- MongoDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving MongoDB requires evidence-backed assessment.
...
- First-party source: https://github.com/mongodb/mongo
...
- [ ] Trace MongoDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check MongoDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check MongoDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check MongoDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check MongoDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check MongoDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If MongoDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a MongoDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- MongoDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a MongoDB architecture and dependency summary with trust boundaries and persistent state.
- Include MongoDB validation commands or tests that are safe for the stated environment.
...
- MongoDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- MongoDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that MongoDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of MongoDB as the basis for migration or production guidance.
