---
name: "cncf-snowflake-reviewer"
description: "Review Snowflake using its official documentation and repository in the App Definition and Development / Database category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Snowflake Reviewer

## Purpose

Review and operate Snowflake using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Snowflake code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Snowflake requires evidence-backed assessment.
...
- First-party source: https://www.snowflake.com/
...
- [ ] Trace Snowflake inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Snowflake defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Snowflake least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Snowflake resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Snowflake release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Snowflake dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Snowflake identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Snowflake change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Snowflake documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Snowflake architecture and dependency summary with trust boundaries and persistent state.
- Include Snowflake validation commands or tests that are safe for the stated environment.
...
- Snowflake identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Snowflake failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Snowflake is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Snowflake as the basis for migration or production guidance.
