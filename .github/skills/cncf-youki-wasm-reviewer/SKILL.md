---
name: "cncf-youki-wasm-reviewer"
description: "Review youki (wasm) using its official documentation and repository in the Wasm / Orchestration & Management category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Youki Wasm Reviewer

## Purpose

Review and operate youki (wasm) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- youki (wasm) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving youki (wasm) requires evidence-backed assessment.
...
- First-party source: https://github.com/youki-dev/youki
...
- [ ] Trace youki (wasm) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check youki (wasm) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check youki (wasm) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check youki (wasm) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check youki (wasm) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check youki (wasm) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If youki (wasm) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a youki (wasm) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- youki (wasm) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a youki (wasm) architecture and dependency summary with trust boundaries and persistent state.
- Include youki (wasm) validation commands or tests that are safe for the stated environment.
...
- youki (wasm) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- youki (wasm) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that youki (wasm) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of youki (wasm) as the basis for migration or production guidance.
