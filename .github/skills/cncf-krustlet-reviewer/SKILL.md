---
name: "cncf-krustlet-reviewer"
description: "Review Krustlet using its official documentation and repository in the Runtime / Container Runtime category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Krustlet Reviewer

## Purpose

Review and operate Krustlet using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Krustlet code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Krustlet requires evidence-backed assessment.
...
- First-party source: https://github.com/krustlet/krustlet
...
- [ ] Trace Krustlet inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Krustlet defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Krustlet least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Krustlet resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Krustlet release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Krustlet dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Krustlet identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Krustlet change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Krustlet documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Krustlet architecture and dependency summary with trust boundaries and persistent state.
- Include Krustlet validation commands or tests that are safe for the stated environment.
...
- Krustlet identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Krustlet failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Krustlet is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Krustlet as the basis for migration or production guidance.
