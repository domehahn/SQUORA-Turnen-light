---
name: "cncf-hyperlight-reviewer"
description: "Review Hyperlight using its official documentation and repository in the Runtime / Container Runtime category. A lightweight, secure container runtime solution designed for modern cloud-native workloads"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Hyperlight Reviewer

## Purpose

Review and operate Hyperlight using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Hyperlight code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Hyperlight requires evidence-backed assessment.
...
- First-party source: https://github.com/hyperlight-dev/hyperlight
...
- [ ] Trace Hyperlight inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Hyperlight defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Hyperlight least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Hyperlight resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Hyperlight release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Hyperlight dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Hyperlight identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Hyperlight change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Hyperlight documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Hyperlight architecture and dependency summary with trust boundaries and persistent state.
- Include Hyperlight validation commands or tests that are safe for the stated environment.
...
- Hyperlight identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Hyperlight failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Hyperlight is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Hyperlight as the basis for migration or production guidance.
