---
name: "cncf-ligato-reviewer"
description: "Review Ligato using its official documentation and repository in the Runtime / Cloud Native Network category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Ligato Reviewer

## Purpose

Review and operate Ligato using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Ligato code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Ligato requires evidence-backed assessment.
...
- First-party source: https://github.com/ligato/vpp-agent
...
- [ ] Trace Ligato inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Ligato defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Ligato least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Ligato resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Ligato release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Ligato dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Ligato identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Ligato change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Ligato documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Ligato architecture and dependency summary with trust boundaries and persistent state.
- Include Ligato validation commands or tests that are safe for the stated environment.
...
- Ligato identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Ligato failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Ligato is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Ligato as the basis for migration or production guidance.
