...
---
# Cncf Inspur Kcsp Reviewer

## Purpose

Review and operate Inspur (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Inspur (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Inspur (KCSP) requires evidence-backed assessment.
...
- First-party source: https://cloud.inspur.com/support/training-certification/index.html
...
- [ ] Trace Inspur (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Inspur (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Inspur (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Inspur (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Inspur (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Inspur (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Inspur (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Inspur (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Inspur (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Inspur (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Inspur (KCSP) validation commands or tests that are safe for the stated environment.
...
- Inspur (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Inspur (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Inspur (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Inspur (KCSP) as the basis for migration or production guidance.
