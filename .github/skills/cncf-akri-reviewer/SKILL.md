---
name: "cncf-akri-reviewer"
description: "Review Akri using its official documentation and repository in the Provisioning / Automation & Configuration category. A Kubernetes Resource Interface for the Edge"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Akri using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Akri code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Akri requires evidence-backed assessment.
...
- First-party source: https://github.com/project-akri/akri
...
- [ ] Trace Akri inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Akri defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Akri least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Akri resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Akri release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Akri dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Akri identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Akri change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Akri documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Akri architecture and dependency summary with trust boundaries and persistent state.
- Include Akri validation commands or tests that are safe for the stated environment.
...
- Akri identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Akri failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Akri is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Akri as the basis for migration or production guidance.
