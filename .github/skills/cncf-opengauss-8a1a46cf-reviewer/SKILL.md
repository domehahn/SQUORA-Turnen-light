---
name: "cncf-opengauss-8a1a46cf-reviewer"
description: "Review openGauss using its official documentation and repository in the App Definition and Development / Database category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Opengauss 8a1a46cf Reviewer

## Purpose

Review and operate openGauss using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- openGauss code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving openGauss requires evidence-backed assessment.
...
- First-party source: https://github.com/opengauss-mirror/openGauss-server
...
- [ ] Trace openGauss inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check openGauss defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check openGauss least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check openGauss resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check openGauss release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check openGauss dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If openGauss identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a openGauss change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- openGauss documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a openGauss architecture and dependency summary with trust boundaries and persistent state.
- Include openGauss validation commands or tests that are safe for the stated environment.
...
- openGauss identity, version, configuration, dependencies, and deployment assumptions are explicit.
- openGauss failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that openGauss is secure, supported, compatible, or suitable.
- Using an unpinned latest version of openGauss as the basis for migration or production guidance.
