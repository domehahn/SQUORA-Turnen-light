---
name: "cncf-cann-reviewer"
description: "Review CANN using its official documentation and repository in the AI Native Infra / Accelerator and SuperPod category. Compute Architecture for Neural Networks"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate CANN using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CANN code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CANN requires evidence-backed assessment.
...
- First-party source: https://gitcode.com/cann
...
- [ ] Trace CANN inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CANN defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CANN least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CANN resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CANN release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CANN dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CANN identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CANN change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CANN documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CANN architecture and dependency summary with trust boundaries and persistent state.
- Include CANN validation commands or tests that are safe for the stated environment.
...
- CANN identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CANN failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CANN is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CANN as the basis for migration or production guidance.
