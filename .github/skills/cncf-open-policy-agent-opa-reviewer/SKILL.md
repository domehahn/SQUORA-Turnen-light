---
name: "cncf-open-policy-agent-opa-reviewer"
description: "Review Open Policy Agent (OPA) using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Open Policy Agent Opa Reviewer

## Purpose

Review and operate Open Policy Agent (OPA) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Open Policy Agent (OPA) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Open Policy Agent (OPA) requires evidence-backed assessment.
...
- First-party source: https://github.com/open-policy-agent/opa
...
- [ ] Trace Open Policy Agent (OPA) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Open Policy Agent (OPA) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Open Policy Agent (OPA) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Open Policy Agent (OPA) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Open Policy Agent (OPA) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Open Policy Agent (OPA) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Open Policy Agent (OPA) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Open Policy Agent (OPA) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Open Policy Agent (OPA) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Open Policy Agent (OPA) architecture and dependency summary with trust boundaries and persistent state.
- Include Open Policy Agent (OPA) validation commands or tests that are safe for the stated environment.
...
- Open Policy Agent (OPA) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Open Policy Agent (OPA) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Open Policy Agent (OPA) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Open Policy Agent (OPA) as the basis for migration or production guidance.
