---
name: "cncf-rookout-reviewer"
description: "Review Rookout using its official documentation and repository in the Observability and Analysis / Observability category. Collect data directly from your live code using non-breaking breakpoints. Debug, observe and understand anything, anytime, anywhere."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Rookout Reviewer

## Purpose

Review and operate Rookout using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Rookout code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Rookout requires evidence-backed assessment.
...
- First-party source: https://www.rookout.com/
...
- [ ] Trace Rookout inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Rookout defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Rookout least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Rookout resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Rookout release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Rookout dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Rookout identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Rookout change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Rookout documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Rookout architecture and dependency summary with trust boundaries and persistent state.
- Include Rookout validation commands or tests that are safe for the stated environment.
...
- Rookout identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Rookout failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Rookout is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Rookout as the basis for migration or production guidance.
