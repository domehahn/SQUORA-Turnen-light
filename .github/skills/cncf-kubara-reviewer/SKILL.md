---
name: "cncf-kubara-reviewer"
description: "Review kubara using its official documentation and repository in the Provisioning / Automation & Configuration category. A single binary CLI tool written in Go to bootstrap, build, and package Kubernetes platforms."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kubara Reviewer

## Purpose

Review and operate kubara using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- kubara code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving kubara requires evidence-backed assessment.
...
- First-party source: https://github.com/kubara-io/kubara
...
- [ ] Trace kubara inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check kubara defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check kubara least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check kubara resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check kubara release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check kubara dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If kubara identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a kubara change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- kubara documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a kubara architecture and dependency summary with trust boundaries and persistent state.
- Include kubara validation commands or tests that are safe for the stated environment.
...
- kubara identity, version, configuration, dependencies, and deployment assumptions are explicit.
- kubara failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that kubara is secure, supported, compatible, or suitable.
- Using an unpinned latest version of kubara as the basis for migration or production guidance.
