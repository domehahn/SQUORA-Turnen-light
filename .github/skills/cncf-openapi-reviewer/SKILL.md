---
name: "cncf-openapi-reviewer"
description: "Review OpenAPI using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Openapi Reviewer

## Purpose

Review and operate OpenAPI using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenAPI code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenAPI requires evidence-backed assessment.
...
- First-party source: https://github.com/OAI/OpenAPI-Specification
...
- [ ] Trace OpenAPI inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenAPI defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenAPI least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenAPI resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenAPI release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenAPI dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenAPI identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenAPI change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenAPI documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenAPI architecture and dependency summary with trust boundaries and persistent state.
- Include OpenAPI validation commands or tests that are safe for the stated environment.
...
- OpenAPI identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenAPI failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenAPI is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenAPI as the basis for migration or production guidance.
