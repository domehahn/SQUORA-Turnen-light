---
name: "cncf-grpc-reviewer"
description: "Review gRPC using its official documentation and repository in the Orchestration & Management / Remote Procedure Call category. A high performance, open source universal RPC framework."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate gRPC using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- gRPC code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving gRPC requires evidence-backed assessment.
...
- First-party source: https://github.com/grpc/grpc
...
- [ ] Trace gRPC inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check gRPC defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check gRPC least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check gRPC resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check gRPC release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check gRPC dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If gRPC identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a gRPC change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- gRPC documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a gRPC architecture and dependency summary with trust boundaries and persistent state.
- Include gRPC validation commands or tests that are safe for the stated environment.
...
- gRPC identity, version, configuration, dependencies, and deployment assumptions are explicit.
- gRPC failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that gRPC is secure, supported, compatible, or suitable.
- Using an unpinned latest version of gRPC as the basis for migration or production guidance.
