---
name: "cncf-brigade-reviewer"
description: "Review Brigade using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Brigade Reviewer

## Purpose

Review and operate Brigade using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Brigade code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Brigade requires evidence-backed assessment.
...
- First-party source: https://github.com/brigadecore/brigade
...
- [ ] Trace Brigade inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Brigade defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Brigade least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Brigade resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Brigade release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Brigade dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Brigade identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Brigade change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Brigade documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Brigade architecture and dependency summary with trust boundaries and persistent state.
- Include Brigade validation commands or tests that are safe for the stated environment.
...
- Brigade identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Brigade failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Brigade is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Brigade as the basis for migration or production guidance.
