...
---
# Cncf Google Cloud Dataflow Reviewer

## Purpose

Review and operate Google Cloud Dataflow using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Google Cloud Dataflow code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Google Cloud Dataflow requires evidence-backed assessment.
...
- First-party source: https://cloud.google.com/dataflow/
...
- [ ] Trace Google Cloud Dataflow inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Google Cloud Dataflow defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Google Cloud Dataflow least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Google Cloud Dataflow resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Google Cloud Dataflow release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Google Cloud Dataflow dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Google Cloud Dataflow identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Google Cloud Dataflow change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Google Cloud Dataflow documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Google Cloud Dataflow architecture and dependency summary with trust boundaries and persistent state.
- Include Google Cloud Dataflow validation commands or tests that are safe for the stated environment.
...
- Google Cloud Dataflow identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Google Cloud Dataflow failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Google Cloud Dataflow is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Google Cloud Dataflow as the basis for migration or production guidance.
