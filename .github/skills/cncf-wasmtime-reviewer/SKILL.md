---
name: "cncf-wasmtime-reviewer"
description: "Review Wasmtime using its official documentation and repository in the Wasm / Runtimes category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Wasmtime Reviewer

## Purpose

Review and operate Wasmtime using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Wasmtime code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Wasmtime requires evidence-backed assessment.
...
- CNCF classification: Wasm / Runtimes
- CNCF project maturity field: not specified
- First-party source: https://github.com/bytecodealliance/wasmtime
...
- [ ] Trace Wasmtime inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Wasmtime defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Wasmtime least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Wasmtime resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Wasmtime release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Wasmtime dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Wasmtime identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Wasmtime change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Wasmtime documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Wasmtime architecture and dependency summary with trust boundaries and persistent state.
- Include Wasmtime validation commands or tests that are safe for the stated environment.
...
- Wasmtime identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Wasmtime failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Wasmtime is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Wasmtime as the basis for migration or production guidance.
