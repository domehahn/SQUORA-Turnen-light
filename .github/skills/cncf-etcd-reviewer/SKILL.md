---
name: "cncf-etcd-reviewer"
description: "Review etcd using its official documentation and repository in the Orchestration & Management / Coordination & Service Discovery category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate etcd using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- etcd code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving etcd requires evidence-backed assessment.
...
- First-party source: https://github.com/etcd-io/etcd
...
- [ ] Trace etcd inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check etcd defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check etcd least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check etcd resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check etcd release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check etcd dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If etcd identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a etcd change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- etcd documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a etcd architecture and dependency summary with trust boundaries and persistent state.
- Include etcd validation commands or tests that are safe for the stated environment.
...
- etcd identity, version, configuration, dependencies, and deployment assumptions are explicit.
- etcd failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that etcd is secure, supported, compatible, or suitable.
- Using an unpinned latest version of etcd as the basis for migration or production guidance.
