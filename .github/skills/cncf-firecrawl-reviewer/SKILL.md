---
name: "cncf-firecrawl-reviewer"
description: "Review Firecrawl using its official documentation and repository in the AI Agent / Agent Tool category. The API to search, scrape, and interact with the web for AI"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Firecrawl Reviewer

## Purpose

Review and operate Firecrawl using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Firecrawl code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Firecrawl requires evidence-backed assessment.
...
- First-party source: https://github.com/firecrawl/firecrawl
...
- [ ] Trace Firecrawl inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Firecrawl defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Firecrawl least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Firecrawl resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Firecrawl release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Firecrawl dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Firecrawl identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Firecrawl change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Firecrawl documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Firecrawl architecture and dependency summary with trust boundaries and persistent state.
- Include Firecrawl validation commands or tests that are safe for the stated environment.
...
- Firecrawl identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Firecrawl failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Firecrawl is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Firecrawl as the basis for migration or production guidance.
