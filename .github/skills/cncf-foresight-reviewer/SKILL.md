---
name: "cncf-foresight-reviewer"
description: "Review Foresight using its official documentation and repository in the Observability and Analysis / Observability category. Monitor workflow and test trends over time, assess the risk of code changes, deal with flaky tests."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Foresight Reviewer

## Purpose

Review and operate Foresight using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Foresight code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Foresight requires evidence-backed assessment.
...
- First-party source: https://www.runforesight.com/
...
- [ ] Trace Foresight inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Foresight defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Foresight least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Foresight resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Foresight release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Foresight dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Foresight identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Foresight change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Foresight documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Foresight architecture and dependency summary with trust boundaries and persistent state.
- Include Foresight validation commands or tests that are safe for the stated environment.
...
- Foresight identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Foresight failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Foresight is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Foresight as the basis for migration or production guidance.
