---
name: "cncf-bpfman-reviewer"
description: "Review bpfman using its official documentation and repository in the Provisioning / Security & Compliance category. An eBPF Manager for Linux and Kubernetes"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Bpfman Reviewer

## Purpose

Review and operate bpfman using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- bpfman code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving bpfman requires evidence-backed assessment.
...
- First-party source: https://github.com/bpfman/bpfman
...
- [ ] Trace bpfman inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check bpfman defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check bpfman least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check bpfman resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check bpfman release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check bpfman dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If bpfman identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a bpfman change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- bpfman documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a bpfman architecture and dependency summary with trust boundaries and persistent state.
- Include bpfman validation commands or tests that are safe for the stated environment.
...
- bpfman identity, version, configuration, dependencies, and deployment assumptions are explicit.
- bpfman failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that bpfman is secure, supported, compatible, or suitable.
- Using an unpinned latest version of bpfman as the basis for migration or production guidance.
