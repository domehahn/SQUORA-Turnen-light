---
name: "cncf-cloudtty-reviewer"
description: "Review CloudTTY using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category. A Friendly Kubernetes CloudShell (Web Terminal)"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Cloudtty Reviewer

## Purpose

Review and operate CloudTTY using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CloudTTY code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CloudTTY requires evidence-backed assessment.
...
- First-party source: https://github.com/cloudtty/cloudtty
...
- [ ] Trace CloudTTY inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CloudTTY defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CloudTTY least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CloudTTY resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CloudTTY release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CloudTTY dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CloudTTY identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CloudTTY change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CloudTTY documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CloudTTY architecture and dependency summary with trust boundaries and persistent state.
- Include CloudTTY validation commands or tests that are safe for the stated environment.
...
- CloudTTY identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CloudTTY failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CloudTTY is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CloudTTY as the basis for migration or production guidance.
