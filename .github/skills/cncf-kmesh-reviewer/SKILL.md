...
Review and operate Kmesh using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kmesh code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kmesh requires evidence-backed assessment.
...
- First-party source: https://github.com/kmesh-net/kmesh
...
- [ ] Trace Kmesh inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kmesh defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kmesh least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kmesh resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kmesh release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kmesh dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kmesh identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kmesh change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kmesh documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kmesh architecture and dependency summary with trust boundaries and persistent state.
- Include Kmesh validation commands or tests that are safe for the stated environment.
...
- Kmesh identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kmesh failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kmesh is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kmesh as the basis for migration or production guidance.
