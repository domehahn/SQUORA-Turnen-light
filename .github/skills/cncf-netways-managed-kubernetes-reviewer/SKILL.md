---
name: "cncf-netways-managed-kubernetes-reviewer"
description: "Review NETWAYS Managed Kubernetes using its official documentation and repository in the Platform / Certified Kubernetes - Hosted category. Build, deploy, and scale flexibly with NETWAYS Managed Kubernetes powered by OpenStack."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Netways Managed Kubernetes Reviewer

## Purpose

Review and operate NETWAYS Managed Kubernetes using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- NETWAYS Managed Kubernetes code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving NETWAYS Managed Kubernetes requires evidence-backed assessment.
...
- First-party source: https://nws.netways.de/en/managed-kubernetes
...
- [ ] Trace NETWAYS Managed Kubernetes inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check NETWAYS Managed Kubernetes defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check NETWAYS Managed Kubernetes least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check NETWAYS Managed Kubernetes resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check NETWAYS Managed Kubernetes release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check NETWAYS Managed Kubernetes dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If NETWAYS Managed Kubernetes identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a NETWAYS Managed Kubernetes change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- NETWAYS Managed Kubernetes documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a NETWAYS Managed Kubernetes architecture and dependency summary with trust boundaries and persistent state.
- Include NETWAYS Managed Kubernetes validation commands or tests that are safe for the stated environment.
...
- NETWAYS Managed Kubernetes identity, version, configuration, dependencies, and deployment assumptions are explicit.
- NETWAYS Managed Kubernetes failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that NETWAYS Managed Kubernetes is secure, supported, compatible, or suitable.
- Using an unpinned latest version of NETWAYS Managed Kubernetes as the basis for migration or production guidance.
