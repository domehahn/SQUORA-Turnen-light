---
name: "cncf-clusternet-reviewer"
description: "Review Clusternet using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. [CNCF Sandbox Project] Managing your Kubernetes clusters (including public, private, edge, etc.) as easily as visiting the Internet"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Clusternet Reviewer

## Purpose

Review and operate Clusternet using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Clusternet code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Clusternet requires evidence-backed assessment.
...
- First-party source: https://github.com/clusternet/clusternet
...
- [ ] Trace Clusternet inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Clusternet defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Clusternet least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Clusternet resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Clusternet release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Clusternet dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Clusternet identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Clusternet change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Clusternet documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Clusternet architecture and dependency summary with trust boundaries and persistent state.
- Include Clusternet validation commands or tests that are safe for the stated environment.
...
- Clusternet identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Clusternet failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Clusternet is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Clusternet as the basis for migration or production guidance.
