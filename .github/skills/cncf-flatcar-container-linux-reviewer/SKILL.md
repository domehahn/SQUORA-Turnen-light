---
name: "cncf-flatcar-container-linux-reviewer"
description: "Review Flatcar Container Linux using its official documentation and repository in the Platform / Certified Kubernetes - Distribution category. A community Linux distribution designed for container workloads, with high security and low maintenance"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Flatcar Container Linux Reviewer

## Purpose

Review and operate Flatcar Container Linux using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Flatcar Container Linux code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Flatcar Container Linux requires evidence-backed assessment.
...
- First-party source: https://github.com/flatcar/Flatcar
...
- [ ] Trace Flatcar Container Linux inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Flatcar Container Linux defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Flatcar Container Linux least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Flatcar Container Linux resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Flatcar Container Linux release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Flatcar Container Linux dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Flatcar Container Linux identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Flatcar Container Linux change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Flatcar Container Linux documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Flatcar Container Linux architecture and dependency summary with trust boundaries and persistent state.
- Include Flatcar Container Linux validation commands or tests that are safe for the stated environment.
...
- Flatcar Container Linux identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Flatcar Container Linux failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Flatcar Container Linux is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Flatcar Container Linux as the basis for migration or production guidance.
