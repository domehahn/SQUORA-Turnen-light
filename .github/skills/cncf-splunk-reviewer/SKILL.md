---
name: "cncf-splunk-reviewer"
description: "Review Splunk using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Splunk Reviewer

## Purpose

Review and operate Splunk using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Splunk code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Splunk requires evidence-backed assessment.
...
- First-party source: https://www.splunk.com/
...
- [ ] Trace Splunk inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Splunk defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Splunk least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Splunk resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Splunk release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Splunk dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Splunk identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Splunk change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Splunk documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Splunk architecture and dependency summary with trust boundaries and persistent state.
- Include Splunk validation commands or tests that are safe for the stated environment.
...
- Splunk identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Splunk failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Splunk is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Splunk as the basis for migration or production guidance.
