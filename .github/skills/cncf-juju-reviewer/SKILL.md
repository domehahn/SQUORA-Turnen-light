---
name: "cncf-juju-reviewer"
description: "Review Juju using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Juju using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Juju code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Juju requires evidence-backed assessment.
...
- First-party source: https://github.com/juju/juju
...
- [ ] Trace Juju inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Juju defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Juju least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Juju resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Juju release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Juju dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Juju identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Juju change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Juju documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Juju architecture and dependency summary with trust boundaries and persistent state.
- Include Juju validation commands or tests that are safe for the stated environment.
...
- Juju identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Juju failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Juju is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Juju as the basis for migration or production guidance.
