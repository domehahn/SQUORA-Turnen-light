---
name: "cncf-quay-reviewer"
description: "Review Quay using its official documentation and repository in the Provisioning / Container Registry category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Quay using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Quay code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Quay requires evidence-backed assessment.
...
- First-party source: https://github.com/quay/quay
...
- [ ] Trace Quay inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Quay defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Quay least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Quay resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Quay release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Quay dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Quay identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Quay change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Quay documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Quay architecture and dependency summary with trust boundaries and persistent state.
- Include Quay validation commands or tests that are safe for the stated environment.
...
- Quay identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Quay failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Quay is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Quay as the basis for migration or production guidance.
