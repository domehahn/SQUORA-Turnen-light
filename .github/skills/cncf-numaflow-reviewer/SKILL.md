---
name: "cncf-numaflow-reviewer"
description: "Review Numaflow using its official documentation and repository in the App Definition and Development / Streaming & Messaging category. Language agnostic K8s native real-time data and stream processing engine"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Numaflow Reviewer

## Purpose

Review and operate Numaflow using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Numaflow code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Numaflow requires evidence-backed assessment.
...
- First-party source: https://github.com/numaproj/numaflow
...
- [ ] Trace Numaflow inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Numaflow defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Numaflow least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Numaflow resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Numaflow release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Numaflow dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Numaflow identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Numaflow change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Numaflow documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Numaflow architecture and dependency summary with trust boundaries and persistent state.
- Include Numaflow validation commands or tests that are safe for the stated environment.
...
- Numaflow identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Numaflow failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Numaflow is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Numaflow as the basis for migration or production guidance.
