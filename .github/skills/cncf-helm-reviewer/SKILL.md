---
name: "cncf-helm-reviewer"
description: "Review Helm using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Helm using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Helm code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Helm requires evidence-backed assessment.
...
- First-party source: https://github.com/helm/helm
...
- [ ] Trace Helm inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Helm defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Helm least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Helm resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Helm release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Helm dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Helm identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Helm change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Helm documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Helm architecture and dependency summary with trust boundaries and persistent state.
- Include Helm validation commands or tests that are safe for the stated environment.
...
- Helm identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Helm failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Helm is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Helm as the basis for migration or production guidance.
