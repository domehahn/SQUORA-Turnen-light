...
---
# Cncf Volcano Engine Kcntp Reviewer

## Purpose

Review and operate Volcano Engine (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Volcano Engine (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Volcano Engine (KCNTP) requires evidence-backed assessment.
...
- [ ] Trace Volcano Engine (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Volcano Engine (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Volcano Engine (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Volcano Engine (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Volcano Engine (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Volcano Engine (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Volcano Engine (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Volcano Engine (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Volcano Engine (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Volcano Engine (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include Volcano Engine (KCNTP) validation commands or tests that are safe for the stated environment.
...
- Volcano Engine (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Volcano Engine (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Volcano Engine (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Volcano Engine (KCNTP) as the basis for migration or production guidance.
