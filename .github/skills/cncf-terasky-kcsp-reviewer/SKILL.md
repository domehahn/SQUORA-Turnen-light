---
name: "cncf-terasky-kcsp-reviewer"
...
---
# Cncf Terasky Kcsp Reviewer

## Purpose

Review and operate TeraSky (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- TeraSky (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving TeraSky (KCSP) requires evidence-backed assessment.
...
- [ ] Trace TeraSky (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check TeraSky (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check TeraSky (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check TeraSky (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check TeraSky (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check TeraSky (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If TeraSky (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a TeraSky (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- TeraSky (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a TeraSky (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include TeraSky (KCSP) validation commands or tests that are safe for the stated environment.
...
- TeraSky (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- TeraSky (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that TeraSky (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of TeraSky (KCSP) as the basis for migration or production guidance.
