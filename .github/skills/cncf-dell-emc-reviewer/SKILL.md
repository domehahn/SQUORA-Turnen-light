---
name: "cncf-dell-emc-reviewer"
description: "Review Dell EMC using its official documentation and repository in the Runtime / Cloud Native Storage category. Data storage that takes you from insights to innovation."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Dell Emc Reviewer

## Purpose

Review and operate Dell EMC using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Dell EMC code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Dell EMC requires evidence-backed assessment.
...
- First-party source: https://www.dell.com/en-us/dt/storage/data-storage.htm
...
- [ ] Trace Dell EMC inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Dell EMC defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Dell EMC least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Dell EMC resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Dell EMC release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Dell EMC dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Dell EMC identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Dell EMC change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Dell EMC documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Dell EMC architecture and dependency summary with trust boundaries and persistent state.
- Include Dell EMC validation commands or tests that are safe for the stated environment.
...
- Dell EMC identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Dell EMC failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Dell EMC is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Dell EMC as the basis for migration or production guidance.
