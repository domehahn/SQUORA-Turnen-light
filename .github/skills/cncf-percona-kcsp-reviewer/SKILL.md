...
---
# Cncf Percona Kcsp Reviewer

## Purpose

Review and operate Percona (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Percona (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Percona (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.percona.com/cloud-native/support-and-services
...
- [ ] Trace Percona (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Percona (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Percona (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Percona (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Percona (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Percona (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Percona (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Percona (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Percona (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Percona (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Percona (KCSP) validation commands or tests that are safe for the stated environment.
...
- Percona (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Percona (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Percona (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Percona (KCSP) as the basis for migration or production guidance.
