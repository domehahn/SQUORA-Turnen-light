...
---
# Cncf Alibaba Cloud Container Service For Kubernetes Reviewer

## Purpose

Review and operate Alibaba Cloud Container Service for Kubernetes using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Alibaba Cloud Container Service for Kubernetes code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Alibaba Cloud Container Service for Kubernetes requires evidence-backed assessment.
...
- First-party source: https://www.alibabacloud.com/product/kubernetes
...
- [ ] Trace Alibaba Cloud Container Service for Kubernetes inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Alibaba Cloud Container Service for Kubernetes defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Alibaba Cloud Container Service for Kubernetes least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Alibaba Cloud Container Service for Kubernetes resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Alibaba Cloud Container Service for Kubernetes release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Alibaba Cloud Container Service for Kubernetes dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Alibaba Cloud Container Service for Kubernetes identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Alibaba Cloud Container Service for Kubernetes change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Alibaba Cloud Container Service for Kubernetes documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Alibaba Cloud Container Service for Kubernetes architecture and dependency summary with trust boundaries and persistent state.
- Include Alibaba Cloud Container Service for Kubernetes validation commands or tests that are safe for the stated environment.
...
- Alibaba Cloud Container Service for Kubernetes identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Alibaba Cloud Container Service for Kubernetes failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Alibaba Cloud Container Service for Kubernetes is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Alibaba Cloud Container Service for Kubernetes as the basis for migration or production guidance.
