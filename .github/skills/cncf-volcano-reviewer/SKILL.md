---
name: "cncf-volcano-reviewer"
description: "Review Volcano using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Volcano using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Volcano code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Volcano requires evidence-backed assessment.
...
- First-party source: https://github.com/volcano-sh/volcano
...
- [ ] Trace Volcano inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Volcano defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Volcano least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Volcano resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Volcano release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Volcano dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Volcano identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Volcano change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Volcano documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Volcano architecture and dependency summary with trust boundaries and persistent state.
- Include Volcano validation commands or tests that are safe for the stated environment.
...
- Volcano identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Volcano failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Volcano is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Volcano as the basis for migration or production guidance.
