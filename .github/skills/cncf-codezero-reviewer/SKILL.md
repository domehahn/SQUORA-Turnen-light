---
name: "cncf-codezero-reviewer"
description: "Review CodeZero using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category. Tools for local collaborative development in pre-production Kubernetes clusters"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Codezero Reviewer

## Purpose

Review and operate CodeZero using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CodeZero code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CodeZero requires evidence-backed assessment.
...
- First-party source: https://codezero.io/
...
- [ ] Trace CodeZero inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CodeZero defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CodeZero least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CodeZero resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CodeZero release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CodeZero dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CodeZero identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CodeZero change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CodeZero documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CodeZero architecture and dependency summary with trust boundaries and persistent state.
- Include CodeZero validation commands or tests that are safe for the stated environment.
...
- CodeZero identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CodeZero failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CodeZero is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CodeZero as the basis for migration or production guidance.
