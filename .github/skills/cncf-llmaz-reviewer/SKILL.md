---
name: "cncf-llmaz-reviewer"
description: "Review llmaz using its official documentation and repository in the Inference / Framework category. Easy, advanced inference platform for large language models on Kubernetes."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate llmaz using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- llmaz code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving llmaz requires evidence-backed assessment.
...
- First-party source: https://github.com/InftyAI/llmaz
...
- [ ] Trace llmaz inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check llmaz defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check llmaz least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check llmaz resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check llmaz release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check llmaz dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If llmaz identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a llmaz change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- llmaz documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a llmaz architecture and dependency summary with trust boundaries and persistent state.
- Include llmaz validation commands or tests that are safe for the stated environment.
...
- llmaz identity, version, configuration, dependencies, and deployment assumptions are explicit.
- llmaz failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that llmaz is secure, supported, compatible, or suitable.
- Using an unpinned latest version of llmaz as the basis for migration or production guidance.
