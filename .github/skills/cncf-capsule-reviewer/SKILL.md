...
---
# Cncf Capsule Reviewer

## Purpose

Review and operate Capsule using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Capsule code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Capsule requires evidence-backed assessment.
...
- First-party source: https://github.com/projectcapsule/capsule
...
- [ ] Trace Capsule inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Capsule defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Capsule least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Capsule resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Capsule release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Capsule dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Capsule identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Capsule change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Capsule documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Capsule architecture and dependency summary with trust boundaries and persistent state.
- Include Capsule validation commands or tests that are safe for the stated environment.
...
- Capsule identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Capsule failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Capsule is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Capsule as the basis for migration or production guidance.
