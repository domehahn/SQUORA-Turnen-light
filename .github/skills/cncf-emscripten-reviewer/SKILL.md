---
name: "cncf-emscripten-reviewer"
description: "Review emscripten using its official documentation and repository in the Wasm / Tooling category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Emscripten Reviewer

## Purpose

Review and operate emscripten using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- emscripten code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving emscripten requires evidence-backed assessment.
...
- First-party source: https://github.com/emscripten-core/emscripten
...
- [ ] Trace emscripten inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check emscripten defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check emscripten least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check emscripten resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check emscripten release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check emscripten dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If emscripten identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a emscripten change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- emscripten documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a emscripten architecture and dependency summary with trust boundaries and persistent state.
- Include emscripten validation commands or tests that are safe for the stated environment.
...
- emscripten identity, version, configuration, dependencies, and deployment assumptions are explicit.
- emscripten failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that emscripten is secure, supported, compatible, or suitable.
- Using an unpinned latest version of emscripten as the basis for migration or production guidance.
