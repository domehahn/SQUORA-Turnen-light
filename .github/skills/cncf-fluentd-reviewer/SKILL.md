---
name: "cncf-fluentd-reviewer"
description: "Review Fluentd using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Fluentd Reviewer

## Purpose

Review and operate Fluentd using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Fluentd code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Fluentd requires evidence-backed assessment.
...
- First-party source: https://github.com/fluent/fluentd
...
- [ ] Trace Fluentd inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Fluentd defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Fluentd least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Fluentd resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Fluentd release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Fluentd dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Fluentd identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Fluentd change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Fluentd documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Fluentd architecture and dependency summary with trust boundaries and persistent state.
- Include Fluentd validation commands or tests that are safe for the stated environment.
...
- Fluentd identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Fluentd failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Fluentd is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Fluentd as the basis for migration or production guidance.
