---
name: "cncf-squash-reviewer"
description: "Review Squash using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Squash Reviewer

## Purpose

Review and operate Squash using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Squash code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Squash requires evidence-backed assessment.
...
- First-party source: https://github.com/solo-io/squash
...
- [ ] Trace Squash inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Squash defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Squash least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Squash resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Squash release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Squash dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Squash identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Squash change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Squash documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Squash architecture and dependency summary with trust boundaries and persistent state.
- Include Squash validation commands or tests that are safe for the stated environment.
...
- Squash identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Squash failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Squash is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Squash as the basis for migration or production guidance.
