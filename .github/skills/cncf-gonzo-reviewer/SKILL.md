...
Review and operate Gonzo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Gonzo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Gonzo requires evidence-backed assessment.
...
- First-party source: https://github.com/control-theory/gonzo
...
- [ ] Trace Gonzo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Gonzo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Gonzo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Gonzo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Gonzo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Gonzo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Gonzo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Gonzo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Gonzo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Gonzo architecture and dependency summary with trust boundaries and persistent state.
- Include Gonzo validation commands or tests that are safe for the stated environment.
...
- Gonzo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Gonzo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Gonzo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Gonzo as the basis for migration or production guidance.
