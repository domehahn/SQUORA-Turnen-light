---
name: "cncf-orientdb-reviewer"
description: "Review OrientDB using its official documentation and repository in the App Definition and Development / Database category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Orientdb Reviewer

## Purpose

Review and operate OrientDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OrientDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OrientDB requires evidence-backed assessment.
...
- First-party source: https://github.com/orientechnologies/orientdb
...
- [ ] Trace OrientDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OrientDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OrientDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OrientDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OrientDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OrientDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OrientDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OrientDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OrientDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OrientDB architecture and dependency summary with trust boundaries and persistent state.
- Include OrientDB validation commands or tests that are safe for the stated environment.
...
- OrientDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OrientDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OrientDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OrientDB as the basis for migration or production guidance.
