---
name: "cncf-hertzbeat-reviewer"
description: "Review HertzBeat using its official documentation and repository in the Observability and Analysis / Observability category. Apache HertzBeat (Incubating) is an open-source, real-time monitoring system with custom monitoring, high performance cluster and agentless capabilities."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Hertzbeat Reviewer

## Purpose

Review and operate HertzBeat using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- HertzBeat code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving HertzBeat requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/hertzbeat
...
- [ ] Trace HertzBeat inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check HertzBeat defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check HertzBeat least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check HertzBeat resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check HertzBeat release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check HertzBeat dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If HertzBeat identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a HertzBeat change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- HertzBeat documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a HertzBeat architecture and dependency summary with trust boundaries and persistent state.
- Include HertzBeat validation commands or tests that are safe for the stated environment.
...
- HertzBeat identity, version, configuration, dependencies, and deployment assumptions are explicit.
- HertzBeat failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that HertzBeat is secure, supported, compatible, or suitable.
- Using an unpinned latest version of HertzBeat as the basis for migration or production guidance.
