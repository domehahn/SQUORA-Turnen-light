---
name: "cncf-wasm2c-reviewer"
description: "Review wasm2c using its official documentation and repository in the Wasm / Tooling category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Wasm2c Reviewer

## Purpose

Review and operate wasm2c using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- wasm2c code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving wasm2c requires evidence-backed assessment.
...
- First-party source: https://github.com/WebAssembly/wabt
...
- [ ] Trace wasm2c inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check wasm2c defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check wasm2c least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check wasm2c resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check wasm2c release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check wasm2c dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If wasm2c identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a wasm2c change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- wasm2c documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a wasm2c architecture and dependency summary with trust boundaries and persistent state.
- Include wasm2c validation commands or tests that are safe for the stated environment.
...
- wasm2c identity, version, configuration, dependencies, and deployment assumptions are explicit.
- wasm2c failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that wasm2c is secure, supported, compatible, or suitable.
- Using an unpinned latest version of wasm2c as the basis for migration or production guidance.
