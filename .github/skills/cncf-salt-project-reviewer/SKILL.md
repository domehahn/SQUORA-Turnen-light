---
name: "cncf-salt-project-reviewer"
description: "Review Salt Project using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Salt Project Reviewer

## Purpose

Review and operate Salt Project using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Salt Project code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Salt Project requires evidence-backed assessment.
...
- First-party source: https://github.com/saltstack/salt
...
- [ ] Trace Salt Project inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Salt Project defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Salt Project least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Salt Project resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Salt Project release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Salt Project dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Salt Project identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Salt Project change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Salt Project documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Salt Project architecture and dependency summary with trust boundaries and persistent state.
- Include Salt Project validation commands or tests that are safe for the stated environment.
...
- Salt Project identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Salt Project failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Salt Project is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Salt Project as the basis for migration or production guidance.
