---
name: "cncf-spiderpool-reviewer"
description: "Review Spiderpool using its official documentation and repository in the Runtime / Cloud Native Network category. Spiderpool is the underlay and RDMA network solution of the Kubernetes, for bare metal, VM and public cloud"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Spiderpool Reviewer

## Purpose

Review and operate Spiderpool using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Spiderpool code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Spiderpool requires evidence-backed assessment.
...
- First-party source: https://github.com/spidernet-io/spiderpool
...
- [ ] Trace Spiderpool inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Spiderpool defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Spiderpool least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Spiderpool resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Spiderpool release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Spiderpool dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Spiderpool identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Spiderpool change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Spiderpool documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Spiderpool architecture and dependency summary with trust boundaries and persistent state.
- Include Spiderpool validation commands or tests that are safe for the stated environment.
...
- Spiderpool identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Spiderpool failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Spiderpool is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Spiderpool as the basis for migration or production guidance.
