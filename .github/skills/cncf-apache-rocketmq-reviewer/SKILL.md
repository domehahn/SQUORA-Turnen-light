---
name: "cncf-apache-rocketmq-reviewer"
description: "Review Apache RocketMQ using its official documentation and repository in the App Definition and Development / Streaming & Messaging category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Apache Rocketmq Reviewer

## Purpose

Review and operate Apache RocketMQ using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Apache RocketMQ code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Apache RocketMQ requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/rocketmq
...
- [ ] Trace Apache RocketMQ inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Apache RocketMQ defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Apache RocketMQ least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Apache RocketMQ resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Apache RocketMQ release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Apache RocketMQ dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Apache RocketMQ identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Apache RocketMQ change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Apache RocketMQ documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Apache RocketMQ architecture and dependency summary with trust boundaries and persistent state.
- Include Apache RocketMQ validation commands or tests that are safe for the stated environment.
...
- Apache RocketMQ identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Apache RocketMQ failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Apache RocketMQ is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Apache RocketMQ as the basis for migration or production guidance.
