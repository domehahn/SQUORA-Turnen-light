---
name: "cncf-maas-reviewer"
description: "Review MAAS using its official documentation and repository in the Provisioning / Automation & Configuration category. Self-service, remote installation of Windows, CentOS, ESXi and Ubuntu on real servers turns your data center into a bare-metal cloud."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate MAAS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- MAAS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving MAAS requires evidence-backed assessment.
...
- First-party source: https://github.com/canonical/maas
...
- [ ] Trace MAAS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check MAAS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check MAAS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check MAAS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check MAAS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check MAAS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If MAAS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a MAAS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- MAAS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a MAAS architecture and dependency summary with trust boundaries and persistent state.
- Include MAAS validation commands or tests that are safe for the stated environment.
...
- MAAS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- MAAS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that MAAS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of MAAS as the basis for migration or production guidance.
