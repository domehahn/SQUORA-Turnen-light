...
---
# Cncf Userver Reviewer

## Purpose

Review and operate userver using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- userver code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving userver requires evidence-backed assessment.
...
- First-party source: https://github.com/userver-framework/userver
...
- [ ] Trace userver inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check userver defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check userver least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check userver resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check userver release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check userver dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If userver identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a userver change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- userver documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a userver architecture and dependency summary with trust boundaries and persistent state.
- Include userver validation commands or tests that are safe for the stated environment.
...
- userver identity, version, configuration, dependencies, and deployment assumptions are explicit.
- userver failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that userver is secure, supported, compatible, or suitable.
- Using an unpinned latest version of userver as the basis for migration or production guidance.
