...
Review and operate Fugue using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Fugue code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Fugue requires evidence-backed assessment.
...
- First-party source: https://www.fugue.co
...
- [ ] Trace Fugue inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Fugue defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Fugue least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Fugue resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Fugue release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Fugue dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Fugue identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Fugue change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Fugue documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Fugue architecture and dependency summary with trust boundaries and persistent state.
- Include Fugue validation commands or tests that are safe for the stated environment.
...
- Fugue identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Fugue failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Fugue is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Fugue as the basis for migration or production guidance.
