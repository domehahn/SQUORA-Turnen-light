---
name: "cncf-rbac-lookup-reviewer"
description: "Review RBAC Lookup using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Rbac Lookup Reviewer

## Purpose

Review and operate RBAC Lookup using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- RBAC Lookup code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving RBAC Lookup requires evidence-backed assessment.
...
- First-party source: https://github.com/FairwindsOps/rbac-lookup
...
- [ ] Trace RBAC Lookup inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check RBAC Lookup defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check RBAC Lookup least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check RBAC Lookup resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check RBAC Lookup release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check RBAC Lookup dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If RBAC Lookup identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a RBAC Lookup change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- RBAC Lookup documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a RBAC Lookup architecture and dependency summary with trust boundaries and persistent state.
- Include RBAC Lookup validation commands or tests that are safe for the stated environment.
...
- RBAC Lookup identity, version, configuration, dependencies, and deployment assumptions are explicit.
- RBAC Lookup failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that RBAC Lookup is secure, supported, compatible, or suitable.
- Using an unpinned latest version of RBAC Lookup as the basis for migration or production guidance.
