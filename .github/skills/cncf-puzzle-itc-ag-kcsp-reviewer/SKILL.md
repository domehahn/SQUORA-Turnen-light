---
name: "cncf-puzzle-itc-ag-kcsp-reviewer"
description: "Review Puzzle ITC AG (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Whether on-premises, in the cloud, or as a managed service –\u00a0we help customers\u00a0create the ideal Kubernetes environment and leverage the best tools from the cloud-native landscape. This results in a standardized, flexible, and automated environment that efficiently bridges the gap between development and operations."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Puzzle Itc Ag Kcsp Reviewer

## Purpose

Review and operate Puzzle ITC AG (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Puzzle ITC AG (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Puzzle ITC AG (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.puzzle.ch/service-products/platformengineering/kubernetes
...
- [ ] Trace Puzzle ITC AG (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Puzzle ITC AG (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Puzzle ITC AG (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Puzzle ITC AG (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Puzzle ITC AG (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Puzzle ITC AG (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Puzzle ITC AG (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Puzzle ITC AG (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Puzzle ITC AG (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Puzzle ITC AG (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Puzzle ITC AG (KCSP) validation commands or tests that are safe for the stated environment.
...
- Puzzle ITC AG (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Puzzle ITC AG (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Puzzle ITC AG (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Puzzle ITC AG (KCSP) as the basis for migration or production guidance.
