---
name: "cncf-dalec-reviewer"
description: "Review Dalec using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category. Dalec provides a declarative format for building system packages and containers from those packages in a secure way for supply chain security."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Dalec using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Dalec code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Dalec requires evidence-backed assessment.
...
- First-party source: https://github.com/project-dalec/dalec
...
- [ ] Trace Dalec inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Dalec defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Dalec least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Dalec resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Dalec release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Dalec dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Dalec identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Dalec change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Dalec documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Dalec architecture and dependency summary with trust boundaries and persistent state.
- Include Dalec validation commands or tests that are safe for the stated environment.
...
- Dalec identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Dalec failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Dalec is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Dalec as the basis for migration or production guidance.
