---
name: "cncf-mantech-solution-kcsp-reviewer"
description: "Review Mantech Solution (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. With over 30 years of experience as an IT solutions provider, we provide Kubernetes technical support for an optimized and stable migration for each enterprise environment. Our engineers are Kubernetes experts and we provide end-to-end management of your Kubernetes operations."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Mantech Solution Kcsp Reviewer

## Purpose

Review and operate Mantech Solution (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Mantech Solution (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Mantech Solution (KCSP) requires evidence-backed assessment.
...
- First-party source: https://accordions.co.kr/kube_eng/
...
- [ ] Trace Mantech Solution (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Mantech Solution (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Mantech Solution (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Mantech Solution (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Mantech Solution (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Mantech Solution (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Mantech Solution (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Mantech Solution (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Mantech Solution (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Mantech Solution (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Mantech Solution (KCSP) validation commands or tests that are safe for the stated environment.
...
- Mantech Solution (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Mantech Solution (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Mantech Solution (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Mantech Solution (KCSP) as the basis for migration or production guidance.
