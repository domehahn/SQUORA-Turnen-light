---
name: "cncf-moonbit-reviewer"
description: "Review MoonBit using its official documentation and repository in the Wasm / Languages category. WASM focused languages"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Moonbit Reviewer

## Purpose

Review and operate MoonBit using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- MoonBit code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving MoonBit requires evidence-backed assessment.
...
- First-party source: https://www.moonbitlang.com/
...
- [ ] Trace MoonBit inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check MoonBit defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check MoonBit least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check MoonBit resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check MoonBit release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check MoonBit dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If MoonBit identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a MoonBit change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- MoonBit documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a MoonBit architecture and dependency summary with trust boundaries and persistent state.
- Include MoonBit validation commands or tests that are safe for the stated environment.
...
- MoonBit identity, version, configuration, dependencies, and deployment assumptions are explicit.
- MoonBit failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that MoonBit is secure, supported, compatible, or suitable.
- Using an unpinned latest version of MoonBit as the basis for migration or production guidance.
