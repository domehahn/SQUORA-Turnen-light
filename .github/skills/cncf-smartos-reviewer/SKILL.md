---
name: "cncf-smartos-reviewer"
description: "Review SmartOS using its official documentation and repository in the Runtime / Container Runtime category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Smartos Reviewer

## Purpose

Review and operate SmartOS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SmartOS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SmartOS requires evidence-backed assessment.
...
- First-party source: https://github.com/TritonDataCenter/smartos-live
...
- [ ] Trace SmartOS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SmartOS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SmartOS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SmartOS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SmartOS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SmartOS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SmartOS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SmartOS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SmartOS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SmartOS architecture and dependency summary with trust boundaries and persistent state.
- Include SmartOS validation commands or tests that are safe for the stated environment.
...
- SmartOS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SmartOS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SmartOS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SmartOS as the basis for migration or production guidance.
