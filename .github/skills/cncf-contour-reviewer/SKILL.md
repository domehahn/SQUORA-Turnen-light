---
name: "cncf-contour-reviewer"
description: "Review Contour using its official documentation and repository in the Orchestration & Management / Service Proxy category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Contour Reviewer

## Purpose

Review and operate Contour using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Contour code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Contour requires evidence-backed assessment.
...
- First-party source: https://github.com/projectcontour/contour
...
- [ ] Trace Contour inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Contour defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Contour least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Contour resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Contour release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Contour dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Contour identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Contour change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Contour documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Contour architecture and dependency summary with trust boundaries and persistent state.
- Include Contour validation commands or tests that are safe for the stated environment.
...
- Contour identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Contour failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Contour is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Contour as the basis for migration or production guidance.
