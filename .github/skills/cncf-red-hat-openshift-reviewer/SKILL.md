---
name: "cncf-red-hat-openshift-reviewer"
description: "Review Red Hat OpenShift using its official documentation and repository in the Platform / Certified Kubernetes - Distribution category. OpenShift® helps organizations focus on building and scaling their business with fully supported enterprise Kubernetes by Red Hat®."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Red Hat OpenShift using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Red Hat OpenShift code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Red Hat OpenShift requires evidence-backed assessment.
...
- First-party source: https://github.com/openshift/kubernetes
...
- [ ] Trace Red Hat OpenShift inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Red Hat OpenShift defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Red Hat OpenShift least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Red Hat OpenShift resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Red Hat OpenShift release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Red Hat OpenShift dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Red Hat OpenShift identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Red Hat OpenShift change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Red Hat OpenShift documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Red Hat OpenShift architecture and dependency summary with trust boundaries and persistent state.
- Include Red Hat OpenShift validation commands or tests that are safe for the stated environment.
...
- Red Hat OpenShift identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Red Hat OpenShift failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Red Hat OpenShift is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Red Hat OpenShift as the basis for migration or production guidance.
