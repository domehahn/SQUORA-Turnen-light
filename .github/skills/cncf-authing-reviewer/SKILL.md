---
name: "cncf-authing-reviewer"
description: "Review Authing using its official documentation and repository in the Provisioning / Key Management category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Authing Reviewer

## Purpose

Review and operate Authing using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Authing code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Authing requires evidence-backed assessment.
...
- First-party source: https://github.com/Authing/Authing
...
- [ ] Trace Authing inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Authing defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Authing least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Authing resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Authing release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Authing dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Authing identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Authing change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Authing documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Authing architecture and dependency summary with trust boundaries and persistent state.
- Include Authing validation commands or tests that are safe for the stated environment.
...
- Authing identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Authing failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Authing is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Authing as the basis for migration or production guidance.
