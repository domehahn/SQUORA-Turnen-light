---
name: "cncf-prometheus-reviewer"
description: "Review Prometheus using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Prometheus Reviewer

## Purpose

Review and operate Prometheus using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Prometheus code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Prometheus requires evidence-backed assessment.
...
- First-party source: https://github.com/prometheus/prometheus
...
- [ ] Trace Prometheus inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Prometheus defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Prometheus least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Prometheus resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Prometheus release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Prometheus dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Prometheus identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Prometheus change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Prometheus documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Prometheus architecture and dependency summary with trust boundaries and persistent state.
- Include Prometheus validation commands or tests that are safe for the stated environment.
...
- Prometheus identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Prometheus failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Prometheus is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Prometheus as the basis for migration or production guidance.
