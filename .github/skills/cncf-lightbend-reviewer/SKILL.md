---
name: "cncf-lightbend-reviewer"
description: "Review Lightbend using its official documentation and repository in the App Definition and Development / Streaming & Messaging category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Lightbend Reviewer

## Purpose

Review and operate Lightbend using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Lightbend code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Lightbend requires evidence-backed assessment.
...
- First-party source: https://www.lightbend.com/
...
- [ ] Trace Lightbend inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Lightbend defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Lightbend least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Lightbend resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Lightbend release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Lightbend dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Lightbend identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Lightbend change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Lightbend documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Lightbend architecture and dependency summary with trust boundaries and persistent state.
- Include Lightbend validation commands or tests that are safe for the stated environment.
...
- Lightbend identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Lightbend failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Lightbend is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Lightbend as the basis for migration or production guidance.
