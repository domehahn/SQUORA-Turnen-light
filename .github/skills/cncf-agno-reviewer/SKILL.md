---
name: "cncf-agno-reviewer"
description: "Review Agno using its official documentation and repository in the AI Agent / Agent Framework category. Build, run, manage agentic software at scale"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Agno using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Agno code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Agno requires evidence-backed assessment.
...
- First-party source: https://github.com/agno-agi/agno
...
- [ ] Trace Agno inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Agno defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Agno least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Agno resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Agno release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Agno dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Agno identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Agno change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Agno documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Agno architecture and dependency summary with trust boundaries and persistent state.
- Include Agno validation commands or tests that are safe for the stated environment.
...
- Agno identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Agno failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Agno is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Agno as the basis for migration or production guidance.
