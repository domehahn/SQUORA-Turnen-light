---
name: "cncf-blue-sentry-kcsp-reviewer"
description: "Review Blue Sentry (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Blue Sentry increases efficiency, security, ensures compliance and helps organizations deliver products faster with our managed DevOps services utilizing Kubernetes, Infrastructure as Code and CI/CD-enabled change management."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Blue Sentry Kcsp Reviewer

## Purpose

Review and operate Blue Sentry (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Blue Sentry (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Blue Sentry (KCSP) requires evidence-backed assessment.
...
- First-party source: https://bluesentry.cloud/solutions/managed-kubernetes-consulting-services
...
- [ ] Trace Blue Sentry (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Blue Sentry (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Blue Sentry (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Blue Sentry (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Blue Sentry (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Blue Sentry (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Blue Sentry (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Blue Sentry (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Blue Sentry (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Blue Sentry (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Blue Sentry (KCSP) validation commands or tests that are safe for the stated environment.
...
- Blue Sentry (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Blue Sentry (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Blue Sentry (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Blue Sentry (KCSP) as the basis for migration or production guidance.
