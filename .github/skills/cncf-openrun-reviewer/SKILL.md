---
name: "cncf-openrun-reviewer"
description: "Review OpenRun using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category. GitOps based deployment platform similar to Google Cloud Run and AWS App Runner. Easily deploy internal tools across a team."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Openrun Reviewer

## Purpose

Review and operate OpenRun using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenRun code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenRun requires evidence-backed assessment.
...
- First-party source: https://github.com/openrundev/openrun
...
- [ ] Trace OpenRun inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenRun defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenRun least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenRun resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenRun release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenRun dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenRun identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenRun change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenRun documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenRun architecture and dependency summary with trust boundaries and persistent state.
- Include OpenRun validation commands or tests that are safe for the stated environment.
...
- OpenRun identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenRun failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenRun is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenRun as the basis for migration or production guidance.
