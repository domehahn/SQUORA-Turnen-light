---
name: "cncf-no-code-reviewer"
description: "Review No Code using its official documentation and repository in the Platform / PaaS/Container Service category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf No Code Reviewer

## Purpose

Review and operate No Code using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- No Code code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving No Code requires evidence-backed assessment.
...
- First-party source: https://github.com/kelseyhightower/nocode
...
- [ ] Trace No Code inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check No Code defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check No Code least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check No Code resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check No Code release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check No Code dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If No Code identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a No Code change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- No Code documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a No Code architecture and dependency summary with trust boundaries and persistent state.
- Include No Code validation commands or tests that are safe for the stated environment.
...
- No Code identity, version, configuration, dependencies, and deployment assumptions are explicit.
- No Code failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that No Code is secure, supported, compatible, or suitable.
- Using an unpinned latest version of No Code as the basis for migration or production guidance.
