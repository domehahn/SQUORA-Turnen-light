---
name: "cncf-pluto-reviewer"
description: "Review Pluto using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Pluto using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Pluto code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Pluto requires evidence-backed assessment.
...
- First-party source: https://github.com/FairwindsOps/pluto
...
- [ ] Trace Pluto inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Pluto defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Pluto least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Pluto resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Pluto release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Pluto dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Pluto identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Pluto change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Pluto documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Pluto architecture and dependency summary with trust boundaries and persistent state.
- Include Pluto validation commands or tests that are safe for the stated environment.
...
- Pluto identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Pluto failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Pluto is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Pluto as the basis for migration or production guidance.
