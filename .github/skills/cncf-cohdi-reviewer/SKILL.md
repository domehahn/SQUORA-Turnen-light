...
Review and operate CoHDI using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CoHDI code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CoHDI requires evidence-backed assessment.
...
- First-party source: https://github.com/CoHDI
...
- [ ] Trace CoHDI inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CoHDI defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CoHDI least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CoHDI resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CoHDI release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CoHDI dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CoHDI identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CoHDI change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CoHDI documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CoHDI architecture and dependency summary with trust boundaries and persistent state.
- Include CoHDI validation commands or tests that are safe for the stated environment.
...
- CoHDI identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CoHDI failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CoHDI is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CoHDI as the basis for migration or production guidance.
