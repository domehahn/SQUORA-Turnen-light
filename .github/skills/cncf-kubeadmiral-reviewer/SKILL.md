---
name: "cncf-kubeadmiral-reviewer"
description: "Review KubeAdmiral using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. KubeAdmiral is a multi-cluster scheduling and orchestration system for Kubernetes."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kubeadmiral Reviewer

## Purpose

Review and operate KubeAdmiral using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeAdmiral code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeAdmiral requires evidence-backed assessment.
...
- First-party source: https://github.com/kubewharf/kubeadmiral
...
- [ ] Trace KubeAdmiral inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeAdmiral defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeAdmiral least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeAdmiral resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeAdmiral release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeAdmiral dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeAdmiral identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeAdmiral change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeAdmiral documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeAdmiral architecture and dependency summary with trust boundaries and persistent state.
- Include KubeAdmiral validation commands or tests that are safe for the stated environment.
...
- KubeAdmiral identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeAdmiral failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeAdmiral is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeAdmiral as the basis for migration or production guidance.
