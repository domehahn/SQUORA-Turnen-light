---
name: "cncf-k8sgpt-reviewer"
description: "Review K8sGPT using its official documentation and repository in the Observability and Analysis / Observability category. Giving Kubernetes Superpowers to everyone"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf K8sgpt Reviewer

## Purpose

Review and operate K8sGPT using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- K8sGPT code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving K8sGPT requires evidence-backed assessment.
...
- First-party source: https://github.com/k8sgpt-ai/k8sgpt
...
- [ ] Trace K8sGPT inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check K8sGPT defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check K8sGPT least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check K8sGPT resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check K8sGPT release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check K8sGPT dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If K8sGPT identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a K8sGPT change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- K8sGPT documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a K8sGPT architecture and dependency summary with trust boundaries and persistent state.
- Include K8sGPT validation commands or tests that are safe for the stated environment.
...
- K8sGPT identity, version, configuration, dependencies, and deployment assumptions are explicit.
- K8sGPT failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that K8sGPT is secure, supported, compatible, or suitable.
- Using an unpinned latest version of K8sGPT as the basis for migration or production guidance.
