---
name: "cncf-openmetrics-reviewer"
description: "Review OpenMetrics using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Openmetrics Reviewer

## Purpose

Review and operate OpenMetrics using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenMetrics code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenMetrics requires evidence-backed assessment.
...
- First-party source: https://github.com/OpenObservability/OpenMetrics
...
- [ ] Trace OpenMetrics inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenMetrics defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenMetrics least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenMetrics resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenMetrics release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenMetrics dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenMetrics identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenMetrics change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenMetrics documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenMetrics architecture and dependency summary with trust boundaries and persistent state.
- Include OpenMetrics validation commands or tests that are safe for the stated environment.
...
- OpenMetrics identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenMetrics failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenMetrics is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenMetrics as the basis for migration or production guidance.
