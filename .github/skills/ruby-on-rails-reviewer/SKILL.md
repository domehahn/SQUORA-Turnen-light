---
name: "ruby-on-rails-reviewer"
description: "Review Rails models, controllers, jobs, migrations, Active Record behavior, security, tests, and deployment safety."
version: "0.1.0"
since: "2026-08-27"
...
- Source code, configuration, dependencies, tests, deployment artifacts, or runtime behavior in Ruby on Rails applications change.
...
- Active Record, migrations, controllers, background jobs, security defaults, RSpec
...
- [ ] Trace Ruby on Rails applications inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Ruby on Rails applications defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Ruby on Rails applications least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Ruby on Rails applications resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Ruby on Rails applications release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
...
- If Ruby on Rails applications identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Ruby on Rails applications change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Ruby on Rails applications documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Ruby on Rails applications architecture and dependency summary with trust boundaries and persistent state.
- Include Ruby on Rails applications validation commands or tests that are safe for the stated environment.
...
- Ruby on Rails applications identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Ruby on Rails applications failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Ruby on Rails applications is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Ruby on Rails applications as the basis for migration or production guidance.
