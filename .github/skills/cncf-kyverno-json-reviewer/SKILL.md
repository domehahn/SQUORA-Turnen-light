---
name: "cncf-kyverno-json-reviewer"
description: "Review Kyverno-JSON using its official documentation and repository in the AI Native Infra / Governance, Policy and Security category. Use Kyverno's powerful, declarative, low-code policies to validate any runtime or configuration data that can be converted to JSON."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kyverno Json Reviewer

## Purpose

Review and operate Kyverno-JSON using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kyverno-JSON code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kyverno-JSON requires evidence-backed assessment.
...
- First-party source: https://github.com/kyverno/kyverno-json
...
- [ ] Trace Kyverno-JSON inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kyverno-JSON defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kyverno-JSON least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kyverno-JSON resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kyverno-JSON release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kyverno-JSON dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kyverno-JSON identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kyverno-JSON change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kyverno-JSON documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kyverno-JSON architecture and dependency summary with trust boundaries and persistent state.
- Include Kyverno-JSON validation commands or tests that are safe for the stated environment.
...
- Kyverno-JSON identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kyverno-JSON failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kyverno-JSON is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kyverno-JSON as the basis for migration or production guidance.
