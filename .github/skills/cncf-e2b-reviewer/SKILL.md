---
name: "cncf-e2b-reviewer"
description: "Review e2b using its official documentation and repository in the AI Native Infra / Workload Runtime category. Open-source, secure environment with real-world tools for enterprise-grade agents."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate e2b using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- e2b code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving e2b requires evidence-backed assessment.
...
- First-party source: https://github.com/e2b-dev/infra
...
- [ ] Trace e2b inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check e2b defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check e2b least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check e2b resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check e2b release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check e2b dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If e2b identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a e2b change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- e2b documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a e2b architecture and dependency summary with trust boundaries and persistent state.
- Include e2b validation commands or tests that are safe for the stated environment.
...
- e2b identity, version, configuration, dependencies, and deployment assumptions are explicit.
- e2b failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that e2b is secure, supported, compatible, or suitable.
- Using an unpinned latest version of e2b as the basis for migration or production guidance.
