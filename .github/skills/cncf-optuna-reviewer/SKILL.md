---
name: "cncf-optuna-reviewer"
description: "Review Optuna using its official documentation and repository in the Training / Post Training category. A hyperparameter optimization framework"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Optuna Reviewer

## Purpose

Review and operate Optuna using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Optuna code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Optuna requires evidence-backed assessment.
...
- First-party source: https://github.com/optuna/optuna
...
- [ ] Trace Optuna inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Optuna defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Optuna least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Optuna resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Optuna release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Optuna dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Optuna identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Optuna change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Optuna documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Optuna architecture and dependency summary with trust boundaries and persistent state.
- Include Optuna validation commands or tests that are safe for the stated environment.
...
- Optuna identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Optuna failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Optuna is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Optuna as the basis for migration or production guidance.
