...
---
# Cncf Kubeops Reviewer

## Purpose

Review and operate KubeOps using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeOps code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeOps requires evidence-backed assessment.
...
- First-party source: https://kubeops.net/
...
- [ ] Trace KubeOps inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeOps defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeOps least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeOps resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeOps release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeOps dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeOps identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeOps change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeOps documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeOps architecture and dependency summary with trust boundaries and persistent state.
- Include KubeOps validation commands or tests that are safe for the stated environment.
...
- KubeOps identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeOps failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeOps is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeOps as the basis for migration or production guidance.
