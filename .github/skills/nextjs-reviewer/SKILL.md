...
description: "Review Next.js routing, Server and Client Components, caching, data fetching, security, deployment, and performance."
version: "0.1.0"
since: "2026-08-27"
...
- Source code, configuration, dependencies, tests, deployment artifacts, or runtime behavior in Next.js applications change.
...
- App Router, Server Components, caching, data fetching, route handlers, deployment runtime
...
- [ ] Trace Next.js applications inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Next.js applications defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Next.js applications least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Next.js applications resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Next.js applications release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
...
- If Next.js applications identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Next.js applications change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Next.js applications documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Next.js applications architecture and dependency summary with trust boundaries and persistent state.
- Include Next.js applications validation commands or tests that are safe for the stated environment.
...
- Next.js applications identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Next.js applications failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Next.js applications is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Next.js applications as the basis for migration or production guidance.
