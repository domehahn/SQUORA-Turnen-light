...
---
# Cncf Agileops Kcsp Reviewer

## Purpose

Review and operate AgileOps (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- AgileOps (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving AgileOps (KCSP) requires evidence-backed assessment.
...
- [ ] Trace AgileOps (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check AgileOps (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check AgileOps (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check AgileOps (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check AgileOps (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check AgileOps (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If AgileOps (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a AgileOps (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- AgileOps (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a AgileOps (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include AgileOps (KCSP) validation commands or tests that are safe for the stated environment.
...
- AgileOps (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- AgileOps (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that AgileOps (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of AgileOps (KCSP) as the basis for migration or production guidance.
