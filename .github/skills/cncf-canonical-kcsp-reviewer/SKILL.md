---
name: "cncf-canonical-kcsp-reviewer"
description: "Review Canonical (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. The Canonical Distribution of Kubernetes enables you to operate Kubernetes clusters on demand on any major public cloud and private infrastructure."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Canonical Kcsp Reviewer

## Purpose

Review and operate Canonical (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Canonical (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Canonical (KCSP) requires evidence-backed assessment.
...
- First-party source: https://ubuntu.com/kubernetes
...
- [ ] Trace Canonical (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Canonical (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Canonical (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Canonical (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Canonical (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Canonical (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Canonical (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Canonical (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Canonical (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Canonical (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Canonical (KCSP) validation commands or tests that are safe for the stated environment.
...
- Canonical (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Canonical (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Canonical (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Canonical (KCSP) as the basis for migration or production guidance.
