---
name: "cncf-nagios-reviewer"
description: "Review Nagios using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Nagios Reviewer

## Purpose

Review and operate Nagios using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Nagios code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Nagios requires evidence-backed assessment.
...
- First-party source: https://github.com/NagiosEnterprises/nagioscore
...
- [ ] Trace Nagios inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Nagios defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Nagios least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Nagios resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Nagios release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Nagios dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Nagios identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Nagios change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Nagios documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Nagios architecture and dependency summary with trust boundaries and persistent state.
- Include Nagios validation commands or tests that are safe for the stated environment.
...
- Nagios identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Nagios failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Nagios is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Nagios as the basis for migration or production guidance.
