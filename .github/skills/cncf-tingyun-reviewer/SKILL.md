---
name: "cncf-tingyun-reviewer"
description: "Review Tingyun using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Tingyun Reviewer

## Purpose

Review and operate Tingyun using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Tingyun code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Tingyun requires evidence-backed assessment.
...
- First-party source: https://www.tingyun.com/tingyun-app
...
- [ ] Trace Tingyun inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Tingyun defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Tingyun least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Tingyun resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Tingyun release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Tingyun dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Tingyun identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Tingyun change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Tingyun documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Tingyun architecture and dependency summary with trust boundaries and persistent state.
- Include Tingyun validation commands or tests that are safe for the stated environment.
...
- Tingyun identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Tingyun failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Tingyun is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Tingyun as the basis for migration or production guidance.
