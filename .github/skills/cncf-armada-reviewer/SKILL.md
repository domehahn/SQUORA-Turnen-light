---
name: "cncf-armada-reviewer"
description: "Review Armada using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. Armada is a multi-Kubernetes cluster batch job scheduler"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Armada Reviewer

## Purpose

Review and operate Armada using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Armada code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Armada requires evidence-backed assessment.
...
- First-party source: https://github.com/armadaproject/armada
...
- [ ] Trace Armada inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Armada defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Armada least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Armada resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Armada release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Armada dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Armada identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Armada change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Armada documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Armada architecture and dependency summary with trust boundaries and persistent state.
- Include Armada validation commands or tests that are safe for the stated environment.
...
- Armada identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Armada failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Armada is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Armada as the basis for migration or production guidance.
