---
name: "cncf-cyclops-reviewer"
description: "Review Cyclops using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category. Cyclops is a customizable UI for Kubernetes workloads."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Cyclops Reviewer

## Purpose

Review and operate Cyclops using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Cyclops code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Cyclops requires evidence-backed assessment.
...
- First-party source: https://github.com/cyclops-ui/cyclops
...
- [ ] Trace Cyclops inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Cyclops defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Cyclops least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Cyclops resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Cyclops release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Cyclops dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Cyclops identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Cyclops change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Cyclops documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Cyclops architecture and dependency summary with trust boundaries and persistent state.
- Include Cyclops validation commands or tests that are safe for the stated environment.
...
- Cyclops identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Cyclops failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Cyclops is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Cyclops as the basis for migration or production guidance.
