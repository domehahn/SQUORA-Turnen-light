---
name: "cncf-runme-notebooks-reviewer"
description: "Review Runme Notebooks using its official documentation and repository in the Provisioning / Automation & Configuration category. A toolchain that turns Markdown into interactive, cloud-native, runnable Notebook experiences for DevOps."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Runme Notebooks Reviewer

## Purpose

Review and operate Runme Notebooks using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Runme Notebooks code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Runme Notebooks requires evidence-backed assessment.
...
- First-party source: https://github.com/runmedev/runme
...
- [ ] Trace Runme Notebooks inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Runme Notebooks defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Runme Notebooks least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Runme Notebooks resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Runme Notebooks release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Runme Notebooks dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Runme Notebooks identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Runme Notebooks change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Runme Notebooks documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Runme Notebooks architecture and dependency summary with trust boundaries and persistent state.
- Include Runme Notebooks validation commands or tests that are safe for the stated environment.
...
- Runme Notebooks identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Runme Notebooks failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Runme Notebooks is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Runme Notebooks as the basis for migration or production guidance.
