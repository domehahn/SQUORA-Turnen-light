...
---
# Cncf Aks Engine For Azure Stack Reviewer

## Purpose

Review and operate AKS Engine for Azure Stack using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- AKS Engine for Azure Stack code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving AKS Engine for Azure Stack requires evidence-backed assessment.
...
- First-party source: https://docs.microsoft.com/en-us/azure-stack/user/azure-stack-kubernetes-aks-engine-overview?view=azs-2501
...
- [ ] Trace AKS Engine for Azure Stack inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check AKS Engine for Azure Stack defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check AKS Engine for Azure Stack least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check AKS Engine for Azure Stack resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check AKS Engine for Azure Stack release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check AKS Engine for Azure Stack dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If AKS Engine for Azure Stack identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a AKS Engine for Azure Stack change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- AKS Engine for Azure Stack documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a AKS Engine for Azure Stack architecture and dependency summary with trust boundaries and persistent state.
- Include AKS Engine for Azure Stack validation commands or tests that are safe for the stated environment.
...
- AKS Engine for Azure Stack identity, version, configuration, dependencies, and deployment assumptions are explicit.
- AKS Engine for Azure Stack failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that AKS Engine for Azure Stack is secure, supported, compatible, or suitable.
- Using an unpinned latest version of AKS Engine for Azure Stack as the basis for migration or production guidance.
