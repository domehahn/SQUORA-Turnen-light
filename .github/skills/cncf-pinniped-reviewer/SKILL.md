---
name: "cncf-pinniped-reviewer"
description: "Review Pinniped using its official documentation and repository in the Provisioning / Key Management category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Pinniped Reviewer

## Purpose

Review and operate Pinniped using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Pinniped code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Pinniped requires evidence-backed assessment.
...
- First-party source: https://github.com/vmware-tanzu/pinniped
...
- [ ] Trace Pinniped inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Pinniped defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Pinniped least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Pinniped resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Pinniped release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Pinniped dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Pinniped identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Pinniped change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Pinniped documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Pinniped architecture and dependency summary with trust boundaries and persistent state.
- Include Pinniped validation commands or tests that are safe for the stated environment.
...
- Pinniped identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Pinniped failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Pinniped is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Pinniped as the basis for migration or production guidance.
