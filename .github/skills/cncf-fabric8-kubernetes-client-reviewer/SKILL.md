...
---
# Cncf Fabric8 Kubernetes Client Reviewer

## Purpose

Review and operate Fabric8 Kubernetes Client using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Fabric8 Kubernetes Client code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Fabric8 Kubernetes Client requires evidence-backed assessment.
...
- First-party source: https://github.com/fabric8io/kubernetes-client
...
- [ ] Trace Fabric8 Kubernetes Client inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Fabric8 Kubernetes Client defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Fabric8 Kubernetes Client least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Fabric8 Kubernetes Client resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Fabric8 Kubernetes Client release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Fabric8 Kubernetes Client dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Fabric8 Kubernetes Client identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Fabric8 Kubernetes Client change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Fabric8 Kubernetes Client documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Fabric8 Kubernetes Client architecture and dependency summary with trust boundaries and persistent state.
- Include Fabric8 Kubernetes Client validation commands or tests that are safe for the stated environment.
...
- Fabric8 Kubernetes Client identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Fabric8 Kubernetes Client failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Fabric8 Kubernetes Client is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Fabric8 Kubernetes Client as the basis for migration or production guidance.
