---
name: "cncf-sops-reviewer"
description: "Review SOPS using its official documentation and repository in the Provisioning / Security & Compliance category. sops is an editor of encrypted files that supports YAML, JSON, ENV, INI and BINARY formats and encrypts with AWS KMS, GCP KMS, Azure Key Vault, age, and PGP."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate SOPS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SOPS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SOPS requires evidence-backed assessment.
...
- First-party source: https://github.com/getsops/sops
...
- [ ] Trace SOPS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SOPS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SOPS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SOPS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SOPS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SOPS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SOPS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SOPS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SOPS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SOPS architecture and dependency summary with trust boundaries and persistent state.
- Include SOPS validation commands or tests that are safe for the stated environment.
...
- SOPS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SOPS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SOPS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SOPS as the basis for migration or production guidance.
