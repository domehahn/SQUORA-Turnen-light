---
name: "cncf-teleport-reviewer"
description: "Review Teleport using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Teleport Reviewer

## Purpose

Review and operate Teleport using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Teleport code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Teleport requires evidence-backed assessment.
...
- First-party source: https://github.com/gravitational/teleport
...
- [ ] Trace Teleport inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Teleport defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Teleport least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Teleport resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Teleport release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Teleport dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Teleport identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Teleport change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Teleport documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Teleport architecture and dependency summary with trust boundaries and persistent state.
- Include Teleport validation commands or tests that are safe for the stated environment.
...
- Teleport identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Teleport failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Teleport is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Teleport as the basis for migration or production guidance.
