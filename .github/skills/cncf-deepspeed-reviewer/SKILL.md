---
name: "cncf-deepspeed-reviewer"
description: "Review DeepSpeed using its official documentation and repository in the Training / Distributed Training category. Deep learning optimization library that makes distributed training and inference easy, efficient, and effective."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Deepspeed Reviewer

## Purpose

Review and operate DeepSpeed using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DeepSpeed code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DeepSpeed requires evidence-backed assessment.
...
- First-party source: https://github.com/microsoft/DeepSpeed
...
- [ ] Trace DeepSpeed inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DeepSpeed defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DeepSpeed least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DeepSpeed resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DeepSpeed release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DeepSpeed dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DeepSpeed identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DeepSpeed change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DeepSpeed documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DeepSpeed architecture and dependency summary with trust boundaries and persistent state.
- Include DeepSpeed validation commands or tests that are safe for the stated environment.
...
- DeepSpeed identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DeepSpeed failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DeepSpeed is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DeepSpeed as the basis for migration or production guidance.
