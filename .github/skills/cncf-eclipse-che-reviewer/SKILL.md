---
name: "cncf-eclipse-che-reviewer"
description: "Review Eclipse Che using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Eclipse Che Reviewer

## Purpose

Review and operate Eclipse Che using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Eclipse Che code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Eclipse Che requires evidence-backed assessment.
...
- First-party source: https://github.com/eclipse-che/che
...
- [ ] Trace Eclipse Che inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Eclipse Che defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Eclipse Che least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Eclipse Che resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Eclipse Che release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Eclipse Che dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Eclipse Che identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Eclipse Che change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Eclipse Che documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Eclipse Che architecture and dependency summary with trust boundaries and persistent state.
- Include Eclipse Che validation commands or tests that are safe for the stated environment.
...
- Eclipse Che identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Eclipse Che failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Eclipse Che is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Eclipse Che as the basis for migration or production guidance.
