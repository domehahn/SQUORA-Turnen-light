...
---
# Cncf Bunnyshell Reviewer

## Purpose

Review and operate Bunnyshell using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Bunnyshell code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Bunnyshell requires evidence-backed assessment.
...
- First-party source: https://bunnyshell.com/
...
- [ ] Trace Bunnyshell inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Bunnyshell defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Bunnyshell least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Bunnyshell resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Bunnyshell release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Bunnyshell dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Bunnyshell identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Bunnyshell change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Bunnyshell documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Bunnyshell architecture and dependency summary with trust boundaries and persistent state.
- Include Bunnyshell validation commands or tests that are safe for the stated environment.
...
- Bunnyshell identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Bunnyshell failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Bunnyshell is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Bunnyshell as the basis for migration or production guidance.
