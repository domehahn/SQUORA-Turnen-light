...
---
# Cncf Vmware Tanzu Kubernetes Grid Reviewer

## Purpose

Review and operate VMware Tanzu Kubernetes Grid using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- VMware Tanzu Kubernetes Grid code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving VMware Tanzu Kubernetes Grid requires evidence-backed assessment.
...
- First-party source: https://tanzu.vmware.com/kubernetes-grid
...
- [ ] Trace VMware Tanzu Kubernetes Grid inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check VMware Tanzu Kubernetes Grid defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check VMware Tanzu Kubernetes Grid least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check VMware Tanzu Kubernetes Grid resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check VMware Tanzu Kubernetes Grid release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check VMware Tanzu Kubernetes Grid dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If VMware Tanzu Kubernetes Grid identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a VMware Tanzu Kubernetes Grid change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- VMware Tanzu Kubernetes Grid documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a VMware Tanzu Kubernetes Grid architecture and dependency summary with trust boundaries and persistent state.
- Include VMware Tanzu Kubernetes Grid validation commands or tests that are safe for the stated environment.
...
- VMware Tanzu Kubernetes Grid identity, version, configuration, dependencies, and deployment assumptions are explicit.
- VMware Tanzu Kubernetes Grid failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that VMware Tanzu Kubernetes Grid is secure, supported, compatible, or suitable.
- Using an unpinned latest version of VMware Tanzu Kubernetes Grid as the basis for migration or production guidance.
