---
name: "cncf-x-cellent-kcsp-reviewer"
description: "Review x-cellent (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. x-cellent provides feature development and support for Kubernetes on-prem based on metal-stack.io, security audits, cloud adoption and cloud native software development."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf X Cellent Kcsp Reviewer

## Purpose

Review and operate x-cellent (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- x-cellent (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving x-cellent (KCSP) requires evidence-backed assessment.
...
- [ ] Trace x-cellent (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check x-cellent (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check x-cellent (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check x-cellent (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check x-cellent (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check x-cellent (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If x-cellent (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a x-cellent (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- x-cellent (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a x-cellent (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include x-cellent (KCSP) validation commands or tests that are safe for the stated environment.
...
- x-cellent (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- x-cellent (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that x-cellent (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of x-cellent (KCSP) as the basis for migration or production guidance.
