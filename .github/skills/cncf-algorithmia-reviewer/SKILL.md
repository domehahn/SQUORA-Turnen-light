---
name: "cncf-algorithmia-reviewer"
description: "Review Algorithmia using its official documentation and repository in the Serverless / Hosted Platform category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Algorithmia Reviewer

## Purpose

Review and operate Algorithmia using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Algorithmia code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Algorithmia requires evidence-backed assessment.
...
- First-party source: https://algorithmia.com/product
...
- [ ] Trace Algorithmia inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Algorithmia defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Algorithmia least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Algorithmia resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Algorithmia release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Algorithmia dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Algorithmia identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Algorithmia change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Algorithmia documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Algorithmia architecture and dependency summary with trust boundaries and persistent state.
- Include Algorithmia validation commands or tests that are safe for the stated environment.
...
- Algorithmia identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Algorithmia failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Algorithmia is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Algorithmia as the basis for migration or production guidance.
