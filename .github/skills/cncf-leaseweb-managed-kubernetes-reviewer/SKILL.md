...
---
# Cncf Leaseweb Managed Kubernetes Reviewer

## Purpose

Review and operate Leaseweb Managed Kubernetes using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Leaseweb Managed Kubernetes code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Leaseweb Managed Kubernetes requires evidence-backed assessment.
...
- First-party source: https://www.leaseweb.com/en/products-services/cloud/kubernetes
...
- [ ] Trace Leaseweb Managed Kubernetes inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Leaseweb Managed Kubernetes defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Leaseweb Managed Kubernetes least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Leaseweb Managed Kubernetes resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Leaseweb Managed Kubernetes release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Leaseweb Managed Kubernetes dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Leaseweb Managed Kubernetes identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Leaseweb Managed Kubernetes change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Leaseweb Managed Kubernetes documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Leaseweb Managed Kubernetes architecture and dependency summary with trust boundaries and persistent state.
- Include Leaseweb Managed Kubernetes validation commands or tests that are safe for the stated environment.
...
- Leaseweb Managed Kubernetes identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Leaseweb Managed Kubernetes failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Leaseweb Managed Kubernetes is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Leaseweb Managed Kubernetes as the basis for migration or production guidance.
