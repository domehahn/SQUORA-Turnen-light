---
name: "cncf-zabbix-reviewer"
description: "Review Zabbix using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Zabbix Reviewer

## Purpose

Review and operate Zabbix using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Zabbix code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Zabbix requires evidence-backed assessment.
...
- First-party source: https://github.com/zabbix/zabbix
...
- [ ] Trace Zabbix inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Zabbix defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Zabbix least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Zabbix resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Zabbix release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Zabbix dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Zabbix identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Zabbix change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Zabbix documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Zabbix architecture and dependency summary with trust boundaries and persistent state.
- Include Zabbix validation commands or tests that are safe for the stated environment.
...
- Zabbix identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Zabbix failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Zabbix is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Zabbix as the basis for migration or production guidance.
