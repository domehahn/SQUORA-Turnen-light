---
name: "cncf-cassandra-reviewer"
description: "Review Cassandra using its official documentation and repository in the App Definition and Development / Database category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Cassandra Reviewer

## Purpose

Review and operate Cassandra using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Cassandra code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Cassandra requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/cassandra
...
- [ ] Trace Cassandra inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Cassandra defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Cassandra least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Cassandra resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Cassandra release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Cassandra dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Cassandra identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Cassandra change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Cassandra documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Cassandra architecture and dependency summary with trust boundaries and persistent state.
- Include Cassandra validation commands or tests that are safe for the stated environment.
...
- Cassandra identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Cassandra failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Cassandra is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Cassandra as the basis for migration or production guidance.
