---
name: "cncf-zig-reviewer"
description: "Review Zig using its official documentation and repository in the Wasm / Languages category. Compiled language to Wasm"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Zig using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Zig code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Zig requires evidence-backed assessment.
...
- First-party source: https://github.com/ziglang/zig
...
- [ ] Trace Zig inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Zig defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Zig least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Zig resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Zig release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Zig dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Zig identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Zig change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Zig documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Zig architecture and dependency summary with trust boundaries and persistent state.
- Include Zig validation commands or tests that are safe for the stated environment.
...
- Zig identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Zig failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Zig is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Zig as the basis for migration or production guidance.
