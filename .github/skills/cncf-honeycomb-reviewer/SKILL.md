---
name: "cncf-honeycomb-reviewer"
description: "Review Honeycomb using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Honeycomb Reviewer

## Purpose

Review and operate Honeycomb using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Honeycomb code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Honeycomb requires evidence-backed assessment.
...
- [ ] Trace Honeycomb inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Honeycomb defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Honeycomb least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Honeycomb resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Honeycomb release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Honeycomb dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Honeycomb identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Honeycomb change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Honeycomb documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Honeycomb architecture and dependency summary with trust boundaries and persistent state.
- Include Honeycomb validation commands or tests that are safe for the stated environment.
...
- Honeycomb identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Honeycomb failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Honeycomb is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Honeycomb as the basis for migration or production guidance.
