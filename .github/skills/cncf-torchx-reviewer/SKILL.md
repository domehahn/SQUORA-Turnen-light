---
name: "cncf-torchx-reviewer"
description: "Review Torchx using its official documentation and repository in the Training / Distributed Training category. Universal job launcher for PyTorch applications. TorchX is designed to have fast iteration time for training/research and support for E2E production ML pipelines when you're ready."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Torchx Reviewer

## Purpose

Review and operate Torchx using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Torchx code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Torchx requires evidence-backed assessment.
...
- First-party source: https://pytorch.org/torchx/latest
...
- [ ] Trace Torchx inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Torchx defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Torchx least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Torchx resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Torchx release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Torchx dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Torchx identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Torchx change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Torchx documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Torchx architecture and dependency summary with trust boundaries and persistent state.
- Include Torchx validation commands or tests that are safe for the stated environment.
...
- Torchx identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Torchx failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Torchx is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Torchx as the basis for migration or production guidance.
