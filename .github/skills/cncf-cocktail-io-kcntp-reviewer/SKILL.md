---
name: "cncf-cocktail-io-kcntp-reviewer"
description: "Review Cocktail.io (KCNTP) using its official documentation and repository in the Special / Kubernetes and Cloud Native Training Partner category. As a CNCF KCSP (Kubernetes Certified Service Provider) and a developer of Cocktail Cloud Kubernetes Orchestration Platform Product, Acornsoft trains clients and prospective clients on Docker and Kubernetes."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Cocktail Io Kcntp Reviewer

## Purpose

Review and operate Cocktail.io (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Cocktail.io (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Cocktail.io (KCNTP) requires evidence-backed assessment.
...
- First-party source: https://www.cocktailcloud.io/login.do
...
- [ ] Trace Cocktail.io (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Cocktail.io (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Cocktail.io (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Cocktail.io (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Cocktail.io (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Cocktail.io (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Cocktail.io (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Cocktail.io (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Cocktail.io (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Cocktail.io (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include Cocktail.io (KCNTP) validation commands or tests that are safe for the stated environment.
...
- Cocktail.io (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Cocktail.io (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Cocktail.io (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Cocktail.io (KCNTP) as the basis for migration or production guidance.
