...
---
# Cncf Apiclarity Reviewer

## Purpose

Review and operate APIClarity using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- APIClarity code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving APIClarity requires evidence-backed assessment.
...
- First-party source: https://github.com/openclarity/apiclarity
...
- [ ] Trace APIClarity inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check APIClarity defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check APIClarity least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check APIClarity resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check APIClarity release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check APIClarity dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If APIClarity identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a APIClarity change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- APIClarity documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a APIClarity architecture and dependency summary with trust boundaries and persistent state.
- Include APIClarity validation commands or tests that are safe for the stated environment.
...
- APIClarity identity, version, configuration, dependencies, and deployment assumptions are explicit.
- APIClarity failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that APIClarity is secure, supported, compatible, or suitable.
- Using an unpinned latest version of APIClarity as the basis for migration or production guidance.
