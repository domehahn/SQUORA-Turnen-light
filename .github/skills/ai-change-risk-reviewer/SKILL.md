...
1. Identify the relevant AI-assisted change risk artifacts, owners, systems, environments, and review boundary.
2. Compare the available artifacts against expected signals such as human approval, change classification, tool call log, diff summary, validation result, rollback plan.
...
- Primary artifacts: AI change proposal, automation boundary, approval record, system criticality, execution plan, audit trail.
- Risk themes: unapproved autonomous change, critical-system impact, ambiguous human oversight, untracked AI rationale, unsafe tool use, missing rollback.
- Evidence signals: human approval, change classification, tool call log, diff summary, validation result, rollback plan.
...
- [ ] Confirm the review boundary covers the right AI-assisted change risk systems, teams, and environments.
- [ ] Inventory and inspect the current AI change proposal.
- [ ] Check whether automation boundary is current, approved, versioned, and owned.
- [ ] Verify that approval record has test, ticket, log, or approval support.
- [ ] Look for unapproved autonomous change and record concrete repository or process evidence.
- [ ] Look for critical-system impact and identify affected assets, services, or stakeholders.
- [ ] Look for ambiguous human oversight and classify the operational or audit impact.
- [ ] Use human approval to validate that the control or practice is operating.
- [ ] Use change classification to confirm ownership, timing, and reproducibility.
- [ ] Check exception, risk-acceptance, and expiry handling for AI-assisted change risk.
...
- If AI change proposal is missing for a critical service, raise at least a high-severity readiness gap.
- If change classification cannot be tied to an owner and approval, treat the outcome as unauditable until corrected.
- If system criticality is present but expired or untested, require validation before accepting residual risk.
...
- Missing or stale AI-assisted change risk artifact.
...
- Critical: a gap in AI-assisted change risk creates immediate outage, data-loss, privilege, regulatory-reporting, or irreversible business risk.
- High: AI change proposal is missing, unowned, untested, or unauditable for a critical service or material change.
- Medium: automation boundary exists but is stale, incomplete, inconsistently enforced, or weakly evidenced.
...
- Findings ordered by severity with affected AI-assisted change risk artifacts and evidence references.
- Coverage note for reviewed artifacts: AI change proposal, automation boundary, approval record, system criticality, execution plan, audit trail.
- Risk note covering relevant themes: unapproved autonomous change, critical-system impact, ambiguous human oversight, untracked AI rationale, unsafe tool use, missing rollback.
- Evidence request list using expected signals: human approval, change classification, tool call log, diff summary, validation result, rollback plan.
- Deliverables or updates needed: AI change risk assessment, approval gap list, automation-boundary recommendation, criticality findings, audit evidence request.
- Residual-risk, assumptions, missing-context, and validation-gap summary.

## Acceptance Criteria

- Relevant AI-assisted change risk artifacts are identified, current, owned, and versioned where applicable.
