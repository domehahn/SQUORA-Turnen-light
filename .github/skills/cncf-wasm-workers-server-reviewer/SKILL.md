---
name: "cncf-wasm-workers-server-reviewer"
description: "Review Wasm Workers Server using its official documentation and repository in the Wasm / Application Frameworks category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Wasm Workers Server Reviewer

## Purpose

Review and operate Wasm Workers Server using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Wasm Workers Server code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Wasm Workers Server requires evidence-backed assessment.
...
- CNCF classification: Wasm / Application Frameworks
- CNCF project maturity field: not specified
- First-party source: https://github.com/vmware-labs/wasm-workers-server
...
- [ ] Trace Wasm Workers Server inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Wasm Workers Server defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Wasm Workers Server least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Wasm Workers Server resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Wasm Workers Server release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Wasm Workers Server dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Wasm Workers Server identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Wasm Workers Server change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Wasm Workers Server documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Wasm Workers Server architecture and dependency summary with trust boundaries and persistent state.
- Include Wasm Workers Server validation commands or tests that are safe for the stated environment.
...
- Wasm Workers Server identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Wasm Workers Server failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Wasm Workers Server is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Wasm Workers Server as the basis for migration or production guidance.
