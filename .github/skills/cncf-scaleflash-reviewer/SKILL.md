---
name: "cncf-scaleflash-reviewer"
description: "Review ScaleFlash using its official documentation and repository in the Runtime / Cloud Native Storage category. ScaleFlash is a leading provider of high-performance software-defined storage products and enterprise cloud solutions."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Scaleflash Reviewer

## Purpose

Review and operate ScaleFlash using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ScaleFlash code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ScaleFlash requires evidence-backed assessment.
...
- First-party source: https://www.scaleflash.com
...
- [ ] Trace ScaleFlash inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ScaleFlash defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ScaleFlash least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ScaleFlash resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ScaleFlash release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ScaleFlash dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ScaleFlash identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ScaleFlash change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ScaleFlash documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ScaleFlash architecture and dependency summary with trust boundaries and persistent state.
- Include ScaleFlash validation commands or tests that are safe for the stated environment.
...
- ScaleFlash identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ScaleFlash failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ScaleFlash is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ScaleFlash as the basis for migration or production guidance.
