---
name: "cncf-ibm-storage-reviewer"
description: "Review IBM Storage using its official documentation and repository in the Runtime / Cloud Native Storage category. IBM Storage for containers, Kubernetes, and Red Hat OpenShift delivers native cloud acceleration to help you build powerful, agile, and persistent storage for private cloud environments."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate IBM Storage using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- IBM Storage code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving IBM Storage requires evidence-backed assessment.
...
- First-party source: https://www.ibm.com/storage/containers
...
- [ ] Trace IBM Storage inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check IBM Storage defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check IBM Storage least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check IBM Storage resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check IBM Storage release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check IBM Storage dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If IBM Storage identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a IBM Storage change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- IBM Storage documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a IBM Storage architecture and dependency summary with trust boundaries and persistent state.
- Include IBM Storage validation commands or tests that are safe for the stated environment.
...
- IBM Storage identity, version, configuration, dependencies, and deployment assumptions are explicit.
- IBM Storage failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that IBM Storage is secure, supported, compatible, or suitable.
- Using an unpinned latest version of IBM Storage as the basis for migration or production guidance.
