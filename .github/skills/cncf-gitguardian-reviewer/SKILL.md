---
name: "cncf-gitguardian-reviewer"
description: "Review GitGuardian using its official documentation and repository in the Provisioning / Security & Compliance category. GitGuardian is the code security platform for the DevOps generation that offers automated Secrets Detection, Infra as Code Security,  and Honeytoken capabilities, facilitating a Secure Software Development Lifecycle for Dev, Sec, and Ops teams."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Gitguardian Reviewer

## Purpose

Review and operate GitGuardian using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- GitGuardian code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving GitGuardian requires evidence-backed assessment.
...
- First-party source: https://www.gitguardian.com/
...
- [ ] Trace GitGuardian inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check GitGuardian defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check GitGuardian least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check GitGuardian resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check GitGuardian release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check GitGuardian dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If GitGuardian identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a GitGuardian change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- GitGuardian documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a GitGuardian architecture and dependency summary with trust boundaries and persistent state.
- Include GitGuardian validation commands or tests that are safe for the stated environment.
...
- GitGuardian identity, version, configuration, dependencies, and deployment assumptions are explicit.
- GitGuardian failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that GitGuardian is secure, supported, compatible, or suitable.
- Using an unpinned latest version of GitGuardian as the basis for migration or production guidance.
