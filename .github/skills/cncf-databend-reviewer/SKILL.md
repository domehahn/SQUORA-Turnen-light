---
name: "cncf-databend-reviewer"
description: "Review Databend using its official documentation and repository in the App Definition and Development / Database category. Databend is a modern Elasticity and Performance cloud data warehouse, activate your object storage for real-time analytics.  Databend Serverless at https://app.databend.com/"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Databend Reviewer

## Purpose

Review and operate Databend using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Databend code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Databend requires evidence-backed assessment.
...
- First-party source: https://github.com/datafuselabs/databend
...
- [ ] Trace Databend inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Databend defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Databend least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Databend resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Databend release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Databend dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Databend identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Databend change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Databend documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Databend architecture and dependency summary with trust boundaries and persistent state.
- Include Databend validation commands or tests that are safe for the stated environment.
...
- Databend identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Databend failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Databend is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Databend as the basis for migration or production guidance.
