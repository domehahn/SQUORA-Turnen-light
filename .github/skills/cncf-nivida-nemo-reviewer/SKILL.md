---
name: "cncf-nivida-nemo-reviewer"
description: "Review Nivida NeMo using its official documentation and repository in the AI Native Infra / Orchestration and Scheduling category. A framework for generative AI"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Nivida Nemo Reviewer

## Purpose

Review and operate Nivida NeMo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Nivida NeMo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Nivida NeMo requires evidence-backed assessment.
...
- First-party source: https://github.com/NVIDIA/NeMo
...
- [ ] Trace Nivida NeMo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Nivida NeMo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Nivida NeMo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Nivida NeMo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Nivida NeMo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Nivida NeMo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Nivida NeMo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Nivida NeMo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Nivida NeMo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Nivida NeMo architecture and dependency summary with trust boundaries and persistent state.
- Include Nivida NeMo validation commands or tests that are safe for the stated environment.
...
- Nivida NeMo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Nivida NeMo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Nivida NeMo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Nivida NeMo as the basis for migration or production guidance.
