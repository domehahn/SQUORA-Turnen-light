---
name: "cncf-coroot-reviewer"
description: "Review Coroot using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Coroot Reviewer

## Purpose

Review and operate Coroot using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Coroot code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Coroot requires evidence-backed assessment.
...
- First-party source: https://github.com/coroot/coroot
...
- [ ] Trace Coroot inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Coroot defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Coroot least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Coroot resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Coroot release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Coroot dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Coroot identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Coroot change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Coroot documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Coroot architecture and dependency summary with trust boundaries and persistent state.
- Include Coroot validation commands or tests that are safe for the stated environment.
...
- Coroot identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Coroot failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Coroot is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Coroot as the basis for migration or production guidance.
