---
name: "cncf-openlit-reviewer"
description: "Review OpenLIT using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Openlit Reviewer

## Purpose

Review and operate OpenLIT using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenLIT code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenLIT requires evidence-backed assessment.
...
- First-party source: https://github.com/openlit/openlit
...
- [ ] Trace OpenLIT inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenLIT defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenLIT least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenLIT resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenLIT release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenLIT dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenLIT identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenLIT change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenLIT documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenLIT architecture and dependency summary with trust boundaries and persistent state.
- Include OpenLIT validation commands or tests that are safe for the stated environment.
...
- OpenLIT identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenLIT failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenLIT is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenLIT as the basis for migration or production guidance.
