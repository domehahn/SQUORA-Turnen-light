---
name: "cncf-ansible-reviewer"
description: "Review Ansible using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Ansible Reviewer

## Purpose

Review and operate Ansible using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Ansible code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Ansible requires evidence-backed assessment.
...
- First-party source: https://github.com/ansible/ansible
...
- [ ] Trace Ansible inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Ansible defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Ansible least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Ansible resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Ansible release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Ansible dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Ansible identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Ansible change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Ansible documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Ansible architecture and dependency summary with trust boundaries and persistent state.
- Include Ansible validation commands or tests that are safe for the stated environment.
...
- Ansible identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Ansible failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Ansible is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Ansible as the basis for migration or production guidance.
