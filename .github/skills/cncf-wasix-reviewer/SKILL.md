---
name: "cncf-wasix-reviewer"
description: "Review WASIX using its official documentation and repository in the Wasm / Application Frameworks category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate WASIX using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- WASIX code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving WASIX requires evidence-backed assessment.
...
- First-party source: https://github.com/wasix-org/wasix-witx
...
- [ ] Trace WASIX inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check WASIX defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check WASIX least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check WASIX resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check WASIX release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check WASIX dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If WASIX identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a WASIX change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- WASIX documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a WASIX architecture and dependency summary with trust boundaries and persistent state.
- Include WASIX validation commands or tests that are safe for the stated environment.
...
- WASIX identity, version, configuration, dependencies, and deployment assumptions are explicit.
- WASIX failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that WASIX is secure, supported, compatible, or suitable.
- Using an unpinned latest version of WASIX as the basis for migration or production guidance.
