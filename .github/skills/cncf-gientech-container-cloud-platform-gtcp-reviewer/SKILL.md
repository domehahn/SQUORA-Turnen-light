...
---
# Cncf Gientech Container Cloud Platform Gtcp Reviewer

## Purpose

Review and operate GienTech Container Cloud Platform (GTCP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- GienTech Container Cloud Platform (GTCP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving GienTech Container Cloud Platform (GTCP) requires evidence-backed assessment.
...
- [ ] Trace GienTech Container Cloud Platform (GTCP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check GienTech Container Cloud Platform (GTCP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check GienTech Container Cloud Platform (GTCP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check GienTech Container Cloud Platform (GTCP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check GienTech Container Cloud Platform (GTCP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check GienTech Container Cloud Platform (GTCP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If GienTech Container Cloud Platform (GTCP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a GienTech Container Cloud Platform (GTCP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- GienTech Container Cloud Platform (GTCP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a GienTech Container Cloud Platform (GTCP) architecture and dependency summary with trust boundaries and persistent state.
- Include GienTech Container Cloud Platform (GTCP) validation commands or tests that are safe for the stated environment.
...
- GienTech Container Cloud Platform (GTCP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- GienTech Container Cloud Platform (GTCP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that GienTech Container Cloud Platform (GTCP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of GienTech Container Cloud Platform (GTCP) as the basis for migration or production guidance.
