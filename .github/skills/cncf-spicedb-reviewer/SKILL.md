...
---
# Cncf Spicedb Reviewer

## Purpose

Review and operate SpiceDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SpiceDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SpiceDB requires evidence-backed assessment.
...
- First-party source: https://github.com/authzed/spicedb
...
- [ ] Trace SpiceDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SpiceDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SpiceDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SpiceDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SpiceDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SpiceDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SpiceDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SpiceDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SpiceDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SpiceDB architecture and dependency summary with trust boundaries and persistent state.
- Include SpiceDB validation commands or tests that are safe for the stated environment.
...
- SpiceDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SpiceDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SpiceDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SpiceDB as the basis for migration or production guidance.
