---
name: "cncf-metallb-reviewer"
description: "Review MetalLB using its official documentation and repository in the Orchestration & Management / Service Proxy category. A network load-balancer implementation for Kubernetes using standard routing protocols"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Metallb Reviewer

## Purpose

Review and operate MetalLB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- MetalLB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving MetalLB requires evidence-backed assessment.
...
- First-party source: https://github.com/metallb/metallb
...
- [ ] Trace MetalLB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check MetalLB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check MetalLB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check MetalLB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check MetalLB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check MetalLB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If MetalLB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a MetalLB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- MetalLB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a MetalLB architecture and dependency summary with trust boundaries and persistent state.
- Include MetalLB validation commands or tests that are safe for the stated environment.
...
- MetalLB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- MetalLB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that MetalLB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of MetalLB as the basis for migration or production guidance.
