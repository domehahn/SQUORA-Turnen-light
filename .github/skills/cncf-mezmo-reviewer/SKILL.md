---
name: "cncf-mezmo-reviewer"
description: "Review Mezmo using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Mezmo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Mezmo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Mezmo requires evidence-backed assessment.
...
- [ ] Trace Mezmo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Mezmo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Mezmo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Mezmo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Mezmo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Mezmo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Mezmo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Mezmo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Mezmo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Mezmo architecture and dependency summary with trust boundaries and persistent state.
- Include Mezmo validation commands or tests that are safe for the stated environment.
...
- Mezmo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Mezmo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Mezmo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Mezmo as the basis for migration or production guidance.
