---
name: "cncf-rkt-reviewer"
description: "Review rkt using its official documentation and repository in the Runtime / Container Runtime category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate rkt using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- rkt code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving rkt requires evidence-backed assessment.
...
- First-party source: https://github.com/rkt/rkt
...
- [ ] Trace rkt inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check rkt defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check rkt least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check rkt resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check rkt release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check rkt dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If rkt identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a rkt change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- rkt documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a rkt architecture and dependency summary with trust boundaries and persistent state.
- Include rkt validation commands or tests that are safe for the stated environment.
...
- rkt identity, version, configuration, dependencies, and deployment assumptions are explicit.
- rkt failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that rkt is secure, supported, compatible, or suitable.
- Using an unpinned latest version of rkt as the basis for migration or production guidance.
