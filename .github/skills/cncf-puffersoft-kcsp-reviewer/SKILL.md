...
---
# Cncf Puffersoft Kcsp Reviewer

## Purpose

Review and operate PufferSoft (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- PufferSoft (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving PufferSoft (KCSP) requires evidence-backed assessment.
...
- First-party source: https://puffersoft.com/kubernetes-certified-service-provider-kcsp/
...
- [ ] Trace PufferSoft (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check PufferSoft (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check PufferSoft (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check PufferSoft (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check PufferSoft (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check PufferSoft (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If PufferSoft (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a PufferSoft (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- PufferSoft (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a PufferSoft (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include PufferSoft (KCSP) validation commands or tests that are safe for the stated environment.
...
- PufferSoft (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- PufferSoft (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that PufferSoft (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of PufferSoft (KCSP) as the basis for migration or production guidance.
