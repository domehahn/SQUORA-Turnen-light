---
name: "cncf-whizus-kcntp-reviewer"
description: "Review WhizUs (KCNTP) using its official documentation and repository in the Special / Kubernetes and Cloud Native Training Partner category. As Enterprise Kubernetes Consultants, we support you to build up your Kubernetes cluster in your chosen environment according to your needs."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Whizus Kcntp Reviewer

## Purpose

Review and operate WhizUs (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- WhizUs (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving WhizUs (KCNTP) requires evidence-backed assessment.
...
- [ ] Trace WhizUs (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check WhizUs (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check WhizUs (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check WhizUs (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check WhizUs (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check WhizUs (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If WhizUs (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a WhizUs (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- WhizUs (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a WhizUs (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include WhizUs (KCNTP) validation commands or tests that are safe for the stated environment.
...
- WhizUs (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- WhizUs (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that WhizUs (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of WhizUs (KCNTP) as the basis for migration or production guidance.
