...
---
# Cncf China Systems Kcsp Reviewer

## Purpose

Review and operate China Systems (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- China Systems (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving China Systems (KCSP) requires evidence-backed assessment.
...
- First-party source: https://chinasystems.com/partners/kubernetes
...
- [ ] Trace China Systems (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check China Systems (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check China Systems (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check China Systems (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check China Systems (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check China Systems (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If China Systems (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a China Systems (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- China Systems (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a China Systems (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include China Systems (KCSP) validation commands or tests that are safe for the stated environment.
...
- China Systems (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- China Systems (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that China Systems (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of China Systems (KCSP) as the basis for migration or production guidance.
