---
name: "cncf-opsmx-reviewer"
description: "Review OpsMx using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category. OpsMx provides OSS Argo Support and services and Intelligent Software Delivery platform based on both Argo and Spinnaker."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate OpsMx using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpsMx code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpsMx requires evidence-backed assessment.
...
- [ ] Trace OpsMx inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpsMx defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpsMx least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpsMx resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpsMx release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpsMx dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpsMx identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpsMx change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpsMx documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpsMx architecture and dependency summary with trust boundaries and persistent state.
- Include OpsMx validation commands or tests that are safe for the stated environment.
...
- OpsMx identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpsMx failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpsMx is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpsMx as the basis for migration or production guidance.
