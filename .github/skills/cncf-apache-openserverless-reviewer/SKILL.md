---
name: "cncf-apache-openserverless-reviewer"
description: "Review Apache OpenServerless using its official documentation and repository in the Serverless / Installable Platform category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Apache Openserverless Reviewer

## Purpose

Review and operate Apache OpenServerless using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Apache OpenServerless code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Apache OpenServerless requires evidence-backed assessment.
...
- CNCF classification: Serverless / Installable Platform
- CNCF project maturity field: not specified
- First-party source: https://github.com/apache/openserverless
...
- [ ] Trace Apache OpenServerless inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Apache OpenServerless defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Apache OpenServerless least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Apache OpenServerless resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Apache OpenServerless release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Apache OpenServerless dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Apache OpenServerless identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Apache OpenServerless change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Apache OpenServerless documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Apache OpenServerless architecture and dependency summary with trust boundaries and persistent state.
- Include Apache OpenServerless validation commands or tests that are safe for the stated environment.
...
- Apache OpenServerless identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Apache OpenServerless failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Apache OpenServerless is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Apache OpenServerless as the basis for migration or production guidance.
