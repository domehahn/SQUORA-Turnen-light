---
name: "cncf-lunatic-reviewer"
description: "Review Lunatic using its official documentation and repository in the Wasm / Runtimes category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Lunatic Reviewer

## Purpose

Review and operate Lunatic using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Lunatic code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Lunatic requires evidence-backed assessment.
...
- First-party source: https://github.com/lunatic-solutions/lunatic
...
- [ ] Trace Lunatic inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Lunatic defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Lunatic least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Lunatic resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Lunatic release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Lunatic dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Lunatic identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Lunatic change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Lunatic documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Lunatic architecture and dependency summary with trust boundaries and persistent state.
- Include Lunatic validation commands or tests that are safe for the stated environment.
...
- Lunatic identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Lunatic failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Lunatic is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Lunatic as the basis for migration or production guidance.
