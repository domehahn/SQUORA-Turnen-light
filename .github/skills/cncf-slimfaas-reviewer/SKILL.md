---
name: "cncf-slimfaas-reviewer"
description: "Review SlimFaaS using its official documentation and repository in the Serverless / Installable Platform category. The slimest and simplest Function As A Service"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Slimfaas Reviewer

## Purpose

Review and operate SlimFaaS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SlimFaaS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SlimFaaS requires evidence-backed assessment.
...
- First-party source: https://github.com/SlimPlanet/SlimFaas
...
- [ ] Trace SlimFaaS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SlimFaaS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SlimFaaS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SlimFaaS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SlimFaaS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SlimFaaS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SlimFaaS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SlimFaaS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SlimFaaS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SlimFaaS architecture and dependency summary with trust boundaries and persistent state.
- Include SlimFaaS validation commands or tests that are safe for the stated environment.
...
- SlimFaaS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SlimFaaS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SlimFaaS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SlimFaaS as the basis for migration or production guidance.
