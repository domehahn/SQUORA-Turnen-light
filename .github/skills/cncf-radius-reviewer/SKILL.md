...
---
# Cncf Radius Reviewer

## Purpose

Review and operate Radius using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Radius code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Radius requires evidence-backed assessment.
...
- First-party source: https://github.com/radius-project/radius
...
- [ ] Trace Radius inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Radius defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Radius least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Radius resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Radius release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Radius dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Radius identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Radius change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Radius documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Radius architecture and dependency summary with trust boundaries and persistent state.
- Include Radius validation commands or tests that are safe for the stated environment.
...
- Radius identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Radius failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Radius is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Radius as the basis for migration or production guidance.
