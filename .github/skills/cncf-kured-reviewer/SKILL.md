...
Review and operate Kured using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kured code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kured requires evidence-backed assessment.
...
- First-party source: https://github.com/kubereboot/kured
...
- [ ] Trace Kured inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kured defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kured least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kured resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kured release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kured dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kured identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kured change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kured documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kured architecture and dependency summary with trust boundaries and persistent state.
- Include Kured validation commands or tests that are safe for the stated environment.
...
- Kured identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kured failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kured is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kured as the basis for migration or production guidance.
