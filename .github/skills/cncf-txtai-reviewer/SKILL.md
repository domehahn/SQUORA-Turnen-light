---
name: "cncf-txtai-reviewer"
description: "Review txtai using its official documentation and repository in the AI Agent / RAG category. All-in-one AI framework for semantic search, LLM orchestration and language model workflows."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate txtai using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- txtai code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving txtai requires evidence-backed assessment.
...
- First-party source: https://github.com/neuml/txtai
...
- [ ] Trace txtai inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check txtai defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check txtai least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check txtai resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check txtai release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check txtai dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If txtai identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a txtai change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- txtai documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a txtai architecture and dependency summary with trust boundaries and persistent state.
- Include txtai validation commands or tests that are safe for the stated environment.
...
- txtai identity, version, configuration, dependencies, and deployment assumptions are explicit.
- txtai failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that txtai is secure, supported, compatible, or suitable.
- Using an unpinned latest version of txtai as the basis for migration or production guidance.
