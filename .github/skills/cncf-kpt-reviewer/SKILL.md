---
name: "cncf-kpt-reviewer"
description: "Review kpt using its official documentation and repository in the Provisioning / Automation & Configuration category. Automate Kubernetes Configuration Editing"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate kpt using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- kpt code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving kpt requires evidence-backed assessment.
...
- First-party source: https://github.com/kptdev/kpt
...
- [ ] Trace kpt inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check kpt defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check kpt least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check kpt resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check kpt release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check kpt dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If kpt identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a kpt change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- kpt documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a kpt architecture and dependency summary with trust boundaries and persistent state.
- Include kpt validation commands or tests that are safe for the stated environment.
...
- kpt identity, version, configuration, dependencies, and deployment assumptions are explicit.
- kpt failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that kpt is secure, supported, compatible, or suitable.
- Using an unpinned latest version of kpt as the basis for migration or production guidance.
