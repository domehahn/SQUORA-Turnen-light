---
name: "cncf-rizhiyi-reviewer"
description: "Review Rizhiyi using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Rizhiyi Reviewer

## Purpose

Review and operate Rizhiyi using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Rizhiyi code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Rizhiyi requires evidence-backed assessment.
...
- First-party source: https://www.rizhiyi.com/
...
- [ ] Trace Rizhiyi inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Rizhiyi defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Rizhiyi least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Rizhiyi resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Rizhiyi release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Rizhiyi dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Rizhiyi identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Rizhiyi change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Rizhiyi documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Rizhiyi architecture and dependency summary with trust boundaries and persistent state.
- Include Rizhiyi validation commands or tests that are safe for the stated environment.
...
- Rizhiyi identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Rizhiyi failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Rizhiyi is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Rizhiyi as the basis for migration or production guidance.
