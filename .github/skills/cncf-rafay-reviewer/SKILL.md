...
Review and operate Rafay using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Rafay code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Rafay requires evidence-backed assessment.
...
- [ ] Trace Rafay inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Rafay defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Rafay least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Rafay resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Rafay release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Rafay dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Rafay identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Rafay change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Rafay documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Rafay architecture and dependency summary with trust boundaries and persistent state.
- Include Rafay validation commands or tests that are safe for the stated environment.
...
- Rafay identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Rafay failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Rafay is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Rafay as the basis for migration or production guidance.
