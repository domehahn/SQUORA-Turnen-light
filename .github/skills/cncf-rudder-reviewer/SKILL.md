---
name: "cncf-rudder-reviewer"
description: "Review Rudder using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Rudder Reviewer

## Purpose

Review and operate Rudder using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Rudder code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Rudder requires evidence-backed assessment.
...
- First-party source: https://github.com/normation/rudder
...
- [ ] Trace Rudder inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Rudder defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Rudder least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Rudder resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Rudder release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Rudder dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Rudder identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Rudder change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Rudder documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Rudder architecture and dependency summary with trust boundaries and persistent state.
- Include Rudder validation commands or tests that are safe for the stated environment.
...
- Rudder identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Rudder failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Rudder is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Rudder as the basis for migration or production guidance.
