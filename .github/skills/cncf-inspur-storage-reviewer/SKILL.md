---
name: "cncf-inspur-storage-reviewer"
description: "Review Inspur Storage using its official documentation and repository in the Runtime / Cloud Native Storage category. Cloud + AI, operate new massive data operations."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Inspur Storage Reviewer

## Purpose

Review and operate Inspur Storage using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Inspur Storage code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Inspur Storage requires evidence-backed assessment.
...
- First-party source: https://www.inspur.com/
...
- [ ] Trace Inspur Storage inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Inspur Storage defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Inspur Storage least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Inspur Storage resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Inspur Storage release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Inspur Storage dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Inspur Storage identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Inspur Storage change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Inspur Storage documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Inspur Storage architecture and dependency summary with trust boundaries and persistent state.
- Include Inspur Storage validation commands or tests that are safe for the stated environment.
...
- Inspur Storage identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Inspur Storage failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Inspur Storage is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Inspur Storage as the basis for migration or production guidance.
