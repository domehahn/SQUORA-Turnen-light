...
---
# Cncf Cyso Kcsp Reviewer

## Purpose

Review and operate Cyso (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Cyso (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Cyso (KCSP) requires evidence-backed assessment.
...
- First-party source: https://cyso.cloud/services/managed-kubernetes/
...
- [ ] Trace Cyso (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Cyso (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Cyso (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Cyso (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Cyso (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Cyso (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Cyso (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Cyso (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Cyso (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Cyso (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Cyso (KCSP) validation commands or tests that are safe for the stated environment.
...
- Cyso (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Cyso (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Cyso (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Cyso (KCSP) as the basis for migration or production guidance.
