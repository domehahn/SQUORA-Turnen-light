---
name: "cncf-cubesandbox-reviewer"
description: "Review CubeSandbox using its official documentation and repository in the AI Native Infra / Workload Runtime category. A high-performance sandbox infrastructure for AI Agent execution with 60ms startup and 5MB memory footprint."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Cubesandbox Reviewer

## Purpose

Review and operate CubeSandbox using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CubeSandbox code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CubeSandbox requires evidence-backed assessment.
...
- CNCF classification: AI Native Infra / Workload Runtime
- CNCF project maturity field: not specified
- First-party source: https://github.com/TencentCloud/CubeSandbox
...
- [ ] Trace CubeSandbox inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CubeSandbox defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CubeSandbox least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CubeSandbox resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CubeSandbox release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CubeSandbox dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CubeSandbox identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CubeSandbox change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CubeSandbox documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CubeSandbox architecture and dependency summary with trust boundaries and persistent state.
- Include CubeSandbox validation commands or tests that are safe for the stated environment.
...
- CubeSandbox identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CubeSandbox failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CubeSandbox is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CubeSandbox as the basis for migration or production guidance.
