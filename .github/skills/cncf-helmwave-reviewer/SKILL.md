---
name: "cncf-helmwave-reviewer"
description: "Review Helmwave using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Helmwave Reviewer

## Purpose

Review and operate Helmwave using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Helmwave code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Helmwave requires evidence-backed assessment.
...
- First-party source: https://github.com/helmwave/helmwave
...
- [ ] Trace Helmwave inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Helmwave defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Helmwave least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Helmwave resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Helmwave release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Helmwave dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Helmwave identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Helmwave change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Helmwave documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Helmwave architecture and dependency summary with trust boundaries and persistent state.
- Include Helmwave validation commands or tests that are safe for the stated environment.
...
- Helmwave identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Helmwave failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Helmwave is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Helmwave as the basis for migration or production guidance.
