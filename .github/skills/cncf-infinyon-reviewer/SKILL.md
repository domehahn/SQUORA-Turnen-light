---
name: "cncf-infinyon-reviewer"
description: "Review Infinyon using its official documentation and repository in the Wasm / Embedded Functions category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Infinyon Reviewer

## Purpose

Review and operate Infinyon using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Infinyon code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Infinyon requires evidence-backed assessment.
...
- [ ] Trace Infinyon inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Infinyon defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Infinyon least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Infinyon resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Infinyon release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Infinyon dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Infinyon identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Infinyon change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Infinyon documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Infinyon architecture and dependency summary with trust boundaries and persistent state.
- Include Infinyon validation commands or tests that are safe for the stated environment.
...
- Infinyon identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Infinyon failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Infinyon is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Infinyon as the basis for migration or production guidance.
