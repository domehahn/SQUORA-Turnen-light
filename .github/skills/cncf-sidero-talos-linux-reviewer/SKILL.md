---
name: "cncf-sidero-talos-linux-reviewer"
description: "Review Sidero Talos Linux using its official documentation and repository in the Platform / Certified Kubernetes - Installer category. Talos Linux is Linux designed for Kubernetes - secure, immutable, and minimal."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Sidero Talos Linux Reviewer

## Purpose

Review and operate Sidero Talos Linux using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Sidero Talos Linux code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Sidero Talos Linux requires evidence-backed assessment.
...
- First-party source: https://github.com/siderolabs/talos
...
- [ ] Trace Sidero Talos Linux inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Sidero Talos Linux defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Sidero Talos Linux least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Sidero Talos Linux resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Sidero Talos Linux release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Sidero Talos Linux dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Sidero Talos Linux identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Sidero Talos Linux change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Sidero Talos Linux documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Sidero Talos Linux architecture and dependency summary with trust boundaries and persistent state.
- Include Sidero Talos Linux validation commands or tests that are safe for the stated environment.
...
- Sidero Talos Linux identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Sidero Talos Linux failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Sidero Talos Linux is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Sidero Talos Linux as the basis for migration or production guidance.
