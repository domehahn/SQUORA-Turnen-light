---
name: "cncf-agent-evaluation-reviewer"
description: "Review Agent Evaluation using its official documentation and repository in the AI Agent / Evaluation category. A generative AI-powered framework for testing virtual agents"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Agent Evaluation Reviewer

## Purpose

Review and operate Agent Evaluation using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Agent Evaluation code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Agent Evaluation requires evidence-backed assessment.
...
- CNCF classification: AI Agent / Evaluation
- CNCF project maturity field: not specified
- First-party source: https://github.com/awslabs/agent-evaluation
...
- [ ] Trace Agent Evaluation inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Agent Evaluation defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Agent Evaluation least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Agent Evaluation resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Agent Evaluation release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Agent Evaluation dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Agent Evaluation identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Agent Evaluation change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Agent Evaluation documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Agent Evaluation architecture and dependency summary with trust boundaries and persistent state.
- Include Agent Evaluation validation commands or tests that are safe for the stated environment.
...
- Agent Evaluation identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Agent Evaluation failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Agent Evaluation is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Agent Evaluation as the basis for migration or production guidance.
