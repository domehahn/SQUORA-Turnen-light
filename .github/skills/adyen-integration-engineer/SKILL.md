...
1. Identify the relevant Adyen payment integration artifacts, owners, systems, environments, and review boundary.
2. Compare the available artifacts against expected signals such as Adyen test payment trace, HMAC validation result, eventCode and pspReference deduplication, idempotent retry response, capture or refund reconciliation, Customer Area test webhook.
...
8. Choose Adyen Sessions or Advanced flow deliberately and map paymentMethods, payments, payments/details, modifications, and webhooks to one merchant reference model.
9. Keep company account, merchant account, regional endpoint, live prefix, API credential, client key, and HMAC key boundaries explicit across test and live environments.
...
- Primary artifacts: Adyen Checkout payment request, merchantReference mapping, Adyen idempotency-key, HMAC webhook validator, pspReference state mapping, capture or refund modification.
- Risk themes: duplicate Adyen payment modification, invalid HMAC acceptance, merchantReference collision, out-of-order webhook regression, regional idempotency assumption, test-live HMAC key mix-up.
- Evidence signals: Adyen test payment trace, HMAC validation result, eventCode and pspReference deduplication, idempotent retry response, capture or refund reconciliation, Customer Area test webhook.
...
- [ ] Confirm the review boundary covers the right Adyen payment integration systems, teams, and environments.
- [ ] Inventory and inspect the current Adyen Checkout payment request.
- [ ] Check whether merchantReference mapping is current, approved, versioned, and owned.
- [ ] Verify that Adyen idempotency-key has test, ticket, log, or approval support.
- [ ] Look for duplicate Adyen payment modification and record concrete repository or process evidence.
- [ ] Look for invalid HMAC acceptance and identify affected assets, services, or stakeholders.
- [ ] Look for merchantReference collision and classify the operational or audit impact.
- [ ] Use Adyen test payment trace to validate that the control or practice is operating.
- [ ] Use HMAC validation result to confirm ownership, timing, and reproducibility.
- [ ] Check exception, risk-acceptance, and expiry handling for Adyen payment integration.
...
- If Adyen Checkout payment request is missing for a critical service, raise at least a high-severity readiness gap.
- If HMAC validation result cannot be tied to an owner and approval, treat the outcome as unauditable until corrected.
- If HMAC webhook validator is present but expired or untested, require validation before accepting residual risk.
...
- Missing or stale Adyen payment integration artifact.
...
- Findings ordered by severity with affected Adyen payment integration artifacts and evidence references.
- Coverage note for reviewed artifacts: Adyen Checkout payment request, merchantReference mapping, Adyen idempotency-key, HMAC webhook validator, pspReference state mapping, capture or refund modification.
- Risk note covering relevant themes: duplicate Adyen payment modification, invalid HMAC acceptance, merchantReference collision, out-of-order webhook regression, regional idempotency assumption, test-live HMAC key mix-up.
- Evidence request list using expected signals: Adyen test payment trace, HMAC validation result, eventCode and pspReference deduplication, idempotent retry response, capture or refund reconciliation, Customer Area test webhook.
- Deliverables or updates needed: Adyen integration design, Checkout state mapping, HMAC webhook review, idempotency and retry policy, Adyen test matrix.
...
- Adyen Sessions or Advanced flow map covering action handling, merchantReference and pspReference correlation, HMAC format, modifications, and reconciliation.

## Acceptance Criteria

- Relevant Adyen payment integration artifacts are identified, current, owned, and versioned where applicable.
...
- Adyen actions, AUTHORISATION and modification events, HMAC validation, duplicate and ordering behavior, regional idempotency, and test/live separation have evidence.
