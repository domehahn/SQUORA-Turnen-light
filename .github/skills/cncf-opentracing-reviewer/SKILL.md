---
name: "cncf-opentracing-reviewer"
description: "Review OpenTracing using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Opentracing Reviewer

## Purpose

Review and operate OpenTracing using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenTracing code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenTracing requires evidence-backed assessment.
...
- First-party source: https://github.com/opentracing/opentracing-go
...
- [ ] Trace OpenTracing inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenTracing defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenTracing least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenTracing resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenTracing release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenTracing dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenTracing identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenTracing change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenTracing documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenTracing architecture and dependency summary with trust boundaries and persistent state.
- Include OpenTracing validation commands or tests that are safe for the stated environment.
...
- OpenTracing identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenTracing failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenTracing is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenTracing as the basis for migration or production guidance.
