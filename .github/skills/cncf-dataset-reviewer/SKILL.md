---
name: "cncf-dataset-reviewer"
description: "Review DataSet using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Dataset Reviewer

## Purpose

Review and operate DataSet using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DataSet code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DataSet requires evidence-backed assessment.
...
- First-party source: https://www.dataset.com
...
- [ ] Trace DataSet inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DataSet defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DataSet least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DataSet resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DataSet release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DataSet dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DataSet identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DataSet change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DataSet documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DataSet architecture and dependency summary with trust boundaries and persistent state.
- Include DataSet validation commands or tests that are safe for the stated environment.
...
- DataSet identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DataSet failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DataSet is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DataSet as the basis for migration or production guidance.
