---
name: "cncf-infoscale-for-kubernetes-reviewer"
...
Review and operate InfoScale for Kubernetes using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- InfoScale for Kubernetes code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving InfoScale for Kubernetes requires evidence-backed assessment.
...
- First-party source: https://www.infoscale.com
...
- [ ] Trace InfoScale for Kubernetes inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check InfoScale for Kubernetes defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check InfoScale for Kubernetes least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check InfoScale for Kubernetes resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check InfoScale for Kubernetes release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check InfoScale for Kubernetes dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If InfoScale for Kubernetes identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a InfoScale for Kubernetes change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- InfoScale for Kubernetes documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a InfoScale for Kubernetes architecture and dependency summary with trust boundaries and persistent state.
- Include InfoScale for Kubernetes validation commands or tests that are safe for the stated environment.
...
- InfoScale for Kubernetes identity, version, configuration, dependencies, and deployment assumptions are explicit.
- InfoScale for Kubernetes failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that InfoScale for Kubernetes is secure, supported, compatible, or suitable.
- Using an unpinned latest version of InfoScale for Kubernetes as the basis for migration or production guidance.
