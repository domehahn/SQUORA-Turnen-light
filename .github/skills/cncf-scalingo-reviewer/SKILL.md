---
name: "cncf-scalingo-reviewer"
description: "Review Scalingo using its official documentation and repository in the Platform / PaaS/Container Service category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Scalingo Reviewer

## Purpose

Review and operate Scalingo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Scalingo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Scalingo requires evidence-backed assessment.
...
- First-party source: https://scalingo.com/
...
- [ ] Trace Scalingo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Scalingo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Scalingo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Scalingo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Scalingo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Scalingo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Scalingo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Scalingo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Scalingo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Scalingo architecture and dependency summary with trust boundaries and persistent state.
- Include Scalingo validation commands or tests that are safe for the stated environment.
...
- Scalingo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Scalingo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Scalingo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Scalingo as the basis for migration or production guidance.
