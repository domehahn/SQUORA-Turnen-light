---
name: "cncf-sonobuoy-reviewer"
description: "Review Sonobuoy using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Sonobuoy Reviewer

## Purpose

Review and operate Sonobuoy using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Sonobuoy code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Sonobuoy requires evidence-backed assessment.
...
- First-party source: https://github.com/vmware-tanzu/sonobuoy
...
- [ ] Trace Sonobuoy inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Sonobuoy defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Sonobuoy least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Sonobuoy resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Sonobuoy release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Sonobuoy dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Sonobuoy identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Sonobuoy change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Sonobuoy documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Sonobuoy architecture and dependency summary with trust boundaries and persistent state.
- Include Sonobuoy validation commands or tests that are safe for the stated environment.
...
- Sonobuoy identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Sonobuoy failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Sonobuoy is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Sonobuoy as the basis for migration or production guidance.
