---
name: "cncf-jenkinsx-reviewer"
description: "Review JenkinsX using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Jenkinsx Reviewer

## Purpose

Review and operate JenkinsX using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- JenkinsX code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving JenkinsX requires evidence-backed assessment.
...
- First-party source: https://github.com/jenkins-x/jx
...
- [ ] Trace JenkinsX inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check JenkinsX defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check JenkinsX least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check JenkinsX resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check JenkinsX release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check JenkinsX dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If JenkinsX identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a JenkinsX change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- JenkinsX documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a JenkinsX architecture and dependency summary with trust boundaries and persistent state.
- Include JenkinsX validation commands or tests that are safe for the stated environment.
...
- JenkinsX identity, version, configuration, dependencies, and deployment assumptions are explicit.
- JenkinsX failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that JenkinsX is secure, supported, compatible, or suitable.
- Using an unpinned latest version of JenkinsX as the basis for migration or production guidance.
