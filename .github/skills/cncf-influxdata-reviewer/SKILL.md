---
name: "cncf-influxdata-reviewer"
description: "Review InfluxData using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Influxdata Reviewer

## Purpose

Review and operate InfluxData using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- InfluxData code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving InfluxData requires evidence-backed assessment.
...
- [ ] Trace InfluxData inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check InfluxData defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check InfluxData least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check InfluxData resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check InfluxData release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check InfluxData dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If InfluxData identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a InfluxData change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- InfluxData documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a InfluxData architecture and dependency summary with trust boundaries and persistent state.
- Include InfluxData validation commands or tests that are safe for the stated environment.
...
- InfluxData identity, version, configuration, dependencies, and deployment assumptions are explicit.
- InfluxData failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that InfluxData is secure, supported, compatible, or suitable.
- Using an unpinned latest version of InfluxData as the basis for migration or production guidance.
