---
name: "cncf-fluid-reviewer"
description: "Review Fluid using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. Fluid is an orchestration platform for elastic data abstraction and acceleration in cloud native environment."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Fluid using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Fluid code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Fluid requires evidence-backed assessment.
...
- First-party source: https://github.com/fluid-cloudnative/fluid
...
- [ ] Trace Fluid inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Fluid defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Fluid least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Fluid resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Fluid release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Fluid dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Fluid identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Fluid change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Fluid documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Fluid architecture and dependency summary with trust boundaries and persistent state.
- Include Fluid validation commands or tests that are safe for the stated environment.
...
- Fluid identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Fluid failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Fluid is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Fluid as the basis for migration or production guidance.
