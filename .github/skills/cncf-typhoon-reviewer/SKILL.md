...
---
# Cncf Typhoon Reviewer

## Purpose

Review and operate Typhoon using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Typhoon code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Typhoon requires evidence-backed assessment.
...
- First-party source: https://github.com/poseidon/typhoon
...
- [ ] Trace Typhoon inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Typhoon defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Typhoon least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Typhoon resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Typhoon release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Typhoon dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Typhoon identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Typhoon change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Typhoon documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Typhoon architecture and dependency summary with trust boundaries and persistent state.
- Include Typhoon validation commands or tests that are safe for the stated environment.
...
- Typhoon identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Typhoon failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Typhoon is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Typhoon as the basis for migration or production guidance.
