---
name: "cncf-ps-cloud-services-kcsp-reviewer"
description: "Review PS Cloud Services (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. We design and implement fault-tolerant workstations based on Kubernetes. Scaling, management, maintenance and support 24/7."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Ps Cloud Services Kcsp Reviewer

## Purpose

Review and operate PS Cloud Services (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- PS Cloud Services (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving PS Cloud Services (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.ps.kz/support/linux#
...
- [ ] Trace PS Cloud Services (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check PS Cloud Services (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check PS Cloud Services (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check PS Cloud Services (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check PS Cloud Services (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check PS Cloud Services (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If PS Cloud Services (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a PS Cloud Services (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- PS Cloud Services (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a PS Cloud Services (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include PS Cloud Services (KCSP) validation commands or tests that are safe for the stated environment.
...
- PS Cloud Services (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- PS Cloud Services (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that PS Cloud Services (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of PS Cloud Services (KCSP) as the basis for migration or production guidance.
