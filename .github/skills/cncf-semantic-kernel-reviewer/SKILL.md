---
name: "cncf-semantic-kernel-reviewer"
description: "Review Semantic Kernel using its official documentation and repository in the AI Agent / Agent Framework category. Integrate cutting-edge LLM technology quickly and easily into your apps."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Semantic Kernel Reviewer

## Purpose

Review and operate Semantic Kernel using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Semantic Kernel code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Semantic Kernel requires evidence-backed assessment.
...
- First-party source: https://github.com/microsoft/semantic-kernel
...
- [ ] Trace Semantic Kernel inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Semantic Kernel defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Semantic Kernel least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Semantic Kernel resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Semantic Kernel release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Semantic Kernel dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Semantic Kernel identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Semantic Kernel change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Semantic Kernel documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Semantic Kernel architecture and dependency summary with trust boundaries and persistent state.
- Include Semantic Kernel validation commands or tests that are safe for the stated environment.
...
- Semantic Kernel identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Semantic Kernel failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Semantic Kernel is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Semantic Kernel as the basis for migration or production guidance.
