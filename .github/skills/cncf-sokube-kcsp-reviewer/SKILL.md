---
name: "cncf-sokube-kcsp-reviewer"
description: "Review SoKube (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. SoKube is a Swiss based company that helps companies making their digital transition to the world of containers and Kubernetes orchestration, with a strong focus on Agility and DevSecOps lifecycle."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Sokube Kcsp Reviewer

## Purpose

Review and operate SoKube (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SoKube (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SoKube (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.sokube.io/en/home
...
- [ ] Trace SoKube (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SoKube (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SoKube (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SoKube (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SoKube (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SoKube (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SoKube (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SoKube (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SoKube (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SoKube (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include SoKube (KCSP) validation commands or tests that are safe for the stated environment.
...
- SoKube (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SoKube (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SoKube (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SoKube (KCSP) as the basis for migration or production guidance.
