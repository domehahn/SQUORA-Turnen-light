---
name: "cncf-devcycle-reviewer"
description: "Review DevCycle using its official documentation and repository in the Observability and Analysis / Feature Flagging category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Devcycle Reviewer

## Purpose

Review and operate DevCycle using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DevCycle code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DevCycle requires evidence-backed assessment.
...
- First-party source: https://devcycle.com
...
- [ ] Trace DevCycle inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DevCycle defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DevCycle least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DevCycle resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DevCycle release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DevCycle dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DevCycle identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DevCycle change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DevCycle documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DevCycle architecture and dependency summary with trust boundaries and persistent state.
- Include DevCycle validation commands or tests that are safe for the stated environment.
...
- DevCycle identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DevCycle failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DevCycle is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DevCycle as the basis for migration or production guidance.
