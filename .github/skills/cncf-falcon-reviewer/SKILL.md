---
name: "cncf-falcon-reviewer"
description: "Review Falcon using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Falcon Reviewer

## Purpose

Review and operate Falcon using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Falcon code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Falcon requires evidence-backed assessment.
...
- First-party source: https://github.com/open-falcon/falcon-plus
...
- [ ] Trace Falcon inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Falcon defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Falcon least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Falcon resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Falcon release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Falcon dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Falcon identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Falcon change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Falcon documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Falcon architecture and dependency summary with trust boundaries and persistent state.
- Include Falcon validation commands or tests that are safe for the stated environment.
...
- Falcon identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Falcon failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Falcon is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Falcon as the basis for migration or production guidance.
