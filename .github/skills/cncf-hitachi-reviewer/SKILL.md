---
name: "cncf-hitachi-reviewer"
description: "Review Hitachi using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Hitachi Reviewer

## Purpose

Review and operate Hitachi using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Hitachi code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Hitachi requires evidence-backed assessment.
...
- [ ] Trace Hitachi inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Hitachi defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Hitachi least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Hitachi resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Hitachi release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Hitachi dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Hitachi identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Hitachi change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Hitachi documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Hitachi architecture and dependency summary with trust boundaries and persistent state.
- Include Hitachi validation commands or tests that are safe for the stated environment.
...
- Hitachi identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Hitachi failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Hitachi is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Hitachi as the basis for migration or production guidance.
