---
name: "cncf-opentelemetry-reviewer"
description: "Review OpenTelemetry using its official documentation and repository in the Observability and Analysis / Observability category. High-quality, ubiquitous, and portable telemetry to enable effective observability"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Opentelemetry Reviewer

## Purpose

Review and operate OpenTelemetry using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenTelemetry code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenTelemetry requires evidence-backed assessment.
...
- First-party source: https://github.com/open-telemetry/community
...
- [ ] Trace OpenTelemetry inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenTelemetry defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenTelemetry least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenTelemetry resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenTelemetry release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenTelemetry dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenTelemetry identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenTelemetry change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenTelemetry documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenTelemetry architecture and dependency summary with trust boundaries and persistent state.
- Include OpenTelemetry validation commands or tests that are safe for the stated environment.
...
- OpenTelemetry identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenTelemetry failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenTelemetry is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenTelemetry as the basis for migration or production guidance.
