---
name: "cncf-flagger-reviewer"
description: "Review Flagger using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Flagger Reviewer

## Purpose

Review and operate Flagger using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Flagger code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Flagger requires evidence-backed assessment.
...
- First-party source: https://github.com/fluxcd/flagger
...
- [ ] Trace Flagger inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Flagger defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Flagger least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Flagger resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Flagger release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Flagger dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Flagger identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Flagger change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Flagger documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Flagger architecture and dependency summary with trust boundaries and persistent state.
- Include Flagger validation commands or tests that are safe for the stated environment.
...
- Flagger identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Flagger failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Flagger is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Flagger as the basis for migration or production guidance.
