---
name: "cncf-fongcon-kcsp-reviewer"
description: "Review Fongcon (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. As an NVIDIA Elite Partner, we deliver expert Kubernetes consulting, implementation, and training services specifically optimized for GPU-accelerated platforms to ensure reliable AI workload orchestration. We empower enterprises to bridge the gap between high-performance hardware and cloud-native ecosystems through professional support and tailored operational excellence."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Fongcon Kcsp Reviewer

## Purpose

Review and operate Fongcon (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Fongcon (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Fongcon (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.fongcon.com.tw/en/website/onepage
...
- [ ] Trace Fongcon (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Fongcon (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Fongcon (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Fongcon (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Fongcon (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Fongcon (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Fongcon (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Fongcon (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Fongcon (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Fongcon (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Fongcon (KCSP) validation commands or tests that are safe for the stated environment.
...
- Fongcon (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Fongcon (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Fongcon (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Fongcon (KCSP) as the basis for migration or production guidance.
