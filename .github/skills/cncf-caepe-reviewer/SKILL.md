---
name: "cncf-caepe-reviewer"
description: "Review CAEPE using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category. CAEPE is a Continuous Deployment platform for Kubernetes. With CAEPE, deploy applications on Kubernetes with confidence."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate CAEPE using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CAEPE code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CAEPE requires evidence-backed assessment.
...
- First-party source: https://caepe.sh/
...
- [ ] Trace CAEPE inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CAEPE defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CAEPE least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CAEPE resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CAEPE release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CAEPE dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CAEPE identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CAEPE change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CAEPE documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CAEPE architecture and dependency summary with trust boundaries and persistent state.
- Include CAEPE validation commands or tests that are safe for the stated environment.
...
- CAEPE identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CAEPE failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CAEPE is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CAEPE as the basis for migration or production guidance.
