...
---
# Cncf Hunter Strategy Kcsp Reviewer

## Purpose

Review and operate Hunter Strategy (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Hunter Strategy (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Hunter Strategy (KCSP) requires evidence-backed assessment.
...
- First-party source: https://hunterstrategy.net/commercial-solutions/kubernetes
...
- [ ] Trace Hunter Strategy (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Hunter Strategy (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Hunter Strategy (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Hunter Strategy (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Hunter Strategy (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Hunter Strategy (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Hunter Strategy (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Hunter Strategy (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Hunter Strategy (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Hunter Strategy (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Hunter Strategy (KCSP) validation commands or tests that are safe for the stated environment.
...
- Hunter Strategy (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Hunter Strategy (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Hunter Strategy (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Hunter Strategy (KCSP) as the basis for migration or production guidance.
