---
name: "cncf-spring-cloud-function-reviewer"
description: "Review Spring Cloud Function using its official documentation and repository in the Serverless / Framework category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Spring Cloud Function Reviewer

## Purpose

Review and operate Spring Cloud Function using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Spring Cloud Function code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Spring Cloud Function requires evidence-backed assessment.
...
- First-party source: https://github.com/spring-cloud/spring-cloud-function
...
- [ ] Trace Spring Cloud Function inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Spring Cloud Function defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Spring Cloud Function least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Spring Cloud Function resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Spring Cloud Function release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Spring Cloud Function dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Spring Cloud Function identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Spring Cloud Function change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Spring Cloud Function documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Spring Cloud Function architecture and dependency summary with trust boundaries and persistent state.
- Include Spring Cloud Function validation commands or tests that are safe for the stated environment.
...
- Spring Cloud Function identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Spring Cloud Function failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Spring Cloud Function is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Spring Cloud Function as the basis for migration or production guidance.
