---
name: "cncf-keep-reviewer"
description: "Review Keep using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Keep using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Keep code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Keep requires evidence-backed assessment.
...
- First-party source: https://github.com/keephq/keep
...
- [ ] Trace Keep inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Keep defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Keep least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Keep resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Keep release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Keep dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Keep identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Keep change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Keep documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Keep architecture and dependency summary with trust boundaries and persistent state.
- Include Keep validation commands or tests that are safe for the stated environment.
...
- Keep identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Keep failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Keep is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Keep as the basis for migration or production guidance.
