---
name: "cncf-sedai-reviewer"
description: "Review Sedai using its official documentation and repository in the Observability and Analysis / Continuous Optimization category. Sedai is an autonomous cloud management platform to optimize cost & performance and remediate issues for Kubernetes & Serverless"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Sedai using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Sedai code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Sedai requires evidence-backed assessment.
...
- First-party source: https://sedai.io/
...
- [ ] Trace Sedai inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Sedai defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Sedai least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Sedai resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Sedai release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Sedai dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Sedai identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Sedai change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Sedai documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Sedai architecture and dependency summary with trust boundaries and persistent state.
- Include Sedai validation commands or tests that are safe for the stated environment.
...
- Sedai identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Sedai failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Sedai is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Sedai as the basis for migration or production guidance.
