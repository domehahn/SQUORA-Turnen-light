...
---
# Cncf Tongtech Kcsp Reviewer

## Purpose

Review and operate TongTech (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- TongTech (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving TongTech (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.tongtech.com/kubernetes.html
...
- [ ] Trace TongTech (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check TongTech (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check TongTech (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check TongTech (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check TongTech (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check TongTech (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If TongTech (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a TongTech (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- TongTech (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a TongTech (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include TongTech (KCSP) validation commands or tests that are safe for the stated environment.
...
- TongTech (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- TongTech (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that TongTech (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of TongTech (KCSP) as the basis for migration or production guidance.
