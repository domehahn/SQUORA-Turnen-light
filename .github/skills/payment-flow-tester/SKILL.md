...
description: "Test authorization, capture, settlement, cancellation, refund, timeout, decline, authentication, webhook delay, and provider outage paths."
version: "0.1.0"
since: "2026-08-27"
...
Design and review payment tests across authorization, capture, settlement, cancellation, refund, timeout, decline, authentication challenge, webhook delay, and provider outage paths. Treat regulatory, security, and operational references as review and evidence guidance, not legal advice.
...
- payment flow testing decisions, controls, or operating practices need independent review.
- A change affects payment flow testing artifacts such as payment scenario matrix, sandbox fixture, provider test clock or simulator, failure-injection test, webhook fixture, ledger assertion.
- The user needs evidence-oriented findings for risks such as happy-path-only coverage, untested ambiguous timeout, missing authentication challenge, double-submit regression, sandbox-production mismatch, incorrect ledger side effect.
- Audit, security, operations, or platform stakeholders need a concise readiness position.
- Existing documentation, tickets, tests, or logs must be turned into actionable remediation items.

## Operating model

1. Identify the relevant payment flow testing artifacts, owners, systems, environments, and review boundary.
2. Compare the available artifacts against expected signals such as authorization and capture trace, decline-code assertion, delayed-webhook test, idempotent retry result, refund test, state-machine coverage report.
...
- Primary artifacts: payment scenario matrix, sandbox fixture, provider test clock or simulator, failure-injection test, webhook fixture, ledger assertion.
- Risk themes: happy-path-only coverage, untested ambiguous timeout, missing authentication challenge, double-submit regression, sandbox-production mismatch, incorrect ledger side effect.
- Evidence signals: authorization and capture trace, decline-code assertion, delayed-webhook test, idempotent retry result, refund test, state-machine coverage report.
...
- [ ] Confirm the review boundary covers the right payment flow testing systems, teams, and environments.
- [ ] Inventory and inspect the current payment scenario matrix.
- [ ] Check whether sandbox fixture is current, approved, versioned, and owned.
- [ ] Verify that provider test clock or simulator has test, ticket, log, or approval support.
- [ ] Look for happy-path-only coverage and record concrete repository or process evidence.
- [ ] Look for untested ambiguous timeout and identify affected assets, services, or stakeholders.
- [ ] Look for missing authentication challenge and classify the operational or audit impact.
- [ ] Use authorization and capture trace to validate that the control or practice is operating.
- [ ] Use decline-code assertion to confirm ownership, timing, and reproducibility.
- [ ] Check exception, risk-acceptance, and expiry handling for payment flow testing.
...
- If payment scenario matrix is missing for a critical service, raise at least a high-severity readiness gap.
- If decline-code assertion cannot be tied to an owner and approval, treat the outcome as unauditable until corrected.
- If failure-injection test is present but expired or untested, require validation before accepting residual risk.
...
- Missing or stale payment flow testing artifact.
...
- Findings ordered by severity with affected payment flow testing artifacts and evidence references.
- Coverage note for reviewed artifacts: payment scenario matrix, sandbox fixture, provider test clock or simulator, failure-injection test, webhook fixture, ledger assertion.
- Risk note covering relevant themes: happy-path-only coverage, untested ambiguous timeout, missing authentication challenge, double-submit regression, sandbox-production mismatch, incorrect ledger side effect.
- Evidence request list using expected signals: authorization and capture trace, decline-code assertion, delayed-webhook test, idempotent retry result, refund test, state-machine coverage report.
- Deliverables or updates needed: payment test strategy, provider sandbox scenario suite, negative-path coverage matrix, state-transition coverage report, production smoke-test plan.
...
- Relevant payment flow testing artifacts are identified, current, owned, and versioned where applicable.
