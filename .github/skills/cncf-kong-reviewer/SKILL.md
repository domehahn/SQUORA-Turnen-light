---
name: "cncf-kong-reviewer"
description: "Review Kong using its official documentation and repository in the Orchestration & Management / API Gateway category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Kong using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kong code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kong requires evidence-backed assessment.
...
- First-party source: https://github.com/Kong/kong
...
- [ ] Trace Kong inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kong defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kong least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kong resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kong release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kong dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kong identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kong change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kong documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kong architecture and dependency summary with trust boundaries and persistent state.
- Include Kong validation commands or tests that are safe for the stated environment.
...
- Kong identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kong failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kong is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kong as the basis for migration or production guidance.
