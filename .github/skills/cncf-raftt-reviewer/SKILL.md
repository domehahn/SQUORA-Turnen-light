---
name: "cncf-raftt-reviewer"
description: "Review Raftt using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Raftt using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Raftt code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Raftt requires evidence-backed assessment.
...
- First-party source: https://www.raftt.io/
...
- [ ] Trace Raftt inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Raftt defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Raftt least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Raftt resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Raftt release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Raftt dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Raftt identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Raftt change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Raftt documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Raftt architecture and dependency summary with trust boundaries and persistent state.
- Include Raftt validation commands or tests that are safe for the stated environment.
...
- Raftt identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Raftt failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Raftt is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Raftt as the basis for migration or production guidance.
