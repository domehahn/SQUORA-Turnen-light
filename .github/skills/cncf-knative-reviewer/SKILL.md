...
---
# Cncf Knative Reviewer

## Purpose

Review and operate Knative using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Knative code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Knative requires evidence-backed assessment.
...
- First-party source: https://github.com/knative/serving
...
- [ ] Trace Knative inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Knative defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Knative least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Knative resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Knative release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Knative dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Knative identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Knative change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Knative documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Knative architecture and dependency summary with trust boundaries and persistent state.
- Include Knative validation commands or tests that are safe for the stated environment.
...
- Knative identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Knative failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Knative is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Knative as the basis for migration or production guidance.
