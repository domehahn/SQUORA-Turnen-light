---
name: "cncf-modsurfer-reviewer"
description: "Review Modsurfer using its official documentation and repository in the Wasm / Debugging & Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Modsurfer Reviewer

## Purpose

Review and operate Modsurfer using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Modsurfer code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Modsurfer requires evidence-backed assessment.
...
- First-party source: https://github.com/dylibso/modsurfer
...
- [ ] Trace Modsurfer inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Modsurfer defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Modsurfer least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Modsurfer resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Modsurfer release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Modsurfer dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Modsurfer identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Modsurfer change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Modsurfer documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Modsurfer architecture and dependency summary with trust boundaries and persistent state.
- Include Modsurfer validation commands or tests that are safe for the stated environment.
...
- Modsurfer identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Modsurfer failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Modsurfer is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Modsurfer as the basis for migration or production guidance.
