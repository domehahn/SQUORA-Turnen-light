---
name: "cncf-centreon-reviewer"
description: "Review Centreon using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Centreon Reviewer

## Purpose

Review and operate Centreon using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Centreon code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Centreon requires evidence-backed assessment.
...
- First-party source: https://github.com/centreon/centreon
...
- [ ] Trace Centreon inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Centreon defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Centreon least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Centreon resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Centreon release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Centreon dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Centreon identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Centreon change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Centreon documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Centreon architecture and dependency summary with trust boundaries and persistent state.
- Include Centreon validation commands or tests that are safe for the stated environment.
...
- Centreon identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Centreon failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Centreon is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Centreon as the basis for migration or production guidance.
