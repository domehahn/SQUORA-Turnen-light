...
---
# Cncf Tata Communications Vayu Kubernetes Service Reviewer

## Purpose

Review and operate Tata Communications Vayu Kubernetes Service using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Tata Communications Vayu Kubernetes Service code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Tata Communications Vayu Kubernetes Service requires evidence-backed assessment.
...
- First-party source: https://www.tatacommunications.com/solutions/cloud/vayu-cloud-platform-services/vayu-cloud-platform-for-kubernetes-solutions/
...
- [ ] Trace Tata Communications Vayu Kubernetes Service inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Tata Communications Vayu Kubernetes Service defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Tata Communications Vayu Kubernetes Service least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Tata Communications Vayu Kubernetes Service resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Tata Communications Vayu Kubernetes Service release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Tata Communications Vayu Kubernetes Service dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Tata Communications Vayu Kubernetes Service identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Tata Communications Vayu Kubernetes Service change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Tata Communications Vayu Kubernetes Service documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Tata Communications Vayu Kubernetes Service architecture and dependency summary with trust boundaries and persistent state.
- Include Tata Communications Vayu Kubernetes Service validation commands or tests that are safe for the stated environment.
...
- Tata Communications Vayu Kubernetes Service identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Tata Communications Vayu Kubernetes Service failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Tata Communications Vayu Kubernetes Service is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Tata Communications Vayu Kubernetes Service as the basis for migration or production guidance.
