---
name: "cncf-golang-reviewer"
description: "Review Golang using its official documentation and repository in the Wasm / Languages category. Compiled language to Wasm"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Golang Reviewer

## Purpose

Review and operate Golang using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Golang code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Golang requires evidence-backed assessment.
...
- First-party source: https://github.com/golang/go
...
- [ ] Trace Golang inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Golang defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Golang least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Golang resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Golang release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Golang dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Golang identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Golang change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Golang documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Golang architecture and dependency summary with trust boundaries and persistent state.
- Include Golang validation commands or tests that are safe for the stated environment.
...
- Golang identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Golang failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Golang is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Golang as the basis for migration or production guidance.
