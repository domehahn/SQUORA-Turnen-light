...
---
# Cncf Vmware Vsphere Kubernetes Service Reviewer

## Purpose

Review and operate VMware vSphere Kubernetes Service using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- VMware vSphere Kubernetes Service code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving VMware vSphere Kubernetes Service requires evidence-backed assessment.
...
- First-party source: https://www.vmware.com/products/cloud-foundation.html
...
- [ ] Trace VMware vSphere Kubernetes Service inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check VMware vSphere Kubernetes Service defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check VMware vSphere Kubernetes Service least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check VMware vSphere Kubernetes Service resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check VMware vSphere Kubernetes Service release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check VMware vSphere Kubernetes Service dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If VMware vSphere Kubernetes Service identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a VMware vSphere Kubernetes Service change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- VMware vSphere Kubernetes Service documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a VMware vSphere Kubernetes Service architecture and dependency summary with trust boundaries and persistent state.
- Include VMware vSphere Kubernetes Service validation commands or tests that are safe for the stated environment.
...
- VMware vSphere Kubernetes Service identity, version, configuration, dependencies, and deployment assumptions are explicit.
- VMware vSphere Kubernetes Service failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that VMware vSphere Kubernetes Service is secure, supported, compatible, or suitable.
- Using an unpinned latest version of VMware vSphere Kubernetes Service as the basis for migration or production guidance.
