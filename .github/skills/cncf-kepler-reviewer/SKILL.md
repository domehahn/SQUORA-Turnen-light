---
name: "cncf-kepler-reviewer"
description: "Review Kepler using its official documentation and repository in the Observability and Analysis / Observability category. Kepler (Kubernetes-based Efficient Power Level Exporter) uses eBPF to probe energy related system stats and exports as Prometheus metrics."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kepler Reviewer

## Purpose

Review and operate Kepler using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kepler code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kepler requires evidence-backed assessment.
...
- First-party source: https://github.com/sustainable-computing-io/kepler
...
- [ ] Trace Kepler inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kepler defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kepler least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kepler resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kepler release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kepler dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kepler identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kepler change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kepler documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kepler architecture and dependency summary with trust boundaries and persistent state.
- Include Kepler validation commands or tests that are safe for the stated environment.
...
- Kepler identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kepler failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kepler is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kepler as the basis for migration or production guidance.
