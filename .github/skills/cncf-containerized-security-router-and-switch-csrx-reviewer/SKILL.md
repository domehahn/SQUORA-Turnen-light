...
---
# Cncf Containerized Security Router And Switch Csrx Reviewer

## Purpose

Review and operate Containerized Security Router and Switch (cSRX) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Containerized Security Router and Switch (cSRX) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Containerized Security Router and Switch (cSRX) requires evidence-backed assessment.
...
- CNCF classification: Special / Certified CNFs
- CNCF project maturity field: not specified
- First-party source: https://www.juniper.net/us/en/products/security/srx-series/csrx-containerized-firewall.html
...
- [ ] Trace Containerized Security Router and Switch (cSRX) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Containerized Security Router and Switch (cSRX) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Containerized Security Router and Switch (cSRX) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Containerized Security Router and Switch (cSRX) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Containerized Security Router and Switch (cSRX) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Containerized Security Router and Switch (cSRX) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Containerized Security Router and Switch (cSRX) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Containerized Security Router and Switch (cSRX) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Containerized Security Router and Switch (cSRX) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Containerized Security Router and Switch (cSRX) architecture and dependency summary with trust boundaries and persistent state.
- Include Containerized Security Router and Switch (cSRX) validation commands or tests that are safe for the stated environment.
...
- Containerized Security Router and Switch (cSRX) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Containerized Security Router and Switch (cSRX) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Containerized Security Router and Switch (cSRX) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Containerized Security Router and Switch (cSRX) as the basis for migration or production guidance.
