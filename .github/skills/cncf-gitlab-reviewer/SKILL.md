---
name: "cncf-gitlab-reviewer"
description: "Review GitLab using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Gitlab Reviewer

## Purpose

Review and operate GitLab using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- GitLab code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving GitLab requires evidence-backed assessment.
...
- First-party source: https://github.com/gitlabhq/gitlabhq
...
- [ ] Trace GitLab inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check GitLab defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check GitLab least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check GitLab resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check GitLab release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check GitLab dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If GitLab identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a GitLab change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- GitLab documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a GitLab architecture and dependency summary with trust boundaries and persistent state.
- Include GitLab validation commands or tests that are safe for the stated environment.
...
- GitLab identity, version, configuration, dependencies, and deployment assumptions are explicit.
- GitLab failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that GitLab is secure, supported, compatible, or suitable.
- Using an unpinned latest version of GitLab as the basis for migration or production guidance.
