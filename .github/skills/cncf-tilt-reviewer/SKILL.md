---
name: "cncf-tilt-reviewer"
description: "Review Tilt using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Tilt using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Tilt code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Tilt requires evidence-backed assessment.
...
- First-party source: https://github.com/tilt-dev/tilt
...
- [ ] Trace Tilt inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Tilt defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Tilt least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Tilt resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Tilt release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Tilt dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Tilt identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Tilt change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Tilt documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Tilt architecture and dependency summary with trust boundaries and persistent state.
- Include Tilt validation commands or tests that are safe for the stated environment.
...
- Tilt identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Tilt failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Tilt is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Tilt as the basis for migration or production guidance.
