---
name: "cncf-syft-reviewer"
description: "Review Syft using its official documentation and repository in the Provisioning / Security & Compliance category. Syft is a CLI tool and library for generating a Software Bill of Materials from container images and filesystems"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Syft using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Syft code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Syft requires evidence-backed assessment.
...
- First-party source: https://github.com/anchore/syft
...
- [ ] Trace Syft inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Syft defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Syft least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Syft resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Syft release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Syft dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Syft identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Syft change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Syft documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Syft architecture and dependency summary with trust boundaries and persistent state.
- Include Syft validation commands or tests that are safe for the stated environment.
...
- Syft identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Syft failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Syft is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Syft as the basis for migration or production guidance.
