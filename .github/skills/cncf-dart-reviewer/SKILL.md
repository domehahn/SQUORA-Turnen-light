---
name: "cncf-dart-reviewer"
description: "Review Dart using its official documentation and repository in the Wasm / Languages category. Managed language"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Dart using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Dart code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Dart requires evidence-backed assessment.
...
- First-party source: https://github.com/dart-lang/sdk
...
- [ ] Trace Dart inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Dart defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Dart least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Dart resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Dart release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Dart dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Dart identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Dart change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Dart documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Dart architecture and dependency summary with trust boundaries and persistent state.
- Include Dart validation commands or tests that are safe for the stated environment.
...
- Dart identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Dart failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Dart is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Dart as the basis for migration or production guidance.
