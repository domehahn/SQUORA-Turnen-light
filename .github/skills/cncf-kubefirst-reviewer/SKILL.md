---
name: "cncf-kubefirst-reviewer"
description: "Review Kubefirst using its official documentation and repository in the Provisioning / Automation & Configuration category. Instant operational open source gitops platforms for platform engineering, infrastructure, and software teams"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kubefirst Reviewer

## Purpose

Review and operate Kubefirst using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kubefirst code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kubefirst requires evidence-backed assessment.
...
- First-party source: https://github.com/kubefirst/kubefirst
...
- [ ] Trace Kubefirst inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kubefirst defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kubefirst least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kubefirst resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kubefirst release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kubefirst dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kubefirst identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kubefirst change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kubefirst documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kubefirst architecture and dependency summary with trust boundaries and persistent state.
- Include Kubefirst validation commands or tests that are safe for the stated environment.
...
- Kubefirst identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kubefirst failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kubefirst is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kubefirst as the basis for migration or production guidance.
