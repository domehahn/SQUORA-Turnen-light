---
name: "cncf-openelb-reviewer"
description: "Review OpenELB using its official documentation and repository in the Orchestration & Management / Service Proxy category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Openelb Reviewer

## Purpose

Review and operate OpenELB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenELB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenELB requires evidence-backed assessment.
...
- First-party source: https://github.com/openelb/openelb
...
- [ ] Trace OpenELB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenELB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenELB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenELB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenELB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenELB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenELB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenELB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenELB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenELB architecture and dependency summary with trust boundaries and persistent state.
- Include OpenELB validation commands or tests that are safe for the stated environment.
...
- OpenELB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenELB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenELB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenELB as the basis for migration or production guidance.
