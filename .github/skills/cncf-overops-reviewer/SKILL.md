---
name: "cncf-overops-reviewer"
description: "Review OverOps using its official documentation and repository in the Observability and Analysis / Observability category. Prevent rapid code changes from impacting customers."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Overops Reviewer

## Purpose

Review and operate OverOps using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OverOps code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OverOps requires evidence-backed assessment.
...
- First-party source: https://www.overops.com/
...
- [ ] Trace OverOps inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OverOps defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OverOps least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OverOps resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OverOps release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OverOps dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OverOps identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OverOps change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OverOps documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OverOps architecture and dependency summary with trust boundaries and persistent state.
- Include OverOps validation commands or tests that are safe for the stated environment.
...
- OverOps identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OverOps failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OverOps is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OverOps as the basis for migration or production guidance.
