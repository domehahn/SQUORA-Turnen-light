---
name: "cncf-afi-ai-reviewer"
description: "Review Afi.ai using its official documentation and repository in the Runtime / Cloud Native Storage category. Afi SaaS backup for Kubernetes provides a fully managed cloud service with secure offsite storage and an easy-to-use web UI"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Afi Ai Reviewer

## Purpose

Review and operate Afi.ai using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Afi.ai code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Afi.ai requires evidence-backed assessment.
...
- First-party source: https://afi.ai/kubernetes-backup
...
- [ ] Trace Afi.ai inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Afi.ai defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Afi.ai least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Afi.ai resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Afi.ai release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Afi.ai dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Afi.ai identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Afi.ai change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Afi.ai documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Afi.ai architecture and dependency summary with trust boundaries and persistent state.
- Include Afi.ai validation commands or tests that are safe for the stated environment.
...
- Afi.ai identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Afi.ai failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Afi.ai is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Afi.ai as the basis for migration or production guidance.
