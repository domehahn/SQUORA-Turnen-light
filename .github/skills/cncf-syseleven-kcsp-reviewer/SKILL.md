---
name: "cncf-syseleven-kcsp-reviewer"
description: "Review SysEleven (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. We support our customers from start to finish, from training to the full operation of Kubernetes setups. We give regular workshops and share our know-how."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Syseleven Kcsp Reviewer

## Purpose

Review and operate SysEleven (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SysEleven (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SysEleven (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.syseleven.de/en/products-services/kubernetes/
...
- [ ] Trace SysEleven (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SysEleven (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SysEleven (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SysEleven (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SysEleven (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SysEleven (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SysEleven (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SysEleven (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SysEleven (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SysEleven (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include SysEleven (KCSP) validation commands or tests that are safe for the stated environment.
...
- SysEleven (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SysEleven (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SysEleven (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SysEleven (KCSP) as the basis for migration or production guidance.
