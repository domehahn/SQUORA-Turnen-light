---
name: "cncf-qiming-kcsp-reviewer"
description: "Review Qiming (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. With Kubernetes as the technical base, QiMing provides full life cycle service solutions such as container orchestration, operation and maintenance, cloud native technical consultation, training, and microservice transformation."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Qiming Kcsp Reviewer

## Purpose

Review and operate Qiming (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Qiming (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Qiming (KCSP) requires evidence-backed assessment.
...
- First-party source: http://www.qm.cn/qm/fw39/yw86/k8syw/index.html
...
- [ ] Trace Qiming (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Qiming (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Qiming (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Qiming (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Qiming (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Qiming (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Qiming (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Qiming (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Qiming (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Qiming (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Qiming (KCSP) validation commands or tests that are safe for the stated environment.
...
- Qiming (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Qiming (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Qiming (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Qiming (KCSP) as the basis for migration or production guidance.
