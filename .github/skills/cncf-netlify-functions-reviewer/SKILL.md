---
name: "cncf-netlify-functions-reviewer"
description: "Review Netlify Functions using its official documentation and repository in the Serverless / Hosted Platform category. Run your application code without servers, scale it automatically, and pay nothing when it's not in use."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Netlify Functions Reviewer

## Purpose

Review and operate Netlify Functions using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Netlify Functions code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Netlify Functions requires evidence-backed assessment.
...
- First-party source: https://www.netlify.com/products/functions/
...
- [ ] Trace Netlify Functions inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Netlify Functions defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Netlify Functions least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Netlify Functions resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Netlify Functions release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Netlify Functions dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Netlify Functions identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Netlify Functions change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Netlify Functions documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Netlify Functions architecture and dependency summary with trust boundaries and persistent state.
- Include Netlify Functions validation commands or tests that are safe for the stated environment.
...
- Netlify Functions identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Netlify Functions failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Netlify Functions is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Netlify Functions as the basis for migration or production guidance.
