---
name: "cncf-kubestellar-reviewer"
description: "Review KubeStellar using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. KubeStellar - a flexible solution for challenges associated with multi-cluster configuration management for edge, multi-cloud, and hybrid cloud"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kubestellar Reviewer

## Purpose

Review and operate KubeStellar using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeStellar code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeStellar requires evidence-backed assessment.
...
- First-party source: https://github.com/kubestellar/kubestellar
...
- [ ] Trace KubeStellar inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeStellar defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeStellar least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeStellar resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeStellar release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeStellar dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeStellar identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeStellar change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeStellar documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeStellar architecture and dependency summary with trust boundaries and persistent state.
- Include KubeStellar validation commands or tests that are safe for the stated environment.
...
- KubeStellar identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeStellar failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeStellar is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeStellar as the basis for migration or production guidance.
