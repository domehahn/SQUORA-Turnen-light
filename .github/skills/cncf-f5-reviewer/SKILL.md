---
name: "cncf-f5-reviewer"
description: "Review F5 using its official documentation and repository in the Orchestration & Management / Service Proxy category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate F5 using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- F5 code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving F5 requires evidence-backed assessment.
...
- [ ] Trace F5 inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check F5 defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check F5 least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check F5 resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check F5 release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check F5 dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If F5 identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a F5 change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- F5 documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a F5 architecture and dependency summary with trust boundaries and persistent state.
- Include F5 validation commands or tests that are safe for the stated environment.
...
- F5 identity, version, configuration, dependencies, and deployment assumptions are explicit.
- F5 failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that F5 is secure, supported, compatible, or suitable.
- Using an unpinned latest version of F5 as the basis for migration or production guidance.
