---
name: "cncf-kubeclipper-reviewer"
description: "Review KubeClipper using its official documentation and repository in the Platform / Certified Kubernetes - Installer category. Manage kubernetes in the most light and convenient way."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kubeclipper Reviewer

## Purpose

Review and operate KubeClipper using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeClipper code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeClipper requires evidence-backed assessment.
...
- First-party source: https://github.com/kubeclipper/kubeclipper
...
- [ ] Trace KubeClipper inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeClipper defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeClipper least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeClipper resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeClipper release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeClipper dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeClipper identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeClipper change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeClipper documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeClipper architecture and dependency summary with trust boundaries and persistent state.
- Include KubeClipper validation commands or tests that are safe for the stated environment.
...
- KubeClipper identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeClipper failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeClipper is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeClipper as the basis for migration or production guidance.
