---
name: "cncf-coredge-kubernetes-platform-reviewer"
description: "Review Coredge Kubernetes Platform using its official documentation and repository in the Platform / Certified Kubernetes - Distribution category. A custom Kubernetes distro."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Coredge Kubernetes Platform Reviewer

## Purpose

Review and operate Coredge Kubernetes Platform using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Coredge Kubernetes Platform code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Coredge Kubernetes Platform requires evidence-backed assessment.
...
- First-party source: https://coredge.io
...
- [ ] Trace Coredge Kubernetes Platform inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Coredge Kubernetes Platform defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Coredge Kubernetes Platform least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Coredge Kubernetes Platform resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Coredge Kubernetes Platform release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Coredge Kubernetes Platform dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Coredge Kubernetes Platform identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Coredge Kubernetes Platform change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Coredge Kubernetes Platform documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Coredge Kubernetes Platform architecture and dependency summary with trust boundaries and persistent state.
- Include Coredge Kubernetes Platform validation commands or tests that are safe for the stated environment.
...
- Coredge Kubernetes Platform identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Coredge Kubernetes Platform failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Coredge Kubernetes Platform is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Coredge Kubernetes Platform as the basis for migration or production guidance.
