---
name: "cncf-nccl-reviewer"
description: "Review NCCL using its official documentation and repository in the AI Native Infra / Accelerator and SuperPod category. Optimized primitives for inter-GPU communication."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate NCCL using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- NCCL code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving NCCL requires evidence-backed assessment.
...
- First-party source: https://github.com/NVIDIA/nccl
...
- [ ] Trace NCCL inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check NCCL defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check NCCL least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check NCCL resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check NCCL release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check NCCL dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If NCCL identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a NCCL change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- NCCL documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a NCCL architecture and dependency summary with trust boundaries and persistent state.
- Include NCCL validation commands or tests that are safe for the stated environment.
...
- NCCL identity, version, configuration, dependencies, and deployment assumptions are explicit.
- NCCL failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that NCCL is secure, supported, compatible, or suitable.
- Using an unpinned latest version of NCCL as the basis for migration or production guidance.
