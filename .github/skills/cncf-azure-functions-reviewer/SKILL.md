...
---
# Cncf Azure Functions Reviewer

## Purpose

Review and operate Azure Functions using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Azure Functions code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Azure Functions requires evidence-backed assessment.
...
- First-party source: https://github.com/Azure/azure-functions-host
...
- [ ] Trace Azure Functions inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Azure Functions defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Azure Functions least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Azure Functions resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Azure Functions release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Azure Functions dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Azure Functions identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Azure Functions change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Azure Functions documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Azure Functions architecture and dependency summary with trust boundaries and persistent state.
- Include Azure Functions validation commands or tests that are safe for the stated environment.
...
- Azure Functions identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Azure Functions failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Azure Functions is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Azure Functions as the basis for migration or production guidance.
