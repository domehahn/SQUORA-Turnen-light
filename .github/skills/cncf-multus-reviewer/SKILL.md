---
name: "cncf-multus-reviewer"
description: "Review Multus using its official documentation and repository in the Runtime / Cloud Native Network category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Multus Reviewer

## Purpose

Review and operate Multus using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Multus code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Multus requires evidence-backed assessment.
...
- First-party source: https://github.com/k8snetworkplumbingwg/multus-cni
...
- [ ] Trace Multus inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Multus defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Multus least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Multus resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Multus release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Multus dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Multus identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Multus change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Multus documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Multus architecture and dependency summary with trust boundaries and persistent state.
- Include Multus validation commands or tests that are safe for the stated environment.
...
- Multus identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Multus failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Multus is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Multus as the basis for migration or production guidance.
