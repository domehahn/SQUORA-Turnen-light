---
name: "cncf-crates-io-reviewer"
description: "Review crates.io using its official documentation and repository in the Wasm / Packaging, Registries & Application Delivery category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Crates Io Reviewer

## Purpose

Review and operate crates.io using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- crates.io code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving crates.io requires evidence-backed assessment.
...
- First-party source: https://github.com/rust-lang/crates.io
...
- [ ] Trace crates.io inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check crates.io defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check crates.io least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check crates.io resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check crates.io release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check crates.io dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If crates.io identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a crates.io change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- crates.io documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a crates.io architecture and dependency summary with trust boundaries and persistent state.
- Include crates.io validation commands or tests that are safe for the stated environment.
...
- crates.io identity, version, configuration, dependencies, and deployment assumptions are explicit.
- crates.io failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that crates.io is secure, supported, compatible, or suitable.
- Using an unpinned latest version of crates.io as the basis for migration or production guidance.
