---
name: "cncf-automq-reviewer"
description: "Review AutoMQ using its official documentation and repository in the App Definition and Development / Streaming & Messaging category. AutoMQ is a cloud-native fork of Kafka by separating storage to S3. 10x cost-effective. Autoscale in seconds. Single-digit ms latency."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Automq Reviewer

## Purpose

Review and operate AutoMQ using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- AutoMQ code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving AutoMQ requires evidence-backed assessment.
...
- First-party source: https://github.com/AutoMQ/automq
...
- [ ] Trace AutoMQ inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check AutoMQ defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check AutoMQ least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check AutoMQ resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check AutoMQ release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check AutoMQ dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If AutoMQ identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a AutoMQ change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- AutoMQ documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a AutoMQ architecture and dependency summary with trust boundaries and persistent state.
- Include AutoMQ validation commands or tests that are safe for the stated environment.
...
- AutoMQ identity, version, configuration, dependencies, and deployment assumptions are explicit.
- AutoMQ failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that AutoMQ is secure, supported, compatible, or suitable.
- Using an unpinned latest version of AutoMQ as the basis for migration or production guidance.
