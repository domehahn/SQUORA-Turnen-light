---
name: "cncf-vshn-ag-kcsp-reviewer"
description: "Review VSHN AG (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. VSHN - The DevOps Company transforms software into online services by automating and managing application workloads. Let us handle the Ops so you can build the future. We provide 24/7 platform engineering and Kubernetes support for software developers, SaaS, agencies, and ISVs."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Vshn Ag Kcsp Reviewer

## Purpose

Review and operate VSHN AG (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- VSHN AG (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving VSHN AG (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.vshn.ch/en/solutions/kubernetes-ready-and-containerized/
...
- [ ] Trace VSHN AG (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check VSHN AG (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check VSHN AG (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check VSHN AG (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check VSHN AG (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check VSHN AG (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If VSHN AG (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a VSHN AG (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- VSHN AG (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a VSHN AG (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include VSHN AG (KCSP) validation commands or tests that are safe for the stated environment.
...
- VSHN AG (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- VSHN AG (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that VSHN AG (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of VSHN AG (KCSP) as the basis for migration or production guidance.
