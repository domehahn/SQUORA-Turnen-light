---
name: "cncf-grype-reviewer"
description: "Review Grype using its official documentation and repository in the Provisioning / Security & Compliance category. A vulnerability scanner for container images and filesystems"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Grype using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Grype code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Grype requires evidence-backed assessment.
...
- First-party source: https://github.com/anchore/grype
...
- [ ] Trace Grype inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Grype defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Grype least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Grype resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Grype release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Grype dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Grype identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Grype change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Grype documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Grype architecture and dependency summary with trust boundaries and persistent state.
- Include Grype validation commands or tests that are safe for the stated environment.
...
- Grype identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Grype failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Grype is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Grype as the basis for migration or production guidance.
