---
name: "cncf-kubefwd-reviewer"
description: "Review kubefwd using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category. Bulk port forwarding Kubernetes services to localhost with unique loopback IPs, enabling multiple services on the same port without conflicts."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kubefwd Reviewer

## Purpose

Review and operate kubefwd using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- kubefwd code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving kubefwd requires evidence-backed assessment.
...
- First-party source: https://github.com/txn2/kubefwd
...
- [ ] Trace kubefwd inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check kubefwd defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check kubefwd least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check kubefwd resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check kubefwd release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check kubefwd dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If kubefwd identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a kubefwd change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- kubefwd documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a kubefwd architecture and dependency summary with trust boundaries and persistent state.
- Include kubefwd validation commands or tests that are safe for the stated environment.
...
- kubefwd identity, version, configuration, dependencies, and deployment assumptions are explicit.
- kubefwd failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that kubefwd is secure, supported, compatible, or suitable.
- Using an unpinned latest version of kubefwd as the basis for migration or production guidance.
