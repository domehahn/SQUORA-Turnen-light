...
---
# Cncf Begasoft Kcsp Reviewer

## Purpose

Review and operate BEGASOFT (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- BEGASOFT (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving BEGASOFT (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.begasoft.ch/infrastructure-managed-cloud/managed-kubernetes-platform
...
- [ ] Trace BEGASOFT (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check BEGASOFT (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check BEGASOFT (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check BEGASOFT (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check BEGASOFT (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check BEGASOFT (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If BEGASOFT (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a BEGASOFT (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- BEGASOFT (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a BEGASOFT (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include BEGASOFT (KCSP) validation commands or tests that are safe for the stated environment.
...
- BEGASOFT (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- BEGASOFT (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that BEGASOFT (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of BEGASOFT (KCSP) as the basis for migration or production guidance.
