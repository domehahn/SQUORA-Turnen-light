---
name: "cncf-selenium-grid-reviewer"
description: "Review Selenium Grid using its official documentation and repository in the AI Agent / Agent Tool category. A browser automation framework and ecosystem."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Selenium Grid Reviewer

## Purpose

Review and operate Selenium Grid using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Selenium Grid code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Selenium Grid requires evidence-backed assessment.
...
- First-party source: https://github.com/SeleniumHQ/selenium
...
- [ ] Trace Selenium Grid inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Selenium Grid defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Selenium Grid least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Selenium Grid resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Selenium Grid release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Selenium Grid dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Selenium Grid identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Selenium Grid change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Selenium Grid documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Selenium Grid architecture and dependency summary with trust boundaries and persistent state.
- Include Selenium Grid validation commands or tests that are safe for the stated environment.
...
- Selenium Grid identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Selenium Grid failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Selenium Grid is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Selenium Grid as the basis for migration or production guidance.
