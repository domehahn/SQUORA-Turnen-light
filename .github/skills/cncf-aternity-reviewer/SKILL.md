---
name: "cncf-aternity-reviewer"
description: "Review Aternity using its official documentation and repository in the Observability and Analysis / Observability category. Cloud-native, scalable, high-definition application performance monitoring (APM) for next gen apps."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Aternity Reviewer

## Purpose

Review and operate Aternity using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Aternity code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Aternity requires evidence-backed assessment.
...
- First-party source: https://www.aternity.com/application-performance-monitoring/
...
- [ ] Trace Aternity inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Aternity defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Aternity least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Aternity resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Aternity release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Aternity dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Aternity identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Aternity change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Aternity documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Aternity architecture and dependency summary with trust boundaries and persistent state.
- Include Aternity validation commands or tests that are safe for the stated environment.
...
- Aternity identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Aternity failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Aternity is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Aternity as the basis for migration or production guidance.
