...
Review and operate werf using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- werf code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving werf requires evidence-backed assessment.
...
- First-party source: https://github.com/werf/werf
...
- [ ] Trace werf inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check werf defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check werf least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check werf resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check werf release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check werf dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If werf identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a werf change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- werf documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a werf architecture and dependency summary with trust boundaries and persistent state.
- Include werf validation commands or tests that are safe for the stated environment.
...
- werf identity, version, configuration, dependencies, and deployment assumptions are explicit.
- werf failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that werf is secure, supported, compatible, or suitable.
- Using an unpinned latest version of werf as the basis for migration or production guidance.
