---
name: "cncf-cubex-reviewer"
description: "Review Cubex using its official documentation and repository in the Platform / Certified Kubernetes - Distribution category. CubeX is the easiest and most powerful PaaS solution for Cloud Native Apps. CubeX adds stability to speed, enabling full DevOps."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Cubex using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Cubex code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Cubex requires evidence-backed assessment.
...
- [ ] Trace Cubex inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Cubex defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Cubex least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Cubex resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Cubex release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Cubex dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Cubex identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Cubex change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Cubex documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Cubex architecture and dependency summary with trust boundaries and persistent state.
- Include Cubex validation commands or tests that are safe for the stated environment.
...
- Cubex identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Cubex failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Cubex is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Cubex as the basis for migration or production guidance.
