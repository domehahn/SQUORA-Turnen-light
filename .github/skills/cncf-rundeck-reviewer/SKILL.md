---
name: "cncf-rundeck-reviewer"
description: "Review Rundeck using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Rundeck Reviewer

## Purpose

Review and operate Rundeck using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Rundeck code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Rundeck requires evidence-backed assessment.
...
- First-party source: https://github.com/rundeck/rundeck
...
- [ ] Trace Rundeck inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Rundeck defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Rundeck least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Rundeck resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Rundeck release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Rundeck dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Rundeck identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Rundeck change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Rundeck documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Rundeck architecture and dependency summary with trust boundaries and persistent state.
- Include Rundeck validation commands or tests that are safe for the stated environment.
...
- Rundeck identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Rundeck failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Rundeck is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Rundeck as the basis for migration or production guidance.
