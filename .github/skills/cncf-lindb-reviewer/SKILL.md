---
name: "cncf-lindb-reviewer"
description: "Review LinDB using its official documentation and repository in the Observability and Analysis / Observability category. LinDB is a scalable, high performance, high availability distributed time series database."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate LinDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- LinDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving LinDB requires evidence-backed assessment.
...
- First-party source: https://github.com/lindb/lindb
...
- [ ] Trace LinDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check LinDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check LinDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check LinDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check LinDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check LinDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If LinDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a LinDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- LinDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a LinDB architecture and dependency summary with trust boundaries and persistent state.
- Include LinDB validation commands or tests that are safe for the stated environment.
...
- LinDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- LinDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that LinDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of LinDB as the basis for migration or production guidance.
