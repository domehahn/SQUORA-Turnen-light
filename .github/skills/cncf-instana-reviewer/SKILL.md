---
name: "cncf-instana-reviewer"
description: "Review Instana using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Instana Reviewer

## Purpose

Review and operate Instana using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Instana code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Instana requires evidence-backed assessment.
...
- First-party source: https://www.instana.com/
...
- [ ] Trace Instana inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Instana defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Instana least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Instana resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Instana release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Instana dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Instana identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Instana change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Instana documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Instana architecture and dependency summary with trust boundaries and persistent state.
- Include Instana validation commands or tests that are safe for the stated environment.
...
- Instana identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Instana failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Instana is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Instana as the basis for migration or production guidance.
