---
name: "cncf-slime-reviewer"
description: "Review Slime using its official documentation and repository in the Orchestration & Management / Service Mesh category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Slime using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Slime code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Slime requires evidence-backed assessment.
...
- First-party source: https://github.com/slime-io/slime
...
- [ ] Trace Slime inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Slime defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Slime least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Slime resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Slime release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Slime dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Slime identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Slime change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Slime documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Slime architecture and dependency summary with trust boundaries and persistent state.
- Include Slime validation commands or tests that are safe for the stated environment.
...
- Slime identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Slime failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Slime is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Slime as the basis for migration or production guidance.
