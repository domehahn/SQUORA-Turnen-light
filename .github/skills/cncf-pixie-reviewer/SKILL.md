---
name: "cncf-pixie-reviewer"
description: "Review Pixie using its official documentation and repository in the Observability and Analysis / Observability category. Open source Kubernetes observability for developers"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Pixie using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Pixie code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Pixie requires evidence-backed assessment.
...
- First-party source: https://github.com/pixie-io/pixie
...
- [ ] Trace Pixie inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Pixie defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Pixie least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Pixie resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Pixie release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Pixie dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Pixie identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Pixie change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Pixie documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Pixie architecture and dependency summary with trust boundaries and persistent state.
- Include Pixie validation commands or tests that are safe for the stated environment.
...
- Pixie identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Pixie failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Pixie is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Pixie as the basis for migration or production guidance.
