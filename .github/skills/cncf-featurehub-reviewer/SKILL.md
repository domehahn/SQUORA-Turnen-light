...
---
# Cncf Featurehub Reviewer

## Purpose

Review and operate FeatureHub using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- FeatureHub code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving FeatureHub requires evidence-backed assessment.
...
- First-party source: https://github.com/featurehub-io/featurehub
...
- [ ] Trace FeatureHub inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check FeatureHub defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check FeatureHub least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check FeatureHub resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check FeatureHub release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check FeatureHub dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If FeatureHub identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a FeatureHub change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- FeatureHub documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a FeatureHub architecture and dependency summary with trust boundaries and persistent state.
- Include FeatureHub validation commands or tests that are safe for the stated environment.
...
- FeatureHub identity, version, configuration, dependencies, and deployment assumptions are explicit.
- FeatureHub failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that FeatureHub is secure, supported, compatible, or suitable.
- Using an unpinned latest version of FeatureHub as the basis for migration or production guidance.
