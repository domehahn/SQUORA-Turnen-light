---
name: "cncf-stackrox-reviewer"
description: "Review StackRox using its official documentation and repository in the Provisioning / Security & Compliance category. StackRox is a Kubernetes-native security platform for cloud-native applications, containers, serverless, and Kubernetes."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Stackrox Reviewer

## Purpose

Review and operate StackRox using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- StackRox code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving StackRox requires evidence-backed assessment.
...
- First-party source: https://github.com/stackrox/stackrox
...
- [ ] Trace StackRox inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check StackRox defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check StackRox least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check StackRox resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check StackRox release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check StackRox dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If StackRox identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a StackRox change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- StackRox documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a StackRox architecture and dependency summary with trust boundaries and persistent state.
- Include StackRox validation commands or tests that are safe for the stated environment.
...
- StackRox identity, version, configuration, dependencies, and deployment assumptions are explicit.
- StackRox failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that StackRox is secure, supported, compatible, or suitable.
- Using an unpinned latest version of StackRox as the basis for migration or production guidance.
