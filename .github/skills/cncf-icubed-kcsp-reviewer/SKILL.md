---
name: "cncf-icubed-kcsp-reviewer"
description: "Review iCubed (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. iCubed provides strategy, transformation and managed services across Application Modernisation, DevOps and Kubernetes."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Icubed Kcsp Reviewer

## Purpose

Review and operate iCubed (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- iCubed (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving iCubed (KCSP) requires evidence-backed assessment.
...
- First-party source: https://icubed.it/en/solutions/kubernetes-professional-services/
...
- [ ] Trace iCubed (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check iCubed (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check iCubed (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check iCubed (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check iCubed (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check iCubed (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If iCubed (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a iCubed (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- iCubed (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a iCubed (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include iCubed (KCSP) validation commands or tests that are safe for the stated environment.
...
- iCubed (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- iCubed (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that iCubed (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of iCubed (KCSP) as the basis for migration or production guidance.
