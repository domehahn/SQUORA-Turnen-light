...
---
# Cncf Octopus Deploy Reviewer

## Purpose

Review and operate Octopus Deploy using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Octopus Deploy code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Octopus Deploy requires evidence-backed assessment.
...
- [ ] Trace Octopus Deploy inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Octopus Deploy defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Octopus Deploy least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Octopus Deploy resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Octopus Deploy release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Octopus Deploy dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Octopus Deploy identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Octopus Deploy change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Octopus Deploy documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Octopus Deploy architecture and dependency summary with trust boundaries and persistent state.
- Include Octopus Deploy validation commands or tests that are safe for the stated environment.
...
- Octopus Deploy identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Octopus Deploy failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Octopus Deploy is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Octopus Deploy as the basis for migration or production guidance.
