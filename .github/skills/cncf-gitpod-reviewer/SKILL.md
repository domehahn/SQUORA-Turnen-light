---
name: "cncf-gitpod-reviewer"
description: "Review Gitpod using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Gitpod Reviewer

## Purpose

Review and operate Gitpod using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Gitpod code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Gitpod requires evidence-backed assessment.
...
- [ ] Trace Gitpod inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Gitpod defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Gitpod least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Gitpod resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Gitpod release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Gitpod dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Gitpod identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Gitpod change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Gitpod documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Gitpod architecture and dependency summary with trust boundaries and persistent state.
- Include Gitpod validation commands or tests that are safe for the stated environment.
...
- Gitpod identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Gitpod failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Gitpod is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Gitpod as the basis for migration or production guidance.
