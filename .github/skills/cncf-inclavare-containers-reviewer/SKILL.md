---
name: "cncf-inclavare-containers-reviewer"
description: "Review Inclavare Containers using its official documentation and repository in the Runtime / Container Runtime category. A novel container runtime, aka confidential container, for cloud-native confidential computing and enclave runtime ecosystem"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Inclavare Containers Reviewer

## Purpose

Review and operate Inclavare Containers using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Inclavare Containers code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Inclavare Containers requires evidence-backed assessment.
...
- First-party source: https://github.com/inclavare-containers/inclavare-containers
...
- [ ] Trace Inclavare Containers inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Inclavare Containers defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Inclavare Containers least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Inclavare Containers resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Inclavare Containers release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Inclavare Containers dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Inclavare Containers identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Inclavare Containers change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Inclavare Containers documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Inclavare Containers architecture and dependency summary with trust boundaries and persistent state.
- Include Inclavare Containers validation commands or tests that are safe for the stated environment.
...
- Inclavare Containers identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Inclavare Containers failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Inclavare Containers is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Inclavare Containers as the basis for migration or production guidance.
