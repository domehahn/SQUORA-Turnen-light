---
name: "cncf-wasmcloud-reviewer"
description: "Review wasmCloud using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Wasmcloud Reviewer

## Purpose

Review and operate wasmCloud using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- wasmCloud code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving wasmCloud requires evidence-backed assessment.
...
- [ ] Trace wasmCloud inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check wasmCloud defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check wasmCloud least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check wasmCloud resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check wasmCloud release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check wasmCloud dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If wasmCloud identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a wasmCloud change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- wasmCloud documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a wasmCloud architecture and dependency summary with trust boundaries and persistent state.
- Include wasmCloud validation commands or tests that are safe for the stated environment.
...
- wasmCloud identity, version, configuration, dependencies, and deployment assumptions are explicit.
- wasmCloud failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that wasmCloud is secure, supported, compatible, or suitable.
- Using an unpinned latest version of wasmCloud as the basis for migration or production guidance.
