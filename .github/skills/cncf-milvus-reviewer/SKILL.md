---
name: "cncf-milvus-reviewer"
description: "Review Milvus using its official documentation and repository in the AI Agent / Vector Database category. A cloud-native vector database, storage for next generation AI applications."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Milvus Reviewer

## Purpose

Review and operate Milvus using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Milvus code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Milvus requires evidence-backed assessment.
...
- First-party source: https://github.com/milvus-io/milvus
...
- [ ] Trace Milvus inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Milvus defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Milvus least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Milvus resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Milvus release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Milvus dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Milvus identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Milvus change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Milvus documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Milvus architecture and dependency summary with trust boundaries and persistent state.
- Include Milvus validation commands or tests that are safe for the stated environment.
...
- Milvus identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Milvus failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Milvus is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Milvus as the basis for migration or production guidance.
