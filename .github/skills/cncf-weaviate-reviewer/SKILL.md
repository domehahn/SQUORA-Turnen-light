---
name: "cncf-weaviate-reviewer"
description: "Review Weaviate using its official documentation and repository in the App Definition and Development / Database category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Weaviate Reviewer

## Purpose

Review and operate Weaviate using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Weaviate code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Weaviate requires evidence-backed assessment.
...
- First-party source: https://github.com/weaviate/weaviate
...
- [ ] Trace Weaviate inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Weaviate defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Weaviate least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Weaviate resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Weaviate release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Weaviate dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Weaviate identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Weaviate change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Weaviate documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Weaviate architecture and dependency summary with trust boundaries and persistent state.
- Include Weaviate validation commands or tests that are safe for the stated environment.
...
- Weaviate identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Weaviate failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Weaviate is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Weaviate as the basis for migration or production guidance.
