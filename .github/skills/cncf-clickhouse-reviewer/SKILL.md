...
Review and operate ClickHouse using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ClickHouse code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ClickHouse requires evidence-backed assessment.
...
- First-party source: https://github.com/ClickHouse/ClickHouse
...
- [ ] Trace ClickHouse inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ClickHouse defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ClickHouse least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ClickHouse resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ClickHouse release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ClickHouse dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ClickHouse identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ClickHouse change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ClickHouse documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ClickHouse architecture and dependency summary with trust boundaries and persistent state.
- Include ClickHouse validation commands or tests that are safe for the stated environment.
...
- ClickHouse identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ClickHouse failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ClickHouse is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ClickHouse as the basis for migration or production guidance.
