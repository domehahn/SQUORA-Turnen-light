---
name: "cncf-temporal-reviewer"
description: "Review Temporal using its official documentation and repository in the AI Agent / Workflow Orchestration category. Temporal service"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Temporal Reviewer

## Purpose

Review and operate Temporal using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Temporal code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Temporal requires evidence-backed assessment.
...
- First-party source: https://github.com/temporalio/temporal
...
- [ ] Trace Temporal inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Temporal defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Temporal least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Temporal resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Temporal release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Temporal dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Temporal identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Temporal change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Temporal documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Temporal architecture and dependency summary with trust boundaries and persistent state.
- Include Temporal validation commands or tests that are safe for the stated environment.
...
- Temporal identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Temporal failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Temporal is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Temporal as the basis for migration or production guidance.
