---
name: "cncf-zstack-kcsp-reviewer"
description: "Review ZStack (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. ZStack enables customers on their Kubernetes journey, helps enterprises focus on their applications rather than managing IT-infrastructure, and offers full life-cycle services including training, consulting, deployment, and pipeline to accelerate their digital transformation."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Zstack Kcsp Reviewer

## Purpose

Review and operate ZStack (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ZStack (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ZStack (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.zstack-cloud.com/
...
- [ ] Trace ZStack (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ZStack (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ZStack (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ZStack (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ZStack (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ZStack (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ZStack (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ZStack (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ZStack (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ZStack (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include ZStack (KCSP) validation commands or tests that are safe for the stated environment.
...
- ZStack (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ZStack (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ZStack (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ZStack (KCSP) as the basis for migration or production guidance.
