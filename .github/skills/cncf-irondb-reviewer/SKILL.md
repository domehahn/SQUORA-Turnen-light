---
name: "cncf-irondb-reviewer"
description: "Review IronDB using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Irondb Reviewer

## Purpose

Review and operate IronDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- IronDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving IronDB requires evidence-backed assessment.
...
- First-party source: https://www.circonus.com
...
- [ ] Trace IronDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check IronDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check IronDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check IronDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check IronDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check IronDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If IronDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a IronDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- IronDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a IronDB architecture and dependency summary with trust boundaries and persistent state.
- Include IronDB validation commands or tests that are safe for the stated environment.
...
- IronDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- IronDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that IronDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of IronDB as the basis for migration or production guidance.
