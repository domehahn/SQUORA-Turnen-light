---
name: "cncf-asus-cloud-infra-reviewer"
description: "Review ASUS Cloud Infra using its official documentation and repository in the Platform / Certified Kubernetes - Distribution category. ASUS Cloud Infra helps you bootstrap a Kubernetes cluster."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Asus Cloud Infra Reviewer

## Purpose

Review and operate ASUS Cloud Infra using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ASUS Cloud Infra code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ASUS Cloud Infra requires evidence-backed assessment.
...
- First-party source: https://www.asuscloud.com/
...
- [ ] Trace ASUS Cloud Infra inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ASUS Cloud Infra defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ASUS Cloud Infra least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ASUS Cloud Infra resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ASUS Cloud Infra release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ASUS Cloud Infra dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ASUS Cloud Infra identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ASUS Cloud Infra change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ASUS Cloud Infra documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ASUS Cloud Infra architecture and dependency summary with trust boundaries and persistent state.
- Include ASUS Cloud Infra validation commands or tests that are safe for the stated environment.
...
- ASUS Cloud Infra identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ASUS Cloud Infra failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ASUS Cloud Infra is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ASUS Cloud Infra as the basis for migration or production guidance.
