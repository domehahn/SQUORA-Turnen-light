---
name: "payment-observability-reviewer"
description: "Review payment conversion, authorization, webhook, refund, reconciliation, latency, provider health, alerts, and privacy-safe telemetry."
version: "0.1.0"
since: "2026-08-27"
...
Review payment telemetry for conversion, authorization, authentication, capture, webhook, refund, reconciliation, latency, provider health, and customer-impact signals without leaking sensitive data. Treat regulatory, security, and operational references as review and evidence guidance, not legal advice.
...
- payment observability decisions, controls, or operating practices need independent review.
- A change affects payment observability artifacts such as payment funnel dashboard, authorization metric, webhook backlog alert, provider latency SLI, reconciliation exception metric, redacted payment trace.
- The user needs evidence-oriented findings for risks such as silent conversion drop, provider outage blind spot, webhook backlog, high-cardinality identifiers, PAN or customer data in telemetry, unactionable payment alert.
- Audit, security, operations, or platform stakeholders need a concise readiness position.
- Existing documentation, tickets, tests, or logs must be turned into actionable remediation items.

## Operating model

1. Identify the relevant payment observability artifacts, owners, systems, environments, and review boundary.
2. Compare the available artifacts against expected signals such as authorization success rate, checkout conversion rate, provider error code distribution, webhook processing lag, reconciliation break count, redaction test.
...
- Primary artifacts: payment funnel dashboard, authorization metric, webhook backlog alert, provider latency SLI, reconciliation exception metric, redacted payment trace.
- Risk themes: silent conversion drop, provider outage blind spot, webhook backlog, high-cardinality identifiers, PAN or customer data in telemetry, unactionable payment alert.
- Evidence signals: authorization success rate, checkout conversion rate, provider error code distribution, webhook processing lag, reconciliation break count, redaction test.
...
- [ ] Confirm the review boundary covers the right payment observability systems, teams, and environments.
- [ ] Inventory and inspect the current payment funnel dashboard.
- [ ] Check whether authorization metric is current, approved, versioned, and owned.
- [ ] Verify that webhook backlog alert has test, ticket, log, or approval support.
- [ ] Look for silent conversion drop and record concrete repository or process evidence.
- [ ] Look for provider outage blind spot and identify affected assets, services, or stakeholders.
- [ ] Look for webhook backlog and classify the operational or audit impact.
- [ ] Use authorization success rate to validate that the control or practice is operating.
- [ ] Use checkout conversion rate to confirm ownership, timing, and reproducibility.
- [ ] Check exception, risk-acceptance, and expiry handling for payment observability.
...
- If payment funnel dashboard is missing for a critical service, raise at least a high-severity readiness gap.
- If checkout conversion rate cannot be tied to an owner and approval, treat the outcome as unauditable until corrected.
- If provider latency SLI is present but expired or untested, require validation before accepting residual risk.
...
- Missing or stale payment observability artifact.
...
- Findings ordered by severity with affected payment observability artifacts and evidence references.
- Coverage note for reviewed artifacts: payment funnel dashboard, authorization metric, webhook backlog alert, provider latency SLI, reconciliation exception metric, redacted payment trace.
- Risk note covering relevant themes: silent conversion drop, provider outage blind spot, webhook backlog, high-cardinality identifiers, PAN or customer data in telemetry, unactionable payment alert.
- Evidence request list using expected signals: authorization success rate, checkout conversion rate, provider error code distribution, webhook processing lag, reconciliation break count, redaction test.
- Deliverables or updates needed: payment observability review, payment SLI and alert catalogue, funnel dashboard recommendations, provider-health runbook, telemetry privacy findings.
...
- Relevant payment observability artifacts are identified, current, owned, and versioned where applicable.
