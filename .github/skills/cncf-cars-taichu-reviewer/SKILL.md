...
---
# Cncf Cars Taichu Reviewer

## Purpose

Review and operate CARS TaiChu using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CARS TaiChu code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CARS TaiChu requires evidence-backed assessment.
...
- First-party source: https://www.rails.cn/xian/en-us/src/prointroduce/kmpp
...
- [ ] Trace CARS TaiChu inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CARS TaiChu defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CARS TaiChu least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CARS TaiChu resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CARS TaiChu release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CARS TaiChu dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CARS TaiChu identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CARS TaiChu change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CARS TaiChu documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CARS TaiChu architecture and dependency summary with trust boundaries and persistent state.
- Include CARS TaiChu validation commands or tests that are safe for the stated environment.
...
- CARS TaiChu identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CARS TaiChu failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CARS TaiChu is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CARS TaiChu as the basis for migration or production guidance.
