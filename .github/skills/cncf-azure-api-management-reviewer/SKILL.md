...
---
# Cncf Azure Api Management Reviewer

## Purpose

Review and operate Azure API Management using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Azure API Management code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Azure API Management requires evidence-backed assessment.
...
- First-party source: https://github.com/azure/api-management
...
- [ ] Trace Azure API Management inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Azure API Management defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Azure API Management least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Azure API Management resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Azure API Management release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Azure API Management dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Azure API Management identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Azure API Management change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Azure API Management documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Azure API Management architecture and dependency summary with trust boundaries and persistent state.
- Include Azure API Management validation commands or tests that are safe for the stated environment.
...
- Azure API Management identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Azure API Management failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Azure API Management is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Azure API Management as the basis for migration or production guidance.
