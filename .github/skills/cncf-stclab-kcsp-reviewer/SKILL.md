...
---
# Cncf Stclab Kcsp Reviewer

## Purpose

Review and operate STCLAB (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- STCLAB (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving STCLAB (KCSP) requires evidence-backed assessment.
...
- First-party source: https://platform.stclab.com/kubernetes
...
- [ ] Trace STCLAB (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check STCLAB (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check STCLAB (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check STCLAB (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check STCLAB (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check STCLAB (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If STCLAB (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a STCLAB (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- STCLAB (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a STCLAB (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include STCLAB (KCSP) validation commands or tests that are safe for the stated environment.
...
- STCLAB (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- STCLAB (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that STCLAB (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of STCLAB (KCSP) as the basis for migration or production guidance.
