---
name: "cncf-bunkerweb-reviewer"
description: "Review BunkerWeb using its official documentation and repository in the Orchestration & Management / Service Proxy category. BunkerWeb is a next-generation, open-source Web Application Firewall (WAF/WAAP), reverse proxy, ingress controller, and API gateway controller."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Bunkerweb Reviewer

## Purpose

Review and operate BunkerWeb using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- BunkerWeb code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving BunkerWeb requires evidence-backed assessment.
...
- First-party source: https://github.com/bunkerity/bunkerweb
...
- [ ] Trace BunkerWeb inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check BunkerWeb defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check BunkerWeb least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check BunkerWeb resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check BunkerWeb release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check BunkerWeb dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If BunkerWeb identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a BunkerWeb change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- BunkerWeb documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a BunkerWeb architecture and dependency summary with trust boundaries and persistent state.
- Include BunkerWeb validation commands or tests that are safe for the stated environment.
...
- BunkerWeb identity, version, configuration, dependencies, and deployment assumptions are explicit.
- BunkerWeb failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that BunkerWeb is secure, supported, compatible, or suitable.
- Using an unpinned latest version of BunkerWeb as the basis for migration or production guidance.
