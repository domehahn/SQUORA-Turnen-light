---
name: "cncf-nimbella-reviewer"
description: "Review Nimbella using its official documentation and repository in the Serverless / Hosted Platform category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Nimbella Reviewer

## Purpose

Review and operate Nimbella using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Nimbella code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Nimbella requires evidence-backed assessment.
...
- First-party source: https://www.digitalocean.com/nimbella
...
- [ ] Trace Nimbella inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Nimbella defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Nimbella least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Nimbella resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Nimbella release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Nimbella dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Nimbella identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Nimbella change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Nimbella documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Nimbella architecture and dependency summary with trust boundaries and persistent state.
- Include Nimbella validation commands or tests that are safe for the stated environment.
...
- Nimbella identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Nimbella failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Nimbella is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Nimbella as the basis for migration or production guidance.
