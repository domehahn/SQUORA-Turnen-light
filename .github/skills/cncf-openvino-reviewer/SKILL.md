---
name: "cncf-openvino-reviewer"
description: "Review OpenVINO using its official documentation and repository in the Inference / Runtime category. OpenVINO™ is an open source toolkit for optimizing and deploying AI inference"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Openvino Reviewer

## Purpose

Review and operate OpenVINO using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenVINO code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenVINO requires evidence-backed assessment.
...
- First-party source: https://github.com/openvinotoolkit/openvino/
...
- [ ] Trace OpenVINO inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenVINO defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenVINO least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenVINO resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenVINO release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenVINO dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenVINO identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenVINO change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenVINO documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenVINO architecture and dependency summary with trust boundaries and persistent state.
- Include OpenVINO validation commands or tests that are safe for the stated environment.
...
- OpenVINO identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenVINO failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenVINO is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenVINO as the basis for migration or production guidance.
