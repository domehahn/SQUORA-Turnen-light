---
name: "cncf-podman-container-tools-reviewer"
description: "Review Podman Container Tools using its official documentation and repository in the Runtime / Container Runtime category. A set of tools providing full management of container lifecycle, including Podman, Buildah, and Skopeo,  which manage containers and images without requiring a daemon or root privileges."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Podman Container Tools Reviewer

## Purpose

Review and operate Podman Container Tools using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Podman Container Tools code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Podman Container Tools requires evidence-backed assessment.
...
- CNCF classification: Runtime / Container Runtime
- CNCF project maturity field: sandbox
- First-party source: https://github.com/podman-container-tools/podman
...
- [ ] Trace Podman Container Tools inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Podman Container Tools defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Podman Container Tools least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Podman Container Tools resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Podman Container Tools release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Podman Container Tools dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Podman Container Tools identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Podman Container Tools change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Podman Container Tools documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Podman Container Tools architecture and dependency summary with trust boundaries and persistent state.
- Include Podman Container Tools validation commands or tests that are safe for the stated environment.
...
- Podman Container Tools identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Podman Container Tools failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Podman Container Tools is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Podman Container Tools as the basis for migration or production guidance.
