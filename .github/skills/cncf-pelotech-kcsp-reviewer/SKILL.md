---
name: "cncf-pelotech-kcsp-reviewer"
description: "Review Pelotech (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Setting clients up for success by crafting Kubernetes first cloud native solutions to their problems."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Pelotech Kcsp Reviewer

## Purpose

Review and operate Pelotech (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Pelotech (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Pelotech (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.pelotech.com/kubernetes-expertise
...
- [ ] Trace Pelotech (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Pelotech (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Pelotech (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Pelotech (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Pelotech (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Pelotech (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Pelotech (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Pelotech (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Pelotech (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Pelotech (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Pelotech (KCSP) validation commands or tests that are safe for the stated environment.
...
- Pelotech (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Pelotech (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Pelotech (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Pelotech (KCSP) as the basis for migration or production guidance.
