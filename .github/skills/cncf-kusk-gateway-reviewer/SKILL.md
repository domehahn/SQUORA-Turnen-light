---
name: "cncf-kusk-gateway-reviewer"
description: "Review Kusk Gateway using its official documentation and repository in the Orchestration & Management / API Gateway category. Kusk Gateway is a self-service API gateway powered by OpenAPI and Envoy. Kusk Gateway is built and maintained by Kubeshop."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kusk Gateway Reviewer

## Purpose

Review and operate Kusk Gateway using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kusk Gateway code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kusk Gateway requires evidence-backed assessment.
...
- First-party source: https://github.com/kubeshop/kusk-gateway
...
- [ ] Trace Kusk Gateway inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kusk Gateway defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kusk Gateway least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kusk Gateway resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kusk Gateway release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kusk Gateway dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kusk Gateway identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kusk Gateway change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kusk Gateway documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kusk Gateway architecture and dependency summary with trust boundaries and persistent state.
- Include Kusk Gateway validation commands or tests that are safe for the stated environment.
...
- Kusk Gateway identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kusk Gateway failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kusk Gateway is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kusk Gateway as the basis for migration or production guidance.
