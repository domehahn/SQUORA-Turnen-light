...
---
# Cncf Syseleven Metakube Reviewer

## Purpose

Review and operate SysEleven MetaKube using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SysEleven MetaKube code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SysEleven MetaKube requires evidence-backed assessment.
...
- First-party source: https://metakube.syseleven.de
...
- [ ] Trace SysEleven MetaKube inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SysEleven MetaKube defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SysEleven MetaKube least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SysEleven MetaKube resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SysEleven MetaKube release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SysEleven MetaKube dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SysEleven MetaKube identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SysEleven MetaKube change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SysEleven MetaKube documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SysEleven MetaKube architecture and dependency summary with trust boundaries and persistent state.
- Include SysEleven MetaKube validation commands or tests that are safe for the stated environment.
...
- SysEleven MetaKube identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SysEleven MetaKube failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SysEleven MetaKube is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SysEleven MetaKube as the basis for migration or production guidance.
