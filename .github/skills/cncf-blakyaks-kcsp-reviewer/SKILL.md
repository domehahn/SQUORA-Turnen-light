...
---
# Cncf Blakyaks Kcsp Reviewer

## Purpose

Review and operate BlakYaks (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- BlakYaks (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving BlakYaks (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.blakyaks.com/services/enterprise-container-platforms
...
- [ ] Trace BlakYaks (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check BlakYaks (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check BlakYaks (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check BlakYaks (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check BlakYaks (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check BlakYaks (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If BlakYaks (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a BlakYaks (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- BlakYaks (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a BlakYaks (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include BlakYaks (KCSP) validation commands or tests that are safe for the stated environment.
...
- BlakYaks (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- BlakYaks (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that BlakYaks (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of BlakYaks (KCSP) as the basis for migration or production guidance.
