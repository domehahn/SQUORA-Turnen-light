---
name: "cncf-xoap-reviewer"
description: "Review XOAP using its official documentation and repository in the Provisioning / Automation & Configuration category. Hybrid IT infrastructure and workplace automation platform"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate XOAP using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- XOAP code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving XOAP requires evidence-backed assessment.
...
- First-party source: https://xoap.io/
...
- [ ] Trace XOAP inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check XOAP defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check XOAP least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check XOAP resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check XOAP release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check XOAP dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If XOAP identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a XOAP change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- XOAP documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a XOAP architecture and dependency summary with trust boundaries and persistent state.
- Include XOAP validation commands or tests that are safe for the stated environment.
...
- XOAP identity, version, configuration, dependencies, and deployment assumptions are explicit.
- XOAP failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that XOAP is secure, supported, compatible, or suitable.
- Using an unpinned latest version of XOAP as the basis for migration or production guidance.
