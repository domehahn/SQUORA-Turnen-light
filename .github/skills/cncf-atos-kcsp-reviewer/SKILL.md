---
name: "cncf-atos-kcsp-reviewer"
description: "Review Atos (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Atos is a leading cloud native services provider, helping enterprises develop, deploy and manage applications using Kubernetes across various landing zones such as AWS, Azure, GCP or others European Cloud Service Providers."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Atos Kcsp Reviewer

## Purpose

Review and operate Atos (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Atos (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Atos (KCSP) requires evidence-backed assessment.
...
- First-party source: https://atos.net/en/lp/accelerate-your-digital-transformation-with-kubernetes-and-cloud-native-technologies
...
- [ ] Trace Atos (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Atos (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Atos (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Atos (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Atos (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Atos (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Atos (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Atos (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Atos (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Atos (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Atos (KCSP) validation commands or tests that are safe for the stated environment.
...
- Atos (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Atos (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Atos (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Atos (KCSP) as the basis for migration or production guidance.
