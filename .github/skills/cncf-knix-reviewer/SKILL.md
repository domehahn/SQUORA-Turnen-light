---
name: "cncf-knix-reviewer"
description: "Review Knix using its official documentation and repository in the Serverless / Installable Platform category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Knix using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Knix code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Knix requires evidence-backed assessment.
...
- First-party source: https://github.com/knix-microfunctions/knix
...
- [ ] Trace Knix inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Knix defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Knix least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Knix resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Knix release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Knix dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Knix identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Knix change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Knix documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Knix architecture and dependency summary with trust boundaries and persistent state.
- Include Knix validation commands or tests that are safe for the stated environment.
...
- Knix identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Knix failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Knix is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Knix as the basis for migration or production guidance.
