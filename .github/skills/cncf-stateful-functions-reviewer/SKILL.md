---
name: "cncf-stateful-functions-reviewer"
description: "Review Stateful Functions using its official documentation and repository in the Serverless / Framework category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Stateful Functions Reviewer

## Purpose

Review and operate Stateful Functions using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Stateful Functions code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Stateful Functions requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/flink-statefun
...
- [ ] Trace Stateful Functions inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Stateful Functions defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Stateful Functions least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Stateful Functions resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Stateful Functions release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Stateful Functions dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Stateful Functions identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Stateful Functions change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Stateful Functions documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Stateful Functions architecture and dependency summary with trust boundaries and persistent state.
- Include Stateful Functions validation commands or tests that are safe for the stated environment.
...
- Stateful Functions identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Stateful Functions failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Stateful Functions is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Stateful Functions as the basis for migration or production guidance.
