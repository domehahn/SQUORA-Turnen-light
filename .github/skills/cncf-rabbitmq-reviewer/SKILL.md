---
name: "cncf-rabbitmq-reviewer"
description: "Review RabbitMQ using its official documentation and repository in the App Definition and Development / Streaming & Messaging category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Rabbitmq Reviewer

## Purpose

Review and operate RabbitMQ using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- RabbitMQ code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving RabbitMQ requires evidence-backed assessment.
...
- First-party source: https://github.com/rabbitmq/rabbitmq-server
...
- [ ] Trace RabbitMQ inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check RabbitMQ defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check RabbitMQ least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check RabbitMQ resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check RabbitMQ release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check RabbitMQ dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If RabbitMQ identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a RabbitMQ change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- RabbitMQ documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a RabbitMQ architecture and dependency summary with trust boundaries and persistent state.
- Include RabbitMQ validation commands or tests that are safe for the stated environment.
...
- RabbitMQ identity, version, configuration, dependencies, and deployment assumptions are explicit.
- RabbitMQ failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that RabbitMQ is secure, supported, compatible, or suitable.
- Using an unpinned latest version of RabbitMQ as the basis for migration or production guidance.
