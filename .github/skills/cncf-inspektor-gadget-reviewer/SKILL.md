---
name: "cncf-inspektor-gadget-reviewer"
description: "Review Inspektor Gadget using its official documentation and repository in the Observability and Analysis / Observability category. Open source eBPF debugging and data collection tool for Kubernetes and Linux"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Inspektor Gadget Reviewer

## Purpose

Review and operate Inspektor Gadget using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Inspektor Gadget code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Inspektor Gadget requires evidence-backed assessment.
...
- First-party source: https://github.com/inspektor-gadget/inspektor-gadget
...
- [ ] Trace Inspektor Gadget inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Inspektor Gadget defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Inspektor Gadget least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Inspektor Gadget resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Inspektor Gadget release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Inspektor Gadget dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Inspektor Gadget identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Inspektor Gadget change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Inspektor Gadget documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Inspektor Gadget architecture and dependency summary with trust boundaries and persistent state.
- Include Inspektor Gadget validation commands or tests that are safe for the stated environment.
...
- Inspektor Gadget identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Inspektor Gadget failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Inspektor Gadget is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Inspektor Gadget as the basis for migration or production guidance.
