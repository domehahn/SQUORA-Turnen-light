---
name: "cncf-apache-camel-k-reviewer"
description: "Review Apache Camel K using its official documentation and repository in the Serverless / Installable Platform category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Apache Camel K Reviewer

## Purpose

Review and operate Apache Camel K using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Apache Camel K code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Apache Camel K requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/camel-k
...
- [ ] Trace Apache Camel K inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Apache Camel K defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Apache Camel K least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Apache Camel K resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Apache Camel K release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Apache Camel K dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Apache Camel K identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Apache Camel K change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Apache Camel K documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Apache Camel K architecture and dependency summary with trust boundaries and persistent state.
- Include Apache Camel K validation commands or tests that are safe for the stated environment.
...
- Apache Camel K identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Apache Camel K failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Apache Camel K is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Apache Camel K as the basis for migration or production guidance.
