---
name: "cncf-reactive-interaction-gateway-reviewer"
description: "Review Reactive Interaction Gateway using its official documentation and repository in the Orchestration & Management / API Gateway category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Reactive Interaction Gateway Reviewer

## Purpose

Review and operate Reactive Interaction Gateway using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Reactive Interaction Gateway code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Reactive Interaction Gateway requires evidence-backed assessment.
...
- CNCF classification: Orchestration & Management / API Gateway
- CNCF project maturity field: not specified
- First-party source: https://github.com/accenture/reactive-interaction-gateway
...
- [ ] Trace Reactive Interaction Gateway inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Reactive Interaction Gateway defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Reactive Interaction Gateway least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Reactive Interaction Gateway resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Reactive Interaction Gateway release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Reactive Interaction Gateway dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Reactive Interaction Gateway identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Reactive Interaction Gateway change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Reactive Interaction Gateway documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Reactive Interaction Gateway architecture and dependency summary with trust boundaries and persistent state.
- Include Reactive Interaction Gateway validation commands or tests that are safe for the stated environment.
...
- Reactive Interaction Gateway identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Reactive Interaction Gateway failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Reactive Interaction Gateway is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Reactive Interaction Gateway as the basis for migration or production guidance.
