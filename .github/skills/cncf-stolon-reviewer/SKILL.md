---
name: "cncf-stolon-reviewer"
description: "Review Stolon using its official documentation and repository in the App Definition and Development / Database category. Cloud Native PostgreSQL High Availability"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Stolon Reviewer

## Purpose

Review and operate Stolon using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Stolon code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Stolon requires evidence-backed assessment.
...
- First-party source: https://github.com/sorintlab/stolon
...
- [ ] Trace Stolon inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Stolon defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Stolon least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Stolon resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Stolon release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Stolon dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Stolon identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Stolon change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Stolon documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Stolon architecture and dependency summary with trust boundaries and persistent state.
- Include Stolon validation commands or tests that are safe for the stated environment.
...
- Stolon identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Stolon failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Stolon is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Stolon as the basis for migration or production guidance.
