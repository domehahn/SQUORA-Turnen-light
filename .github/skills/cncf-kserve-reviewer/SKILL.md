---
name: "cncf-kserve-reviewer"
description: "Review KServe using its official documentation and repository in the Inference / Framework category. Standardized Distributed Generative and Predictive AI Inference Platform for Scalable, Multi-Framework Deployment on Kubernetes"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kserve Reviewer

## Purpose

Review and operate KServe using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KServe code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KServe requires evidence-backed assessment.
...
- First-party source: https://github.com/kserve/kserve
...
- [ ] Trace KServe inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KServe defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KServe least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KServe resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KServe release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KServe dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KServe identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KServe change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KServe documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KServe architecture and dependency summary with trust boundaries and persistent state.
- Include KServe validation commands or tests that are safe for the stated environment.
...
- KServe identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KServe failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KServe is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KServe as the basis for migration or production guidance.
