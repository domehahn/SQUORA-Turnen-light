---
name: "cncf-deepshore-kcsp-reviewer"
description: "Review Deepshore (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Deepshore provides Kubernetes support in planning, installation and operating your container platform."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Deepshore Kcsp Reviewer

## Purpose

Review and operate Deepshore (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Deepshore (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Deepshore (KCSP) requires evidence-backed assessment.
...
- [ ] Trace Deepshore (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Deepshore (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Deepshore (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Deepshore (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Deepshore (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Deepshore (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Deepshore (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Deepshore (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Deepshore (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Deepshore (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Deepshore (KCSP) validation commands or tests that are safe for the stated environment.
...
- Deepshore (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Deepshore (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Deepshore (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Deepshore (KCSP) as the basis for migration or production guidance.
