---
name: "cncf-tarantool-reviewer"
description: "Review Tarantool using its official documentation and repository in the App Definition and Development / Database category. In-memory computing platform consisting of a database and an application server"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Tarantool Reviewer

## Purpose

Review and operate Tarantool using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Tarantool code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Tarantool requires evidence-backed assessment.
...
- First-party source: https://github.com/tarantool/tarantool
...
- [ ] Trace Tarantool inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Tarantool defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Tarantool least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Tarantool resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Tarantool release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Tarantool dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Tarantool identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Tarantool change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Tarantool documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Tarantool architecture and dependency summary with trust boundaries and persistent state.
- Include Tarantool validation commands or tests that are safe for the stated environment.
...
- Tarantool identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Tarantool failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Tarantool is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Tarantool as the basis for migration or production guidance.
