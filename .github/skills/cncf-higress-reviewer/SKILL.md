---
name: "cncf-higress-reviewer"
description: "Review Higress using its official documentation and repository in the Orchestration & Management / API Gateway category. An AI-native API gateway built on Envoy for ingress, microservice, and LLM traffic"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Higress Reviewer

## Purpose

Review and operate Higress using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Higress code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Higress requires evidence-backed assessment.
...
- First-party source: https://github.com/higress-group/higress
...
- [ ] Trace Higress inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Higress defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Higress least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Higress resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Higress release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Higress dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Higress identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Higress change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Higress documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Higress architecture and dependency summary with trust boundaries and persistent state.
- Include Higress validation commands or tests that are safe for the stated environment.
...
- Higress identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Higress failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Higress is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Higress as the basis for migration or production guidance.
