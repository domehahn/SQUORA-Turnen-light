---
name: "cncf-tracee-reviewer"
description: "Review Tracee using its official documentation and repository in the Provisioning / Security & Compliance category. Tracee helps you understand your systems and applications behavior, and apply runtime security to them. It uses Linux eBPF technology and strong security techniques to instrument the system and expose relevant events for you to consume and process."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Tracee Reviewer

## Purpose

Review and operate Tracee using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Tracee code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Tracee requires evidence-backed assessment.
...
- First-party source: https://github.com/aquasecurity/tracee
...
- [ ] Trace Tracee inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Tracee defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Tracee least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Tracee resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Tracee release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Tracee dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Tracee identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Tracee change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Tracee documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Tracee architecture and dependency summary with trust boundaries and persistent state.
- Include Tracee validation commands or tests that are safe for the stated environment.
...
- Tracee identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Tracee failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Tracee is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Tracee as the basis for migration or production guidance.
