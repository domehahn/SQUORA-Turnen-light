---
name: "cncf-switch-cloud-kubernetes-sck-reviewer"
description: "Review Switch Cloud Kubernetes (SCK) using its official documentation and repository in the Platform / Certified Kubernetes - Hosted category. Switch Cloud Kubernetes (SCK) is a Kubernetes as a Service platform for the education, research and innovation landscape in Switzerland."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Switch Cloud Kubernetes Sck Reviewer

## Purpose

Review and operate Switch Cloud Kubernetes (SCK) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Switch Cloud Kubernetes (SCK) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Switch Cloud Kubernetes (SCK) requires evidence-backed assessment.
...
- First-party source: https://www.switch.ch/en/competencies/cloud
...
- [ ] Trace Switch Cloud Kubernetes (SCK) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Switch Cloud Kubernetes (SCK) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Switch Cloud Kubernetes (SCK) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Switch Cloud Kubernetes (SCK) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Switch Cloud Kubernetes (SCK) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Switch Cloud Kubernetes (SCK) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Switch Cloud Kubernetes (SCK) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Switch Cloud Kubernetes (SCK) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Switch Cloud Kubernetes (SCK) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Switch Cloud Kubernetes (SCK) architecture and dependency summary with trust boundaries and persistent state.
- Include Switch Cloud Kubernetes (SCK) validation commands or tests that are safe for the stated environment.
...
- Switch Cloud Kubernetes (SCK) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Switch Cloud Kubernetes (SCK) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Switch Cloud Kubernetes (SCK) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Switch Cloud Kubernetes (SCK) as the basis for migration or production guidance.
