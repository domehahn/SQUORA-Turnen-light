...
---
# Cncf Kubescape Reviewer

## Purpose

Review and operate Kubescape using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kubescape code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kubescape requires evidence-backed assessment.
...
- First-party source: https://github.com/kubescape/kubescape
...
- [ ] Trace Kubescape inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kubescape defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kubescape least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kubescape resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kubescape release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kubescape dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kubescape identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kubescape change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kubescape documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kubescape architecture and dependency summary with trust boundaries and persistent state.
- Include Kubescape validation commands or tests that are safe for the stated environment.
...
- Kubescape identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kubescape failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kubescape is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kubescape as the basis for migration or production guidance.
