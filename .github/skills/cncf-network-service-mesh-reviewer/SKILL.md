---
name: "cncf-network-service-mesh-reviewer"
description: "Review Network Service Mesh using its official documentation and repository in the Runtime / Cloud Native Network category. The Hybrid/Multi-cloud IP Service Mesh"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Network Service Mesh Reviewer

## Purpose

Review and operate Network Service Mesh using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Network Service Mesh code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Network Service Mesh requires evidence-backed assessment.
...
- First-party source: https://github.com/networkservicemesh/api
...
- [ ] Trace Network Service Mesh inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Network Service Mesh defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Network Service Mesh least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Network Service Mesh resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Network Service Mesh release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Network Service Mesh dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Network Service Mesh identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Network Service Mesh change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Network Service Mesh documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Network Service Mesh architecture and dependency summary with trust boundaries and persistent state.
- Include Network Service Mesh validation commands or tests that are safe for the stated environment.
...
- Network Service Mesh identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Network Service Mesh failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Network Service Mesh is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Network Service Mesh as the basis for migration or production guidance.
