---
name: "cncf-oauth2-proxy-reviewer"
description: "Review OAuth2 Proxy using its official documentation and repository in the Provisioning / Security & Compliance category. A generic reverse proxy that provides authentication with Google, Azure, OpenID Connect and many more identity providers."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Oauth2 Proxy Reviewer

## Purpose

Review and operate OAuth2 Proxy using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OAuth2 Proxy code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OAuth2 Proxy requires evidence-backed assessment.
...
- First-party source: https://github.com/oauth2-proxy/oauth2-proxy
...
- [ ] Trace OAuth2 Proxy inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OAuth2 Proxy defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OAuth2 Proxy least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OAuth2 Proxy resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OAuth2 Proxy release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OAuth2 Proxy dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OAuth2 Proxy identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OAuth2 Proxy change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OAuth2 Proxy documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OAuth2 Proxy architecture and dependency summary with trust boundaries and persistent state.
- Include OAuth2 Proxy validation commands or tests that are safe for the stated environment.
...
- OAuth2 Proxy identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OAuth2 Proxy failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OAuth2 Proxy is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OAuth2 Proxy as the basis for migration or production guidance.
