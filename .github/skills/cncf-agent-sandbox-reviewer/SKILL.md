---
name: "cncf-agent-sandbox-reviewer"
description: "Review agent-sandbox using its official documentation and repository in the AI Native Infra / Orchestration and Scheduling category. agent-sandbox enables easy management of isolated, stateful, singleton workloads, ideal for use cases like AI agent runtimes."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Agent Sandbox Reviewer

## Purpose

Review and operate agent-sandbox using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- agent-sandbox code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving agent-sandbox requires evidence-backed assessment.
...
- CNCF classification: AI Native Infra / Orchestration and Scheduling
- CNCF project maturity field: not specified
- First-party source: https://github.com/kubernetes-sigs/agent-sandbox
...
- [ ] Trace agent-sandbox inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check agent-sandbox defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check agent-sandbox least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check agent-sandbox resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check agent-sandbox release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check agent-sandbox dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If agent-sandbox identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a agent-sandbox change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- agent-sandbox documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a agent-sandbox architecture and dependency summary with trust boundaries and persistent state.
- Include agent-sandbox validation commands or tests that are safe for the stated environment.
...
- agent-sandbox identity, version, configuration, dependencies, and deployment assumptions are explicit.
- agent-sandbox failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that agent-sandbox is secure, supported, compatible, or suitable.
- Using an unpinned latest version of agent-sandbox as the basis for migration or production guidance.
