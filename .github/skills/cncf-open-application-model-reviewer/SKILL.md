---
name: "cncf-open-application-model-reviewer"
description: "Review Open Application Model using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Open Application Model Reviewer

## Purpose

Review and operate Open Application Model using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Open Application Model code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Open Application Model requires evidence-backed assessment.
...
- First-party source: https://github.com/oam-dev/spec
...
- [ ] Trace Open Application Model inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Open Application Model defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Open Application Model least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Open Application Model resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Open Application Model release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Open Application Model dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Open Application Model identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Open Application Model change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Open Application Model documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Open Application Model architecture and dependency summary with trust boundaries and persistent state.
- Include Open Application Model validation commands or tests that are safe for the stated environment.
...
- Open Application Model identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Open Application Model failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Open Application Model is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Open Application Model as the basis for migration or production guidance.
