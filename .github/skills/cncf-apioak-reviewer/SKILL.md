---
name: "cncf-apioak-reviewer"
description: "Review APIOAK using its official documentation and repository in the Orchestration & Management / API Gateway category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Apioak Reviewer

## Purpose

Review and operate APIOAK using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- APIOAK code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving APIOAK requires evidence-backed assessment.
...
- First-party source: https://github.com/apioak/apioak
...
- [ ] Trace APIOAK inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check APIOAK defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check APIOAK least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check APIOAK resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check APIOAK release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check APIOAK dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If APIOAK identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a APIOAK change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- APIOAK documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a APIOAK architecture and dependency summary with trust boundaries and persistent state.
- Include APIOAK validation commands or tests that are safe for the stated environment.
...
- APIOAK identity, version, configuration, dependencies, and deployment assumptions are explicit.
- APIOAK failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that APIOAK is secure, supported, compatible, or suitable.
- Using an unpinned latest version of APIOAK as the basis for migration or production guidance.
