...
Review and operate FOSSA using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- FOSSA code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving FOSSA requires evidence-backed assessment.
...
- First-party source: https://github.com/fossas/fossa-cli
...
- [ ] Trace FOSSA inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check FOSSA defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check FOSSA least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check FOSSA resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check FOSSA release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check FOSSA dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If FOSSA identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a FOSSA change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- FOSSA documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a FOSSA architecture and dependency summary with trust boundaries and persistent state.
- Include FOSSA validation commands or tests that are safe for the stated environment.
...
- FOSSA identity, version, configuration, dependencies, and deployment assumptions are explicit.
- FOSSA failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that FOSSA is secure, supported, compatible, or suitable.
- Using an unpinned latest version of FOSSA as the basis for migration or production guidance.
