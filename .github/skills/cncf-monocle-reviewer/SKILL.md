---
name: "cncf-monocle-reviewer"
description: "Review Monocle using its official documentation and repository in the Observability and Analysis / Observability category. The goal of the open source Monocle project is to help GenAI developers trace their applications. Hosted in incubation as a Sandbox project in LF AI & Data."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Monocle Reviewer

## Purpose

Review and operate Monocle using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Monocle code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Monocle requires evidence-backed assessment.
...
- First-party source: https://github.com/monocle2ai/monocle
...
- [ ] Trace Monocle inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Monocle defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Monocle least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Monocle resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Monocle release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Monocle dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Monocle identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Monocle change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Monocle documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Monocle architecture and dependency summary with trust boundaries and persistent state.
- Include Monocle validation commands or tests that are safe for the stated environment.
...
- Monocle identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Monocle failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Monocle is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Monocle as the basis for migration or production guidance.
