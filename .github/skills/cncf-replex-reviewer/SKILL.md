...
---
# Cncf Replex Reviewer

## Purpose

Review and operate Replex using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Replex code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Replex requires evidence-backed assessment.
...
- First-party source: https://www.replex.io/
...
- [ ] Trace Replex inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Replex defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Replex least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Replex resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Replex release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Replex dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Replex identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Replex change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Replex documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Replex architecture and dependency summary with trust boundaries and persistent state.
- Include Replex validation commands or tests that are safe for the stated environment.
...
- Replex identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Replex failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Replex is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Replex as the basis for migration or production guidance.
