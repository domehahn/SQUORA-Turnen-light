...
Review and operate TinyGo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- TinyGo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving TinyGo requires evidence-backed assessment.
...
- First-party source: https://github.com/tinygo-org/tinygo
...
- [ ] Trace TinyGo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check TinyGo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check TinyGo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check TinyGo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check TinyGo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check TinyGo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If TinyGo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a TinyGo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- TinyGo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a TinyGo architecture and dependency summary with trust boundaries and persistent state.
- Include TinyGo validation commands or tests that are safe for the stated environment.
...
- TinyGo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- TinyGo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that TinyGo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of TinyGo as the basis for migration or production guidance.
