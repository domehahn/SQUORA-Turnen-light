---
name: "cncf-wabt-reviewer"
description: "Review Wabt using its official documentation and repository in the Wasm / Tooling category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Wabt using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Wabt code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Wabt requires evidence-backed assessment.
...
- [ ] Trace Wabt inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Wabt defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Wabt least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Wabt resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Wabt release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Wabt dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Wabt identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Wabt change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Wabt documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Wabt architecture and dependency summary with trust boundaries and persistent state.
- Include Wabt validation commands or tests that are safe for the stated environment.
...
- Wabt identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Wabt failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Wabt is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Wabt as the basis for migration or production guidance.
