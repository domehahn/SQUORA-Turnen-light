---
name: "cncf-dex-reviewer"
description: "Review Dex using its official documentation and repository in the Provisioning / Security & Compliance category. OpenID Connect (OIDC) identity and OAuth 2.0 provider with pluggable connectors"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Dex using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Dex code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Dex requires evidence-backed assessment.
...
- First-party source: https://github.com/dexidp/dex
...
- [ ] Trace Dex inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Dex defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Dex least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Dex resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Dex release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Dex dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Dex identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Dex change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Dex documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Dex architecture and dependency summary with trust boundaries and persistent state.
- Include Dex validation commands or tests that are safe for the stated environment.
...
- Dex identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Dex failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Dex is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Dex as the basis for migration or production guidance.
