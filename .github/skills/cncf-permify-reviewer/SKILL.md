---
name: "cncf-permify-reviewer"
description: "Review Permify using its official documentation and repository in the Provisioning / Security & Compliance category. Permify is an open-source authorization as a service inspired by Google Zanzibar, designed to build and manage fine-grained and scalable authorization systems for any application."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Permify Reviewer

## Purpose

Review and operate Permify using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Permify code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Permify requires evidence-backed assessment.
...
- First-party source: https://github.com/Permify/permify
...
- [ ] Trace Permify inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Permify defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Permify least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Permify resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Permify release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Permify dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Permify identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Permify change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Permify documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Permify architecture and dependency summary with trust boundaries and persistent state.
- Include Permify validation commands or tests that are safe for the stated environment.
...
- Permify identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Permify failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Permify is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Permify as the basis for migration or production guidance.
