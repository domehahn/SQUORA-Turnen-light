---
name: "cncf-apache-flink-reviewer"
description: "Review Apache Flink using its official documentation and repository in the Data / Data Architecture category. Open source stream processing framework with powerful stream- and batch-processing capabilities."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Apache Flink Reviewer

## Purpose

Review and operate Apache Flink using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Apache Flink code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Apache Flink requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/flink
...
- [ ] Trace Apache Flink inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Apache Flink defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Apache Flink least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Apache Flink resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Apache Flink release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Apache Flink dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Apache Flink identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Apache Flink change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Apache Flink documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Apache Flink architecture and dependency summary with trust boundaries and persistent state.
- Include Apache Flink validation commands or tests that are safe for the stated environment.
...
- Apache Flink identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Apache Flink failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Apache Flink is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Apache Flink as the basis for migration or production guidance.
