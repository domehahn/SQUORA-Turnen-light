---
name: "cncf-truefullstaq-kcntp-reviewer"
...
Review and operate TrueFullstaq (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- TrueFullstaq (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving TrueFullstaq (KCNTP) requires evidence-backed assessment.
...
- First-party source: https://www.truefullstaq.com/en/training 
...
- [ ] Trace TrueFullstaq (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check TrueFullstaq (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check TrueFullstaq (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check TrueFullstaq (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check TrueFullstaq (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check TrueFullstaq (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If TrueFullstaq (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a TrueFullstaq (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- TrueFullstaq (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a TrueFullstaq (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include TrueFullstaq (KCNTP) validation commands or tests that are safe for the stated environment.
...
- TrueFullstaq (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- TrueFullstaq (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that TrueFullstaq (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of TrueFullstaq (KCNTP) as the basis for migration or production guidance.
