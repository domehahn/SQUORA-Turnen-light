---
name: "cncf-nexclipper-reviewer"
description: "Review NexClipper using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Nexclipper Reviewer

## Purpose

Review and operate NexClipper using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- NexClipper code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving NexClipper requires evidence-backed assessment.
...
- First-party source: https://github.com/NexClipper/NexClipper
...
- [ ] Trace NexClipper inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check NexClipper defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check NexClipper least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check NexClipper resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check NexClipper release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check NexClipper dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If NexClipper identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a NexClipper change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- NexClipper documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a NexClipper architecture and dependency summary with trust boundaries and persistent state.
- Include NexClipper validation commands or tests that are safe for the stated environment.
...
- NexClipper identity, version, configuration, dependencies, and deployment assumptions are explicit.
- NexClipper failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that NexClipper is secure, supported, compatible, or suitable.
- Using an unpinned latest version of NexClipper as the basis for migration or production guidance.
