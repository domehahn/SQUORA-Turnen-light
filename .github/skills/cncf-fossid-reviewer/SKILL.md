---
name: "cncf-fossid-reviewer"
description: "Review FOSSID using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Fossid Reviewer

## Purpose

Review and operate FOSSID using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- FOSSID code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving FOSSID requires evidence-backed assessment.
...
- First-party source: https://www.fossid.com/
...
- [ ] Trace FOSSID inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check FOSSID defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check FOSSID least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check FOSSID resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check FOSSID release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check FOSSID dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If FOSSID identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a FOSSID change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- FOSSID documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a FOSSID architecture and dependency summary with trust boundaries and persistent state.
- Include FOSSID validation commands or tests that are safe for the stated environment.
...
- FOSSID identity, version, configuration, dependencies, and deployment assumptions are explicit.
- FOSSID failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that FOSSID is secure, supported, compatible, or suitable.
- Using an unpinned latest version of FOSSID as the basis for migration or production guidance.
