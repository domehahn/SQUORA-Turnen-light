---
name: "cncf-avisi-ame-reviewer"
description: "Review Avisi AME using its official documentation and repository in the Platform / Certified Kubernetes - Hosted category. AME provides a managed Kubernetes platform that delivers seamless, easy, and secure production-ready cluster management across public, private, and hybrid cloud environments."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Avisi Ame Reviewer

## Purpose

Review and operate Avisi AME using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Avisi AME code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Avisi AME requires evidence-backed assessment.
...
- First-party source: https://docs.avisi.cloud
...
- [ ] Trace Avisi AME inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Avisi AME defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Avisi AME least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Avisi AME resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Avisi AME release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Avisi AME dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Avisi AME identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Avisi AME change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Avisi AME documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Avisi AME architecture and dependency summary with trust boundaries and persistent state.
- Include Avisi AME validation commands or tests that are safe for the stated environment.
...
- Avisi AME identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Avisi AME failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Avisi AME is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Avisi AME as the basis for migration or production guidance.
