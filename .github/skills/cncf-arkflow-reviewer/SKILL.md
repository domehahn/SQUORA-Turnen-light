---
name: "cncf-arkflow-reviewer"
description: "Review ArkFlow using its official documentation and repository in the App Definition and Development / Streaming & Messaging category. High-performance Rust stream processing engine, providing powerful data stream processing capabilities, supporting multiple input/output sources and processors."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate ArkFlow using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ArkFlow code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ArkFlow requires evidence-backed assessment.
...
- First-party source: https://github.com/arkflow-rs/arkflow
...
- [ ] Trace ArkFlow inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ArkFlow defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ArkFlow least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ArkFlow resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ArkFlow release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ArkFlow dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ArkFlow identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ArkFlow change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ArkFlow documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ArkFlow architecture and dependency summary with trust boundaries and persistent state.
- Include ArkFlow validation commands or tests that are safe for the stated environment.
...
- ArkFlow identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ArkFlow failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ArkFlow is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ArkFlow as the basis for migration or production guidance.
