---
name: "cncf-distribution-reviewer"
description: "Review Distribution using its official documentation and repository in the Provisioning / Container Registry category. The toolkit to pack, ship, store, and deliver container content"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Distribution Reviewer

## Purpose

Review and operate Distribution using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Distribution code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Distribution requires evidence-backed assessment.
...
- First-party source: https://github.com/distribution/distribution
...
- [ ] Trace Distribution inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Distribution defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Distribution least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Distribution resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Distribution release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Distribution dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Distribution identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Distribution change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Distribution documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Distribution architecture and dependency summary with trust boundaries and persistent state.
- Include Distribution validation commands or tests that are safe for the stated environment.
...
- Distribution identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Distribution failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Distribution is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Distribution as the basis for migration or production guidance.
