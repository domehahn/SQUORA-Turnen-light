---
name: "cncf-sigstore-reviewer"
description: "Review sigstore using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Sigstore Reviewer

## Purpose

Review and operate sigstore using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- sigstore code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving sigstore requires evidence-backed assessment.
...
- First-party source: https://github.com/sigstore/sigstore
...
- [ ] Trace sigstore inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check sigstore defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check sigstore least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check sigstore resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check sigstore release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check sigstore dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If sigstore identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a sigstore change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- sigstore documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a sigstore architecture and dependency summary with trust boundaries and persistent state.
- Include sigstore validation commands or tests that are safe for the stated environment.
...
- sigstore identity, version, configuration, dependencies, and deployment assumptions are explicit.
- sigstore failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that sigstore is secure, supported, compatible, or suitable.
- Using an unpinned latest version of sigstore as the basis for migration or production guidance.
