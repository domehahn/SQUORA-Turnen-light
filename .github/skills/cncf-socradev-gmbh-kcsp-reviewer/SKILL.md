---
name: "cncf-socradev-gmbh-kcsp-reviewer"
description: "Review socradev Gmbh (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. We provide expert consulting on Kubernetes and the broader Kubernetes ecosystem, helping you implement scalable, secure, and automated infrastructure solutions tailored to your needs."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Socradev Gmbh Kcsp Reviewer

## Purpose

Review and operate socradev Gmbh (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- socradev Gmbh (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving socradev Gmbh (KCSP) requires evidence-backed assessment.
...
- First-party source: https://socra.dev/kubernetes-und-it-infrastruktur/
...
- [ ] Trace socradev Gmbh (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check socradev Gmbh (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check socradev Gmbh (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check socradev Gmbh (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check socradev Gmbh (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check socradev Gmbh (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If socradev Gmbh (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a socradev Gmbh (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- socradev Gmbh (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a socradev Gmbh (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include socradev Gmbh (KCSP) validation commands or tests that are safe for the stated environment.
...
- socradev Gmbh (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- socradev Gmbh (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that socradev Gmbh (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of socradev Gmbh (KCSP) as the basis for migration or production guidance.
