---
name: "cncf-cumulus-reviewer"
description: "Review Cumulus using its official documentation and repository in the Runtime / Cloud Native Network category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Cumulus Reviewer

## Purpose

Review and operate Cumulus using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Cumulus code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Cumulus requires evidence-backed assessment.
...
- First-party source: https://www.nvidia.com/en-us/networking/ethernet-switching
...
- [ ] Trace Cumulus inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Cumulus defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Cumulus least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Cumulus resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Cumulus release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Cumulus dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Cumulus identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Cumulus change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Cumulus documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Cumulus architecture and dependency summary with trust boundaries and persistent state.
- Include Cumulus validation commands or tests that are safe for the stated environment.
...
- Cumulus identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Cumulus failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Cumulus is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Cumulus as the basis for migration or production guidance.
