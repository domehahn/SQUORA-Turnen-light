---
name: "cncf-lumigo-reviewer"
description: "Review Lumigo using its official documentation and repository in the Serverless / Tools category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Lumigo Reviewer

## Purpose

Review and operate Lumigo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Lumigo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Lumigo requires evidence-backed assessment.
...
- First-party source: https://lumigo.io/
...
- [ ] Trace Lumigo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Lumigo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Lumigo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Lumigo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Lumigo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Lumigo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Lumigo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Lumigo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Lumigo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Lumigo architecture and dependency summary with trust boundaries and persistent state.
- Include Lumigo validation commands or tests that are safe for the stated environment.
...
- Lumigo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Lumigo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Lumigo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Lumigo as the basis for migration or production guidance.
