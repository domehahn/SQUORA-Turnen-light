---
name: "cncf-cloudevents-reviewer"
description: "Review CloudEvents using its official documentation and repository in the App Definition and Development / Streaming & Messaging category. Standardizing common eventing metadata and their location to help with event identification and routing."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Cloudevents Reviewer

## Purpose

Review and operate CloudEvents using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CloudEvents code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CloudEvents requires evidence-backed assessment.
...
- First-party source: https://github.com/cloudevents/spec
...
- [ ] Trace CloudEvents inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CloudEvents defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CloudEvents least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CloudEvents resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CloudEvents release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CloudEvents dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CloudEvents identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CloudEvents change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CloudEvents documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CloudEvents architecture and dependency summary with trust boundaries and persistent state.
- Include CloudEvents validation commands or tests that are safe for the stated environment.
...
- CloudEvents identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CloudEvents failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CloudEvents is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CloudEvents as the basis for migration or production guidance.
