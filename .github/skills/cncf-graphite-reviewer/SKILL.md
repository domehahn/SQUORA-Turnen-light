---
name: "cncf-graphite-reviewer"
description: "Review Graphite using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Graphite Reviewer

## Purpose

Review and operate Graphite using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Graphite code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Graphite requires evidence-backed assessment.
...
- First-party source: https://github.com/graphite-project/graphite-web
...
- [ ] Trace Graphite inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Graphite defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Graphite least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Graphite resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Graphite release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Graphite dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Graphite identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Graphite change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Graphite documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Graphite architecture and dependency summary with trust boundaries and persistent state.
- Include Graphite validation commands or tests that are safe for the stated environment.
...
- Graphite identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Graphite failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Graphite is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Graphite as the basis for migration or production guidance.
