---
name: "cncf-prodyna-kcsp-reviewer"
description: "Review PRODYNA (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. PRODYNA designs, implements, and operates Kubernetes-based custom applications for mid- to large size enterprises in all of Europe."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Prodyna Kcsp Reviewer

## Purpose

Review and operate PRODYNA (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- PRODYNA (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving PRODYNA (KCSP) requires evidence-backed assessment.
...
- [ ] Trace PRODYNA (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check PRODYNA (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check PRODYNA (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check PRODYNA (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check PRODYNA (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check PRODYNA (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If PRODYNA (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a PRODYNA (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- PRODYNA (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a PRODYNA (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include PRODYNA (KCSP) validation commands or tests that are safe for the stated environment.
...
- PRODYNA (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- PRODYNA (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that PRODYNA (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of PRODYNA (KCSP) as the basis for migration or production guidance.
