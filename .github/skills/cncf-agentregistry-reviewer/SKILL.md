...
---
# Cncf Agentregistry Reviewer

## Purpose

Review and operate agentregistry using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- agentregistry code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving agentregistry requires evidence-backed assessment.
...
- First-party source: https://github.com/agentregistry-dev/agentregistry
...
- [ ] Trace agentregistry inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check agentregistry defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check agentregistry least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check agentregistry resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check agentregistry release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check agentregistry dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If agentregistry identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a agentregistry change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- agentregistry documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a agentregistry architecture and dependency summary with trust boundaries and persistent state.
- Include agentregistry validation commands or tests that are safe for the stated environment.
...
- agentregistry identity, version, configuration, dependencies, and deployment assumptions are explicit.
- agentregistry failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that agentregistry is secure, supported, compatible, or suitable.
- Using an unpinned latest version of agentregistry as the basis for migration or production guidance.
