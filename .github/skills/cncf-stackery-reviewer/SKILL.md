---
name: "cncf-stackery-reviewer"
description: "Review Stackery using its official documentation and repository in the Serverless / Tools category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Stackery Reviewer

## Purpose

Review and operate Stackery using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Stackery code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Stackery requires evidence-backed assessment.
...
- First-party source: https://www.stackery.io/
...
- [ ] Trace Stackery inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Stackery defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Stackery least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Stackery resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Stackery release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Stackery dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Stackery identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Stackery change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Stackery documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Stackery architecture and dependency summary with trust boundaries and persistent state.
- Include Stackery validation commands or tests that are safe for the stated environment.
...
- Stackery identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Stackery failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Stackery is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Stackery as the basis for migration or production guidance.
