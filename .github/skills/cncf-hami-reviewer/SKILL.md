---
name: "cncf-hami-reviewer"
description: "Review HAMi using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. Heterogeneous AI Computing Virtualization Middleware"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate HAMi using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- HAMi code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving HAMi requires evidence-backed assessment.
...
- First-party source: https://github.com/Project-HAMi/HAMi
...
- [ ] Trace HAMi inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check HAMi defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check HAMi least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check HAMi resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check HAMi release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check HAMi dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If HAMi identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a HAMi change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- HAMi documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a HAMi architecture and dependency summary with trust boundaries and persistent state.
- Include HAMi validation commands or tests that are safe for the stated environment.
...
- HAMi identity, version, configuration, dependencies, and deployment assumptions are explicit.
- HAMi failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that HAMi is secure, supported, compatible, or suitable.
- Using an unpinned latest version of HAMi as the basis for migration or production guidance.
