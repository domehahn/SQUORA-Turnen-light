---
name: "cncf-connect-rpc-reviewer"
description: "Review Connect RPC using its official documentation and repository in the Orchestration & Management / Remote Procedure Call category. Connect is a family of libraries for building browser and gRPC-compatible HTTP APIs."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Connect Rpc Reviewer

## Purpose

Review and operate Connect RPC using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Connect RPC code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Connect RPC requires evidence-backed assessment.
...
- First-party source: https://github.com/connectrpc/connect-go
...
- [ ] Trace Connect RPC inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Connect RPC defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Connect RPC least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Connect RPC resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Connect RPC release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Connect RPC dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Connect RPC identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Connect RPC change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Connect RPC documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Connect RPC architecture and dependency summary with trust boundaries and persistent state.
- Include Connect RPC validation commands or tests that are safe for the stated environment.
...
- Connect RPC identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Connect RPC failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Connect RPC is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Connect RPC as the basis for migration or production guidance.
