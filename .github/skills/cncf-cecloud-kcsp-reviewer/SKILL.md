---
name: "cncf-cecloud-kcsp-reviewer"
description: "Review CECloud (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. CEcloud helps customers with cloud native best practices for modernizing applications."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Cecloud Kcsp Reviewer

## Purpose

Review and operate CECloud (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CECloud (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CECloud (KCSP) requires evidence-backed assessment.
...
- First-party source: https://cecloud.com/dynamic//product/ceapaas.html
...
- [ ] Trace CECloud (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CECloud (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CECloud (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CECloud (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CECloud (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CECloud (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CECloud (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CECloud (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CECloud (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CECloud (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include CECloud (KCSP) validation commands or tests that are safe for the stated environment.
...
- CECloud (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CECloud (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CECloud (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CECloud (KCSP) as the basis for migration or production guidance.
