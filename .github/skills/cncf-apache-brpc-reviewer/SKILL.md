---
name: "cncf-apache-brpc-reviewer"
description: "Review Apache bRPC using its official documentation and repository in the Orchestration & Management / Remote Procedure Call category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Apache Brpc Reviewer

## Purpose

Review and operate Apache bRPC using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Apache bRPC code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Apache bRPC requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/brpc
...
- [ ] Trace Apache bRPC inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Apache bRPC defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Apache bRPC least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Apache bRPC resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Apache bRPC release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Apache bRPC dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Apache bRPC identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Apache bRPC change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Apache bRPC documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Apache bRPC architecture and dependency summary with trust boundaries and persistent state.
- Include Apache bRPC validation commands or tests that are safe for the stated environment.
...
- Apache bRPC identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Apache bRPC failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Apache bRPC is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Apache bRPC as the basis for migration or production guidance.
