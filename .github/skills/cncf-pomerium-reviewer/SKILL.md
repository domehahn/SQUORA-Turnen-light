---
name: "cncf-pomerium-reviewer"
description: "Review Pomerium using its official documentation and repository in the Provisioning / Security & Compliance category. Pomerium is an identity-aware access proxy that enables teams to implement true zero-trust security, eliminating the need for a VPN. Pomerium ensures that every single call is authenticated and authorized, providing a secure and seamless user experience."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Pomerium Reviewer

## Purpose

Review and operate Pomerium using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Pomerium code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Pomerium requires evidence-backed assessment.
...
- First-party source: https://github.com/pomerium/pomerium
...
- [ ] Trace Pomerium inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Pomerium defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Pomerium least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Pomerium resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Pomerium release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Pomerium dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Pomerium identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Pomerium change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Pomerium documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Pomerium architecture and dependency summary with trust boundaries and persistent state.
- Include Pomerium validation commands or tests that are safe for the stated environment.
...
- Pomerium identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Pomerium failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Pomerium is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Pomerium as the basis for migration or production guidance.
