...
---
# Cncf Effectual Kcsp Reviewer

## Purpose

Review and operate Effectual (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Effectual (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Effectual (KCSP) requires evidence-backed assessment.
...
- First-party source: https://effectual.com/kubernetes/
...
- [ ] Trace Effectual (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Effectual (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Effectual (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Effectual (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Effectual (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Effectual (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Effectual (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Effectual (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Effectual (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Effectual (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Effectual (KCSP) validation commands or tests that are safe for the stated environment.
...
- Effectual (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Effectual (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Effectual (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Effectual (KCSP) as the basis for migration or production guidance.
