...
---
# Cncf Github Actions Reviewer

## Purpose

Review and operate GitHub Actions using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- GitHub Actions code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving GitHub Actions requires evidence-backed assessment.
...
- First-party source: https://github.com/features/actions
...
- [ ] Trace GitHub Actions inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check GitHub Actions defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check GitHub Actions least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check GitHub Actions resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check GitHub Actions release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check GitHub Actions dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If GitHub Actions identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a GitHub Actions change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- GitHub Actions documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a GitHub Actions architecture and dependency summary with trust boundaries and persistent state.
- Include GitHub Actions validation commands or tests that are safe for the stated environment.
...
- GitHub Actions identity, version, configuration, dependencies, and deployment assumptions are explicit.
- GitHub Actions failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that GitHub Actions is secure, supported, compatible, or suitable.
- Using an unpinned latest version of GitHub Actions as the basis for migration or production guidance.
