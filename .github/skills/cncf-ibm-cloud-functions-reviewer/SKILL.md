---
name: "cncf-ibm-cloud-functions-reviewer"
description: "Review IBM Cloud Functions using its official documentation and repository in the Serverless / Hosted Platform category. Run your application code without servers, scale it automatically, and pay nothing when it's not in use."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Ibm Cloud Functions Reviewer

## Purpose

Review and operate IBM Cloud Functions using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- IBM Cloud Functions code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving IBM Cloud Functions requires evidence-backed assessment.
...
- First-party source: https://cloud.ibm.com/functions/
...
- [ ] Trace IBM Cloud Functions inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check IBM Cloud Functions defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check IBM Cloud Functions least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check IBM Cloud Functions resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check IBM Cloud Functions release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check IBM Cloud Functions dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If IBM Cloud Functions identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a IBM Cloud Functions change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- IBM Cloud Functions documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a IBM Cloud Functions architecture and dependency summary with trust boundaries and persistent state.
- Include IBM Cloud Functions validation commands or tests that are safe for the stated environment.
...
- IBM Cloud Functions identity, version, configuration, dependencies, and deployment assumptions are explicit.
- IBM Cloud Functions failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that IBM Cloud Functions is secure, supported, compatible, or suitable.
- Using an unpinned latest version of IBM Cloud Functions as the basis for migration or production guidance.
