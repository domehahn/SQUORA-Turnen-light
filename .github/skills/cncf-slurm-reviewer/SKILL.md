---
name: "cncf-slurm-reviewer"
description: "Review Slurm using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. Slurm is an open source, fault-tolerant, and highly scalable cluster management and job scheduling system for large and small Linux clusters."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Slurm using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Slurm code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Slurm requires evidence-backed assessment.
...
- First-party source: https://github.com/SchedMD/slurm
...
- [ ] Trace Slurm inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Slurm defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Slurm least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Slurm resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Slurm release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Slurm dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Slurm identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Slurm change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Slurm documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Slurm architecture and dependency summary with trust boundaries and persistent state.
- Include Slurm validation commands or tests that are safe for the stated environment.
...
- Slurm identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Slurm failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Slurm is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Slurm as the basis for migration or production guidance.
