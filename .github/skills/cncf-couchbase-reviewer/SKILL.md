---
name: "cncf-couchbase-reviewer"
description: "Review Couchbase using its official documentation and repository in the App Definition and Development / Database category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Couchbase Reviewer

## Purpose

Review and operate Couchbase using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Couchbase code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Couchbase requires evidence-backed assessment.
...
- First-party source: https://github.com/couchbase/manifest
...
- [ ] Trace Couchbase inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Couchbase defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Couchbase least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Couchbase resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Couchbase release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Couchbase dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Couchbase identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Couchbase change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Couchbase documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Couchbase architecture and dependency summary with trust boundaries and persistent state.
- Include Couchbase validation commands or tests that are safe for the stated environment.
...
- Couchbase identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Couchbase failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Couchbase is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Couchbase as the basis for migration or production guidance.
