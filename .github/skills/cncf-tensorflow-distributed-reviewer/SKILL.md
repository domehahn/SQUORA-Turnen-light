...
---
# Cncf Tensorflow Distributed Reviewer

## Purpose

Review and operate Tensorflow Distributed using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Tensorflow Distributed code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Tensorflow Distributed requires evidence-backed assessment.
...
- First-party source: https://www.tensorflow.org/guide/distributed_training
...
- [ ] Trace Tensorflow Distributed inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Tensorflow Distributed defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Tensorflow Distributed least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Tensorflow Distributed resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Tensorflow Distributed release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Tensorflow Distributed dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Tensorflow Distributed identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Tensorflow Distributed change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Tensorflow Distributed documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Tensorflow Distributed architecture and dependency summary with trust boundaries and persistent state.
- Include Tensorflow Distributed validation commands or tests that are safe for the stated environment.
...
- Tensorflow Distributed identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Tensorflow Distributed failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Tensorflow Distributed is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Tensorflow Distributed as the basis for migration or production guidance.
