---
name: "virtualization-reviewer"
description: "Review VM and virtualization architecture, images, isolation, networking, storage, snapshots, patching, and capacity."
version: "0.1.0"
since: "2026-08-27"
...
- Source code, configuration, dependencies, tests, deployment artifacts, or runtime behavior in virtualization platforms change.
...
- hypervisors, VM images, isolation, virtual networking, storage and snapshots, capacity
...
- [ ] Trace virtualization platforms inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check virtualization platforms defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check virtualization platforms least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check virtualization platforms resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check virtualization platforms release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
...
- If virtualization platforms identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a virtualization platforms change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- virtualization platforms documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a virtualization platforms architecture and dependency summary with trust boundaries and persistent state.
- Include virtualization platforms validation commands or tests that are safe for the stated environment.
...
- virtualization platforms identity, version, configuration, dependencies, and deployment assumptions are explicit.
- virtualization platforms failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that virtualization platforms is secure, supported, compatible, or suitable.
- Using an unpinned latest version of virtualization platforms as the basis for migration or production guidance.
