---
name: "cncf-witc-reviewer"
description: "Review Witc using its official documentation and repository in the Wasm / Tooling category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Witc using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Witc code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Witc requires evidence-backed assessment.
...
- First-party source: https://github.com/second-state/witc
...
- [ ] Trace Witc inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Witc defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Witc least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Witc resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Witc release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Witc dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Witc identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Witc change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Witc documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Witc architecture and dependency summary with trust boundaries and persistent state.
- Include Witc validation commands or tests that are safe for the stated environment.
...
- Witc identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Witc failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Witc is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Witc as the basis for migration or production guidance.
