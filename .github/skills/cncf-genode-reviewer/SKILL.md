---
name: "cncf-genode-reviewer"
description: "Review Genode using its official documentation and repository in the Wasm / Edge/Bare metal category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Genode Reviewer

## Purpose

Review and operate Genode using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Genode code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Genode requires evidence-backed assessment.
...
- First-party source: https://github.com/genodelabs/genode
...
- [ ] Trace Genode inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Genode defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Genode least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Genode resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Genode release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Genode dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Genode identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Genode change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Genode documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Genode architecture and dependency summary with trust boundaries and persistent state.
- Include Genode validation commands or tests that are safe for the stated environment.
...
- Genode identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Genode failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Genode is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Genode as the basis for migration or production guidance.
