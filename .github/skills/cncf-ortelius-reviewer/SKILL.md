---
name: "cncf-ortelius-reviewer"
description: "Review Ortelius using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Ortelius Reviewer

## Purpose

Review and operate Ortelius using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Ortelius code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Ortelius requires evidence-backed assessment.
...
- First-party source: https://github.com/ortelius/ortelius
...
- [ ] Trace Ortelius inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Ortelius defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Ortelius least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Ortelius resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Ortelius release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Ortelius dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Ortelius identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Ortelius change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Ortelius documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Ortelius architecture and dependency summary with trust boundaries and persistent state.
- Include Ortelius validation commands or tests that are safe for the stated environment.
...
- Ortelius identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Ortelius failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Ortelius is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Ortelius as the basis for migration or production guidance.
