...
---
# Cncf Lunar Dev Reviewer

## Purpose

Review and operate Lunar.dev using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Lunar.dev code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Lunar.dev requires evidence-backed assessment.
...
- First-party source: https://github.com/TheLunarCompany/lunar
...
- [ ] Trace Lunar.dev inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Lunar.dev defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Lunar.dev least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Lunar.dev resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Lunar.dev release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Lunar.dev dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Lunar.dev identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Lunar.dev change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Lunar.dev documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Lunar.dev architecture and dependency summary with trust boundaries and persistent state.
- Include Lunar.dev validation commands or tests that are safe for the stated environment.
...
- Lunar.dev identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Lunar.dev failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Lunar.dev is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Lunar.dev as the basis for migration or production guidance.
