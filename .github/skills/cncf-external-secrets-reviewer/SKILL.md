---
name: "cncf-external-secrets-reviewer"
description: "Review external-secrets using its official documentation and repository in the Provisioning / Security & Compliance category. External Secrets Operator reads information from a third-party service like AWS Secrets Manager and automatically injects the values as Kubernetes Secrets."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf External Secrets Reviewer

## Purpose

Review and operate external-secrets using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- external-secrets code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving external-secrets requires evidence-backed assessment.
...
- First-party source: https://github.com/external-secrets/external-secrets
...
- [ ] Trace external-secrets inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check external-secrets defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check external-secrets least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check external-secrets resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check external-secrets release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check external-secrets dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If external-secrets identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a external-secrets change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- external-secrets documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a external-secrets architecture and dependency summary with trust boundaries and persistent state.
- Include external-secrets validation commands or tests that are safe for the stated environment.
...
- external-secrets identity, version, configuration, dependencies, and deployment assumptions are explicit.
- external-secrets failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that external-secrets is secure, supported, compatible, or suitable.
- Using an unpinned latest version of external-secrets as the basis for migration or production guidance.
