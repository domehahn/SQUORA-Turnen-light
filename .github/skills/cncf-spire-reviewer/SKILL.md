...
Review and operate SPIRE using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SPIRE code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SPIRE requires evidence-backed assessment.
...
- First-party source: https://github.com/spiffe/spire
...
- [ ] Trace SPIRE inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SPIRE defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SPIRE least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SPIRE resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SPIRE release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SPIRE dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SPIRE identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SPIRE change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SPIRE documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SPIRE architecture and dependency summary with trust boundaries and persistent state.
- Include SPIRE validation commands or tests that are safe for the stated environment.
...
- SPIRE identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SPIRE failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SPIRE is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SPIRE as the basis for migration or production guidance.
