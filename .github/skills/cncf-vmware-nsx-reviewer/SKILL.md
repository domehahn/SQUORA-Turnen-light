---
name: "cncf-vmware-nsx-reviewer"
description: "Review VMware NSX using its official documentation and repository in the Runtime / Cloud Native Network category. VMware NSX® Data Center delivers virtualized networking and security entirely in software, completing a key pillar of the Software-defined Data Center (SDDC), and enabling the virtual cloud network to connect and protect across data centers, clouds, and applications."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Vmware Nsx Reviewer

## Purpose

Review and operate VMware NSX using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- VMware NSX code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving VMware NSX requires evidence-backed assessment.
...
- First-party source: https://www.vmware.com/products/nsx.html
...
- [ ] Trace VMware NSX inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check VMware NSX defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check VMware NSX least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check VMware NSX resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check VMware NSX release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check VMware NSX dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If VMware NSX identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a VMware NSX change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- VMware NSX documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a VMware NSX architecture and dependency summary with trust boundaries and persistent state.
- Include VMware NSX validation commands or tests that are safe for the stated environment.
...
- VMware NSX identity, version, configuration, dependencies, and deployment assumptions are explicit.
- VMware NSX failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that VMware NSX is secure, supported, compatible, or suitable.
- Using an unpinned latest version of VMware NSX as the basis for migration or production guidance.
