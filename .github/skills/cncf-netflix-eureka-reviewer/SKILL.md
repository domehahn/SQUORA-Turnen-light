---
name: "cncf-netflix-eureka-reviewer"
description: "Review Netflix Eureka using its official documentation and repository in the Orchestration & Management / Coordination & Service Discovery category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Netflix Eureka Reviewer

## Purpose

Review and operate Netflix Eureka using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Netflix Eureka code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Netflix Eureka requires evidence-backed assessment.
...
- First-party source: https://github.com/Netflix/eureka
...
- [ ] Trace Netflix Eureka inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Netflix Eureka defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Netflix Eureka least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Netflix Eureka resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Netflix Eureka release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Netflix Eureka dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Netflix Eureka identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Netflix Eureka change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Netflix Eureka documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Netflix Eureka architecture and dependency summary with trust boundaries and persistent state.
- Include Netflix Eureka validation commands or tests that are safe for the stated environment.
...
- Netflix Eureka identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Netflix Eureka failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Netflix Eureka is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Netflix Eureka as the basis for migration or production guidance.
