---
name: "cncf-krator-reviewer"
description: "Review Krator using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Krator Reviewer

## Purpose

Review and operate Krator using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Krator code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Krator requires evidence-backed assessment.
...
- First-party source: https://github.com/krator-rs/krator
...
- [ ] Trace Krator inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Krator defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Krator least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Krator resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Krator release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Krator dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Krator identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Krator change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Krator documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Krator architecture and dependency summary with trust boundaries and persistent state.
- Include Krator validation commands or tests that are safe for the stated environment.
...
- Krator identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Krator failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Krator is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Krator as the basis for migration or production guidance.
