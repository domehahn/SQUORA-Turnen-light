---
name: "cncf-percona-server-for-mysql-reviewer"
description: "Review Percona Server for MySQL using its official documentation and repository in the App Definition and Development / Database category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Percona Server For Mysql Reviewer

## Purpose

Review and operate Percona Server for MySQL using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Percona Server for MySQL code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Percona Server for MySQL requires evidence-backed assessment.
...
- First-party source: https://github.com/percona/percona-server
...
- [ ] Trace Percona Server for MySQL inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Percona Server for MySQL defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Percona Server for MySQL least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Percona Server for MySQL resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Percona Server for MySQL release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Percona Server for MySQL dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Percona Server for MySQL identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Percona Server for MySQL change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Percona Server for MySQL documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Percona Server for MySQL architecture and dependency summary with trust boundaries and persistent state.
- Include Percona Server for MySQL validation commands or tests that are safe for the stated environment.
...
- Percona Server for MySQL identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Percona Server for MySQL failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Percona Server for MySQL is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Percona Server for MySQL as the basis for migration or production guidance.
