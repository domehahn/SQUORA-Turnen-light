...
---
# Cncf Aws App Mesh Reviewer

## Purpose

Review and operate AWS App Mesh using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- AWS App Mesh code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving AWS App Mesh requires evidence-backed assessment.
...
- First-party source: https://aws.amazon.com/app-mesh/
...
- [ ] Trace AWS App Mesh inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check AWS App Mesh defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check AWS App Mesh least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check AWS App Mesh resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check AWS App Mesh release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check AWS App Mesh dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If AWS App Mesh identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a AWS App Mesh change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- AWS App Mesh documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a AWS App Mesh architecture and dependency summary with trust boundaries and persistent state.
- Include AWS App Mesh validation commands or tests that are safe for the stated environment.
...
- AWS App Mesh identity, version, configuration, dependencies, and deployment assumptions are explicit.
- AWS App Mesh failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that AWS App Mesh is secure, supported, compatible, or suitable.
- Using an unpinned latest version of AWS App Mesh as the basis for migration or production guidance.
