---
name: "cncf-xline-reviewer"
description: "Review Xline using its official documentation and repository in the Orchestration & Management / Coordination & Service Discovery category. Xline is a high-performance geo-distributed metadata management system, which is compatible with the ETCD interface."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Xline using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Xline code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Xline requires evidence-backed assessment.
...
- First-party source: https://github.com/xline-kv/Xline
...
- [ ] Trace Xline inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Xline defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Xline least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Xline resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Xline release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Xline dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Xline identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Xline change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Xline documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Xline architecture and dependency summary with trust boundaries and persistent state.
- Include Xline validation commands or tests that are safe for the stated environment.
...
- Xline identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Xline failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Xline is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Xline as the basis for migration or production guidance.
