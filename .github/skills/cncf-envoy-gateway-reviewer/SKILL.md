...
---
# Cncf Envoy Gateway Reviewer

## Purpose

Review and operate Envoy Gateway using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Envoy Gateway code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Envoy Gateway requires evidence-backed assessment.
...
- First-party source: https://github.com/envoyproxy/gateway
...
- [ ] Trace Envoy Gateway inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Envoy Gateway defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Envoy Gateway least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Envoy Gateway resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Envoy Gateway release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Envoy Gateway dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Envoy Gateway identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Envoy Gateway change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Envoy Gateway documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Envoy Gateway architecture and dependency summary with trust boundaries and persistent state.
- Include Envoy Gateway validation commands or tests that are safe for the stated environment.
...
- Envoy Gateway identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Envoy Gateway failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Envoy Gateway is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Envoy Gateway as the basis for migration or production guidance.
