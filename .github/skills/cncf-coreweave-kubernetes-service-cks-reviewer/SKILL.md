---
name: "cncf-coreweave-kubernetes-service-cks-reviewer"
description: "Review CoreWeave Kubernetes Service (CKS) using its official documentation and repository in the Platform / Certified Kubernetes - Hosted category. CKS is a managed Kubernetes environment purpose-built for building, training, and deploying AI applications."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Coreweave Kubernetes Service Cks Reviewer

## Purpose

Review and operate CoreWeave Kubernetes Service (CKS) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CoreWeave Kubernetes Service (CKS) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CoreWeave Kubernetes Service (CKS) requires evidence-backed assessment.
...
- First-party source: https://www.coreweave.com/products/coreweave-kubernetes-service
...
- [ ] Trace CoreWeave Kubernetes Service (CKS) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CoreWeave Kubernetes Service (CKS) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CoreWeave Kubernetes Service (CKS) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CoreWeave Kubernetes Service (CKS) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CoreWeave Kubernetes Service (CKS) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CoreWeave Kubernetes Service (CKS) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CoreWeave Kubernetes Service (CKS) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CoreWeave Kubernetes Service (CKS) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CoreWeave Kubernetes Service (CKS) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CoreWeave Kubernetes Service (CKS) architecture and dependency summary with trust boundaries and persistent state.
- Include CoreWeave Kubernetes Service (CKS) validation commands or tests that are safe for the stated environment.
...
- CoreWeave Kubernetes Service (CKS) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CoreWeave Kubernetes Service (CKS) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CoreWeave Kubernetes Service (CKS) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CoreWeave Kubernetes Service (CKS) as the basis for migration or production guidance.
