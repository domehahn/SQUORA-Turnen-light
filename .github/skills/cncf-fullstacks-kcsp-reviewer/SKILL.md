---
name: "cncf-fullstacks-kcsp-reviewer"
description: "Review FullStackS (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. We live Cloud, Automation, Container & Kubernetes, CI/CD & Application Performance Monitoring! We have an holistic approach to provide our customers a Application-Delivery-Platform."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Fullstacks Kcsp Reviewer

## Purpose

Review and operate FullStackS (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- FullStackS (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving FullStackS (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.fullstacks.eu/about-3
...
- [ ] Trace FullStackS (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check FullStackS (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check FullStackS (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check FullStackS (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check FullStackS (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check FullStackS (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If FullStackS (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a FullStackS (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- FullStackS (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a FullStackS (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include FullStackS (KCSP) validation commands or tests that are safe for the stated environment.
...
- FullStackS (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- FullStackS (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that FullStackS (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of FullStackS (KCSP) as the basis for migration or production guidance.
