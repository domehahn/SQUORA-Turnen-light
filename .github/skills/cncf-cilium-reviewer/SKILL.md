---
name: "cncf-cilium-reviewer"
description: "Review Cilium using its official documentation and repository in the Runtime / Cloud Native Network category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Cilium Reviewer

## Purpose

Review and operate Cilium using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Cilium code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Cilium requires evidence-backed assessment.
...
- First-party source: https://github.com/cilium/cilium
...
- [ ] Trace Cilium inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Cilium defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Cilium least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Cilium resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Cilium release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Cilium dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Cilium identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Cilium change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Cilium documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Cilium architecture and dependency summary with trust boundaries and persistent state.
- Include Cilium validation commands or tests that are safe for the stated environment.
...
- Cilium identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Cilium failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Cilium is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Cilium as the basis for migration or production guidance.
