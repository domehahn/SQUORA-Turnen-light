...
---
# Cncf Testkube Reviewer

## Purpose

Review and operate Testkube using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Testkube code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Testkube requires evidence-backed assessment.
...
- First-party source: https://github.com/kubeshop/testkube
...
- [ ] Trace Testkube inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Testkube defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Testkube least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Testkube resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Testkube release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Testkube dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Testkube identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Testkube change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Testkube documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Testkube architecture and dependency summary with trust boundaries and persistent state.
- Include Testkube validation commands or tests that are safe for the stated environment.
...
- Testkube identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Testkube failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Testkube is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Testkube as the basis for migration or production guidance.
