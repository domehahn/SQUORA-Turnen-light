---
name: "cncf-akka-reviewer"
description: "Review Akka using its official documentation and repository in the Platform / PaaS/Container Service category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Akka using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Akka code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Akka requires evidence-backed assessment.
...
- First-party source: https://github.com/akka/akka
...
- [ ] Trace Akka inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Akka defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Akka least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Akka resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Akka release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Akka dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Akka identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Akka change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Akka documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Akka architecture and dependency summary with trust boundaries and persistent state.
- Include Akka validation commands or tests that are safe for the stated environment.
...
- Akka identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Akka failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Akka is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Akka as the basis for migration or production guidance.
