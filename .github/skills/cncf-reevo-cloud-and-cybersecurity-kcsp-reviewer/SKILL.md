---
name: "cncf-reevo-cloud-and-cybersecurity-kcsp-reviewer"
...
---
# Cncf Reevo Cloud And Cybersecurity Kcsp Reviewer

## Purpose

Review and operate ReeVo Cloud & CyberSecurity (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ReeVo Cloud & CyberSecurity (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ReeVo Cloud & CyberSecurity (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.reevo.it/en/professional-service-cloud-native
...
- [ ] Trace ReeVo Cloud & CyberSecurity (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ReeVo Cloud & CyberSecurity (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ReeVo Cloud & CyberSecurity (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ReeVo Cloud & CyberSecurity (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ReeVo Cloud & CyberSecurity (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ReeVo Cloud & CyberSecurity (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ReeVo Cloud & CyberSecurity (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ReeVo Cloud & CyberSecurity (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ReeVo Cloud & CyberSecurity (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ReeVo Cloud & CyberSecurity (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include ReeVo Cloud & CyberSecurity (KCSP) validation commands or tests that are safe for the stated environment.
...
- ReeVo Cloud & CyberSecurity (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ReeVo Cloud & CyberSecurity (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ReeVo Cloud & CyberSecurity (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ReeVo Cloud & CyberSecurity (KCSP) as the basis for migration or production guidance.
