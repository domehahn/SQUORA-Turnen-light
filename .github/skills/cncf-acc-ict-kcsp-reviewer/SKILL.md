---
name: "cncf-acc-ict-kcsp-reviewer"
description: "Review ACC ICT (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. ACC ICT uses Kubernetes to deliver continuity for mission-critical solutions in demanding complex multi-cloud & public-cloud environments. We design, build and maintain these environments and offer training."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Acc Ict Kcsp Reviewer

## Purpose

Review and operate ACC ICT (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ACC ICT (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ACC ICT (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.acc-ict.com/managed-kubernetes
...
- [ ] Trace ACC ICT (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ACC ICT (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ACC ICT (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ACC ICT (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ACC ICT (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ACC ICT (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ACC ICT (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ACC ICT (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ACC ICT (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ACC ICT (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include ACC ICT (KCSP) validation commands or tests that are safe for the stated environment.
...
- ACC ICT (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ACC ICT (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ACC ICT (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ACC ICT (KCSP) as the basis for migration or production guidance.
