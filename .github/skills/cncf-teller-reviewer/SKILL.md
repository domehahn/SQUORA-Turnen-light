---
name: "cncf-teller-reviewer"
description: "Review Teller using its official documentation and repository in the Provisioning / Key Management category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Teller Reviewer

## Purpose

Review and operate Teller using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Teller code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Teller requires evidence-backed assessment.
...
- First-party source: https://github.com/tellerops/teller
...
- [ ] Trace Teller inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Teller defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Teller least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Teller resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Teller release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Teller dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Teller identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Teller change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Teller documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Teller architecture and dependency summary with trust boundaries and persistent state.
- Include Teller validation commands or tests that are safe for the stated environment.
...
- Teller identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Teller failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Teller is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Teller as the basis for migration or production guidance.
