---
name: "cncf-qingcloud-kubernetes-engine-qke-reviewer"
description: "Review QingCloud Kubernetes Engine (QKE) using its official documentation and repository in the Platform / Certified Kubernetes - Hosted category. QKE can provision a K8s/KubeSphere cluster in minutes automatically by just a few clicks and inputs."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Qingcloud Kubernetes Engine Qke Reviewer

## Purpose

Review and operate QingCloud Kubernetes Engine (QKE) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- QingCloud Kubernetes Engine (QKE) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving QingCloud Kubernetes Engine (QKE) requires evidence-backed assessment.
...
- First-party source: https://console.qingcloud.com/paas/qke/clusters
...
- [ ] Trace QingCloud Kubernetes Engine (QKE) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check QingCloud Kubernetes Engine (QKE) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check QingCloud Kubernetes Engine (QKE) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check QingCloud Kubernetes Engine (QKE) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check QingCloud Kubernetes Engine (QKE) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check QingCloud Kubernetes Engine (QKE) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If QingCloud Kubernetes Engine (QKE) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a QingCloud Kubernetes Engine (QKE) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- QingCloud Kubernetes Engine (QKE) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a QingCloud Kubernetes Engine (QKE) architecture and dependency summary with trust boundaries and persistent state.
- Include QingCloud Kubernetes Engine (QKE) validation commands or tests that are safe for the stated environment.
...
- QingCloud Kubernetes Engine (QKE) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- QingCloud Kubernetes Engine (QKE) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that QingCloud Kubernetes Engine (QKE) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of QingCloud Kubernetes Engine (QKE) as the basis for migration or production guidance.
