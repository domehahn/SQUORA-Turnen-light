---
name: "cncf-yellowdog-reviewer"
description: "Review YellowDog using its official documentation and repository in the Platform / PaaS/Container Service category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Yellowdog Reviewer

## Purpose

Review and operate YellowDog using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- YellowDog code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving YellowDog requires evidence-backed assessment.
...
- First-party source: https://yellowdog.co/
...
- [ ] Trace YellowDog inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check YellowDog defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check YellowDog least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check YellowDog resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check YellowDog release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check YellowDog dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If YellowDog identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a YellowDog change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- YellowDog documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a YellowDog architecture and dependency summary with trust boundaries and persistent state.
- Include YellowDog validation commands or tests that are safe for the stated environment.
...
- YellowDog identity, version, configuration, dependencies, and deployment assumptions are explicit.
- YellowDog failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that YellowDog is secure, supported, compatible, or suitable.
- Using an unpinned latest version of YellowDog as the basis for migration or production guidance.
