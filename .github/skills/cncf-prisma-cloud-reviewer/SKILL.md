---
name: "cncf-prisma-cloud-reviewer"
description: "Review Prisma Cloud using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Prisma Cloud Reviewer

## Purpose

Review and operate Prisma Cloud using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Prisma Cloud code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Prisma Cloud requires evidence-backed assessment.
...
- [ ] Trace Prisma Cloud inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Prisma Cloud defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Prisma Cloud least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Prisma Cloud resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Prisma Cloud release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Prisma Cloud dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Prisma Cloud identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Prisma Cloud change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Prisma Cloud documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Prisma Cloud architecture and dependency summary with trust boundaries and persistent state.
- Include Prisma Cloud validation commands or tests that are safe for the stated environment.
...
- Prisma Cloud identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Prisma Cloud failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Prisma Cloud is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Prisma Cloud as the basis for migration or production guidance.
