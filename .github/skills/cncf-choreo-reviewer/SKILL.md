---
name: "cncf-choreo-reviewer"
description: "Review Choreo using its official documentation and repository in the Platform / PaaS/Container Service category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Choreo Reviewer

## Purpose

Review and operate Choreo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Choreo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Choreo requires evidence-backed assessment.
...
- First-party source: https://wso2.com/choreo/
...
- [ ] Trace Choreo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Choreo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Choreo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Choreo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Choreo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Choreo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Choreo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Choreo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Choreo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Choreo architecture and dependency summary with trust boundaries and persistent state.
- Include Choreo validation commands or tests that are safe for the stated environment.
...
- Choreo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Choreo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Choreo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Choreo as the basis for migration or production guidance.
