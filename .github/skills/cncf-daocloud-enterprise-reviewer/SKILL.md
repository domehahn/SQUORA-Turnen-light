---
name: "cncf-daocloud-enterprise-reviewer"
description: "Review DaoCloud Enterprise using its official documentation and repository in the Platform / Certified Kubernetes - Distribution category. Daocloud helps you provide a reliable and consistent basic support environment to meet the high SLA requirements of enterprise critical applications."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Daocloud Enterprise Reviewer

## Purpose

Review and operate DaoCloud Enterprise using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DaoCloud Enterprise code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DaoCloud Enterprise requires evidence-backed assessment.
...
- First-party source: https://www.daocloud.io/en/
...
- [ ] Trace DaoCloud Enterprise inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DaoCloud Enterprise defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DaoCloud Enterprise least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DaoCloud Enterprise resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DaoCloud Enterprise release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DaoCloud Enterprise dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DaoCloud Enterprise identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DaoCloud Enterprise change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DaoCloud Enterprise documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DaoCloud Enterprise architecture and dependency summary with trust boundaries and persistent state.
- Include DaoCloud Enterprise validation commands or tests that are safe for the stated environment.
...
- DaoCloud Enterprise identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DaoCloud Enterprise failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DaoCloud Enterprise is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DaoCloud Enterprise as the basis for migration or production guidance.
