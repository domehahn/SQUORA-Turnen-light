---
name: "cncf-teuto-net-kcsp-reviewer"
description: "Review teuto.net (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Kubernetes made in Germany - Setup - Hosting - highly skilled DevOps team"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Teuto Net Kcsp Reviewer

## Purpose

Review and operate teuto.net (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- teuto.net (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving teuto.net (KCSP) requires evidence-backed assessment.
...
- First-party source: https://teuto.net/produkte-leistungen/consulting-2/
...
- [ ] Trace teuto.net (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check teuto.net (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check teuto.net (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check teuto.net (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check teuto.net (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check teuto.net (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If teuto.net (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a teuto.net (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- teuto.net (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a teuto.net (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include teuto.net (KCSP) validation commands or tests that are safe for the stated environment.
...
- teuto.net (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- teuto.net (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that teuto.net (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of teuto.net (KCSP) as the basis for migration or production guidance.
