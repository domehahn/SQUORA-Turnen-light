---
name: "cncf-isulad-reviewer"
description: "Review iSulad using its official documentation and repository in the Runtime / Container Runtime category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Isulad Reviewer

## Purpose

Review and operate iSulad using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- iSulad code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving iSulad requires evidence-backed assessment.
...
- First-party source: https://github.com/openeuler-mirror/iSulad
...
- [ ] Trace iSulad inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check iSulad defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check iSulad least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check iSulad resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check iSulad release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check iSulad dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If iSulad identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a iSulad change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- iSulad documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a iSulad architecture and dependency summary with trust boundaries and persistent state.
- Include iSulad validation commands or tests that are safe for the stated environment.
...
- iSulad identity, version, configuration, dependencies, and deployment assumptions are explicit.
- iSulad failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that iSulad is secure, supported, compatible, or suitable.
- Using an unpinned latest version of iSulad as the basis for migration or production guidance.
