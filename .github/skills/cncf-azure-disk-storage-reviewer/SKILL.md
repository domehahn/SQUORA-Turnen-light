---
name: "cncf-azure-disk-storage-reviewer"
description: "Review Azure Disk Storage using its official documentation and repository in the Runtime / Cloud Native Storage category. Get HDD/SSD durability, scalability, availability, and security you need for all your workloads—from mission-critical workloads to test scenarios."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Azure Disk Storage using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Azure Disk Storage code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Azure Disk Storage requires evidence-backed assessment.
...
- First-party source: https://azure.microsoft.com/en-us/services/storage/disks/
...
- [ ] Trace Azure Disk Storage inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Azure Disk Storage defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Azure Disk Storage least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Azure Disk Storage resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Azure Disk Storage release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Azure Disk Storage dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Azure Disk Storage identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Azure Disk Storage change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Azure Disk Storage documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Azure Disk Storage architecture and dependency summary with trust boundaries and persistent state.
- Include Azure Disk Storage validation commands or tests that are safe for the stated environment.
...
- Azure Disk Storage identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Azure Disk Storage failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Azure Disk Storage is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Azure Disk Storage as the basis for migration or production guidance.
