---
name: "cncf-kubeflow-katib-reviewer"
description: "Review Kubeflow Katib using its official documentation and repository in the Training / Post Training category. Automated Machine Learning on Kubernetes"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kubeflow Katib Reviewer

## Purpose

Review and operate Kubeflow Katib using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kubeflow Katib code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kubeflow Katib requires evidence-backed assessment.
...
- CNCF classification: Training / Post Training
- CNCF project maturity field: not specified
- First-party source: https://github.com/kubeflow/katib
...
- [ ] Trace Kubeflow Katib inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kubeflow Katib defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kubeflow Katib least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kubeflow Katib resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kubeflow Katib release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kubeflow Katib dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kubeflow Katib identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kubeflow Katib change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kubeflow Katib documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kubeflow Katib architecture and dependency summary with trust boundaries and persistent state.
- Include Kubeflow Katib validation commands or tests that are safe for the stated environment.
...
- Kubeflow Katib identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kubeflow Katib failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kubeflow Katib is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kubeflow Katib as the basis for migration or production guidance.
