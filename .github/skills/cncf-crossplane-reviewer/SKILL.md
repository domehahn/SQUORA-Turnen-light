...
---
# Cncf Crossplane Reviewer

## Purpose

Review and operate Crossplane using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Crossplane code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Crossplane requires evidence-backed assessment.
...
- First-party source: https://github.com/crossplane/crossplane
...
- [ ] Trace Crossplane inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Crossplane defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Crossplane least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Crossplane resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Crossplane release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Crossplane dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Crossplane identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Crossplane change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Crossplane documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Crossplane architecture and dependency summary with trust boundaries and persistent state.
- Include Crossplane validation commands or tests that are safe for the stated environment.
...
- Crossplane identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Crossplane failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Crossplane is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Crossplane as the basis for migration or production guidance.
