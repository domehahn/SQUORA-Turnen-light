---
name: "cncf-flowmill-reviewer"
description: "Review Flowmill using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Flowmill Reviewer

## Purpose

Review and operate Flowmill using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Flowmill code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Flowmill requires evidence-backed assessment.
...
- First-party source: https://www.splunk.com/en_us/investor-relations/acquisitions/flowmill.html
...
- [ ] Trace Flowmill inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Flowmill defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Flowmill least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Flowmill resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Flowmill release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Flowmill dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Flowmill identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Flowmill change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Flowmill documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Flowmill architecture and dependency summary with trust boundaries and persistent state.
- Include Flowmill validation commands or tests that are safe for the stated environment.
...
- Flowmill identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Flowmill failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Flowmill is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Flowmill as the basis for migration or production guidance.
