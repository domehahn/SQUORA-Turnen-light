---
name: "cncf-ambient-it-kcsp-reviewer"
description: "Review Ambient IT (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Ambient IT offers professional training as well as technical audits of your DevOps / Kubernetes infrastructures."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Ambient It Kcsp Reviewer

## Purpose

Review and operate Ambient IT (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Ambient IT (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Ambient IT (KCSP) requires evidence-backed assessment.
...
- [ ] Trace Ambient IT (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Ambient IT (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Ambient IT (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Ambient IT (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Ambient IT (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Ambient IT (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Ambient IT (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Ambient IT (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Ambient IT (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Ambient IT (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Ambient IT (KCSP) validation commands or tests that are safe for the stated environment.
...
- Ambient IT (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Ambient IT (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Ambient IT (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Ambient IT (KCSP) as the basis for migration or production guidance.
