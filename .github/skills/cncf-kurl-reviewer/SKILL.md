...
Review and operate kURL using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- kURL code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving kURL requires evidence-backed assessment.
...
- First-party source: https://github.com/replicatedhq/kurl
...
- [ ] Trace kURL inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check kURL defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check kURL least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check kURL resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check kURL release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check kURL dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If kURL identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a kURL change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- kURL documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a kURL architecture and dependency summary with trust boundaries and persistent state.
- Include kURL validation commands or tests that are safe for the stated environment.
...
- kURL identity, version, configuration, dependencies, and deployment assumptions are explicit.
- kURL failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that kURL is secure, supported, compatible, or suitable.
- Using an unpinned latest version of kURL as the basis for migration or production guidance.
