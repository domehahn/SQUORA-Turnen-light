---
name: "cncf-yomo-reviewer"
description: "Review YOMO using its official documentation and repository in the Wasm / Embedded Functions category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate YOMO using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- YOMO code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving YOMO requires evidence-backed assessment.
...
- First-party source: https://github.com/yomorun/yomo
...
- [ ] Trace YOMO inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check YOMO defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check YOMO least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check YOMO resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check YOMO release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check YOMO dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If YOMO identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a YOMO change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- YOMO documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a YOMO architecture and dependency summary with trust boundaries and persistent state.
- Include YOMO validation commands or tests that are safe for the stated environment.
...
- YOMO identity, version, configuration, dependencies, and deployment assumptions are explicit.
- YOMO failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that YOMO is secure, supported, compatible, or suitable.
- Using an unpinned latest version of YOMO as the basis for migration or production guidance.
