---
name: "cncf-fairwinds-kcsp-reviewer"
description: "Review Fairwinds (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Fairwinds, the Kubernetes enablement company, offers services, open source, and software to help organizations run secure, efficient and reliable Kubernetes infrastructure."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Fairwinds Kcsp Reviewer

## Purpose

Review and operate Fairwinds (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Fairwinds (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Fairwinds (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.fairwinds.com/managed-kubernetes
...
- [ ] Trace Fairwinds (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Fairwinds (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Fairwinds (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Fairwinds (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Fairwinds (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Fairwinds (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Fairwinds (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Fairwinds (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Fairwinds (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Fairwinds (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Fairwinds (KCSP) validation commands or tests that are safe for the stated environment.
...
- Fairwinds (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Fairwinds (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Fairwinds (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Fairwinds (KCSP) as the basis for migration or production guidance.
