---
name: "cncf-encore-reviewer"
description: "Review Encore using its official documentation and repository in the Serverless / Framework category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Encore Reviewer

## Purpose

Review and operate Encore using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Encore code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Encore requires evidence-backed assessment.
...
- First-party source: https://github.com/encoredev/encore
...
- [ ] Trace Encore inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Encore defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Encore least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Encore resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Encore release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Encore dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Encore identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Encore change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Encore documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Encore architecture and dependency summary with trust boundaries and persistent state.
- Include Encore validation commands or tests that are safe for the stated environment.
...
- Encore identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Encore failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Encore is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Encore as the basis for migration or production guidance.
