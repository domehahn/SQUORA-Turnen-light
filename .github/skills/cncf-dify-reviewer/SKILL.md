---
name: "cncf-dify-reviewer"
description: "Review Dify using its official documentation and repository in the AI Agent / Workflow Orchestration category. Production-ready platform for agentic workflow development."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Dify using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Dify code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Dify requires evidence-backed assessment.
...
- First-party source: https://github.com/langgenius/dify
...
- [ ] Trace Dify inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Dify defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Dify least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Dify resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Dify release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Dify dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Dify identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Dify change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Dify documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Dify architecture and dependency summary with trust boundaries and persistent state.
- Include Dify validation commands or tests that are safe for the stated environment.
...
- Dify identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Dify failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Dify is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Dify as the basis for migration or production guidance.
