---
name: "cncf-qingteng-reviewer"
description: "Review Qingteng using its official documentation and repository in the Provisioning / Security & Compliance category. Qingteng Honeycomb · Container Security Platform can display risk scenarios visually for enterprises through continuous monitoring and analysis of the container security status, and protects the container environment during the entire life cycle (build, ship, and run) of the container."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Qingteng Reviewer

## Purpose

Review and operate Qingteng using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Qingteng code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Qingteng requires evidence-backed assessment.
...
- First-party source: https://qingteng.cn/
...
- [ ] Trace Qingteng inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Qingteng defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Qingteng least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Qingteng resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Qingteng release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Qingteng dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Qingteng identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Qingteng change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Qingteng documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Qingteng architecture and dependency summary with trust boundaries and persistent state.
- Include Qingteng validation commands or tests that are safe for the stated environment.
...
- Qingteng identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Qingteng failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Qingteng is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Qingteng as the basis for migration or production guidance.
