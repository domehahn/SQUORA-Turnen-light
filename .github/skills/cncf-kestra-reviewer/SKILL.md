---
name: "cncf-kestra-reviewer"
description: "Review Kestra using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. Scalable, event-driven, language-agnostic orchestration and scheduling platform to manage millions of workflows declaratively in code."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kestra Reviewer

## Purpose

Review and operate Kestra using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kestra code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kestra requires evidence-backed assessment.
...
- First-party source: https://github.com/kestra-io/kestra
...
- [ ] Trace Kestra inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kestra defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kestra least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kestra resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kestra release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kestra dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kestra identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kestra change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kestra documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kestra architecture and dependency summary with trust boundaries and persistent state.
- Include Kestra validation commands or tests that are safe for the stated environment.
...
- Kestra identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kestra failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kestra is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kestra as the basis for migration or production guidance.
