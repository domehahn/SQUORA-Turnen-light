---
name: "cncf-skywalking-reviewer"
description: "Review SkyWalking using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Skywalking Reviewer

## Purpose

Review and operate SkyWalking using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SkyWalking code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SkyWalking requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/skywalking
...
- [ ] Trace SkyWalking inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SkyWalking defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SkyWalking least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SkyWalking resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SkyWalking release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SkyWalking dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SkyWalking identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SkyWalking change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SkyWalking documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SkyWalking architecture and dependency summary with trust boundaries and persistent state.
- Include SkyWalking validation commands or tests that are safe for the stated environment.
...
- SkyWalking identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SkyWalking failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SkyWalking is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SkyWalking as the basis for migration or production guidance.
