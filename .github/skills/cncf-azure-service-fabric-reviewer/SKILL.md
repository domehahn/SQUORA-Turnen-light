---
name: "cncf-azure-service-fabric-reviewer"
description: "Review Azure Service Fabric using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Azure Service Fabric Reviewer

## Purpose

Review and operate Azure Service Fabric using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Azure Service Fabric code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Azure Service Fabric requires evidence-backed assessment.
...
- First-party source: https://github.com/Microsoft/service-fabric
...
- [ ] Trace Azure Service Fabric inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Azure Service Fabric defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Azure Service Fabric least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Azure Service Fabric resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Azure Service Fabric release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Azure Service Fabric dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Azure Service Fabric identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Azure Service Fabric change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Azure Service Fabric documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Azure Service Fabric architecture and dependency summary with trust boundaries and persistent state.
- Include Azure Service Fabric validation commands or tests that are safe for the stated environment.
...
- Azure Service Fabric identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Azure Service Fabric failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Azure Service Fabric is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Azure Service Fabric as the basis for migration or production guidance.
