---
name: "cncf-strands-reviewer"
description: "Review Strands using its official documentation and repository in the AI Agent / Agent Framework category. A model-driven approach to building AI agents in just a few lines of code."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Strands Reviewer

## Purpose

Review and operate Strands using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Strands code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Strands requires evidence-backed assessment.
...
- First-party source: https://github.com/strands-agents/sdk-python
...
- [ ] Trace Strands inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Strands defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Strands least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Strands resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Strands release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Strands dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Strands identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Strands change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Strands documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Strands architecture and dependency summary with trust boundaries and persistent state.
- Include Strands validation commands or tests that are safe for the stated environment.
...
- Strands identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Strands failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Strands is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Strands as the basis for migration or production guidance.
