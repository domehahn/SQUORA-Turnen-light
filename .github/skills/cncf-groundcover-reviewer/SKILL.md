...
---
# Cncf Groundcover Reviewer

## Purpose

Review and operate groundcover using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- groundcover code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving groundcover requires evidence-backed assessment.
...
- First-party source: https://groundcover.com
...
- [ ] Trace groundcover inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check groundcover defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check groundcover least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check groundcover resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check groundcover release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check groundcover dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If groundcover identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a groundcover change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- groundcover documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a groundcover architecture and dependency summary with trust boundaries and persistent state.
- Include groundcover validation commands or tests that are safe for the stated environment.
...
- groundcover identity, version, configuration, dependencies, and deployment assumptions are explicit.
- groundcover failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that groundcover is secure, supported, compatible, or suitable.
- Using an unpinned latest version of groundcover as the basis for migration or production guidance.
