---
name: "cncf-traefik-reviewer"
description: "Review Traefik using its official documentation and repository in the Orchestration & Management / API Gateway category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Traefik Reviewer

## Purpose

Review and operate Traefik using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Traefik code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Traefik requires evidence-backed assessment.
...
- First-party source: https://github.com/traefik/traefik
...
- [ ] Trace Traefik inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Traefik defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Traefik least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Traefik resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Traefik release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Traefik dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Traefik identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Traefik change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Traefik documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Traefik architecture and dependency summary with trust boundaries and persistent state.
- Include Traefik validation commands or tests that are safe for the stated environment.
...
- Traefik identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Traefik failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Traefik is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Traefik as the basis for migration or production guidance.
