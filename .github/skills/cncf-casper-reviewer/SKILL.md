---
name: "cncf-casper-reviewer"
description: "Review Casper using its official documentation and repository in the Wasm / Decentralized Platforms category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Casper Reviewer

## Purpose

Review and operate Casper using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Casper code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Casper requires evidence-backed assessment.
...
- First-party source: https://github.com/casper-network/casper-node
...
- [ ] Trace Casper inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Casper defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Casper least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Casper resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Casper release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Casper dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Casper identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Casper change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Casper documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Casper architecture and dependency summary with trust boundaries and persistent state.
- Include Casper validation commands or tests that are safe for the stated environment.
...
- Casper identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Casper failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Casper is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Casper as the basis for migration or production guidance.
