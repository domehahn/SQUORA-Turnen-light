---
name: "cncf-sidekick-reviewer"
description: "Review Sidekick using its official documentation and repository in the Observability and Analysis / Observability category. Collect traces, exception stacks and generate logs on-demand without stopping & redeploying your applications."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Sidekick Reviewer

## Purpose

Review and operate Sidekick using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Sidekick code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Sidekick requires evidence-backed assessment.
...
- First-party source: https://github.com/runsidekick/sidekick
...
- [ ] Trace Sidekick inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Sidekick defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Sidekick least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Sidekick resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Sidekick release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Sidekick dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Sidekick identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Sidekick change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Sidekick documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Sidekick architecture and dependency summary with trust boundaries and persistent state.
- Include Sidekick validation commands or tests that are safe for the stated environment.
...
- Sidekick identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Sidekick failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Sidekick is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Sidekick as the basis for migration or production guidance.
