---
name: "cncf-pinpoint-reviewer"
description: "Review Pinpoint using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Pinpoint Reviewer

## Purpose

Review and operate Pinpoint using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Pinpoint code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Pinpoint requires evidence-backed assessment.
...
- First-party source: https://github.com/pinpoint-apm/pinpoint
...
- [ ] Trace Pinpoint inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Pinpoint defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Pinpoint least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Pinpoint resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Pinpoint release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Pinpoint dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Pinpoint identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Pinpoint change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Pinpoint documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Pinpoint architecture and dependency summary with trust boundaries and persistent state.
- Include Pinpoint validation commands or tests that are safe for the stated environment.
...
- Pinpoint identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Pinpoint failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Pinpoint is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Pinpoint as the basis for migration or production guidance.
