---
name: "cncf-mogdb-reviewer"
description: "Review MogDB using its official documentation and repository in the App Definition and Development / Database category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate MogDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- MogDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving MogDB requires evidence-backed assessment.
...
- First-party source: https://www.mogdb.io/en
...
- [ ] Trace MogDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check MogDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check MogDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check MogDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check MogDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check MogDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If MogDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a MogDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- MogDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a MogDB architecture and dependency summary with trust boundaries and persistent state.
- Include MogDB validation commands or tests that are safe for the stated environment.
...
- MogDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- MogDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that MogDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of MogDB as the basis for migration or production guidance.
