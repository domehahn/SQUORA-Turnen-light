---
name: "cncf-ondat-reviewer"
description: "Review Ondat using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Ondat using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Ondat code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Ondat requires evidence-backed assessment.
...
- First-party source: https://docs.ondat.io/
...
- [ ] Trace Ondat inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Ondat defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Ondat least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Ondat resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Ondat release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Ondat dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Ondat identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Ondat change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Ondat documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Ondat architecture and dependency summary with trust boundaries and persistent state.
- Include Ondat validation commands or tests that are safe for the stated environment.
...
- Ondat identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Ondat failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Ondat is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Ondat as the basis for migration or production guidance.
