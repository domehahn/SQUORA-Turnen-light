---
name: "cncf-coder-reviewer"
description: "Review Coder using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category. Self-hosted cloud development environments consistently provisioned as code and pre-configured for developer activity on day one."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Coder using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Coder code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Coder requires evidence-backed assessment.
...
- First-party source: https://github.com/coder/coder
...
- [ ] Trace Coder inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Coder defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Coder least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Coder resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Coder release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Coder dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Coder identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Coder change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Coder documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Coder architecture and dependency summary with trust boundaries and persistent state.
- Include Coder validation commands or tests that are safe for the stated environment.
...
- Coder identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Coder failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Coder is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Coder as the basis for migration or production guidance.
