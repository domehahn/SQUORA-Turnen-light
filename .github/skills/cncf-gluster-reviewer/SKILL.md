---
name: "cncf-gluster-reviewer"
description: "Review Gluster using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Gluster Reviewer

## Purpose

Review and operate Gluster using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Gluster code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Gluster requires evidence-backed assessment.
...
- First-party source: https://github.com/gluster/glusterfs
...
- [ ] Trace Gluster inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Gluster defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Gluster least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Gluster resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Gluster release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Gluster dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Gluster identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Gluster change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Gluster documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Gluster architecture and dependency summary with trust boundaries and persistent state.
- Include Gluster validation commands or tests that are safe for the stated environment.
...
- Gluster identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Gluster failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Gluster is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Gluster as the basis for migration or production guidance.
