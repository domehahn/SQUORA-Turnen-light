---
name: "cncf-chaosblade-reviewer"
description: "Review Chaosblade using its official documentation and repository in the Observability and Analysis / Chaos Engineering category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Chaosblade Reviewer

## Purpose

Review and operate Chaosblade using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Chaosblade code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Chaosblade requires evidence-backed assessment.
...
- First-party source: https://github.com/chaosblade-io/chaosblade
...
- [ ] Trace Chaosblade inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Chaosblade defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Chaosblade least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Chaosblade resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Chaosblade release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Chaosblade dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Chaosblade identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Chaosblade change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Chaosblade documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Chaosblade architecture and dependency summary with trust boundaries and persistent state.
- Include Chaosblade validation commands or tests that are safe for the stated environment.
...
- Chaosblade identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Chaosblade failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Chaosblade is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Chaosblade as the basis for migration or production guidance.
