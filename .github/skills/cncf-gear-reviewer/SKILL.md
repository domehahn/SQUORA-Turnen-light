---
name: "cncf-gear-reviewer"
description: "Review GEAR using its official documentation and repository in the Wasm / Decentralized Platforms category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate GEAR using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- GEAR code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving GEAR requires evidence-backed assessment.
...
- First-party source: https://github.com/gear-tech/gear
...
- [ ] Trace GEAR inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check GEAR defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check GEAR least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check GEAR resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check GEAR release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check GEAR dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If GEAR identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a GEAR change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- GEAR documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a GEAR architecture and dependency summary with trust boundaries and persistent state.
- Include GEAR validation commands or tests that are safe for the stated environment.
...
- GEAR identity, version, configuration, dependencies, and deployment assumptions are explicit.
- GEAR failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that GEAR is secure, supported, compatible, or suitable.
- Using an unpinned latest version of GEAR as the basis for migration or production guidance.
