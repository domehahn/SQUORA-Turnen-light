---
name: "cncf-envoy-reviewer"
description: "Review Envoy using its official documentation and repository in the Orchestration & Management / Service Proxy category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Envoy using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Envoy code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Envoy requires evidence-backed assessment.
...
- [ ] Trace Envoy inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Envoy defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Envoy least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Envoy resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Envoy release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Envoy dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Envoy identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Envoy change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Envoy documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Envoy architecture and dependency summary with trust boundaries and persistent state.
- Include Envoy validation commands or tests that are safe for the stated environment.
...
- Envoy identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Envoy failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Envoy is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Envoy as the basis for migration or production guidance.
