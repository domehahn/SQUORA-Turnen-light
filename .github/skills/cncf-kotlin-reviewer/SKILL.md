---
name: "cncf-kotlin-reviewer"
description: "Review Kotlin using its official documentation and repository in the Wasm / Languages category. Managed language"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kotlin Reviewer

## Purpose

Review and operate Kotlin using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kotlin code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kotlin requires evidence-backed assessment.
...
- First-party source: https://github.com/JetBrains/kotlin
...
- [ ] Trace Kotlin inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kotlin defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kotlin least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kotlin resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kotlin release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kotlin dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kotlin identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kotlin change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kotlin documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kotlin architecture and dependency summary with trust boundaries and persistent state.
- Include Kotlin validation commands or tests that are safe for the stated environment.
...
- Kotlin identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kotlin failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kotlin is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kotlin as the basis for migration or production guidance.
