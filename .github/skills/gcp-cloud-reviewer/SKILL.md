---
name: "gcp-cloud-reviewer"
description: "Review GCP organizations, projects, IAM, networking, compute, data services, observability, cost, and resilience."
version: "0.1.0"
since: "2026-08-27"
...
- Source code, configuration, dependencies, tests, deployment artifacts, or runtime behavior in Google Cloud architecture change.
...
- organizations and projects, IAM, VPC, compute, data services, Cloud Audit Logs and Monitoring
...
- [ ] Trace Google Cloud architecture inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Google Cloud architecture defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Google Cloud architecture least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Google Cloud architecture resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Google Cloud architecture release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
...
- If Google Cloud architecture identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Google Cloud architecture change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Google Cloud architecture documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Google Cloud architecture architecture and dependency summary with trust boundaries and persistent state.
- Include Google Cloud architecture validation commands or tests that are safe for the stated environment.
...
- Google Cloud architecture identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Google Cloud architecture failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Google Cloud architecture is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Google Cloud architecture as the basis for migration or production guidance.
