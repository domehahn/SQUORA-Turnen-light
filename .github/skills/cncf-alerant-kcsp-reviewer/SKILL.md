---
name: "cncf-alerant-kcsp-reviewer"
description: "Review Alerant (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Alerant delivers enterprise-grade Platform Engineering, Application Modernization, and AI solutions, leveraging AI-assisted engineering techniques. We design, deploy, and support Kubernetes-based platforms, accelerate DevOps and CI/CD adoption, and guide application migration and modernization in the cloud."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Alerant Kcsp Reviewer

## Purpose

Review and operate Alerant (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Alerant (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Alerant (KCSP) requires evidence-backed assessment.
...
- [ ] Trace Alerant (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Alerant (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Alerant (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Alerant (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Alerant (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Alerant (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Alerant (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Alerant (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Alerant (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Alerant (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Alerant (KCSP) validation commands or tests that are safe for the stated environment.
...
- Alerant (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Alerant (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Alerant (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Alerant (KCSP) as the basis for migration or production guidance.
