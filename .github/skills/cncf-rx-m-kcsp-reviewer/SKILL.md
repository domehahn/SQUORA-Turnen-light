...
---
# Cncf Rx M Kcsp Reviewer

## Purpose

Review and operate RX-M (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- RX-M (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving RX-M (KCSP) requires evidence-backed assessment.
...
- First-party source: https://rx-m.com/cloud-native-consulting/
...
- [ ] Trace RX-M (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check RX-M (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check RX-M (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check RX-M (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check RX-M (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check RX-M (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If RX-M (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a RX-M (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- RX-M (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a RX-M (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include RX-M (KCSP) validation commands or tests that are safe for the stated environment.
...
- RX-M (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- RX-M (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that RX-M (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of RX-M (KCSP) as the basis for migration or production guidance.
