---
name: "cncf-unleash-reviewer"
description: "Review Unleash using its official documentation and repository in the Observability and Analysis / Feature Flagging category. Open-source feature management platform"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Unleash Reviewer

## Purpose

Review and operate Unleash using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Unleash code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Unleash requires evidence-backed assessment.
...
- First-party source: https://github.com/Unleash/unleash
...
- [ ] Trace Unleash inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Unleash defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Unleash least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Unleash resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Unleash release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Unleash dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Unleash identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Unleash change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Unleash documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Unleash architecture and dependency summary with trust boundaries and persistent state.
- Include Unleash validation commands or tests that are safe for the stated environment.
...
- Unleash identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Unleash failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Unleash is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Unleash as the basis for migration or production guidance.
