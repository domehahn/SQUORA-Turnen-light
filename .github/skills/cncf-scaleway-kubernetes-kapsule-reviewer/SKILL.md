...
---
# Cncf Scaleway Kubernetes Kapsule Reviewer

## Purpose

Review and operate Scaleway Kubernetes Kapsule using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Scaleway Kubernetes Kapsule code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Scaleway Kubernetes Kapsule requires evidence-backed assessment.
...
- First-party source: https://www.scaleway.com/en/betas/
...
- [ ] Trace Scaleway Kubernetes Kapsule inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Scaleway Kubernetes Kapsule defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Scaleway Kubernetes Kapsule least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Scaleway Kubernetes Kapsule resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Scaleway Kubernetes Kapsule release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Scaleway Kubernetes Kapsule dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Scaleway Kubernetes Kapsule identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Scaleway Kubernetes Kapsule change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Scaleway Kubernetes Kapsule documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Scaleway Kubernetes Kapsule architecture and dependency summary with trust boundaries and persistent state.
- Include Scaleway Kubernetes Kapsule validation commands or tests that are safe for the stated environment.
...
- Scaleway Kubernetes Kapsule identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Scaleway Kubernetes Kapsule failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Scaleway Kubernetes Kapsule is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Scaleway Kubernetes Kapsule as the basis for migration or production guidance.
