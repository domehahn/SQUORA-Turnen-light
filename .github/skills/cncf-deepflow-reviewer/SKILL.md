---
name: "cncf-deepflow-reviewer"
description: "Review DeepFlow using its official documentation and repository in the AI Native Infra / Observability category. DeepFlow leverages eBPF to collect signals such as performance metrics, distributed tracing, and CPU & GPU profiling. Its goal is to provide zero-code observability for complex cloud-native and AI applications, particularly those involving LLM training and inference using frameworks like PyTorch."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Deepflow Reviewer

## Purpose

Review and operate DeepFlow using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DeepFlow code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DeepFlow requires evidence-backed assessment.
...
- CNCF classification: AI Native Infra / Observability; Observability and Analysis / Observability
- CNCF project maturity field: not specified
- First-party source: https://github.com/deepflowys/deepflow
...
- [ ] Trace DeepFlow inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DeepFlow defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DeepFlow least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DeepFlow resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DeepFlow release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DeepFlow dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DeepFlow identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DeepFlow change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DeepFlow documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DeepFlow architecture and dependency summary with trust boundaries and persistent state.
- Include DeepFlow validation commands or tests that are safe for the stated environment.
...
- DeepFlow identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DeepFlow failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DeepFlow is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DeepFlow as the basis for migration or production guidance.
