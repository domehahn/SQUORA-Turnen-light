---
name: "cncf-passage-reviewer"
description: "Review Passage using its official documentation and repository in the Provisioning / Security & Compliance category. Passage makes it easy for developers to implement passwordless customer identity in any application."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Passage Reviewer

## Purpose

Review and operate Passage using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Passage code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Passage requires evidence-backed assessment.
...
- First-party source: https://passage.id
...
- [ ] Trace Passage inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Passage defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Passage least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Passage resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Passage release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Passage dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Passage identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Passage change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Passage documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Passage architecture and dependency summary with trust boundaries and persistent state.
- Include Passage validation commands or tests that are safe for the stated environment.
...
- Passage identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Passage failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Passage is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Passage as the basis for migration or production guidance.
