---
name: "cncf-pytorch-reviewer"
description: "Review PyTorch using its official documentation and repository in the Data / Data Science category. About Tensors and Dynamic neural networks in Python with strong GPU acceleration"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Pytorch Reviewer

## Purpose

Review and operate PyTorch using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- PyTorch code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving PyTorch requires evidence-backed assessment.
...
- First-party source: https://pytorch.org
...
- [ ] Trace PyTorch inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check PyTorch defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check PyTorch least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check PyTorch resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check PyTorch release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check PyTorch dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If PyTorch identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a PyTorch change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- PyTorch documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a PyTorch architecture and dependency summary with trust boundaries and persistent state.
- Include PyTorch validation commands or tests that are safe for the stated environment.
...
- PyTorch identity, version, configuration, dependencies, and deployment assumptions are explicit.
- PyTorch failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that PyTorch is secure, supported, compatible, or suitable.
- Using an unpinned latest version of PyTorch as the basis for migration or production guidance.
