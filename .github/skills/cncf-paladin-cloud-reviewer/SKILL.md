---
name: "cncf-paladin-cloud-reviewer"
description: "Review Paladin Cloud using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Paladin Cloud Reviewer

## Purpose

Review and operate Paladin Cloud using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Paladin Cloud code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Paladin Cloud requires evidence-backed assessment.
...
- First-party source: https://github.com/PaladinCloud/CE
...
- [ ] Trace Paladin Cloud inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Paladin Cloud defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Paladin Cloud least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Paladin Cloud resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Paladin Cloud release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Paladin Cloud dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Paladin Cloud identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Paladin Cloud change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Paladin Cloud documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Paladin Cloud architecture and dependency summary with trust boundaries and persistent state.
- Include Paladin Cloud validation commands or tests that are safe for the stated environment.
...
- Paladin Cloud identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Paladin Cloud failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Paladin Cloud is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Paladin Cloud as the basis for migration or production guidance.
