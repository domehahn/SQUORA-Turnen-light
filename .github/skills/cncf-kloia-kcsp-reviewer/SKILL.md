...
---
# Cncf Kloia Kcsp Reviewer

## Purpose

Review and operate kloia (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- kloia (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving kloia (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.kloia.com/kubernetes
...
- [ ] Trace kloia (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check kloia (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check kloia (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check kloia (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check kloia (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check kloia (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If kloia (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a kloia (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- kloia (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a kloia (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include kloia (KCSP) validation commands or tests that are safe for the stated environment.
...
- kloia (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- kloia (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that kloia (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of kloia (KCSP) as the basis for migration or production guidance.
