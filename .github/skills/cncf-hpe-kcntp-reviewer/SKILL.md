---
name: "cncf-hpe-kcntp-reviewer"
description: "Review HPE (KCNTP) using its official documentation and repository in the Special / Kubernetes and Cloud Native Training Partner category. HPE is a leader in the hybrid cloud market offering Kubernetes solutions that are supported by professional services and education."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Hpe Kcntp Reviewer

## Purpose

Review and operate HPE (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- HPE (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving HPE (KCNTP) requires evidence-backed assessment.
...
- First-party source: https://education.hpe.com/ww/en/training/portfolio/cloud.html
...
- [ ] Trace HPE (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check HPE (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check HPE (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check HPE (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check HPE (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check HPE (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If HPE (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a HPE (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- HPE (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a HPE (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include HPE (KCNTP) validation commands or tests that are safe for the stated environment.
...
- HPE (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- HPE (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that HPE (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of HPE (KCNTP) as the basis for migration or production guidance.
