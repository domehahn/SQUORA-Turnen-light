---
name: "cncf-iguazio-reviewer"
description: "Review iguazio using its official documentation and repository in the App Definition and Development / Database category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Iguazio Reviewer

## Purpose

Review and operate iguazio using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- iguazio code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving iguazio requires evidence-backed assessment.
...
- First-party source: https://www.iguazio.com/
...
- [ ] Trace iguazio inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check iguazio defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check iguazio least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check iguazio resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check iguazio release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check iguazio dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If iguazio identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a iguazio change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- iguazio documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a iguazio architecture and dependency summary with trust boundaries and persistent state.
- Include iguazio validation commands or tests that are safe for the stated environment.
...
- iguazio identity, version, configuration, dependencies, and deployment assumptions are explicit.
- iguazio failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that iguazio is secure, supported, compatible, or suitable.
- Using an unpinned latest version of iguazio as the basis for migration or production guidance.
