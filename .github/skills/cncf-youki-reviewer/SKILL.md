---
name: "cncf-youki-reviewer"
description: "Review youki using its official documentation and repository in the Runtime / Container Runtime category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate youki using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- youki code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving youki requires evidence-backed assessment.
...
- [ ] Trace youki inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check youki defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check youki least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check youki resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check youki release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check youki dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If youki identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a youki change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- youki documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a youki architecture and dependency summary with trust boundaries and persistent state.
- Include youki validation commands or tests that are safe for the stated environment.
...
- youki identity, version, configuration, dependencies, and deployment assumptions are explicit.
- youki failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that youki is secure, supported, compatible, or suitable.
- Using an unpinned latest version of youki as the basis for migration or production guidance.
