...
---
# Cncf Simplyblock Reviewer

## Purpose

Review and operate Simplyblock using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Simplyblock code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Simplyblock requires evidence-backed assessment.
...
- First-party source: https://simplyblock.io/
...
- [ ] Trace Simplyblock inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Simplyblock defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Simplyblock least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Simplyblock resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Simplyblock release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Simplyblock dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Simplyblock identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Simplyblock change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Simplyblock documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Simplyblock architecture and dependency summary with trust boundaries and persistent state.
- Include Simplyblock validation commands or tests that are safe for the stated environment.
...
- Simplyblock identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Simplyblock failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Simplyblock is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Simplyblock as the basis for migration or production guidance.
