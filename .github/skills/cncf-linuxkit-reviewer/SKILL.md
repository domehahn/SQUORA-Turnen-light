---
name: "cncf-linuxkit-reviewer"
description: "Review LinuxKit using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Linuxkit Reviewer

## Purpose

Review and operate LinuxKit using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- LinuxKit code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving LinuxKit requires evidence-backed assessment.
...
- First-party source: https://github.com/linuxkit/linuxkit
...
- [ ] Trace LinuxKit inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check LinuxKit defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check LinuxKit least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check LinuxKit resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check LinuxKit release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check LinuxKit dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If LinuxKit identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a LinuxKit change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- LinuxKit documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a LinuxKit architecture and dependency summary with trust boundaries and persistent state.
- Include LinuxKit validation commands or tests that are safe for the stated environment.
...
- LinuxKit identity, version, configuration, dependencies, and deployment assumptions are explicit.
- LinuxKit failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that LinuxKit is secure, supported, compatible, or suitable.
- Using an unpinned latest version of LinuxKit as the basis for migration or production guidance.
