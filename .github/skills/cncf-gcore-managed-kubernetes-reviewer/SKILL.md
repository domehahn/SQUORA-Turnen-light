...
---
# Cncf Gcore Managed Kubernetes Reviewer

## Purpose

Review and operate Gcore Managed Kubernetes using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Gcore Managed Kubernetes code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Gcore Managed Kubernetes requires evidence-backed assessment.
...
- First-party source: https://gcore.com/cloud/managed-kubernetes
...
- [ ] Trace Gcore Managed Kubernetes inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Gcore Managed Kubernetes defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Gcore Managed Kubernetes least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Gcore Managed Kubernetes resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Gcore Managed Kubernetes release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Gcore Managed Kubernetes dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Gcore Managed Kubernetes identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Gcore Managed Kubernetes change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Gcore Managed Kubernetes documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Gcore Managed Kubernetes architecture and dependency summary with trust boundaries and persistent state.
- Include Gcore Managed Kubernetes validation commands or tests that are safe for the stated environment.
...
- Gcore Managed Kubernetes identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Gcore Managed Kubernetes failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Gcore Managed Kubernetes is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Gcore Managed Kubernetes as the basis for migration or production guidance.
