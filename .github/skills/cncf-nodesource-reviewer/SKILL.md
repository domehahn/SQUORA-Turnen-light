---
name: "cncf-nodesource-reviewer"
description: "Review NodeSource using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Nodesource Reviewer

## Purpose

Review and operate NodeSource using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- NodeSource code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving NodeSource requires evidence-backed assessment.
...
- First-party source: https://nodesource.com/
...
- [ ] Trace NodeSource inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check NodeSource defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check NodeSource least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check NodeSource resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check NodeSource release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check NodeSource dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If NodeSource identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a NodeSource change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- NodeSource documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a NodeSource architecture and dependency summary with trust boundaries and persistent state.
- Include NodeSource validation commands or tests that are safe for the stated environment.
...
- NodeSource identity, version, configuration, dependencies, and deployment assumptions are explicit.
- NodeSource failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that NodeSource is secure, supported, compatible, or suitable.
- Using an unpinned latest version of NodeSource as the basis for migration or production guidance.
