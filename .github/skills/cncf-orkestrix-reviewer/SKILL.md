...
---
# Cncf Orkestrix Reviewer

## Purpose

Review and operate ORKESTRIX using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ORKESTRIX code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ORKESTRIX requires evidence-backed assessment.
...
- [ ] Trace ORKESTRIX inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ORKESTRIX defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ORKESTRIX least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ORKESTRIX resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ORKESTRIX release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ORKESTRIX dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ORKESTRIX identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ORKESTRIX change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ORKESTRIX documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ORKESTRIX architecture and dependency summary with trust boundaries and persistent state.
- Include ORKESTRIX validation commands or tests that are safe for the stated environment.
...
- ORKESTRIX identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ORKESTRIX failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ORKESTRIX is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ORKESTRIX as the basis for migration or production guidance.
