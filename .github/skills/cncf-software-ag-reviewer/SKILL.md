...
---
# Cncf Software Ag Reviewer

## Purpose

Review and operate Software AG using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Software AG code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Software AG requires evidence-backed assessment.
...
- First-party source: https://www.softwareag.com/en_corporate/platform/adabas-natural/cloud-transformation.html
...
- [ ] Trace Software AG inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Software AG defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Software AG least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Software AG resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Software AG release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Software AG dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Software AG identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Software AG change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Software AG documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Software AG architecture and dependency summary with trust boundaries and persistent state.
- Include Software AG validation commands or tests that are safe for the stated environment.
...
- Software AG identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Software AG failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Software AG is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Software AG as the basis for migration or production guidance.
