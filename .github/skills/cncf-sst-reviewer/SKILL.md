---
name: "cncf-sst-reviewer"
description: "Review SST using its official documentation and repository in the Serverless / Framework category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate SST using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SST code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SST requires evidence-backed assessment.
...
- First-party source: https://github.com/sst/sst
...
- [ ] Trace SST inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SST defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SST least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SST resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SST release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SST dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SST identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SST change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SST documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SST architecture and dependency summary with trust boundaries and persistent state.
- Include SST validation commands or tests that are safe for the stated environment.
...
- SST identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SST failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SST is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SST as the basis for migration or production guidance.
