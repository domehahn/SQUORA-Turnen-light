...
---
# Cncf Starlingx Reviewer

## Purpose

Review and operate StarlingX using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- StarlingX code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving StarlingX requires evidence-backed assessment.
...
- First-party source: https://github.com/starlingx/config
...
- [ ] Trace StarlingX inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check StarlingX defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check StarlingX least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check StarlingX resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check StarlingX release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check StarlingX dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If StarlingX identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a StarlingX change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- StarlingX documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a StarlingX architecture and dependency summary with trust boundaries and persistent state.
- Include StarlingX validation commands or tests that are safe for the stated environment.
...
- StarlingX identity, version, configuration, dependencies, and deployment assumptions are explicit.
- StarlingX failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that StarlingX is secure, supported, compatible, or suitable.
- Using an unpinned latest version of StarlingX as the basis for migration or production guidance.
