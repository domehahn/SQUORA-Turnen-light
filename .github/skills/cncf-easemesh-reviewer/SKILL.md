---
name: "cncf-easemesh-reviewer"
description: "Review EaseMesh using its official documentation and repository in the Orchestration & Management / Service Mesh category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Easemesh Reviewer

## Purpose

Review and operate EaseMesh using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- EaseMesh code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving EaseMesh requires evidence-backed assessment.
...
- First-party source: https://github.com/megaease/easemesh
...
- [ ] Trace EaseMesh inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check EaseMesh defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check EaseMesh least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check EaseMesh resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check EaseMesh release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check EaseMesh dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If EaseMesh identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a EaseMesh change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- EaseMesh documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a EaseMesh architecture and dependency summary with trust boundaries and persistent state.
- Include EaseMesh validation commands or tests that are safe for the stated environment.
...
- EaseMesh identity, version, configuration, dependencies, and deployment assumptions are explicit.
- EaseMesh failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that EaseMesh is secure, supported, compatible, or suitable.
- Using an unpinned latest version of EaseMesh as the basis for migration or production guidance.
