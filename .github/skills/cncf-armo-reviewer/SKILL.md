---
name: "cncf-armo-reviewer"
description: "Review ARMO using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate ARMO using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ARMO code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ARMO requires evidence-backed assessment.
...
- [ ] Trace ARMO inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ARMO defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ARMO least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ARMO resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ARMO release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ARMO dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ARMO identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ARMO change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ARMO documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ARMO architecture and dependency summary with trust boundaries and persistent state.
- Include ARMO validation commands or tests that are safe for the stated environment.
...
- ARMO identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ARMO failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ARMO is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ARMO as the basis for migration or production guidance.
