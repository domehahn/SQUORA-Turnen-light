---
name: "cncf-wiit-kcsp-reviewer"
description: "Review WIIT (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. We provide CNCF certified and highly compliant managed kubernetes for edge and cloud. From consulting to day 2 operations. Made in Europe - 100% data protection and ISO27001 certified."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Wiit Kcsp Reviewer

## Purpose

Review and operate WIIT (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- WIIT (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving WIIT (KCSP) requires evidence-backed assessment.
...
- [ ] Trace WIIT (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check WIIT (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check WIIT (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check WIIT (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check WIIT (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check WIIT (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If WIIT (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a WIIT (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- WIIT (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a WIIT (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include WIIT (KCSP) validation commands or tests that are safe for the stated environment.
...
- WIIT (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- WIIT (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that WIIT (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of WIIT (KCSP) as the basis for migration or production guidance.
