---
name: "cncf-bfe-reviewer"
description: "Review BFE using its official documentation and repository in the Orchestration & Management / Service Proxy category. Open-source layer 7 load balancer derived from proprietary Baidu FrontEnd"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate BFE using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- BFE code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving BFE requires evidence-backed assessment.
...
- First-party source: https://github.com/bfenetworks/bfe
...
- [ ] Trace BFE inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check BFE defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check BFE least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check BFE resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check BFE release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check BFE dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If BFE identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a BFE change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- BFE documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a BFE architecture and dependency summary with trust boundaries and persistent state.
- Include BFE validation commands or tests that are safe for the stated environment.
...
- BFE identity, version, configuration, dependencies, and deployment assumptions are explicit.
- BFE failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that BFE is secure, supported, compatible, or suitable.
- Using an unpinned latest version of BFE as the basis for migration or production guidance.
