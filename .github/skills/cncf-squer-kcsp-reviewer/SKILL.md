---
name: "cncf-squer-kcsp-reviewer"
description: "Review SQUER (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. We support customers in utilizing Kubernetes best for their needs. This includes building truly cloud-native applications and Kubernetes-based developer platforms."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Squer Kcsp Reviewer

## Purpose

Review and operate SQUER (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SQUER (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SQUER (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.squer.io/services/kubernetes
...
- [ ] Trace SQUER (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SQUER (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SQUER (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SQUER (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SQUER (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SQUER (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SQUER (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SQUER (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SQUER (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SQUER (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include SQUER (KCSP) validation commands or tests that are safe for the stated environment.
...
- SQUER (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SQUER (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SQUER (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SQUER (KCSP) as the basis for migration or production guidance.
