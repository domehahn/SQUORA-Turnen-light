---
name: "cncf-cast-ai-reviewer"
description: "Review cast.ai using its official documentation and repository in the Observability and Analysis / Continuous Optimization category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Cast Ai Reviewer

## Purpose

Review and operate cast.ai using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- cast.ai code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving cast.ai requires evidence-backed assessment.
...
- First-party source: https://cast.ai/
...
- [ ] Trace cast.ai inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check cast.ai defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check cast.ai least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check cast.ai resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check cast.ai release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check cast.ai dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If cast.ai identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a cast.ai change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- cast.ai documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a cast.ai architecture and dependency summary with trust boundaries and persistent state.
- Include cast.ai validation commands or tests that are safe for the stated environment.
...
- cast.ai identity, version, configuration, dependencies, and deployment assumptions are explicit.
- cast.ai failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that cast.ai is secure, supported, compatible, or suitable.
- Using an unpinned latest version of cast.ai as the basis for migration or production guidance.
