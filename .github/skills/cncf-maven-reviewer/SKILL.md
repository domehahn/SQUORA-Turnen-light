...
Review and operate Maven using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Maven code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Maven requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/maven
...
- [ ] Trace Maven inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Maven defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Maven least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Maven resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Maven release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Maven dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Maven identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Maven change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Maven documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Maven architecture and dependency summary with trust boundaries and persistent state.
- Include Maven validation commands or tests that are safe for the stated environment.
...
- Maven identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Maven failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Maven is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Maven as the basis for migration or production guidance.
