---
name: "cncf-kuma-reviewer"
description: "Review Kuma using its official documentation and repository in the Orchestration & Management / Service Mesh category. The universal Envoy service mesh for distributed service connectivity"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Kuma using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kuma code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kuma requires evidence-backed assessment.
...
- First-party source: https://github.com/kumahq/kuma
...
- [ ] Trace Kuma inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kuma defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kuma least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kuma resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kuma release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kuma dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kuma identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kuma change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kuma documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kuma architecture and dependency summary with trust boundaries and persistent state.
- Include Kuma validation commands or tests that are safe for the stated environment.
...
- Kuma identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kuma failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kuma is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kuma as the basis for migration or production guidance.
