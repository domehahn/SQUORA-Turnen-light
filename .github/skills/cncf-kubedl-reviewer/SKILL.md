---
name: "cncf-kubedl-reviewer"
description: "Review KubeDL using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kubedl Reviewer

## Purpose

Review and operate KubeDL using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeDL code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeDL requires evidence-backed assessment.
...
- First-party source: https://github.com/kubedl-io/kubedl
...
- [ ] Trace KubeDL inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeDL defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeDL least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeDL resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeDL release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeDL dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeDL identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeDL change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeDL documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeDL architecture and dependency summary with trust boundaries and persistent state.
- Include KubeDL validation commands or tests that are safe for the stated environment.
...
- KubeDL identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeDL failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeDL is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeDL as the basis for migration or production guidance.
