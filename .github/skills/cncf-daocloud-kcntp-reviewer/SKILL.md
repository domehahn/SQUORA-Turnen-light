...
---
# Cncf Daocloud Kcntp Reviewer

## Purpose

Review and operate DaoCloud (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DaoCloud (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DaoCloud (KCNTP) requires evidence-backed assessment.
...
- [ ] Trace DaoCloud (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DaoCloud (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DaoCloud (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DaoCloud (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DaoCloud (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DaoCloud (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DaoCloud (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DaoCloud (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DaoCloud (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DaoCloud (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include DaoCloud (KCNTP) validation commands or tests that are safe for the stated environment.
...
- DaoCloud (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DaoCloud (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DaoCloud (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DaoCloud (KCNTP) as the basis for migration or production guidance.
