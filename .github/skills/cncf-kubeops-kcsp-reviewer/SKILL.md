---
name: "cncf-kubeops-kcsp-reviewer"
description: "Review KubeOps (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. KubeOps provides assistance with all Kubernetes-related issues, like setting up brand-new systems, managed services for existing Kubernetes infrastructure, our own software solutions based on Open Source technology, as well as consulting,  training and certification."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kubeops Kcsp Reviewer

## Purpose

Review and operate KubeOps (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeOps (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeOps (KCSP) requires evidence-backed assessment.
...
- [ ] Trace KubeOps (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeOps (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeOps (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeOps (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeOps (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeOps (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeOps (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeOps (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeOps (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeOps (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include KubeOps (KCSP) validation commands or tests that are safe for the stated environment.
...
- KubeOps (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeOps (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeOps (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeOps (KCSP) as the basis for migration or production guidance.
