---
name: "cncf-sumo-logic-reviewer"
description: "Review Sumo Logic using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Sumo Logic Reviewer

## Purpose

Review and operate Sumo Logic using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Sumo Logic code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Sumo Logic requires evidence-backed assessment.
...
- First-party source: https://www.sumologic.com/
...
- [ ] Trace Sumo Logic inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Sumo Logic defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Sumo Logic least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Sumo Logic resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Sumo Logic release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Sumo Logic dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Sumo Logic identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Sumo Logic change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Sumo Logic documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Sumo Logic architecture and dependency summary with trust boundaries and persistent state.
- Include Sumo Logic validation commands or tests that are safe for the stated environment.
...
- Sumo Logic identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Sumo Logic failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Sumo Logic is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Sumo Logic as the basis for migration or production guidance.
