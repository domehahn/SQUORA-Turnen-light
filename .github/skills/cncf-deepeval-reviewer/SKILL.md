---
name: "cncf-deepeval-reviewer"
description: "Review DeepEval using its official documentation and repository in the AI Agent / Evaluation category. The LLM Evaluation Framework"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Deepeval Reviewer

## Purpose

Review and operate DeepEval using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DeepEval code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DeepEval requires evidence-backed assessment.
...
- First-party source: https://github.com/confident-ai/deepeval
...
- [ ] Trace DeepEval inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DeepEval defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DeepEval least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DeepEval resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DeepEval release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DeepEval dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DeepEval identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DeepEval change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DeepEval documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DeepEval architecture and dependency summary with trust boundaries and persistent state.
- Include DeepEval validation commands or tests that are safe for the stated environment.
...
- DeepEval identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DeepEval failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DeepEval is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DeepEval as the basis for migration or production guidance.
