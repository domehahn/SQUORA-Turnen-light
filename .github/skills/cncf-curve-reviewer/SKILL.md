...
Review and operate Curve using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Curve code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Curve requires evidence-backed assessment.
...
- First-party source: https://github.com/opencurve/curve
...
- [ ] Trace Curve inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Curve defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Curve least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Curve resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Curve release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Curve dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Curve identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Curve change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Curve documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Curve architecture and dependency summary with trust boundaries and persistent state.
- Include Curve validation commands or tests that are safe for the stated environment.
...
- Curve identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Curve failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Curve is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Curve as the basis for migration or production guidance.
