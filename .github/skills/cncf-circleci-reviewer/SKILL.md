---
name: "cncf-circleci-reviewer"
description: "Review CircleCI using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Circleci Reviewer

## Purpose

Review and operate CircleCI using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CircleCI code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CircleCI requires evidence-backed assessment.
...
- [ ] Trace CircleCI inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CircleCI defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CircleCI least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CircleCI resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CircleCI release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CircleCI dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CircleCI identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CircleCI change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CircleCI documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CircleCI architecture and dependency summary with trust boundaries and persistent state.
- Include CircleCI validation commands or tests that are safe for the stated environment.
...
- CircleCI identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CircleCI failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CircleCI is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CircleCI as the basis for migration or production guidance.
