---
name: "cncf-cerbos-reviewer"
description: "Review Cerbos using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Cerbos Reviewer

## Purpose

Review and operate Cerbos using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Cerbos code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Cerbos requires evidence-backed assessment.
...
- First-party source: https://github.com/cerbos/cerbos
...
- [ ] Trace Cerbos inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Cerbos defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Cerbos least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Cerbos resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Cerbos release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Cerbos dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Cerbos identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Cerbos change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Cerbos documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Cerbos architecture and dependency summary with trust boundaries and persistent state.
- Include Cerbos validation commands or tests that are safe for the stated environment.
...
- Cerbos identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Cerbos failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Cerbos is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Cerbos as the basis for migration or production guidance.
