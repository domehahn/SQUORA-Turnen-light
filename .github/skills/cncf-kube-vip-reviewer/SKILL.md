---
name: "cncf-kube-vip-reviewer"
description: "Review kube-vip using its official documentation and repository in the Runtime / Cloud Native Network category. Kubernetes Virtual IP and Load-Balancer for both control plane and Kubernetes services"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kube Vip Reviewer

## Purpose

Review and operate kube-vip using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- kube-vip code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving kube-vip requires evidence-backed assessment.
...
- First-party source: https://github.com/kube-vip/kube-vip
...
- [ ] Trace kube-vip inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check kube-vip defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check kube-vip least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check kube-vip resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check kube-vip release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check kube-vip dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If kube-vip identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a kube-vip change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- kube-vip documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a kube-vip architecture and dependency summary with trust boundaries and persistent state.
- Include kube-vip validation commands or tests that are safe for the stated environment.
...
- kube-vip identity, version, configuration, dependencies, and deployment assumptions are explicit.
- kube-vip failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that kube-vip is secure, supported, compatible, or suitable.
- Using an unpinned latest version of kube-vip as the basis for migration or production guidance.
