---
name: "cncf-netdata-reviewer"
description: "Review Netdata using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Netdata Reviewer

## Purpose

Review and operate Netdata using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Netdata code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Netdata requires evidence-backed assessment.
...
- First-party source: https://github.com/netdata/netdata
...
- [ ] Trace Netdata inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Netdata defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Netdata least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Netdata resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Netdata release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Netdata dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Netdata identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Netdata change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Netdata documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Netdata architecture and dependency summary with trust boundaries and persistent state.
- Include Netdata validation commands or tests that are safe for the stated environment.
...
- Netdata identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Netdata failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Netdata is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Netdata as the basis for migration or production guidance.
