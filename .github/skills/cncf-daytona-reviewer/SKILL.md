---
name: "cncf-daytona-reviewer"
description: "Review Daytona using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Daytona Reviewer

## Purpose

Review and operate Daytona using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Daytona code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Daytona requires evidence-backed assessment.
...
- First-party source: https://github.com/daytonaio/daytona
...
- [ ] Trace Daytona inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Daytona defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Daytona least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Daytona resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Daytona release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Daytona dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Daytona identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Daytona change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Daytona documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Daytona architecture and dependency summary with trust boundaries and persistent state.
- Include Daytona validation commands or tests that are safe for the stated environment.
...
- Daytona identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Daytona failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Daytona is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Daytona as the basis for migration or production guidance.
