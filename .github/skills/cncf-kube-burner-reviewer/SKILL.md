---
name: "cncf-kube-burner-reviewer"
description: "Review Kube-burner using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category. Kubernetes performance and scale test orchestration framework written in golang"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kube Burner Reviewer

## Purpose

Review and operate Kube-burner using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kube-burner code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kube-burner requires evidence-backed assessment.
...
- First-party source: https://github.com/kube-burner/kube-burner
...
- [ ] Trace Kube-burner inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kube-burner defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kube-burner least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kube-burner resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kube-burner release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kube-burner dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kube-burner identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kube-burner change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kube-burner documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kube-burner architecture and dependency summary with trust boundaries and persistent state.
- Include Kube-burner validation commands or tests that are safe for the stated environment.
...
- Kube-burner identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kube-burner failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kube-burner is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kube-burner as the basis for migration or production guidance.
