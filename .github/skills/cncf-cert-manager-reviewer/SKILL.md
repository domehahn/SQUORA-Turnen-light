---
name: "cncf-cert-manager-reviewer"
description: "Review cert-manager using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Cert Manager Reviewer

## Purpose

Review and operate cert-manager using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- cert-manager code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving cert-manager requires evidence-backed assessment.
...
- First-party source: https://github.com/cert-manager/cert-manager
...
- [ ] Trace cert-manager inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check cert-manager defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check cert-manager least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check cert-manager resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check cert-manager release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check cert-manager dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If cert-manager identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a cert-manager change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- cert-manager documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a cert-manager architecture and dependency summary with trust boundaries and persistent state.
- Include cert-manager validation commands or tests that are safe for the stated environment.
...
- cert-manager identity, version, configuration, dependencies, and deployment assumptions are explicit.
- cert-manager failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that cert-manager is secure, supported, compatible, or suitable.
- Using an unpinned latest version of cert-manager as the basis for migration or production guidance.
