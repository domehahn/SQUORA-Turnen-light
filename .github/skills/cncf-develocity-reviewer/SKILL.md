...
Review and operate Develocity using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Develocity code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Develocity requires evidence-backed assessment.
...
- First-party source: https://gradle.com/
...
- [ ] Trace Develocity inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Develocity defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Develocity least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Develocity resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Develocity release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Develocity dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Develocity identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Develocity change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Develocity documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Develocity architecture and dependency summary with trust boundaries and persistent state.
- Include Develocity validation commands or tests that are safe for the stated environment.
...
- Develocity identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Develocity failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Develocity is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Develocity as the basis for migration or production guidance.
