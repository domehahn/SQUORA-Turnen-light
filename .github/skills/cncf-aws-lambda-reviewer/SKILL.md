---
name: "cncf-aws-lambda-reviewer"
description: "Review AWS Lambda using its official documentation and repository in the Serverless / Hosted Platform category. AWS Lambda lets you run code without provisioning or managing servers. You pay only for the compute time you consume - there is no charge when your code is not running."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Aws Lambda Reviewer

## Purpose

Review and operate AWS Lambda using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- AWS Lambda code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving AWS Lambda requires evidence-backed assessment.
...
- First-party source: https://aws.amazon.com/lambda/
...
- [ ] Trace AWS Lambda inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check AWS Lambda defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check AWS Lambda least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check AWS Lambda resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check AWS Lambda release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check AWS Lambda dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If AWS Lambda identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a AWS Lambda change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- AWS Lambda documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a AWS Lambda architecture and dependency summary with trust boundaries and persistent state.
- Include AWS Lambda validation commands or tests that are safe for the stated environment.
...
- AWS Lambda identity, version, configuration, dependencies, and deployment assumptions are explicit.
- AWS Lambda failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that AWS Lambda is secure, supported, compatible, or suitable.
- Using an unpinned latest version of AWS Lambda as the basis for migration or production guidance.
