...
---
# Cncf Oscal Compass Reviewer

## Purpose

Review and operate OSCAL-COMPASS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OSCAL-COMPASS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OSCAL-COMPASS requires evidence-backed assessment.
...
- First-party source: https://github.com/oscal-compass/compliance-trestle
...
- [ ] Trace OSCAL-COMPASS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OSCAL-COMPASS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OSCAL-COMPASS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OSCAL-COMPASS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OSCAL-COMPASS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OSCAL-COMPASS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OSCAL-COMPASS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OSCAL-COMPASS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OSCAL-COMPASS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OSCAL-COMPASS architecture and dependency summary with trust boundaries and persistent state.
- Include OSCAL-COMPASS validation commands or tests that are safe for the stated environment.
...
- OSCAL-COMPASS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OSCAL-COMPASS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OSCAL-COMPASS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OSCAL-COMPASS as the basis for migration or production guidance.
