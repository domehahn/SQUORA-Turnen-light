---
name: "cncf-wit-binddgen-reviewer"
description: "Review Wit-binddgen using its official documentation and repository in the Wasm / Tooling category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Wit Binddgen Reviewer

## Purpose

Review and operate Wit-binddgen using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Wit-binddgen code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Wit-binddgen requires evidence-backed assessment.
...
- First-party source: https://github.com/bytecodealliance/wit-bindgen
...
- [ ] Trace Wit-binddgen inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Wit-binddgen defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Wit-binddgen least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Wit-binddgen resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Wit-binddgen release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Wit-binddgen dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Wit-binddgen identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Wit-binddgen change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Wit-binddgen documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Wit-binddgen architecture and dependency summary with trust boundaries and persistent state.
- Include Wit-binddgen validation commands or tests that are safe for the stated environment.
...
- Wit-binddgen identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Wit-binddgen failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Wit-binddgen is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Wit-binddgen as the basis for migration or production guidance.
