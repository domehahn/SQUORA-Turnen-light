---
name: "cncf-apache-streampipes-reviewer"
description: "Review Apache StreamPipes using its official documentation and repository in the App Definition and Development / Streaming & Messaging category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Apache Streampipes Reviewer

## Purpose

Review and operate Apache StreamPipes using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Apache StreamPipes code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Apache StreamPipes requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/streampipes
...
- [ ] Trace Apache StreamPipes inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Apache StreamPipes defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Apache StreamPipes least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Apache StreamPipes resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Apache StreamPipes release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Apache StreamPipes dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Apache StreamPipes identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Apache StreamPipes change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Apache StreamPipes documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Apache StreamPipes architecture and dependency summary with trust boundaries and persistent state.
- Include Apache StreamPipes validation commands or tests that are safe for the stated environment.
...
- Apache StreamPipes identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Apache StreamPipes failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Apache StreamPipes is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Apache StreamPipes as the basis for migration or production guidance.
