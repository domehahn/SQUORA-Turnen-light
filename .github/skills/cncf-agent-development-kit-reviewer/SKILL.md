---
name: "cncf-agent-development-kit-reviewer"
description: "Review Agent Development Kit using its official documentation and repository in the AI Agent / Agent Framework category. An open-source, code-first Python toolkit for building, evaluating, and deploying sophisticated AI agents with flexibility and control."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Agent Development Kit Reviewer

## Purpose

Review and operate Agent Development Kit using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Agent Development Kit code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Agent Development Kit requires evidence-backed assessment.
...
- CNCF classification: AI Agent / Agent Framework
- CNCF project maturity field: not specified
- First-party source: https://github.com/google/adk-python
...
- [ ] Trace Agent Development Kit inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Agent Development Kit defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Agent Development Kit least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Agent Development Kit resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Agent Development Kit release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Agent Development Kit dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Agent Development Kit identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Agent Development Kit change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Agent Development Kit documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Agent Development Kit architecture and dependency summary with trust boundaries and persistent state.
- Include Agent Development Kit validation commands or tests that are safe for the stated environment.
...
- Agent Development Kit identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Agent Development Kit failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Agent Development Kit is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Agent Development Kit as the basis for migration or production guidance.
