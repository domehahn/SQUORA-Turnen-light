...
---
# Cncf Catalyst By Zoho Reviewer

## Purpose

Review and operate Catalyst by Zoho using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Catalyst by Zoho code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Catalyst by Zoho requires evidence-backed assessment.
...
- First-party source: https://catalyst.zoho.com/?utm_source=cncf
...
- [ ] Trace Catalyst by Zoho inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Catalyst by Zoho defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Catalyst by Zoho least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Catalyst by Zoho resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Catalyst by Zoho release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Catalyst by Zoho dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Catalyst by Zoho identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Catalyst by Zoho change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Catalyst by Zoho documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Catalyst by Zoho architecture and dependency summary with trust boundaries and persistent state.
- Include Catalyst by Zoho validation commands or tests that are safe for the stated environment.
...
- Catalyst by Zoho identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Catalyst by Zoho failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Catalyst by Zoho is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Catalyst by Zoho as the basis for migration or production guidance.
