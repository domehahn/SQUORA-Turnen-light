---
name: "cncf-timescale-reviewer"
description: "Review Timescale using its official documentation and repository in the App Definition and Development / Database category. PostgreSQL for time‑series. TimescaleDB is the leading open-source relational database for time-series data. Fully managed or self‑hosted."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Timescale Reviewer

## Purpose

Review and operate Timescale using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Timescale code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Timescale requires evidence-backed assessment.
...
- First-party source: https://github.com/timescale/timescaledb
...
- [ ] Trace Timescale inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Timescale defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Timescale least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Timescale resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Timescale release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Timescale dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Timescale identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Timescale change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Timescale documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Timescale architecture and dependency summary with trust boundaries and persistent state.
- Include Timescale validation commands or tests that are safe for the stated environment.
...
- Timescale identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Timescale failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Timescale is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Timescale as the basis for migration or production guidance.
