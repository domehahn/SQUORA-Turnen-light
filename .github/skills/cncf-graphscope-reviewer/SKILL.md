---
name: "cncf-graphscope-reviewer"
description: "Review GraphScope using its official documentation and repository in the App Definition and Development / Database category. A One-Stop Large-Scale Graph Computing System from Alibaba"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Graphscope Reviewer

## Purpose

Review and operate GraphScope using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- GraphScope code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving GraphScope requires evidence-backed assessment.
...
- First-party source: https://github.com/alibaba/graphscope
...
- [ ] Trace GraphScope inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check GraphScope defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check GraphScope least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check GraphScope resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check GraphScope release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check GraphScope dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If GraphScope identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a GraphScope change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- GraphScope documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a GraphScope architecture and dependency summary with trust boundaries and persistent state.
- Include GraphScope validation commands or tests that are safe for the stated environment.
...
- GraphScope identity, version, configuration, dependencies, and deployment assumptions are explicit.
- GraphScope failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that GraphScope is secure, supported, compatible, or suitable.
- Using an unpinned latest version of GraphScope as the basis for migration or production guidance.
