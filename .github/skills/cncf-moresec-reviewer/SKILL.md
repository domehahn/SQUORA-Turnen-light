...
Review and operate MoreSec using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- MoreSec code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving MoreSec requires evidence-backed assessment.
...
- First-party source: https://www.moresec.cn/
...
- [ ] Trace MoreSec inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check MoreSec defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check MoreSec least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check MoreSec resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check MoreSec release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check MoreSec dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If MoreSec identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a MoreSec change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- MoreSec documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a MoreSec architecture and dependency summary with trust boundaries and persistent state.
- Include MoreSec validation commands or tests that are safe for the stated environment.
...
- MoreSec identity, version, configuration, dependencies, and deployment assumptions are explicit.
- MoreSec failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that MoreSec is secure, supported, compatible, or suitable.
- Using an unpinned latest version of MoreSec as the basis for migration or production guidance.
