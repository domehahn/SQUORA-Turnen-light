---
name: "cncf-second-state-functions-reviewer"
description: "Review Second State Functions using its official documentation and repository in the Serverless / Hosted Platform category. High-performance Function as a Service (FaaS), powered by WasmEdge."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Second State Functions Reviewer

## Purpose

Review and operate Second State Functions using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Second State Functions code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Second State Functions requires evidence-backed assessment.
...
- First-party source: https://www.secondstate.io/faas
...
- [ ] Trace Second State Functions inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Second State Functions defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Second State Functions least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Second State Functions resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Second State Functions release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Second State Functions dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Second State Functions identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Second State Functions change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Second State Functions documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Second State Functions architecture and dependency summary with trust boundaries and persistent state.
- Include Second State Functions validation commands or tests that are safe for the stated environment.
...
- Second State Functions identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Second State Functions failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Second State Functions is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Second State Functions as the basis for migration or production guidance.
