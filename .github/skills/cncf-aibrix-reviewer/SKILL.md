---
name: "cncf-aibrix-reviewer"
description: "Review AIBrix using its official documentation and repository in the Inference / Framework category. Cost-efficient and pluggable Infrastructure components for GenAI inference."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Aibrix Reviewer

## Purpose

Review and operate AIBrix using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- AIBrix code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving AIBrix requires evidence-backed assessment.
...
- First-party source: https://github.com/vllm-project/aibrix
...
- [ ] Trace AIBrix inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check AIBrix defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check AIBrix least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check AIBrix resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check AIBrix release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check AIBrix dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If AIBrix identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a AIBrix change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- AIBrix documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a AIBrix architecture and dependency summary with trust boundaries and persistent state.
- Include AIBrix validation commands or tests that are safe for the stated environment.
...
- AIBrix identity, version, configuration, dependencies, and deployment assumptions are explicit.
- AIBrix failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that AIBrix is secure, supported, compatible, or suitable.
- Using an unpinned latest version of AIBrix as the basis for migration or production guidance.
