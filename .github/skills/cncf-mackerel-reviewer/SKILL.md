---
name: "cncf-mackerel-reviewer"
description: "Review Mackerel using its official documentation and repository in the Observability and Analysis / Observability category. The server monitoring platform we always wanted."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Mackerel Reviewer

## Purpose

Review and operate Mackerel using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Mackerel code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Mackerel requires evidence-backed assessment.
...
- First-party source: https://en.mackerel.io
...
- [ ] Trace Mackerel inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Mackerel defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Mackerel least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Mackerel resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Mackerel release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Mackerel dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Mackerel identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Mackerel change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Mackerel documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Mackerel architecture and dependency summary with trust boundaries and persistent state.
- Include Mackerel validation commands or tests that are safe for the stated environment.
...
- Mackerel identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Mackerel failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Mackerel is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Mackerel as the basis for migration or production guidance.
