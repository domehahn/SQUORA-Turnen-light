---
name: "cncf-kitops-reviewer"
description: "Review KitOps using its official documentation and repository in the Provisioning / Automation & Configuration category. An open standard for packaging, managing, and deploying ML models and artifacts across different systems"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kitops Reviewer

## Purpose

Review and operate KitOps using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KitOps code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KitOps requires evidence-backed assessment.
...
- First-party source: https://github.com/kitops-ml/kitops
...
- [ ] Trace KitOps inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KitOps defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KitOps least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KitOps resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KitOps release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KitOps dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KitOps identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KitOps change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KitOps documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KitOps architecture and dependency summary with trust boundaries and persistent state.
- Include KitOps validation commands or tests that are safe for the stated environment.
...
- KitOps identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KitOps failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KitOps is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KitOps as the basis for migration or production guidance.
