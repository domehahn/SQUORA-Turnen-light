...
---
# Cncf Deeperthanblue Kcsp Reviewer

## Purpose

Review and operate DeeperThanBlue (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DeeperThanBlue (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DeeperThanBlue (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.deeperthanblue.co.uk/solutions/cloud/kubernetes-certified-service-provider/
...
- [ ] Trace DeeperThanBlue (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DeeperThanBlue (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DeeperThanBlue (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DeeperThanBlue (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DeeperThanBlue (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DeeperThanBlue (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DeeperThanBlue (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DeeperThanBlue (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DeeperThanBlue (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DeeperThanBlue (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include DeeperThanBlue (KCSP) validation commands or tests that are safe for the stated environment.
...
- DeeperThanBlue (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DeeperThanBlue (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DeeperThanBlue (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DeeperThanBlue (KCSP) as the basis for migration or production guidance.
