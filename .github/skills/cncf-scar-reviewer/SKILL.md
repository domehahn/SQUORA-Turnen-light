---
name: "cncf-scar-reviewer"
description: "Review SCAR using its official documentation and repository in the Serverless / Tools category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate SCAR using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SCAR code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SCAR requires evidence-backed assessment.
...
- First-party source: https://github.com/grycap/scar
...
- [ ] Trace SCAR inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SCAR defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SCAR least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SCAR resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SCAR release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SCAR dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SCAR identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SCAR change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SCAR documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SCAR architecture and dependency summary with trust boundaries and persistent state.
- Include SCAR validation commands or tests that are safe for the stated environment.
...
- SCAR identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SCAR failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SCAR is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SCAR as the basis for migration or production guidance.
