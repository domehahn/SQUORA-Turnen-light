---
name: "cncf-mewz-reviewer"
description: "Review Mewz using its official documentation and repository in the Wasm / Runtimes category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Mewz using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Mewz code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Mewz requires evidence-backed assessment.
...
- First-party source: https://github.com/mewz-project/mewz
...
- [ ] Trace Mewz inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Mewz defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Mewz least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Mewz resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Mewz release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Mewz dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Mewz identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Mewz change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Mewz documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Mewz architecture and dependency summary with trust boundaries and persistent state.
- Include Mewz validation commands or tests that are safe for the stated environment.
...
- Mewz identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Mewz failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Mewz is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Mewz as the basis for migration or production guidance.
