---
name: "cncf-sel4-reviewer"
description: "Review seL4 using its official documentation and repository in the Wasm / Edge/Bare metal category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate seL4 using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- seL4 code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving seL4 requires evidence-backed assessment.
...
- First-party source: https://github.com/seL4/seL4
...
- [ ] Trace seL4 inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check seL4 defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check seL4 least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check seL4 resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check seL4 release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check seL4 dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If seL4 identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a seL4 change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- seL4 documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a seL4 architecture and dependency summary with trust boundaries and persistent state.
- Include seL4 validation commands or tests that are safe for the stated environment.
...
- seL4 identity, version, configuration, dependencies, and deployment assumptions are explicit.
- seL4 failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that seL4 is secure, supported, compatible, or suitable.
- Using an unpinned latest version of seL4 as the basis for migration or production guidance.
