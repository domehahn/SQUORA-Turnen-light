---
name: "cncf-huawei-kcntp-reviewer"
description: "Review Huawei (KCNTP) using its official documentation and repository in the Special / Kubernetes and Cloud Native Training Partner category. HUAWEI CLOUD provides customers with stable, reliable, secure, and sustainably growing cloud services. It helps large enterprises address challenges in cloud transformation and enables them to take better advantages of potential business opportunities. It also helps small- and medium-sized enterprises expand their business growth and rise to challenges."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Huawei Kcntp Reviewer

## Purpose

Review and operate Huawei (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Huawei (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Huawei (KCNTP) requires evidence-backed assessment.
...
- [ ] Trace Huawei (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Huawei (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Huawei (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Huawei (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Huawei (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Huawei (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Huawei (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Huawei (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Huawei (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Huawei (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include Huawei (KCNTP) validation commands or tests that are safe for the stated environment.
...
- Huawei (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Huawei (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Huawei (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Huawei (KCNTP) as the basis for migration or production guidance.
