---
name: "cncf-helios-reviewer"
description: "Review Helios using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Helios Reviewer

## Purpose

Review and operate Helios using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Helios code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Helios requires evidence-backed assessment.
...
- First-party source: https://www.gethelios.dev/
...
- [ ] Trace Helios inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Helios defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Helios least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Helios resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Helios release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Helios dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Helios identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Helios change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Helios documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Helios architecture and dependency summary with trust boundaries and persistent state.
- Include Helios validation commands or tests that are safe for the stated environment.
...
- Helios identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Helios failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Helios is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Helios as the basis for migration or production guidance.
