...
---
# Cncf Whitestack Whitemist Reviewer

## Purpose

Review and operate Whitestack WhiteMist using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Whitestack WhiteMist code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Whitestack WhiteMist requires evidence-backed assessment.
...
- First-party source: https://whitestack.com/products/kubernetes-based-containers-orchestration
...
- [ ] Trace Whitestack WhiteMist inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Whitestack WhiteMist defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Whitestack WhiteMist least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Whitestack WhiteMist resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Whitestack WhiteMist release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Whitestack WhiteMist dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Whitestack WhiteMist identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Whitestack WhiteMist change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Whitestack WhiteMist documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Whitestack WhiteMist architecture and dependency summary with trust boundaries and persistent state.
- Include Whitestack WhiteMist validation commands or tests that are safe for the stated environment.
...
- Whitestack WhiteMist identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Whitestack WhiteMist failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Whitestack WhiteMist is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Whitestack WhiteMist as the basis for migration or production guidance.
