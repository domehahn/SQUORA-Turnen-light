---
name: "cncf-xsky-reviewer"
description: "Review XSKY using its official documentation and repository in the Runtime / Cloud Native Storage category. XSKY (Beijing) Data Technology Co., Ltd. is a high-tech enterprise focusing on software defined infrastructure, providing software defined distributed storage product of enterprise-grade and helping customers achieve innovation in data structure."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate XSKY using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- XSKY code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving XSKY requires evidence-backed assessment.
...
- First-party source: https://www.xsky.com
...
- [ ] Trace XSKY inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check XSKY defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check XSKY least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check XSKY resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check XSKY release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check XSKY dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If XSKY identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a XSKY change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- XSKY documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a XSKY architecture and dependency summary with trust boundaries and persistent state.
- Include XSKY validation commands or tests that are safe for the stated environment.
...
- XSKY identity, version, configuration, dependencies, and deployment assumptions are explicit.
- XSKY failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that XSKY is secure, supported, compatible, or suitable.
- Using an unpinned latest version of XSKY as the basis for migration or production guidance.
