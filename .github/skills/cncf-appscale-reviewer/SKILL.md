---
name: "cncf-appscale-reviewer"
description: "Review AppScale using its official documentation and repository in the Serverless / Installable Platform category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Appscale Reviewer

## Purpose

Review and operate AppScale using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- AppScale code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving AppScale requires evidence-backed assessment.
...
- First-party source: https://github.com/AppScale/gts
...
- [ ] Trace AppScale inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check AppScale defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check AppScale least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check AppScale resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check AppScale release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check AppScale dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If AppScale identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a AppScale change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- AppScale documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a AppScale architecture and dependency summary with trust boundaries and persistent state.
- Include AppScale validation commands or tests that are safe for the stated environment.
...
- AppScale identity, version, configuration, dependencies, and deployment assumptions are explicit.
- AppScale failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that AppScale is secure, supported, compatible, or suitable.
- Using an unpinned latest version of AppScale as the basis for migration or production guidance.
