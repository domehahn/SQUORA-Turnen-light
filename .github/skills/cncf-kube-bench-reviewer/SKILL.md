---
name: "cncf-kube-bench-reviewer"
description: "Review kube-bench using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kube Bench Reviewer

## Purpose

Review and operate kube-bench using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- kube-bench code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving kube-bench requires evidence-backed assessment.
...
- First-party source: https://github.com/aquasecurity/kube-bench
...
- [ ] Trace kube-bench inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check kube-bench defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check kube-bench least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check kube-bench resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check kube-bench release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check kube-bench dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If kube-bench identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a kube-bench change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- kube-bench documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a kube-bench architecture and dependency summary with trust boundaries and persistent state.
- Include kube-bench validation commands or tests that are safe for the stated environment.
...
- kube-bench identity, version, configuration, dependencies, and deployment assumptions are explicit.
- kube-bench failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that kube-bench is secure, supported, compatible, or suitable.
- Using an unpinned latest version of kube-bench as the basis for migration or production guidance.
