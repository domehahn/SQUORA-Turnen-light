...
---
# Cncf Vmware Vsphere Reviewer

## Purpose

Review and operate VMware vSphere using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- VMware vSphere code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving VMware vSphere requires evidence-backed assessment.
...
- First-party source: https://www.vmware.com/products/vsphere.html
...
- [ ] Trace VMware vSphere inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check VMware vSphere defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check VMware vSphere least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check VMware vSphere resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check VMware vSphere release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check VMware vSphere dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If VMware vSphere identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a VMware vSphere change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- VMware vSphere documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a VMware vSphere architecture and dependency summary with trust boundaries and persistent state.
- Include VMware vSphere validation commands or tests that are safe for the stated environment.
...
- VMware vSphere identity, version, configuration, dependencies, and deployment assumptions are explicit.
- VMware vSphere failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that VMware vSphere is secure, supported, compatible, or suitable.
- Using an unpinned latest version of VMware vSphere as the basis for migration or production guidance.
