---
name: "cncf-krkn-reviewer"
description: "Review Krkn using its official documentation and repository in the Observability and Analysis / Chaos Engineering category. Chaos testing tool for Kubernetes to identify bottlenecks and improve resilience and performance under failure conditions."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Krkn using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Krkn code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Krkn requires evidence-backed assessment.
...
- First-party source: https://github.com/krkn-chaos/krkn
...
- [ ] Trace Krkn inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Krkn defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Krkn least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Krkn resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Krkn release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Krkn dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Krkn identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Krkn change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Krkn documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Krkn architecture and dependency summary with trust boundaries and persistent state.
- Include Krkn validation commands or tests that are safe for the stated environment.
...
- Krkn identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Krkn failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Krkn is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Krkn as the basis for migration or production guidance.
