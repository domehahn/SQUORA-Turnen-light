---
name: "rust-reviewer"
description: "Review Rust ownership, lifetimes, unsafe boundaries, concurrency, error handling, Cargo dependencies, and tests."
version: "0.1.0"
since: "2026-08-27"
...
- Source code, configuration, dependencies, tests, deployment artifacts, or runtime behavior in Rust engineering change.
...
- ownership and borrowing, lifetimes, unsafe code, Send and Sync, Result handling, Cargo
...
- [ ] Trace Rust engineering inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Rust engineering defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Rust engineering least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Rust engineering resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Rust engineering release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
...
- If Rust engineering identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Rust engineering change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Rust engineering documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Rust engineering architecture and dependency summary with trust boundaries and persistent state.
- Include Rust engineering validation commands or tests that are safe for the stated environment.
...
- Rust engineering identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Rust engineering failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Rust engineering is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Rust engineering as the basis for migration or production guidance.
