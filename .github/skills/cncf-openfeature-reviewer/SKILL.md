---
name: "cncf-openfeature-reviewer"
description: "Review OpenFeature using its official documentation and repository in the Observability and Analysis / Feature Flagging category. Standardizing Feature Flagging for Everyone"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Openfeature Reviewer

## Purpose

Review and operate OpenFeature using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenFeature code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenFeature requires evidence-backed assessment.
...
- First-party source: https://github.com/open-feature/spec
...
- [ ] Trace OpenFeature inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenFeature defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenFeature least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenFeature resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenFeature release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenFeature dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenFeature identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenFeature change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenFeature documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenFeature architecture and dependency summary with trust boundaries and persistent state.
- Include OpenFeature validation commands or tests that are safe for the stated environment.
...
- OpenFeature identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenFeature failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenFeature is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenFeature as the basis for migration or production guidance.
