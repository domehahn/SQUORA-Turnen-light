---
name: "cncf-kubeflow-training-operator-reviewer"
description: "Review Kubeflow Training Operator using its official documentation and repository in the Training / Distributed Training category. Training operators on Kubernetes"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kubeflow Training Operator Reviewer

## Purpose

Review and operate Kubeflow Training Operator using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kubeflow Training Operator code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kubeflow Training Operator requires evidence-backed assessment.
...
- First-party source: https://github.com/kubeflow/training-operator
...
- [ ] Trace Kubeflow Training Operator inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kubeflow Training Operator defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kubeflow Training Operator least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kubeflow Training Operator resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kubeflow Training Operator release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kubeflow Training Operator dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kubeflow Training Operator identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kubeflow Training Operator change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kubeflow Training Operator documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kubeflow Training Operator architecture and dependency summary with trust boundaries and persistent state.
- Include Kubeflow Training Operator validation commands or tests that are safe for the stated environment.
...
- Kubeflow Training Operator identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kubeflow Training Operator failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kubeflow Training Operator is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kubeflow Training Operator as the basis for migration or production guidance.
