---
name: "cncf-tesseral-reviewer"
description: "Review Tesseral using its official documentation and repository in the Provisioning / Security & Compliance category. Tesseral is the open source platform for managing identity and access in business software."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Tesseral Reviewer

## Purpose

Review and operate Tesseral using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Tesseral code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Tesseral requires evidence-backed assessment.
...
- First-party source: https://github.com/tesseral-labs/tesseral
...
- [ ] Trace Tesseral inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Tesseral defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Tesseral least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Tesseral resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Tesseral release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Tesseral dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Tesseral identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Tesseral change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Tesseral documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Tesseral architecture and dependency summary with trust boundaries and persistent state.
- Include Tesseral validation commands or tests that are safe for the stated environment.
...
- Tesseral identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Tesseral failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Tesseral is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Tesseral as the basis for migration or production guidance.
