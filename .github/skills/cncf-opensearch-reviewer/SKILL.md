---
name: "cncf-opensearch-reviewer"
description: "Review OpenSearch using its official documentation and repository in the Observability and Analysis / Observability category. OpenSearch is a community-driven, Apache 2.0-licensed open source search and analytics suite that makes it easy to ingest, search, visualize, and analyze data."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Opensearch Reviewer

## Purpose

Review and operate OpenSearch using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenSearch code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenSearch requires evidence-backed assessment.
...
- First-party source: https://github.com/opensearch-project/opensearch
...
- [ ] Trace OpenSearch inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenSearch defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenSearch least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenSearch resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenSearch release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenSearch dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenSearch identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenSearch change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenSearch documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenSearch architecture and dependency summary with trust boundaries and persistent state.
- Include OpenSearch validation commands or tests that are safe for the stated environment.
...
- OpenSearch identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenSearch failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenSearch is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenSearch as the basis for migration or production guidance.
