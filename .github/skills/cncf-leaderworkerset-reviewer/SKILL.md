---
name: "cncf-leaderworkerset-reviewer"
description: "Review LeaderWorkerSet using its official documentation and repository in the AI Native Infra / Orchestration and Scheduling category. An API for deploying a group of pods as a unit of replication."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Leaderworkerset Reviewer

## Purpose

Review and operate LeaderWorkerSet using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- LeaderWorkerSet code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving LeaderWorkerSet requires evidence-backed assessment.
...
- First-party source: https://github.com/kubernetes-sigs/lws
...
- [ ] Trace LeaderWorkerSet inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check LeaderWorkerSet defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check LeaderWorkerSet least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check LeaderWorkerSet resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check LeaderWorkerSet release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check LeaderWorkerSet dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If LeaderWorkerSet identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a LeaderWorkerSet change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- LeaderWorkerSet documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a LeaderWorkerSet architecture and dependency summary with trust boundaries and persistent state.
- Include LeaderWorkerSet validation commands or tests that are safe for the stated environment.
...
- LeaderWorkerSet identity, version, configuration, dependencies, and deployment assumptions are explicit.
- LeaderWorkerSet failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that LeaderWorkerSet is secure, supported, compatible, or suitable.
- Using an unpinned latest version of LeaderWorkerSet as the basis for migration or production guidance.
