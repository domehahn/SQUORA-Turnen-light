---
name: "cncf-argonix-reviewer"
description: "Review Argonix using its official documentation and repository in the Observability and Analysis / Observability category. Sovereign AI-powered SRE platform combining observability, cloud security posture management, FinOps and incident response for cloud-native environments."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Argonix Reviewer

## Purpose

Review and operate Argonix using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Argonix code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Argonix requires evidence-backed assessment.
...
- First-party source: https://argonix.io
...
- [ ] Trace Argonix inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Argonix defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Argonix least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Argonix resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Argonix release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Argonix dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Argonix identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Argonix change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Argonix documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Argonix architecture and dependency summary with trust boundaries and persistent state.
- Include Argonix validation commands or tests that are safe for the stated environment.
...
- Argonix identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Argonix failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Argonix is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Argonix as the basis for migration or production guidance.
