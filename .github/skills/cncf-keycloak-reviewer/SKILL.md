---
name: "cncf-keycloak-reviewer"
description: "Review Keycloak using its official documentation and repository in the Provisioning / Security & Compliance category. Keycloak is an open-source identity and access management solution for modern applications and services,  built on top of industry security standard protocols."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Keycloak Reviewer

## Purpose

Review and operate Keycloak using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Keycloak code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Keycloak requires evidence-backed assessment.
...
- First-party source: https://github.com/keycloak/keycloak
...
- [ ] Trace Keycloak inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Keycloak defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Keycloak least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Keycloak resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Keycloak release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Keycloak dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Keycloak identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Keycloak change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Keycloak documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Keycloak architecture and dependency summary with trust boundaries and persistent state.
- Include Keycloak validation commands or tests that are safe for the stated environment.
...
- Keycloak identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Keycloak failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Keycloak is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Keycloak as the basis for migration or production guidance.
