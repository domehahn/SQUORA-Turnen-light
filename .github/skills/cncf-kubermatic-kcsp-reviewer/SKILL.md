---
name: "cncf-kubermatic-kcsp-reviewer"
description: "Review Kubermatic (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Kubermatic empowers organizations to fully automate their Kubernetes operations across multi-cloud, edge, and on-prem environments, simplifying the management of thousands of clusters for leading enterprises. Leveraging our deep expertise as a top 15 contributor to the Kubernetes project, we provide professional services and enterprise-grade software to safely accelerate your cloud-native transformation like we've done for leading companies like Lufthansa, Bosch, and T-Systems."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kubermatic Kcsp Reviewer

## Purpose

Review and operate Kubermatic (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kubermatic (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kubermatic (KCSP) requires evidence-backed assessment.
...
- [ ] Trace Kubermatic (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kubermatic (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kubermatic (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kubermatic (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kubermatic (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kubermatic (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kubermatic (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kubermatic (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kubermatic (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kubermatic (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Kubermatic (KCSP) validation commands or tests that are safe for the stated environment.
...
- Kubermatic (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kubermatic (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kubermatic (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kubermatic (KCSP) as the basis for migration or production guidance.
