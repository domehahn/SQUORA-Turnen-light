---
name: "cncf-hexa-reviewer"
description: "Review Hexa using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Hexa using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Hexa code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Hexa requires evidence-backed assessment.
...
- First-party source: https://github.com/hexa-org/policy-orchestrator
...
- [ ] Trace Hexa inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Hexa defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Hexa least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Hexa resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Hexa release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Hexa dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Hexa identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Hexa change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Hexa documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Hexa architecture and dependency summary with trust boundaries and persistent state.
- Include Hexa validation commands or tests that are safe for the stated environment.
...
- Hexa identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Hexa failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Hexa is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Hexa as the basis for migration or production guidance.
