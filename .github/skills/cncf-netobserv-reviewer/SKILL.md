---
name: "cncf-netobserv-reviewer"
description: "Review NetObserv using its official documentation and repository in the Observability and Analysis / Observability category. NetObserv provides CNI-agnostic network observability of Kubernetes clusters."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Netobserv Reviewer

## Purpose

Review and operate NetObserv using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- NetObserv code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving NetObserv requires evidence-backed assessment.
...
- First-party source: https://github.com/netobserv/network-observability-operator
...
- [ ] Trace NetObserv inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check NetObserv defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check NetObserv least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check NetObserv resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check NetObserv release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check NetObserv dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If NetObserv identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a NetObserv change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- NetObserv documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a NetObserv architecture and dependency summary with trust boundaries and persistent state.
- Include NetObserv validation commands or tests that are safe for the stated environment.
...
- NetObserv identity, version, configuration, dependencies, and deployment assumptions are explicit.
- NetObserv failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that NetObserv is secure, supported, compatible, or suitable.
- Using an unpinned latest version of NetObserv as the basis for migration or production guidance.
