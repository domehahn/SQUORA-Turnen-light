---
name: "cncf-yellowbrick-reviewer"
description: "Review Yellowbrick using its official documentation and repository in the App Definition and Development / Database category. Yellowbrick offers a SQL data warehouse built from the ground up on Kubernetes."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Yellowbrick Reviewer

## Purpose

Review and operate Yellowbrick using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Yellowbrick code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Yellowbrick requires evidence-backed assessment.
...
- [ ] Trace Yellowbrick inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Yellowbrick defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Yellowbrick least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Yellowbrick resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Yellowbrick release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Yellowbrick dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Yellowbrick identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Yellowbrick change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Yellowbrick documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Yellowbrick architecture and dependency summary with trust boundaries and persistent state.
- Include Yellowbrick validation commands or tests that are safe for the stated environment.
...
- Yellowbrick identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Yellowbrick failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Yellowbrick is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Yellowbrick as the basis for migration or production guidance.
