---
name: "cncf-hazelcast-imdg-reviewer"
description: "Review Hazelcast IMDG using its official documentation and repository in the App Definition and Development / Database category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Hazelcast Imdg Reviewer

## Purpose

Review and operate Hazelcast IMDG using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Hazelcast IMDG code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Hazelcast IMDG requires evidence-backed assessment.
...
- First-party source: https://github.com/hazelcast/hazelcast
...
- [ ] Trace Hazelcast IMDG inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Hazelcast IMDG defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Hazelcast IMDG least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Hazelcast IMDG resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Hazelcast IMDG release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Hazelcast IMDG dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Hazelcast IMDG identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Hazelcast IMDG change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Hazelcast IMDG documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Hazelcast IMDG architecture and dependency summary with trust boundaries and persistent state.
- Include Hazelcast IMDG validation commands or tests that are safe for the stated environment.
...
- Hazelcast IMDG identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Hazelcast IMDG failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Hazelcast IMDG is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Hazelcast IMDG as the basis for migration or production guidance.
