---
name: "cncf-daocloud-kcsp-reviewer"
description: "Review DaoCloud (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. We provide enterprise-level cloud native application platform that supports both Kubernetes and Docker Swarm."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Daocloud Kcsp Reviewer

## Purpose

Review and operate DaoCloud (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DaoCloud (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DaoCloud (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.daocloud.io/dce
...
- [ ] Trace DaoCloud (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DaoCloud (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DaoCloud (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DaoCloud (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DaoCloud (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DaoCloud (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DaoCloud (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DaoCloud (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DaoCloud (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DaoCloud (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include DaoCloud (KCSP) validation commands or tests that are safe for the stated environment.
...
- DaoCloud (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DaoCloud (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DaoCloud (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DaoCloud (KCSP) as the basis for migration or production guidance.
