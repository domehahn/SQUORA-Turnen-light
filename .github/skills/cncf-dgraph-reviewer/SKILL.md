---
name: "cncf-dgraph-reviewer"
description: "Review Dgraph using its official documentation and repository in the App Definition and Development / Database category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Dgraph Reviewer

## Purpose

Review and operate Dgraph using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Dgraph code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Dgraph requires evidence-backed assessment.
...
- First-party source: https://github.com/dgraph-io/dgraph
...
- [ ] Trace Dgraph inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Dgraph defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Dgraph least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Dgraph resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Dgraph release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Dgraph dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Dgraph identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Dgraph change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Dgraph documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Dgraph architecture and dependency summary with trust boundaries and persistent state.
- Include Dgraph validation commands or tests that are safe for the stated environment.
...
- Dgraph identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Dgraph failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Dgraph is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Dgraph as the basis for migration or production guidance.
