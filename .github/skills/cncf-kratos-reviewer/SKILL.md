...
---
# Cncf Kratos Reviewer

## Purpose

Review and operate kratos using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- kratos code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving kratos requires evidence-backed assessment.
...
- First-party source: https://github.com/go-kratos/kratos
...
- [ ] Trace kratos inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check kratos defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check kratos least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check kratos resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check kratos release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check kratos dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If kratos identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a kratos change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- kratos documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a kratos architecture and dependency summary with trust boundaries and persistent state.
- Include kratos validation commands or tests that are safe for the stated environment.
...
- kratos identity, version, configuration, dependencies, and deployment assumptions are explicit.
- kratos failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that kratos is secure, supported, compatible, or suitable.
- Using an unpinned latest version of kratos as the basis for migration or production guidance.
