---
name: "cncf-tetragon-reviewer"
description: "Review Tetragon using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Tetragon Reviewer

## Purpose

Review and operate Tetragon using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Tetragon code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Tetragon requires evidence-backed assessment.
...
- First-party source: https://github.com/cilium/tetragon
...
- [ ] Trace Tetragon inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Tetragon defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Tetragon least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Tetragon resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Tetragon release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Tetragon dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Tetragon identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Tetragon change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Tetragon documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Tetragon architecture and dependency summary with trust boundaries and persistent state.
- Include Tetragon validation commands or tests that are safe for the stated environment.
...
- Tetragon identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Tetragon failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Tetragon is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Tetragon as the basis for migration or production guidance.
