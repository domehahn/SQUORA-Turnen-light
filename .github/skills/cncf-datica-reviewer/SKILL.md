---
name: "cncf-datica-reviewer"
description: "Review Datica using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Datica Reviewer

## Purpose

Review and operate Datica using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Datica code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Datica requires evidence-backed assessment.
...
- First-party source: https://www.linkedin.com/company/datica-inc/about/
...
- [ ] Trace Datica inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Datica defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Datica least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Datica resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Datica release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Datica dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Datica identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Datica change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Datica documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Datica architecture and dependency summary with trust boundaries and persistent state.
- Include Datica validation commands or tests that are safe for the stated environment.
...
- Datica identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Datica failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Datica is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Datica as the basis for migration or production guidance.
