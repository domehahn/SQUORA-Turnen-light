---
name: "cncf-parsec-reviewer"
description: "Review Parsec using its official documentation and repository in the Provisioning / Security & Compliance category. Platform AbstRaction for SECurity service"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Parsec Reviewer

## Purpose

Review and operate Parsec using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Parsec code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Parsec requires evidence-backed assessment.
...
- First-party source: https://github.com/parallaxsecond/parsec
...
- [ ] Trace Parsec inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Parsec defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Parsec least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Parsec resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Parsec release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Parsec dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Parsec identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Parsec change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Parsec documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Parsec architecture and dependency summary with trust boundaries and persistent state.
- Include Parsec validation commands or tests that are safe for the stated environment.
...
- Parsec identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Parsec failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Parsec is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Parsec as the basis for migration or production guidance.
