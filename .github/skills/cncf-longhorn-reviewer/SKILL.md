---
name: "cncf-longhorn-reviewer"
description: "Review Longhorn using its official documentation and repository in the Runtime / Cloud Native Storage category. Cloud-native distributed storage for Kubernetes"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Longhorn Reviewer

## Purpose

Review and operate Longhorn using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Longhorn code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Longhorn requires evidence-backed assessment.
...
- First-party source: https://github.com/longhorn/longhorn
...
- [ ] Trace Longhorn inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Longhorn defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Longhorn least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Longhorn resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Longhorn release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Longhorn dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Longhorn identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Longhorn change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Longhorn documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Longhorn architecture and dependency summary with trust boundaries and persistent state.
- Include Longhorn validation commands or tests that are safe for the stated environment.
...
- Longhorn identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Longhorn failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Longhorn is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Longhorn as the basis for migration or production guidance.
