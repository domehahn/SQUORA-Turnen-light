---
name: "cncf-sglang-reviewer"
description: "Review SGLang using its official documentation and repository in the Inference / Runtime category. SGLang powers fast, scalable inference for large language and multimodal models."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Sglang Reviewer

## Purpose

Review and operate SGLang using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SGLang code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SGLang requires evidence-backed assessment.
...
- First-party source: https://github.com/sgl-project/sglang
...
- [ ] Trace SGLang inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SGLang defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SGLang least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SGLang resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SGLang release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SGLang dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SGLang identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SGLang change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SGLang documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SGLang architecture and dependency summary with trust boundaries and persistent state.
- Include SGLang validation commands or tests that are safe for the stated environment.
...
- SGLang identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SGLang failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SGLang is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SGLang as the basis for migration or production guidance.
