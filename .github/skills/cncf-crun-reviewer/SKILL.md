---
name: "cncf-crun-reviewer"
description: "Review crun using its official documentation and repository in the Wasm / Orchestration & Management category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate crun using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- crun code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving crun requires evidence-backed assessment.
...
- First-party source: https://github.com/containers/crun
...
- [ ] Trace crun inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check crun defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check crun least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check crun resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check crun release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check crun dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If crun identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a crun change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- crun documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a crun architecture and dependency summary with trust boundaries and persistent state.
- Include crun validation commands or tests that are safe for the stated environment.
...
- crun identity, version, configuration, dependencies, and deployment assumptions are explicit.
- crun failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that crun is secure, supported, compatible, or suitable.
- Using an unpinned latest version of crun as the basis for migration or production guidance.
