---
name: "cncf-promptfoo-reviewer"
description: "Review Promptfoo using its official documentation and repository in the AI Agent / Evaluation category. Test your prompts, agents, and RAGs. Red teaming/pentesting/vulnerability scanning for AI. Compare performance of GPT, Claude, Gemini, Llama, and more. Simple declarative configs with command line and CI/CD integration. Used by OpenAI and Anthropic."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Promptfoo Reviewer

## Purpose

Review and operate Promptfoo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Promptfoo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Promptfoo requires evidence-backed assessment.
...
- First-party source: https://github.com/promptfoo/promptfoo
...
- [ ] Trace Promptfoo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Promptfoo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Promptfoo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Promptfoo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Promptfoo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Promptfoo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Promptfoo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Promptfoo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Promptfoo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Promptfoo architecture and dependency summary with trust boundaries and persistent state.
- Include Promptfoo validation commands or tests that are safe for the stated environment.
...
- Promptfoo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Promptfoo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Promptfoo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Promptfoo as the basis for migration or production guidance.
