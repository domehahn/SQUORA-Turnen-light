...
---
# Cncf Ibm Cloud Kubernetes Service Reviewer

## Purpose

Review and operate IBM Cloud Kubernetes Service using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- IBM Cloud Kubernetes Service code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving IBM Cloud Kubernetes Service requires evidence-backed assessment.
...
- First-party source: https://www.ibm.com/cloud/kubernetes-service
...
- [ ] Trace IBM Cloud Kubernetes Service inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check IBM Cloud Kubernetes Service defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check IBM Cloud Kubernetes Service least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check IBM Cloud Kubernetes Service resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check IBM Cloud Kubernetes Service release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check IBM Cloud Kubernetes Service dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If IBM Cloud Kubernetes Service identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a IBM Cloud Kubernetes Service change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- IBM Cloud Kubernetes Service documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a IBM Cloud Kubernetes Service architecture and dependency summary with trust boundaries and persistent state.
- Include IBM Cloud Kubernetes Service validation commands or tests that are safe for the stated environment.
...
- IBM Cloud Kubernetes Service identity, version, configuration, dependencies, and deployment assumptions are explicit.
- IBM Cloud Kubernetes Service failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that IBM Cloud Kubernetes Service is secure, supported, compatible, or suitable.
- Using an unpinned latest version of IBM Cloud Kubernetes Service as the basis for migration or production guidance.
