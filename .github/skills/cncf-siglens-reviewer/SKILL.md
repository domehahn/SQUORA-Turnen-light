...
---
# Cncf Siglens Reviewer

## Purpose

Review and operate SigLens using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SigLens code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SigLens requires evidence-backed assessment.
...
- First-party source: https://github.com/siglens/siglens
...
- [ ] Trace SigLens inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SigLens defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SigLens least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SigLens resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SigLens release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SigLens dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SigLens identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SigLens change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SigLens documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SigLens architecture and dependency summary with trust boundaries and persistent state.
- Include SigLens validation commands or tests that are safe for the stated environment.
...
- SigLens identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SigLens failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SigLens is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SigLens as the basis for migration or production guidance.
