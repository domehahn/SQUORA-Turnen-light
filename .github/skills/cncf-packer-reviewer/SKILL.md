---
name: "cncf-packer-reviewer"
description: "Review Packer using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Packer Reviewer

## Purpose

Review and operate Packer using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Packer code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Packer requires evidence-backed assessment.
...
- First-party source: https://github.com/hashicorp/packer
...
- [ ] Trace Packer inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Packer defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Packer least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Packer resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Packer release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Packer dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Packer identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Packer change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Packer documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Packer architecture and dependency summary with trust boundaries and persistent state.
- Include Packer validation commands or tests that are safe for the stated environment.
...
- Packer identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Packer failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Packer is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Packer as the basis for migration or production guidance.
