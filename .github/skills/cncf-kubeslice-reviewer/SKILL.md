---
name: "cncf-kubeslice-reviewer"
description: "Review KubeSlice using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. Applications provided by Avesha Systems, ready to launch on Kubernetes using Kubernetes Helm"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kubeslice Reviewer

## Purpose

Review and operate KubeSlice using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeSlice code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeSlice requires evidence-backed assessment.
...
- First-party source: https://github.com/kubeslice/kubeslice
...
- [ ] Trace KubeSlice inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeSlice defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeSlice least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeSlice resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeSlice release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeSlice dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeSlice identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeSlice change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeSlice documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeSlice architecture and dependency summary with trust boundaries and persistent state.
- Include KubeSlice validation commands or tests that are safe for the stated environment.
...
- KubeSlice identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeSlice failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeSlice is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeSlice as the basis for migration or production guidance.
