...
Review and operate Nuweba using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Nuweba code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Nuweba requires evidence-backed assessment.
...
- First-party source: https://www.nuweba.com
...
- [ ] Trace Nuweba inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Nuweba defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Nuweba least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Nuweba resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Nuweba release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Nuweba dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Nuweba identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Nuweba change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Nuweba documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Nuweba architecture and dependency summary with trust boundaries and persistent state.
- Include Nuweba validation commands or tests that are safe for the stated environment.
...
- Nuweba identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Nuweba failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Nuweba is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Nuweba as the basis for migration or production guidance.
