---
name: "cncf-kubectl-mcp-server-reviewer"
description: "Review kubectl-mcp-server using its official documentation and repository in the Provisioning / Automation & Configuration category. AI-native Kubernetes management through Model Context Protocol (MCP) with 220+ tools for natural language cluster operations."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kubectl Mcp Server Reviewer

## Purpose

Review and operate kubectl-mcp-server using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- kubectl-mcp-server code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving kubectl-mcp-server requires evidence-backed assessment.
...
- First-party source: https://github.com/rohitg00/kubectl-mcp-server
...
- [ ] Trace kubectl-mcp-server inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check kubectl-mcp-server defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check kubectl-mcp-server least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check kubectl-mcp-server resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check kubectl-mcp-server release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check kubectl-mcp-server dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If kubectl-mcp-server identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a kubectl-mcp-server change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- kubectl-mcp-server documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a kubectl-mcp-server architecture and dependency summary with trust boundaries and persistent state.
- Include kubectl-mcp-server validation commands or tests that are safe for the stated environment.
...
- kubectl-mcp-server identity, version, configuration, dependencies, and deployment assumptions are explicit.
- kubectl-mcp-server failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that kubectl-mcp-server is secure, supported, compatible, or suitable.
- Using an unpinned latest version of kubectl-mcp-server as the basis for migration or production guidance.
