---
name: "cncf-hidora-kcsp-reviewer"
description: "Review Hidora (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Hidora provides fully managed Kubernetes services, supporting cluster setup, migration, and operations. Hosted in Swiss datacenters, our solutions ensure sovereignty, reliability, and expert local support."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Hidora Kcsp Reviewer

## Purpose

Review and operate Hidora (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Hidora (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Hidora (KCSP) requires evidence-backed assessment.
...
- First-party source: https://hikube.cloud/en/cloud-support-expertise
...
- [ ] Trace Hidora (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Hidora (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Hidora (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Hidora (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Hidora (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Hidora (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Hidora (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Hidora (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Hidora (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Hidora (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Hidora (KCSP) validation commands or tests that are safe for the stated environment.
...
- Hidora (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Hidora (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Hidora (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Hidora (KCSP) as the basis for migration or production guidance.
