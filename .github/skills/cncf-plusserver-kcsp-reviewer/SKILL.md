---
name: "cncf-plusserver-kcsp-reviewer"
description: "Review plusserver (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. plusserver experts offer individualized guidance covering all phases of your Kubernetes journey, including planning, migration and problem-solving of your Kubernetes clusters."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Plusserver Kcsp Reviewer

## Purpose

Review and operate plusserver (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- plusserver (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving plusserver (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.plusserver.com/en/produkte/managed-kubernetes
...
- [ ] Trace plusserver (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check plusserver (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check plusserver (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check plusserver (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check plusserver (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check plusserver (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If plusserver (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a plusserver (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- plusserver (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a plusserver (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include plusserver (KCSP) validation commands or tests that are safe for the stated environment.
...
- plusserver (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- plusserver (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that plusserver (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of plusserver (KCSP) as the basis for migration or production guidance.
