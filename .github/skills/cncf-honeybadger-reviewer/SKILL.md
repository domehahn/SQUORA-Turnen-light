---
name: "cncf-honeybadger-reviewer"
description: "Review Honeybadger using its official documentation and repository in the Observability and Analysis / Observability category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Honeybadger Reviewer

## Purpose

Review and operate Honeybadger using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Honeybadger code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Honeybadger requires evidence-backed assessment.
...
- First-party source: https://www.honeybadger.io/
...
- [ ] Trace Honeybadger inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Honeybadger defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Honeybadger least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Honeybadger resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Honeybadger release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Honeybadger dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Honeybadger identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Honeybadger change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Honeybadger documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Honeybadger architecture and dependency summary with trust boundaries and persistent state.
- Include Honeybadger validation commands or tests that are safe for the stated environment.
...
- Honeybadger identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Honeybadger failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Honeybadger is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Honeybadger as the basis for migration or production guidance.
