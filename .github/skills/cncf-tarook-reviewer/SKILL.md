---
name: "cncf-tarook-reviewer"
description: "Review Tarook using its official documentation and repository in the Platform / Certified Kubernetes - Distribution category. Holistic life cycle management of Kubernetes clusters on bare metal or OpenStack"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Tarook Reviewer

## Purpose

Review and operate Tarook using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Tarook code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Tarook requires evidence-backed assessment.
...
- First-party source: https://github.com/alasca-association/tarook
...
- [ ] Trace Tarook inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Tarook defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Tarook least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Tarook resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Tarook release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Tarook dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Tarook identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Tarook change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Tarook documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Tarook architecture and dependency summary with trust boundaries and persistent state.
- Include Tarook validation commands or tests that are safe for the stated environment.
...
- Tarook identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Tarook failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Tarook is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Tarook as the basis for migration or production guidance.
