---
name: "cncf-kubean-reviewer"
description: "Review Kubean using its official documentation and repository in the Provisioning / Automation & Configuration category. Product ready cluster lifecycle management toolchains based on kubespray and other cluster LCM engine."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kubean Reviewer

## Purpose

Review and operate Kubean using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kubean code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kubean requires evidence-backed assessment.
...
- First-party source: https://github.com/kubean-io/kubean
...
- [ ] Trace Kubean inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kubean defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kubean least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kubean resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kubean release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kubean dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kubean identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kubean change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kubean documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kubean architecture and dependency summary with trust boundaries and persistent state.
- Include Kubean validation commands or tests that are safe for the stated environment.
...
- Kubean identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kubean failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kubean is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kubean as the basis for migration or production guidance.
