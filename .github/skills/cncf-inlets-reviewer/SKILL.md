---
name: "cncf-inlets-reviewer"
description: "Review Inlets using its official documentation and repository in the Orchestration & Management / Service Proxy category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Inlets Reviewer

## Purpose

Review and operate Inlets using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Inlets code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Inlets requires evidence-backed assessment.
...
- First-party source: https://github.com/inlets/inlets-pro
...
- [ ] Trace Inlets inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Inlets defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Inlets least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Inlets resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Inlets release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Inlets dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Inlets identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Inlets change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Inlets documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Inlets architecture and dependency summary with trust boundaries and persistent state.
- Include Inlets validation commands or tests that are safe for the stated environment.
...
- Inlets identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Inlets failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Inlets is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Inlets as the basis for migration or production guidance.
