---
name: "cncf-ibm-turbonomic-reviewer"
...
Review and operate IBM Turbonomic using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- IBM Turbonomic code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving IBM Turbonomic requires evidence-backed assessment.
...
- First-party source: https://www.ibm.com/products/turbonomic
...
- [ ] Trace IBM Turbonomic inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check IBM Turbonomic defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check IBM Turbonomic least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check IBM Turbonomic resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check IBM Turbonomic release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check IBM Turbonomic dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If IBM Turbonomic identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a IBM Turbonomic change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- IBM Turbonomic documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a IBM Turbonomic architecture and dependency summary with trust boundaries and persistent state.
- Include IBM Turbonomic validation commands or tests that are safe for the stated environment.
...
- IBM Turbonomic identity, version, configuration, dependencies, and deployment assumptions are explicit.
- IBM Turbonomic failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that IBM Turbonomic is secure, supported, compatible, or suitable.
- Using an unpinned latest version of IBM Turbonomic as the basis for migration or production guidance.
