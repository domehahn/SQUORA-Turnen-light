---
name: "cncf-alcide-reviewer"
description: "Review Alcide using its official documentation and repository in the Provisioning / Security & Compliance category. Alcide provides dev-to-production security for workloads running in Kubernetes & Istio"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Alcide Reviewer

## Purpose

Review and operate Alcide using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Alcide code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Alcide requires evidence-backed assessment.
...
- First-party source: https://github.com/alcideio
...
- [ ] Trace Alcide inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Alcide defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Alcide least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Alcide resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Alcide release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Alcide dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Alcide identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Alcide change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Alcide documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Alcide architecture and dependency summary with trust boundaries and persistent state.
- Include Alcide validation commands or tests that are safe for the stated environment.
...
- Alcide identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Alcide failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Alcide is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Alcide as the basis for migration or production guidance.
