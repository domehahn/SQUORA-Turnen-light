---
name: "cncf-topaz-reviewer"
description: "Review Topaz using its official documentation and repository in the Provisioning / Security & Compliance category. Cloud-native authorization for modern applications and APIs, combining the best of Open Policy Agent and Google Zanzibar"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Topaz using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Topaz code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Topaz requires evidence-backed assessment.
...
- First-party source: https://github.com/aserto-dev/topaz
...
- [ ] Trace Topaz inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Topaz defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Topaz least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Topaz resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Topaz release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Topaz dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Topaz identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Topaz change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Topaz documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Topaz architecture and dependency summary with trust boundaries and persistent state.
- Include Topaz validation commands or tests that are safe for the stated environment.
...
- Topaz identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Topaz failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Topaz is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Topaz as the basis for migration or production guidance.
