...
---
# Cncf Oracle Functions Reviewer

## Purpose

Review and operate Oracle Functions using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Oracle Functions code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Oracle Functions requires evidence-backed assessment.
...
- First-party source: https://www.oracle.com/cloud-native/functions
...
- [ ] Trace Oracle Functions inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Oracle Functions defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Oracle Functions least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Oracle Functions resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Oracle Functions release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Oracle Functions dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Oracle Functions identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Oracle Functions change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Oracle Functions documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Oracle Functions architecture and dependency summary with trust boundaries and persistent state.
- Include Oracle Functions validation commands or tests that are safe for the stated environment.
...
- Oracle Functions identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Oracle Functions failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Oracle Functions is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Oracle Functions as the basis for migration or production guidance.
