---
name: "cncf-winterjs-reviewer"
description: "Review WinterJS using its official documentation and repository in the Wasm / Application Frameworks category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Winterjs Reviewer

## Purpose

Review and operate WinterJS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- WinterJS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving WinterJS requires evidence-backed assessment.
...
- First-party source: https://github.com/wasmerio/winterjs
...
- [ ] Trace WinterJS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check WinterJS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check WinterJS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check WinterJS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check WinterJS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check WinterJS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If WinterJS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a WinterJS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- WinterJS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a WinterJS architecture and dependency summary with trust boundaries and persistent state.
- Include WinterJS validation commands or tests that are safe for the stated environment.
...
- WinterJS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- WinterJS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that WinterJS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of WinterJS as the basis for migration or production guidance.
