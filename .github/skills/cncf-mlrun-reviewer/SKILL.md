---
name: "cncf-mlrun-reviewer"
description: "Review MLRun using its official documentation and repository in the AI Native Infra / Continuous Integration and Delivery category. Machine Learning automation and tracking"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate MLRun using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- MLRun code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving MLRun requires evidence-backed assessment.
...
- First-party source: https://github.com/mlrun/mlrun
...
- [ ] Trace MLRun inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check MLRun defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check MLRun least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check MLRun resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check MLRun release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check MLRun dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If MLRun identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a MLRun change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- MLRun documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a MLRun architecture and dependency summary with trust boundaries and persistent state.
- Include MLRun validation commands or tests that are safe for the stated environment.
...
- MLRun identity, version, configuration, dependencies, and deployment assumptions are explicit.
- MLRun failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that MLRun is secure, supported, compatible, or suitable.
- Using an unpinned latest version of MLRun as the basis for migration or production guidance.
