---
name: "cncf-razee-reviewer"
description: "Review Razee using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Razee using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Razee code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Razee requires evidence-backed assessment.
...
- First-party source: https://github.com/razee-io/Razee
...
- [ ] Trace Razee inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Razee defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Razee least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Razee resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Razee release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Razee dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Razee identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Razee change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Razee documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Razee architecture and dependency summary with trust boundaries and persistent state.
- Include Razee validation commands or tests that are safe for the stated environment.
...
- Razee identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Razee failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Razee is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Razee as the basis for migration or production guidance.
