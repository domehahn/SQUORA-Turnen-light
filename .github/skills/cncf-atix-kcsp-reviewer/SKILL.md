---
name: "cncf-atix-kcsp-reviewer"
description: "Review ATIX (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. ATIX plans, builds & runs Kubernetes clusters. We provide operational support and teach companies how to profit from using Kubernetes."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Atix Kcsp Reviewer

## Purpose

Review and operate ATIX (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ATIX (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ATIX (KCSP) requires evidence-backed assessment.
...
- First-party source: https://atix.de/en/services/technologies/container-platforms/kubernetes/
...
- [ ] Trace ATIX (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ATIX (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ATIX (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ATIX (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ATIX (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ATIX (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ATIX (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ATIX (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ATIX (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ATIX (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include ATIX (KCSP) validation commands or tests that are safe for the stated environment.
...
- ATIX (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ATIX (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ATIX (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ATIX (KCSP) as the basis for migration or production guidance.
