---
name: "cncf-northflank-reviewer"
description: "Review Northflank using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category. Northflank is a unified developer platform for building, deploying, and managing applications on Kubernetes."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Northflank Reviewer

## Purpose

Review and operate Northflank using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Northflank code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Northflank requires evidence-backed assessment.
...
- [ ] Trace Northflank inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Northflank defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Northflank least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Northflank resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Northflank release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Northflank dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Northflank identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Northflank change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Northflank documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Northflank architecture and dependency summary with trust boundaries and persistent state.
- Include Northflank validation commands or tests that are safe for the stated environment.
...
- Northflank identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Northflank failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Northflank is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Northflank as the basis for migration or production guidance.
