---
name: "cncf-tsuru-reviewer"
description: "Review Tsuru using its official documentation and repository in the Platform / PaaS/Container Service category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Tsuru using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Tsuru code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Tsuru requires evidence-backed assessment.
...
- First-party source: https://github.com/tsuru/tsuru
...
- [ ] Trace Tsuru inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Tsuru defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Tsuru least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Tsuru resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Tsuru release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Tsuru dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Tsuru identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Tsuru change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Tsuru documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Tsuru architecture and dependency summary with trust boundaries and persistent state.
- Include Tsuru validation commands or tests that are safe for the stated environment.
...
- Tsuru identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Tsuru failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Tsuru is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Tsuru as the basis for migration or production guidance.
