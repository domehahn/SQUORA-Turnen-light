---
name: "cncf-cloudflare-workers-wasm-reviewer"
description: "Review Cloudflare Workers (Wasm) using its official documentation and repository in the Wasm / Hosted Platforms category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Cloudflare Workers Wasm Reviewer

## Purpose

Review and operate Cloudflare Workers (Wasm) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Cloudflare Workers (Wasm) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Cloudflare Workers (Wasm) requires evidence-backed assessment.
...
- First-party source: https://workers.cloudflare.com/
...
- [ ] Trace Cloudflare Workers (Wasm) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Cloudflare Workers (Wasm) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Cloudflare Workers (Wasm) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Cloudflare Workers (Wasm) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Cloudflare Workers (Wasm) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Cloudflare Workers (Wasm) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Cloudflare Workers (Wasm) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Cloudflare Workers (Wasm) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Cloudflare Workers (Wasm) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Cloudflare Workers (Wasm) architecture and dependency summary with trust boundaries and persistent state.
- Include Cloudflare Workers (Wasm) validation commands or tests that are safe for the stated environment.
...
- Cloudflare Workers (Wasm) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Cloudflare Workers (Wasm) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Cloudflare Workers (Wasm) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Cloudflare Workers (Wasm) as the basis for migration or production guidance.
