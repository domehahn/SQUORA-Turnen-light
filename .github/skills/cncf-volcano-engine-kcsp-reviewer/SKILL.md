...
---
# Cncf Volcano Engine Kcsp Reviewer

## Purpose

Review and operate Volcano Engine (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Volcano Engine (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Volcano Engine (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.volcengine.com/training/cka
...
- [ ] Trace Volcano Engine (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Volcano Engine (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Volcano Engine (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Volcano Engine (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Volcano Engine (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Volcano Engine (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Volcano Engine (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Volcano Engine (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Volcano Engine (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Volcano Engine (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Volcano Engine (KCSP) validation commands or tests that are safe for the stated environment.
...
- Volcano Engine (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Volcano Engine (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Volcano Engine (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Volcano Engine (KCSP) as the basis for migration or production guidance.
