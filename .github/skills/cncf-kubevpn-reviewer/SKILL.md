...
---
# Cncf Kubevpn Reviewer

## Purpose

Review and operate KubeVPN using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeVPN code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeVPN requires evidence-backed assessment.
...
- First-party source: https://github.com/kubenetworks/kubevpn
...
- [ ] Trace KubeVPN inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeVPN defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeVPN least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeVPN resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeVPN release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeVPN dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeVPN identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeVPN change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeVPN documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeVPN architecture and dependency summary with trust boundaries and persistent state.
- Include KubeVPN validation commands or tests that are safe for the stated environment.
...
- KubeVPN identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeVPN failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeVPN is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeVPN as the basis for migration or production guidance.
