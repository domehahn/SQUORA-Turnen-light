---
name: "cncf-ekuiper-reviewer"
description: "Review eKuiper using its official documentation and repository in the Wasm / Embedded Functions category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Ekuiper Reviewer

## Purpose

Review and operate eKuiper using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- eKuiper code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving eKuiper requires evidence-backed assessment.
...
- First-party source: https://github.com/lf-edge/ekuiper
...
- [ ] Trace eKuiper inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check eKuiper defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check eKuiper least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check eKuiper resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check eKuiper release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check eKuiper dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If eKuiper identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a eKuiper change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- eKuiper documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a eKuiper architecture and dependency summary with trust boundaries and persistent state.
- Include eKuiper validation commands or tests that are safe for the stated environment.
...
- eKuiper identity, version, configuration, dependencies, and deployment assumptions are explicit.
- eKuiper failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that eKuiper is secure, supported, compatible, or suitable.
- Using an unpinned latest version of eKuiper as the basis for migration or production guidance.
