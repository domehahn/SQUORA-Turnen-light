---
name: "cncf-microcks-reviewer"
description: "Review Microcks using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Microcks Reviewer

## Purpose

Review and operate Microcks using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Microcks code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Microcks requires evidence-backed assessment.
...
- First-party source: https://github.com/microcks/microcks
...
- [ ] Trace Microcks inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Microcks defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Microcks least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Microcks resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Microcks release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Microcks dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Microcks identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Microcks change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Microcks documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Microcks architecture and dependency summary with trust boundaries and persistent state.
- Include Microcks validation commands or tests that are safe for the stated environment.
...
- Microcks identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Microcks failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Microcks is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Microcks as the basis for migration or production guidance.
