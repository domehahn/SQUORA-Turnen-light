...
---
# Cncf Easeagent Reviewer

## Purpose

Review and operate EaseAgent using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- EaseAgent code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving EaseAgent requires evidence-backed assessment.
...
- First-party source: https://github.com/megaease/easeagent
...
- [ ] Trace EaseAgent inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check EaseAgent defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check EaseAgent least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check EaseAgent resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check EaseAgent release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check EaseAgent dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If EaseAgent identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a EaseAgent change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- EaseAgent documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a EaseAgent architecture and dependency summary with trust boundaries and persistent state.
- Include EaseAgent validation commands or tests that are safe for the stated environment.
...
- EaseAgent identity, version, configuration, dependencies, and deployment assumptions are explicit.
- EaseAgent failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that EaseAgent is secure, supported, compatible, or suitable.
- Using an unpinned latest version of EaseAgent as the basis for migration or production guidance.
