---
name: "cncf-docker-hub-reviewer"
description: "Review Docker Hub using its official documentation and repository in the Wasm / Packaging, Registries & Application Delivery category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Docker Hub Reviewer

## Purpose

Review and operate Docker Hub using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Docker Hub code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Docker Hub requires evidence-backed assessment.
...
- First-party source: https://hub.docker.com/
...
- [ ] Trace Docker Hub inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Docker Hub defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Docker Hub least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Docker Hub resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Docker Hub release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Docker Hub dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Docker Hub identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Docker Hub change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Docker Hub documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Docker Hub architecture and dependency summary with trust boundaries and persistent state.
- Include Docker Hub validation commands or tests that are safe for the stated environment.
...
- Docker Hub identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Docker Hub failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Docker Hub is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Docker Hub as the basis for migration or production guidance.
