---
name: "cncf-kata-containers-reviewer"
description: "Review Kata Containers using its official documentation and repository in the Runtime / Container Runtime category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kata Containers Reviewer

## Purpose

Review and operate Kata Containers using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kata Containers code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kata Containers requires evidence-backed assessment.
...
- First-party source: https://github.com/kata-containers/kata-containers
...
- [ ] Trace Kata Containers inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kata Containers defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kata Containers least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kata Containers resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kata Containers release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kata Containers dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kata Containers identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kata Containers change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kata Containers documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kata Containers architecture and dependency summary with trust boundaries and persistent state.
- Include Kata Containers validation commands or tests that are safe for the stated environment.
...
- Kata Containers identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kata Containers failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kata Containers is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kata Containers as the basis for migration or production guidance.
