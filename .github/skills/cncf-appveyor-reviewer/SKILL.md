---
name: "cncf-appveyor-reviewer"
description: "Review Appveyor using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Appveyor Reviewer

## Purpose

Review and operate Appveyor using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Appveyor code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Appveyor requires evidence-backed assessment.
...
- First-party source: https://www.appveyor.com/
...
- [ ] Trace Appveyor inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Appveyor defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Appveyor least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Appveyor resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Appveyor release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Appveyor dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Appveyor identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Appveyor change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Appveyor documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Appveyor architecture and dependency summary with trust boundaries and persistent state.
- Include Appveyor validation commands or tests that are safe for the stated environment.
...
- Appveyor identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Appveyor failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Appveyor is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Appveyor as the basis for migration or production guidance.
