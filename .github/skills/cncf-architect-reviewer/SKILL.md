---
name: "cncf-architect-reviewer"
description: "Review Architect using its official documentation and repository in the Serverless / Framework category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Architect Reviewer

## Purpose

Review and operate Architect using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Architect code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Architect requires evidence-backed assessment.
...
- First-party source: https://github.com/architect/functions
...
- [ ] Trace Architect inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Architect defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Architect least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Architect resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Architect release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Architect dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Architect identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Architect change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Architect documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Architect architecture and dependency summary with trust boundaries and persistent state.
- Include Architect validation commands or tests that are safe for the stated environment.
...
- Architect identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Architect failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Architect is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Architect as the basis for migration or production guidance.
