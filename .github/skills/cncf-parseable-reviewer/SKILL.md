...
---
# Cncf Parseable Reviewer

## Purpose

Review and operate Parseable using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Parseable code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Parseable requires evidence-backed assessment.
...
- First-party source: https://github.com/parseablehq/parseable
...
- [ ] Trace Parseable inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Parseable defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Parseable least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Parseable resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Parseable release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Parseable dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Parseable identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Parseable change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Parseable documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Parseable architecture and dependency summary with trust boundaries and persistent state.
- Include Parseable validation commands or tests that are safe for the stated environment.
...
- Parseable identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Parseable failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Parseable is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Parseable as the basis for migration or production guidance.
