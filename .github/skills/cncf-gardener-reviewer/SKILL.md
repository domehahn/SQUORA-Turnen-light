---
name: "cncf-gardener-reviewer"
description: "Review Gardener using its official documentation and repository in the Platform / Certified Kubernetes - Installer category. The Gardener implements automated management and operation of Kubernetes clusters as a service and aims to support that service on multiple  Cloud providers."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Gardener using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Gardener code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Gardener requires evidence-backed assessment.
...
- First-party source: https://github.com/gardener/gardener
...
- [ ] Trace Gardener inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Gardener defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Gardener least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Gardener resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Gardener release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Gardener dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Gardener identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Gardener change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Gardener documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Gardener architecture and dependency summary with trust boundaries and persistent state.
- Include Gardener validation commands or tests that are safe for the stated environment.
...
- Gardener identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Gardener failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Gardener is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Gardener as the basis for migration or production guidance.
