---
name: "cncf-storpool-reviewer"
description: "Review StorPool using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Storpool Reviewer

## Purpose

Review and operate StorPool using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- StorPool code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving StorPool requires evidence-backed assessment.
...
- First-party source: https://www.storpool.com/
...
- [ ] Trace StorPool inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check StorPool defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check StorPool least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check StorPool resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check StorPool release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check StorPool dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If StorPool identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a StorPool change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- StorPool documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a StorPool architecture and dependency summary with trust boundaries and persistent state.
- Include StorPool validation commands or tests that are safe for the stated environment.
...
- StorPool identity, version, configuration, dependencies, and deployment assumptions are explicit.
- StorPool failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that StorPool is secure, supported, compatible, or suitable.
- Using an unpinned latest version of StorPool as the basis for migration or production guidance.
