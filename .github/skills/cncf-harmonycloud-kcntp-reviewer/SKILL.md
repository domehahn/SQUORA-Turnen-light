---
name: "cncf-harmonycloud-kcntp-reviewer"
description: "Review HarmonyCloud (KCNTP) using its official documentation and repository in the Special / Kubernetes and Cloud Native Training Partner category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Harmonycloud Kcntp Reviewer

## Purpose

Review and operate HarmonyCloud (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- HarmonyCloud (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving HarmonyCloud (KCNTP) requires evidence-backed assessment.
...
- [ ] Trace HarmonyCloud (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check HarmonyCloud (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check HarmonyCloud (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check HarmonyCloud (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check HarmonyCloud (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check HarmonyCloud (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If HarmonyCloud (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a HarmonyCloud (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- HarmonyCloud (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a HarmonyCloud (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include HarmonyCloud (KCNTP) validation commands or tests that are safe for the stated environment.
...
- HarmonyCloud (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- HarmonyCloud (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that HarmonyCloud (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of HarmonyCloud (KCNTP) as the basis for migration or production guidance.
