---
name: "cncf-gremlin-reviewer"
description: "Review Gremlin using its official documentation and repository in the Observability and Analysis / Chaos Engineering category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Gremlin Reviewer

## Purpose

Review and operate Gremlin using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Gremlin code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Gremlin requires evidence-backed assessment.
...
- First-party source: https://www.gremlin.com/
...
- [ ] Trace Gremlin inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Gremlin defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Gremlin least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Gremlin resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Gremlin release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Gremlin dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Gremlin identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Gremlin change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Gremlin documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Gremlin architecture and dependency summary with trust boundaries and persistent state.
- Include Gremlin validation commands or tests that are safe for the stated environment.
...
- Gremlin identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Gremlin failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Gremlin is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Gremlin as the basis for migration or production guidance.
