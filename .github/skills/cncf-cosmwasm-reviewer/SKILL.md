---
name: "cncf-cosmwasm-reviewer"
description: "Review CosmWasm using its official documentation and repository in the Wasm / Decentralized Platforms category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Cosmwasm Reviewer

## Purpose

Review and operate CosmWasm using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CosmWasm code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CosmWasm requires evidence-backed assessment.
...
- First-party source: https://github.com/CosmWasm/cosmwasm
...
- [ ] Trace CosmWasm inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CosmWasm defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CosmWasm least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CosmWasm resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CosmWasm release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CosmWasm dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CosmWasm identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CosmWasm change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CosmWasm documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CosmWasm architecture and dependency summary with trust boundaries and persistent state.
- Include CosmWasm validation commands or tests that are safe for the stated environment.
...
- CosmWasm identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CosmWasm failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CosmWasm is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CosmWasm as the basis for migration or production guidance.
