---
name: "cncf-eraser-reviewer"
description: "Review Eraser using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. Eraser uses vulnerability data to remove non-running images from all Kubernetes nodes in a cluster."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Eraser Reviewer

## Purpose

Review and operate Eraser using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Eraser code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Eraser requires evidence-backed assessment.
...
- First-party source: https://github.com/eraser-dev/eraser
...
- [ ] Trace Eraser inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Eraser defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Eraser least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Eraser resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Eraser release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Eraser dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Eraser identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Eraser change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Eraser documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Eraser architecture and dependency summary with trust boundaries and persistent state.
- Include Eraser validation commands or tests that are safe for the stated environment.
...
- Eraser identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Eraser failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Eraser is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Eraser as the basis for migration or production guidance.
