---
name: "cncf-open-service-broker-api-reviewer"
description: "Review Open Service Broker API using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Open Service Broker Api Reviewer

## Purpose

Review and operate Open Service Broker API using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Open Service Broker API code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Open Service Broker API requires evidence-backed assessment.
...
- First-party source: https://github.com/openservicebrokerapi/servicebroker
...
- [ ] Trace Open Service Broker API inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Open Service Broker API defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Open Service Broker API least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Open Service Broker API resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Open Service Broker API release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Open Service Broker API dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Open Service Broker API identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Open Service Broker API change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Open Service Broker API documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Open Service Broker API architecture and dependency summary with trust boundaries and persistent state.
- Include Open Service Broker API validation commands or tests that are safe for the stated environment.
...
- Open Service Broker API identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Open Service Broker API failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Open Service Broker API is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Open Service Broker API as the basis for migration or production guidance.
