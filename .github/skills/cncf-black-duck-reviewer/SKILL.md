...
---
# Cncf Black Duck Reviewer

## Purpose

Review and operate Black Duck using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Black Duck code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Black Duck requires evidence-backed assessment.
...
- First-party source: https://www.synopsys.com/software-integrity/security-testing/software-composition-analysis.html
...
- [ ] Trace Black Duck inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Black Duck defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Black Duck least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Black Duck resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Black Duck release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Black Duck dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Black Duck identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Black Duck change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Black Duck documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Black Duck architecture and dependency summary with trust boundaries and persistent state.
- Include Black Duck validation commands or tests that are safe for the stated environment.
...
- Black Duck identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Black Duck failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Black Duck is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Black Duck as the basis for migration or production guidance.
