---
name: "cncf-artifact-hub-reviewer"
description: "Review Artifact Hub using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Artifact Hub Reviewer

## Purpose

Review and operate Artifact Hub using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Artifact Hub code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Artifact Hub requires evidence-backed assessment.
...
- First-party source: https://github.com/artifacthub/hub
...
- [ ] Trace Artifact Hub inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Artifact Hub defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Artifact Hub least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Artifact Hub resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Artifact Hub release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Artifact Hub dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Artifact Hub identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Artifact Hub change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Artifact Hub documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Artifact Hub architecture and dependency summary with trust boundaries and persistent state.
- Include Artifact Hub validation commands or tests that are safe for the stated environment.
...
- Artifact Hub identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Artifact Hub failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Artifact Hub is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Artifact Hub as the basis for migration or production guidance.
