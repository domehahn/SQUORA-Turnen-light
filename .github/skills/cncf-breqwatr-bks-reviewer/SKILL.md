---
name: "cncf-breqwatr-bks-reviewer"
description: "Review Breqwatr BKS using its official documentation and repository in the Platform / Certified Kubernetes - Hosted category. BKS is a hosted Kubernetes service on OpenStack, leveraging Cluster API to manage Kubernetes cluster lifecycle."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Breqwatr Bks Reviewer

## Purpose

Review and operate Breqwatr BKS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Breqwatr BKS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Breqwatr BKS requires evidence-backed assessment.
...
- First-party source: https://www.breqwatr.com/product
...
- [ ] Trace Breqwatr BKS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Breqwatr BKS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Breqwatr BKS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Breqwatr BKS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Breqwatr BKS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Breqwatr BKS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Breqwatr BKS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Breqwatr BKS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Breqwatr BKS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Breqwatr BKS architecture and dependency summary with trust boundaries and persistent state.
- Include Breqwatr BKS validation commands or tests that are safe for the stated environment.
...
- Breqwatr BKS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Breqwatr BKS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Breqwatr BKS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Breqwatr BKS as the basis for migration or production guidance.
