---
name: "cncf-swift-reviewer"
description: "Review Swift using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Swift using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Swift code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Swift requires evidence-backed assessment.
...
- First-party source: https://github.com/openstack/swift
...
- [ ] Trace Swift inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Swift defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Swift least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Swift resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Swift release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Swift dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Swift identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Swift change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Swift documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Swift architecture and dependency summary with trust boundaries and persistent state.
- Include Swift validation commands or tests that are safe for the stated environment.
...
- Swift identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Swift failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Swift is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Swift as the basis for migration or production guidance.
