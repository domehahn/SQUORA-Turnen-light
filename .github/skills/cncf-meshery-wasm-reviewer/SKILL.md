---
name: "cncf-meshery-wasm-reviewer"
description: "Review Meshery (Wasm) using its official documentation and repository in the Wasm / Orchestration & Management category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Meshery Wasm Reviewer

## Purpose

Review and operate Meshery (Wasm) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Meshery (Wasm) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Meshery (Wasm) requires evidence-backed assessment.
...
- First-party source: https://github.com/meshery/meshery
...
- [ ] Trace Meshery (Wasm) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Meshery (Wasm) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Meshery (Wasm) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Meshery (Wasm) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Meshery (Wasm) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Meshery (Wasm) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Meshery (Wasm) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Meshery (Wasm) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Meshery (Wasm) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Meshery (Wasm) architecture and dependency summary with trust boundaries and persistent state.
- Include Meshery (Wasm) validation commands or tests that are safe for the stated environment.
...
- Meshery (Wasm) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Meshery (Wasm) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Meshery (Wasm) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Meshery (Wasm) as the basis for migration or production guidance.
