...
---
# Cncf Grafana Pyroscope Reviewer

## Purpose

Review and operate Grafana Pyroscope using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Grafana Pyroscope code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Grafana Pyroscope requires evidence-backed assessment.
...
- First-party source: https://github.com/grafana/pyroscope
...
- [ ] Trace Grafana Pyroscope inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Grafana Pyroscope defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Grafana Pyroscope least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Grafana Pyroscope resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Grafana Pyroscope release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Grafana Pyroscope dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Grafana Pyroscope identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Grafana Pyroscope change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Grafana Pyroscope documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Grafana Pyroscope architecture and dependency summary with trust boundaries and persistent state.
- Include Grafana Pyroscope validation commands or tests that are safe for the stated environment.
...
- Grafana Pyroscope identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Grafana Pyroscope failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Grafana Pyroscope is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Grafana Pyroscope as the basis for migration or production guidance.
