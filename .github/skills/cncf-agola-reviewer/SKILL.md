...
Review and operate Agola using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Agola code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Agola requires evidence-backed assessment.
...
- First-party source: https://github.com/agola-io/agola
...
- [ ] Trace Agola inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Agola defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Agola least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Agola resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Agola release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Agola dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Agola identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Agola change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Agola documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Agola architecture and dependency summary with trust boundaries and persistent state.
- Include Agola validation commands or tests that are safe for the stated environment.
...
- Agola identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Agola failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Agola is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Agola as the basis for migration or production guidance.
