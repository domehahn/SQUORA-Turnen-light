---
name: "cncf-horovod-reviewer"
description: "Review Horovod using its official documentation and repository in the Training / Distributed Training category. Distributed deep learning training framework for TensorFlow, Keras, PyTorch, and Apache MXNet."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Horovod Reviewer

## Purpose

Review and operate Horovod using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Horovod code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Horovod requires evidence-backed assessment.
...
- First-party source: https://github.com/horovod/horovod
...
- [ ] Trace Horovod inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Horovod defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Horovod least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Horovod resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Horovod release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Horovod dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Horovod identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Horovod change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Horovod documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Horovod architecture and dependency summary with trust boundaries and persistent state.
- Include Horovod validation commands or tests that are safe for the stated environment.
...
- Horovod identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Horovod failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Horovod is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Horovod as the basis for migration or production guidance.
