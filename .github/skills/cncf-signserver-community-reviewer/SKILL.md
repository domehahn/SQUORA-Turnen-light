---
name: "cncf-signserver-community-reviewer"
description: "Review SignServer Community using its official documentation and repository in the Provisioning / Security & Compliance category. Sign code, containers, and attestations and create timestamps with the open-source, x509 certificate-based signing software SignServer."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Signserver Community Reviewer

## Purpose

Review and operate SignServer Community using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SignServer Community code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SignServer Community requires evidence-backed assessment.
...
- First-party source: https://github.com/Keyfactor/signserver-ce
...
- [ ] Trace SignServer Community inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SignServer Community defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SignServer Community least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SignServer Community resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SignServer Community release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SignServer Community dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SignServer Community identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SignServer Community change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SignServer Community documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SignServer Community architecture and dependency summary with trust boundaries and persistent state.
- Include SignServer Community validation commands or tests that are safe for the stated environment.
...
- SignServer Community identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SignServer Community failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SignServer Community is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SignServer Community as the basis for migration or production guidance.
