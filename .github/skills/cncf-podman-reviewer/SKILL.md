---
name: "cncf-podman-reviewer"
description: "Review Podman using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Podman Reviewer

## Purpose

Review and operate Podman using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Podman code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Podman requires evidence-backed assessment.
...
- [ ] Trace Podman inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Podman defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Podman least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Podman resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Podman release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Podman dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Podman identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Podman change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Podman documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Podman architecture and dependency summary with trust boundaries and persistent state.
- Include Podman validation commands or tests that are safe for the stated environment.
...
- Podman identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Podman failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Podman is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Podman as the basis for migration or production guidance.
