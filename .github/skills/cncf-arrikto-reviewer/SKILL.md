---
name: "cncf-arrikto-reviewer"
description: "Review Arrikto using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Arrikto Reviewer

## Purpose

Review and operate Arrikto using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Arrikto code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Arrikto requires evidence-backed assessment.
...
- First-party source: https://www.arrikto.com/
...
- [ ] Trace Arrikto inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Arrikto defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Arrikto least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Arrikto resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Arrikto release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Arrikto dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Arrikto identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Arrikto change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Arrikto documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Arrikto architecture and dependency summary with trust boundaries and persistent state.
- Include Arrikto validation commands or tests that are safe for the stated environment.
...
- Arrikto identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Arrikto failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Arrikto is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Arrikto as the basis for migration or production guidance.
