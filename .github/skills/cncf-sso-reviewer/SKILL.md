---
name: "cncf-sso-reviewer"
description: "Review sso using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate sso using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- sso code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving sso requires evidence-backed assessment.
...
- First-party source: https://github.com/buzzfeed/sso
...
- [ ] Trace sso inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check sso defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check sso least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check sso resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check sso release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check sso dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If sso identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a sso change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- sso documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a sso architecture and dependency summary with trust boundaries and persistent state.
- Include sso validation commands or tests that are safe for the stated environment.
...
- sso identity, version, configuration, dependencies, and deployment assumptions are explicit.
- sso failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that sso is secure, supported, compatible, or suitable.
- Using an unpinned latest version of sso as the basis for migration or production guidance.
