---
name: "cncf-metarget-reviewer"
description: "Review Metarget using its official documentation and repository in the Provisioning / Security & Compliance category. Metarget is a framework providing automatic constructions of vulnerable cloud native infrastructures."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Metarget Reviewer

## Purpose

Review and operate Metarget using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Metarget code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Metarget requires evidence-backed assessment.
...
- First-party source: https://github.com/Metarget/metarget
...
- [ ] Trace Metarget inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Metarget defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Metarget least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Metarget resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Metarget release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Metarget dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Metarget identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Metarget change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Metarget documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Metarget architecture and dependency summary with trust boundaries and persistent state.
- Include Metarget validation commands or tests that are safe for the stated environment.
...
- Metarget identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Metarget failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Metarget is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Metarget as the basis for migration or production guidance.
