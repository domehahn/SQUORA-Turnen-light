---
name: "cncf-drasi-reviewer"
description: "Review Drasi using its official documentation and repository in the App Definition and Development / Streaming & Messaging category. A data change processing platform to simplify change-driven systems that need to detect, evaluate, and react to data changes quickly and efficiently at scale."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Drasi using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Drasi code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Drasi requires evidence-backed assessment.
...
- First-party source: http://github.com/drasi-project/drasi-platform
...
- [ ] Trace Drasi inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Drasi defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Drasi least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Drasi resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Drasi release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Drasi dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Drasi identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Drasi change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Drasi documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Drasi architecture and dependency summary with trust boundaries and persistent state.
- Include Drasi validation commands or tests that are safe for the stated environment.
...
- Drasi identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Drasi failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Drasi is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Drasi as the basis for migration or production guidance.
