---
name: "cncf-dapr-reviewer"
description: "Review Dapr using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Dapr using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Dapr code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Dapr requires evidence-backed assessment.
...
- First-party source: https://github.com/dapr/dapr
...
- [ ] Trace Dapr inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Dapr defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Dapr least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Dapr resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Dapr release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Dapr dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Dapr identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Dapr change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Dapr documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Dapr architecture and dependency summary with trust boundaries and persistent state.
- Include Dapr validation commands or tests that are safe for the stated environment.
...
- Dapr identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Dapr failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Dapr is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Dapr as the basis for migration or production guidance.
