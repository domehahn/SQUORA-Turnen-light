---
name: "cncf-confidential-containers-reviewer"
description: "Review Confidential Containers using its official documentation and repository in the Provisioning / Security & Compliance category. Confidential Containers is an open source community working to enable cloud native  confidential computing by leveraging Trusted Execution Environments to protect  containers and data."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Confidential Containers Reviewer

## Purpose

Review and operate Confidential Containers using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Confidential Containers code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Confidential Containers requires evidence-backed assessment.
...
- First-party source: https://github.com/confidential-containers/confidential-containers
...
- [ ] Trace Confidential Containers inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Confidential Containers defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Confidential Containers least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Confidential Containers resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Confidential Containers release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Confidential Containers dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Confidential Containers identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Confidential Containers change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Confidential Containers documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Confidential Containers architecture and dependency summary with trust boundaries and persistent state.
- Include Confidential Containers validation commands or tests that are safe for the stated environment.
...
- Confidential Containers identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Confidential Containers failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Confidential Containers is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Confidential Containers as the basis for migration or production guidance.
