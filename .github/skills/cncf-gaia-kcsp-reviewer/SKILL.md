---
name: "cncf-gaia-kcsp-reviewer"
description: "Review Gaia (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Gaia, a multi-cloud solutions provider, offers Kubernetes implementation and configuration services, application container migrations, and professional IT training. We help our customers adopt DevSecOps, build CI/CD, and migrate applications to Kubernetes."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Gaia Kcsp Reviewer

## Purpose

Review and operate Gaia (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Gaia (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Gaia (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.gaia.net/tc/services/20/Kubernetes
...
- [ ] Trace Gaia (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Gaia (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Gaia (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Gaia (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Gaia (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Gaia (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Gaia (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Gaia (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Gaia (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Gaia (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Gaia (KCSP) validation commands or tests that are safe for the stated environment.
...
- Gaia (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Gaia (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Gaia (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Gaia (KCSP) as the basis for migration or production guidance.
