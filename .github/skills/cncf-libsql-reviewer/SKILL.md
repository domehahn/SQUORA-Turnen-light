---
name: "cncf-libsql-reviewer"
description: "Review libSQL using its official documentation and repository in the Wasm / Embedded Functions category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Libsql Reviewer

## Purpose

Review and operate libSQL using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- libSQL code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving libSQL requires evidence-backed assessment.
...
- First-party source: https://github.com/tursodatabase/libsql
...
- [ ] Trace libSQL inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check libSQL defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check libSQL least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check libSQL resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check libSQL release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check libSQL dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If libSQL identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a libSQL change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- libSQL documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a libSQL architecture and dependency summary with trust boundaries and persistent state.
- Include libSQL validation commands or tests that are safe for the stated environment.
...
- libSQL identity, version, configuration, dependencies, and deployment assumptions are explicit.
- libSQL failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that libSQL is secure, supported, compatible, or suitable.
- Using an unpinned latest version of libSQL as the basis for migration or production guidance.
