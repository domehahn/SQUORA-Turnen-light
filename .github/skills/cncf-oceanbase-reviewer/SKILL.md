---
name: "cncf-oceanbase-reviewer"
description: "Review OceanBase using its official documentation and repository in the App Definition and Development / Database category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Oceanbase Reviewer

## Purpose

Review and operate OceanBase using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OceanBase code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OceanBase requires evidence-backed assessment.
...
- First-party source: https://github.com/oceanbase/oceanbase
...
- [ ] Trace OceanBase inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OceanBase defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OceanBase least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OceanBase resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OceanBase release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OceanBase dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OceanBase identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OceanBase change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OceanBase documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OceanBase architecture and dependency summary with trust boundaries and persistent state.
- Include OceanBase validation commands or tests that are safe for the stated environment.
...
- OceanBase identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OceanBase failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OceanBase is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OceanBase as the basis for migration or production guidance.
