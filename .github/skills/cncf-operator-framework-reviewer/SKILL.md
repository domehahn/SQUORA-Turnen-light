---
name: "cncf-operator-framework-reviewer"
description: "Review Operator Framework using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Operator Framework Reviewer

## Purpose

Review and operate Operator Framework using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Operator Framework code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Operator Framework requires evidence-backed assessment.
...
- First-party source: https://github.com/operator-framework/operator-sdk
...
- [ ] Trace Operator Framework inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Operator Framework defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Operator Framework least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Operator Framework resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Operator Framework release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Operator Framework dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Operator Framework identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Operator Framework change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Operator Framework documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Operator Framework architecture and dependency summary with trust boundaries and persistent state.
- Include Operator Framework validation commands or tests that are safe for the stated environment.
...
- Operator Framework identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Operator Framework failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Operator Framework is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Operator Framework as the basis for migration or production guidance.
