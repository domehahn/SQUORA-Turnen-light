---
name: "cncf-gloo-reviewer"
description: "Review Gloo using its official documentation and repository in the Orchestration & Management / API Gateway category."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Gloo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Gloo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Gloo requires evidence-backed assessment.
...
- First-party source: https://github.com/solo-io/gloo
...
- [ ] Trace Gloo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Gloo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Gloo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Gloo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Gloo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Gloo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Gloo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Gloo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Gloo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Gloo architecture and dependency summary with trust boundaries and persistent state.
- Include Gloo validation commands or tests that are safe for the stated environment.
...
- Gloo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Gloo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Gloo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Gloo as the basis for migration or production guidance.
