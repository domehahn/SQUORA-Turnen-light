---
name: "cncf-vscode-wasm-extension-reviewer"
description: "Review VSCode Wasm Extension using its official documentation and repository in the Wasm / Tooling category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Vscode Wasm Extension Reviewer

## Purpose

Review and operate VSCode Wasm Extension using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- VSCode Wasm Extension code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving VSCode Wasm Extension requires evidence-backed assessment.
...
- CNCF classification: Wasm / Tooling
- CNCF project maturity field: not specified
- First-party source: https://github.com/wasmerio/vscode-wasm
...
- [ ] Trace VSCode Wasm Extension inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check VSCode Wasm Extension defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check VSCode Wasm Extension least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check VSCode Wasm Extension resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check VSCode Wasm Extension release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check VSCode Wasm Extension dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If VSCode Wasm Extension identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a VSCode Wasm Extension change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- VSCode Wasm Extension documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a VSCode Wasm Extension architecture and dependency summary with trust boundaries and persistent state.
- Include VSCode Wasm Extension validation commands or tests that are safe for the stated environment.
...
- VSCode Wasm Extension identity, version, configuration, dependencies, and deployment assumptions are explicit.
- VSCode Wasm Extension failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that VSCode Wasm Extension is secure, supported, compatible, or suitable.
- Using an unpinned latest version of VSCode Wasm Extension as the basis for migration or production guidance.
