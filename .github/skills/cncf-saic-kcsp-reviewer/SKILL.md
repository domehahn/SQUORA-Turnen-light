---
name: "cncf-saic-kcsp-reviewer"
description: "Review SAIC (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Application Modernization services rapidly migrate your apps to Kubernetes and incrementally transform them into cloud native apps."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Saic Kcsp Reviewer

## Purpose

Review and operate SAIC (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SAIC (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SAIC (KCSP) requires evidence-backed assessment.
...
- [ ] Trace SAIC (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SAIC (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SAIC (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SAIC (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SAIC (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SAIC (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SAIC (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SAIC (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SAIC (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SAIC (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include SAIC (KCSP) validation commands or tests that are safe for the stated environment.
...
- SAIC (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SAIC (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SAIC (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SAIC (KCSP) as the basis for migration or production guidance.
