...
---
# Cncf Middleware Reviewer

## Purpose

Review and operate Middleware using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Middleware code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Middleware requires evidence-backed assessment.
...
- [ ] Trace Middleware inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Middleware defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Middleware least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Middleware resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Middleware release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Middleware dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Middleware identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Middleware change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Middleware documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Middleware architecture and dependency summary with trust boundaries and persistent state.
- Include Middleware validation commands or tests that are safe for the stated environment.
...
- Middleware identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Middleware failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Middleware is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Middleware as the basis for migration or production guidance.
