---
name: "cncf-presto-reviewer"
description: "Review Presto using its official documentation and repository in the App Definition and Development / Database category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Presto Reviewer

## Purpose

Review and operate Presto using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Presto code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Presto requires evidence-backed assessment.
...
- First-party source: https://github.com/prestodb/presto
...
- [ ] Trace Presto inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Presto defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Presto least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Presto resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Presto release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Presto dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Presto identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Presto change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Presto documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Presto architecture and dependency summary with trust boundaries and persistent state.
- Include Presto validation commands or tests that are safe for the stated environment.
...
- Presto identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Presto failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Presto is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Presto as the basis for migration or production guidance.
