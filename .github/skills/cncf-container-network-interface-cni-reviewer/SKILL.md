---
name: "cncf-container-network-interface-cni-reviewer"
description: "Review Container Network Interface (CNI) using its official documentation and repository in the Runtime / Cloud Native Network category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Container Network Interface Cni Reviewer

## Purpose

Review and operate Container Network Interface (CNI) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Container Network Interface (CNI) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Container Network Interface (CNI) requires evidence-backed assessment.
...
- First-party source: https://github.com/containernetworking/cni
...
- [ ] Trace Container Network Interface (CNI) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Container Network Interface (CNI) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Container Network Interface (CNI) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Container Network Interface (CNI) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Container Network Interface (CNI) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Container Network Interface (CNI) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Container Network Interface (CNI) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Container Network Interface (CNI) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Container Network Interface (CNI) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Container Network Interface (CNI) architecture and dependency summary with trust boundaries and persistent state.
- Include Container Network Interface (CNI) validation commands or tests that are safe for the stated environment.
...
- Container Network Interface (CNI) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Container Network Interface (CNI) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Container Network Interface (CNI) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Container Network Interface (CNI) as the basis for migration or production guidance.
