---
name: "cncf-salad-reviewer"
description: "Review Salad using its official documentation and repository in the Platform / PaaS/Container Service category. Deploy containers across 10,000s of GPU powered nodes on the world's largest distributed computing platform."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Salad using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Salad code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Salad requires evidence-backed assessment.
...
- First-party source: https://www.salad.com
...
- [ ] Trace Salad inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Salad defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Salad least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Salad resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Salad release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Salad dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Salad identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Salad change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Salad documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Salad architecture and dependency summary with trust boundaries and persistent state.
- Include Salad validation commands or tests that are safe for the stated environment.
...
- Salad identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Salad failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Salad is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Salad as the basis for migration or production guidance.
