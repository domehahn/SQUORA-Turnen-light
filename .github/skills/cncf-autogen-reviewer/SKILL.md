---
name: "cncf-autogen-reviewer"
description: "Review Autogen using its official documentation and repository in the AI Agent / Agent Framework category. A programming framework for agentic AI"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Autogen Reviewer

## Purpose

Review and operate Autogen using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Autogen code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Autogen requires evidence-backed assessment.
...
- First-party source: https://github.com/microsoft/autogen
...
- [ ] Trace Autogen inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Autogen defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Autogen least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Autogen resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Autogen release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Autogen dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Autogen identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Autogen change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Autogen documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Autogen architecture and dependency summary with trust boundaries and persistent state.
- Include Autogen validation commands or tests that are safe for the stated environment.
...
- Autogen identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Autogen failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Autogen is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Autogen as the basis for migration or production guidance.
