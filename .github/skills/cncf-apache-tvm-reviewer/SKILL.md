---
name: "cncf-apache-tvm-reviewer"
description: "Review Apache TVM using its official documentation and repository in the Wasm / AI/Machine Learning category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Apache Tvm Reviewer

## Purpose

Review and operate Apache TVM using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Apache TVM code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Apache TVM requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/tvm
...
- [ ] Trace Apache TVM inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Apache TVM defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Apache TVM least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Apache TVM resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Apache TVM release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Apache TVM dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Apache TVM identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Apache TVM change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Apache TVM documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Apache TVM architecture and dependency summary with trust boundaries and persistent state.
- Include Apache TVM validation commands or tests that are safe for the stated environment.
...
- Apache TVM identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Apache TVM failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Apache TVM is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Apache TVM as the basis for migration or production guidance.
