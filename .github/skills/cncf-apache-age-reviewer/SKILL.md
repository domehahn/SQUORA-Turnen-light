---
name: "cncf-apache-age-reviewer"
description: "Review Apache AGE using its official documentation and repository in the AI Agent / Knowledge Graph category. Graph database optimized for fast analysis and real-time data processing. It is provided as an extension to PostgreSQL."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Apache Age Reviewer

## Purpose

Review and operate Apache AGE using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Apache AGE code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Apache AGE requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/age
...
- [ ] Trace Apache AGE inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Apache AGE defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Apache AGE least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Apache AGE resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Apache AGE release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Apache AGE dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Apache AGE identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Apache AGE change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Apache AGE documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Apache AGE architecture and dependency summary with trust boundaries and persistent state.
- Include Apache AGE validation commands or tests that are safe for the stated environment.
...
- Apache AGE identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Apache AGE failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Apache AGE is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Apache AGE as the basis for migration or production guidance.
