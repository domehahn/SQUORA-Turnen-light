---
name: "cncf-kaito-reviewer"
description: "Review KAITO using its official documentation and repository in the Inference / Framework category. Kubernetes AI Toolchain Operator (KAITO) simplifies LLM inference, tuning, and RAG workloads on Kubernetes."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate KAITO using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KAITO code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KAITO requires evidence-backed assessment.
...
- First-party source: https://github.com/kaito-project/kaito
...
- [ ] Trace KAITO inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KAITO defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KAITO least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KAITO resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KAITO release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KAITO dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KAITO identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KAITO change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KAITO documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KAITO architecture and dependency summary with trust boundaries and persistent state.
- Include KAITO validation commands or tests that are safe for the stated environment.
...
- KAITO identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KAITO failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KAITO is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KAITO as the basis for migration or production guidance.
