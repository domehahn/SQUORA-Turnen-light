...
description: "Review Go code for idioms, concurrency, context propagation, errors, interfaces, modules, tests, and performance."
version: "0.1.0"
since: "2026-08-27"
...
- Source code, configuration, dependencies, tests, deployment artifacts, or runtime behavior in Go engineering change.
...
- goroutines and channels, context cancellation, error wrapping, interfaces, modules, race detection
...
- [ ] Trace Go engineering inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Go engineering defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Go engineering least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Go engineering resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Go engineering release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
...
- If Go engineering identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Go engineering change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Go engineering documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Go engineering architecture and dependency summary with trust boundaries and persistent state.
- Include Go engineering validation commands or tests that are safe for the stated environment.
...
- Go engineering identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Go engineering failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Go engineering is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Go engineering as the basis for migration or production guidance.
