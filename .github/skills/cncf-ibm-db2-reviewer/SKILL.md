...
---
# Cncf Ibm Db2 Reviewer

## Purpose

Review and operate IBM Db2 using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- IBM Db2 code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving IBM Db2 requires evidence-backed assessment.
...
- First-party source: https://www.ibm.com/analytics/db2
...
- [ ] Trace IBM Db2 inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check IBM Db2 defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check IBM Db2 least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check IBM Db2 resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check IBM Db2 release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check IBM Db2 dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If IBM Db2 identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a IBM Db2 change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- IBM Db2 documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a IBM Db2 architecture and dependency summary with trust boundaries and persistent state.
- Include IBM Db2 validation commands or tests that are safe for the stated environment.
...
- IBM Db2 identity, version, configuration, dependencies, and deployment assumptions are explicit.
- IBM Db2 failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that IBM Db2 is secure, supported, compatible, or suitable.
- Using an unpinned latest version of IBM Db2 as the basis for migration or production guidance.
