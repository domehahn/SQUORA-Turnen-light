---
name: "cncf-threatmapper-reviewer"
description: "Review ThreatMapper using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Threatmapper Reviewer

## Purpose

Review and operate ThreatMapper using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ThreatMapper code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ThreatMapper requires evidence-backed assessment.
...
- First-party source: https://github.com/deepfence/ThreatMapper
...
- [ ] Trace ThreatMapper inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ThreatMapper defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ThreatMapper least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ThreatMapper resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ThreatMapper release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ThreatMapper dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ThreatMapper identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ThreatMapper change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ThreatMapper documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ThreatMapper architecture and dependency summary with trust boundaries and persistent state.
- Include ThreatMapper validation commands or tests that are safe for the stated environment.
...
- ThreatMapper identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ThreatMapper failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ThreatMapper is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ThreatMapper as the basis for migration or production guidance.
