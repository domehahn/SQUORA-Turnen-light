---
name: "cncf-stackhawk-reviewer"
description: "Review StackHawk using its official documentation and repository in the Provisioning / Security & Compliance category. StackHawk helps developers find and fix application security vulnerabilities before they hit production with CI/CD automation."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Stackhawk Reviewer

## Purpose

Review and operate StackHawk using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- StackHawk code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving StackHawk requires evidence-backed assessment.
...
- First-party source: https://www.stackhawk.com/index.html
...
- [ ] Trace StackHawk inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check StackHawk defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check StackHawk least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check StackHawk resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check StackHawk release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check StackHawk dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If StackHawk identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a StackHawk change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- StackHawk documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a StackHawk architecture and dependency summary with trust boundaries and persistent state.
- Include StackHawk validation commands or tests that are safe for the stated environment.
...
- StackHawk identity, version, configuration, dependencies, and deployment assumptions are explicit.
- StackHawk failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that StackHawk is secure, supported, compatible, or suitable.
- Using an unpinned latest version of StackHawk as the basis for migration or production guidance.
