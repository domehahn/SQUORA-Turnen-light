---
name: "cncf-kube-router-reviewer"
description: "Review Kube-router using its official documentation and repository in the Runtime / Cloud Native Network category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Kube Router Reviewer

## Purpose

Review and operate Kube-router using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kube-router code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kube-router requires evidence-backed assessment.
...
- First-party source: https://github.com/cloudnativelabs/kube-router
...
- [ ] Trace Kube-router inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kube-router defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kube-router least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kube-router resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kube-router release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kube-router dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kube-router identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kube-router change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kube-router documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kube-router architecture and dependency summary with trust boundaries and persistent state.
- Include Kube-router validation commands or tests that are safe for the stated environment.
...
- Kube-router identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kube-router failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kube-router is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kube-router as the basis for migration or production guidance.
