---
name: "cncf-composefs-reviewer"
description: "Review composefs using its official documentation and repository in the Runtime / Container Runtime category. A project that combines Linux kernel features to provide read-only mountable filesystem trees stacking on top of an underlying \"lower\" Linux filesystem, particularly useful for mounting container images."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Composefs Reviewer

## Purpose

Review and operate composefs using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- composefs code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving composefs requires evidence-backed assessment.
...
- First-party source: https://github.com/containers/composefs
...
- [ ] Trace composefs inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check composefs defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check composefs least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check composefs resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check composefs release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check composefs dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If composefs identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a composefs change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- composefs documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a composefs architecture and dependency summary with trust boundaries and persistent state.
- Include composefs validation commands or tests that are safe for the stated environment.
...
- composefs identity, version, configuration, dependencies, and deployment assumptions are explicit.
- composefs failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that composefs is secure, supported, compatible, or suitable.
- Using an unpinned latest version of composefs as the basis for migration or production guidance.
