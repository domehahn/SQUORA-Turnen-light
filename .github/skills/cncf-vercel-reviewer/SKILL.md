---
name: "cncf-vercel-reviewer"
description: "Review Vercel using its official documentation and repository in the Serverless / Hosted Platform category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Vercel Reviewer

## Purpose

Review and operate Vercel using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Vercel code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Vercel requires evidence-backed assessment.
...
- First-party source: https://vercel.com/
...
- [ ] Trace Vercel inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Vercel defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Vercel least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Vercel resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Vercel release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Vercel dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Vercel identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Vercel change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Vercel documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Vercel architecture and dependency summary with trust boundaries and persistent state.
- Include Vercel validation commands or tests that are safe for the stated environment.
...
- Vercel identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Vercel failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Vercel is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Vercel as the basis for migration or production guidance.
