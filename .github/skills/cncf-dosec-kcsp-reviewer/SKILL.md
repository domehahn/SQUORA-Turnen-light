---
name: "cncf-dosec-kcsp-reviewer"
description: "Review Dosec (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Dosec provides Kubernetes services including technical support, consulting, security, and professional training."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Dosec Kcsp Reviewer

## Purpose

Review and operate Dosec (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Dosec (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Dosec (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.dosec.cn/service.html
...
- [ ] Trace Dosec (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Dosec (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Dosec (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Dosec (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Dosec (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Dosec (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Dosec (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Dosec (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Dosec (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Dosec (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Dosec (KCSP) validation commands or tests that are safe for the stated environment.
...
- Dosec (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Dosec (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Dosec (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Dosec (KCSP) as the basis for migration or production guidance.
