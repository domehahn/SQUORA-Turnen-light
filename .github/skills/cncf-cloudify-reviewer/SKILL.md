---
name: "cncf-cloudify-reviewer"
description: "Review Cloudify using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Cloudify Reviewer

## Purpose

Review and operate Cloudify using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Cloudify code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Cloudify requires evidence-backed assessment.
...
- First-party source: https://github.com/cloudify-cosmo/cloudify-manager
...
- [ ] Trace Cloudify inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Cloudify defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Cloudify least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Cloudify resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Cloudify release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Cloudify dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Cloudify identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Cloudify change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Cloudify documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Cloudify architecture and dependency summary with trust boundaries and persistent state.
- Include Cloudify validation commands or tests that are safe for the stated environment.
...
- Cloudify identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Cloudify failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Cloudify is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Cloudify as the basis for migration or production guidance.
