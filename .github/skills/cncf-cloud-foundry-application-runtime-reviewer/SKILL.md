...
---
# Cncf Cloud Foundry Application Runtime Reviewer

## Purpose

Review and operate Cloud Foundry Application Runtime using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Cloud Foundry Application Runtime code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Cloud Foundry Application Runtime requires evidence-backed assessment.
...
- CNCF classification: Platform / PaaS/Container Service
- CNCF project maturity field: not specified
- First-party source: https://github.com/cloudfoundry/cli
...
- [ ] Trace Cloud Foundry Application Runtime inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Cloud Foundry Application Runtime defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Cloud Foundry Application Runtime least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Cloud Foundry Application Runtime resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Cloud Foundry Application Runtime release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Cloud Foundry Application Runtime dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Cloud Foundry Application Runtime identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Cloud Foundry Application Runtime change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Cloud Foundry Application Runtime documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Cloud Foundry Application Runtime architecture and dependency summary with trust boundaries and persistent state.
- Include Cloud Foundry Application Runtime validation commands or tests that are safe for the stated environment.
...
- Cloud Foundry Application Runtime identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Cloud Foundry Application Runtime failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Cloud Foundry Application Runtime is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Cloud Foundry Application Runtime as the basis for migration or production guidance.
