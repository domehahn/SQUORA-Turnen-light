---
name: "cncf-alluxio-reviewer"
description: "Review Alluxio using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Alluxio Reviewer

## Purpose

Review and operate Alluxio using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Alluxio code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Alluxio requires evidence-backed assessment.
...
- First-party source: https://github.com/alluxio/alluxio
...
- [ ] Trace Alluxio inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Alluxio defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Alluxio least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Alluxio resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Alluxio release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Alluxio dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Alluxio identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Alluxio change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Alluxio documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Alluxio architecture and dependency summary with trust boundaries and persistent state.
- Include Alluxio validation commands or tests that are safe for the stated environment.
...
- Alluxio identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Alluxio failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Alluxio is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Alluxio as the basis for migration or production guidance.
