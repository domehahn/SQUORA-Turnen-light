---
name: "cncf-text-generation-inference-tgi-reviewer"
description: "Review text-generation-inference (TGI) using its official documentation and repository in the Inference / Runtime category. Large Language Model Text Generation Inference"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Text Generation Inference Tgi Reviewer

## Purpose

Review and operate text-generation-inference (TGI) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- text-generation-inference (TGI) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving text-generation-inference (TGI) requires evidence-backed assessment.
...
- CNCF classification: Inference / Runtime
- CNCF project maturity field: not specified
- First-party source: https://github.com/huggingface/text-generation-inference
...
- [ ] Trace text-generation-inference (TGI) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check text-generation-inference (TGI) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check text-generation-inference (TGI) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check text-generation-inference (TGI) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check text-generation-inference (TGI) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check text-generation-inference (TGI) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If text-generation-inference (TGI) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a text-generation-inference (TGI) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- text-generation-inference (TGI) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a text-generation-inference (TGI) architecture and dependency summary with trust boundaries and persistent state.
- Include text-generation-inference (TGI) validation commands or tests that are safe for the stated environment.
...
- text-generation-inference (TGI) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- text-generation-inference (TGI) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that text-generation-inference (TGI) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of text-generation-inference (TGI) as the basis for migration or production guidance.
