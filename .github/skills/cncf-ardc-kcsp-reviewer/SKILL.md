---
name: "cncf-ardc-kcsp-reviewer"
description: "Review ARDC (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. ARDC's Kubernetes service offering involves engaging the Australian research community in the adoption of Kubernetes to deliver cloud native research platforms. The core pillars of the service are community engagement via the ARCOS community, consultancy and support for platform research projects; training and implementations for researchers and research software engineers."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Ardc Kcsp Reviewer

## Purpose

Review and operate ARDC (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ARDC (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ARDC (KCSP) requires evidence-backed assessment.
...
- First-party source: https://arcos.ardc.edu.au/k8s-services
...
- [ ] Trace ARDC (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ARDC (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ARDC (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ARDC (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ARDC (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ARDC (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ARDC (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ARDC (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ARDC (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ARDC (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include ARDC (KCSP) validation commands or tests that are safe for the stated environment.
...
- ARDC (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ARDC (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ARDC (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ARDC (KCSP) as the basis for migration or production guidance.
