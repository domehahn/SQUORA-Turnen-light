---
name: "cncf-pipecd-reviewer"
description: "Review PipeCD using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category. GitOps style continuous delivery platform that provides consistent deployment and operations experience for any applications"
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Pipecd Reviewer

## Purpose

Review and operate PipeCD using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- PipeCD code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving PipeCD requires evidence-backed assessment.
...
- First-party source: https://github.com/pipe-cd/pipecd
...
- [ ] Trace PipeCD inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check PipeCD defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check PipeCD least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check PipeCD resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check PipeCD release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check PipeCD dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If PipeCD identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a PipeCD change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- PipeCD documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a PipeCD architecture and dependency summary with trust boundaries and persistent state.
- Include PipeCD validation commands or tests that are safe for the stated environment.
...
- PipeCD identity, version, configuration, dependencies, and deployment assumptions are explicit.
- PipeCD failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that PipeCD is secure, supported, compatible, or suitable.
- Using an unpinned latest version of PipeCD as the basis for migration or production guidance.
