...
---
# Cncf Podman Desktop Reviewer

## Purpose

Review and operate Podman Desktop using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Podman Desktop code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Podman Desktop requires evidence-backed assessment.
...
- First-party source: https://github.com/podman-desktop/podman-desktop
...
- [ ] Trace Podman Desktop inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Podman Desktop defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Podman Desktop least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Podman Desktop resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Podman Desktop release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Podman Desktop dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Podman Desktop identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Podman Desktop change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Podman Desktop documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Podman Desktop architecture and dependency summary with trust boundaries and persistent state.
- Include Podman Desktop validation commands or tests that are safe for the stated environment.
...
- Podman Desktop identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Podman Desktop failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Podman Desktop is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Podman Desktop as the basis for migration or production guidance.
