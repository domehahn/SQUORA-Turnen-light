---
name: "cncf-istio-reviewer"
description: "Review Istio using its official documentation and repository in the Orchestration & Management / Service Mesh category. Simplify observability, traffic management, security, and policy with the Istio service mesh."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate Istio using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Istio code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Istio requires evidence-backed assessment.
...
- [ ] Trace Istio inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Istio defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Istio least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Istio resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Istio release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Istio dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Istio identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Istio change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Istio documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Istio architecture and dependency summary with trust boundaries and persistent state.
- Include Istio validation commands or tests that are safe for the stated environment.
...
- Istio identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Istio failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Istio is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Istio as the basis for migration or production guidance.
