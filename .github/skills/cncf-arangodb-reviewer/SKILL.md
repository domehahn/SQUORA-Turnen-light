---
name: "cncf-arangodb-reviewer"
description: "Review ArangoDB using its official documentation and repository in the App Definition and Development / Database category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Arangodb Reviewer

## Purpose

Review and operate ArangoDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ArangoDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ArangoDB requires evidence-backed assessment.
...
- First-party source: https://github.com/arangodb/arangodb
...
- [ ] Trace ArangoDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ArangoDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ArangoDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ArangoDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ArangoDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ArangoDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ArangoDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ArangoDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ArangoDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ArangoDB architecture and dependency summary with trust boundaries and persistent state.
- Include ArangoDB validation commands or tests that are safe for the stated environment.
...
- ArangoDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ArangoDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ArangoDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ArangoDB as the basis for migration or production guidance.
