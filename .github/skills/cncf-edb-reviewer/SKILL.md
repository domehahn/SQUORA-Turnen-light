---
name: "cncf-edb-reviewer"
description: "Review EDB using its official documentation and repository in the App Definition and Development / Database category. EDB Postgres for Kubernetes is a PostgreSQL operator based on CloudNativePG"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate EDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- EDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving EDB requires evidence-backed assessment.
...
- First-party source: https://github.com/EnterpriseDB/cloudnative-pg
...
- [ ] Trace EDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check EDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check EDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check EDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check EDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check EDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If EDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a EDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- EDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a EDB architecture and dependency summary with trust boundaries and persistent state.
- Include EDB validation commands or tests that are safe for the stated environment.
...
- EDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- EDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that EDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of EDB as the basis for migration or production guidance.
