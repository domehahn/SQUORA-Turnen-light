---
name: "cncf-k3s-reviewer"
description: "Review k3s using its official documentation and repository in the Platform / Certified Kubernetes - Distribution category. Lightweight Kubernetes"
version: "0.1.0"
since: "2026-08-27"
...
Review and operate k3s using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- k3s code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving k3s requires evidence-backed assessment.
...
- First-party source: https://github.com/k3s-io/k3s
...
- [ ] Trace k3s inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check k3s defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check k3s least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check k3s resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check k3s release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check k3s dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If k3s identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a k3s change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- k3s documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a k3s architecture and dependency summary with trust boundaries and persistent state.
- Include k3s validation commands or tests that are safe for the stated environment.
...
- k3s identity, version, configuration, dependencies, and deployment assumptions are explicit.
- k3s failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that k3s is secure, supported, compatible, or suitable.
- Using an unpinned latest version of k3s as the basis for migration or production guidance.
