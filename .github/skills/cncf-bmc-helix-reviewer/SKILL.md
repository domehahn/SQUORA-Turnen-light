---
name: "cncf-bmc-helix-reviewer"
description: "Review BMC Helix using its official documentation and repository in the Observability and Analysis / Continuous Optimization category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Bmc Helix Reviewer

## Purpose

Review and operate BMC Helix using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- BMC Helix code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving BMC Helix requires evidence-backed assessment.
...
- First-party source: https://www.bmc.com/it-solutions/bmc-helix.html
...
- [ ] Trace BMC Helix inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check BMC Helix defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check BMC Helix least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check BMC Helix resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check BMC Helix release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check BMC Helix dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If BMC Helix identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a BMC Helix change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- BMC Helix documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a BMC Helix architecture and dependency summary with trust boundaries and persistent state.
- Include BMC Helix validation commands or tests that are safe for the stated environment.
...
- BMC Helix identity, version, configuration, dependencies, and deployment assumptions are explicit.
- BMC Helix failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that BMC Helix is secure, supported, compatible, or suitable.
- Using an unpinned latest version of BMC Helix as the basis for migration or production guidance.
