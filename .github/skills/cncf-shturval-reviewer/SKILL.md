---
name: "cncf-shturval-reviewer"
description: "Review Shturval using its official documentation and repository in the Platform / Certified Kubernetes - Distribution category. The Shturval platform is engineered for complex air-gapped Enterprise environments with the highest security and automation requirements."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Shturval Reviewer

## Purpose

Review and operate Shturval using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Shturval code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Shturval requires evidence-backed assessment.
...
- First-party source: https://shturval.tech/
...
- [ ] Trace Shturval inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Shturval defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Shturval least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Shturval resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Shturval release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Shturval dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Shturval identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Shturval change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Shturval documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Shturval architecture and dependency summary with trust boundaries and persistent state.
- Include Shturval validation commands or tests that are safe for the stated environment.
...
- Shturval identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Shturval failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Shturval is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Shturval as the basis for migration or production guidance.
