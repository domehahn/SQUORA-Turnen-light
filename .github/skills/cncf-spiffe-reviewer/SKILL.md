---
name: "cncf-spiffe-reviewer"
description: "Review SPIFFE using its official documentation and repository in the Provisioning / Key Management category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Spiffe Reviewer

## Purpose

Review and operate SPIFFE using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SPIFFE code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SPIFFE requires evidence-backed assessment.
...
- First-party source: https://github.com/spiffe/spiffe
...
- [ ] Trace SPIFFE inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SPIFFE defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SPIFFE least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SPIFFE resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SPIFFE release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SPIFFE dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SPIFFE identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SPIFFE change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SPIFFE documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SPIFFE architecture and dependency summary with trust boundaries and persistent state.
- Include SPIFFE validation commands or tests that are safe for the stated environment.
...
- SPIFFE identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SPIFFE failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SPIFFE is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SPIFFE as the basis for migration or production guidance.
