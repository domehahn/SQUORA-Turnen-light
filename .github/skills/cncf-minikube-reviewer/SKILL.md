---
name: "cncf-minikube-reviewer"
description: "Review minikube using its official documentation and repository in the Platform / Certified Kubernetes - Installer category. minikube runs a local Kubernetes cluster on macOS, Linux, and Windows."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Minikube Reviewer

## Purpose

Review and operate minikube using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- minikube code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving minikube requires evidence-backed assessment.
...
- First-party source: https://github.com/kubernetes/minikube
...
- [ ] Trace minikube inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check minikube defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check minikube least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check minikube resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check minikube release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check minikube dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If minikube identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a minikube change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- minikube documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a minikube architecture and dependency summary with trust boundaries and persistent state.
- Include minikube validation commands or tests that are safe for the stated environment.
...
- minikube identity, version, configuration, dependencies, and deployment assumptions are explicit.
- minikube failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that minikube is secure, supported, compatible, or suitable.
- Using an unpinned latest version of minikube as the basis for migration or production guidance.
