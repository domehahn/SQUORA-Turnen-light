---
name: "cncf-superedge-wasm-reviewer"
description: "Review SuperEdge (Wasm) using its official documentation and repository in the Wasm / Orchestration & Management category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Superedge Wasm Reviewer

## Purpose

Review and operate SuperEdge (Wasm) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SuperEdge (Wasm) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SuperEdge (Wasm) requires evidence-backed assessment.
...
- [ ] Trace SuperEdge (Wasm) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SuperEdge (Wasm) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SuperEdge (Wasm) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SuperEdge (Wasm) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SuperEdge (Wasm) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SuperEdge (Wasm) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SuperEdge (Wasm) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SuperEdge (Wasm) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SuperEdge (Wasm) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SuperEdge (Wasm) architecture and dependency summary with trust boundaries and persistent state.
- Include SuperEdge (Wasm) validation commands or tests that are safe for the stated environment.
...
- SuperEdge (Wasm) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SuperEdge (Wasm) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SuperEdge (Wasm) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SuperEdge (Wasm) as the basis for migration or production guidance.
