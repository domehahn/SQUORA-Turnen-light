---
name: "cncf-vllm-reviewer"
description: "Review vLLM using its official documentation and repository in the Inference / Runtime category. vLLM is a fast and easy-to-use library for LLM inference and serving."
version: "0.1.0"
since: "2026-08-27"
...
Review and operate vLLM using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- vLLM code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving vLLM requires evidence-backed assessment.
...
- First-party source: https://github.com/vllm-project/vllm
...
- [ ] Trace vLLM inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check vLLM defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check vLLM least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check vLLM resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check vLLM release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check vLLM dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If vLLM identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a vLLM change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- vLLM documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a vLLM architecture and dependency summary with trust boundaries and persistent state.
- Include vLLM validation commands or tests that are safe for the stated environment.
...
- vLLM identity, version, configuration, dependencies, and deployment assumptions are explicit.
- vLLM failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that vLLM is secure, supported, compatible, or suitable.
- Using an unpinned latest version of vLLM as the basis for migration or production guidance.
