---
name: "cncf-zitadel-reviewer"
description: "Review Zitadel using its official documentation and repository in the Provisioning / Security & Compliance category. ZITADEL is the identity infrastructure, simplified for you."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Zitadel Reviewer

## Purpose

Review and operate Zitadel using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Zitadel code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Zitadel requires evidence-backed assessment.
...
- First-party source: https://github.com/zitadel/zitadel
...
- [ ] Trace Zitadel inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Zitadel defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Zitadel least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Zitadel resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Zitadel release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Zitadel dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Zitadel identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Zitadel change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Zitadel documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Zitadel architecture and dependency summary with trust boundaries and persistent state.
- Include Zitadel validation commands or tests that are safe for the stated environment.
...
- Zitadel identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Zitadel failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Zitadel is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Zitadel as the basis for migration or production guidance.
