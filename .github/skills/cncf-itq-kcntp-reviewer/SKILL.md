...
---
# Cncf Itq Kcntp Reviewer

## Purpose

Review and operate ITQ (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ITQ (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ITQ (KCNTP) requires evidence-backed assessment.
...
- First-party source: https://itq.eu/master-kubernetes-cloud-native-and-automation-with-itq
...
- [ ] Trace ITQ (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ITQ (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ITQ (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ITQ (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ITQ (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ITQ (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ITQ (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ITQ (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ITQ (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ITQ (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include ITQ (KCNTP) validation commands or tests that are safe for the stated environment.
...
- ITQ (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ITQ (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ITQ (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ITQ (KCNTP) as the basis for migration or production guidance.
