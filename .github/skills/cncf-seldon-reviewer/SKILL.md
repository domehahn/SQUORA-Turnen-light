---
name: "cncf-seldon-reviewer"
description: "Review Seldon using its official documentation and repository in the Inference / Runtime category. An MLOps framework to package, deploy, monitor and manage thousands of production machine learning models."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Seldon Reviewer

## Purpose

Review and operate Seldon using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Seldon code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Seldon requires evidence-backed assessment.
...
- First-party source: https://github.com/SeldonIO/seldon-core
...
- [ ] Trace Seldon inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Seldon defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Seldon least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Seldon resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Seldon release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Seldon dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Seldon identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Seldon change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Seldon documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Seldon architecture and dependency summary with trust boundaries and persistent state.
- Include Seldon validation commands or tests that are safe for the stated environment.
...
- Seldon identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Seldon failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Seldon is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Seldon as the basis for migration or production guidance.
