---
name: "cncf-diamanti-reviewer"
description: "Review Diamanti using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "0.1.0"
since: "2026-08-27"
...
---
# Cncf Diamanti Reviewer

## Purpose

Review and operate Diamanti using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Diamanti code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Diamanti requires evidence-backed assessment.
...
- [ ] Trace Diamanti inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Diamanti defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Diamanti least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Diamanti resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Diamanti release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Diamanti dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Diamanti identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Diamanti change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Diamanti documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Diamanti architecture and dependency summary with trust boundaries and persistent state.
- Include Diamanti validation commands or tests that are safe for the stated environment.
...
- Diamanti identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Diamanti failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Diamanti is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Diamanti as the basis for migration or production guidance.
