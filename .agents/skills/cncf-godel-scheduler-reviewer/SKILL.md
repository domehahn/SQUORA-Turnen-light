---
name: "cncf-godel-scheduler-reviewer"
description: "Review Godel-Scheduler using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. Godel-Scheduler is a unified scheduler for both online and offline tasks."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Godel Scheduler Reviewer

## Purpose

Review and operate Godel-Scheduler using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Godel-Scheduler code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Godel-Scheduler requires evidence-backed assessment.
...
- First-party source: https://github.com/kubewharf/godel-scheduler
...
- [ ] Trace Godel-Scheduler inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Godel-Scheduler defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Godel-Scheduler least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Godel-Scheduler resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Godel-Scheduler release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Godel-Scheduler dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Godel-Scheduler identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Godel-Scheduler change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Godel-Scheduler documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Godel-Scheduler architecture and dependency summary with trust boundaries and persistent state.
- Include Godel-Scheduler validation commands or tests that are safe for the stated environment.
...
- Godel-Scheduler identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Godel-Scheduler failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Godel-Scheduler is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Godel-Scheduler as the basis for migration or production guidance.
