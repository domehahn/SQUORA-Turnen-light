...
---
# Cncf Ibm Cloud Code Engine Reviewer

## Purpose

Review and operate IBM Cloud Code Engine using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- IBM Cloud Code Engine code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving IBM Cloud Code Engine requires evidence-backed assessment.
...
- First-party source: https://cloud.ibm.com/login
...
- [ ] Trace IBM Cloud Code Engine inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check IBM Cloud Code Engine defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check IBM Cloud Code Engine least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check IBM Cloud Code Engine resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check IBM Cloud Code Engine release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check IBM Cloud Code Engine dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If IBM Cloud Code Engine identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a IBM Cloud Code Engine change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- IBM Cloud Code Engine documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a IBM Cloud Code Engine architecture and dependency summary with trust boundaries and persistent state.
- Include IBM Cloud Code Engine validation commands or tests that are safe for the stated environment.
...
- IBM Cloud Code Engine identity, version, configuration, dependencies, and deployment assumptions are explicit.
- IBM Cloud Code Engine failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that IBM Cloud Code Engine is secure, supported, compatible, or suitable.
- Using an unpinned latest version of IBM Cloud Code Engine as the basis for migration or production guidance.
