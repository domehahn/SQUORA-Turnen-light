...
---
# Cncf Kubereport Reviewer

## Purpose

Review and operate KubeReport using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeReport code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeReport requires evidence-backed assessment.
...
- First-party source: https://github.com/kubesuiteorg/kubereport
...
- [ ] Trace KubeReport inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeReport defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeReport least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeReport resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeReport release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeReport dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeReport identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeReport change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeReport documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeReport architecture and dependency summary with trust boundaries and persistent state.
- Include KubeReport validation commands or tests that are safe for the stated environment.
...
- KubeReport identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeReport failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeReport is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeReport as the basis for migration or production guidance.
