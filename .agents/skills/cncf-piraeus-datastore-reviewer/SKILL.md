---
name: "cncf-piraeus-datastore-reviewer"
description: "Review Piraeus Datastore using its official documentation and repository in the Runtime / Cloud Native Storage category. The Piraeus Operator manages LINSTOR clusters in Kubernetes."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Piraeus Datastore Reviewer

## Purpose

Review and operate Piraeus Datastore using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Piraeus Datastore code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Piraeus Datastore requires evidence-backed assessment.
...
- First-party source: https://github.com/piraeusdatastore/piraeus-operator
...
- [ ] Trace Piraeus Datastore inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Piraeus Datastore defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Piraeus Datastore least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Piraeus Datastore resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Piraeus Datastore release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Piraeus Datastore dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Piraeus Datastore identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Piraeus Datastore change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Piraeus Datastore documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Piraeus Datastore architecture and dependency summary with trust boundaries and persistent state.
- Include Piraeus Datastore validation commands or tests that are safe for the stated environment.
...
- Piraeus Datastore identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Piraeus Datastore failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Piraeus Datastore is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Piraeus Datastore as the basis for migration or production guidance.
