---
name: "cncf-fairwinds-insights-reviewer"
description: "Review Fairwinds Insights using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Fairwinds Insights Reviewer

## Purpose

Review and operate Fairwinds Insights using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Fairwinds Insights code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Fairwinds Insights requires evidence-backed assessment.
...
- First-party source: https://fairwinds.com/insights
...
- [ ] Trace Fairwinds Insights inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Fairwinds Insights defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Fairwinds Insights least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Fairwinds Insights resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Fairwinds Insights release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Fairwinds Insights dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Fairwinds Insights identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Fairwinds Insights change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Fairwinds Insights documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Fairwinds Insights architecture and dependency summary with trust boundaries and persistent state.
- Include Fairwinds Insights validation commands or tests that are safe for the stated environment.
...
- Fairwinds Insights identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Fairwinds Insights failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Fairwinds Insights is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Fairwinds Insights as the basis for migration or production guidance.
