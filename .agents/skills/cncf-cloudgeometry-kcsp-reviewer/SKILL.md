...
---
# Cncf Cloudgeometry Kcsp Reviewer

## Purpose

Review and operate CloudGeometry (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CloudGeometry (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CloudGeometry (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.cloudgeometry.io/blueprints/kubernetes-ci-cd-adoption-blueprint
...
- [ ] Trace CloudGeometry (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CloudGeometry (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CloudGeometry (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CloudGeometry (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CloudGeometry (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CloudGeometry (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CloudGeometry (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CloudGeometry (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CloudGeometry (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CloudGeometry (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include CloudGeometry (KCSP) validation commands or tests that are safe for the stated environment.
...
- CloudGeometry (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CloudGeometry (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CloudGeometry (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CloudGeometry (KCSP) as the basis for migration or production guidance.
