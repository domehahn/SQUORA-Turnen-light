---
name: "cncf-check-point-reviewer"
description: "Review Check Point using its official documentation and repository in the Provisioning / Security & Compliance category. Check Point Software Technologies Ltd. is a leading provider of cyber security solutions to corporate enterprises and governments globally. Check Point protects over 100,000 organizations of all sizes."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Check Point Reviewer

## Purpose

Review and operate Check Point using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Check Point code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Check Point requires evidence-backed assessment.
...
- First-party source: https://www.checkpoint.com
...
- [ ] Trace Check Point inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Check Point defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Check Point least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Check Point resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Check Point release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Check Point dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Check Point identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Check Point change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Check Point documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Check Point architecture and dependency summary with trust boundaries and persistent state.
- Include Check Point validation commands or tests that are safe for the stated environment.
...
- Check Point identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Check Point failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Check Point is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Check Point as the basis for migration or production guidance.
