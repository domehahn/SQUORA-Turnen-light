---
name: "cncf-checkmk-reviewer"
description: "Review Checkmk using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Checkmk Reviewer

## Purpose

Review and operate Checkmk using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Checkmk code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Checkmk requires evidence-backed assessment.
...
- First-party source: https://github.com/Checkmk/checkmk
...
- [ ] Trace Checkmk inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Checkmk defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Checkmk least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Checkmk resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Checkmk release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Checkmk dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Checkmk identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Checkmk change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Checkmk documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Checkmk architecture and dependency summary with trust boundaries and persistent state.
- Include Checkmk validation commands or tests that are safe for the stated environment.
...
- Checkmk identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Checkmk failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Checkmk is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Checkmk as the basis for migration or production guidance.
