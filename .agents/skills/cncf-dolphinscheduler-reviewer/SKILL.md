---
name: "cncf-dolphinscheduler-reviewer"
description: "Review DolphinScheduler using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Dolphinscheduler Reviewer

## Purpose

Review and operate DolphinScheduler using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DolphinScheduler code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DolphinScheduler requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/dolphinscheduler
...
- [ ] Trace DolphinScheduler inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DolphinScheduler defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DolphinScheduler least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DolphinScheduler resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DolphinScheduler release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DolphinScheduler dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DolphinScheduler identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DolphinScheduler change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DolphinScheduler documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DolphinScheduler architecture and dependency summary with trust boundaries and persistent state.
- Include DolphinScheduler validation commands or tests that are safe for the stated environment.
...
- DolphinScheduler identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DolphinScheduler failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DolphinScheduler is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DolphinScheduler as the basis for migration or production guidance.
