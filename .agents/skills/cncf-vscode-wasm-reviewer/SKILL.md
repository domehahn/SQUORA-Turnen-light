---
name: "cncf-vscode-wasm-reviewer"
description: "Review vscode-wasm using its official documentation and repository in the Wasm / Application Frameworks category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Vscode Wasm Reviewer

## Purpose

Review and operate vscode-wasm using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- vscode-wasm code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving vscode-wasm requires evidence-backed assessment.
...
- First-party source: https://github.com/microsoft/vscode-wasm
...
- [ ] Trace vscode-wasm inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check vscode-wasm defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check vscode-wasm least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check vscode-wasm resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check vscode-wasm release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check vscode-wasm dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If vscode-wasm identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a vscode-wasm change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- vscode-wasm documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a vscode-wasm architecture and dependency summary with trust boundaries and persistent state.
- Include vscode-wasm validation commands or tests that are safe for the stated environment.
...
- vscode-wasm identity, version, configuration, dependencies, and deployment assumptions are explicit.
- vscode-wasm failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that vscode-wasm is secure, supported, compatible, or suitable.
- Using an unpinned latest version of vscode-wasm as the basis for migration or production guidance.
