---
name: "cncf-sysdig-kcsp-reviewer"
description: "Review Sysdig (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Sysdig offers professional services, technical account management, support and training. These services are designed to help companies get up-and-running with Sysdig and find best practices with their Kubernetes platform and cloud native applications."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Sysdig Kcsp Reviewer

## Purpose

Review and operate Sysdig (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Sysdig (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Sysdig (KCSP) requires evidence-backed assessment.
...
- First-party source: https://sysdig.com/platform-architecture
...
- [ ] Trace Sysdig (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Sysdig (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Sysdig (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Sysdig (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Sysdig (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Sysdig (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Sysdig (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Sysdig (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Sysdig (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Sysdig (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Sysdig (KCSP) validation commands or tests that are safe for the stated environment.
...
- Sysdig (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Sysdig (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Sysdig (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Sysdig (KCSP) as the basis for migration or production guidance.
