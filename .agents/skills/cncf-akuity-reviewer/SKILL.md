...
Review and operate Akuity using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Akuity code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Akuity requires evidence-backed assessment.
...
- [ ] Trace Akuity inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Akuity defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Akuity least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Akuity resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Akuity release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Akuity dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Akuity identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Akuity change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Akuity documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Akuity architecture and dependency summary with trust boundaries and persistent state.
- Include Akuity validation commands or tests that are safe for the stated environment.
...
- Akuity identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Akuity failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Akuity is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Akuity as the basis for migration or production guidance.
