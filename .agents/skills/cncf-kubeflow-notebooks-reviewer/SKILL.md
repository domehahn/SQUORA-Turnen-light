---
name: "cncf-kubeflow-notebooks-reviewer"
description: "Review Kubeflow Notebooks using its official documentation and repository in the Data / Data Science category. Machine Learning Toolkit for Kubernetes"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kubeflow Notebooks Reviewer

## Purpose

Review and operate Kubeflow Notebooks using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kubeflow Notebooks code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kubeflow Notebooks requires evidence-backed assessment.
...
- First-party source: https://github.com/kubeflow/kubeflow
...
- [ ] Trace Kubeflow Notebooks inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kubeflow Notebooks defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kubeflow Notebooks least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kubeflow Notebooks resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kubeflow Notebooks release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kubeflow Notebooks dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kubeflow Notebooks identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kubeflow Notebooks change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kubeflow Notebooks documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kubeflow Notebooks architecture and dependency summary with trust boundaries and persistent state.
- Include Kubeflow Notebooks validation commands or tests that are safe for the stated environment.
...
- Kubeflow Notebooks identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kubeflow Notebooks failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kubeflow Notebooks is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kubeflow Notebooks as the basis for migration or production guidance.
