---
name: "cncf-crunchy-postgres-operator-reviewer"
description: "Review Crunchy Postgres Operator using its official documentation and repository in the App Definition and Development / Database category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Crunchy Postgres Operator Reviewer

## Purpose

Review and operate Crunchy Postgres Operator using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Crunchy Postgres Operator code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Crunchy Postgres Operator requires evidence-backed assessment.
...
- First-party source: https://github.com/CrunchyData/postgres-operator
...
- [ ] Trace Crunchy Postgres Operator inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Crunchy Postgres Operator defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Crunchy Postgres Operator least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Crunchy Postgres Operator resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Crunchy Postgres Operator release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Crunchy Postgres Operator dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Crunchy Postgres Operator identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Crunchy Postgres Operator change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Crunchy Postgres Operator documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Crunchy Postgres Operator architecture and dependency summary with trust boundaries and persistent state.
- Include Crunchy Postgres Operator validation commands or tests that are safe for the stated environment.
...
- Crunchy Postgres Operator identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Crunchy Postgres Operator failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Crunchy Postgres Operator is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Crunchy Postgres Operator as the basis for migration or production guidance.
