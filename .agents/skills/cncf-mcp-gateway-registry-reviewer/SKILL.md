---
name: "cncf-mcp-gateway-registry-reviewer"
description: "Review MCP Gateway Registry using its official documentation and repository in the AI Native Infra / Gateway category. Enterprise-ready MCP Gateway & Registry that centralizes AI development tools with secure OAuth authentication, dynamic tool discovery, and unified access for both autonomous AI agents and AI coding assistants. Transform scattered MCP server chaos into governed, auditable tool access with Keycloak/Entra integration."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Mcp Gateway Registry Reviewer

## Purpose

Review and operate MCP Gateway Registry using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- MCP Gateway Registry code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving MCP Gateway Registry requires evidence-backed assessment.
...
- CNCF classification: AI Native Infra / Gateway
- CNCF project maturity field: not specified
- First-party source: https://github.com/agentic-community/mcp-gateway-registry
...
- [ ] Trace MCP Gateway Registry inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check MCP Gateway Registry defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check MCP Gateway Registry least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check MCP Gateway Registry resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check MCP Gateway Registry release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check MCP Gateway Registry dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If MCP Gateway Registry identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a MCP Gateway Registry change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- MCP Gateway Registry documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a MCP Gateway Registry architecture and dependency summary with trust boundaries and persistent state.
- Include MCP Gateway Registry validation commands or tests that are safe for the stated environment.
...
- MCP Gateway Registry identity, version, configuration, dependencies, and deployment assumptions are explicit.
- MCP Gateway Registry failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that MCP Gateway Registry is secure, supported, compatible, or suitable.
- Using an unpinned latest version of MCP Gateway Registry as the basis for migration or production guidance.
