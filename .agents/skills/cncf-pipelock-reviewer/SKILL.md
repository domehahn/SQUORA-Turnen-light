...
---
# Cncf Pipelock Reviewer

## Purpose

Review and operate Pipelock using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Pipelock code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Pipelock requires evidence-backed assessment.
...
- First-party source: https://github.com/luckyPipewrench/pipelock
...
- [ ] Trace Pipelock inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Pipelock defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Pipelock least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Pipelock resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Pipelock release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Pipelock dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Pipelock identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Pipelock change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Pipelock documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Pipelock architecture and dependency summary with trust boundaries and persistent state.
- Include Pipelock validation commands or tests that are safe for the stated environment.
...
- Pipelock identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Pipelock failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Pipelock is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Pipelock as the basis for migration or production guidance.
