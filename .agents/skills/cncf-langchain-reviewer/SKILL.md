---
name: "cncf-langchain-reviewer"
description: "Review LangChain using its official documentation and repository in the AI Agent / Agent Framework category. The agent engineering platform"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Langchain Reviewer

## Purpose

Review and operate LangChain using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- LangChain code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving LangChain requires evidence-backed assessment.
...
- First-party source: https://github.com/langchain-ai/langchain
...
- [ ] Trace LangChain inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check LangChain defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check LangChain least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check LangChain resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check LangChain release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check LangChain dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If LangChain identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a LangChain change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- LangChain documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a LangChain architecture and dependency summary with trust boundaries and persistent state.
- Include LangChain validation commands or tests that are safe for the stated environment.
...
- LangChain identity, version, configuration, dependencies, and deployment assumptions are explicit.
- LangChain failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that LangChain is secure, supported, compatible, or suitable.
- Using an unpinned latest version of LangChain as the basis for migration or production guidance.
