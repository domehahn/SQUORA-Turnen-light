---
name: "cncf-whitesource-reviewer"
description: "Review WhiteSource using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Whitesource Reviewer

## Purpose

Review and operate WhiteSource using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- WhiteSource code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving WhiteSource requires evidence-backed assessment.
...
- First-party source: https://www.whitesourcesoftware.com/
...
- [ ] Trace WhiteSource inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check WhiteSource defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check WhiteSource least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check WhiteSource resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check WhiteSource release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check WhiteSource dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If WhiteSource identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a WhiteSource change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- WhiteSource documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a WhiteSource architecture and dependency summary with trust boundaries and persistent state.
- Include WhiteSource validation commands or tests that are safe for the stated environment.
...
- WhiteSource identity, version, configuration, dependencies, and deployment assumptions are explicit.
- WhiteSource failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that WhiteSource is secure, supported, compatible, or suitable.
- Using an unpinned latest version of WhiteSource as the basis for migration or production guidance.
