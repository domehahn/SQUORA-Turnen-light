---
name: "cncf-apache-storm-reviewer"
description: "Review Apache Storm using its official documentation and repository in the App Definition and Development / Streaming & Messaging category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Apache Storm Reviewer

## Purpose

Review and operate Apache Storm using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Apache Storm code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Apache Storm requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/storm
...
- [ ] Trace Apache Storm inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Apache Storm defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Apache Storm least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Apache Storm resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Apache Storm release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Apache Storm dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Apache Storm identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Apache Storm change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Apache Storm documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Apache Storm architecture and dependency summary with trust boundaries and persistent state.
- Include Apache Storm validation commands or tests that are safe for the stated environment.
...
- Apache Storm identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Apache Storm failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Apache Storm is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Apache Storm as the basis for migration or production guidance.
