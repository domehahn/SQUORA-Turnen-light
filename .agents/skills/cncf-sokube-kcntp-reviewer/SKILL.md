---
name: "cncf-sokube-kcntp-reviewer"
description: "Review SoKube (KCNTP) using its official documentation and repository in the Special / Kubernetes and Cloud Native Training Partner category. SoKube is pure consulting company that help companies entering the world of Containers & Kubernetes, using a comprehensive SDLC approach from Dev to Production, and using best practices coming from Agile, CI/CD, DevSecOps, SRE, GitOps."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Sokube Kcntp Reviewer

## Purpose

Review and operate SoKube (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SoKube (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SoKube (KCNTP) requires evidence-backed assessment.
...
- First-party source: https://www.sokube.io/en/services/training
...
- [ ] Trace SoKube (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SoKube (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SoKube (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SoKube (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SoKube (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SoKube (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SoKube (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SoKube (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SoKube (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SoKube (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include SoKube (KCNTP) validation commands or tests that are safe for the stated environment.
...
- SoKube (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SoKube (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SoKube (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SoKube (KCNTP) as the basis for migration or production guidance.
