...
Review and operate Fortio using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Fortio code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Fortio requires evidence-backed assessment.
...
- First-party source: https://github.com/fortio/fortio
...
- [ ] Trace Fortio inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Fortio defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Fortio least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Fortio resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Fortio release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Fortio dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Fortio identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Fortio change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Fortio documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Fortio architecture and dependency summary with trust boundaries and persistent state.
- Include Fortio validation commands or tests that are safe for the stated environment.
...
- Fortio identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Fortio failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Fortio is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Fortio as the basis for migration or production guidance.
