...
---
# Cncf Porter Reviewer

## Purpose

Review and operate Porter using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Porter code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Porter requires evidence-backed assessment.
...
- First-party source: https://github.com/getporter/porter
...
- [ ] Trace Porter inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Porter defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Porter least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Porter resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Porter release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Porter dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Porter identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Porter change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Porter documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Porter architecture and dependency summary with trust boundaries and persistent state.
- Include Porter validation commands or tests that are safe for the stated environment.
...
- Porter identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Porter failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Porter is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Porter as the basis for migration or production guidance.
