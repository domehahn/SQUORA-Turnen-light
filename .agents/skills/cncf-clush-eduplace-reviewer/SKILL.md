...
---
# Cncf Clush Eduplace Reviewer

## Purpose

Review and operate Clush EduPlace using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Clush EduPlace code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Clush EduPlace requires evidence-backed assessment.
...
- First-party source: https://www.clush.ai/clushworkplace
...
- [ ] Trace Clush EduPlace inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Clush EduPlace defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Clush EduPlace least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Clush EduPlace resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Clush EduPlace release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Clush EduPlace dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Clush EduPlace identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Clush EduPlace change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Clush EduPlace documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Clush EduPlace architecture and dependency summary with trust boundaries and persistent state.
- Include Clush EduPlace validation commands or tests that are safe for the stated environment.
...
- Clush EduPlace identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Clush EduPlace failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Clush EduPlace is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Clush EduPlace as the basis for migration or production guidance.
