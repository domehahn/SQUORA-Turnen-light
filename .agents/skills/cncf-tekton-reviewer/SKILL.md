...
---
# Cncf Tekton Reviewer

## Purpose

Review and operate Tekton using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Tekton code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Tekton requires evidence-backed assessment.
...
- First-party source: https://github.com/tektoncd/pipeline
...
- [ ] Trace Tekton inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Tekton defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Tekton least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Tekton resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Tekton release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Tekton dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Tekton identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Tekton change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Tekton documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Tekton architecture and dependency summary with trust boundaries and persistent state.
- Include Tekton validation commands or tests that are safe for the stated environment.
...
- Tekton identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Tekton failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Tekton is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Tekton as the basis for migration or production guidance.
