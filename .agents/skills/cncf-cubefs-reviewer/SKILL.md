---
name: "cncf-cubefs-reviewer"
description: "Review CubeFS using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Cubefs Reviewer

## Purpose

Review and operate CubeFS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CubeFS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CubeFS requires evidence-backed assessment.
...
- First-party source: https://github.com/cubeFS/cubefs
...
- [ ] Trace CubeFS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CubeFS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CubeFS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CubeFS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CubeFS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CubeFS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CubeFS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CubeFS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CubeFS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CubeFS architecture and dependency summary with trust boundaries and persistent state.
- Include CubeFS validation commands or tests that are safe for the stated environment.
...
- CubeFS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CubeFS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CubeFS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CubeFS as the basis for migration or production guidance.
