---
name: "cncf-cartography-reviewer"
description: "Review Cartography using its official documentation and repository in the Provisioning / Security & Compliance category. Cartography is a Python tool that consolidates infrastructure assets and the relationships between them in an intuitive graph view."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Cartography Reviewer

## Purpose

Review and operate Cartography using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Cartography code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Cartography requires evidence-backed assessment.
...
- First-party source: https://github.com/cartography-cncf/cartography
...
- [ ] Trace Cartography inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Cartography defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Cartography least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Cartography resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Cartography release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Cartography dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Cartography identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Cartography change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Cartography documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Cartography architecture and dependency summary with trust boundaries and persistent state.
- Include Cartography validation commands or tests that are safe for the stated environment.
...
- Cartography identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Cartography failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Cartography is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Cartography as the basis for migration or production guidance.
