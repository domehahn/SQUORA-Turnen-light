---
name: "cncf-harbor-reviewer"
description: "Review Harbor using its official documentation and repository in the Provisioning / Container Registry category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Harbor Reviewer

## Purpose

Review and operate Harbor using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Harbor code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Harbor requires evidence-backed assessment.
...
- [ ] Trace Harbor inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Harbor defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Harbor least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Harbor resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Harbor release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Harbor dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Harbor identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Harbor change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Harbor documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Harbor architecture and dependency summary with trust boundaries and persistent state.
- Include Harbor validation commands or tests that are safe for the stated environment.
...
- Harbor identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Harbor failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Harbor is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Harbor as the basis for migration or production guidance.
