---
name: "cncf-controltheory-reviewer"
description: "Review ControlTheory using its official documentation and repository in the Observability and Analysis / Continuous Optimization category. ControlTheory builds practical observability tools for modern SREs - open-source tools and AI-powered platforms that distill telemetry down to essential answers."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Controltheory Reviewer

## Purpose

Review and operate ControlTheory using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ControlTheory code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ControlTheory requires evidence-backed assessment.
...
- [ ] Trace ControlTheory inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ControlTheory defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ControlTheory least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ControlTheory resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ControlTheory release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ControlTheory dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ControlTheory identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ControlTheory change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ControlTheory documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ControlTheory architecture and dependency summary with trust boundaries and persistent state.
- Include ControlTheory validation commands or tests that are safe for the stated environment.
...
- ControlTheory identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ControlTheory failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ControlTheory is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ControlTheory as the basis for migration or production guidance.
