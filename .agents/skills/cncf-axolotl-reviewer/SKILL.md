---
name: "cncf-axolotl-reviewer"
description: "Review Axolotl using its official documentation and repository in the Training / Post Training category. Go ahead and axolotl questions"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Axolotl Reviewer

## Purpose

Review and operate Axolotl using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Axolotl code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Axolotl requires evidence-backed assessment.
...
- First-party source: https://github.com/axolotl-ai-cloud/axolotl
...
- [ ] Trace Axolotl inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Axolotl defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Axolotl least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Axolotl resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Axolotl release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Axolotl dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Axolotl identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Axolotl change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Axolotl documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Axolotl architecture and dependency summary with trust boundaries and persistent state.
- Include Axolotl validation commands or tests that are safe for the stated environment.
...
- Axolotl identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Axolotl failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Axolotl is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Axolotl as the basis for migration or production guidance.
