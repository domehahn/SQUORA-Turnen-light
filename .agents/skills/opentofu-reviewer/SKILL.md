...
description: "Review OpenTofu modules, providers, plans, state, drift, lifecycle, tests, and safe apply or destroy boundaries."
version: "1.0.0"
since: "2026-08-27"
...
- Source code, configuration, dependencies, tests, deployment artifacts, or runtime behavior in OpenTofu infrastructure as code change.
...
- modules, providers, plan changes, state encryption, lifecycle, tests
...
- [ ] Trace OpenTofu infrastructure as code inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenTofu infrastructure as code defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenTofu infrastructure as code least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenTofu infrastructure as code resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenTofu infrastructure as code release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
...
- If OpenTofu infrastructure as code identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenTofu infrastructure as code change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenTofu infrastructure as code documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenTofu infrastructure as code architecture and dependency summary with trust boundaries and persistent state.
- Include OpenTofu infrastructure as code validation commands or tests that are safe for the stated environment.
...
- OpenTofu infrastructure as code identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenTofu infrastructure as code failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenTofu infrastructure as code is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenTofu infrastructure as code as the basis for migration or production guidance.
