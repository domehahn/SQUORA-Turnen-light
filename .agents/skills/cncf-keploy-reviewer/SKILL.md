---
name: "cncf-keploy-reviewer"
description: "Review Keploy using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Keploy Reviewer

## Purpose

Review and operate Keploy using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Keploy code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Keploy requires evidence-backed assessment.
...
- First-party source: https://github.com/keploy/keploy
...
- [ ] Trace Keploy inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Keploy defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Keploy least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Keploy resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Keploy release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Keploy dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Keploy identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Keploy change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Keploy documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Keploy architecture and dependency summary with trust boundaries and persistent state.
- Include Keploy validation commands or tests that are safe for the stated environment.
...
- Keploy identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Keploy failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Keploy is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Keploy as the basis for migration or production guidance.
