---
name: "cncf-kratix-reviewer"
description: "Review Kratix using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kratix Reviewer

## Purpose

Review and operate Kratix using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kratix code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kratix requires evidence-backed assessment.
...
- First-party source: https://github.com/syntasso/kratix
...
- [ ] Trace Kratix inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kratix defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kratix least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kratix resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kratix release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kratix dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kratix identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kratix change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kratix documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kratix architecture and dependency summary with trust boundaries and persistent state.
- Include Kratix validation commands or tests that are safe for the stated environment.
...
- Kratix identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kratix failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kratix is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kratix as the basis for migration or production guidance.
