---
name: "cncf-opsani-reviewer"
description: "Review Opsani using its official documentation and repository in the Observability and Analysis / Continuous Optimization category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Opsani Reviewer

## Purpose

Review and operate Opsani using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Opsani code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Opsani requires evidence-backed assessment.
...
- First-party source: https://opsani.com/
...
- [ ] Trace Opsani inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Opsani defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Opsani least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Opsani resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Opsani release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Opsani dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Opsani identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Opsani change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Opsani documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Opsani architecture and dependency summary with trust boundaries and persistent state.
- Include Opsani validation commands or tests that are safe for the stated environment.
...
- Opsani identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Opsani failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Opsani is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Opsani as the basis for migration or production guidance.
