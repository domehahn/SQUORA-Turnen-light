---
name: "cncf-agent2agent-reviewer"
description: "Review Agent2agent using its official documentation and repository in the AI Agent / Protocol category. Agent2Agent (A2A) is an open protocol enabling communication and interoperability between opaque agentic applications."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Agent2agent Reviewer

## Purpose

Review and operate Agent2agent using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Agent2agent code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Agent2agent requires evidence-backed assessment.
...
- First-party source: https://github.com/a2aproject/A2A
...
- [ ] Trace Agent2agent inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Agent2agent defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Agent2agent least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Agent2agent resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Agent2agent release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Agent2agent dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Agent2agent identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Agent2agent change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Agent2agent documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Agent2agent architecture and dependency summary with trust boundaries and persistent state.
- Include Agent2agent validation commands or tests that are safe for the stated environment.
...
- Agent2agent identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Agent2agent failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Agent2agent is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Agent2agent as the basis for migration or production guidance.
