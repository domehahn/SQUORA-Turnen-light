---
name: "cncf-open-mpi-reviewer"
description: "Review Open MPI using its official documentation and repository in the Training / Distributed Training category. A High Performance Message Passing Library"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Open Mpi Reviewer

## Purpose

Review and operate Open MPI using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Open MPI code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Open MPI requires evidence-backed assessment.
...
- First-party source: https://www-lb.open-mpi.org/
...
- [ ] Trace Open MPI inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Open MPI defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Open MPI least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Open MPI resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Open MPI release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Open MPI dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Open MPI identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Open MPI change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Open MPI documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Open MPI architecture and dependency summary with trust boundaries and persistent state.
- Include Open MPI validation commands or tests that are safe for the stated environment.
...
- Open MPI identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Open MPI failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Open MPI is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Open MPI as the basis for migration or production guidance.
