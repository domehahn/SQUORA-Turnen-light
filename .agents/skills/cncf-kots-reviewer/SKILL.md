...
Review and operate KOTS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KOTS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KOTS requires evidence-backed assessment.
...
- First-party source: https://github.com/replicatedhq/kots
...
- [ ] Trace KOTS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KOTS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KOTS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KOTS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KOTS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KOTS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KOTS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KOTS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KOTS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KOTS architecture and dependency summary with trust boundaries and persistent state.
- Include KOTS validation commands or tests that are safe for the stated environment.
...
- KOTS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KOTS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KOTS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KOTS as the basis for migration or production guidance.
