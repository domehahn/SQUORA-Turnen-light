---
name: "ruby-reviewer"
description: "Review Ruby code for idioms, object design, metaprogramming boundaries, Bundler hygiene, tests, and performance."
version: "1.0.0"
since: "2026-08-27"
...
- Source code, configuration, dependencies, tests, deployment artifacts, or runtime behavior in Ruby engineering change.
...
- object design, blocks and enumerables, metaprogramming, Bundler, RSpec, runtime performance
...
- [ ] Trace Ruby engineering inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Ruby engineering defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Ruby engineering least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Ruby engineering resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Ruby engineering release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
...
- If Ruby engineering identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Ruby engineering change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Ruby engineering documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Ruby engineering architecture and dependency summary with trust boundaries and persistent state.
- Include Ruby engineering validation commands or tests that are safe for the stated environment.
...
- Ruby engineering identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Ruby engineering failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Ruby engineering is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Ruby engineering as the basis for migration or production guidance.
