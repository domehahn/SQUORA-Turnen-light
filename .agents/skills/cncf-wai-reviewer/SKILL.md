---
name: "cncf-wai-reviewer"
description: "Review WAI using its official documentation and repository in the Wasm / Tooling category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate WAI using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- WAI code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving WAI requires evidence-backed assessment.
...
- First-party source: https://github.com/wasmerio/wai
...
- [ ] Trace WAI inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check WAI defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check WAI least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check WAI resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check WAI release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check WAI dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If WAI identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a WAI change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- WAI documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a WAI architecture and dependency summary with trust boundaries and persistent state.
- Include WAI validation commands or tests that are safe for the stated environment.
...
- WAI identity, version, configuration, dependencies, and deployment assumptions are explicit.
- WAI failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that WAI is secure, supported, compatible, or suitable.
- Using an unpinned latest version of WAI as the basis for migration or production guidance.
