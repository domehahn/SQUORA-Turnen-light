...
---
# Cncf Orkestrix Mk8s Qks Reviewer

## Purpose

Review and operate ORKESTRIX mK8s (QKS) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ORKESTRIX mK8s (QKS) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ORKESTRIX mK8s (QKS) requires evidence-backed assessment.
...
- [ ] Trace ORKESTRIX mK8s (QKS) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ORKESTRIX mK8s (QKS) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ORKESTRIX mK8s (QKS) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ORKESTRIX mK8s (QKS) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ORKESTRIX mK8s (QKS) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ORKESTRIX mK8s (QKS) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ORKESTRIX mK8s (QKS) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ORKESTRIX mK8s (QKS) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ORKESTRIX mK8s (QKS) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ORKESTRIX mK8s (QKS) architecture and dependency summary with trust boundaries and persistent state.
- Include ORKESTRIX mK8s (QKS) validation commands or tests that are safe for the stated environment.
...
- ORKESTRIX mK8s (QKS) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ORKESTRIX mK8s (QKS) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ORKESTRIX mK8s (QKS) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ORKESTRIX mK8s (QKS) as the basis for migration or production guidance.
