---
name: "cncf-scaleops-reviewer"
description: "Review ScaleOps using its official documentation and repository in the Observability and Analysis / Continuous Optimization category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Scaleops Reviewer

## Purpose

Review and operate ScaleOps using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ScaleOps code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ScaleOps requires evidence-backed assessment.
...
- [ ] Trace ScaleOps inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ScaleOps defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ScaleOps least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ScaleOps resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ScaleOps release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ScaleOps dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ScaleOps identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ScaleOps change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ScaleOps documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ScaleOps architecture and dependency summary with trust boundaries and persistent state.
- Include ScaleOps validation commands or tests that are safe for the stated environment.
...
- ScaleOps identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ScaleOps failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ScaleOps is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ScaleOps as the basis for migration or production guidance.
