---
name: "cncf-yrcloudfile-reviewer"
description: "Review YRCloudFile using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Yrcloudfile Reviewer

## Purpose

Review and operate YRCloudFile using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- YRCloudFile code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving YRCloudFile requires evidence-backed assessment.
...
- First-party source: https://www.yanrongyun.com/en-us/products/yrfs
...
- [ ] Trace YRCloudFile inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check YRCloudFile defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check YRCloudFile least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check YRCloudFile resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check YRCloudFile release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check YRCloudFile dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If YRCloudFile identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a YRCloudFile change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- YRCloudFile documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a YRCloudFile architecture and dependency summary with trust boundaries and persistent state.
- Include YRCloudFile validation commands or tests that are safe for the stated environment.
...
- YRCloudFile identity, version, configuration, dependencies, and deployment assumptions are explicit.
- YRCloudFile failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that YRCloudFile is secure, supported, compatible, or suitable.
- Using an unpinned latest version of YRCloudFile as the basis for migration or production guidance.
