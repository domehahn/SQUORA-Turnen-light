---
name: "cncf-cdevents-reviewer"
description: "Review CDEvents using its official documentation and repository in the App Definition and Development / Streaming & Messaging category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Cdevents Reviewer

## Purpose

Review and operate CDEvents using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CDEvents code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CDEvents requires evidence-backed assessment.
...
- First-party source: https://github.com/cdevents/spec
...
- [ ] Trace CDEvents inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CDEvents defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CDEvents least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CDEvents resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CDEvents release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CDEvents dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CDEvents identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CDEvents change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CDEvents documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CDEvents architecture and dependency summary with trust boundaries and persistent state.
- Include CDEvents validation commands or tests that are safe for the stated environment.
...
- CDEvents identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CDEvents failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CDEvents is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CDEvents as the basis for migration or production guidance.
