---
name: "cncf-netapp-reviewer"
description: "Review NetApp using its official documentation and repository in the Runtime / Cloud Native Storage category. Build your Data Fabric on the industry’s broadest portfolio of all-flash, hybrid-flash, and object storage systems."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Netapp Reviewer

## Purpose

Review and operate NetApp using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- NetApp code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving NetApp requires evidence-backed assessment.
...
- First-party source: https://www.netapp.com/data-storage
...
- [ ] Trace NetApp inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check NetApp defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check NetApp least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check NetApp resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check NetApp release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check NetApp dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If NetApp identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a NetApp change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- NetApp documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a NetApp architecture and dependency summary with trust boundaries and persistent state.
- Include NetApp validation commands or tests that are safe for the stated environment.
...
- NetApp identity, version, configuration, dependencies, and deployment assumptions are explicit.
- NetApp failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that NetApp is secure, supported, compatible, or suitable.
- Using an unpinned latest version of NetApp as the basis for migration or production guidance.
