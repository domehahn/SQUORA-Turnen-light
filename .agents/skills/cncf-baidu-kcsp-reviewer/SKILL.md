---
name: "cncf-baidu-kcsp-reviewer"
description: "Review Baidu (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Baidu Cloud Container Engine is an enterprise-level container platform, the core of CCE is leveraging on open source technologies including Kubernetes and Docker."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Baidu Kcsp Reviewer

## Purpose

Review and operate Baidu (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Baidu (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Baidu (KCSP) requires evidence-backed assessment.
...
- [ ] Trace Baidu (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Baidu (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Baidu (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Baidu (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Baidu (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Baidu (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Baidu (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Baidu (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Baidu (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Baidu (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Baidu (KCSP) validation commands or tests that are safe for the stated environment.
...
- Baidu (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Baidu (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Baidu (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Baidu (KCSP) as the basis for migration or production guidance.
