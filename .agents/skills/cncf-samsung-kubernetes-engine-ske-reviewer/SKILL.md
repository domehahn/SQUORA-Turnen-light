...
---
# Cncf Samsung Kubernetes Engine Ske Reviewer

## Purpose

Review and operate Samsung Kubernetes Engine (SKE) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Samsung Kubernetes Engine (SKE) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Samsung Kubernetes Engine (SKE) requires evidence-backed assessment.
...
- First-party source: https://cloud.samsungsds.com/serviceportal/product/container/k8sEngine.html
...
- [ ] Trace Samsung Kubernetes Engine (SKE) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Samsung Kubernetes Engine (SKE) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Samsung Kubernetes Engine (SKE) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Samsung Kubernetes Engine (SKE) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Samsung Kubernetes Engine (SKE) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Samsung Kubernetes Engine (SKE) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Samsung Kubernetes Engine (SKE) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Samsung Kubernetes Engine (SKE) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Samsung Kubernetes Engine (SKE) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Samsung Kubernetes Engine (SKE) architecture and dependency summary with trust boundaries and persistent state.
- Include Samsung Kubernetes Engine (SKE) validation commands or tests that are safe for the stated environment.
...
- Samsung Kubernetes Engine (SKE) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Samsung Kubernetes Engine (SKE) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Samsung Kubernetes Engine (SKE) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Samsung Kubernetes Engine (SKE) as the basis for migration or production guidance.
