---
name: "cncf-andes-digital-kcsp-reviewer"
description: "Review Andes Digital (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. We empower teams to succeed with Kubernetes through consulting, implementation, and 24/7 support. Our solutions accelerate cloud-native adoption, improve developer productivity, and reduce operational risk."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Andes Digital Kcsp Reviewer

## Purpose

Review and operate Andes Digital (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Andes Digital (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Andes Digital (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.andesdigital.com/en/services/managed-services/kubernetes-management-and-maintenance/
...
- [ ] Trace Andes Digital (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Andes Digital (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Andes Digital (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Andes Digital (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Andes Digital (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Andes Digital (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Andes Digital (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Andes Digital (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Andes Digital (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Andes Digital (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Andes Digital (KCSP) validation commands or tests that are safe for the stated environment.
...
- Andes Digital (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Andes Digital (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Andes Digital (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Andes Digital (KCSP) as the basis for migration or production guidance.
