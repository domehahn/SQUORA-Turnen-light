---
name: "cncf-k0s-reviewer"
description: "Review k0s using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate k0s using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- k0s code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving k0s requires evidence-backed assessment.
...
- First-party source: https://github.com/k0sproject/k0s
...
- [ ] Trace k0s inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check k0s defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check k0s least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check k0s resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check k0s release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check k0s dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If k0s identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a k0s change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- k0s documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a k0s architecture and dependency summary with trust boundaries and persistent state.
- Include k0s validation commands or tests that are safe for the stated environment.
...
- k0s identity, version, configuration, dependencies, and deployment assumptions are explicit.
- k0s failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that k0s is secure, supported, compatible, or suitable.
- Using an unpinned latest version of k0s as the basis for migration or production guidance.
