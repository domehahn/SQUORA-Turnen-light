---
name: "cncf-6wind-virtual-service-router-vsr-reviewer"
description: "Review 6WIND Virtual Service Router (VSR) using its official documentation and repository in the Special / Certified CNFs category. 6WIND VSR is a Virtual Service Router that leverages 6WIND's core technology to deliver network services solutions into virtual and cloud environments quickly, efficiently, and cost-effectively. 6WIND VSR enables CSPs, Enterprises, and Cloud Providers to benefit from our leading high-performance network services for building efficient, flexible, and reliable networks."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf 6wind Virtual Service Router Vsr Reviewer

## Purpose

Review and operate 6WIND Virtual Service Router (VSR) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- 6WIND Virtual Service Router (VSR) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving 6WIND Virtual Service Router (VSR) requires evidence-backed assessment.
...
- [ ] Trace 6WIND Virtual Service Router (VSR) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check 6WIND Virtual Service Router (VSR) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check 6WIND Virtual Service Router (VSR) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check 6WIND Virtual Service Router (VSR) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check 6WIND Virtual Service Router (VSR) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check 6WIND Virtual Service Router (VSR) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If 6WIND Virtual Service Router (VSR) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a 6WIND Virtual Service Router (VSR) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- 6WIND Virtual Service Router (VSR) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a 6WIND Virtual Service Router (VSR) architecture and dependency summary with trust boundaries and persistent state.
- Include 6WIND Virtual Service Router (VSR) validation commands or tests that are safe for the stated environment.
...
- 6WIND Virtual Service Router (VSR) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- 6WIND Virtual Service Router (VSR) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that 6WIND Virtual Service Router (VSR) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of 6WIND Virtual Service Router (VSR) as the basis for migration or production guidance.
