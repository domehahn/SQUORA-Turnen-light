---
name: "cncf-meshery-reviewer"
description: "Review Meshery using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Meshery Reviewer

## Purpose

Review and operate Meshery using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Meshery code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Meshery requires evidence-backed assessment.
...
- [ ] Trace Meshery inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Meshery defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Meshery least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Meshery resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Meshery release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Meshery dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Meshery identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Meshery change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Meshery documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Meshery architecture and dependency summary with trust boundaries and persistent state.
- Include Meshery validation commands or tests that are safe for the stated environment.
...
- Meshery identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Meshery failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Meshery is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Meshery as the basis for migration or production guidance.
