---
name: "cncf-quickwit-reviewer"
description: "Review Quickwit using its official documentation and repository in the Observability and Analysis / Observability category. Sub-second search & analytics engine on cloud storage"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Quickwit Reviewer

## Purpose

Review and operate Quickwit using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Quickwit code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Quickwit requires evidence-backed assessment.
...
- First-party source: https://github.com/quickwit-oss/quickwit
...
- [ ] Trace Quickwit inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Quickwit defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Quickwit least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Quickwit resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Quickwit release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Quickwit dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Quickwit identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Quickwit change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Quickwit documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Quickwit architecture and dependency summary with trust boundaries and persistent state.
- Include Quickwit validation commands or tests that are safe for the stated environment.
...
- Quickwit identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Quickwit failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Quickwit is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Quickwit as the basis for migration or production guidance.
