---
name: "cncf-split-reviewer"
description: "Review Split using its official documentation and repository in the Observability and Analysis / Feature Flagging category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Split using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Split code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Split requires evidence-backed assessment.
...
- First-party source: https://split.io
...
- [ ] Trace Split inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Split defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Split least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Split resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Split release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Split dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Split identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Split change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Split documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Split architecture and dependency summary with trust boundaries and persistent state.
- Include Split validation commands or tests that are safe for the stated environment.
...
- Split identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Split failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Split is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Split as the basis for migration or production guidance.
