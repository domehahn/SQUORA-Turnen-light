---
name: "cncf-spiderlightning-reviewer"
description: "Review SpiderLightning using its official documentation and repository in the Wasm / Application Frameworks category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Spiderlightning Reviewer

## Purpose

Review and operate SpiderLightning using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SpiderLightning code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SpiderLightning requires evidence-backed assessment.
...
- First-party source: https://github.com/deislabs/spiderlightning
...
- [ ] Trace SpiderLightning inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SpiderLightning defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SpiderLightning least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SpiderLightning resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SpiderLightning release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SpiderLightning dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SpiderLightning identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SpiderLightning change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SpiderLightning documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SpiderLightning architecture and dependency summary with trust boundaries and persistent state.
- Include SpiderLightning validation commands or tests that are safe for the stated environment.
...
- SpiderLightning identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SpiderLightning failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SpiderLightning is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SpiderLightning as the basis for migration or production guidance.
