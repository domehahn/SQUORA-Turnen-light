...
---
# Cncf Openeverest Reviewer

## Purpose

Review and operate OpenEverest using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenEverest code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenEverest requires evidence-backed assessment.
...
- First-party source: https://github.com/openeverest/openeverest
...
- [ ] Trace OpenEverest inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenEverest defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenEverest least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenEverest resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenEverest release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenEverest dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenEverest identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenEverest change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenEverest documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenEverest architecture and dependency summary with trust boundaries and persistent state.
- Include OpenEverest validation commands or tests that are safe for the stated environment.
...
- OpenEverest identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenEverest failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenEverest is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenEverest as the basis for migration or production guidance.
