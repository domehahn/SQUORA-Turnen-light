---
name: "cncf-dx-o2-by-broadcom-reviewer"
description: "Review DX O2 by Broadcom using its official documentation and repository in the Observability and Analysis / Observability category. DX O2 is an AIOps & Observability platform that offers infrastructure, applications & service assurance across Mainframe to mobile in hybrid cloud data center."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Dx O2 By Broadcom Reviewer

## Purpose

Review and operate DX O2 by Broadcom using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DX O2 by Broadcom code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DX O2 by Broadcom requires evidence-backed assessment.
...
- First-party source: https://www.broadcom.com/products/software/aiops-observability/operational-observability
...
- [ ] Trace DX O2 by Broadcom inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DX O2 by Broadcom defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DX O2 by Broadcom least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DX O2 by Broadcom resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DX O2 by Broadcom release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DX O2 by Broadcom dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DX O2 by Broadcom identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DX O2 by Broadcom change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DX O2 by Broadcom documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DX O2 by Broadcom architecture and dependency summary with trust boundaries and persistent state.
- Include DX O2 by Broadcom validation commands or tests that are safe for the stated environment.
...
- DX O2 by Broadcom identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DX O2 by Broadcom failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DX O2 by Broadcom is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DX O2 by Broadcom as the basis for migration or production guidance.
