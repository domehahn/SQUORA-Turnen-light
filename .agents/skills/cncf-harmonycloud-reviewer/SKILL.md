---
name: "cncf-harmonycloud-reviewer"
description: "Review harmonycloud using its official documentation and repository in the Observability and Analysis / Observability category. Provide eBPF-based cloud native monitoring platform, which aim to achieve efficient triage of issues"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Harmonycloud Reviewer

## Purpose

Review and operate harmonycloud using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- harmonycloud code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving harmonycloud requires evidence-backed assessment.
...
- First-party source: https://github.com/KindlingProject/kindling
...
- [ ] Trace harmonycloud inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check harmonycloud defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check harmonycloud least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check harmonycloud resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check harmonycloud release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check harmonycloud dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If harmonycloud identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a harmonycloud change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- harmonycloud documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a harmonycloud architecture and dependency summary with trust boundaries and persistent state.
- Include harmonycloud validation commands or tests that are safe for the stated environment.
...
- harmonycloud identity, version, configuration, dependencies, and deployment assumptions are explicit.
- harmonycloud failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that harmonycloud is secure, supported, compatible, or suitable.
- Using an unpinned latest version of harmonycloud as the basis for migration or production guidance.
