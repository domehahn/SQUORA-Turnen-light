---
name: "cncf-taubyte-reviewer"
description: "Review Taubyte using its official documentation and repository in the Wasm / Hosted Platforms category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Taubyte Reviewer

## Purpose

Review and operate Taubyte using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Taubyte code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Taubyte requires evidence-backed assessment.
...
- First-party source: https://taubyte.com/
...
- [ ] Trace Taubyte inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Taubyte defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Taubyte least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Taubyte resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Taubyte release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Taubyte dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Taubyte identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Taubyte change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Taubyte documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Taubyte architecture and dependency summary with trust boundaries and persistent state.
- Include Taubyte validation commands or tests that are safe for the stated environment.
...
- Taubyte identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Taubyte failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Taubyte is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Taubyte as the basis for migration or production guidance.
