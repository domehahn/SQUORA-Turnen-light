---
name: "cncf-cri-o-reviewer"
description: "Review CRI-O using its official documentation and repository in the Runtime / Container Runtime category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate CRI-O using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CRI-O code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CRI-O requires evidence-backed assessment.
...
- [ ] Trace CRI-O inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CRI-O defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CRI-O least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CRI-O resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CRI-O release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CRI-O dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CRI-O identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CRI-O change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CRI-O documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CRI-O architecture and dependency summary with trust boundaries and persistent state.
- Include CRI-O validation commands or tests that are safe for the stated environment.
...
- CRI-O identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CRI-O failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CRI-O is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CRI-O as the basis for migration or production guidance.
