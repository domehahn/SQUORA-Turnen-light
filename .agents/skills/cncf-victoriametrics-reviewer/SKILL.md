---
name: "cncf-victoriametrics-reviewer"
description: "Review VictoriaMetrics using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Victoriametrics Reviewer

## Purpose

Review and operate VictoriaMetrics using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- VictoriaMetrics code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving VictoriaMetrics requires evidence-backed assessment.
...
- First-party source: https://github.com/VictoriaMetrics/VictoriaMetrics
...
- [ ] Trace VictoriaMetrics inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check VictoriaMetrics defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check VictoriaMetrics least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check VictoriaMetrics resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check VictoriaMetrics release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check VictoriaMetrics dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If VictoriaMetrics identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a VictoriaMetrics change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- VictoriaMetrics documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a VictoriaMetrics architecture and dependency summary with trust boundaries and persistent state.
- Include VictoriaMetrics validation commands or tests that are safe for the stated environment.
...
- VictoriaMetrics identity, version, configuration, dependencies, and deployment assumptions are explicit.
- VictoriaMetrics failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that VictoriaMetrics is secure, supported, compatible, or suitable.
- Using an unpinned latest version of VictoriaMetrics as the basis for migration or production guidance.
