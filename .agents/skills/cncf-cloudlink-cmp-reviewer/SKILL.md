...
---
# Cncf Cloudlink Cmp Reviewer

## Purpose

Review and operate CloudLink-CMP using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CloudLink-CMP code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CloudLink-CMP requires evidence-backed assessment.
...
- First-party source: https://www.bessystem.com/product/31f9842c24e5441394b85bfdc7a9d7df/info
...
- [ ] Trace CloudLink-CMP inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CloudLink-CMP defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CloudLink-CMP least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CloudLink-CMP resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CloudLink-CMP release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CloudLink-CMP dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CloudLink-CMP identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CloudLink-CMP change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CloudLink-CMP documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CloudLink-CMP architecture and dependency summary with trust boundaries and persistent state.
- Include CloudLink-CMP validation commands or tests that are safe for the stated environment.
...
- CloudLink-CMP identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CloudLink-CMP failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CloudLink-CMP is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CloudLink-CMP as the basis for migration or production guidance.
