---
name: "cncf-open-service-mesh-reviewer"
description: "Review Open Service Mesh using its official documentation and repository in the Orchestration & Management / Service Mesh category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Open Service Mesh Reviewer

## Purpose

Review and operate Open Service Mesh using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Open Service Mesh code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Open Service Mesh requires evidence-backed assessment.
...
- First-party source: https://github.com/openservicemesh/osm
...
- [ ] Trace Open Service Mesh inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Open Service Mesh defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Open Service Mesh least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Open Service Mesh resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Open Service Mesh release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Open Service Mesh dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Open Service Mesh identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Open Service Mesh change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Open Service Mesh documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Open Service Mesh architecture and dependency summary with trust boundaries and persistent state.
- Include Open Service Mesh validation commands or tests that are safe for the stated environment.
...
- Open Service Mesh identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Open Service Mesh failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Open Service Mesh is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Open Service Mesh as the basis for migration or production guidance.
