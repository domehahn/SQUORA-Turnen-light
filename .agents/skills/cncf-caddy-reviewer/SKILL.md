---
name: "cncf-caddy-reviewer"
description: "Review Caddy using its official documentation and repository in the Orchestration & Management / Service Proxy category. Caddy is a powerful, enterprise-ready, open source web server with automatic HTTPS written in Go"
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Caddy using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Caddy code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Caddy requires evidence-backed assessment.
...
- First-party source: https://github.com/caddyserver/caddy
...
- [ ] Trace Caddy inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Caddy defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Caddy least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Caddy resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Caddy release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Caddy dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Caddy identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Caddy change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Caddy documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Caddy architecture and dependency summary with trust boundaries and persistent state.
- Include Caddy validation commands or tests that are safe for the stated environment.
...
- Caddy identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Caddy failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Caddy is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Caddy as the basis for migration or production guidance.
