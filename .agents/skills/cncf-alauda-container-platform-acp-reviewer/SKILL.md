---
name: "cncf-alauda-container-platform-acp-reviewer"
description: "Review Alauda Container Platform (ACP) using its official documentation and repository in the Platform / Certified Kubernetes - Distribution category. A Cloud-Native Application Platform based on Kubernetes"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Alauda Container Platform Acp Reviewer

## Purpose

Review and operate Alauda Container Platform (ACP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Alauda Container Platform (ACP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Alauda Container Platform (ACP) requires evidence-backed assessment.
...
- First-party source: http://www.alauda.cn/product/detail/id/364.html
...
- [ ] Trace Alauda Container Platform (ACP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Alauda Container Platform (ACP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Alauda Container Platform (ACP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Alauda Container Platform (ACP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Alauda Container Platform (ACP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Alauda Container Platform (ACP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Alauda Container Platform (ACP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Alauda Container Platform (ACP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Alauda Container Platform (ACP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Alauda Container Platform (ACP) architecture and dependency summary with trust boundaries and persistent state.
- Include Alauda Container Platform (ACP) validation commands or tests that are safe for the stated environment.
...
- Alauda Container Platform (ACP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Alauda Container Platform (ACP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Alauda Container Platform (ACP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Alauda Container Platform (ACP) as the basis for migration or production guidance.
