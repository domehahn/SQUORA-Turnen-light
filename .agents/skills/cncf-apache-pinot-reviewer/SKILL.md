---
name: "cncf-apache-pinot-reviewer"
description: "Review Apache Pinot using its official documentation and repository in the Data / Data Architecture category. A realtime distributed OLAP datastore."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Apache Pinot Reviewer

## Purpose

Review and operate Apache Pinot using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Apache Pinot code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Apache Pinot requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/pinot
...
- [ ] Trace Apache Pinot inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Apache Pinot defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Apache Pinot least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Apache Pinot resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Apache Pinot release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Apache Pinot dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Apache Pinot identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Apache Pinot change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Apache Pinot documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Apache Pinot architecture and dependency summary with trust boundaries and persistent state.
- Include Apache Pinot validation commands or tests that are safe for the stated environment.
...
- Apache Pinot identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Apache Pinot failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Apache Pinot is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Apache Pinot as the basis for migration or production guidance.
