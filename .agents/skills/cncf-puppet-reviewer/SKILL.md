---
name: "cncf-puppet-reviewer"
description: "Review Puppet using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Puppet Reviewer

## Purpose

Review and operate Puppet using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Puppet code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Puppet requires evidence-backed assessment.
...
- First-party source: https://github.com/puppetlabs/puppet
...
- [ ] Trace Puppet inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Puppet defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Puppet least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Puppet resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Puppet release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Puppet dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Puppet identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Puppet change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Puppet documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Puppet architecture and dependency summary with trust boundaries and persistent state.
- Include Puppet validation commands or tests that are safe for the stated environment.
...
- Puppet identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Puppet failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Puppet is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Puppet as the basis for migration or production guidance.
