...
description: "Review FastAPI schemas, dependency injection, async paths, authentication, validation, OpenAPI, tests, and operations."
version: "1.0.0"
since: "2026-08-27"
...
- Source code, configuration, dependencies, tests, deployment artifacts, or runtime behavior in FastAPI services change.
...
- Pydantic models, dependency injection, async endpoints, authentication, OpenAPI, ASGI operations
...
- [ ] Trace FastAPI services inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check FastAPI services defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check FastAPI services least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check FastAPI services resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check FastAPI services release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
...
- If FastAPI services identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a FastAPI services change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- FastAPI services documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a FastAPI services architecture and dependency summary with trust boundaries and persistent state.
- Include FastAPI services validation commands or tests that are safe for the stated environment.
...
- FastAPI services identity, version, configuration, dependencies, and deployment assumptions are explicit.
- FastAPI services failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that FastAPI services is secure, supported, compatible, or suitable.
- Using an unpinned latest version of FastAPI services as the basis for migration or production guidance.
