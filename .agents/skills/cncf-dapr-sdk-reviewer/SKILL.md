---
name: "cncf-dapr-sdk-reviewer"
description: "Review Dapr SDK using its official documentation and repository in the Wasm / Application Frameworks category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Dapr Sdk Reviewer

## Purpose

Review and operate Dapr SDK using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Dapr SDK code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Dapr SDK requires evidence-backed assessment.
...
- First-party source: https://github.com/second-state/dapr-sdk-wasmedge
...
- [ ] Trace Dapr SDK inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Dapr SDK defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Dapr SDK least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Dapr SDK resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Dapr SDK release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Dapr SDK dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Dapr SDK identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Dapr SDK change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Dapr SDK documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Dapr SDK architecture and dependency summary with trust boundaries and persistent state.
- Include Dapr SDK validation commands or tests that are safe for the stated environment.
...
- Dapr SDK identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Dapr SDK failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Dapr SDK is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Dapr SDK as the basis for migration or production guidance.
