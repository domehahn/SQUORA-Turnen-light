---
name: "cncf-terraform-reviewer"
description: "Review Terraform using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Terraform Reviewer

## Purpose

Review and operate Terraform using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Terraform code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Terraform requires evidence-backed assessment.
...
- First-party source: https://github.com/hashicorp/terraform
...
- [ ] Trace Terraform inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Terraform defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Terraform least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Terraform resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Terraform release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Terraform dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Terraform identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Terraform change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Terraform documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Terraform architecture and dependency summary with trust boundaries and persistent state.
- Include Terraform validation commands or tests that are safe for the stated environment.
...
- Terraform identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Terraform failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Terraform is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Terraform as the basis for migration or production guidance.
