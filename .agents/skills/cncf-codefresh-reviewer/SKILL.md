---
name: "cncf-codefresh-reviewer"
description: "Review Codefresh using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Codefresh Reviewer

## Purpose

Review and operate Codefresh using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Codefresh code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Codefresh requires evidence-backed assessment.
...
- First-party source: https://codefresh.io/
...
- [ ] Trace Codefresh inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Codefresh defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Codefresh least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Codefresh resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Codefresh release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Codefresh dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Codefresh identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Codefresh change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Codefresh documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Codefresh architecture and dependency summary with trust boundaries and persistent state.
- Include Codefresh validation commands or tests that are safe for the stated environment.
...
- Codefresh identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Codefresh failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Codefresh is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Codefresh as the basis for migration or production guidance.
