---
name: "cncf-kubeinvaders-reviewer"
description: "Review Kubeinvaders using its official documentation and repository in the Observability and Analysis / Chaos Engineering category. Gamified Chaos Engineering Tool for K8s"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kubeinvaders Reviewer

## Purpose

Review and operate Kubeinvaders using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kubeinvaders code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kubeinvaders requires evidence-backed assessment.
...
- First-party source: https://github.com/lucky-sideburn/kubeinvaders
...
- [ ] Trace Kubeinvaders inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kubeinvaders defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kubeinvaders least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kubeinvaders resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kubeinvaders release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kubeinvaders dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kubeinvaders identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kubeinvaders change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kubeinvaders documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kubeinvaders architecture and dependency summary with trust boundaries and persistent state.
- Include Kubeinvaders validation commands or tests that are safe for the stated environment.
...
- Kubeinvaders identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kubeinvaders failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kubeinvaders is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kubeinvaders as the basis for migration or production guidance.
