---
name: "cncf-kubeflow-mpi-operator-reviewer"
description: "Review Kubeflow MPI Operator using its official documentation and repository in the Training / Distributed Training category. MPI Operator to manage all-reduce distributed training and HPC workloads."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kubeflow Mpi Operator Reviewer

## Purpose

Review and operate Kubeflow MPI Operator using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kubeflow MPI Operator code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kubeflow MPI Operator requires evidence-backed assessment.
...
- First-party source: https://github.com/kubeflow/mpi-operator
...
- [ ] Trace Kubeflow MPI Operator inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kubeflow MPI Operator defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kubeflow MPI Operator least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kubeflow MPI Operator resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kubeflow MPI Operator release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kubeflow MPI Operator dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kubeflow MPI Operator identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kubeflow MPI Operator change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kubeflow MPI Operator documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kubeflow MPI Operator architecture and dependency summary with trust boundaries and persistent state.
- Include Kubeflow MPI Operator validation commands or tests that are safe for the stated environment.
...
- Kubeflow MPI Operator identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kubeflow MPI Operator failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kubeflow MPI Operator is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kubeflow MPI Operator as the basis for migration or production guidance.
