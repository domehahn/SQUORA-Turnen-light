---
name: "cncf-bosh-reviewer"
description: "Review BOSH using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate BOSH using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- BOSH code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving BOSH requires evidence-backed assessment.
...
- First-party source: https://github.com/cloudfoundry/bosh
...
- [ ] Trace BOSH inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check BOSH defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check BOSH least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check BOSH resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check BOSH release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check BOSH dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If BOSH identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a BOSH change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- BOSH documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a BOSH architecture and dependency summary with trust boundaries and persistent state.
- Include BOSH validation commands or tests that are safe for the stated environment.
...
- BOSH identity, version, configuration, dependencies, and deployment assumptions are explicit.
- BOSH failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that BOSH is secure, supported, compatible, or suitable.
- Using an unpinned latest version of BOSH as the basis for migration or production guidance.
