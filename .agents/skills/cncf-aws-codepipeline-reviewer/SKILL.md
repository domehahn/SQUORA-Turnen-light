...
---
# Cncf Aws Codepipeline Reviewer

## Purpose

Review and operate AWS CodePipeline using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- AWS CodePipeline code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving AWS CodePipeline requires evidence-backed assessment.
...
- First-party source: https://aws.amazon.com/codepipeline/
...
- [ ] Trace AWS CodePipeline inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check AWS CodePipeline defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check AWS CodePipeline least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check AWS CodePipeline resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check AWS CodePipeline release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check AWS CodePipeline dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If AWS CodePipeline identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a AWS CodePipeline change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- AWS CodePipeline documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a AWS CodePipeline architecture and dependency summary with trust boundaries and persistent state.
- Include AWS CodePipeline validation commands or tests that are safe for the stated environment.
...
- AWS CodePipeline identity, version, configuration, dependencies, and deployment assumptions are explicit.
- AWS CodePipeline failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that AWS CodePipeline is secure, supported, compatible, or suitable.
- Using an unpinned latest version of AWS CodePipeline as the basis for migration or production guidance.
