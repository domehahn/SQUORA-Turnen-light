---
name: "cncf-kubevela-reviewer"
description: "Review KubeVela using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kubevela Reviewer

## Purpose

Review and operate KubeVela using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeVela code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeVela requires evidence-backed assessment.
...
- First-party source: https://github.com/kubevela/kubevela
...
- [ ] Trace KubeVela inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeVela defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeVela least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeVela resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeVela release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeVela dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeVela identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeVela change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeVela documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeVela architecture and dependency summary with trust boundaries and persistent state.
- Include KubeVela validation commands or tests that are safe for the stated environment.
...
- KubeVela identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeVela failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeVela is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeVela as the basis for migration or production guidance.
