---
name: "cncf-apache-openwhisk-reviewer"
description: "Review Apache OpenWhisk using its official documentation and repository in the Serverless / Installable Platform category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Apache Openwhisk Reviewer

## Purpose

Review and operate Apache OpenWhisk using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Apache OpenWhisk code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Apache OpenWhisk requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/openwhisk
...
- [ ] Trace Apache OpenWhisk inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Apache OpenWhisk defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Apache OpenWhisk least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Apache OpenWhisk resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Apache OpenWhisk release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Apache OpenWhisk dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Apache OpenWhisk identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Apache OpenWhisk change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Apache OpenWhisk documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Apache OpenWhisk architecture and dependency summary with trust boundaries and persistent state.
- Include Apache OpenWhisk validation commands or tests that are safe for the stated environment.
...
- Apache OpenWhisk identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Apache OpenWhisk failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Apache OpenWhisk is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Apache OpenWhisk as the basis for migration or production guidance.
