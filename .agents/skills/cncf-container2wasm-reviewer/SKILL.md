---
name: "cncf-container2wasm-reviewer"
description: "Review container2wasm using its official documentation and repository in the Wasm / Orchestration & Management category. A tool to run containers on Wasm-enabled environments."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Container2wasm Reviewer

## Purpose

Review and operate container2wasm using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- container2wasm code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving container2wasm requires evidence-backed assessment.
...
- CNCF classification: Wasm / Orchestration & Management
- CNCF project maturity field: sandbox
- First-party source: https://github.com/container2wasm/container2wasm
...
- [ ] Trace container2wasm inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check container2wasm defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check container2wasm least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check container2wasm resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check container2wasm release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check container2wasm dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If container2wasm identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a container2wasm change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- container2wasm documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a container2wasm architecture and dependency summary with trust boundaries and persistent state.
- Include container2wasm validation commands or tests that are safe for the stated environment.
...
- container2wasm identity, version, configuration, dependencies, and deployment assumptions are explicit.
- container2wasm failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that container2wasm is secure, supported, compatible, or suitable.
- Using an unpinned latest version of container2wasm as the basis for migration or production guidance.
