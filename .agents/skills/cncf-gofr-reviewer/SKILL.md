---
name: "cncf-gofr-reviewer"
description: "Review GoFr using its official documentation and repository in the Orchestration & Management / Remote Procedure Call category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate GoFr using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- GoFr code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving GoFr requires evidence-backed assessment.
...
- First-party source: https://github.com/gofr-dev/gofr
...
- [ ] Trace GoFr inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check GoFr defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check GoFr least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check GoFr resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check GoFr release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check GoFr dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If GoFr identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a GoFr change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- GoFr documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a GoFr architecture and dependency summary with trust boundaries and persistent state.
- Include GoFr validation commands or tests that are safe for the stated environment.
...
- GoFr identity, version, configuration, dependencies, and deployment assumptions are explicit.
- GoFr failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that GoFr is secure, supported, compatible, or suitable.
- Using an unpinned latest version of GoFr as the basis for migration or production guidance.
