---
name: "cncf-sogou-c-plus-plus-workflow-reviewer"
description: "Review Sogou C++ Workflow using its official documentation and repository in the Platform / PaaS/Container Service category. C++ Parallel Computing and Asynchronous Networking Engine."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Sogou C++ Workflow using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Sogou C++ Workflow code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Sogou C++ Workflow requires evidence-backed assessment.
...
- First-party source: https://github.com/sogou/workflow
...
- [ ] Trace Sogou C++ Workflow inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Sogou C++ Workflow defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Sogou C++ Workflow least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Sogou C++ Workflow resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Sogou C++ Workflow release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Sogou C++ Workflow dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Sogou C++ Workflow identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Sogou C++ Workflow change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Sogou C++ Workflow documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Sogou C++ Workflow architecture and dependency summary with trust boundaries and persistent state.
- Include Sogou C++ Workflow validation commands or tests that are safe for the stated environment.
...
- Sogou C++ Workflow identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Sogou C++ Workflow failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Sogou C++ Workflow is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Sogou C++ Workflow as the basis for migration or production guidance.
