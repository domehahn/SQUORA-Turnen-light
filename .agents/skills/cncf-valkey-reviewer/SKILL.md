---
name: "cncf-valkey-reviewer"
description: "Review Valkey using its official documentation and repository in the App Definition and Development / Database category. A flexible distributed key-value datastore that is optimized for caching and other realtime workloads."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Valkey Reviewer

## Purpose

Review and operate Valkey using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Valkey code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Valkey requires evidence-backed assessment.
...
- First-party source: https://github.com/valkey-io/valkey
...
- [ ] Trace Valkey inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Valkey defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Valkey least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Valkey resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Valkey release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Valkey dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Valkey identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Valkey change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Valkey documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Valkey architecture and dependency summary with trust boundaries and persistent state.
- Include Valkey validation commands or tests that are safe for the stated environment.
...
- Valkey identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Valkey failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Valkey is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Valkey as the basis for migration or production guidance.
