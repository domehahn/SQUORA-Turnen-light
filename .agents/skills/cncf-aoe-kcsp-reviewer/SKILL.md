---
name: "cncf-aoe-kcsp-reviewer"
description: "Review AOE (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. AOE is your trusted partner in navigating the dynamic world of container orchestration. With a rich history dating back to Kubernetes' inception in 2014, our experts bring unparalleled expertise to the table. We offer services around Kubernetes Consulting, Knowledge Transfer and Trainings as well as Operation and Implementation."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Aoe Kcsp Reviewer

## Purpose

Review and operate AOE (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- AOE (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving AOE (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.aoe.com/en/kubernetes-services.html
...
- [ ] Trace AOE (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check AOE (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check AOE (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check AOE (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check AOE (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check AOE (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If AOE (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a AOE (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- AOE (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a AOE (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include AOE (KCSP) validation commands or tests that are safe for the stated environment.
...
- AOE (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- AOE (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that AOE (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of AOE (KCSP) as the basis for migration or production guidance.
