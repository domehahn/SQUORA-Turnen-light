---
name: "php-reviewer"
description: "Review modern PHP code for type safety, Composer hygiene, framework boundaries, security, tests, and performance."
version: "1.0.0"
since: "2026-08-27"
...
- Source code, configuration, dependencies, tests, deployment artifacts, or runtime behavior in PHP engineering change.
...
- strict types, Composer, PSR standards, request validation, PHPUnit, runtime performance
...
- [ ] Trace PHP engineering inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check PHP engineering defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check PHP engineering least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check PHP engineering resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check PHP engineering release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
...
- If PHP engineering identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a PHP engineering change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- PHP engineering documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a PHP engineering architecture and dependency summary with trust boundaries and persistent state.
- Include PHP engineering validation commands or tests that are safe for the stated environment.
...
- PHP engineering identity, version, configuration, dependencies, and deployment assumptions are explicit.
- PHP engineering failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that PHP engineering is secure, supported, compatible, or suitable.
- Using an unpinned latest version of PHP engineering as the basis for migration or production guidance.
