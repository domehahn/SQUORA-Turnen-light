---
name: "cncf-buildpacks-reviewer"
description: "Review Buildpacks using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Buildpacks Reviewer

## Purpose

Review and operate Buildpacks using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Buildpacks code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Buildpacks requires evidence-backed assessment.
...
- First-party source: https://github.com/buildpacks/pack
...
- [ ] Trace Buildpacks inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Buildpacks defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Buildpacks least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Buildpacks resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Buildpacks release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Buildpacks dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Buildpacks identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Buildpacks change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Buildpacks documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Buildpacks architecture and dependency summary with trust boundaries and persistent state.
- Include Buildpacks validation commands or tests that are safe for the stated environment.
...
- Buildpacks identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Buildpacks failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Buildpacks is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Buildpacks as the basis for migration or production guidance.
