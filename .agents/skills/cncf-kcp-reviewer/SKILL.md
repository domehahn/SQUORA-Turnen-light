---
name: "cncf-kcp-reviewer"
description: "Review kcp using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate kcp using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- kcp code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving kcp requires evidence-backed assessment.
...
- First-party source: https://github.com/kcp-dev/kcp
...
- [ ] Trace kcp inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check kcp defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check kcp least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check kcp resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check kcp release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check kcp dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If kcp identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a kcp change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- kcp documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a kcp architecture and dependency summary with trust boundaries and persistent state.
- Include kcp validation commands or tests that are safe for the stated environment.
...
- kcp identity, version, configuration, dependencies, and deployment assumptions are explicit.
- kcp failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that kcp is secure, supported, compatible, or suitable.
- Using an unpinned latest version of kcp as the basis for migration or production guidance.
