---
name: "cncf-uqbar-reviewer"
description: "Review Uqbar using its official documentation and repository in the App Definition and Development / Database category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Uqbar using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Uqbar code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Uqbar requires evidence-backed assessment.
...
- First-party source: https://mogdb.io/en/uqbar/
...
- [ ] Trace Uqbar inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Uqbar defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Uqbar least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Uqbar resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Uqbar release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Uqbar dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Uqbar identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Uqbar change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Uqbar documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Uqbar architecture and dependency summary with trust boundaries and persistent state.
- Include Uqbar validation commands or tests that are safe for the stated environment.
...
- Uqbar identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Uqbar failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Uqbar is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Uqbar as the basis for migration or production guidance.
