---
name: "cncf-coredns-reviewer"
description: "Review CoreDNS using its official documentation and repository in the Orchestration & Management / Coordination & Service Discovery category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Coredns Reviewer

## Purpose

Review and operate CoreDNS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CoreDNS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CoreDNS requires evidence-backed assessment.
...
- First-party source: https://github.com/coredns/coredns
...
- [ ] Trace CoreDNS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CoreDNS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CoreDNS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CoreDNS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CoreDNS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CoreDNS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CoreDNS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CoreDNS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CoreDNS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CoreDNS architecture and dependency summary with trust boundaries and persistent state.
- Include CoreDNS validation commands or tests that are safe for the stated environment.
...
- CoreDNS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CoreDNS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CoreDNS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CoreDNS as the basis for migration or production guidance.
