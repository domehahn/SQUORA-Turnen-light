...
---
# Cncf Vmware Application Catalog Reviewer

## Purpose

Review and operate VMware Application Catalog using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- VMware Application Catalog code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving VMware Application Catalog requires evidence-backed assessment.
...
- First-party source: https://tanzu.vmware.com/application-catalog
...
- [ ] Trace VMware Application Catalog inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check VMware Application Catalog defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check VMware Application Catalog least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check VMware Application Catalog resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check VMware Application Catalog release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check VMware Application Catalog dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If VMware Application Catalog identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a VMware Application Catalog change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- VMware Application Catalog documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a VMware Application Catalog architecture and dependency summary with trust boundaries and persistent state.
- Include VMware Application Catalog validation commands or tests that are safe for the stated environment.
...
- VMware Application Catalog identity, version, configuration, dependencies, and deployment assumptions are explicit.
- VMware Application Catalog failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that VMware Application Catalog is secure, supported, compatible, or suitable.
- Using an unpinned latest version of VMware Application Catalog as the basis for migration or production guidance.
