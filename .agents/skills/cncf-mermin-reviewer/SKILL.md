...
---
# Cncf Mermin Reviewer

## Purpose

Review and operate Mermin using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Mermin code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Mermin requires evidence-backed assessment.
...
- First-party source: https://github.com/elastiflow/mermin
...
- [ ] Trace Mermin inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Mermin defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Mermin least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Mermin resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Mermin release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Mermin dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Mermin identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Mermin change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Mermin documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Mermin architecture and dependency summary with trust boundaries and persistent state.
- Include Mermin validation commands or tests that are safe for the stated environment.
...
- Mermin identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Mermin failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Mermin is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Mermin as the basis for migration or production guidance.
