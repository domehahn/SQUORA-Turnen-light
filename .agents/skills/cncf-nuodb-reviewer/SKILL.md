---
name: "cncf-nuodb-reviewer"
description: "Review NuoDB using its official documentation and repository in the App Definition and Development / Database category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate NuoDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- NuoDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving NuoDB requires evidence-backed assessment.
...
- First-party source: https://www.nuodb.com/
...
- [ ] Trace NuoDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check NuoDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check NuoDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check NuoDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check NuoDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check NuoDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If NuoDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a NuoDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- NuoDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a NuoDB architecture and dependency summary with trust boundaries and persistent state.
- Include NuoDB validation commands or tests that are safe for the stated environment.
...
- NuoDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- NuoDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that NuoDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of NuoDB as the basis for migration or production guidance.
