---
name: "cncf-siddhi-reviewer"
description: "Review Siddhi using its official documentation and repository in the App Definition and Development / Streaming & Messaging category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Siddhi Reviewer

## Purpose

Review and operate Siddhi using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Siddhi code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Siddhi requires evidence-backed assessment.
...
- First-party source: https://github.com/siddhi-io/siddhi
...
- [ ] Trace Siddhi inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Siddhi defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Siddhi least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Siddhi resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Siddhi release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Siddhi dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Siddhi identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Siddhi change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Siddhi documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Siddhi architecture and dependency summary with trust boundaries and persistent state.
- Include Siddhi validation commands or tests that are safe for the stated environment.
...
- Siddhi identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Siddhi failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Siddhi is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Siddhi as the basis for migration or production guidance.
