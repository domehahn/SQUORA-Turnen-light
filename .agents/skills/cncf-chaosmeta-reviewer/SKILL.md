---
name: "cncf-chaosmeta-reviewer"
description: "Review ChaosMeta using its official documentation and repository in the Observability and Analysis / Chaos Engineering category. Provides a one-stop drill platform for the whole life cycle of access detection, fault injection, fault measurement, recovery measurement, injection recovery, etc., and has a built-in rich risk fault injection library for business, application, resources, etc., which can quickly mine the potential risks of the application system and Check the emergency fresh-keeping ability."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Chaosmeta Reviewer

## Purpose

Review and operate ChaosMeta using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ChaosMeta code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ChaosMeta requires evidence-backed assessment.
...
- First-party source: https://chaosmeta.gitbook.io/chaosmeta-en
...
- [ ] Trace ChaosMeta inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ChaosMeta defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ChaosMeta least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ChaosMeta resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ChaosMeta release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ChaosMeta dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ChaosMeta identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ChaosMeta change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ChaosMeta documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ChaosMeta architecture and dependency summary with trust boundaries and persistent state.
- Include ChaosMeta validation commands or tests that are safe for the stated environment.
...
- ChaosMeta identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ChaosMeta failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ChaosMeta is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ChaosMeta as the basis for migration or production guidance.
