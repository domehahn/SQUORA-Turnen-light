---
name: "cncf-kcl-reviewer"
description: "Review KCL using its official documentation and repository in the Provisioning / Automation & Configuration category. A constraint-based record & functional language mainly used in configuration and policy scenarios."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate KCL using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KCL code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KCL requires evidence-backed assessment.
...
- First-party source: https://github.com/kcl-lang/kcl
...
- [ ] Trace KCL inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KCL defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KCL least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KCL resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KCL release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KCL dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KCL identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KCL change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KCL documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KCL architecture and dependency summary with trust boundaries and persistent state.
- Include KCL validation commands or tests that are safe for the stated environment.
...
- KCL identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KCL failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KCL is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KCL as the basis for migration or production guidance.
