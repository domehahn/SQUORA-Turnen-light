---
name: "cncf-wasm3-reviewer"
description: "Review wasm3 using its official documentation and repository in the Wasm / Runtimes category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate wasm3 using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- wasm3 code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving wasm3 requires evidence-backed assessment.
...
- First-party source: https://github.com/wasm3/wasm3
...
- [ ] Trace wasm3 inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check wasm3 defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check wasm3 least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check wasm3 resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check wasm3 release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check wasm3 dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If wasm3 identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a wasm3 change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- wasm3 documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a wasm3 architecture and dependency summary with trust boundaries and persistent state.
- Include wasm3 validation commands or tests that are safe for the stated environment.
...
- wasm3 identity, version, configuration, dependencies, and deployment assumptions are explicit.
- wasm3 failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that wasm3 is secure, supported, compatible, or suitable.
- Using an unpinned latest version of wasm3 as the basis for migration or production guidance.
