---
name: "cncf-avap-reviewer"
description: "Review AVAP using its official documentation and repository in the Orchestration & Management / API Gateway category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate AVAP using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- AVAP code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving AVAP requires evidence-backed assessment.
...
- [ ] Trace AVAP inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check AVAP defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check AVAP least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check AVAP resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check AVAP release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check AVAP dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If AVAP identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a AVAP change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- AVAP documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a AVAP architecture and dependency summary with trust boundaries and persistent state.
- Include AVAP validation commands or tests that are safe for the stated environment.
...
- AVAP identity, version, configuration, dependencies, and deployment assumptions are explicit.
- AVAP failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that AVAP is secure, supported, compatible, or suitable.
- Using an unpinned latest version of AVAP as the basis for migration or production guidance.
