---
name: "cncf-skooner-reviewer"
description: "Review Skooner using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Skooner Reviewer

## Purpose

Review and operate Skooner using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Skooner code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Skooner requires evidence-backed assessment.
...
- First-party source: https://github.com/skooner-k8s/skooner
...
- [ ] Trace Skooner inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Skooner defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Skooner least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Skooner resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Skooner release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Skooner dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Skooner identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Skooner change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Skooner documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Skooner architecture and dependency summary with trust boundaries and persistent state.
- Include Skooner validation commands or tests that are safe for the stated environment.
...
- Skooner identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Skooner failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Skooner is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Skooner as the basis for migration or production guidance.
