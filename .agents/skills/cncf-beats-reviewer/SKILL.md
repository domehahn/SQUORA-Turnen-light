---
name: "cncf-beats-reviewer"
description: "Review Beats using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Beats using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Beats code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Beats requires evidence-backed assessment.
...
- First-party source: https://github.com/elastic/beats
...
- [ ] Trace Beats inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Beats defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Beats least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Beats resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Beats release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Beats dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Beats identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Beats change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Beats documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Beats architecture and dependency summary with trust boundaries and persistent state.
- Include Beats validation commands or tests that are safe for the stated environment.
...
- Beats identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Beats failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Beats is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Beats as the basis for migration or production guidance.
