---
name: "cncf-mysql-reviewer"
description: "Review MySQL using its official documentation and repository in the App Definition and Development / Database category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate MySQL using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- MySQL code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving MySQL requires evidence-backed assessment.
...
- First-party source: https://github.com/mysql/mysql-server
...
- [ ] Trace MySQL inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check MySQL defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check MySQL least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check MySQL resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check MySQL release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check MySQL dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If MySQL identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a MySQL change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- MySQL documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a MySQL architecture and dependency summary with trust boundaries and persistent state.
- Include MySQL validation commands or tests that are safe for the stated environment.
...
- MySQL identity, version, configuration, dependencies, and deployment assumptions are explicit.
- MySQL failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that MySQL is secure, supported, compatible, or suitable.
- Using an unpinned latest version of MySQL as the basis for migration or production guidance.
