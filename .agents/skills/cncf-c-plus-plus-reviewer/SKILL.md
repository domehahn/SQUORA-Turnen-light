---
name: "cncf-c-plus-plus-reviewer"
description: "Review C++ using its official documentation and repository in the Wasm / Languages category. Compiled language to Wasm"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf C Plus Plus Reviewer

## Purpose

Review and operate C++ using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- C++ code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving C++ requires evidence-backed assessment.
...
- First-party source: https://github.com/cplusplus/draft
...
- [ ] Trace C++ inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check C++ defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check C++ least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check C++ resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check C++ release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check C++ dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If C++ identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a C++ change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- C++ documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a C++ architecture and dependency summary with trust boundaries and persistent state.
- Include C++ validation commands or tests that are safe for the stated environment.
...
- C++ identity, version, configuration, dependencies, and deployment assumptions are explicit.
- C++ failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that C++ is secure, supported, compatible, or suitable.
- Using an unpinned latest version of C++ as the basis for migration or production guidance.
