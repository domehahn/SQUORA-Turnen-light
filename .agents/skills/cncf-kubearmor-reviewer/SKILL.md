---
name: "cncf-kubearmor-reviewer"
description: "Review KubeArmor using its official documentation and repository in the Provisioning / Security & Compliance category. Runtime protection for Kubernetes & other cloud Workloads. Kubearmor provides a observability and policy enforcement system to restrict any unwanted, malicious behaviour of cloud-native workloads at runtime."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kubearmor Reviewer

## Purpose

Review and operate KubeArmor using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeArmor code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeArmor requires evidence-backed assessment.
...
- First-party source: https://github.com/kubearmor/kubearmor
...
- [ ] Trace KubeArmor inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeArmor defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeArmor least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeArmor resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeArmor release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeArmor dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeArmor identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeArmor change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeArmor documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeArmor architecture and dependency summary with trust boundaries and persistent state.
- Include KubeArmor validation commands or tests that are safe for the stated environment.
...
- KubeArmor identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeArmor failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeArmor is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeArmor as the basis for migration or production guidance.
