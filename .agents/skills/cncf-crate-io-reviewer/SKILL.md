---
name: "cncf-crate-io-reviewer"
description: "Review Crate.io using its official documentation and repository in the App Definition and Development / Database category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Crate Io Reviewer

## Purpose

Review and operate Crate.io using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Crate.io code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Crate.io requires evidence-backed assessment.
...
- First-party source: https://github.com/crate/crate
...
- [ ] Trace Crate.io inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Crate.io defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Crate.io least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Crate.io resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Crate.io release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Crate.io dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Crate.io identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Crate.io change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Crate.io documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Crate.io architecture and dependency summary with trust boundaries and persistent state.
- Include Crate.io validation commands or tests that are safe for the stated environment.
...
- Crate.io identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Crate.io failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Crate.io is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Crate.io as the basis for migration or production guidance.
