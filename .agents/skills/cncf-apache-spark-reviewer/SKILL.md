---
name: "cncf-apache-spark-reviewer"
description: "Review Apache Spark using its official documentation and repository in the App Definition and Development / Streaming & Messaging category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Apache Spark Reviewer

## Purpose

Review and operate Apache Spark using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Apache Spark code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Apache Spark requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/spark
...
- [ ] Trace Apache Spark inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Apache Spark defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Apache Spark least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Apache Spark resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Apache Spark release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Apache Spark dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Apache Spark identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Apache Spark change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Apache Spark documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Apache Spark architecture and dependency summary with trust boundaries and persistent state.
- Include Apache Spark validation commands or tests that are safe for the stated environment.
...
- Apache Spark identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Apache Spark failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Apache Spark is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Apache Spark as the basis for migration or production guidance.
