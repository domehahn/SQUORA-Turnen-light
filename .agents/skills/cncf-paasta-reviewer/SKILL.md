---
name: "cncf-paasta-reviewer"
description: "Review PaaSTA using its official documentation and repository in the Platform / PaaS/Container Service category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Paasta Reviewer

## Purpose

Review and operate PaaSTA using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- PaaSTA code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving PaaSTA requires evidence-backed assessment.
...
- First-party source: https://github.com/Yelp/paasta
...
- [ ] Trace PaaSTA inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check PaaSTA defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check PaaSTA least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check PaaSTA resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check PaaSTA release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check PaaSTA dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If PaaSTA identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a PaaSTA change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- PaaSTA documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a PaaSTA architecture and dependency summary with trust boundaries and persistent state.
- Include PaaSTA validation commands or tests that are safe for the stated environment.
...
- PaaSTA identity, version, configuration, dependencies, and deployment assumptions are explicit.
- PaaSTA failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that PaaSTA is secure, supported, compatible, or suitable.
- Using an unpinned latest version of PaaSTA as the basis for migration or production guidance.
