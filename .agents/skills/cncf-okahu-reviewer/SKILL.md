---
name: "cncf-okahu-reviewer"
description: "Review Okahu using its official documentation and repository in the AI Native Infra / Observability category. AI Observability as a Service to help enterprises building LLM-based apps in the cloud move to production."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Okahu using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Okahu code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Okahu requires evidence-backed assessment.
...
- [ ] Trace Okahu inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Okahu defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Okahu least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Okahu resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Okahu release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Okahu dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Okahu identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Okahu change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Okahu documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Okahu architecture and dependency summary with trust boundaries and persistent state.
- Include Okahu validation commands or tests that are safe for the stated environment.
...
- Okahu identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Okahu failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Okahu is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Okahu as the basis for migration or production guidance.
