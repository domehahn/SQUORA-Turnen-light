---
name: "cncf-openresty-reviewer"
description: "Review OpenResty using its official documentation and repository in the Orchestration & Management / Service Proxy category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Openresty Reviewer

## Purpose

Review and operate OpenResty using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenResty code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenResty requires evidence-backed assessment.
...
- First-party source: https://github.com/openresty/openresty
...
- [ ] Trace OpenResty inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenResty defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenResty least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenResty resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenResty release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenResty dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenResty identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenResty change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenResty documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenResty architecture and dependency summary with trust boundaries and persistent state.
- Include OpenResty validation commands or tests that are safe for the stated environment.
...
- OpenResty identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenResty failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenResty is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenResty as the basis for migration or production guidance.
