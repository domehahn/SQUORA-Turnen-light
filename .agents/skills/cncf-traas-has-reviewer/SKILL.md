...
---
# Cncf Traas Has Reviewer

## Purpose

Review and operate TRaaS HAS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- TRaaS HAS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving TRaaS HAS requires evidence-backed assessment.
...
- [ ] Trace TRaaS HAS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check TRaaS HAS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check TRaaS HAS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check TRaaS HAS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check TRaaS HAS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check TRaaS HAS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If TRaaS HAS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a TRaaS HAS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- TRaaS HAS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a TRaaS HAS architecture and dependency summary with trust boundaries and persistent state.
- Include TRaaS HAS validation commands or tests that are safe for the stated environment.
...
- TRaaS HAS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- TRaaS HAS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that TRaaS HAS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of TRaaS HAS as the basis for migration or production guidance.
