---
name: "cncf-shipwright-reviewer"
description: "Review Shipwright using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Shipwright Reviewer

## Purpose

Review and operate Shipwright using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Shipwright code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Shipwright requires evidence-backed assessment.
...
- First-party source: https://github.com/shipwright-io/build
...
- [ ] Trace Shipwright inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Shipwright defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Shipwright least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Shipwright resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Shipwright release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Shipwright dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Shipwright identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Shipwright change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Shipwright documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Shipwright architecture and dependency summary with trust boundaries and persistent state.
- Include Shipwright validation commands or tests that are safe for the stated environment.
...
- Shipwright identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Shipwright failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Shipwright is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Shipwright as the basis for migration or production guidance.
