---
name: "cncf-devspace-reviewer"
description: "Review DevSpace using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category. Client-Only Developer Tool for Cloud-Native Development with Kubernetes"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Devspace Reviewer

## Purpose

Review and operate DevSpace using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DevSpace code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DevSpace requires evidence-backed assessment.
...
- First-party source: https://github.com/devspace-sh/devspace
...
- [ ] Trace DevSpace inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DevSpace defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DevSpace least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DevSpace resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DevSpace release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DevSpace dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DevSpace identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DevSpace change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DevSpace documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DevSpace architecture and dependency summary with trust boundaries and persistent state.
- Include DevSpace validation commands or tests that are safe for the stated environment.
...
- DevSpace identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DevSpace failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DevSpace is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DevSpace as the basis for migration or production guidance.
