...
---
# Cncf Vmware Tanzu Cloudhealth Reviewer

## Purpose

Review and operate VMware Tanzu CloudHealth using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- VMware Tanzu CloudHealth code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving VMware Tanzu CloudHealth requires evidence-backed assessment.
...
- First-party source: https://cloudhealth.vmware.com
...
- [ ] Trace VMware Tanzu CloudHealth inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check VMware Tanzu CloudHealth defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check VMware Tanzu CloudHealth least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check VMware Tanzu CloudHealth resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check VMware Tanzu CloudHealth release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check VMware Tanzu CloudHealth dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If VMware Tanzu CloudHealth identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a VMware Tanzu CloudHealth change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- VMware Tanzu CloudHealth documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a VMware Tanzu CloudHealth architecture and dependency summary with trust boundaries and persistent state.
- Include VMware Tanzu CloudHealth validation commands or tests that are safe for the stated environment.
...
- VMware Tanzu CloudHealth identity, version, configuration, dependencies, and deployment assumptions are explicit.
- VMware Tanzu CloudHealth failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that VMware Tanzu CloudHealth is secure, supported, compatible, or suitable.
- Using an unpinned latest version of VMware Tanzu CloudHealth as the basis for migration or production guidance.
