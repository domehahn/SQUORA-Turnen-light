---
name: "cncf-polaris-reviewer"
description: "Review Polaris using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Polaris Reviewer

## Purpose

Review and operate Polaris using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Polaris code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Polaris requires evidence-backed assessment.
...
- First-party source: https://github.com/FairwindsOps/polaris
...
- [ ] Trace Polaris inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Polaris defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Polaris least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Polaris resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Polaris release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Polaris dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Polaris identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Polaris change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Polaris documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Polaris architecture and dependency summary with trust boundaries and persistent state.
- Include Polaris validation commands or tests that are safe for the stated environment.
...
- Polaris identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Polaris failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Polaris is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Polaris as the basis for migration or production guidance.
