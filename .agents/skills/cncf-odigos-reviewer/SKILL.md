---
name: "cncf-odigos-reviewer"
description: "Review Odigos using its official documentation and repository in the Observability and Analysis / Observability category. Enterprise-Grade OpenTelemetry for superior application performance monitoring."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Odigos Reviewer

## Purpose

Review and operate Odigos using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Odigos code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Odigos requires evidence-backed assessment.
...
- First-party source: https://github.com/odigos-io/odigos
...
- [ ] Trace Odigos inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Odigos defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Odigos least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Odigos resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Odigos release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Odigos dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Odigos identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Odigos change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Odigos documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Odigos architecture and dependency summary with trust boundaries and persistent state.
- Include Odigos validation commands or tests that are safe for the stated environment.
...
- Odigos identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Odigos failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Odigos is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Odigos as the basis for migration or production guidance.
