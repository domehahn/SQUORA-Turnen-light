...
---
# Cncf Vcluster Reviewer

## Purpose

Review and operate vCluster using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- vCluster code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving vCluster requires evidence-backed assessment.
...
- First-party source: https://github.com/loft-sh/vcluster
...
- [ ] Trace vCluster inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check vCluster defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check vCluster least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check vCluster resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check vCluster release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check vCluster dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If vCluster identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a vCluster change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- vCluster documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a vCluster architecture and dependency summary with trust boundaries and persistent state.
- Include vCluster validation commands or tests that are safe for the stated environment.
...
- vCluster identity, version, configuration, dependencies, and deployment assumptions are explicit.
- vCluster failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that vCluster is secure, supported, compatible, or suitable.
- Using an unpinned latest version of vCluster as the basis for migration or production guidance.
