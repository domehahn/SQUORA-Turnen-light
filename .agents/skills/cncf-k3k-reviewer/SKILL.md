---
name: "cncf-k3k-reviewer"
description: "Review K3k using its official documentation and repository in the Platform / Certified Kubernetes - Distribution category. Virtual Kubernetes cluster"
version: "1.0.0"
since: "2026-08-27"
...
Review and operate K3k using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- K3k code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving K3k requires evidence-backed assessment.
...
- First-party source: https://github.com/rancher/k3k
...
- [ ] Trace K3k inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check K3k defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check K3k least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check K3k resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check K3k release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check K3k dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If K3k identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a K3k change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- K3k documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a K3k architecture and dependency summary with trust boundaries and persistent state.
- Include K3k validation commands or tests that are safe for the stated environment.
...
- K3k identity, version, configuration, dependencies, and deployment assumptions are explicit.
- K3k failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that K3k is secure, supported, compatible, or suitable.
- Using an unpinned latest version of K3k as the basis for migration or production guidance.
