...
---
# Cncf Clusterpedia Reviewer

## Purpose

Review and operate Clusterpedia using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Clusterpedia code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Clusterpedia requires evidence-backed assessment.
...
- First-party source: https://github.com/clusterpedia-io/clusterpedia
...
- [ ] Trace Clusterpedia inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Clusterpedia defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Clusterpedia least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Clusterpedia resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Clusterpedia release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Clusterpedia dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Clusterpedia identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Clusterpedia change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Clusterpedia documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Clusterpedia architecture and dependency summary with trust boundaries and persistent state.
- Include Clusterpedia validation commands or tests that are safe for the stated environment.
...
- Clusterpedia identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Clusterpedia failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Clusterpedia is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Clusterpedia as the basis for migration or production guidance.
