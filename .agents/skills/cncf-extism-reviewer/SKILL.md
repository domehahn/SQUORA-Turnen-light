---
name: "cncf-extism-reviewer"
description: "Review Extism using its official documentation and repository in the Wasm / Application Frameworks category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Extism Reviewer

## Purpose

Review and operate Extism using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Extism code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Extism requires evidence-backed assessment.
...
- First-party source: https://github.com/extism/extism
...
- [ ] Trace Extism inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Extism defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Extism least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Extism resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Extism release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Extism dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Extism identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Extism change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Extism documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Extism architecture and dependency summary with trust boundaries and persistent state.
- Include Extism validation commands or tests that are safe for the stated environment.
...
- Extism identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Extism failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Extism is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Extism as the basis for migration or production guidance.
