---
name: "cncf-storm-reply-kcsp-reviewer"
description: "Review Storm Reply (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. We provide services along the entire cloud value chain and are specialized in the design, implementation and 24X7 operation of innovative cloud-native solutions."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Storm Reply Kcsp Reviewer

## Purpose

Review and operate Storm Reply (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Storm Reply (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Storm Reply (KCSP) requires evidence-backed assessment.
...
- [ ] Trace Storm Reply (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Storm Reply (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Storm Reply (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Storm Reply (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Storm Reply (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Storm Reply (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Storm Reply (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Storm Reply (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Storm Reply (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Storm Reply (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Storm Reply (KCSP) validation commands or tests that are safe for the stated environment.
...
- Storm Reply (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Storm Reply (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Storm Reply (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Storm Reply (KCSP) as the basis for migration or production guidance.
