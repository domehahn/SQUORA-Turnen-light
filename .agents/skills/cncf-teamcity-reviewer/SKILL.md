---
name: "cncf-teamcity-reviewer"
description: "Review TeamCity using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category. Powerful Continuous Integration out of the box"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Teamcity Reviewer

## Purpose

Review and operate TeamCity using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- TeamCity code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving TeamCity requires evidence-backed assessment.
...
- First-party source: https://www.jetbrains.com/teamcity/
...
- [ ] Trace TeamCity inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check TeamCity defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check TeamCity least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check TeamCity resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check TeamCity release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check TeamCity dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If TeamCity identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a TeamCity change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- TeamCity documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a TeamCity architecture and dependency summary with trust boundaries and persistent state.
- Include TeamCity validation commands or tests that are safe for the stated environment.
...
- TeamCity identity, version, configuration, dependencies, and deployment assumptions are explicit.
- TeamCity failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that TeamCity is secure, supported, compatible, or suitable.
- Using an unpinned latest version of TeamCity as the basis for migration or production guidance.
