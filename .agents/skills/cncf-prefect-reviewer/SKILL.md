---
name: "cncf-prefect-reviewer"
description: "Review Prefect using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Prefect Reviewer

## Purpose

Review and operate Prefect using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Prefect code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Prefect requires evidence-backed assessment.
...
- First-party source: https://github.com/PrefectHQ/prefect
...
- [ ] Trace Prefect inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Prefect defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Prefect least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Prefect resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Prefect release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Prefect dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Prefect identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Prefect change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Prefect documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Prefect architecture and dependency summary with trust boundaries and persistent state.
- Include Prefect validation commands or tests that are safe for the stated environment.
...
- Prefect identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Prefect failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Prefect is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Prefect as the basis for migration or production guidance.
