---
name: "cncf-schemahero-reviewer"
description: "Review SchemaHero using its official documentation and repository in the App Definition and Development / Database category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Schemahero Reviewer

## Purpose

Review and operate SchemaHero using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SchemaHero code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SchemaHero requires evidence-backed assessment.
...
- First-party source: https://github.com/schemahero/schemahero
...
- [ ] Trace SchemaHero inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SchemaHero defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SchemaHero least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SchemaHero resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SchemaHero release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SchemaHero dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SchemaHero identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SchemaHero change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SchemaHero documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SchemaHero architecture and dependency summary with trust boundaries and persistent state.
- Include SchemaHero validation commands or tests that are safe for the stated environment.
...
- SchemaHero identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SchemaHero failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SchemaHero is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SchemaHero as the basis for migration or production guidance.
