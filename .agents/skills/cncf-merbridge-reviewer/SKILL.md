---
name: "cncf-merbridge-reviewer"
description: "Review Merbridge using its official documentation and repository in the Orchestration & Management / Service Mesh category. Use eBPF to speed up your Service Mesh like crossing an Einstein-Rosen Bridge."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Merbridge Reviewer

## Purpose

Review and operate Merbridge using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Merbridge code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Merbridge requires evidence-backed assessment.
...
- First-party source: https://github.com/merbridge/merbridge
...
- [ ] Trace Merbridge inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Merbridge defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Merbridge least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Merbridge resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Merbridge release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Merbridge dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Merbridge identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Merbridge change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Merbridge documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Merbridge architecture and dependency summary with trust boundaries and persistent state.
- Include Merbridge validation commands or tests that are safe for the stated environment.
...
- Merbridge identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Merbridge failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Merbridge is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Merbridge as the basis for migration or production guidance.
