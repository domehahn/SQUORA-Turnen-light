---
name: "cncf-kuasar-reviewer"
description: "Review Kuasar using its official documentation and repository in the Runtime / Container Runtime category. A multi-sandbox container runtime that provides cloud-native, all-scenario multiple sandbox container solutions."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kuasar Reviewer

## Purpose

Review and operate Kuasar using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kuasar code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kuasar requires evidence-backed assessment.
...
- [ ] Trace Kuasar inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kuasar defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kuasar least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kuasar resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kuasar release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kuasar dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kuasar identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kuasar change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kuasar documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kuasar architecture and dependency summary with trust boundaries and persistent state.
- Include Kuasar validation commands or tests that are safe for the stated environment.
...
- Kuasar identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kuasar failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kuasar is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kuasar as the basis for migration or production guidance.
