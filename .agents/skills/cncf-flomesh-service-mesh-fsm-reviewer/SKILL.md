...
---
# Cncf Flomesh Service Mesh Fsm Reviewer

## Purpose

Review and operate Flomesh Service Mesh (FSM) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Flomesh Service Mesh (FSM) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Flomesh Service Mesh (FSM) requires evidence-backed assessment.
...
- First-party source: https://github.com/flomesh-io/fsm
...
- [ ] Trace Flomesh Service Mesh (FSM) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Flomesh Service Mesh (FSM) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Flomesh Service Mesh (FSM) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Flomesh Service Mesh (FSM) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Flomesh Service Mesh (FSM) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Flomesh Service Mesh (FSM) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Flomesh Service Mesh (FSM) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Flomesh Service Mesh (FSM) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Flomesh Service Mesh (FSM) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Flomesh Service Mesh (FSM) architecture and dependency summary with trust boundaries and persistent state.
- Include Flomesh Service Mesh (FSM) validation commands or tests that are safe for the stated environment.
...
- Flomesh Service Mesh (FSM) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Flomesh Service Mesh (FSM) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Flomesh Service Mesh (FSM) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Flomesh Service Mesh (FSM) as the basis for migration or production guidance.
