---
name: "cncf-nuclio-reviewer"
description: "Review Nuclio using its official documentation and repository in the Serverless / Installable Platform category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Nuclio Reviewer

## Purpose

Review and operate Nuclio using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Nuclio code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Nuclio requires evidence-backed assessment.
...
- First-party source: https://github.com/nuclio/nuclio
...
- [ ] Trace Nuclio inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Nuclio defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Nuclio least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Nuclio resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Nuclio release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Nuclio dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Nuclio identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Nuclio change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Nuclio documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Nuclio architecture and dependency summary with trust boundaries and persistent state.
- Include Nuclio validation commands or tests that are safe for the stated environment.
...
- Nuclio identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Nuclio failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Nuclio is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Nuclio as the basis for migration or production guidance.
