---
name: "cncf-whatap-reviewer"
description: "Review WhaTap using its official documentation and repository in the Observability and Analysis / Observability category. WhaTap Labs provides an integrated monitoring service for DevOps that analyzes application performance issue running on kubernetes in real time."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Whatap Reviewer

## Purpose

Review and operate WhaTap using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- WhaTap code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving WhaTap requires evidence-backed assessment.
...
- First-party source: https://whatap.io/en
...
- [ ] Trace WhaTap inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check WhaTap defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check WhaTap least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check WhaTap resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check WhaTap release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check WhaTap dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If WhaTap identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a WhaTap change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- WhaTap documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a WhaTap architecture and dependency summary with trust boundaries and persistent state.
- Include WhaTap validation commands or tests that are safe for the stated environment.
...
- WhaTap identity, version, configuration, dependencies, and deployment assumptions are explicit.
- WhaTap failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that WhaTap is secure, supported, compatible, or suitable.
- Using an unpinned latest version of WhaTap as the basis for migration or production guidance.
