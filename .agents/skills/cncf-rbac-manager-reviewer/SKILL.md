---
name: "cncf-rbac-manager-reviewer"
...
Review and operate RBAC Manager using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- RBAC Manager code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving RBAC Manager requires evidence-backed assessment.
...
- First-party source: https://github.com/FairwindsOps/rbac-manager
...
- [ ] Trace RBAC Manager inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check RBAC Manager defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check RBAC Manager least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check RBAC Manager resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check RBAC Manager release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check RBAC Manager dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If RBAC Manager identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a RBAC Manager change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- RBAC Manager documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a RBAC Manager architecture and dependency summary with trust boundaries and persistent state.
- Include RBAC Manager validation commands or tests that are safe for the stated environment.
...
- RBAC Manager identity, version, configuration, dependencies, and deployment assumptions are explicit.
- RBAC Manager failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that RBAC Manager is secure, supported, compatible, or suitable.
- Using an unpinned latest version of RBAC Manager as the basis for migration or production guidance.
