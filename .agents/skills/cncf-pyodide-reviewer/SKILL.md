---
name: "cncf-pyodide-reviewer"
description: "Review Pyodide using its official documentation and repository in the Wasm / Languages category. Pyodide is a Python distribution for the browser and Node.js base WebAssembly"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Pyodide Reviewer

## Purpose

Review and operate Pyodide using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Pyodide code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Pyodide requires evidence-backed assessment.
...
- First-party source: https://github.com/pyodide/pyodide
...
- [ ] Trace Pyodide inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Pyodide defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Pyodide least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Pyodide resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Pyodide release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Pyodide dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Pyodide identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Pyodide change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Pyodide documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Pyodide architecture and dependency summary with trust boundaries and persistent state.
- Include Pyodide validation commands or tests that are safe for the stated environment.
...
- Pyodide identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Pyodide failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Pyodide is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Pyodide as the basis for migration or production guidance.
