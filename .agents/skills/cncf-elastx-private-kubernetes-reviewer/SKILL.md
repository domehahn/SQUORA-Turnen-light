...
---
# Cncf Elastx Private Kubernetes Reviewer

## Purpose

Review and operate Elastx Private Kubernetes using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Elastx Private Kubernetes code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Elastx Private Kubernetes requires evidence-backed assessment.
...
- First-party source: https://elastx.se/en/kubernetes/
...
- [ ] Trace Elastx Private Kubernetes inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Elastx Private Kubernetes defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Elastx Private Kubernetes least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Elastx Private Kubernetes resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Elastx Private Kubernetes release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Elastx Private Kubernetes dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Elastx Private Kubernetes identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Elastx Private Kubernetes change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Elastx Private Kubernetes documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Elastx Private Kubernetes architecture and dependency summary with trust boundaries and persistent state.
- Include Elastx Private Kubernetes validation commands or tests that are safe for the stated environment.
...
- Elastx Private Kubernetes identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Elastx Private Kubernetes failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Elastx Private Kubernetes is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Elastx Private Kubernetes as the basis for migration or production guidance.
