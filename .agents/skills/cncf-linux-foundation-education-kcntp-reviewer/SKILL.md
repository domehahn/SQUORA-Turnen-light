...
---
# Cncf Linux Foundation Education Kcntp Reviewer

## Purpose

Review and operate Linux Foundation Education (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Linux Foundation Education (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Linux Foundation Education (KCNTP) requires evidence-backed assessment.
...
- First-party source: https://training.linuxfoundation.org/cloud-containers/
...
- [ ] Trace Linux Foundation Education (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Linux Foundation Education (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Linux Foundation Education (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Linux Foundation Education (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Linux Foundation Education (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Linux Foundation Education (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Linux Foundation Education (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Linux Foundation Education (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Linux Foundation Education (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Linux Foundation Education (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include Linux Foundation Education (KCNTP) validation commands or tests that are safe for the stated environment.
...
- Linux Foundation Education (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Linux Foundation Education (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Linux Foundation Education (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Linux Foundation Education (KCNTP) as the basis for migration or production guidance.
