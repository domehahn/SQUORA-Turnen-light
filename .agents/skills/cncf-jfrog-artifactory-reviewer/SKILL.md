---
name: "cncf-jfrog-artifactory-reviewer"
description: "Review JFrog Artifactory using its official documentation and repository in the Provisioning / Container Registry category. Shipping updates continuously and automatically has become a critical element of any successful operation. JFrog is revolutionizing the software world with the practice of Continuous Update, with a speed and continuity that forever changes the way organizations manage and release software."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Jfrog Artifactory Reviewer

## Purpose

Review and operate JFrog Artifactory using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- JFrog Artifactory code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving JFrog Artifactory requires evidence-backed assessment.
...
- First-party source: https://jfrog.com/artifactory/
...
- [ ] Trace JFrog Artifactory inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check JFrog Artifactory defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check JFrog Artifactory least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check JFrog Artifactory resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check JFrog Artifactory release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check JFrog Artifactory dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If JFrog Artifactory identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a JFrog Artifactory change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- JFrog Artifactory documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a JFrog Artifactory architecture and dependency summary with trust boundaries and persistent state.
- Include JFrog Artifactory validation commands or tests that are safe for the stated environment.
...
- JFrog Artifactory identity, version, configuration, dependencies, and deployment assumptions are explicit.
- JFrog Artifactory failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that JFrog Artifactory is secure, supported, compatible, or suitable.
- Using an unpinned latest version of JFrog Artifactory as the basis for migration or production guidance.
