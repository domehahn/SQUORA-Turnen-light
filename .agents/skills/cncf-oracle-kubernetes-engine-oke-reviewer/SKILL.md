...
---
# Cncf Oracle Kubernetes Engine Oke Reviewer

## Purpose

Review and operate Oracle Kubernetes Engine (OKE) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Oracle Kubernetes Engine (OKE) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Oracle Kubernetes Engine (OKE) requires evidence-backed assessment.
...
- First-party source: https://www.oracle.com/cloud/cloud-native/kubernetes-engine
...
- [ ] Trace Oracle Kubernetes Engine (OKE) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Oracle Kubernetes Engine (OKE) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Oracle Kubernetes Engine (OKE) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Oracle Kubernetes Engine (OKE) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Oracle Kubernetes Engine (OKE) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Oracle Kubernetes Engine (OKE) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Oracle Kubernetes Engine (OKE) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Oracle Kubernetes Engine (OKE) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Oracle Kubernetes Engine (OKE) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Oracle Kubernetes Engine (OKE) architecture and dependency summary with trust boundaries and persistent state.
- Include Oracle Kubernetes Engine (OKE) validation commands or tests that are safe for the stated environment.
...
- Oracle Kubernetes Engine (OKE) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Oracle Kubernetes Engine (OKE) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Oracle Kubernetes Engine (OKE) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Oracle Kubernetes Engine (OKE) as the basis for migration or production guidance.
