...
---
# Cncf Itgix Kcsp Reviewer

## Purpose

Review and operate ITGix (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ITGix (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ITGix (KCSP) requires evidence-backed assessment.
...
- First-party source: https://itgix.com/kubernetes-services/
...
- [ ] Trace ITGix (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ITGix (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ITGix (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ITGix (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ITGix (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ITGix (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ITGix (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ITGix (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ITGix (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ITGix (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include ITGix (KCSP) validation commands or tests that are safe for the stated environment.
...
- ITGix (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ITGix (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ITGix (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ITGix (KCSP) as the basis for migration or production guidance.
