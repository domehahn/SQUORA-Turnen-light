...
---
# Cncf Mirrord Reviewer

## Purpose

Review and operate mirrord using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- mirrord code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving mirrord requires evidence-backed assessment.
...
- First-party source: https://github.com/metalbear-co/mirrord
...
- [ ] Trace mirrord inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check mirrord defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check mirrord least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check mirrord resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check mirrord release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check mirrord dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If mirrord identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a mirrord change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- mirrord documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a mirrord architecture and dependency summary with trust boundaries and persistent state.
- Include mirrord validation commands or tests that are safe for the stated environment.
...
- mirrord identity, version, configuration, dependencies, and deployment assumptions are explicit.
- mirrord failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that mirrord is secure, supported, compatible, or suitable.
- Using an unpinned latest version of mirrord as the basis for migration or production guidance.
