---
name: "cncf-flink-reviewer"
description: "Review Flink using its official documentation and repository in the App Definition and Development / Streaming & Messaging category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Flink using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Flink code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Flink requires evidence-backed assessment.
...
- [ ] Trace Flink inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Flink defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Flink least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Flink resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Flink release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Flink dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Flink identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Flink change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Flink documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Flink architecture and dependency summary with trust boundaries and persistent state.
- Include Flink validation commands or tests that are safe for the stated environment.
...
- Flink identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Flink failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Flink is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Flink as the basis for migration or production guidance.
