...
Review and operate Vald using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Vald code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Vald requires evidence-backed assessment.
...
- First-party source: https://github.com/vdaas/vald
...
- [ ] Trace Vald inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Vald defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Vald least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Vald resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Vald release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Vald dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Vald identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Vald change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Vald documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Vald architecture and dependency summary with trust boundaries and persistent state.
- Include Vald validation commands or tests that are safe for the stated environment.
...
- Vald identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Vald failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Vald is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Vald as the basis for migration or production guidance.
