---
name: "cncf-wasmedge-quickjs-reviewer"
description: "Review WasmEdge-Quickjs using its official documentation and repository in the Wasm / Languages category. Scripting languages that support Wasm"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Wasmedge Quickjs Reviewer

## Purpose

Review and operate WasmEdge-Quickjs using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- WasmEdge-Quickjs code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving WasmEdge-Quickjs requires evidence-backed assessment.
...
- First-party source: https://github.com/second-state/wasmedge-quickjs
...
- [ ] Trace WasmEdge-Quickjs inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check WasmEdge-Quickjs defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check WasmEdge-Quickjs least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check WasmEdge-Quickjs resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check WasmEdge-Quickjs release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check WasmEdge-Quickjs dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If WasmEdge-Quickjs identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a WasmEdge-Quickjs change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- WasmEdge-Quickjs documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a WasmEdge-Quickjs architecture and dependency summary with trust boundaries and persistent state.
- Include WasmEdge-Quickjs validation commands or tests that are safe for the stated environment.
...
- WasmEdge-Quickjs identity, version, configuration, dependencies, and deployment assumptions are explicit.
- WasmEdge-Quickjs failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that WasmEdge-Quickjs is secure, supported, compatible, or suitable.
- Using an unpinned latest version of WasmEdge-Quickjs as the basis for migration or production guidance.
