---
name: "cncf-yugabytedb-reviewer"
description: "Review YugabyteDB using its official documentation and repository in the App Definition and Development / Database category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Yugabytedb Reviewer

## Purpose

Review and operate YugabyteDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- YugabyteDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving YugabyteDB requires evidence-backed assessment.
...
- First-party source: https://github.com/YugaByte/yugabyte-db
...
- [ ] Trace YugabyteDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check YugabyteDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check YugabyteDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check YugabyteDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check YugabyteDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check YugabyteDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If YugabyteDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a YugabyteDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- YugabyteDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a YugabyteDB architecture and dependency summary with trust boundaries and persistent state.
- Include YugabyteDB validation commands or tests that are safe for the stated environment.
...
- YugabyteDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- YugabyteDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that YugabyteDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of YugabyteDB as the basis for migration or production guidance.
