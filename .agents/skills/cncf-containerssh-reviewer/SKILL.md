...
---
# Cncf Containerssh Reviewer

## Purpose

Review and operate ContainerSSH using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ContainerSSH code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ContainerSSH requires evidence-backed assessment.
...
- First-party source: https://github.com/containerssh/containerssh
...
- [ ] Trace ContainerSSH inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ContainerSSH defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ContainerSSH least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ContainerSSH resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ContainerSSH release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ContainerSSH dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ContainerSSH identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ContainerSSH change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ContainerSSH documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ContainerSSH architecture and dependency summary with trust boundaries and persistent state.
- Include ContainerSSH validation commands or tests that are safe for the stated environment.
...
- ContainerSSH identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ContainerSSH failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ContainerSSH is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ContainerSSH as the basis for migration or production guidance.
