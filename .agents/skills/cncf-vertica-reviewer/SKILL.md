...
---
# Cncf Vertica Reviewer

## Purpose

Review and operate Vertica using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Vertica code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Vertica requires evidence-backed assessment.
...
- First-party source: https://www.vertica.com/
...
- [ ] Trace Vertica inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Vertica defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Vertica least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Vertica resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Vertica release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Vertica dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Vertica identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Vertica change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Vertica documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Vertica architecture and dependency summary with trust boundaries and persistent state.
- Include Vertica validation commands or tests that are safe for the stated environment.
...
- Vertica identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Vertica failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Vertica is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Vertica as the basis for migration or production guidance.
