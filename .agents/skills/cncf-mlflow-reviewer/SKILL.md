---
name: "cncf-mlflow-reviewer"
description: "Review Mlflow using its official documentation and repository in the AI Native Infra / Continuous Integration and Delivery category. The open source AI engineering platform for agents, LLMs, and ML models. MLflow enables teams of all sizes to debug, evaluate, monitor, and optimize production-quality AI applications while controlling costs and managing access to models and data."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Mlflow Reviewer

## Purpose

Review and operate Mlflow using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Mlflow code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Mlflow requires evidence-backed assessment.
...
- First-party source: https://github.com/mlflow/mlflow
...
- [ ] Trace Mlflow inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Mlflow defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Mlflow least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Mlflow resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Mlflow release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Mlflow dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Mlflow identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Mlflow change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Mlflow documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Mlflow architecture and dependency summary with trust boundaries and persistent state.
- Include Mlflow validation commands or tests that are safe for the stated environment.
...
- Mlflow identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Mlflow failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Mlflow is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Mlflow as the basis for migration or production guidance.
