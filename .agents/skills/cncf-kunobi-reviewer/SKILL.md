...
---
# Cncf Kunobi Reviewer

## Purpose

Review and operate Kunobi using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kunobi code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kunobi requires evidence-backed assessment.
...
- First-party source: https://kunobi.ninja
...
- [ ] Trace Kunobi inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kunobi defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kunobi least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kunobi resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kunobi release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kunobi dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kunobi identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kunobi change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kunobi documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kunobi architecture and dependency summary with trust boundaries and persistent state.
- Include Kunobi validation commands or tests that are safe for the stated environment.
...
- Kunobi identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kunobi failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kunobi is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kunobi as the basis for migration or production guidance.
