---
name: "cncf-sensu-reviewer"
description: "Review Sensu using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Sensu using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Sensu code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Sensu requires evidence-backed assessment.
...
- First-party source: https://github.com/sensu/sensu-go
...
- [ ] Trace Sensu inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Sensu defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Sensu least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Sensu resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Sensu release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Sensu dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Sensu identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Sensu change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Sensu documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Sensu architecture and dependency summary with trust boundaries and persistent state.
- Include Sensu validation commands or tests that are safe for the stated environment.
...
- Sensu identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Sensu failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Sensu is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Sensu as the basis for migration or production guidance.
