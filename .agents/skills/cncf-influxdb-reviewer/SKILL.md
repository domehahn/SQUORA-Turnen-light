---
name: "cncf-influxdb-reviewer"
description: "Review Influxdb using its official documentation and repository in the AI Native Infra / Observability category. InfluxDB is an open source time series database written in Rust, using Apache Arrow, Apache Parquet, and Apache DataFusion as its foundational building blocks."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Influxdb Reviewer

## Purpose

Review and operate Influxdb using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Influxdb code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Influxdb requires evidence-backed assessment.
...
- First-party source: https://github.com/influxdata/influxdb
...
- [ ] Trace Influxdb inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Influxdb defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Influxdb least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Influxdb resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Influxdb release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Influxdb dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Influxdb identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Influxdb change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Influxdb documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Influxdb architecture and dependency summary with trust boundaries and persistent state.
- Include Influxdb validation commands or tests that are safe for the stated environment.
...
- Influxdb identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Influxdb failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Influxdb is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Influxdb as the basis for migration or production guidance.
