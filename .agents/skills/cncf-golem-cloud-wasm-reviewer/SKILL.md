---
name: "cncf-golem-cloud-wasm-reviewer"
description: "Review Golem Cloud (Wasm) using its official documentation and repository in the Wasm / Hosted Platforms category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Golem Cloud Wasm Reviewer

## Purpose

Review and operate Golem Cloud (Wasm) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Golem Cloud (Wasm) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Golem Cloud (Wasm) requires evidence-backed assessment.
...
- First-party source: https://www.golem.cloud/
...
- [ ] Trace Golem Cloud (Wasm) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Golem Cloud (Wasm) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Golem Cloud (Wasm) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Golem Cloud (Wasm) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Golem Cloud (Wasm) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Golem Cloud (Wasm) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Golem Cloud (Wasm) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Golem Cloud (Wasm) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Golem Cloud (Wasm) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Golem Cloud (Wasm) architecture and dependency summary with trust boundaries and persistent state.
- Include Golem Cloud (Wasm) validation commands or tests that are safe for the stated environment.
...
- Golem Cloud (Wasm) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Golem Cloud (Wasm) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Golem Cloud (Wasm) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Golem Cloud (Wasm) as the basis for migration or production guidance.
