---
name: "cncf-catalyst-kubernetes-service-reviewer"
description: "Review Catalyst Kubernetes Service using its official documentation and repository in the Platform / Certified Kubernetes - Hosted category. Catalyst Kubernetes Service makes it easy for you to deploy, manage, and scale Kubernetes clusters to run containerised applications on the Catalyst Cloud."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Catalyst Kubernetes Service Reviewer

## Purpose

Review and operate Catalyst Kubernetes Service using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Catalyst Kubernetes Service code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Catalyst Kubernetes Service requires evidence-backed assessment.
...
- First-party source: https://github.com/catalyst-cloud/magnum
...
- [ ] Trace Catalyst Kubernetes Service inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Catalyst Kubernetes Service defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Catalyst Kubernetes Service least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Catalyst Kubernetes Service resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Catalyst Kubernetes Service release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Catalyst Kubernetes Service dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Catalyst Kubernetes Service identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Catalyst Kubernetes Service change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Catalyst Kubernetes Service documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Catalyst Kubernetes Service architecture and dependency summary with trust boundaries and persistent state.
- Include Catalyst Kubernetes Service validation commands or tests that are safe for the stated environment.
...
- Catalyst Kubernetes Service identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Catalyst Kubernetes Service failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Catalyst Kubernetes Service is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Catalyst Kubernetes Service as the basis for migration or production guidance.
