---
name: "cncf-swisscom-kubernetes-service-reviewer"
description: "Review Swisscom Kubernetes Service using its official documentation and repository in the Platform / Certified Kubernetes - Hosted category. Kubernetes Service is a managed Kubernetes offering in Swisscom’s Private Cloud, based on open-source technology and CNCF tools."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Swisscom Kubernetes Service Reviewer

## Purpose

Review and operate Swisscom Kubernetes Service using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Swisscom Kubernetes Service code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Swisscom Kubernetes Service requires evidence-backed assessment.
...
- First-party source: https://www.swisscom.ch/en/business/enterprise/offer/cloud/cloudnative/container-services.html
...
- [ ] Trace Swisscom Kubernetes Service inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Swisscom Kubernetes Service defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Swisscom Kubernetes Service least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Swisscom Kubernetes Service resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Swisscom Kubernetes Service release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Swisscom Kubernetes Service dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Swisscom Kubernetes Service identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Swisscom Kubernetes Service change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Swisscom Kubernetes Service documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Swisscom Kubernetes Service architecture and dependency summary with trust boundaries and persistent state.
- Include Swisscom Kubernetes Service validation commands or tests that are safe for the stated environment.
...
- Swisscom Kubernetes Service identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Swisscom Kubernetes Service failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Swisscom Kubernetes Service is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Swisscom Kubernetes Service as the basis for migration or production guidance.
