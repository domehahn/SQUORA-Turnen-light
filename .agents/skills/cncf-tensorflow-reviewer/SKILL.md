---
name: "cncf-tensorflow-reviewer"
description: "Review TensorFlow using its official documentation and repository in the Data / Data Science category. An Open Source Machine Learning Framework for Everyone"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Tensorflow Reviewer

## Purpose

Review and operate TensorFlow using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- TensorFlow code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving TensorFlow requires evidence-backed assessment.
...
- First-party source: https://github.com/tensorflow
...
- [ ] Trace TensorFlow inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check TensorFlow defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check TensorFlow least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check TensorFlow resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check TensorFlow release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check TensorFlow dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If TensorFlow identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a TensorFlow change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- TensorFlow documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a TensorFlow architecture and dependency summary with trust boundaries and persistent state.
- Include TensorFlow validation commands or tests that are safe for the stated environment.
...
- TensorFlow identity, version, configuration, dependencies, and deployment assumptions are explicit.
- TensorFlow failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that TensorFlow is secure, supported, compatible, or suitable.
- Using an unpinned latest version of TensorFlow as the basis for migration or production guidance.
