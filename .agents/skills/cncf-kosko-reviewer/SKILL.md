---
name: "cncf-kosko-reviewer"
description: "Review Kosko using its official documentation and repository in the Provisioning / Automation & Configuration category. Kosko can help you organize your Kubernetes manifests in TypeScript, manage multiple environments, ensure type safety using OpenAPI schema, and find issues in your manifests."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Kosko using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kosko code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kosko requires evidence-backed assessment.
...
- First-party source: https://github.com/tommy351/kosko
...
- [ ] Trace Kosko inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kosko defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kosko least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kosko resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kosko release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kosko dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kosko identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kosko change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kosko documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kosko architecture and dependency summary with trust boundaries and persistent state.
- Include Kosko validation commands or tests that are safe for the stated environment.
...
- Kosko identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kosko failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kosko is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kosko as the basis for migration or production guidance.
