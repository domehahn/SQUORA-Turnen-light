...
description: "Review Kotlin code for null safety, coroutines, sealed models, Java interop, Gradle configuration, and tests."
version: "1.0.0"
since: "2026-08-27"
...
- Source code, configuration, dependencies, tests, deployment artifacts, or runtime behavior in Kotlin engineering change.
...
- null safety, coroutines, sealed classes, Java interop, Gradle, testing
...
- [ ] Trace Kotlin engineering inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kotlin engineering defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kotlin engineering least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kotlin engineering resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kotlin engineering release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
...
- If Kotlin engineering identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kotlin engineering change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kotlin engineering documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kotlin engineering architecture and dependency summary with trust boundaries and persistent state.
- Include Kotlin engineering validation commands or tests that are safe for the stated environment.
...
- Kotlin engineering identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kotlin engineering failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kotlin engineering is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kotlin engineering as the basis for migration or production guidance.
