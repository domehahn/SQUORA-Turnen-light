...
---
# Cncf Msap Ai Reviewer

## Purpose

Review and operate MSAP.ai using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- MSAP.ai code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving MSAP.ai requires evidence-backed assessment.
...
- [ ] Trace MSAP.ai inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check MSAP.ai defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check MSAP.ai least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check MSAP.ai resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check MSAP.ai release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check MSAP.ai dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If MSAP.ai identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a MSAP.ai change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- MSAP.ai documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a MSAP.ai architecture and dependency summary with trust boundaries and persistent state.
- Include MSAP.ai validation commands or tests that are safe for the stated environment.
...
- MSAP.ai identity, version, configuration, dependencies, and deployment assumptions are explicit.
- MSAP.ai failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that MSAP.ai is secure, supported, compatible, or suitable.
- Using an unpinned latest version of MSAP.ai as the basis for migration or production guidance.
