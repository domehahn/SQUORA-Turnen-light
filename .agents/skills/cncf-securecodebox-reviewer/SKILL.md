...
---
# Cncf Securecodebox Reviewer

## Purpose

Review and operate secureCodeBox using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- secureCodeBox code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving secureCodeBox requires evidence-backed assessment.
...
- First-party source: https://github.com/secureCodeBox/secureCodeBox
...
- [ ] Trace secureCodeBox inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check secureCodeBox defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check secureCodeBox least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check secureCodeBox resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check secureCodeBox release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check secureCodeBox dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If secureCodeBox identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a secureCodeBox change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- secureCodeBox documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a secureCodeBox architecture and dependency summary with trust boundaries and persistent state.
- Include secureCodeBox validation commands or tests that are safe for the stated environment.
...
- secureCodeBox identity, version, configuration, dependencies, and deployment assumptions are explicit.
- secureCodeBox failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that secureCodeBox is secure, supported, compatible, or suitable.
- Using an unpinned latest version of secureCodeBox as the basis for migration or production guidance.
