---
name: "cncf-lima-reviewer"
description: "Review Lima using its official documentation and repository in the Runtime / Container Runtime category. Linux virtual machines, typically on macOS, for running containerd"
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Lima using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Lima code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Lima requires evidence-backed assessment.
...
- First-party source: https://github.com/lima-vm/lima
...
- [ ] Trace Lima inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Lima defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Lima least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Lima resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Lima release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Lima dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Lima identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Lima change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Lima documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Lima architecture and dependency summary with trust boundaries and persistent state.
- Include Lima validation commands or tests that are safe for the stated environment.
...
- Lima identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Lima failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Lima is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Lima as the basis for migration or production guidance.
