---
name: "cncf-radar-reviewer"
description: "Review Radar using its official documentation and repository in the Observability and Analysis / Observability category. Open-source Kubernetes visibility tool with topology visualization, event timeline, Helm management, and built-in MCP server for AI tools."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Radar using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Radar code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Radar requires evidence-backed assessment.
...
- First-party source: https://github.com/skyhook-io/radar
...
- [ ] Trace Radar inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Radar defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Radar least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Radar resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Radar release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Radar dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Radar identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Radar change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Radar documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Radar architecture and dependency summary with trust boundaries and persistent state.
- Include Radar validation commands or tests that are safe for the stated environment.
...
- Radar identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Radar failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Radar is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Radar as the basis for migration or production guidance.
