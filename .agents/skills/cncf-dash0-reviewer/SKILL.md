...
Review and operate Dash0 using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Dash0 code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Dash0 requires evidence-backed assessment.
...
- [ ] Trace Dash0 inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Dash0 defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Dash0 least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Dash0 resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Dash0 release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Dash0 dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Dash0 identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Dash0 change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Dash0 documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Dash0 architecture and dependency summary with trust boundaries and persistent state.
- Include Dash0 validation commands or tests that are safe for the stated environment.
...
- Dash0 identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Dash0 failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Dash0 is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Dash0 as the basis for migration or production guidance.
