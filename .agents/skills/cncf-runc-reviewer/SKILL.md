---
name: "cncf-runc-reviewer"
description: "Review runc using its official documentation and repository in the Runtime / Container Runtime category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate runc using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- runc code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving runc requires evidence-backed assessment.
...
- First-party source: https://github.com/opencontainers/runc
...
- [ ] Trace runc inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check runc defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check runc least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check runc resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check runc release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check runc dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If runc identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a runc change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- runc documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a runc architecture and dependency summary with trust boundaries and persistent state.
- Include runc validation commands or tests that are safe for the stated environment.
...
- runc identity, version, configuration, dependencies, and deployment assumptions are explicit.
- runc failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that runc is secure, supported, compatible, or suitable.
- Using an unpinned latest version of runc as the basis for migration or production guidance.
