---
name: "cncf-expontech-reviewer"
description: "Review ExponTech using its official documentation and repository in the Runtime / Cloud Native Storage category. ExponTech is a provider of overall data infrastructure solutions based on a new generation of distributed architecture."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Expontech Reviewer

## Purpose

Review and operate ExponTech using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ExponTech code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ExponTech requires evidence-backed assessment.
...
- First-party source: https://www.expontech.com/
...
- [ ] Trace ExponTech inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ExponTech defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ExponTech least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ExponTech resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ExponTech release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ExponTech dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ExponTech identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ExponTech change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ExponTech documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ExponTech architecture and dependency summary with trust boundaries and persistent state.
- Include ExponTech validation commands or tests that are safe for the stated environment.
...
- ExponTech identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ExponTech failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ExponTech is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ExponTech as the basis for migration or production guidance.
