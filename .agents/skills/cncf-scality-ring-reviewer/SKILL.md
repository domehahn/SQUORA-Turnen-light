---
name: "cncf-scality-ring-reviewer"
description: "Review Scality RING using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Scality Ring Reviewer

## Purpose

Review and operate Scality RING using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Scality RING code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Scality RING requires evidence-backed assessment.
...
- First-party source: https://www.scality.com/products/ring/
...
- [ ] Trace Scality RING inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Scality RING defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Scality RING least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Scality RING resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Scality RING release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Scality RING dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Scality RING identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Scality RING change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Scality RING documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Scality RING architecture and dependency summary with trust boundaries and persistent state.
- Include Scality RING validation commands or tests that are safe for the stated environment.
...
- Scality RING identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Scality RING failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Scality RING is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Scality RING as the basis for migration or production guidance.
