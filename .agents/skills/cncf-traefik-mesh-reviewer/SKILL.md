---
name: "cncf-traefik-mesh-reviewer"
...
Review and operate Traefik Mesh using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Traefik Mesh code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Traefik Mesh requires evidence-backed assessment.
...
- First-party source: https://github.com/traefik/mesh
...
- [ ] Trace Traefik Mesh inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Traefik Mesh defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Traefik Mesh least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Traefik Mesh resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Traefik Mesh release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Traefik Mesh dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Traefik Mesh identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Traefik Mesh change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Traefik Mesh documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Traefik Mesh architecture and dependency summary with trust boundaries and persistent state.
- Include Traefik Mesh validation commands or tests that are safe for the stated environment.
...
- Traefik Mesh identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Traefik Mesh failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Traefik Mesh is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Traefik Mesh as the basis for migration or production guidance.
