---
name: "cncf-marqo-reviewer"
description: "Review marqo using its official documentation and repository in the AI Agent / Vector Database category. Ecommerce Search and Discovery"
version: "1.0.0"
since: "2026-08-27"
...
Review and operate marqo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- marqo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving marqo requires evidence-backed assessment.
...
- First-party source: https://github.com/marqo-ai/marqo
...
- [ ] Trace marqo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check marqo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check marqo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check marqo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check marqo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check marqo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If marqo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a marqo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- marqo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a marqo architecture and dependency summary with trust boundaries and persistent state.
- Include marqo validation commands or tests that are safe for the stated environment.
...
- marqo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- marqo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that marqo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of marqo as the basis for migration or production guidance.
