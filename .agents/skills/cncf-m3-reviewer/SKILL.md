---
name: "cncf-m3-reviewer"
description: "Review M3 using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate M3 using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- M3 code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving M3 requires evidence-backed assessment.
...
- First-party source: https://github.com/m3db/m3
...
- [ ] Trace M3 inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check M3 defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check M3 least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check M3 resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check M3 release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check M3 dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If M3 identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a M3 change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- M3 documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a M3 architecture and dependency summary with trust boundaries and persistent state.
- Include M3 validation commands or tests that are safe for the stated environment.
...
- M3 identity, version, configuration, dependencies, and deployment assumptions are explicit.
- M3 failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that M3 is secure, supported, compatible, or suitable.
- Using an unpinned latest version of M3 as the basis for migration or production guidance.
