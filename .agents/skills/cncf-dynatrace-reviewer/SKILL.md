---
name: "cncf-dynatrace-reviewer"
description: "Review Dynatrace using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Dynatrace Reviewer

## Purpose

Review and operate Dynatrace using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Dynatrace code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Dynatrace requires evidence-backed assessment.
...
- [ ] Trace Dynatrace inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Dynatrace defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Dynatrace least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Dynatrace resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Dynatrace release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Dynatrace dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Dynatrace identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Dynatrace change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Dynatrace documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Dynatrace architecture and dependency summary with trust boundaries and persistent state.
- Include Dynatrace validation commands or tests that are safe for the stated environment.
...
- Dynatrace identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Dynatrace failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Dynatrace is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Dynatrace as the basis for migration or production guidance.
