...
---
# Cncf Google Cloud Run Reviewer

## Purpose

Review and operate Google Cloud Run using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Google Cloud Run code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Google Cloud Run requires evidence-backed assessment.
...
- First-party source: https://cloud.google.com/run/
...
- [ ] Trace Google Cloud Run inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Google Cloud Run defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Google Cloud Run least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Google Cloud Run resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Google Cloud Run release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Google Cloud Run dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Google Cloud Run identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Google Cloud Run change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Google Cloud Run documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Google Cloud Run architecture and dependency summary with trust boundaries and persistent state.
- Include Google Cloud Run validation commands or tests that are safe for the stated environment.
...
- Google Cloud Run identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Google Cloud Run failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Google Cloud Run is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Google Cloud Run as the basis for migration or production guidance.
