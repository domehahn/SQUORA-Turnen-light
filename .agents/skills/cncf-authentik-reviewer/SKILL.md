---
name: "cncf-authentik-reviewer"
description: "Review authentik using its official documentation and repository in the Provisioning / Security & Compliance category. authentik is an open-source Identity Provider that emphasizes flexibility and versatility, with support for a wide set of protocols."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Authentik Reviewer

## Purpose

Review and operate authentik using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- authentik code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving authentik requires evidence-backed assessment.
...
- First-party source: https://github.com/goauthentik/authentik
...
- [ ] Trace authentik inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check authentik defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check authentik least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check authentik resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check authentik release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check authentik dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If authentik identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a authentik change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- authentik documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a authentik architecture and dependency summary with trust boundaries and persistent state.
- Include authentik validation commands or tests that are safe for the stated environment.
...
- authentik identity, version, configuration, dependencies, and deployment assumptions are explicit.
- authentik failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that authentik is secure, supported, compatible, or suitable.
- Using an unpinned latest version of authentik as the basis for migration or production guidance.
