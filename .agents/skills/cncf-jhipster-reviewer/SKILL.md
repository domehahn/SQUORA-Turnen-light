---
name: "cncf-jhipster-reviewer"
description: "Review JHipster using its official documentation and repository in the Platform / PaaS/Container Service category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Jhipster Reviewer

## Purpose

Review and operate JHipster using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- JHipster code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving JHipster requires evidence-backed assessment.
...
- First-party source: https://github.com/jhipster/generator-jhipster
...
- [ ] Trace JHipster inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check JHipster defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check JHipster least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check JHipster resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check JHipster release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check JHipster dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If JHipster identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a JHipster change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- JHipster documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a JHipster architecture and dependency summary with trust boundaries and persistent state.
- Include JHipster validation commands or tests that are safe for the stated environment.
...
- JHipster identity, version, configuration, dependencies, and deployment assumptions are explicit.
- JHipster failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that JHipster is secure, supported, compatible, or suitable.
- Using an unpinned latest version of JHipster as the basis for migration or production guidance.
