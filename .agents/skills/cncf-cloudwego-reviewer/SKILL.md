---
name: "cncf-cloudwego-reviewer"
description: "Review CloudWeGo using its official documentation and repository in the Orchestration & Management / Remote Procedure Call category. CloudWeGo is ByteDance's open source Golang-centric middleware that can be used to quickly build enterprise-class cloud native architectures."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Cloudwego Reviewer

## Purpose

Review and operate CloudWeGo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CloudWeGo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CloudWeGo requires evidence-backed assessment.
...
- CNCF classification: Orchestration & Management / Remote Procedure Call
- CNCF project maturity field: not specified
- First-party source: https://github.com/cloudwego/kitex
...
- [ ] Trace CloudWeGo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CloudWeGo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CloudWeGo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CloudWeGo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CloudWeGo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CloudWeGo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CloudWeGo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CloudWeGo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CloudWeGo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CloudWeGo architecture and dependency summary with trust boundaries and persistent state.
- Include CloudWeGo validation commands or tests that are safe for the stated environment.
...
- CloudWeGo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CloudWeGo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CloudWeGo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CloudWeGo as the basis for migration or production guidance.
