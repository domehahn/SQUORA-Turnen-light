---
name: "cncf-dosec-reviewer"
description: "Review Dosec using its official documentation and repository in the Provisioning / Security & Compliance category. Dosec Security Platform provides security protection capabilities for containers, kubernetes, microservices, serverless, and DevOps."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Dosec using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Dosec code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Dosec requires evidence-backed assessment.
...
- [ ] Trace Dosec inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Dosec defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Dosec least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Dosec resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Dosec release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Dosec dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Dosec identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Dosec change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Dosec documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Dosec architecture and dependency summary with trust boundaries and persistent state.
- Include Dosec validation commands or tests that are safe for the stated environment.
...
- Dosec identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Dosec failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Dosec is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Dosec as the basis for migration or production guidance.
