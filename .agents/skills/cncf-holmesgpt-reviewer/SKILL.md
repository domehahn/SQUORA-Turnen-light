...
---
# Cncf Holmesgpt Reviewer

## Purpose

Review and operate HolmesGPT using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- HolmesGPT code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving HolmesGPT requires evidence-backed assessment.
...
- First-party source: https://github.com/HolmesGPT/holmesgpt
...
- [ ] Trace HolmesGPT inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check HolmesGPT defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check HolmesGPT least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check HolmesGPT resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check HolmesGPT release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check HolmesGPT dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If HolmesGPT identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a HolmesGPT change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- HolmesGPT documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a HolmesGPT architecture and dependency summary with trust boundaries and persistent state.
- Include HolmesGPT validation commands or tests that are safe for the stated environment.
...
- HolmesGPT identity, version, configuration, dependencies, and deployment assumptions are explicit.
- HolmesGPT failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that HolmesGPT is secure, supported, compatible, or suitable.
- Using an unpinned latest version of HolmesGPT as the basis for migration or production guidance.
