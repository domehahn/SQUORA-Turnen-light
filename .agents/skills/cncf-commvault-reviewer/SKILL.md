---
name: "cncf-commvault-reviewer"
description: "Review Commvault using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Commvault Reviewer

## Purpose

Review and operate Commvault using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Commvault code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Commvault requires evidence-backed assessment.
...
- First-party source: https://www.commvault.com/software-defined-storage
...
- [ ] Trace Commvault inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Commvault defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Commvault least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Commvault resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Commvault release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Commvault dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Commvault identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Commvault change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Commvault documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Commvault architecture and dependency summary with trust boundaries and persistent state.
- Include Commvault validation commands or tests that are safe for the stated environment.
...
- Commvault identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Commvault failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Commvault is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Commvault as the basis for migration or production guidance.
