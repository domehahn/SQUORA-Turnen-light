---
name: "cncf-epsilla-reviewer"
description: "Review Epsilla using its official documentation and repository in the App Definition and Development / Database category. 10x faster vector database and one-stop RAGaaS platform for building LLM applications"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Epsilla Reviewer

## Purpose

Review and operate Epsilla using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Epsilla code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Epsilla requires evidence-backed assessment.
...
- First-party source: https://github.com/epsilla-cloud/vectordb
...
- [ ] Trace Epsilla inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Epsilla defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Epsilla least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Epsilla resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Epsilla release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Epsilla dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Epsilla identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Epsilla change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Epsilla documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Epsilla architecture and dependency summary with trust boundaries and persistent state.
- Include Epsilla validation commands or tests that are safe for the stated environment.
...
- Epsilla identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Epsilla failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Epsilla is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Epsilla as the basis for migration or production guidance.
