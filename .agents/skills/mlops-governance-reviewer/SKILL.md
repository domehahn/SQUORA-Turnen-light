...
1. Identify the relevant MLOps governance artifacts, owners, systems, environments, and review boundary.
2. Compare the available artifacts against expected signals such as model version, dataset hash, evaluation report, approval record, serving metric, registry transition.
...
- Evidence signals: model version, dataset hash, evaluation report, approval record, serving metric, registry transition.
...
- [ ] Confirm the review boundary covers the right MLOps governance systems, teams, and environments.
- [ ] Inventory and inspect the current model registry entry.
- [ ] Check whether training dataset is current, approved, versioned, and owned.
- [ ] Verify that feature pipeline has test, ticket, log, or approval support.
- [ ] Look for unreproducible model and record concrete repository or process evidence.
- [ ] Look for biased dataset and identify affected assets, services, or stakeholders.
- [ ] Look for silent drift and classify the operational or audit impact.
- [ ] Use model version to validate that the control or practice is operating.
- [ ] Use dataset hash to confirm ownership, timing, and reproducibility.
- [ ] Check exception, risk-acceptance, and expiry handling for MLOps governance.
...
- If model registry entry is missing for a critical service, raise at least a high-severity readiness gap.
- If dataset hash cannot be tied to an owner and approval, treat the outcome as unauditable until corrected.
- If bias evaluation is present but expired or untested, require validation before accepting residual risk.
...
- Missing or stale MLOps governance artifact.
...
- Critical: a gap in MLOps governance creates immediate outage, data-loss, privilege, regulatory-reporting, or irreversible business risk.
- High: model registry entry is missing, unowned, untested, or unauditable for a critical service or material change.
- Medium: training dataset exists but is stale, incomplete, inconsistently enforced, or weakly evidenced.
...
- Findings ordered by severity with affected MLOps governance artifacts and evidence references.
- Coverage note for reviewed artifacts: model registry entry, training dataset, feature pipeline, bias evaluation, drift monitor, deployment gate.
- Risk note covering relevant themes: unreproducible model, biased dataset, silent drift, unapproved promotion, missing lineage, weak rollback.
- Evidence request list using expected signals: model version, dataset hash, evaluation report, approval record, serving metric, registry transition.
- Deliverables or updates needed: MLOps governance review, lineage and reproducibility findings, bias and drift recommendations, deployment gate gaps, model approval notes.
- Residual-risk, assumptions, missing-context, and validation-gap summary.

## Acceptance Criteria

- Relevant MLOps governance artifacts are identified, current, owned, and versioned where applicable.
