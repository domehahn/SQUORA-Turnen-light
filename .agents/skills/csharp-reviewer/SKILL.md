...
description: "Review C# and .NET code for async correctness, dependency injection, resource disposal, nullable types, tests, and performance."
version: "1.0.0"
since: "2026-08-27"
...
- Source code, configuration, dependencies, tests, deployment artifacts, or runtime behavior in C# and .NET engineering change.
...
- async and await, dependency injection, IDisposable, nullable reference types, LINQ, NuGet
...
- [ ] Trace C# and .NET engineering inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check C# and .NET engineering defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check C# and .NET engineering least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check C# and .NET engineering resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check C# and .NET engineering release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
...
- If C# and .NET engineering identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a C# and .NET engineering change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- C# and .NET engineering documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a C# and .NET engineering architecture and dependency summary with trust boundaries and persistent state.
- Include C# and .NET engineering validation commands or tests that are safe for the stated environment.
...
- C# and .NET engineering identity, version, configuration, dependencies, and deployment assumptions are explicit.
- C# and .NET engineering failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that C# and .NET engineering is secure, supported, compatible, or suitable.
- Using an unpinned latest version of C# and .NET engineering as the basis for migration or production guidance.
