...
Review and operate k6 using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- k6 code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving k6 requires evidence-backed assessment.
...
- First-party source: https://github.com/grafana/k6
...
- [ ] Trace k6 inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check k6 defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check k6 least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check k6 resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check k6 release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check k6 dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If k6 identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a k6 change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- k6 documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a k6 architecture and dependency summary with trust boundaries and persistent state.
- Include k6 validation commands or tests that are safe for the stated environment.
...
- k6 identity, version, configuration, dependencies, and deployment assumptions are explicit.
- k6 failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that k6 is secure, supported, compatible, or suitable.
- Using an unpinned latest version of k6 as the basis for migration or production guidance.
