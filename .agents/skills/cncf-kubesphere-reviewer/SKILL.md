...
---
# Cncf Kubesphere Reviewer

## Purpose

Review and operate Kubesphere using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kubesphere code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kubesphere requires evidence-backed assessment.
...
- First-party source: https://github.com/kubesphere/kubesphere
...
- [ ] Trace Kubesphere inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kubesphere defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kubesphere least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kubesphere resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kubesphere release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kubesphere dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kubesphere identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kubesphere change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kubesphere documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kubesphere architecture and dependency summary with trust boundaries and persistent state.
- Include Kubesphere validation commands or tests that are safe for the stated environment.
...
- Kubesphere identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kubesphere failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kubesphere is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kubesphere as the basis for migration or production guidance.
