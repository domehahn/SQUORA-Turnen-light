...
description: "Design subscription billing for plans, trials, invoices, proration, usage, renewals, dunning, cancellation, and entitlements."
version: "1.0.0"
since: "2026-08-27"
...
Design and implement subscription billing for plans, trials, invoices, proration, usage, renewals, dunning, cancellation, entitlements, and asynchronous billing events. Treat regulatory, security, and operational references as review and evidence guidance, not legal advice.
...
- subscription billing decisions, controls, or operating practices need independent review.
- A change affects subscription billing artifacts such as subscription state machine, plan and price catalogue, invoice lifecycle, proration rule, usage-meter record, dunning workflow.
- The user needs evidence-oriented findings for risks such as incorrect proration, entitlement-payment drift, duplicate renewal, unbounded retry or dunning, retroactive price change, missing cancellation effective date.
- Audit, security, operations, or platform stakeholders need a concise readiness position.
- Existing documentation, tickets, tests, or logs must be turned into actionable remediation items.

## Operating model

1. Identify the relevant subscription billing artifacts, owners, systems, environments, and review boundary.
2. Compare the available artifacts against expected signals such as billing-cycle test, invoice calculation, test-clock scenario, usage aggregation result, renewal webhook, entitlement reconciliation.
...
- Primary artifacts: subscription state machine, plan and price catalogue, invoice lifecycle, proration rule, usage-meter record, dunning workflow.
- Risk themes: incorrect proration, entitlement-payment drift, duplicate renewal, unbounded retry or dunning, retroactive price change, missing cancellation effective date.
- Evidence signals: billing-cycle test, invoice calculation, test-clock scenario, usage aggregation result, renewal webhook, entitlement reconciliation.
...
- [ ] Confirm the review boundary covers the right subscription billing systems, teams, and environments.
- [ ] Inventory and inspect the current subscription state machine.
- [ ] Check whether plan and price catalogue is current, approved, versioned, and owned.
- [ ] Verify that invoice lifecycle has test, ticket, log, or approval support.
- [ ] Look for incorrect proration and record concrete repository or process evidence.
- [ ] Look for entitlement-payment drift and identify affected assets, services, or stakeholders.
- [ ] Look for duplicate renewal and classify the operational or audit impact.
- [ ] Use billing-cycle test to validate that the control or practice is operating.
- [ ] Use invoice calculation to confirm ownership, timing, and reproducibility.
- [ ] Check exception, risk-acceptance, and expiry handling for subscription billing.
...
- If subscription state machine is missing for a critical service, raise at least a high-severity readiness gap.
- If invoice calculation cannot be tied to an owner and approval, treat the outcome as unauditable until corrected.
- If proration rule is present but expired or untested, require validation before accepting residual risk.
...
- Missing or stale subscription billing artifact.
...
- Findings ordered by severity with affected subscription billing artifacts and evidence references.
- Coverage note for reviewed artifacts: subscription state machine, plan and price catalogue, invoice lifecycle, proration rule, usage-meter record, dunning workflow.
- Risk note covering relevant themes: incorrect proration, entitlement-payment drift, duplicate renewal, unbounded retry or dunning, retroactive price change, missing cancellation effective date.
- Evidence request list using expected signals: billing-cycle test, invoice calculation, test-clock scenario, usage aggregation result, renewal webhook, entitlement reconciliation.
- Deliverables or updates needed: subscription billing design, plan-change matrix, invoice and proration tests, dunning policy, entitlement synchronization plan.
...
- Relevant subscription billing artifacts are identified, current, owned, and versioned where applicable.
