---
name: "cncf-upcloud-managed-kubernetes-reviewer"
description: "Review UpCloud Managed Kubernetes using its official documentation and repository in the Platform / Certified Kubernetes - Hosted category. UpCloud Managed Kubernetes automates the deployment, scaling and management of container workloads."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Upcloud Managed Kubernetes Reviewer

## Purpose

Review and operate UpCloud Managed Kubernetes using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- UpCloud Managed Kubernetes code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving UpCloud Managed Kubernetes requires evidence-backed assessment.
...
- First-party source: https://upcloud.com/products/managed-kubernetes
...
- [ ] Trace UpCloud Managed Kubernetes inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check UpCloud Managed Kubernetes defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check UpCloud Managed Kubernetes least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check UpCloud Managed Kubernetes resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check UpCloud Managed Kubernetes release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check UpCloud Managed Kubernetes dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If UpCloud Managed Kubernetes identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a UpCloud Managed Kubernetes change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- UpCloud Managed Kubernetes documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a UpCloud Managed Kubernetes architecture and dependency summary with trust boundaries and persistent state.
- Include UpCloud Managed Kubernetes validation commands or tests that are safe for the stated environment.
...
- UpCloud Managed Kubernetes identity, version, configuration, dependencies, and deployment assumptions are explicit.
- UpCloud Managed Kubernetes failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that UpCloud Managed Kubernetes is secure, supported, compatible, or suitable.
- Using an unpinned latest version of UpCloud Managed Kubernetes as the basis for migration or production guidance.
