...
---
# Cncf Openchoreo Reviewer

## Purpose

Review and operate OpenChoreo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenChoreo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenChoreo requires evidence-backed assessment.
...
- First-party source: https://github.com/openchoreo/openchoreo
...
- [ ] Trace OpenChoreo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenChoreo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenChoreo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenChoreo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenChoreo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenChoreo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenChoreo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenChoreo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenChoreo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenChoreo architecture and dependency summary with trust boundaries and persistent state.
- Include OpenChoreo validation commands or tests that are safe for the stated environment.
...
- OpenChoreo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenChoreo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenChoreo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenChoreo as the basis for migration or production guidance.
