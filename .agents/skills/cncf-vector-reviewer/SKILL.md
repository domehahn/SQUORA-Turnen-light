---
name: "cncf-vector-reviewer"
description: "Review Vector using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Vector Reviewer

## Purpose

Review and operate Vector using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Vector code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Vector requires evidence-backed assessment.
...
- First-party source: https://github.com/vectordotdev/vector
...
- [ ] Trace Vector inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Vector defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Vector least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Vector resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Vector release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Vector dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Vector identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Vector change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Vector documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Vector architecture and dependency summary with trust boundaries and persistent state.
- Include Vector validation commands or tests that are safe for the stated environment.
...
- Vector identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Vector failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Vector is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Vector as the basis for migration or production guidance.
