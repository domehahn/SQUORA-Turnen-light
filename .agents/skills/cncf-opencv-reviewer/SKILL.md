---
name: "cncf-opencv-reviewer"
description: "Review OpenCV using its official documentation and repository in the Wasm / AI/Machine Learning category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Opencv Reviewer

## Purpose

Review and operate OpenCV using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenCV code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenCV requires evidence-backed assessment.
...
- First-party source: https://github.com/opencv/opencv
...
- [ ] Trace OpenCV inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenCV defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenCV least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenCV resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenCV release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenCV dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenCV identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenCV change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenCV documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenCV architecture and dependency summary with trust boundaries and persistent state.
- Include OpenCV validation commands or tests that are safe for the stated environment.
...
- OpenCV identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenCV failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenCV is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenCV as the basis for migration or production guidance.
