---
name: "cncf-kubezoo-reviewer"
description: "Review KubeZoo using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kubezoo Reviewer

## Purpose

Review and operate KubeZoo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeZoo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeZoo requires evidence-backed assessment.
...
- First-party source: https://github.com/kubewharf/kubezoo
...
- [ ] Trace KubeZoo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeZoo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeZoo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeZoo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeZoo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeZoo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeZoo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeZoo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeZoo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeZoo architecture and dependency summary with trust boundaries and persistent state.
- Include KubeZoo validation commands or tests that are safe for the stated environment.
...
- KubeZoo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeZoo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeZoo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeZoo as the basis for migration or production guidance.
