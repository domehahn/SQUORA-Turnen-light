---
name: "cncf-wasmex-reviewer"
description: "Review Wasmex using its official documentation and repository in the Wasm / Application Frameworks category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Wasmex Reviewer

## Purpose

Review and operate Wasmex using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Wasmex code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Wasmex requires evidence-backed assessment.
...
- First-party source: https://github.com/tessi/wasmex
...
- [ ] Trace Wasmex inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Wasmex defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Wasmex least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Wasmex resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Wasmex release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Wasmex dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Wasmex identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Wasmex change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Wasmex documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Wasmex architecture and dependency summary with trust boundaries and persistent state.
- Include Wasmex validation commands or tests that are safe for the stated environment.
...
- Wasmex identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Wasmex failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Wasmex is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Wasmex as the basis for migration or production guidance.
