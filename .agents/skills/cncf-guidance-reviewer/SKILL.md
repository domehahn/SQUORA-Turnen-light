---
name: "cncf-guidance-reviewer"
description: "Review Guidance using its official documentation and repository in the AI Agent / Structured Output category. A guidance language for controlling large language models."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Guidance Reviewer

## Purpose

Review and operate Guidance using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Guidance code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Guidance requires evidence-backed assessment.
...
- First-party source: https://github.com/guidance-ai/guidance
...
- [ ] Trace Guidance inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Guidance defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Guidance least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Guidance resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Guidance release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Guidance dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Guidance identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Guidance change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Guidance documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Guidance architecture and dependency summary with trust boundaries and persistent state.
- Include Guidance validation commands or tests that are safe for the stated environment.
...
- Guidance identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Guidance failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Guidance is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Guidance as the basis for migration or production guidance.
