---
name: "cncf-appdynamics-reviewer"
description: "Review AppDynamics using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Appdynamics Reviewer

## Purpose

Review and operate AppDynamics using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- AppDynamics code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving AppDynamics requires evidence-backed assessment.
...
- First-party source: https://www.appdynamics.com/
...
- [ ] Trace AppDynamics inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check AppDynamics defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check AppDynamics least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check AppDynamics resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check AppDynamics release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check AppDynamics dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If AppDynamics identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a AppDynamics change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- AppDynamics documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a AppDynamics architecture and dependency summary with trust boundaries and persistent state.
- Include AppDynamics validation commands or tests that are safe for the stated environment.
...
- AppDynamics identity, version, configuration, dependencies, and deployment assumptions are explicit.
- AppDynamics failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that AppDynamics is secure, supported, compatible, or suitable.
- Using an unpinned latest version of AppDynamics as the basis for migration or production guidance.
