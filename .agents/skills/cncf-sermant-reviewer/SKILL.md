---
name: "cncf-sermant-reviewer"
description: "Review Sermant using its official documentation and repository in the Orchestration & Management / Service Mesh category. Sermant a proxyless service mesh solution based on Javaagent."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Sermant Reviewer

## Purpose

Review and operate Sermant using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Sermant code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Sermant requires evidence-backed assessment.
...
- First-party source: https://github.com/sermant-io/Sermant
...
- [ ] Trace Sermant inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Sermant defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Sermant least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Sermant resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Sermant release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Sermant dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Sermant identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Sermant change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Sermant documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Sermant architecture and dependency summary with trust boundaries and persistent state.
- Include Sermant validation commands or tests that are safe for the stated environment.
...
- Sermant identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Sermant failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Sermant is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Sermant as the basis for migration or production guidance.
