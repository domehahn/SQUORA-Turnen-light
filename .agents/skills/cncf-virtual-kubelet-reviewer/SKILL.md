---
name: "cncf-virtual-kubelet-reviewer"
description: "Review Virtual Kubelet using its official documentation and repository in the Runtime / Container Runtime category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Virtual Kubelet Reviewer

## Purpose

Review and operate Virtual Kubelet using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Virtual Kubelet code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Virtual Kubelet requires evidence-backed assessment.
...
- First-party source: https://github.com/virtual-kubelet/virtual-kubelet
...
- [ ] Trace Virtual Kubelet inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Virtual Kubelet defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Virtual Kubelet least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Virtual Kubelet resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Virtual Kubelet release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Virtual Kubelet dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Virtual Kubelet identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Virtual Kubelet change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Virtual Kubelet documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Virtual Kubelet architecture and dependency summary with trust boundaries and persistent state.
- Include Virtual Kubelet validation commands or tests that are safe for the stated environment.
...
- Virtual Kubelet identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Virtual Kubelet failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Virtual Kubelet is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Virtual Kubelet as the basis for migration or production guidance.
