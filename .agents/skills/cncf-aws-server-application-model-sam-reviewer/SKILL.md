---
name: "cncf-aws-server-application-model-sam-reviewer"
description: "Review AWS Server Application Model (SAM) using its official documentation and repository in the Serverless / Framework category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Aws Server Application Model Sam Reviewer

## Purpose

Review and operate AWS Server Application Model (SAM) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- AWS Server Application Model (SAM) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving AWS Server Application Model (SAM) requires evidence-backed assessment.
...
- CNCF classification: Serverless / Framework
- CNCF project maturity field: not specified
- First-party source: https://github.com/aws/serverless-application-model
...
- [ ] Trace AWS Server Application Model (SAM) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check AWS Server Application Model (SAM) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check AWS Server Application Model (SAM) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check AWS Server Application Model (SAM) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check AWS Server Application Model (SAM) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check AWS Server Application Model (SAM) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If AWS Server Application Model (SAM) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a AWS Server Application Model (SAM) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- AWS Server Application Model (SAM) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a AWS Server Application Model (SAM) architecture and dependency summary with trust boundaries and persistent state.
- Include AWS Server Application Model (SAM) validation commands or tests that are safe for the stated environment.
...
- AWS Server Application Model (SAM) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- AWS Server Application Model (SAM) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that AWS Server Application Model (SAM) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of AWS Server Application Model (SAM) as the basis for migration or production guidance.
