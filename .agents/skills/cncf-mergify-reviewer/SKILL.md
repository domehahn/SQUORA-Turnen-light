---
name: "cncf-mergify-reviewer"
description: "Review Mergify using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category. Optimize your CI/CD pipeline using automation and merge queues"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Mergify Reviewer

## Purpose

Review and operate Mergify using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Mergify code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Mergify requires evidence-backed assessment.
...
- First-party source: https://mergify.com/
...
- [ ] Trace Mergify inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Mergify defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Mergify least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Mergify resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Mergify release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Mergify dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Mergify identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Mergify change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Mergify documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Mergify architecture and dependency summary with trust boundaries and persistent state.
- Include Mergify validation commands or tests that are safe for the stated environment.
...
- Mergify identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Mergify failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Mergify is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Mergify as the basis for migration or production guidance.
