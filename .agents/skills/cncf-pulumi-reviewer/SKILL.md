---
name: "cncf-pulumi-reviewer"
description: "Review Pulumi using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Pulumi Reviewer

## Purpose

Review and operate Pulumi using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Pulumi code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Pulumi requires evidence-backed assessment.
...
- First-party source: https://github.com/pulumi/pulumi
...
- [ ] Trace Pulumi inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Pulumi defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Pulumi least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Pulumi resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Pulumi release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Pulumi dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Pulumi identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Pulumi change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Pulumi documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Pulumi architecture and dependency summary with trust boundaries and persistent state.
- Include Pulumi validation commands or tests that are safe for the stated environment.
...
- Pulumi identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Pulumi failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Pulumi is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Pulumi as the basis for migration or production guidance.
