---
name: "cncf-datacore-kcsp-reviewer"
description: "Review DataCore (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. DataCore offers container-native storage software solutions that simplify the deployment and management of stateful applications on Kubernetes. Implementation and support services are also available."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Datacore Kcsp Reviewer

## Purpose

Review and operate DataCore (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DataCore (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DataCore (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.datacore.com/support/openebs/
...
- [ ] Trace DataCore (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DataCore (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DataCore (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DataCore (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DataCore (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DataCore (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DataCore (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DataCore (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DataCore (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DataCore (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include DataCore (KCSP) validation commands or tests that are safe for the stated environment.
...
- DataCore (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DataCore (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DataCore (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DataCore (KCSP) as the basis for migration or production guidance.
