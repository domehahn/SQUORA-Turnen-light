---
name: "cncf-avro-reviewer"
description: "Review Avro using its official documentation and repository in the Orchestration & Management / Remote Procedure Call category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Avro using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Avro code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Avro requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/avro
...
- [ ] Trace Avro inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Avro defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Avro least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Avro resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Avro release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Avro dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Avro identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Avro change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Avro documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Avro architecture and dependency summary with trust boundaries and persistent state.
- Include Avro validation commands or tests that are safe for the stated environment.
...
- Avro identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Avro failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Avro is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Avro as the basis for migration or production guidance.
