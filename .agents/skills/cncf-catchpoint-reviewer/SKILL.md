---
name: "cncf-catchpoint-reviewer"
description: "Review Catchpoint using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Catchpoint Reviewer

## Purpose

Review and operate Catchpoint using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Catchpoint code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Catchpoint requires evidence-backed assessment.
...
- First-party source: https://www.catchpoint.com/
...
- [ ] Trace Catchpoint inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Catchpoint defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Catchpoint least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Catchpoint resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Catchpoint release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Catchpoint dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Catchpoint identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Catchpoint change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Catchpoint documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Catchpoint architecture and dependency summary with trust boundaries and persistent state.
- Include Catchpoint validation commands or tests that are safe for the stated environment.
...
- Catchpoint identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Catchpoint failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Catchpoint is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Catchpoint as the basis for migration or production guidance.
