---
name: "cncf-eunomia-bpf-reviewer"
description: "Review eunomia-bpf using its official documentation and repository in the Wasm / Application Frameworks category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Eunomia Bpf Reviewer

## Purpose

Review and operate eunomia-bpf using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- eunomia-bpf code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving eunomia-bpf requires evidence-backed assessment.
...
- First-party source: https://github.com/eunomia-bpf/eunomia-bpf
...
- [ ] Trace eunomia-bpf inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check eunomia-bpf defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check eunomia-bpf least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check eunomia-bpf resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check eunomia-bpf release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check eunomia-bpf dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If eunomia-bpf identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a eunomia-bpf change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- eunomia-bpf documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a eunomia-bpf architecture and dependency summary with trust boundaries and persistent state.
- Include eunomia-bpf validation commands or tests that are safe for the stated environment.
...
- eunomia-bpf identity, version, configuration, dependencies, and deployment assumptions are explicit.
- eunomia-bpf failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that eunomia-bpf is secure, supported, compatible, or suitable.
- Using an unpinned latest version of eunomia-bpf as the basis for migration or production guidance.
