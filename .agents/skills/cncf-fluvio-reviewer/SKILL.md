---
name: "cncf-fluvio-reviewer"
description: "Review Fluvio using its official documentation and repository in the App Definition and Development / Streaming & Messaging category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Fluvio Reviewer

## Purpose

Review and operate Fluvio using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Fluvio code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Fluvio requires evidence-backed assessment.
...
- First-party source: https://github.com/infinyon/fluvio
...
- [ ] Trace Fluvio inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Fluvio defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Fluvio least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Fluvio resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Fluvio release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Fluvio dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Fluvio identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Fluvio change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Fluvio documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Fluvio architecture and dependency summary with trust boundaries and persistent state.
- Include Fluvio validation commands or tests that are safe for the stated environment.
...
- Fluvio identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Fluvio failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Fluvio is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Fluvio as the basis for migration or production guidance.
