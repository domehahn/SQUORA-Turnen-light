---
name: "cncf-cortex-reviewer"
description: "Review Cortex using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Cortex Reviewer

## Purpose

Review and operate Cortex using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Cortex code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Cortex requires evidence-backed assessment.
...
- First-party source: https://github.com/cortexproject/cortex
...
- [ ] Trace Cortex inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Cortex defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Cortex least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Cortex resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Cortex release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Cortex dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Cortex identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Cortex change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Cortex documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Cortex architecture and dependency summary with trust boundaries and persistent state.
- Include Cortex validation commands or tests that are safe for the stated environment.
...
- Cortex identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Cortex failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Cortex is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Cortex as the basis for migration or production guidance.
