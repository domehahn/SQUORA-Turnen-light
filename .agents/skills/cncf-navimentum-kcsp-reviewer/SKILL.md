---
name: "cncf-navimentum-kcsp-reviewer"
description: "Review Navimentum (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Navimentum Software Corporation provides Kubernetes special technical services including consulting, training, implementation, and technical support, as well as customized research & development, business operations and other advanced services in China."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Navimentum Kcsp Reviewer

## Purpose

Review and operate Navimentum (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Navimentum (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Navimentum (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.navimentum.com/nv/product/detail/1635215864250257410
...
- [ ] Trace Navimentum (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Navimentum (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Navimentum (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Navimentum (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Navimentum (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Navimentum (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Navimentum (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Navimentum (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Navimentum (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Navimentum (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Navimentum (KCSP) validation commands or tests that are safe for the stated environment.
...
- Navimentum (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Navimentum (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Navimentum (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Navimentum (KCSP) as the basis for migration or production guidance.
