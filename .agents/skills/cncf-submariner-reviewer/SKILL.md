---
name: "cncf-submariner-reviewer"
description: "Review Submariner using its official documentation and repository in the Runtime / Cloud Native Network category. Submariner enables direct networking between Pods and Services in different Kubernetes clusters, either on-premises or in the cloud."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Submariner Reviewer

## Purpose

Review and operate Submariner using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Submariner code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Submariner requires evidence-backed assessment.
...
- First-party source: https://github.com/submariner-io/submariner
...
- [ ] Trace Submariner inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Submariner defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Submariner least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Submariner resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Submariner release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Submariner dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Submariner identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Submariner change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Submariner documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Submariner architecture and dependency summary with trust boundaries and persistent state.
- Include Submariner validation commands or tests that are safe for the stated environment.
...
- Submariner identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Submariner failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Submariner is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Submariner as the basis for migration or production guidance.
