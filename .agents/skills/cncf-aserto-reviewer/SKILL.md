---
name: "cncf-aserto-reviewer"
description: "Review Aserto using its official documentation and repository in the Provisioning / Security & Compliance category. Fine-grained, policy-based, real-time authorization for APIs and microservices. Aserto is the maintainer of the Topaz and Open Policy Containers OSS projects."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Aserto Reviewer

## Purpose

Review and operate Aserto using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Aserto code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Aserto requires evidence-backed assessment.
...
- First-party source: https://github.com/aserto-dev/runtime
...
- [ ] Trace Aserto inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Aserto defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Aserto least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Aserto resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Aserto release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Aserto dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Aserto identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Aserto change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Aserto documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Aserto architecture and dependency summary with trust boundaries and persistent state.
- Include Aserto validation commands or tests that are safe for the stated environment.
...
- Aserto identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Aserto failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Aserto is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Aserto as the basis for migration or production guidance.
