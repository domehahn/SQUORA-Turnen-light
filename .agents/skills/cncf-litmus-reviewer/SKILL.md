---
name: "cncf-litmus-reviewer"
description: "Review Litmus using its official documentation and repository in the Observability and Analysis / Chaos Engineering category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Litmus Reviewer

## Purpose

Review and operate Litmus using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Litmus code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Litmus requires evidence-backed assessment.
...
- First-party source: https://github.com/litmuschaos/litmus
...
- [ ] Trace Litmus inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Litmus defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Litmus least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Litmus resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Litmus release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Litmus dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Litmus identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Litmus change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Litmus documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Litmus architecture and dependency summary with trust boundaries and persistent state.
- Include Litmus validation commands or tests that are safe for the stated environment.
...
- Litmus identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Litmus failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Litmus is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Litmus as the basis for migration or production guidance.
