---
name: "cncf-opencost-reviewer"
description: "Review OpenCost using its official documentation and repository in the Observability and Analysis / Continuous Optimization category. OpenCost provides visibility into current and historical Kubernetes spend and resource allocation."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Opencost Reviewer

## Purpose

Review and operate OpenCost using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenCost code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenCost requires evidence-backed assessment.
...
- First-party source: https://github.com/opencost/opencost
...
- [ ] Trace OpenCost inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenCost defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenCost least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenCost resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenCost release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenCost dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenCost identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenCost change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenCost documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenCost architecture and dependency summary with trust boundaries and persistent state.
- Include OpenCost validation commands or tests that are safe for the stated environment.
...
- OpenCost identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenCost failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenCost is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenCost as the basis for migration or production guidance.
