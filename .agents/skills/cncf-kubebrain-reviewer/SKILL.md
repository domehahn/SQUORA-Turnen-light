---
name: "cncf-kubebrain-reviewer"
description: "Review KubeBrain using its official documentation and repository in the Orchestration & Management / Coordination & Service Discovery category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kubebrain Reviewer

## Purpose

Review and operate KubeBrain using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeBrain code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeBrain requires evidence-backed assessment.
...
- First-party source: https://github.com/kubewharf/kubebrain
...
- [ ] Trace KubeBrain inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeBrain defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeBrain least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeBrain resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeBrain release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeBrain dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeBrain identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeBrain change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeBrain documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeBrain architecture and dependency summary with trust boundaries and persistent state.
- Include KubeBrain validation commands or tests that are safe for the stated environment.
...
- KubeBrain identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeBrain failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeBrain is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeBrain as the basis for migration or production guidance.
