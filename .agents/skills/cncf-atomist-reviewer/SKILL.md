---
name: "cncf-atomist-reviewer"
description: "Review Atomist using its official documentation and repository in the Platform / PaaS/Container Service category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Atomist Reviewer

## Purpose

Review and operate Atomist using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Atomist code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Atomist requires evidence-backed assessment.
...
- First-party source: https://atomist.com/
...
- [ ] Trace Atomist inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Atomist defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Atomist least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Atomist resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Atomist release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Atomist dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Atomist identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Atomist change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Atomist documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Atomist architecture and dependency summary with trust boundaries and persistent state.
- Include Atomist validation commands or tests that are safe for the stated environment.
...
- Atomist identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Atomist failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Atomist is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Atomist as the basis for migration or production guidance.
