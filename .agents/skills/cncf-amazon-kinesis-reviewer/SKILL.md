...
---
# Cncf Amazon Kinesis Reviewer

## Purpose

Review and operate Amazon Kinesis using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Amazon Kinesis code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Amazon Kinesis requires evidence-backed assessment.
...
- First-party source: https://aws.amazon.com/kinesis/
...
- [ ] Trace Amazon Kinesis inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Amazon Kinesis defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Amazon Kinesis least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Amazon Kinesis resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Amazon Kinesis release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Amazon Kinesis dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Amazon Kinesis identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Amazon Kinesis change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Amazon Kinesis documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Amazon Kinesis architecture and dependency summary with trust boundaries and persistent state.
- Include Amazon Kinesis validation commands or tests that are safe for the stated environment.
...
- Amazon Kinesis identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Amazon Kinesis failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Amazon Kinesis is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Amazon Kinesis as the basis for migration or production guidance.
