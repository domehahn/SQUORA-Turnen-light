---
name: "cncf-starrocks-reviewer"
description: "Review StarRocks using its official documentation and repository in the App Definition and Development / Database category. StarRocks is a next-gen sub-second MPP database for full analytics scenarios, including multi-dimensional analytics, real-time analytics and ad-hoc query."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Starrocks Reviewer

## Purpose

Review and operate StarRocks using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- StarRocks code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving StarRocks requires evidence-backed assessment.
...
- First-party source: https://github.com/StarRocks/starrocks
...
- [ ] Trace StarRocks inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check StarRocks defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check StarRocks least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check StarRocks resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check StarRocks release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check StarRocks dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If StarRocks identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a StarRocks change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- StarRocks documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a StarRocks architecture and dependency summary with trust boundaries and persistent state.
- Include StarRocks validation commands or tests that are safe for the stated environment.
...
- StarRocks identity, version, configuration, dependencies, and deployment assumptions are explicit.
- StarRocks failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that StarRocks is secure, supported, compatible, or suitable.
- Using an unpinned latest version of StarRocks as the basis for migration or production guidance.
