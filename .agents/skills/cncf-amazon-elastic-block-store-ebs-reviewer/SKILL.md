---
name: "cncf-amazon-elastic-block-store-ebs-reviewer"
description: "Review Amazon Elastic Block Store (EBS) using its official documentation and repository in the Runtime / Cloud Native Storage category. Amazon Elastic Block Store (EBS) is an easy to use, high performance block storage service designed for use with Amazon Elastic Compute Cloud (EC2) for both throughput and transaction intensive workloads at any scale."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Amazon Elastic Block Store Ebs Reviewer

## Purpose

Review and operate Amazon Elastic Block Store (EBS) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Amazon Elastic Block Store (EBS) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Amazon Elastic Block Store (EBS) requires evidence-backed assessment.
...
- First-party source: https://aws.amazon.com/ebs/
...
- [ ] Trace Amazon Elastic Block Store (EBS) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Amazon Elastic Block Store (EBS) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Amazon Elastic Block Store (EBS) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Amazon Elastic Block Store (EBS) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Amazon Elastic Block Store (EBS) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Amazon Elastic Block Store (EBS) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Amazon Elastic Block Store (EBS) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Amazon Elastic Block Store (EBS) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Amazon Elastic Block Store (EBS) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Amazon Elastic Block Store (EBS) architecture and dependency summary with trust boundaries and persistent state.
- Include Amazon Elastic Block Store (EBS) validation commands or tests that are safe for the stated environment.
...
- Amazon Elastic Block Store (EBS) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Amazon Elastic Block Store (EBS) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Amazon Elastic Block Store (EBS) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Amazon Elastic Block Store (EBS) as the basis for migration or production guidance.
