---
name: "cncf-new-relic-reviewer"
description: "Review New Relic using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf New Relic Reviewer

## Purpose

Review and operate New Relic using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- New Relic code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving New Relic requires evidence-backed assessment.
...
- First-party source: https://newrelic.com/
...
- [ ] Trace New Relic inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check New Relic defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check New Relic least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check New Relic resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check New Relic release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check New Relic dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If New Relic identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a New Relic change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- New Relic documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a New Relic architecture and dependency summary with trust boundaries and persistent state.
- Include New Relic validation commands or tests that are safe for the stated environment.
...
- New Relic identity, version, configuration, dependencies, and deployment assumptions are explicit.
- New Relic failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that New Relic is secure, supported, compatible, or suitable.
- Using an unpinned latest version of New Relic as the basis for migration or production guidance.
