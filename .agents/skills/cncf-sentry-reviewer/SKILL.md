---
name: "cncf-sentry-reviewer"
description: "Review Sentry using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Sentry Reviewer

## Purpose

Review and operate Sentry using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Sentry code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Sentry requires evidence-backed assessment.
...
- First-party source: https://github.com/getsentry/sentry
...
- [ ] Trace Sentry inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Sentry defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Sentry least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Sentry resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Sentry release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Sentry dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Sentry identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Sentry change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Sentry documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Sentry architecture and dependency summary with trust boundaries and persistent state.
- Include Sentry validation commands or tests that are safe for the stated environment.
...
- Sentry identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Sentry failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Sentry is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Sentry as the basis for migration or production guidance.
