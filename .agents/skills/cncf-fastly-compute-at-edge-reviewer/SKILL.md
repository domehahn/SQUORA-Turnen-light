---
name: "cncf-fastly-compute-at-edge-reviewer"
description: "Review Fastly Compute@Edge using its official documentation and repository in the Wasm / Hosted Platforms category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Fastly Compute At Edge Reviewer

## Purpose

Review and operate Fastly Compute@Edge using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Fastly Compute@Edge code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Fastly Compute@Edge requires evidence-backed assessment.
...
- First-party source: https://www.fastly.com/products/edge-compute
...
- [ ] Trace Fastly Compute@Edge inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Fastly Compute@Edge defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Fastly Compute@Edge least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Fastly Compute@Edge resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Fastly Compute@Edge release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Fastly Compute@Edge dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Fastly Compute@Edge identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Fastly Compute@Edge change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Fastly Compute@Edge documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Fastly Compute@Edge architecture and dependency summary with trust boundaries and persistent state.
- Include Fastly Compute@Edge validation commands or tests that are safe for the stated environment.
...
- Fastly Compute@Edge identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Fastly Compute@Edge failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Fastly Compute@Edge is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Fastly Compute@Edge as the basis for migration or production guidance.
