---
name: "cncf-scale-reviewer"
description: "Review Scale using its official documentation and repository in the Wasm / Application Frameworks category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Scale using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Scale code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Scale requires evidence-backed assessment.
...
- First-party source: https://github.com/loopholelabs/scale
...
- [ ] Trace Scale inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Scale defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Scale least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Scale resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Scale release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Scale dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Scale identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Scale change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Scale documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Scale architecture and dependency summary with trust boundaries and persistent state.
- Include Scale validation commands or tests that are safe for the stated environment.
...
- Scale identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Scale failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Scale is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Scale as the basis for migration or production guidance.
