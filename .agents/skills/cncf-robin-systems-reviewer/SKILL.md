---
name: "cncf-robin-systems-reviewer"
description: "Review Robin Systems using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Robin Systems Reviewer

## Purpose

Review and operate Robin Systems using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Robin Systems code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Robin Systems requires evidence-backed assessment.
...
- [ ] Trace Robin Systems inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Robin Systems defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Robin Systems least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Robin Systems resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Robin Systems release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Robin Systems dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Robin Systems identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Robin Systems change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Robin Systems documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Robin Systems architecture and dependency summary with trust boundaries and persistent state.
- Include Robin Systems validation commands or tests that are safe for the stated environment.
...
- Robin Systems identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Robin Systems failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Robin Systems is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Robin Systems as the basis for migration or production guidance.
