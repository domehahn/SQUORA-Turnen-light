---
name: "cncf-screwdriver-reviewer"
description: "Review Screwdriver using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Screwdriver Reviewer

## Purpose

Review and operate Screwdriver using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Screwdriver code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Screwdriver requires evidence-backed assessment.
...
- First-party source: https://github.com/screwdriver-cd/screwdriver
...
- [ ] Trace Screwdriver inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Screwdriver defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Screwdriver least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Screwdriver resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Screwdriver release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Screwdriver dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Screwdriver identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Screwdriver change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Screwdriver documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Screwdriver architecture and dependency summary with trust boundaries and persistent state.
- Include Screwdriver validation commands or tests that are safe for the stated environment.
...
- Screwdriver identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Screwdriver failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Screwdriver is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Screwdriver as the basis for migration or production guidance.
