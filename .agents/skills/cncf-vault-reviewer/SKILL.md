---
name: "cncf-vault-reviewer"
description: "Review Vault using its official documentation and repository in the Provisioning / Key Management category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Vault using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Vault code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Vault requires evidence-backed assessment.
...
- First-party source: https://github.com/hashicorp/vault
...
- [ ] Trace Vault inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Vault defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Vault least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Vault resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Vault release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Vault dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Vault identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Vault change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Vault documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Vault architecture and dependency summary with trust boundaries and persistent state.
- Include Vault validation commands or tests that are safe for the stated environment.
...
- Vault identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Vault failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Vault is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Vault as the basis for migration or production guidance.
