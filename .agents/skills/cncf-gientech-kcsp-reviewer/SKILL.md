---
name: "cncf-gientech-kcsp-reviewer"
description: "Review GienTech (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Gientech can help customers understand and deploy Kubernetes container technology deeply. We provides customized Kubernetes services including consulting, training, implementation and technical support."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Gientech Kcsp Reviewer

## Purpose

Review and operate GienTech (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- GienTech (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving GienTech (KCSP) requires evidence-backed assessment.
...
- First-party source: https://en.gientech.com/consulting/Kubernetes.html
...
- [ ] Trace GienTech (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check GienTech (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check GienTech (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check GienTech (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check GienTech (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check GienTech (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If GienTech (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a GienTech (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- GienTech (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a GienTech (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include GienTech (KCSP) validation commands or tests that are safe for the stated environment.
...
- GienTech (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- GienTech (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that GienTech (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of GienTech (KCSP) as the basis for migration or production guidance.
