---
name: "cncf-sysdig-reviewer"
description: "Review sysdig using its official documentation and repository in the Observability and Analysis / Observability category. sysdig is a simple tool with deep system visibility for exploration and troubleshooting, with native support for containers."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Sysdig Reviewer

## Purpose

Review and operate sysdig using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- sysdig code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving sysdig requires evidence-backed assessment.
...
- First-party source: https://github.com/draios/sysdig
...
- [ ] Trace sysdig inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check sysdig defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check sysdig least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check sysdig resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check sysdig release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check sysdig dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If sysdig identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a sysdig change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- sysdig documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a sysdig architecture and dependency summary with trust boundaries and persistent state.
- Include sysdig validation commands or tests that are safe for the stated environment.
...
- sysdig identity, version, configuration, dependencies, and deployment assumptions are explicit.
- sysdig failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that sysdig is secure, supported, compatible, or suitable.
- Using an unpinned latest version of sysdig as the basis for migration or production guidance.
