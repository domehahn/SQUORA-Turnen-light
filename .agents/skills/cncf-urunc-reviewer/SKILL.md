---
name: "cncf-urunc-reviewer"
description: "Review urunc using its official documentation and repository in the Runtime / Container Runtime category. A CRI-compatible runtime for running unikernels and application kernels as containers.  urunc bridges the gap between unikernels and containerized environments, enabling seamless  integration with cloud-native architectures while maintaining OCI compatibility."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate urunc using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- urunc code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving urunc requires evidence-backed assessment.
...
- First-party source: https://github.com/urunc-dev/urunc
...
- [ ] Trace urunc inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check urunc defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check urunc least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check urunc resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check urunc release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check urunc dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If urunc identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a urunc change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- urunc documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a urunc architecture and dependency summary with trust boundaries and persistent state.
- Include urunc validation commands or tests that are safe for the stated environment.
...
- urunc identity, version, configuration, dependencies, and deployment assumptions are explicit.
- urunc failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that urunc is secure, supported, compatible, or suitable.
- Using an unpinned latest version of urunc as the basis for migration or production guidance.
