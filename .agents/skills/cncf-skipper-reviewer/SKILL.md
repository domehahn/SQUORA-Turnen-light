---
name: "cncf-skipper-reviewer"
description: "Review Skipper using its official documentation and repository in the Orchestration & Management / Service Proxy category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Skipper Reviewer

## Purpose

Review and operate Skipper using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Skipper code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Skipper requires evidence-backed assessment.
...
- First-party source: https://github.com/zalando/skipper
...
- [ ] Trace Skipper inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Skipper defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Skipper least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Skipper resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Skipper release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Skipper dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Skipper identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Skipper change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Skipper documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Skipper architecture and dependency summary with trust boundaries and persistent state.
- Include Skipper validation commands or tests that are safe for the stated environment.
...
- Skipper identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Skipper failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Skipper is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Skipper as the basis for migration or production guidance.
