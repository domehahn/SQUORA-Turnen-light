---
name: "cncf-steamhaus-kcsp-reviewer"
description: "Review Steamhaus (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Steamhaus are AWS cloud native experts, specialising in designing, building and operating containers and serverless platforms. We help startups scale fast and securely, and we help enterprises accelerate and derisk modernisation and transformation."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Steamhaus Kcsp Reviewer

## Purpose

Review and operate Steamhaus (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Steamhaus (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Steamhaus (KCSP) requires evidence-backed assessment.
...
- [ ] Trace Steamhaus (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Steamhaus (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Steamhaus (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Steamhaus (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Steamhaus (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Steamhaus (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Steamhaus (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Steamhaus (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Steamhaus (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Steamhaus (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Steamhaus (KCSP) validation commands or tests that are safe for the stated environment.
...
- Steamhaus (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Steamhaus (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Steamhaus (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Steamhaus (KCSP) as the basis for migration or production guidance.
