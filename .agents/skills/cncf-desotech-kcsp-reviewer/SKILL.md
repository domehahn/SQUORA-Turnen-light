---
name: "cncf-desotech-kcsp-reviewer"
description: "Review Desotech (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Desotech provides training and consulting services for the world’s leading containerization platform and can customize it to your business needs."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Desotech Kcsp Reviewer

## Purpose

Review and operate Desotech (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Desotech (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Desotech (KCSP) requires evidence-backed assessment.
...
- [ ] Trace Desotech (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Desotech (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Desotech (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Desotech (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Desotech (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Desotech (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Desotech (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Desotech (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Desotech (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Desotech (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Desotech (KCSP) validation commands or tests that are safe for the stated environment.
...
- Desotech (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Desotech (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Desotech (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Desotech (KCSP) as the basis for migration or production guidance.
