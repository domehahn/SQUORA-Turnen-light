...
Review and operate Distr using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Distr code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Distr requires evidence-backed assessment.
...
- First-party source: https://github.com/distr-sh/distr
...
- [ ] Trace Distr inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Distr defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Distr least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Distr resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Distr release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Distr dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Distr identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Distr change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Distr documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Distr architecture and dependency summary with trust boundaries and persistent state.
- Include Distr validation commands or tests that are safe for the stated environment.
...
- Distr identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Distr failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Distr is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Distr as the basis for migration or production guidance.
