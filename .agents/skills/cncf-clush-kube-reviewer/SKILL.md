...
---
# Cncf Clush Kube Reviewer

## Purpose

Review and operate Clush Kube using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Clush Kube code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Clush Kube requires evidence-backed assessment.
...
- First-party source: https://www.clush.ai/en/clushkube
...
- [ ] Trace Clush Kube inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Clush Kube defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Clush Kube least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Clush Kube resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Clush Kube release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Clush Kube dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Clush Kube identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Clush Kube change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Clush Kube documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Clush Kube architecture and dependency summary with trust boundaries and persistent state.
- Include Clush Kube validation commands or tests that are safe for the stated environment.
...
- Clush Kube identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Clush Kube failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Clush Kube is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Clush Kube as the basis for migration or production guidance.
