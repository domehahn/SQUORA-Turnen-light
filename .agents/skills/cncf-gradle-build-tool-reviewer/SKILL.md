...
---
# Cncf Gradle Build Tool Reviewer

## Purpose

Review and operate Gradle Build Tool using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Gradle Build Tool code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Gradle Build Tool requires evidence-backed assessment.
...
- First-party source: https://github.com/gradle/gradle
...
- [ ] Trace Gradle Build Tool inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Gradle Build Tool defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Gradle Build Tool least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Gradle Build Tool resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Gradle Build Tool release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Gradle Build Tool dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Gradle Build Tool identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Gradle Build Tool change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Gradle Build Tool documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Gradle Build Tool architecture and dependency summary with trust boundaries and persistent state.
- Include Gradle Build Tool validation commands or tests that are safe for the stated environment.
...
- Gradle Build Tool identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Gradle Build Tool failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Gradle Build Tool is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Gradle Build Tool as the basis for migration or production guidance.
