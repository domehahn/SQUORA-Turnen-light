...
---
# Cncf Bronto Reviewer

## Purpose

Review and operate Bronto using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Bronto code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Bronto requires evidence-backed assessment.
...
- First-party source: https://bronto.io/
...
- [ ] Trace Bronto inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Bronto defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Bronto least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Bronto resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Bronto release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Bronto dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Bronto identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Bronto change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Bronto documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Bronto architecture and dependency summary with trust boundaries and persistent state.
- Include Bronto validation commands or tests that are safe for the stated environment.
...
- Bronto identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Bronto failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Bronto is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Bronto as the basis for migration or production guidance.
