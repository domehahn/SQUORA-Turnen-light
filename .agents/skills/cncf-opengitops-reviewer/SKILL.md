...
---
# Cncf Opengitops Reviewer

## Purpose

Review and operate OpenGitOps using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenGitOps code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenGitOps requires evidence-backed assessment.
...
- First-party source: https://github.com/open-gitops/project
...
- [ ] Trace OpenGitOps inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenGitOps defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenGitOps least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenGitOps resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenGitOps release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenGitOps dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenGitOps identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenGitOps change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenGitOps documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenGitOps architecture and dependency summary with trust boundaries and persistent state.
- Include OpenGitOps validation commands or tests that are safe for the stated environment.
...
- OpenGitOps identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenGitOps failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenGitOps is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenGitOps as the basis for migration or production guidance.
