---
name: "cncf-rafay-kcsp-reviewer"
description: "Review Rafay (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Rafay Systems delivers a turnkey offering along with white-glove support to assist enterprise customers automate Kubernetes cluster management and application operations at scale across public clouds, data centers and Edge environments."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Rafay Kcsp Reviewer

## Purpose

Review and operate Rafay (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Rafay (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Rafay (KCSP) requires evidence-backed assessment.
...
- [ ] Trace Rafay (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Rafay (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Rafay (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Rafay (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Rafay (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Rafay (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Rafay (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Rafay (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Rafay (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Rafay (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Rafay (KCSP) validation commands or tests that are safe for the stated environment.
...
- Rafay (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Rafay (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Rafay (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Rafay (KCSP) as the basis for migration or production guidance.
