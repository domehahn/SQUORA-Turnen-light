---
name: "cncf-dashbird-reviewer"
description: "Review Dashbird using its official documentation and repository in the Serverless / Tools category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Dashbird Reviewer

## Purpose

Review and operate Dashbird using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Dashbird code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Dashbird requires evidence-backed assessment.
...
- First-party source: https://dashbird.io/
...
- [ ] Trace Dashbird inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Dashbird defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Dashbird least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Dashbird resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Dashbird release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Dashbird dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Dashbird identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Dashbird change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Dashbird documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Dashbird architecture and dependency summary with trust boundaries and persistent state.
- Include Dashbird validation commands or tests that are safe for the stated environment.
...
- Dashbird identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Dashbird failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Dashbird is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Dashbird as the basis for migration or production guidance.
