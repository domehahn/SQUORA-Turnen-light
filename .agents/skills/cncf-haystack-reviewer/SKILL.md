...
---
# Cncf Haystack Reviewer

## Purpose

Review and operate Haystack using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Haystack code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Haystack requires evidence-backed assessment.
...
- First-party source: https://github.com/deepset-ai/haystack
...
- [ ] Trace Haystack inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Haystack defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Haystack least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Haystack resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Haystack release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Haystack dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Haystack identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Haystack change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Haystack documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Haystack architecture and dependency summary with trust boundaries and persistent state.
- Include Haystack validation commands or tests that are safe for the stated environment.
...
- Haystack identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Haystack failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Haystack is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Haystack as the basis for migration or production guidance.
