---
name: "cncf-clush-kcsp-reviewer"
description: "Review Clush (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Unified Management of Kubernetes and Cloud Native Environments, Accelerating Enterprise AI through Expertise in GPUOps and MLOps Platforms."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Clush Kcsp Reviewer

## Purpose

Review and operate Clush (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Clush (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Clush (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.clush.ai/en/k8s
...
- [ ] Trace Clush (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Clush (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Clush (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Clush (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Clush (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Clush (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Clush (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Clush (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Clush (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Clush (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Clush (KCSP) validation commands or tests that are safe for the stated environment.
...
- Clush (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Clush (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Clush (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Clush (KCSP) as the basis for migration or production guidance.
