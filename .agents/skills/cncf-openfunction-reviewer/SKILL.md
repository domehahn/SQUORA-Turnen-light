---
name: "cncf-openfunction-reviewer"
description: "Review OpenFunction using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Openfunction Reviewer

## Purpose

Review and operate OpenFunction using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenFunction code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenFunction requires evidence-backed assessment.
...
- First-party source: https://github.com/OpenFunction/OpenFunction
...
- [ ] Trace OpenFunction inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenFunction defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenFunction least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenFunction resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenFunction release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenFunction dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenFunction identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenFunction change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenFunction documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenFunction architecture and dependency summary with trust boundaries and persistent state.
- Include OpenFunction validation commands or tests that are safe for the stated environment.
...
- OpenFunction identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenFunction failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenFunction is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenFunction as the basis for migration or production guidance.
