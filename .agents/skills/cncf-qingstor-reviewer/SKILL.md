---
name: "cncf-qingstor-reviewer"
description: "Review QingStor using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Qingstor Reviewer

## Purpose

Review and operate QingStor using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- QingStor code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving QingStor requires evidence-backed assessment.
...
- First-party source: https://www.qingcloud.com/products/qingstor/
...
- [ ] Trace QingStor inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check QingStor defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check QingStor least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check QingStor resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check QingStor release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check QingStor dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If QingStor identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a QingStor change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- QingStor documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a QingStor architecture and dependency summary with trust boundaries and persistent state.
- Include QingStor validation commands or tests that are safe for the stated environment.
...
- QingStor identity, version, configuration, dependencies, and deployment assumptions are explicit.
- QingStor failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that QingStor is secure, supported, compatible, or suitable.
- Using an unpinned latest version of QingStor as the basis for migration or production guidance.
