---
name: "cncf-qumulo-reviewer"
description: "Review Qumulo using its official documentation and repository in the Runtime / Cloud Native Storage category. Qumulo provides file storage that delivers billion file count, petabyte scale, gigabyte throughput in the public cloud and AWS GovCloud(US) with all-flash and hybrid SSD/HDD options."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Qumulo Reviewer

## Purpose

Review and operate Qumulo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Qumulo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Qumulo requires evidence-backed assessment.
...
- First-party source: https://www.qumulo.com/
...
- [ ] Trace Qumulo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Qumulo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Qumulo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Qumulo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Qumulo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Qumulo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Qumulo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Qumulo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Qumulo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Qumulo architecture and dependency summary with trust boundaries and persistent state.
- Include Qumulo validation commands or tests that are safe for the stated environment.
...
- Qumulo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Qumulo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Qumulo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Qumulo as the basis for migration or production guidance.
