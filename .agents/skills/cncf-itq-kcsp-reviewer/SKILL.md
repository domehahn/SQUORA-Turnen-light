---
name: "cncf-itq-kcsp-reviewer"
description: "Review ITQ (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. ITQ, a European consulting company, specializes in deploying and managing Kubernetes for organizations of all sizes. Our services are designed to help you navigate the complexities of Kubernetes, allowing you to focus on delivering value."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Itq Kcsp Reviewer

## Purpose

Review and operate ITQ (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ITQ (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ITQ (KCSP) requires evidence-backed assessment.
...
- First-party source: https://itq.eu/technology/kubernetes/
...
- [ ] Trace ITQ (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ITQ (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ITQ (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ITQ (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ITQ (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ITQ (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ITQ (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ITQ (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ITQ (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ITQ (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include ITQ (KCSP) validation commands or tests that are safe for the stated environment.
...
- ITQ (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ITQ (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ITQ (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ITQ (KCSP) as the basis for migration or production guidance.
