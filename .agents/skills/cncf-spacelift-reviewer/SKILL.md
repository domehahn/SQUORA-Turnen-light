...
---
# Cncf Spacelift Reviewer

## Purpose

Review and operate Spacelift using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Spacelift code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Spacelift requires evidence-backed assessment.
...
- First-party source: https://spacelift.io
...
- [ ] Trace Spacelift inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Spacelift defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Spacelift least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Spacelift resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Spacelift release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Spacelift dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Spacelift identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Spacelift change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Spacelift documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Spacelift architecture and dependency summary with trust boundaries and persistent state.
- Include Spacelift validation commands or tests that are safe for the stated environment.
...
- Spacelift identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Spacelift failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Spacelift is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Spacelift as the basis for migration or production guidance.
