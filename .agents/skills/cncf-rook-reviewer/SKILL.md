---
name: "cncf-rook-reviewer"
description: "Review Rook using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Rook using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Rook code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Rook requires evidence-backed assessment.
...
- First-party source: https://github.com/rook/rook
...
- [ ] Trace Rook inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Rook defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Rook least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Rook resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Rook release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Rook dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Rook identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Rook change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Rook documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Rook architecture and dependency summary with trust boundaries and persistent state.
- Include Rook validation commands or tests that are safe for the stated environment.
...
- Rook identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Rook failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Rook is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Rook as the basis for migration or production guidance.
