...
---
# Cncf Iomesh Reviewer

## Purpose

Review and operate IOMesh using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- IOMesh code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving IOMesh requires evidence-backed assessment.
...
- First-party source: https://iomesh.com/
...
- [ ] Trace IOMesh inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check IOMesh defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check IOMesh least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check IOMesh resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check IOMesh release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check IOMesh dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If IOMesh identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a IOMesh change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- IOMesh documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a IOMesh architecture and dependency summary with trust boundaries and persistent state.
- Include IOMesh validation commands or tests that are safe for the stated environment.
...
- IOMesh identity, version, configuration, dependencies, and deployment assumptions are explicit.
- IOMesh failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that IOMesh is secure, supported, compatible, or suitable.
- Using an unpinned latest version of IOMesh as the basis for migration or production guidance.
