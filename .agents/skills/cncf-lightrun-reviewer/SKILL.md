---
name: "cncf-lightrun-reviewer"
description: "Review Lightrun using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Lightrun Reviewer

## Purpose

Review and operate Lightrun using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Lightrun code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Lightrun requires evidence-backed assessment.
...
- First-party source: https://www.lightrun.com/
...
- [ ] Trace Lightrun inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Lightrun defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Lightrun least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Lightrun resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Lightrun release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Lightrun dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Lightrun identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Lightrun change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Lightrun documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Lightrun architecture and dependency summary with trust boundaries and persistent state.
- Include Lightrun validation commands or tests that are safe for the stated environment.
...
- Lightrun identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Lightrun failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Lightrun is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Lightrun as the basis for migration or production guidance.
