---
name: "cncf-aqua-reviewer"
description: "Review Aqua using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Aqua using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Aqua code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Aqua requires evidence-backed assessment.
...
- [ ] Trace Aqua inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Aqua defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Aqua least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Aqua resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Aqua release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Aqua dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Aqua identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Aqua change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Aqua documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Aqua architecture and dependency summary with trust boundaries and persistent state.
- Include Aqua validation commands or tests that are safe for the stated environment.
...
- Aqua identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Aqua failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Aqua is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Aqua as the basis for migration or production guidance.
