...
---
# Cncf Micrometer Reviewer

## Purpose

Review and operate Micrometer using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Micrometer code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Micrometer requires evidence-backed assessment.
...
- First-party source: https://github.com/micrometer-metrics/micrometer
...
- [ ] Trace Micrometer inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Micrometer defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Micrometer least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Micrometer resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Micrometer release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Micrometer dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Micrometer identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Micrometer change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Micrometer documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Micrometer architecture and dependency summary with trust boundaries and persistent state.
- Include Micrometer validation commands or tests that are safe for the stated environment.
...
- Micrometer identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Micrometer failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Micrometer is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Micrometer as the basis for migration or production guidance.
