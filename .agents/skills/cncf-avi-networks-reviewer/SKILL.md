---
name: "cncf-avi-networks-reviewer"
description: "Review Avi Networks using its official documentation and repository in the Orchestration & Management / Service Proxy category. Avi Networks drives automation and intelligence across multi-cloud environments with intent-based application services, including LB and WAF"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Avi Networks Reviewer

## Purpose

Review and operate Avi Networks using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Avi Networks code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Avi Networks requires evidence-backed assessment.
...
- First-party source: https://avinetworks.com/
...
- [ ] Trace Avi Networks inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Avi Networks defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Avi Networks least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Avi Networks resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Avi Networks release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Avi Networks dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Avi Networks identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Avi Networks change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Avi Networks documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Avi Networks architecture and dependency summary with trust boundaries and persistent state.
- Include Avi Networks validation commands or tests that are safe for the stated environment.
...
- Avi Networks identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Avi Networks failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Avi Networks is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Avi Networks as the basis for migration or production guidance.
