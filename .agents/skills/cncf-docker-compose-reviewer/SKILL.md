---
name: "cncf-docker-compose-reviewer"
description: "Review Docker Compose using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Docker Compose Reviewer

## Purpose

Review and operate Docker Compose using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Docker Compose code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Docker Compose requires evidence-backed assessment.
...
- First-party source: https://github.com/docker/compose
...
- [ ] Trace Docker Compose inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Docker Compose defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Docker Compose least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Docker Compose resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Docker Compose release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Docker Compose dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Docker Compose identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Docker Compose change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Docker Compose documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Docker Compose architecture and dependency summary with trust boundaries and persistent state.
- Include Docker Compose validation commands or tests that are safe for the stated environment.
...
- Docker Compose identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Docker Compose failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Docker Compose is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Docker Compose as the basis for migration or production guidance.
