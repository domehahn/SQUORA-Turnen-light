---
name: "cncf-okestro-viola-reviewer"
description: "Review OKESTRO VIOLA using its official documentation and repository in the Platform / Certified Kubernetes - Hosted category. VIOLA is a managed Kubernetes service that allows you to easily deploy a Kubernetes cluster in Okestro."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Okestro Viola Reviewer

## Purpose

Review and operate OKESTRO VIOLA using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OKESTRO VIOLA code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OKESTRO VIOLA requires evidence-backed assessment.
...
- First-party source: https://www.okestro.com/product/viola.php
...
- [ ] Trace OKESTRO VIOLA inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OKESTRO VIOLA defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OKESTRO VIOLA least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OKESTRO VIOLA resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OKESTRO VIOLA release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OKESTRO VIOLA dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OKESTRO VIOLA identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OKESTRO VIOLA change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OKESTRO VIOLA documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OKESTRO VIOLA architecture and dependency summary with trust boundaries and persistent state.
- Include OKESTRO VIOLA validation commands or tests that are safe for the stated environment.
...
- OKESTRO VIOLA identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OKESTRO VIOLA failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OKESTRO VIOLA is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OKESTRO VIOLA as the basis for migration or production guidance.
