---
name: "cncf-epsagon-reviewer"
description: "Review Epsagon using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Epsagon Reviewer

## Purpose

Review and operate Epsagon using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Epsagon code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Epsagon requires evidence-backed assessment.
...
- First-party source: https://github.com/epsagon
...
- [ ] Trace Epsagon inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Epsagon defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Epsagon least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Epsagon resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Epsagon release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Epsagon dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Epsagon identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Epsagon change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Epsagon documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Epsagon architecture and dependency summary with trust boundaries and persistent state.
- Include Epsagon validation commands or tests that are safe for the stated environment.
...
- Epsagon identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Epsagon failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Epsagon is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Epsagon as the basis for migration or production guidance.
