---
name: "cncf-falco-reviewer"
description: "Review Falco using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Falco using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Falco code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Falco requires evidence-backed assessment.
...
- First-party source: https://github.com/falcosecurity/falco
...
- [ ] Trace Falco inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Falco defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Falco least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Falco resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Falco release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Falco dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Falco identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Falco change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Falco documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Falco architecture and dependency summary with trust boundaries and persistent state.
- Include Falco validation commands or tests that are safe for the stated environment.
...
- Falco identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Falco failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Falco is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Falco as the basis for migration or production guidance.
