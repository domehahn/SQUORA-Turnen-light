---
name: "cncf-mirantis-kubernetes-engine-reviewer"
description: "Review Mirantis Kubernetes Engine using its official documentation and repository in the Platform / Certified Kubernetes - Distribution category. Mirantis Kubernetes Engine Kubernetes distribution."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Mirantis Kubernetes Engine Reviewer

## Purpose

Review and operate Mirantis Kubernetes Engine using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Mirantis Kubernetes Engine code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Mirantis Kubernetes Engine requires evidence-backed assessment.
...
- [ ] Trace Mirantis Kubernetes Engine inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Mirantis Kubernetes Engine defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Mirantis Kubernetes Engine least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Mirantis Kubernetes Engine resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Mirantis Kubernetes Engine release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Mirantis Kubernetes Engine dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Mirantis Kubernetes Engine identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Mirantis Kubernetes Engine change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Mirantis Kubernetes Engine documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Mirantis Kubernetes Engine architecture and dependency summary with trust boundaries and persistent state.
- Include Mirantis Kubernetes Engine validation commands or tests that are safe for the stated environment.
...
- Mirantis Kubernetes Engine identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Mirantis Kubernetes Engine failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Mirantis Kubernetes Engine is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Mirantis Kubernetes Engine as the basis for migration or production guidance.
