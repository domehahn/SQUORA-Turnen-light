---
name: "cncf-agones-reviewer"
description: "Review Agones using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. Agones is a library for hosting, running, and scaling dedicated game servers on Kubernetes."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Agones Reviewer

## Purpose

Review and operate Agones using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Agones code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Agones requires evidence-backed assessment.
...
- First-party source: https://github.com/agones-dev/agones
...
- [ ] Trace Agones inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Agones defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Agones least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Agones resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Agones release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Agones dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Agones identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Agones change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Agones documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Agones architecture and dependency summary with trust boundaries and persistent state.
- Include Agones validation commands or tests that are safe for the stated environment.
...
- Agones identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Agones failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Agones is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Agones as the basis for migration or production guidance.
