---
name: "cncf-logiq-reviewer"
description: "Review Logiq using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Logiq using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Logiq code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Logiq requires evidence-backed assessment.
...
- First-party source: https://logiq.ai/
...
- [ ] Trace Logiq inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Logiq defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Logiq least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Logiq resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Logiq release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Logiq dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Logiq identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Logiq change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Logiq documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Logiq architecture and dependency summary with trust boundaries and persistent state.
- Include Logiq validation commands or tests that are safe for the stated environment.
...
- Logiq identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Logiq failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Logiq is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Logiq as the basis for migration or production guidance.
