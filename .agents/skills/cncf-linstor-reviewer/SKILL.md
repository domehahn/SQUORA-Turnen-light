---
name: "cncf-linstor-reviewer"
description: "Review LINSTOR using its official documentation and repository in the Runtime / Cloud Native Storage category. High Performance Software-Defined Block Storage for container, cloud and virtualisation. Fully integrated with Docker, Kubernetes, Openstack, Proxmox etc."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Linstor Reviewer

## Purpose

Review and operate LINSTOR using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- LINSTOR code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving LINSTOR requires evidence-backed assessment.
...
- First-party source: https://github.com/LINBIT/linstor-server
...
- [ ] Trace LINSTOR inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check LINSTOR defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check LINSTOR least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check LINSTOR resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check LINSTOR release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check LINSTOR dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If LINSTOR identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a LINSTOR change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- LINSTOR documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a LINSTOR architecture and dependency summary with trust boundaries and persistent state.
- Include LINSTOR validation commands or tests that are safe for the stated environment.
...
- LINSTOR identity, version, configuration, dependencies, and deployment assumptions are explicit.
- LINSTOR failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that LINSTOR is secure, supported, compatible, or suitable.
- Using an unpinned latest version of LINSTOR as the basis for migration or production guidance.
