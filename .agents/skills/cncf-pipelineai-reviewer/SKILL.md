---
name: "cncf-pipelineai-reviewer"
description: "Review PipelineAI using its official documentation and repository in the Serverless / Installable Platform category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Pipelineai Reviewer

## Purpose

Review and operate PipelineAI using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- PipelineAI code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving PipelineAI requires evidence-backed assessment.
...
- First-party source: https://github.com/pipelineai/pipeline
...
- [ ] Trace PipelineAI inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check PipelineAI defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check PipelineAI least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check PipelineAI resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check PipelineAI release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check PipelineAI dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If PipelineAI identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a PipelineAI change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- PipelineAI documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a PipelineAI architecture and dependency summary with trust boundaries and persistent state.
- Include PipelineAI validation commands or tests that are safe for the stated environment.
...
- PipelineAI identity, version, configuration, dependencies, and deployment assumptions are explicit.
- PipelineAI failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that PipelineAI is secure, supported, compatible, or suitable.
- Using an unpinned latest version of PipelineAI as the basis for migration or production guidance.
