...
Review and operate yoke using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- yoke code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving yoke requires evidence-backed assessment.
...
- First-party source: https://github.com/yokecd/yoke
...
- [ ] Trace yoke inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check yoke defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check yoke least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check yoke resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check yoke release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check yoke dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If yoke identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a yoke change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- yoke documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a yoke architecture and dependency summary with trust boundaries and persistent state.
- Include yoke validation commands or tests that are safe for the stated environment.
...
- yoke identity, version, configuration, dependencies, and deployment assumptions are explicit.
- yoke failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that yoke is secure, supported, compatible, or suitable.
- Using an unpinned latest version of yoke as the basis for migration or production guidance.
