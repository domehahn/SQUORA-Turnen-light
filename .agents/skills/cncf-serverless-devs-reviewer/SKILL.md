---
name: "cncf-serverless-devs-reviewer"
description: "Review Serverless Devs using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. Serverless Devs developer tool ( Serverless Devs 开发者工具 )"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Serverless Devs Reviewer

## Purpose

Review and operate Serverless Devs using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Serverless Devs code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Serverless Devs requires evidence-backed assessment.
...
- First-party source: https://github.com/serverless-devs/serverless-devs
...
- [ ] Trace Serverless Devs inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Serverless Devs defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Serverless Devs least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Serverless Devs resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Serverless Devs release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Serverless Devs dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Serverless Devs identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Serverless Devs change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Serverless Devs documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Serverless Devs architecture and dependency summary with trust boundaries and persistent state.
- Include Serverless Devs validation commands or tests that are safe for the stated environment.
...
- Serverless Devs identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Serverless Devs failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Serverless Devs is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Serverless Devs as the basis for migration or production guidance.
