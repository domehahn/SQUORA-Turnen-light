---
name: "cncf-3-shake-kcsp-reviewer"
description: "Review 3-Shake (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf 3 Shake Kcsp Reviewer

## Purpose

Review and operate 3-Shake (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- 3-Shake (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving 3-Shake (KCSP) requires evidence-backed assessment.
...
- First-party source: https://3-shake.com/
...
- [ ] Trace 3-Shake (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check 3-Shake (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check 3-Shake (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check 3-Shake (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check 3-Shake (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check 3-Shake (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If 3-Shake (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a 3-Shake (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- 3-Shake (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a 3-Shake (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include 3-Shake (KCSP) validation commands or tests that are safe for the stated environment.
...
- 3-Shake (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- 3-Shake (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that 3-Shake (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of 3-Shake (KCSP) as the basis for migration or production guidance.
