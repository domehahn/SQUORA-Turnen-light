---
name: "cncf-yld-kcsp-reviewer"
description: "Review YLD (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. We modernise both practices and software. Applying expertise in a range of CNCF technologies and an inherent culture of innovation."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Yld Kcsp Reviewer

## Purpose

Review and operate YLD (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- YLD (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving YLD (KCSP) requires evidence-backed assessment.
...
- [ ] Trace YLD (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check YLD (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check YLD (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check YLD (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check YLD (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check YLD (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If YLD (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a YLD (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- YLD (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a YLD (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include YLD (KCSP) validation commands or tests that are safe for the stated environment.
...
- YLD (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- YLD (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that YLD (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of YLD (KCSP) as the basis for migration or production guidance.
