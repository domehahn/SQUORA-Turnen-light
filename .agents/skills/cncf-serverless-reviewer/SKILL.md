---
name: "cncf-serverless-reviewer"
description: "Review Serverless using its official documentation and repository in the Serverless / Framework category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Serverless Reviewer

## Purpose

Review and operate Serverless using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Serverless code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Serverless requires evidence-backed assessment.
...
- First-party source: https://github.com/serverless/serverless
...
- [ ] Trace Serverless inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Serverless defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Serverless least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Serverless resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Serverless release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Serverless dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Serverless identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Serverless change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Serverless documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Serverless architecture and dependency summary with trust boundaries and persistent state.
- Include Serverless validation commands or tests that are safe for the stated environment.
...
- Serverless identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Serverless failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Serverless is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Serverless as the basis for migration or production guidance.
