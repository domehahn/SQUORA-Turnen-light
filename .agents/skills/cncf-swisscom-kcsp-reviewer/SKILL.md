---
name: "cncf-swisscom-kcsp-reviewer"
description: "Review Swisscom (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Swisscom’s experienced cloud native experts support Swiss customers end to end on their journey with the Kubernetes ecosystem."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Swisscom Kcsp Reviewer

## Purpose

Review and operate Swisscom (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Swisscom (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Swisscom (KCSP) requires evidence-backed assessment.
...
- First-party source: https://swisscom.com/container
...
- [ ] Trace Swisscom (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Swisscom (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Swisscom (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Swisscom (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Swisscom (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Swisscom (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Swisscom (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Swisscom (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Swisscom (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Swisscom (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Swisscom (KCSP) validation commands or tests that are safe for the stated environment.
...
- Swisscom (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Swisscom (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Swisscom (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Swisscom (KCSP) as the basis for migration or production guidance.
