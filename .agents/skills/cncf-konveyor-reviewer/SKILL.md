---
name: "cncf-konveyor-reviewer"
description: "Review Konveyor using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category. Documentation for Konveyor Community"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Konveyor Reviewer

## Purpose

Review and operate Konveyor using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Konveyor code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Konveyor requires evidence-backed assessment.
...
- First-party source: https://github.com/konveyor/operator
...
- [ ] Trace Konveyor inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Konveyor defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Konveyor least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Konveyor resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Konveyor release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Konveyor dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Konveyor identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Konveyor change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Konveyor documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Konveyor architecture and dependency summary with trust boundaries and persistent state.
- Include Konveyor validation commands or tests that are safe for the stated environment.
...
- Konveyor identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Konveyor failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Konveyor is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Konveyor as the basis for migration or production guidance.
