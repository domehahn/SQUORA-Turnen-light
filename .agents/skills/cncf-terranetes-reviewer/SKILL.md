---
name: "cncf-terranetes-reviewer"
description: "Review Terranetes using its official documentation and repository in the Provisioning / Automation & Configuration category. The Appvia Terranetes Controller manages the life-cycle of Terraform and OpenTofu resources defined and built inside Kubernetes. This allows teams running workloads inside the cluster to self-serve application dependencies and reuse the wealth of Terraform modules already written."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Terranetes Reviewer

## Purpose

Review and operate Terranetes using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Terranetes code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Terranetes requires evidence-backed assessment.
...
- First-party source: https://github.com/appvia/terranetes-controller
...
- [ ] Trace Terranetes inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Terranetes defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Terranetes least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Terranetes resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Terranetes release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Terranetes dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Terranetes identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Terranetes change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Terranetes documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Terranetes architecture and dependency summary with trust boundaries and persistent state.
- Include Terranetes validation commands or tests that are safe for the stated environment.
...
- Terranetes identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Terranetes failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Terranetes is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Terranetes as the basis for migration or production guidance.
