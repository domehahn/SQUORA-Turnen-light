---
name: "cncf-hasura-graphql-engine-reviewer"
description: "Review Hasura GraphQL Engine using its official documentation and repository in the Serverless / Tools category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Hasura Graphql Engine Reviewer

## Purpose

Review and operate Hasura GraphQL Engine using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Hasura GraphQL Engine code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Hasura GraphQL Engine requires evidence-backed assessment.
...
- First-party source: https://github.com/hasura/graphql-engine
...
- [ ] Trace Hasura GraphQL Engine inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Hasura GraphQL Engine defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Hasura GraphQL Engine least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Hasura GraphQL Engine resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Hasura GraphQL Engine release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Hasura GraphQL Engine dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Hasura GraphQL Engine identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Hasura GraphQL Engine change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Hasura GraphQL Engine documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Hasura GraphQL Engine architecture and dependency summary with trust boundaries and persistent state.
- Include Hasura GraphQL Engine validation commands or tests that are safe for the stated environment.
...
- Hasura GraphQL Engine identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Hasura GraphQL Engine failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Hasura GraphQL Engine is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Hasura GraphQL Engine as the basis for migration or production guidance.
