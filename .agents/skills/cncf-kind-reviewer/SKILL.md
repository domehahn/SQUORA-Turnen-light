---
name: "cncf-kind-reviewer"
description: "Review kind using its official documentation and repository in the Platform / Certified Kubernetes - Installer category. kind creates local multi-node Kubernetes clusters using Docker container nodes."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate kind using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- kind code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving kind requires evidence-backed assessment.
...
- First-party source: https://github.com/kubernetes-sigs/kind
...
- [ ] Trace kind inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check kind defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check kind least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check kind resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check kind release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check kind dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If kind identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a kind change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- kind documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a kind architecture and dependency summary with trust boundaries and persistent state.
- Include kind validation commands or tests that are safe for the stated environment.
...
- kind identity, version, configuration, dependencies, and deployment assumptions are explicit.
- kind failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that kind is secure, supported, compatible, or suitable.
- Using an unpinned latest version of kind as the basis for migration or production guidance.
