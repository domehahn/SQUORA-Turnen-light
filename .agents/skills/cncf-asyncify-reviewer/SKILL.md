---
name: "cncf-asyncify-reviewer"
description: "Review Asyncify using its official documentation and repository in the Wasm / Tooling category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Asyncify Reviewer

## Purpose

Review and operate Asyncify using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Asyncify code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Asyncify requires evidence-backed assessment.
...
- First-party source: https://github.com/GoogleChromeLabs/asyncify
...
- [ ] Trace Asyncify inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Asyncify defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Asyncify least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Asyncify resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Asyncify release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Asyncify dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Asyncify identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Asyncify change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Asyncify documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Asyncify architecture and dependency summary with trust boundaries and persistent state.
- Include Asyncify validation commands or tests that are safe for the stated environment.
...
- Asyncify identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Asyncify failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Asyncify is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Asyncify as the basis for migration or production guidance.
