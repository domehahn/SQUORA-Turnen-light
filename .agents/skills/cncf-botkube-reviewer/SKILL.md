---
name: "cncf-botkube-reviewer"
description: "Review Botkube using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Botkube Reviewer

## Purpose

Review and operate Botkube using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Botkube code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Botkube requires evidence-backed assessment.
...
- First-party source: https://github.com/kubeshop/botkube
...
- [ ] Trace Botkube inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Botkube defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Botkube least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Botkube resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Botkube release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Botkube dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Botkube identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Botkube change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Botkube documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Botkube architecture and dependency summary with trust boundaries and persistent state.
- Include Botkube validation commands or tests that are safe for the stated environment.
...
- Botkube identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Botkube failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Botkube is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Botkube as the basis for migration or production guidance.
