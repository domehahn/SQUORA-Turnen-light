...
---
# Cncf Cyso Managed Kubernetes Reviewer

## Purpose

Review and operate CYSO Managed Kubernetes using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CYSO Managed Kubernetes code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CYSO Managed Kubernetes requires evidence-backed assessment.
...
- First-party source: https://cyso.cloud/managed-kubernetes/
...
- [ ] Trace CYSO Managed Kubernetes inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CYSO Managed Kubernetes defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CYSO Managed Kubernetes least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CYSO Managed Kubernetes resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CYSO Managed Kubernetes release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CYSO Managed Kubernetes dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CYSO Managed Kubernetes identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CYSO Managed Kubernetes change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CYSO Managed Kubernetes documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CYSO Managed Kubernetes architecture and dependency summary with trust boundaries and persistent state.
- Include CYSO Managed Kubernetes validation commands or tests that are safe for the stated environment.
...
- CYSO Managed Kubernetes identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CYSO Managed Kubernetes failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CYSO Managed Kubernetes is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CYSO Managed Kubernetes as the basis for migration or production guidance.
