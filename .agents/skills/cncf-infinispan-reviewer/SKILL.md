---
name: "cncf-infinispan-reviewer"
description: "Review Infinispan using its official documentation and repository in the App Definition and Development / Database category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Infinispan Reviewer

## Purpose

Review and operate Infinispan using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Infinispan code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Infinispan requires evidence-backed assessment.
...
- First-party source: https://github.com/infinispan/infinispan
...
- [ ] Trace Infinispan inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Infinispan defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Infinispan least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Infinispan resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Infinispan release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Infinispan dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Infinispan identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Infinispan change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Infinispan documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Infinispan architecture and dependency summary with trust boundaries and persistent state.
- Include Infinispan validation commands or tests that are safe for the stated environment.
...
- Infinispan identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Infinispan failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Infinispan is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Infinispan as the basis for migration or production guidance.
