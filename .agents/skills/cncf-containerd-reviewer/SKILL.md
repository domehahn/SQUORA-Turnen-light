---
name: "cncf-containerd-reviewer"
description: "Review containerd using its official documentation and repository in the Runtime / Container Runtime category. An open and reliable container runtime"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Containerd Reviewer

## Purpose

Review and operate containerd using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- containerd code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving containerd requires evidence-backed assessment.
...
- [ ] Trace containerd inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check containerd defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check containerd least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check containerd resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check containerd release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check containerd dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If containerd identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a containerd change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- containerd documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a containerd architecture and dependency summary with trust boundaries and persistent state.
- Include containerd validation commands or tests that are safe for the stated environment.
...
- containerd identity, version, configuration, dependencies, and deployment assumptions are explicit.
- containerd failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that containerd is secure, supported, compatible, or suitable.
- Using an unpinned latest version of containerd as the basis for migration or production guidance.
