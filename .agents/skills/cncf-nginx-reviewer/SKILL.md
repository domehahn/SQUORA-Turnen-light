---
name: "cncf-nginx-reviewer"
description: "Review NGINX using its official documentation and repository in the Orchestration & Management / Service Proxy category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate NGINX using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- NGINX code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving NGINX requires evidence-backed assessment.
...
- [ ] Trace NGINX inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check NGINX defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check NGINX least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check NGINX resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check NGINX release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check NGINX dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If NGINX identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a NGINX change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- NGINX documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a NGINX architecture and dependency summary with trust boundaries and persistent state.
- Include NGINX validation commands or tests that are safe for the stated environment.
...
- NGINX identity, version, configuration, dependencies, and deployment assumptions are explicit.
- NGINX failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that NGINX is secure, supported, compatible, or suitable.
- Using an unpinned latest version of NGINX as the basis for migration or production guidance.
