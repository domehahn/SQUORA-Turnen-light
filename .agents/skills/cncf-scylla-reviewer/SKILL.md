---
name: "cncf-scylla-reviewer"
description: "Review Scylla using its official documentation and repository in the App Definition and Development / Database category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Scylla Reviewer

## Purpose

Review and operate Scylla using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Scylla code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Scylla requires evidence-backed assessment.
...
- [ ] Trace Scylla inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Scylla defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Scylla least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Scylla resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Scylla release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Scylla dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Scylla identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Scylla change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Scylla documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Scylla architecture and dependency summary with trust boundaries and persistent state.
- Include Scylla validation commands or tests that are safe for the stated environment.
...
- Scylla identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Scylla failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Scylla is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Scylla as the basis for migration or production guidance.
