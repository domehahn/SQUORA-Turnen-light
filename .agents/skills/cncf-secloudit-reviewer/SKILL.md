...
---
# Cncf Secloudit Reviewer

## Purpose

Review and operate SECloudit using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SECloudit code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SECloudit requires evidence-backed assessment.
...
- First-party source: https://www.innogrid.com/secloudit
...
- [ ] Trace SECloudit inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SECloudit defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SECloudit least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SECloudit resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SECloudit release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SECloudit dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SECloudit identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SECloudit change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SECloudit documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SECloudit architecture and dependency summary with trust boundaries and persistent state.
- Include SECloudit validation commands or tests that are safe for the stated environment.
...
- SECloudit identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SECloudit failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SECloudit is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SECloudit as the basis for migration or production guidance.
