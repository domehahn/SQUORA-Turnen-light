---
name: "cncf-lagoon-reviewer"
description: "Review Lagoon using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category. Build and Deploy System for OpenShift & Kubernetes"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Lagoon Reviewer

## Purpose

Review and operate Lagoon using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Lagoon code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Lagoon requires evidence-backed assessment.
...
- First-party source: https://github.com/uselagoon/lagoon
...
- [ ] Trace Lagoon inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Lagoon defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Lagoon least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Lagoon resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Lagoon release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Lagoon dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Lagoon identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Lagoon change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Lagoon documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Lagoon architecture and dependency summary with trust boundaries and persistent state.
- Include Lagoon validation commands or tests that are safe for the stated environment.
...
- Lagoon identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Lagoon failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Lagoon is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Lagoon as the basis for migration or production guidance.
