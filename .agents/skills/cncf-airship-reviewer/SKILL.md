---
name: "cncf-airship-reviewer"
description: "Review Airship using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Airship Reviewer

## Purpose

Review and operate Airship using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Airship code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Airship requires evidence-backed assessment.
...
- First-party source: https://github.com/airshipit/treasuremap
...
- [ ] Trace Airship inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Airship defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Airship least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Airship resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Airship release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Airship dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Airship identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Airship change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Airship documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Airship architecture and dependency summary with trust boundaries and persistent state.
- Include Airship validation commands or tests that are safe for the stated environment.
...
- Airship identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Airship failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Airship is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Airship as the basis for migration or production guidance.
