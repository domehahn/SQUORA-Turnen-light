---
name: "cncf-inngest-reviewer"
description: "Review Inngest using its official documentation and repository in the AI Agent / Workflow Orchestration category. The leading workflow orchestration platform. Run stateful step functions and AI workflows on serverless, servers, or the edge."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Inngest Reviewer

## Purpose

Review and operate Inngest using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Inngest code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Inngest requires evidence-backed assessment.
...
- First-party source: https://github.com/inngest/inngest
...
- [ ] Trace Inngest inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Inngest defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Inngest least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Inngest resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Inngest release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Inngest dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Inngest identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Inngest change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Inngest documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Inngest architecture and dependency summary with trust boundaries and persistent state.
- Include Inngest validation commands or tests that are safe for the stated environment.
...
- Inngest identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Inngest failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Inngest is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Inngest as the basis for migration or production guidance.
