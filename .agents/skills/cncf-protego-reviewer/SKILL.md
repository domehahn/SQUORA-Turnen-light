---
name: "cncf-protego-reviewer"
description: "Review Protego using its official documentation and repository in the Serverless / Security category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Protego Reviewer

## Purpose

Review and operate Protego using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Protego code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Protego requires evidence-backed assessment.
...
- [ ] Trace Protego inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Protego defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Protego least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Protego resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Protego release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Protego dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Protego identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Protego change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Protego documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Protego architecture and dependency summary with trust boundaries and persistent state.
- Include Protego validation commands or tests that are safe for the stated environment.
...
- Protego identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Protego failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Protego is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Protego as the basis for migration or production guidance.
