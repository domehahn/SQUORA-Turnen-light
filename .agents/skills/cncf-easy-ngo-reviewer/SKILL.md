---
name: "cncf-easy-ngo-reviewer"
description: "Review Easy-Ngo using its official documentation and repository in the Orchestration & Management / Remote Procedure Call category. easy-ngo is a fast, out-of-the-box microservice development framework based on the Go language."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Easy Ngo Reviewer

## Purpose

Review and operate Easy-Ngo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Easy-Ngo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Easy-Ngo requires evidence-backed assessment.
...
- First-party source: https://github.com/NetEase-Media/easy-ngo
...
- [ ] Trace Easy-Ngo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Easy-Ngo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Easy-Ngo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Easy-Ngo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Easy-Ngo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Easy-Ngo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Easy-Ngo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Easy-Ngo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Easy-Ngo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Easy-Ngo architecture and dependency summary with trust boundaries and persistent state.
- Include Easy-Ngo validation commands or tests that are safe for the stated environment.
...
- Easy-Ngo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Easy-Ngo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Easy-Ngo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Easy-Ngo as the basis for migration or production guidance.
