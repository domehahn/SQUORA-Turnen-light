---
name: "cncf-openio-reviewer"
description: "Review OpenIO using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Openio Reviewer

## Purpose

Review and operate OpenIO using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenIO code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenIO requires evidence-backed assessment.
...
- First-party source: https://github.com/open-io/oio-sds
...
- [ ] Trace OpenIO inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenIO defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenIO least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenIO resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenIO release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenIO dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenIO identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenIO change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenIO documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenIO architecture and dependency summary with trust boundaries and persistent state.
- Include OpenIO validation commands or tests that are safe for the stated environment.
...
- OpenIO identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenIO failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenIO is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenIO as the basis for migration or production guidance.
