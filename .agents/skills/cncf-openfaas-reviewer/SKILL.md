---
name: "cncf-openfaas-reviewer"
description: "Review OpenFaaS using its official documentation and repository in the Serverless / Installable Platform category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Openfaas Reviewer

## Purpose

Review and operate OpenFaaS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenFaaS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenFaaS requires evidence-backed assessment.
...
- First-party source: https://github.com/openfaas/faas
...
- [ ] Trace OpenFaaS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenFaaS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenFaaS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenFaaS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenFaaS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenFaaS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenFaaS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenFaaS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenFaaS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenFaaS architecture and dependency summary with trust boundaries and persistent state.
- Include OpenFaaS validation commands or tests that are safe for the stated environment.
...
- OpenFaaS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenFaaS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenFaaS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenFaaS as the basis for migration or production guidance.
