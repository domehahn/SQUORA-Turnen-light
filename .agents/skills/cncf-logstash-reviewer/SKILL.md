---
name: "cncf-logstash-reviewer"
description: "Review Logstash using its official documentation and repository in the Observability and Analysis / Observability category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Logstash Reviewer

## Purpose

Review and operate Logstash using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Logstash code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Logstash requires evidence-backed assessment.
...
- First-party source: https://github.com/elastic/logstash
...
- [ ] Trace Logstash inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Logstash defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Logstash least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Logstash resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Logstash release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Logstash dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Logstash identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Logstash change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Logstash documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Logstash architecture and dependency summary with trust boundaries and persistent state.
- Include Logstash validation commands or tests that are safe for the stated environment.
...
- Logstash identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Logstash failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Logstash is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Logstash as the basis for migration or production guidance.
