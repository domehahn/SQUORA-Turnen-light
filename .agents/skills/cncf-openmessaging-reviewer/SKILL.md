---
name: "cncf-openmessaging-reviewer"
description: "Review OpenMessaging using its official documentation and repository in the App Definition and Development / Streaming & Messaging category. Cloud-oriented, simple, flexible, vendor-neutral and language-independent standards for messaging"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Openmessaging Reviewer

## Purpose

Review and operate OpenMessaging using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenMessaging code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenMessaging requires evidence-backed assessment.
...
- First-party source: https://github.com/openmessaging/openmessaging-java
...
- [ ] Trace OpenMessaging inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenMessaging defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenMessaging least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenMessaging resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenMessaging release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenMessaging dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenMessaging identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenMessaging change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenMessaging documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenMessaging architecture and dependency summary with trust boundaries and persistent state.
- Include OpenMessaging validation commands or tests that are safe for the stated environment.
...
- OpenMessaging identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenMessaging failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenMessaging is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenMessaging as the basis for migration or production guidance.
