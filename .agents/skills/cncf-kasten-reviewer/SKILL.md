---
name: "cncf-kasten-reviewer"
description: "Review Kasten using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kasten Reviewer

## Purpose

Review and operate Kasten using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kasten code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kasten requires evidence-backed assessment.
...
- First-party source: https://kasten.io/
...
- [ ] Trace Kasten inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kasten defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kasten least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kasten resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kasten release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kasten dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kasten identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kasten change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kasten documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kasten architecture and dependency summary with trust boundaries and persistent state.
- Include Kasten validation commands or tests that are safe for the stated environment.
...
- Kasten identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kasten failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kasten is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kasten as the basis for migration or production guidance.
