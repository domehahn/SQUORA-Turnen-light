---
name: "cncf-terrascan-reviewer"
description: "Review Terrascan using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Terrascan Reviewer

## Purpose

Review and operate Terrascan using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Terrascan code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Terrascan requires evidence-backed assessment.
...
- First-party source: https://github.com/tenable/terrascan
...
- [ ] Trace Terrascan inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Terrascan defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Terrascan least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Terrascan resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Terrascan release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Terrascan dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Terrascan identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Terrascan change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Terrascan documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Terrascan architecture and dependency summary with trust boundaries and persistent state.
- Include Terrascan validation commands or tests that are safe for the stated environment.
...
- Terrascan identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Terrascan failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Terrascan is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Terrascan as the basis for migration or production guidance.
