---
name: "cncf-nirmata-kcsp-reviewer"
description: "Review Nirmata (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Nirmata simplifies kubernetes for application development teams."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Nirmata Kcsp Reviewer

## Purpose

Review and operate Nirmata (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Nirmata (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Nirmata (KCSP) requires evidence-backed assessment.
...
- [ ] Trace Nirmata (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Nirmata (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Nirmata (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Nirmata (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Nirmata (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Nirmata (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Nirmata (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Nirmata (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Nirmata (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Nirmata (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Nirmata (KCSP) validation commands or tests that are safe for the stated environment.
...
- Nirmata (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Nirmata (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Nirmata (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Nirmata (KCSP) as the basis for migration or production guidance.
