...
---
# Cncf Doit Kcsp Reviewer

## Purpose

Review and operate DoiT (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- DoiT (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving DoiT (KCSP) requires evidence-backed assessment.
...
- [ ] Trace DoiT (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check DoiT (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check DoiT (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check DoiT (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check DoiT (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check DoiT (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If DoiT (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a DoiT (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- DoiT (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a DoiT (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include DoiT (KCSP) validation commands or tests that are safe for the stated environment.
...
- DoiT (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- DoiT (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that DoiT (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of DoiT (KCSP) as the basis for migration or production guidance.
