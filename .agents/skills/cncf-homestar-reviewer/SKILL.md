---
name: "cncf-homestar-reviewer"
description: "Review homestar using its official documentation and repository in the Wasm / Application Frameworks category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Homestar Reviewer

## Purpose

Review and operate homestar using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- homestar code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving homestar requires evidence-backed assessment.
...
- First-party source: https://github.com/ipvm-wg/homestar
...
- [ ] Trace homestar inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check homestar defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check homestar least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check homestar resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check homestar release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check homestar dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If homestar identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a homestar change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- homestar documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a homestar architecture and dependency summary with trust boundaries and persistent state.
- Include homestar validation commands or tests that are safe for the stated environment.
...
- homestar identity, version, configuration, dependencies, and deployment assumptions are explicit.
- homestar failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that homestar is secure, supported, compatible, or suitable.
- Using an unpinned latest version of homestar as the basis for migration or production guidance.
