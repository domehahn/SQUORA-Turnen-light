---
name: "cncf-kai-scheduler-reviewer"
description: "Review KAI Scheduler using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category. KAI Scheduler is a robust, efficient, and scalable Kubernetes scheduler that optimizes GPU resource allocation for AI workloads in large-scale clusters."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kai Scheduler Reviewer

## Purpose

Review and operate KAI Scheduler using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KAI Scheduler code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KAI Scheduler requires evidence-backed assessment.
...
- First-party source: https://github.com/kai-scheduler/KAI-Scheduler
...
- [ ] Trace KAI Scheduler inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KAI Scheduler defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KAI Scheduler least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KAI Scheduler resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KAI Scheduler release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KAI Scheduler dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KAI Scheduler identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KAI Scheduler change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KAI Scheduler documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KAI Scheduler architecture and dependency summary with trust boundaries and persistent state.
- Include KAI Scheduler validation commands or tests that are safe for the stated environment.
...
- KAI Scheduler identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KAI Scheduler failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KAI Scheduler is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KAI Scheduler as the basis for migration or production guidance.
