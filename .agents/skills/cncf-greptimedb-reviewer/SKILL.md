---
name: "cncf-greptimedb-reviewer"
description: "Review GreptimeDB using its official documentation and repository in the App Definition and Development / Database category. GreptimeDB is an open-source, cloud-native time series database which also has powerful analytical features"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Greptimedb Reviewer

## Purpose

Review and operate GreptimeDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- GreptimeDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving GreptimeDB requires evidence-backed assessment.
...
- First-party source: https://github.com/grepTimeTeam/greptimedb
...
- [ ] Trace GreptimeDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check GreptimeDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check GreptimeDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check GreptimeDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check GreptimeDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check GreptimeDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If GreptimeDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a GreptimeDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- GreptimeDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a GreptimeDB architecture and dependency summary with trust boundaries and persistent state.
- Include GreptimeDB validation commands or tests that are safe for the stated environment.
...
- GreptimeDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- GreptimeDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that GreptimeDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of GreptimeDB as the basis for migration or production guidance.
