...
---
# Cncf Deepchecks Reviewer

## Purpose

Review and operate Deepchecks using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Deepchecks code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Deepchecks requires evidence-backed assessment.
...
- First-party source: https://github.com/deepchecks/deepchecks
...
- [ ] Trace Deepchecks inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Deepchecks defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Deepchecks least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Deepchecks resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Deepchecks release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Deepchecks dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Deepchecks identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Deepchecks change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Deepchecks documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Deepchecks architecture and dependency summary with trust boundaries and persistent state.
- Include Deepchecks validation commands or tests that are safe for the stated environment.
...
- Deepchecks identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Deepchecks failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Deepchecks is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Deepchecks as the basis for migration or production guidance.
