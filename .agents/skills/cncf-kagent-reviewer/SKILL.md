---
name: "cncf-kagent-reviewer"
description: "Review kagent using its official documentation and repository in the Provisioning / Automation & Configuration category. Kagent is an open source programming framework designed for DevOps and platform engineers to run AI agents in Kubernetes"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kagent Reviewer

## Purpose

Review and operate kagent using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- kagent code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving kagent requires evidence-backed assessment.
...
- First-party source: https://github.com/kagent-dev/kagent
...
- [ ] Trace kagent inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check kagent defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check kagent least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check kagent resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check kagent release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check kagent dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If kagent identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a kagent change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- kagent documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a kagent architecture and dependency summary with trust boundaries and persistent state.
- Include kagent validation commands or tests that are safe for the stated environment.
...
- kagent identity, version, configuration, dependencies, and deployment assumptions are explicit.
- kagent failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that kagent is secure, supported, compatible, or suitable.
- Using an unpinned latest version of kagent as the basis for migration or production guidance.
