---
name: "cncf-datera-reviewer"
description: "Review Datera using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Datera Reviewer

## Purpose

Review and operate Datera using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Datera code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Datera requires evidence-backed assessment.
...
- First-party source: https://datera.io/
...
- [ ] Trace Datera inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Datera defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Datera least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Datera resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Datera release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Datera dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Datera identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Datera change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Datera documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Datera architecture and dependency summary with trust boundaries and persistent state.
- Include Datera validation commands or tests that are safe for the stated environment.
...
- Datera identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Datera failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Datera is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Datera as the basis for migration or production guidance.
