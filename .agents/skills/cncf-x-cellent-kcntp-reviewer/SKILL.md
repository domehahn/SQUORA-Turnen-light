---
name: "cncf-x-cellent-kcntp-reviewer"
description: "Review x-cellent (KCNTP) using its official documentation and repository in the Special / Kubernetes and Cloud Native Training Partner category. We design and develop cloud solutions with an agile mindset, creating value for our customers and the open source community. An important point in this respect is our customized training. So we offer not only basic and advanced training but also individual training based on the wishes of our customers. We build your cloud native future."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf X Cellent Kcntp Reviewer

## Purpose

Review and operate x-cellent (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- x-cellent (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving x-cellent (KCNTP) requires evidence-backed assessment.
...
- First-party source: https://www.x-cellent.com/doing
...
- [ ] Trace x-cellent (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check x-cellent (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check x-cellent (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check x-cellent (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check x-cellent (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check x-cellent (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If x-cellent (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a x-cellent (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- x-cellent (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a x-cellent (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include x-cellent (KCNTP) validation commands or tests that are safe for the stated environment.
...
- x-cellent (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- x-cellent (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that x-cellent (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of x-cellent (KCNTP) as the basis for migration or production guidance.
