---
name: "cncf-orca-security-reviewer"
description: "Review Orca Security using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Orca Security Reviewer

## Purpose

Review and operate Orca Security using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Orca Security code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Orca Security requires evidence-backed assessment.
...
- First-party source: https://orca.security
...
- [ ] Trace Orca Security inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Orca Security defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Orca Security least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Orca Security resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Orca Security release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Orca Security dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Orca Security identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Orca Security change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Orca Security documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Orca Security architecture and dependency summary with trust boundaries and persistent state.
- Include Orca Security validation commands or tests that are safe for the stated environment.
...
- Orca Security identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Orca Security failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Orca Security is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Orca Security as the basis for migration or production guidance.
