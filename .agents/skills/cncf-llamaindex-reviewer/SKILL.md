---
name: "cncf-llamaindex-reviewer"
description: "Review LlamaIndex using its official documentation and repository in the AI Agent / RAG category. LlamaIndex is the leading document agent and OCR platform"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Llamaindex Reviewer

## Purpose

Review and operate LlamaIndex using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- LlamaIndex code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving LlamaIndex requires evidence-backed assessment.
...
- First-party source: https://github.com/run-llama/llama_index
...
- [ ] Trace LlamaIndex inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check LlamaIndex defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check LlamaIndex least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check LlamaIndex resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check LlamaIndex release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check LlamaIndex dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If LlamaIndex identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a LlamaIndex change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- LlamaIndex documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a LlamaIndex architecture and dependency summary with trust boundaries and persistent state.
- Include LlamaIndex validation commands or tests that are safe for the stated environment.
...
- LlamaIndex identity, version, configuration, dependencies, and deployment assumptions are explicit.
- LlamaIndex failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that LlamaIndex is secure, supported, compatible, or suitable.
- Using an unpinned latest version of LlamaIndex as the basis for migration or production guidance.
