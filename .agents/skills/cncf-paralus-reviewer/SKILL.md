---
name: "cncf-paralus-reviewer"
description: "Review Paralus using its official documentation and repository in the Provisioning / Security & Compliance category. Paralus is a free, open source tool that enables controlled, audited access to Kubernetes infrastructure and Zero trust Kubernetes with zero friction."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Paralus Reviewer

## Purpose

Review and operate Paralus using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Paralus code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Paralus requires evidence-backed assessment.
...
- First-party source: https://github.com/paralus/paralus
...
- [ ] Trace Paralus inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Paralus defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Paralus least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Paralus resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Paralus release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Paralus dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Paralus identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Paralus change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Paralus documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Paralus architecture and dependency summary with trust boundaries and persistent state.
- Include Paralus validation commands or tests that are safe for the stated environment.
...
- Paralus identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Paralus failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Paralus is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Paralus as the basis for migration or production guidance.
