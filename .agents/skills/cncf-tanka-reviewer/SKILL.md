...
Review and operate Tanka using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Tanka code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Tanka requires evidence-backed assessment.
...
- First-party source: https://github.com/grafana/tanka
...
- [ ] Trace Tanka inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Tanka defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Tanka least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Tanka resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Tanka release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Tanka dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Tanka identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Tanka change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Tanka documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Tanka architecture and dependency summary with trust boundaries and persistent state.
- Include Tanka validation commands or tests that are safe for the stated environment.
...
- Tanka identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Tanka failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Tanka is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Tanka as the basis for migration or production guidance.
