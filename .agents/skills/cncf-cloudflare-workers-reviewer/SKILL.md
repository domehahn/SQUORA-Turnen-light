...
---
# Cncf Cloudflare Workers Reviewer

## Purpose

Review and operate Cloudflare Workers using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Cloudflare Workers code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Cloudflare Workers requires evidence-backed assessment.
...
- [ ] Trace Cloudflare Workers inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Cloudflare Workers defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Cloudflare Workers least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Cloudflare Workers resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Cloudflare Workers release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Cloudflare Workers dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Cloudflare Workers identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Cloudflare Workers change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Cloudflare Workers documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Cloudflare Workers architecture and dependency summary with trust boundaries and persistent state.
- Include Cloudflare Workers validation commands or tests that are safe for the stated environment.
...
- Cloudflare Workers identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Cloudflare Workers failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Cloudflare Workers is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Cloudflare Workers as the basis for migration or production guidance.
