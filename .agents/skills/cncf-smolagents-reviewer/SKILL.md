---
name: "cncf-smolagents-reviewer"
description: "Review Smolagents using its official documentation and repository in the AI Agent / Agent Framework category. A barebones library for agents that think in code."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Smolagents Reviewer

## Purpose

Review and operate Smolagents using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Smolagents code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Smolagents requires evidence-backed assessment.
...
- First-party source: https://github.com/huggingface/smolagents
...
- [ ] Trace Smolagents inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Smolagents defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Smolagents least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Smolagents resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Smolagents release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Smolagents dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Smolagents identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Smolagents change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Smolagents documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Smolagents architecture and dependency summary with trust boundaries and persistent state.
- Include Smolagents validation commands or tests that are safe for the stated environment.
...
- Smolagents identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Smolagents failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Smolagents is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Smolagents as the basis for migration or production guidance.
