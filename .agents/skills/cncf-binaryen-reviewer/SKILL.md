---
name: "cncf-binaryen-reviewer"
description: "Review Binaryen using its official documentation and repository in the Wasm / Tooling category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Binaryen Reviewer

## Purpose

Review and operate Binaryen using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Binaryen code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Binaryen requires evidence-backed assessment.
...
- First-party source: https://github.com/WebAssembly/binaryen
...
- [ ] Trace Binaryen inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Binaryen defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Binaryen least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Binaryen resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Binaryen release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Binaryen dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Binaryen identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Binaryen change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Binaryen documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Binaryen architecture and dependency summary with trust boundaries and persistent state.
- Include Binaryen validation commands or tests that are safe for the stated environment.
...
- Binaryen identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Binaryen failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Binaryen is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Binaryen as the basis for migration or production guidance.
