---
name: "cncf-envoy-ai-gateway-reviewer"
description: "Review Envoy-ai-gateway using its official documentation and repository in the AI Native Infra / Gateway category. Manages Unified Access to Generative AI Services built on Envoy Gateway"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Envoy Ai Gateway Reviewer

## Purpose

Review and operate Envoy-ai-gateway using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Envoy-ai-gateway code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Envoy-ai-gateway requires evidence-backed assessment.
...
- First-party source: https://github.com/envoyproxy/ai-gateway
...
- [ ] Trace Envoy-ai-gateway inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Envoy-ai-gateway defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Envoy-ai-gateway least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Envoy-ai-gateway resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Envoy-ai-gateway release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Envoy-ai-gateway dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Envoy-ai-gateway identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Envoy-ai-gateway change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Envoy-ai-gateway documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Envoy-ai-gateway architecture and dependency summary with trust boundaries and persistent state.
- Include Envoy-ai-gateway validation commands or tests that are safe for the stated environment.
...
- Envoy-ai-gateway identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Envoy-ai-gateway failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Envoy-ai-gateway is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Envoy-ai-gateway as the basis for migration or production guidance.
