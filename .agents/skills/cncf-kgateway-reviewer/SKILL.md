---
name: "cncf-kgateway-reviewer"
description: "Review Kgateway using its official documentation and repository in the AI Native Infra / Gateway category. An Envoy-powered, Kubernetes-native API Gateway that integrates Kubernetes Gateway API with a control plane for API connectivity in any cloud environment."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kgateway Reviewer

## Purpose

Review and operate Kgateway using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kgateway code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kgateway requires evidence-backed assessment.
...
- CNCF classification: AI Native Infra / Gateway; Orchestration & Management / API Gateway
- CNCF project maturity field: sandbox
- First-party source: https://github.com/kgateway-dev/kgateway
...
- [ ] Trace Kgateway inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kgateway defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kgateway least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kgateway resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kgateway release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kgateway dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kgateway identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kgateway change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kgateway documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kgateway architecture and dependency summary with trust boundaries and persistent state.
- Include Kgateway validation commands or tests that are safe for the stated environment.
...
- Kgateway identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kgateway failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kgateway is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kgateway as the basis for migration or production guidance.
