---
name: "cncf-nuvotex-kcsp-reviewer"
description: "Review Nuvotex (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Nuvotex provides a variety of Kubernetes services, including training, consulting, technical support, and implementation."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Nuvotex Kcsp Reviewer

## Purpose

Review and operate Nuvotex (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Nuvotex (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Nuvotex (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.nuvotex.de/kubernetes/
...
- [ ] Trace Nuvotex (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Nuvotex (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Nuvotex (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Nuvotex (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Nuvotex (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Nuvotex (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Nuvotex (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Nuvotex (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Nuvotex (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Nuvotex (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Nuvotex (KCSP) validation commands or tests that are safe for the stated environment.
...
- Nuvotex (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Nuvotex (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Nuvotex (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Nuvotex (KCSP) as the basis for migration or production guidance.
