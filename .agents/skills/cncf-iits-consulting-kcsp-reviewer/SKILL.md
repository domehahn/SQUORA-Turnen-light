...
---
# Cncf Iits Consulting Kcsp Reviewer

## Purpose

Review and operate iits-consulting (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- iits-consulting (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving iits-consulting (KCSP) requires evidence-backed assessment.
...
- First-party source: https://iits-consulting.de/kubernetes/
...
- [ ] Trace iits-consulting (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check iits-consulting (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check iits-consulting (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check iits-consulting (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check iits-consulting (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check iits-consulting (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If iits-consulting (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a iits-consulting (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- iits-consulting (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a iits-consulting (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include iits-consulting (KCSP) validation commands or tests that are safe for the stated environment.
...
- iits-consulting (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- iits-consulting (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that iits-consulting (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of iits-consulting (KCSP) as the basis for migration or production guidance.
