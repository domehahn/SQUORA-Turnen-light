...
Review and operate NATS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- NATS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving NATS requires evidence-backed assessment.
...
- First-party source: https://github.com/nats-io/nats-server
...
- [ ] Trace NATS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check NATS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check NATS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check NATS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check NATS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check NATS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If NATS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a NATS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- NATS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a NATS architecture and dependency summary with trust boundaries and persistent state.
- Include NATS validation commands or tests that are safe for the stated environment.
...
- NATS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- NATS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that NATS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of NATS as the basis for migration or production guidance.
