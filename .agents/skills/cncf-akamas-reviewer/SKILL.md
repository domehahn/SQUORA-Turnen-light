---
name: "cncf-akamas-reviewer"
description: "Review Akamas using its official documentation and repository in the Observability and Analysis / Continuous Optimization category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Akamas Reviewer

## Purpose

Review and operate Akamas using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Akamas code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Akamas requires evidence-backed assessment.
...
- [ ] Trace Akamas inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Akamas defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Akamas least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Akamas resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Akamas release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Akamas dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Akamas identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Akamas change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Akamas documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Akamas architecture and dependency summary with trust boundaries and persistent state.
- Include Akamas validation commands or tests that are safe for the stated environment.
...
- Akamas identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Akamas failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Akamas is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Akamas as the basis for migration or production guidance.
