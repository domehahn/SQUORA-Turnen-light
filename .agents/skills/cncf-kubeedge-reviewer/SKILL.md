---
name: "cncf-kubeedge-reviewer"
description: "Review KubeEdge using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kubeedge Reviewer

## Purpose

Review and operate KubeEdge using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeEdge code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeEdge requires evidence-backed assessment.
...
- [ ] Trace KubeEdge inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeEdge defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeEdge least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeEdge resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeEdge release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeEdge dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeEdge identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeEdge change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeEdge documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeEdge architecture and dependency summary with trust boundaries and persistent state.
- Include KubeEdge validation commands or tests that are safe for the stated environment.
...
- KubeEdge identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeEdge failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeEdge is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeEdge as the basis for migration or production guidance.
