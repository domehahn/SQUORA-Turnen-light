---
name: "cncf-trickster-reviewer"
description: "Review Trickster using its official documentation and repository in the Observability and Analysis / Observability category. Open Source HTTP Reverse Proxy Cache and Time Series Dashboard Accelerator"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Trickster Reviewer

## Purpose

Review and operate Trickster using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Trickster code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Trickster requires evidence-backed assessment.
...
- First-party source: https://github.com/trickstercache/trickster
...
- [ ] Trace Trickster inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Trickster defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Trickster least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Trickster resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Trickster release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Trickster dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Trickster identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Trickster change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Trickster documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Trickster architecture and dependency summary with trust boundaries and persistent state.
- Include Trickster validation commands or tests that are safe for the stated environment.
...
- Trickster identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Trickster failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Trickster is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Trickster as the basis for migration or production guidance.
