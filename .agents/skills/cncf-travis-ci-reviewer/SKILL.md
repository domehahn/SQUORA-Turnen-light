...
Review and operate Travis CI using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Travis CI code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Travis CI requires evidence-backed assessment.
...
- First-party source: https://github.com/travis-ci/travis-web
...
- [ ] Trace Travis CI inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Travis CI defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Travis CI least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Travis CI resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Travis CI release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Travis CI dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Travis CI identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Travis CI change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Travis CI documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Travis CI architecture and dependency summary with trust boundaries and persistent state.
- Include Travis CI validation commands or tests that are safe for the stated environment.
...
- Travis CI identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Travis CI failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Travis CI is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Travis CI as the basis for migration or production guidance.
