---
name: "cncf-ripple-reviewer"
description: "Review Ripple using its official documentation and repository in the Wasm / Decentralized Platforms category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Ripple Reviewer

## Purpose

Review and operate Ripple using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Ripple code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Ripple requires evidence-backed assessment.
...
- First-party source: https://ripple.com/
...
- [ ] Trace Ripple inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Ripple defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Ripple least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Ripple resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Ripple release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Ripple dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Ripple identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Ripple change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Ripple documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Ripple architecture and dependency summary with trust boundaries and persistent state.
- Include Ripple validation commands or tests that are safe for the stated environment.
...
- Ripple identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Ripple failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Ripple is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Ripple as the basis for migration or production guidance.
