---
name: "cncf-bamboo-reviewer"
description: "Review Bamboo using its official documentation and repository in the App Definition and Development / Continuous Integration & Delivery category. Bamboo Server is the choice of professional teams for continuous integration, deployment, and delivery"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Bamboo Reviewer

## Purpose

Review and operate Bamboo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Bamboo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Bamboo requires evidence-backed assessment.
...
- First-party source: https://www.atlassian.com/software/bamboo
...
- [ ] Trace Bamboo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Bamboo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Bamboo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Bamboo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Bamboo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Bamboo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Bamboo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Bamboo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Bamboo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Bamboo architecture and dependency summary with trust boundaries and persistent state.
- Include Bamboo validation commands or tests that are safe for the stated environment.
...
- Bamboo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Bamboo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Bamboo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Bamboo as the basis for migration or production guidance.
