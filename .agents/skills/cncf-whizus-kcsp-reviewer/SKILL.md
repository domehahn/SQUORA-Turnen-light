---
name: "cncf-whizus-kcsp-reviewer"
description: "Review WhizUs (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. As Enterprise Kubernetes Consultants, we support you to build up your Kubernetes cluster in your chosen environment according to your needs."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Whizus Kcsp Reviewer

## Purpose

Review and operate WhizUs (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- WhizUs (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving WhizUs (KCSP) requires evidence-backed assessment.
...
- [ ] Trace WhizUs (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check WhizUs (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check WhizUs (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check WhizUs (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check WhizUs (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check WhizUs (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If WhizUs (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a WhizUs (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- WhizUs (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a WhizUs (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include WhizUs (KCSP) validation commands or tests that are safe for the stated environment.
...
- WhizUs (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- WhizUs (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that WhizUs (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of WhizUs (KCSP) as the basis for migration or production guidance.
