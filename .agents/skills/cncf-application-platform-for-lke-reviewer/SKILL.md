---
name: "cncf-application-platform-for-lke-reviewer"
description: "Review Application Platform for LKE using its official documentation and repository in the Platform / PaaS/Container Service category. Application Platform for Linode Kubernetes Engine (LKE)"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Application Platform For Lke Reviewer

## Purpose

Review and operate Application Platform for LKE using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Application Platform for LKE code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Application Platform for LKE requires evidence-backed assessment.
...
- First-party source: https://github.com/linode/apl-core
...
- [ ] Trace Application Platform for LKE inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Application Platform for LKE defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Application Platform for LKE least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Application Platform for LKE resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Application Platform for LKE release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Application Platform for LKE dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Application Platform for LKE identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Application Platform for LKE change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Application Platform for LKE documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Application Platform for LKE architecture and dependency summary with trust boundaries and persistent state.
- Include Application Platform for LKE validation commands or tests that are safe for the stated environment.
...
- Application Platform for LKE identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Application Platform for LKE failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Application Platform for LKE is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Application Platform for LKE as the basis for migration or production guidance.
