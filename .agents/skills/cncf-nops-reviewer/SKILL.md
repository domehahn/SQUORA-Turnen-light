---
name: "cncf-nops-reviewer"
description: "Review nOps using its official documentation and repository in the Observability and Analysis / Continuous Optimization category. nOps is an end-to-end AWS and Kubernetes optimization platform designed to simplify and automate cloud tracking, allocation, and optimization."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate nOps using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- nOps code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving nOps requires evidence-backed assessment.
...
- First-party source: https://github.com/nops-io/nops-k8s-agent
...
- [ ] Trace nOps inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check nOps defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check nOps least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check nOps resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check nOps release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check nOps dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If nOps identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a nOps change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- nOps documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a nOps architecture and dependency summary with trust boundaries and persistent state.
- Include nOps validation commands or tests that are safe for the stated environment.
...
- nOps identity, version, configuration, dependencies, and deployment assumptions are explicit.
- nOps failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that nOps is secure, supported, compatible, or suitable.
- Using an unpinned latest version of nOps as the basis for migration or production guidance.
