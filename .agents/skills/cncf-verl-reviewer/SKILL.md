---
name: "cncf-verl-reviewer"
description: "Review verl using its official documentation and repository in the Training / Post Training category. Volcano Engine Reinforcement Learning for LLMs"
version: "1.0.0"
since: "2026-08-27"
...
Review and operate verl using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- verl code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving verl requires evidence-backed assessment.
...
- First-party source: https://github.com/verl-project/verl
...
- [ ] Trace verl inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check verl defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check verl least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check verl resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check verl release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check verl dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If verl identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a verl change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- verl documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a verl architecture and dependency summary with trust boundaries and persistent state.
- Include verl validation commands or tests that are safe for the stated environment.
...
- verl identity, version, configuration, dependencies, and deployment assumptions are explicit.
- verl failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that verl is secure, supported, compatible, or suitable.
- Using an unpinned latest version of verl as the basis for migration or production guidance.
