---
name: "cncf-chef-infra-reviewer"
description: "Review Chef Infra using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Chef Infra Reviewer

## Purpose

Review and operate Chef Infra using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Chef Infra code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Chef Infra requires evidence-backed assessment.
...
- First-party source: https://github.com/chef/chef
...
- [ ] Trace Chef Infra inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Chef Infra defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Chef Infra least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Chef Infra resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Chef Infra release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Chef Infra dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Chef Infra identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Chef Infra change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Chef Infra documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Chef Infra architecture and dependency summary with trust boundaries and persistent state.
- Include Chef Infra validation commands or tests that are safe for the stated environment.
...
- Chef Infra identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Chef Infra failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Chef Infra is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Chef Infra as the basis for migration or production guidance.
