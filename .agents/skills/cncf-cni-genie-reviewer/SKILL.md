---
name: "cncf-cni-genie-reviewer"
description: "Review CNI-Genie using its official documentation and repository in the Runtime / Cloud Native Network category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Cni Genie Reviewer

## Purpose

Review and operate CNI-Genie using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CNI-Genie code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CNI-Genie requires evidence-backed assessment.
...
- First-party source: https://github.com/cni-genie/CNI-Genie
...
- [ ] Trace CNI-Genie inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CNI-Genie defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CNI-Genie least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CNI-Genie resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CNI-Genie release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CNI-Genie dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CNI-Genie identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CNI-Genie change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CNI-Genie documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CNI-Genie architecture and dependency summary with trust boundaries and persistent state.
- Include CNI-Genie validation commands or tests that are safe for the stated environment.
...
- CNI-Genie identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CNI-Genie failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CNI-Genie is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CNI-Genie as the basis for migration or production guidance.
