---
name: "cncf-instructor-reviewer"
description: "Review Instructor using its official documentation and repository in the AI Agent / Structured Output category. structured outputs for llms"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Instructor Reviewer

## Purpose

Review and operate Instructor using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Instructor code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Instructor requires evidence-backed assessment.
...
- CNCF classification: AI Agent / Structured Output
- CNCF project maturity field: not specified
- First-party source: https://github.com/567-labs/instructor
...
- [ ] Trace Instructor inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Instructor defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Instructor least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Instructor resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Instructor release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Instructor dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Instructor identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Instructor change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Instructor documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Instructor architecture and dependency summary with trust boundaries and persistent state.
- Include Instructor validation commands or tests that are safe for the stated environment.
...
- Instructor identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Instructor failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Instructor is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Instructor as the basis for migration or production guidance.
