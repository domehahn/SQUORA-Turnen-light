---
name: "cncf-rackner-kcsp-reviewer"
description: "Review Rackner (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Rackner is a cloud native consultancy where we work closely with our clients to build, secure, and operate on any cloud with Kubernetes."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Rackner Kcsp Reviewer

## Purpose

Review and operate Rackner (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Rackner (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Rackner (KCSP) requires evidence-backed assessment.
...
- First-party source: https://rackner.com/kubernetes/
...
- [ ] Trace Rackner (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Rackner (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Rackner (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Rackner (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Rackner (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Rackner (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Rackner (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Rackner (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Rackner (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Rackner (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Rackner (KCSP) validation commands or tests that are safe for the stated environment.
...
- Rackner (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Rackner (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Rackner (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Rackner (KCSP) as the basis for migration or production guidance.
