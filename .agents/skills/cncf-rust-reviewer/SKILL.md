---
name: "cncf-rust-reviewer"
description: "Review Rust using its official documentation and repository in the Wasm / Languages category. Compiled language to Wasm"
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Rust using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Rust code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Rust requires evidence-backed assessment.
...
- First-party source: https://github.com/rust-lang/rust
...
- [ ] Trace Rust inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Rust defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Rust least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Rust resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Rust release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Rust dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Rust identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Rust change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Rust documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Rust architecture and dependency summary with trust boundaries and persistent state.
- Include Rust validation commands or tests that are safe for the stated environment.
...
- Rust identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Rust failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Rust is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Rust as the basis for migration or production guidance.
