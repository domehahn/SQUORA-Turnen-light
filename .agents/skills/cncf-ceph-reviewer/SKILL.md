---
name: "cncf-ceph-reviewer"
description: "Review Ceph using its official documentation and repository in the Runtime / Cloud Native Storage category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Ceph using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Ceph code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Ceph requires evidence-backed assessment.
...
- First-party source: https://github.com/ceph/ceph
...
- [ ] Trace Ceph inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Ceph defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Ceph least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Ceph resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Ceph release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Ceph dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Ceph identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Ceph change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Ceph documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Ceph architecture and dependency summary with trust boundaries and persistent state.
- Include Ceph validation commands or tests that are safe for the stated environment.
...
- Ceph identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Ceph failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Ceph is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Ceph as the basis for migration or production guidance.
