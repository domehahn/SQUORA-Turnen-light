---
name: "cncf-snapt-nova-reviewer"
description: "Review Snapt Nova using its official documentation and repository in the Orchestration & Management / Service Proxy category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Snapt Nova Reviewer

## Purpose

Review and operate Snapt Nova using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Snapt Nova code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Snapt Nova requires evidence-backed assessment.
...
- First-party source: https://github.com/snapt
...
- [ ] Trace Snapt Nova inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Snapt Nova defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Snapt Nova least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Snapt Nova resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Snapt Nova release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Snapt Nova dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Snapt Nova identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Snapt Nova change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Snapt Nova documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Snapt Nova architecture and dependency summary with trust boundaries and persistent state.
- Include Snapt Nova validation commands or tests that are safe for the stated environment.
...
- Snapt Nova identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Snapt Nova failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Snapt Nova is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Snapt Nova as the basis for migration or production guidance.
