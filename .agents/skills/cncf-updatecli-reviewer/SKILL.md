---
name: "cncf-updatecli-reviewer"
description: "Review Updatecli using its official documentation and repository in the Provisioning / Automation & Configuration category. Updatecli is a declarative dependency management tool for Git repositories."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Updatecli Reviewer

## Purpose

Review and operate Updatecli using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Updatecli code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Updatecli requires evidence-backed assessment.
...
- First-party source: https://github.com/updatecli/updatecli
...
- [ ] Trace Updatecli inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Updatecli defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Updatecli least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Updatecli resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Updatecli release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Updatecli dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Updatecli identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Updatecli change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Updatecli documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Updatecli architecture and dependency summary with trust boundaries and persistent state.
- Include Updatecli validation commands or tests that are safe for the stated environment.
...
- Updatecli identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Updatecli failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Updatecli is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Updatecli as the basis for migration or production guidance.
