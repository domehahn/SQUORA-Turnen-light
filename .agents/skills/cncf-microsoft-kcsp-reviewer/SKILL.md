---
name: "cncf-microsoft-kcsp-reviewer"
description: "Review Microsoft (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Microsoft Services provides a comprehensive set of offerings to deploy and secure Kubernetes on Azure as well as containerize applications and migrate them from on-premises or other Clouds to Azure."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Microsoft Kcsp Reviewer

## Purpose

Review and operate Microsoft (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Microsoft (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Microsoft (KCSP) requires evidence-backed assessment.
...
- [ ] Trace Microsoft (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Microsoft (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Microsoft (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Microsoft (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Microsoft (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Microsoft (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Microsoft (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Microsoft (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Microsoft (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Microsoft (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include Microsoft (KCSP) validation commands or tests that are safe for the stated environment.
...
- Microsoft (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Microsoft (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Microsoft (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Microsoft (KCSP) as the basis for migration or production guidance.
