---
name: "cncf-redis-reviewer"
description: "Review Redis using its official documentation and repository in the App Definition and Development / Database category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Redis using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Redis code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Redis requires evidence-backed assessment.
...
- First-party source: https://github.com/redis/redis
...
- [ ] Trace Redis inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Redis defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Redis least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Redis resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Redis release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Redis dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Redis identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Redis change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Redis documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Redis architecture and dependency summary with trust boundaries and persistent state.
- Include Redis validation commands or tests that are safe for the stated environment.
...
- Redis identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Redis failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Redis is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Redis as the basis for migration or production guidance.
