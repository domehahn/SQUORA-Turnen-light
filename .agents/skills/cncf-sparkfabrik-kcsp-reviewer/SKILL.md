---
name: "cncf-sparkfabrik-kcsp-reviewer"
description: "Review SparkFabrik (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. We provide cloud native custom development and services on top of Kubernetes, helping our customers to transition to the cloud native era."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Sparkfabrik Kcsp Reviewer

## Purpose

Review and operate SparkFabrik (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SparkFabrik (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SparkFabrik (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.sparkfabrik.com/en/services/cloud-native-services/kubernetes-consultancy
...
- [ ] Trace SparkFabrik (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SparkFabrik (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SparkFabrik (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SparkFabrik (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SparkFabrik (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SparkFabrik (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SparkFabrik (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SparkFabrik (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SparkFabrik (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SparkFabrik (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include SparkFabrik (KCSP) validation commands or tests that are safe for the stated environment.
...
- SparkFabrik (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SparkFabrik (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SparkFabrik (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SparkFabrik (KCSP) as the basis for migration or production guidance.
