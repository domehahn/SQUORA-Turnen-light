---
name: "cncf-eventmesh-reviewer"
description: "Review EventMesh using its official documentation and repository in the Serverless / Framework category. EventMesh is a new generation serverless event middleware for building distributed event-driven applications."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Eventmesh Reviewer

## Purpose

Review and operate EventMesh using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- EventMesh code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving EventMesh requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/eventmesh
...
- [ ] Trace EventMesh inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check EventMesh defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check EventMesh least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check EventMesh resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check EventMesh release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check EventMesh dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If EventMesh identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a EventMesh change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- EventMesh documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a EventMesh architecture and dependency summary with trust boundaries and persistent state.
- Include EventMesh validation commands or tests that are safe for the stated environment.
...
- EventMesh identity, version, configuration, dependencies, and deployment assumptions are explicit.
- EventMesh failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that EventMesh is secure, supported, compatible, or suitable.
- Using an unpinned latest version of EventMesh as the basis for migration or production guidance.
