---
name: "cncf-go-feature-flag-reviewer"
description: "Review GO Feature Flag using its official documentation and repository in the Observability and Analysis / Feature Flagging category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Go Feature Flag Reviewer

## Purpose

Review and operate GO Feature Flag using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- GO Feature Flag code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving GO Feature Flag requires evidence-backed assessment.
...
- First-party source: https://github.com/thomaspoignant/go-feature-flag
...
- [ ] Trace GO Feature Flag inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check GO Feature Flag defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check GO Feature Flag least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check GO Feature Flag resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check GO Feature Flag release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check GO Feature Flag dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If GO Feature Flag identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a GO Feature Flag change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- GO Feature Flag documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a GO Feature Flag architecture and dependency summary with trust boundaries and persistent state.
- Include GO Feature Flag validation commands or tests that are safe for the stated environment.
...
- GO Feature Flag identity, version, configuration, dependencies, and deployment assumptions are explicit.
- GO Feature Flag failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that GO Feature Flag is secure, supported, compatible, or suitable.
- Using an unpinned latest version of GO Feature Flag as the basis for migration or production guidance.
