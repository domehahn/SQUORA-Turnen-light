---
name: "cncf-awesome-information-technology-kcsp-reviewer"
description: "Review awesome information technology (KCSP) using its official documentation and repository in the Special / Kubernetes Certified Service Provider category. Independent consulting, development of specialized and individual Kubernetes clusters, and fully automated deployments"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Awesome Information Technology Kcsp Reviewer

## Purpose

Review and operate awesome information technology (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- awesome information technology (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving awesome information technology (KCSP) requires evidence-backed assessment.
...
- First-party source: https://awesome-it.de/infrastructure/kubernetes
...
- [ ] Trace awesome information technology (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check awesome information technology (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check awesome information technology (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check awesome information technology (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check awesome information technology (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check awesome information technology (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If awesome information technology (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a awesome information technology (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- awesome information technology (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a awesome information technology (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include awesome information technology (KCSP) validation commands or tests that are safe for the stated environment.
...
- awesome information technology (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- awesome information technology (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that awesome information technology (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of awesome information technology (KCSP) as the basis for migration or production guidance.
