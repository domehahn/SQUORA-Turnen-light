---
name: "cncf-unikraft-reviewer"
description: "Review Unikraft using its official documentation and repository in the Wasm / Edge/Bare metal category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Unikraft Reviewer

## Purpose

Review and operate Unikraft using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Unikraft code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Unikraft requires evidence-backed assessment.
...
- CNCF classification: Wasm / Edge/Bare metal
- CNCF project maturity field: not specified
- First-party source: https://github.com/unikraft/unikraft
...
- [ ] Trace Unikraft inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Unikraft defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Unikraft least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Unikraft resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Unikraft release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Unikraft dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Unikraft identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Unikraft change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Unikraft documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Unikraft architecture and dependency summary with trust boundaries and persistent state.
- Include Unikraft validation commands or tests that are safe for the stated environment.
...
- Unikraft identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Unikraft failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Unikraft is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Unikraft as the basis for migration or production guidance.
