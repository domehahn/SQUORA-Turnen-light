---
name: "cncf-kudo-reviewer"
description: "Review KUDO using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category. Kubernetes Universal Declarative Operator"
version: "1.0.0"
since: "2026-08-27"
...
Review and operate KUDO using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KUDO code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KUDO requires evidence-backed assessment.
...
- First-party source: https://github.com/kudobuilder/kudo
...
- [ ] Trace KUDO inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KUDO defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KUDO least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KUDO resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KUDO release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KUDO dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KUDO identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KUDO change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KUDO documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KUDO architecture and dependency summary with trust boundaries and persistent state.
- Include KUDO validation commands or tests that are safe for the stated environment.
...
- KUDO identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KUDO failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KUDO is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KUDO as the basis for migration or production guidance.
