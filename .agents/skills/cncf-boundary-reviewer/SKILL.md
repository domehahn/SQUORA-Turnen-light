---
name: "cncf-boundary-reviewer"
description: "Review Boundary using its official documentation and repository in the Provisioning / Security & Compliance category. Boundary is designed to grant access to critical systems using the principle of least privilege, solving challenges organizations encounter when users need to securely access applications and machines."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Boundary Reviewer

## Purpose

Review and operate Boundary using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Boundary code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Boundary requires evidence-backed assessment.
...
- First-party source: https://github.com/hashicorp/boundary
...
- [ ] Trace Boundary inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Boundary defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Boundary least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Boundary resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Boundary release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Boundary dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Boundary identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Boundary change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Boundary documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Boundary architecture and dependency summary with trust boundaries and persistent state.
- Include Boundary validation commands or tests that are safe for the stated environment.
...
- Boundary identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Boundary failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Boundary is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Boundary as the basis for migration or production guidance.
