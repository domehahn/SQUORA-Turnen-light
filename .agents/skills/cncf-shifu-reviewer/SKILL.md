---
name: "cncf-shifu-reviewer"
description: "Review Shifu using its official documentation and repository in the Provisioning / Automation & Configuration category. Shifu is the next generation cloud native IoT development framework that could 10X your IoT software development"
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Shifu using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Shifu code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Shifu requires evidence-backed assessment.
...
- First-party source: https://github.com/edgenesis/shifu
...
- [ ] Trace Shifu inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Shifu defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Shifu least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Shifu resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Shifu release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Shifu dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Shifu identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Shifu change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Shifu documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Shifu architecture and dependency summary with trust boundaries and persistent state.
- Include Shifu validation commands or tests that are safe for the stated environment.
...
- Shifu identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Shifu failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Shifu is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Shifu as the basis for migration or production guidance.
