---
name: "cncf-kube-hunter-reviewer"
description: "Review kube-hunter using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kube Hunter Reviewer

## Purpose

Review and operate kube-hunter using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- kube-hunter code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving kube-hunter requires evidence-backed assessment.
...
- First-party source: https://github.com/aquasecurity/kube-hunter
...
- [ ] Trace kube-hunter inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check kube-hunter defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check kube-hunter least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check kube-hunter resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check kube-hunter release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check kube-hunter dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If kube-hunter identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a kube-hunter change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- kube-hunter documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a kube-hunter architecture and dependency summary with trust boundaries and persistent state.
- Include kube-hunter validation commands or tests that are safe for the stated environment.
...
- kube-hunter identity, version, configuration, dependencies, and deployment assumptions are explicit.
- kube-hunter failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that kube-hunter is secure, supported, compatible, or suitable.
- Using an unpinned latest version of kube-hunter as the basis for migration or production guidance.
