...
---
# Cncf Accordion Reviewer

## Purpose

Review and operate Accordion using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Accordion code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Accordion requires evidence-backed assessment.
...
- [ ] Trace Accordion inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Accordion defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Accordion least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Accordion resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Accordion release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Accordion dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Accordion identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Accordion change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Accordion documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Accordion architecture and dependency summary with trust boundaries and persistent state.
- Include Accordion validation commands or tests that are safe for the stated environment.
...
- Accordion identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Accordion failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Accordion is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Accordion as the basis for migration or production guidance.
