---
name: "cncf-nitric-reviewer"
description: "Review Nitric using its official documentation and repository in the Serverless / Framework category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Nitric Reviewer

## Purpose

Review and operate Nitric using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Nitric code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Nitric requires evidence-backed assessment.
...
- First-party source: https://github.com/nitrictech/nitric
...
- [ ] Trace Nitric inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Nitric defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Nitric least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Nitric resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Nitric release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Nitric dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Nitric identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Nitric change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Nitric documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Nitric architecture and dependency summary with trust boundaries and persistent state.
- Include Nitric validation commands or tests that are safe for the stated environment.
...
- Nitric identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Nitric failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Nitric is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Nitric as the basis for migration or production guidance.
