---
name: "cncf-opensergo-reviewer"
description: "Review OpenSergo using its official documentation and repository in the Orchestration & Management / Service Mesh category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Opensergo Reviewer

## Purpose

Review and operate OpenSergo using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenSergo code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenSergo requires evidence-backed assessment.
...
- First-party source: https://github.com/opensergo/opensergo-specification
...
- [ ] Trace OpenSergo inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenSergo defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenSergo least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenSergo resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenSergo release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenSergo dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenSergo identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenSergo change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenSergo documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenSergo architecture and dependency summary with trust boundaries and persistent state.
- Include OpenSergo validation commands or tests that are safe for the stated environment.
...
- OpenSergo identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenSergo failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenSergo is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenSergo as the basis for migration or production guidance.
