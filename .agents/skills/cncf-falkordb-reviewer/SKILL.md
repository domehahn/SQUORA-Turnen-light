---
name: "cncf-falkordb-reviewer"
description: "Review FalkorDB using its official documentation and repository in the AI Agent / Knowledge Graph category. A super fast Graph Database uses GraphBLAS under the hood for its sparse adjacency matrix graph representation. Our goal is to provide the best Knowledge Graph for LLM (GraphRAG)."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Falkordb Reviewer

## Purpose

Review and operate FalkorDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- FalkorDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving FalkorDB requires evidence-backed assessment.
...
- CNCF classification: AI Agent / Knowledge Graph
- CNCF project maturity field: not specified
- First-party source: https://github.com/FalkorDB/FalkorDB
...
- [ ] Trace FalkorDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check FalkorDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check FalkorDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check FalkorDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check FalkorDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check FalkorDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If FalkorDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a FalkorDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- FalkorDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a FalkorDB architecture and dependency summary with trust boundaries and persistent state.
- Include FalkorDB validation commands or tests that are safe for the stated environment.
...
- FalkorDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- FalkorDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that FalkorDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of FalkorDB as the basis for migration or production guidance.
