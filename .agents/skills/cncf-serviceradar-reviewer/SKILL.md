---
name: "cncf-serviceradar-reviewer"
description: "Review ServiceRadar using its official documentation and repository in the Observability and Analysis / Observability category. Open Source network management and observability platform."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Serviceradar Reviewer

## Purpose

Review and operate ServiceRadar using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ServiceRadar code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ServiceRadar requires evidence-backed assessment.
...
- First-party source: https://github.com/carverauto/serviceradar
...
- [ ] Trace ServiceRadar inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ServiceRadar defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ServiceRadar least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ServiceRadar resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ServiceRadar release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ServiceRadar dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ServiceRadar identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ServiceRadar change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ServiceRadar documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ServiceRadar architecture and dependency summary with trust boundaries and persistent state.
- Include ServiceRadar validation commands or tests that are safe for the stated environment.
...
- ServiceRadar identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ServiceRadar failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ServiceRadar is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ServiceRadar as the basis for migration or production guidance.
