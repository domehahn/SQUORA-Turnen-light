---
name: "cncf-wavm-reviewer"
description: "Review WAVM using its official documentation and repository in the Wasm / Runtimes category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate WAVM using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- WAVM code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving WAVM requires evidence-backed assessment.
...
- First-party source: https://github.com/WAVM/WAVM
...
- [ ] Trace WAVM inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check WAVM defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check WAVM least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check WAVM resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check WAVM release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check WAVM dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If WAVM identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a WAVM change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- WAVM documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a WAVM architecture and dependency summary with trust boundaries and persistent state.
- Include WAVM validation commands or tests that are safe for the stated environment.
...
- WAVM identity, version, configuration, dependencies, and deployment assumptions are explicit.
- WAVM failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that WAVM is secure, supported, compatible, or suitable.
- Using an unpinned latest version of WAVM as the basis for migration or production guidance.
