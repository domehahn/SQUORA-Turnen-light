---
name: "cncf-atlantis-reviewer"
description: "Review Atlantis using its official documentation and repository in the Provisioning / Automation & Configuration category. Terraform Pull Request Automation for Teams"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Atlantis Reviewer

## Purpose

Review and operate Atlantis using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Atlantis code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Atlantis requires evidence-backed assessment.
...
- First-party source: https://github.com/runatlantis/atlantis
...
- [ ] Trace Atlantis inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Atlantis defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Atlantis least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Atlantis resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Atlantis release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Atlantis dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Atlantis identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Atlantis change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Atlantis documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Atlantis architecture and dependency summary with trust boundaries and persistent state.
- Include Atlantis validation commands or tests that are safe for the stated environment.
...
- Atlantis identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Atlantis failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Atlantis is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Atlantis as the basis for migration or production guidance.
