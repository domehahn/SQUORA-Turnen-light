---
name: "cncf-tidb-reviewer"
description: "Review TiDB using its official documentation and repository in the App Definition and Development / Database category."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate TiDB using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- TiDB code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving TiDB requires evidence-backed assessment.
...
- First-party source: https://github.com/pingcap/tidb
...
- [ ] Trace TiDB inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check TiDB defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check TiDB least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check TiDB resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check TiDB release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check TiDB dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If TiDB identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a TiDB change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- TiDB documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a TiDB architecture and dependency summary with trust boundaries and persistent state.
- Include TiDB validation commands or tests that are safe for the stated environment.
...
- TiDB identity, version, configuration, dependencies, and deployment assumptions are explicit.
- TiDB failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that TiDB is secure, supported, compatible, or suitable.
- Using an unpinned latest version of TiDB as the basis for migration or production guidance.
