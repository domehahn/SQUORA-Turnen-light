---
name: "cncf-sva-kcntp-reviewer"
description: "Review SVA (KCNTP) using its official documentation and repository in the Special / Kubernetes and Cloud Native Training Partner category. The services of the SVA include consulting, conception, implementation and operation of Kubernetes-based solutions as well as solution-specific and tailor-made trainings and exam preparations."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Sva Kcntp Reviewer

## Purpose

Review and operate SVA (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SVA (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SVA (KCNTP) requires evidence-backed assessment.
...
- First-party source: https://www.sva.de/de/services/devops-academy
...
- [ ] Trace SVA (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SVA (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SVA (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SVA (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SVA (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SVA (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SVA (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SVA (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SVA (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SVA (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include SVA (KCNTP) validation commands or tests that are safe for the stated environment.
...
- SVA (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SVA (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SVA (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SVA (KCNTP) as the basis for migration or production guidance.
