---
name: "cncf-ovn-kubernetes-reviewer"
description: "Review OVN-Kubernetes using its official documentation and repository in the Runtime / Cloud Native Network category. A robust Kubernetes networking platform"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Ovn Kubernetes Reviewer

## Purpose

Review and operate OVN-Kubernetes using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OVN-Kubernetes code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OVN-Kubernetes requires evidence-backed assessment.
...
- First-party source: https://github.com/ovn-kubernetes/ovn-kubernetes
...
- [ ] Trace OVN-Kubernetes inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OVN-Kubernetes defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OVN-Kubernetes least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OVN-Kubernetes resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OVN-Kubernetes release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OVN-Kubernetes dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OVN-Kubernetes identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OVN-Kubernetes change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OVN-Kubernetes documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OVN-Kubernetes architecture and dependency summary with trust boundaries and persistent state.
- Include OVN-Kubernetes validation commands or tests that are safe for the stated environment.
...
- OVN-Kubernetes identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OVN-Kubernetes failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OVN-Kubernetes is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OVN-Kubernetes as the basis for migration or production guidance.
