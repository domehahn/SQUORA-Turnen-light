---
name: "cncf-mogenius-kubernetes-platform-reviewer"
description: "Review mogenius Kubernetes Platform using its official documentation and repository in the Platform / Certified Kubernetes - Distribution category. mogenius combines comprehensive Kubernetes management with developer‑focused platform services."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Mogenius Kubernetes Platform Reviewer

## Purpose

Review and operate mogenius Kubernetes Platform using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- mogenius Kubernetes Platform code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving mogenius Kubernetes Platform requires evidence-backed assessment.
...
- First-party source: https://mogenius.com/platform
...
- [ ] Trace mogenius Kubernetes Platform inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check mogenius Kubernetes Platform defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check mogenius Kubernetes Platform least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check mogenius Kubernetes Platform resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check mogenius Kubernetes Platform release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check mogenius Kubernetes Platform dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If mogenius Kubernetes Platform identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a mogenius Kubernetes Platform change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- mogenius Kubernetes Platform documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a mogenius Kubernetes Platform architecture and dependency summary with trust boundaries and persistent state.
- Include mogenius Kubernetes Platform validation commands or tests that are safe for the stated environment.
...
- mogenius Kubernetes Platform identity, version, configuration, dependencies, and deployment assumptions are explicit.
- mogenius Kubernetes Platform failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that mogenius Kubernetes Platform is secure, supported, compatible, or suitable.
- Using an unpinned latest version of mogenius Kubernetes Platform as the basis for migration or production guidance.
