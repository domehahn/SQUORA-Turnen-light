---
name: "cncf-flows-network-reviewer"
description: "Review flows.network using its official documentation and repository in the Wasm / Hosted Platforms category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Flows Network Reviewer

## Purpose

Review and operate flows.network using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- flows.network code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving flows.network requires evidence-backed assessment.
...
- First-party source: https://flows.network/
...
- [ ] Trace flows.network inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check flows.network defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check flows.network least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check flows.network resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check flows.network release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check flows.network dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If flows.network identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a flows.network change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- flows.network documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a flows.network architecture and dependency summary with trust boundaries and persistent state.
- Include flows.network validation commands or tests that are safe for the stated environment.
...
- flows.network identity, version, configuration, dependencies, and deployment assumptions are explicit.
- flows.network failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that flows.network is secure, supported, compatible, or suitable.
- Using an unpinned latest version of flows.network as the basis for migration or production guidance.
