---
name: "cncf-emissary-ingress-reviewer"
description: "Review Emissary-Ingress using its official documentation and repository in the Orchestration & Management / API Gateway category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Emissary Ingress Reviewer

## Purpose

Review and operate Emissary-Ingress using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Emissary-Ingress code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Emissary-Ingress requires evidence-backed assessment.
...
- First-party source: https://github.com/emissary-ingress/emissary
...
- [ ] Trace Emissary-Ingress inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Emissary-Ingress defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Emissary-Ingress least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Emissary-Ingress resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Emissary-Ingress release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Emissary-Ingress dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Emissary-Ingress identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Emissary-Ingress change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Emissary-Ingress documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Emissary-Ingress architecture and dependency summary with trust boundaries and persistent state.
- Include Emissary-Ingress validation commands or tests that are safe for the stated environment.
...
- Emissary-Ingress identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Emissary-Ingress failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Emissary-Ingress is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Emissary-Ingress as the basis for migration or production guidance.
