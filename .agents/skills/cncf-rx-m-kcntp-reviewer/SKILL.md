...
Review and operate RX-M (KCNTP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- RX-M (KCNTP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving RX-M (KCNTP) requires evidence-backed assessment.
...
- First-party source: https://rx-m.com/cloudnative-training-courses/
...
- [ ] Trace RX-M (KCNTP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check RX-M (KCNTP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check RX-M (KCNTP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check RX-M (KCNTP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check RX-M (KCNTP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check RX-M (KCNTP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If RX-M (KCNTP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a RX-M (KCNTP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- RX-M (KCNTP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a RX-M (KCNTP) architecture and dependency summary with trust boundaries and persistent state.
- Include RX-M (KCNTP) validation commands or tests that are safe for the stated environment.
...
- RX-M (KCNTP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- RX-M (KCNTP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that RX-M (KCNTP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of RX-M (KCNTP) as the basis for migration or production guidance.
