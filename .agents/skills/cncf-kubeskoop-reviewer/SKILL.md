---
name: "cncf-kubeskoop-reviewer"
description: "Review KubeSkoop using its official documentation and repository in the Observability and Analysis / Observability category. KubeSkoop is a network monitoring & diagnosis suite for Kubernetes."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Kubeskoop Reviewer

## Purpose

Review and operate KubeSkoop using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- KubeSkoop code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving KubeSkoop requires evidence-backed assessment.
...
- First-party source: https://github.com/alibaba/kubeskoop
...
- [ ] Trace KubeSkoop inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check KubeSkoop defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check KubeSkoop least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check KubeSkoop resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check KubeSkoop release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check KubeSkoop dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If KubeSkoop identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a KubeSkoop change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- KubeSkoop documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a KubeSkoop architecture and dependency summary with trust boundaries and persistent state.
- Include KubeSkoop validation commands or tests that are safe for the stated environment.
...
- KubeSkoop identity, version, configuration, dependencies, and deployment assumptions are explicit.
- KubeSkoop failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that KubeSkoop is secure, supported, compatible, or suitable.
- Using an unpinned latest version of KubeSkoop as the basis for migration or production guidance.
