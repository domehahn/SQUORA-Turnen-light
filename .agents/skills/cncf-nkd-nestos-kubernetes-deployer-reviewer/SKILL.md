...
---
# Cncf Nkd Nestos Kubernetes Deployer Reviewer

## Purpose

Review and operate NKD - NestOS Kubernetes Deployer using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- NKD - NestOS Kubernetes Deployer code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving NKD - NestOS Kubernetes Deployer requires evidence-backed assessment.
...
- First-party source: https://github.com/openeuler-mirror/nestos-kubernetes-deployer
...
- [ ] Trace NKD - NestOS Kubernetes Deployer inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check NKD - NestOS Kubernetes Deployer defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check NKD - NestOS Kubernetes Deployer least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check NKD - NestOS Kubernetes Deployer resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check NKD - NestOS Kubernetes Deployer release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check NKD - NestOS Kubernetes Deployer dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If NKD - NestOS Kubernetes Deployer identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a NKD - NestOS Kubernetes Deployer change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- NKD - NestOS Kubernetes Deployer documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a NKD - NestOS Kubernetes Deployer architecture and dependency summary with trust boundaries and persistent state.
- Include NKD - NestOS Kubernetes Deployer validation commands or tests that are safe for the stated environment.
...
- NKD - NestOS Kubernetes Deployer identity, version, configuration, dependencies, and deployment assumptions are explicit.
- NKD - NestOS Kubernetes Deployer failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that NKD - NestOS Kubernetes Deployer is secure, supported, compatible, or suitable.
- Using an unpinned latest version of NKD - NestOS Kubernetes Deployer as the basis for migration or production guidance.
