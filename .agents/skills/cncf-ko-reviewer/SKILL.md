---
name: "cncf-ko-reviewer"
description: "Review ko using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category. ko is a tool to simplify building Go container images"
version: "1.0.0"
since: "2026-08-27"
...
Review and operate ko using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ko code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ko requires evidence-backed assessment.
...
- First-party source: https://github.com/ko-build/ko
...
- [ ] Trace ko inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ko defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ko least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ko resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ko release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ko dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ko identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ko change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ko documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ko architecture and dependency summary with trust boundaries and persistent state.
- Include ko validation commands or tests that are safe for the stated environment.
...
- ko identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ko failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ko is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ko as the basis for migration or production guidance.
