...
---
# Cncf Azure Monitor Reviewer

## Purpose

Review and operate Azure Monitor using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Azure Monitor code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Azure Monitor requires evidence-backed assessment.
...
- First-party source: https://azure.microsoft.com/en-us/services/monitor/
...
- [ ] Trace Azure Monitor inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Azure Monitor defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Azure Monitor least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Azure Monitor resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Azure Monitor release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Azure Monitor dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Azure Monitor identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Azure Monitor change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Azure Monitor documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Azure Monitor architecture and dependency summary with trust boundaries and persistent state.
- Include Azure Monitor validation commands or tests that are safe for the stated environment.
...
- Azure Monitor identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Azure Monitor failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Azure Monitor is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Azure Monitor as the basis for migration or production guidance.
