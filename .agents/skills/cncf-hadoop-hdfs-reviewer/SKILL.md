---
name: "cncf-hadoop-hdfs-reviewer"
description: "Review Hadoop HDFS using its official documentation and repository in the Data / Data Architecture category. Open source framework works by rapidly transferring data between nodes. It's often used by companies who need to handle and store big data."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Hadoop Hdfs Reviewer

## Purpose

Review and operate Hadoop HDFS using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Hadoop HDFS code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Hadoop HDFS requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/hadoop
...
- [ ] Trace Hadoop HDFS inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Hadoop HDFS defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Hadoop HDFS least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Hadoop HDFS resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Hadoop HDFS release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Hadoop HDFS dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Hadoop HDFS identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Hadoop HDFS change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Hadoop HDFS documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Hadoop HDFS architecture and dependency summary with trust boundaries and persistent state.
- Include Hadoop HDFS validation commands or tests that are safe for the stated environment.
...
- Hadoop HDFS identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Hadoop HDFS failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Hadoop HDFS is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Hadoop HDFS as the basis for migration or production guidance.
