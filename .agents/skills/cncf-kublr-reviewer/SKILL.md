---
name: "cncf-kublr-reviewer"
description: "Review Kublr using its official documentation and repository in the Platform / Certified Kubernetes - Distribution category. Accelerate and control the deployment, scaling, monitoring and management of your containerized applications."
version: "1.0.0"
since: "2026-08-27"
...
Review and operate Kublr using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Kublr code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Kublr requires evidence-backed assessment.
...
- [ ] Trace Kublr inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Kublr defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Kublr least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Kublr resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Kublr release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Kublr dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Kublr identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Kublr change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Kublr documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Kublr architecture and dependency summary with trust boundaries and persistent state.
- Include Kublr validation commands or tests that are safe for the stated environment.
...
- Kublr identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Kublr failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Kublr is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Kublr as the basis for migration or production guidance.
