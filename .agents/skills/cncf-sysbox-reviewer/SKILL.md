---
name: "cncf-sysbox-reviewer"
description: "Review Sysbox using its official documentation and repository in the Runtime / Container Runtime category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Sysbox Reviewer

## Purpose

Review and operate Sysbox using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Sysbox code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Sysbox requires evidence-backed assessment.
...
- First-party source: https://github.com/nestybox/sysbox
...
- [ ] Trace Sysbox inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Sysbox defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Sysbox least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Sysbox resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Sysbox release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Sysbox dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Sysbox identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Sysbox change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Sysbox documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Sysbox architecture and dependency summary with trust boundaries and persistent state.
- Include Sysbox validation commands or tests that are safe for the stated environment.
...
- Sysbox identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Sysbox failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Sysbox is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Sysbox as the basis for migration or production guidance.
