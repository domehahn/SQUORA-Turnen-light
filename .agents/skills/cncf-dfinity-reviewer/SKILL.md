---
name: "cncf-dfinity-reviewer"
description: "Review Dfinity using its official documentation and repository in the Wasm / Decentralized Platforms category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Dfinity Reviewer

## Purpose

Review and operate Dfinity using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Dfinity code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Dfinity requires evidence-backed assessment.
...
- First-party source: https://github.com/dfinity/ic
...
- [ ] Trace Dfinity inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Dfinity defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Dfinity least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Dfinity resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Dfinity release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Dfinity dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Dfinity identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Dfinity change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Dfinity documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Dfinity architecture and dependency summary with trust boundaries and persistent state.
- Include Dfinity validation commands or tests that are safe for the stated environment.
...
- Dfinity identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Dfinity failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Dfinity is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Dfinity as the basis for migration or production guidance.
