---
name: "cncf-hatchet-reviewer"
description: "Review Hatchet using its official documentation and repository in the AI Agent / Workflow Orchestration category. Run Background Tasks at Scale"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Hatchet Reviewer

## Purpose

Review and operate Hatchet using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Hatchet code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Hatchet requires evidence-backed assessment.
...
- First-party source: https://github.com/hatchet-dev/hatchet
...
- [ ] Trace Hatchet inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Hatchet defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Hatchet least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Hatchet resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Hatchet release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Hatchet dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Hatchet identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Hatchet change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Hatchet documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Hatchet architecture and dependency summary with trust boundaries and persistent state.
- Include Hatchet validation commands or tests that are safe for the stated environment.
...
- Hatchet identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Hatchet failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Hatchet is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Hatchet as the basis for migration or production guidance.
