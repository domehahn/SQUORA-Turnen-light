---
name: "cncf-spinkube-reviewer"
description: "Review SpinKube using its official documentation and repository in the Wasm / Orchestration & Management category. Open source platform for efficiently running (containerless) Spin-based WebAssembly (Wasm) applications on Kubernetes."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Spinkube Reviewer

## Purpose

Review and operate SpinKube using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SpinKube code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SpinKube requires evidence-backed assessment.
...
- First-party source: https://github.com/spinframework/spin-operator
...
- [ ] Trace SpinKube inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SpinKube defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SpinKube least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SpinKube resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SpinKube release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SpinKube dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SpinKube identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SpinKube change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SpinKube documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SpinKube architecture and dependency summary with trust boundaries and persistent state.
- Include SpinKube validation commands or tests that are safe for the stated environment.
...
- SpinKube identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SpinKube failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SpinKube is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SpinKube as the basis for migration or production guidance.
