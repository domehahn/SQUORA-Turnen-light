---
name: "cncf-openstack-reviewer"
description: "Review OpenStack using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Openstack Reviewer

## Purpose

Review and operate OpenStack using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- OpenStack code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving OpenStack requires evidence-backed assessment.
...
- First-party source: https://github.com/openstack/openstack
...
- [ ] Trace OpenStack inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check OpenStack defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check OpenStack least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check OpenStack resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check OpenStack release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check OpenStack dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If OpenStack identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a OpenStack change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- OpenStack documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a OpenStack architecture and dependency summary with trust boundaries and persistent state.
- Include OpenStack validation commands or tests that are safe for the stated environment.
...
- OpenStack identity, version, configuration, dependencies, and deployment assumptions are explicit.
- OpenStack failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that OpenStack is secure, supported, compatible, or suitable.
- Using an unpinned latest version of OpenStack as the basis for migration or production guidance.
