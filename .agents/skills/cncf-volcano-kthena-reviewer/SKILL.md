---
name: "cncf-volcano-kthena-reviewer"
description: "Review Volcano Kthena using its official documentation and repository in the Inference / Framework category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Volcano Kthena Reviewer

## Purpose

Review and operate Volcano Kthena using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Volcano Kthena code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Volcano Kthena requires evidence-backed assessment.
...
- First-party source: https://github.com/volcano-sh/kthena
...
- [ ] Trace Volcano Kthena inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Volcano Kthena defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Volcano Kthena least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Volcano Kthena resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Volcano Kthena release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Volcano Kthena dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Volcano Kthena identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Volcano Kthena change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Volcano Kthena documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Volcano Kthena architecture and dependency summary with trust boundaries and persistent state.
- Include Volcano Kthena validation commands or tests that are safe for the stated environment.
...
- Volcano Kthena identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Volcano Kthena failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Volcano Kthena is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Volcano Kthena as the basis for migration or production guidance.
