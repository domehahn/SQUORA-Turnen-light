---
name: "cncf-seatunnel-reviewer"
description: "Review SeaTunnel using its official documentation and repository in the App Definition and Development / Streaming & Messaging category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Seatunnel Reviewer

## Purpose

Review and operate SeaTunnel using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- SeaTunnel code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving SeaTunnel requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/seatunnel
...
- [ ] Trace SeaTunnel inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check SeaTunnel defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check SeaTunnel least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check SeaTunnel resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check SeaTunnel release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check SeaTunnel dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If SeaTunnel identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a SeaTunnel change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- SeaTunnel documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a SeaTunnel architecture and dependency summary with trust boundaries and persistent state.
- Include SeaTunnel validation commands or tests that are safe for the stated environment.
...
- SeaTunnel identity, version, configuration, dependencies, and deployment assumptions are explicit.
- SeaTunnel failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that SeaTunnel is secure, supported, compatible, or suitable.
- Using an unpinned latest version of SeaTunnel as the basis for migration or production guidance.
