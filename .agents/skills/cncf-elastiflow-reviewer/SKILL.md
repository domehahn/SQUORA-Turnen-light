---
name: "cncf-elastiflow-reviewer"
description: "Review ElastiFlow using its official documentation and repository in the Observability and Analysis / Observability category. ElastiFlow brings network observability and insights to OpenTelemetry."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Elastiflow Reviewer

## Purpose

Review and operate ElastiFlow using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- ElastiFlow code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving ElastiFlow requires evidence-backed assessment.
...
- [ ] Trace ElastiFlow inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check ElastiFlow defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check ElastiFlow least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check ElastiFlow resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check ElastiFlow release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check ElastiFlow dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If ElastiFlow identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a ElastiFlow change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- ElastiFlow documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a ElastiFlow architecture and dependency summary with trust boundaries and persistent state.
- Include ElastiFlow validation commands or tests that are safe for the stated environment.
...
- ElastiFlow identity, version, configuration, dependencies, and deployment assumptions are explicit.
- ElastiFlow failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that ElastiFlow is secure, supported, compatible, or suitable.
- Using an unpinned latest version of ElastiFlow as the basis for migration or production guidance.
