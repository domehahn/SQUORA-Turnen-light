---
name: "cncf-cfengine-reviewer"
description: "Review CFEngine using its official documentation and repository in the Provisioning / Automation & Configuration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Cfengine Reviewer

## Purpose

Review and operate CFEngine using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- CFEngine code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving CFEngine requires evidence-backed assessment.
...
- First-party source: https://github.com/cfengine/core
...
- [ ] Trace CFEngine inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check CFEngine defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check CFEngine least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check CFEngine resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check CFEngine release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check CFEngine dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If CFEngine identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a CFEngine change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- CFEngine documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a CFEngine architecture and dependency summary with trust boundaries and persistent state.
- Include CFEngine validation commands or tests that are safe for the stated environment.
...
- CFEngine identity, version, configuration, dependencies, and deployment assumptions are explicit.
- CFEngine failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that CFEngine is secure, supported, compatible, or suitable.
- Using an unpinned latest version of CFEngine as the basis for migration or production guidance.
