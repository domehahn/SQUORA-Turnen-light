---
name: "cncf-omnistrate-member-reviewer"
...
Review and operate Omnistrate (member) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Omnistrate (member) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Omnistrate (member) requires evidence-backed assessment.
...
- First-party source: https://omnistrate.ai
...
- [ ] Trace Omnistrate (member) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Omnistrate (member) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Omnistrate (member) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Omnistrate (member) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Omnistrate (member) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Omnistrate (member) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Omnistrate (member) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Omnistrate (member) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Omnistrate (member) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Omnistrate (member) architecture and dependency summary with trust boundaries and persistent state.
- Include Omnistrate (member) validation commands or tests that are safe for the stated environment.
...
- Omnistrate (member) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Omnistrate (member) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Omnistrate (member) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Omnistrate (member) as the basis for migration or production guidance.
