...
---
# Cncf Arcfra Kubernetes Engine Reviewer

## Purpose

Review and operate Arcfra Kubernetes Engine using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Arcfra Kubernetes Engine code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Arcfra Kubernetes Engine requires evidence-backed assessment.
...
- First-party source: https://www.arcfra.com/kubernetes-engine/
...
- [ ] Trace Arcfra Kubernetes Engine inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Arcfra Kubernetes Engine defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Arcfra Kubernetes Engine least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Arcfra Kubernetes Engine resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Arcfra Kubernetes Engine release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Arcfra Kubernetes Engine dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Arcfra Kubernetes Engine identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Arcfra Kubernetes Engine change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Arcfra Kubernetes Engine documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Arcfra Kubernetes Engine architecture and dependency summary with trust boundaries and persistent state.
- Include Arcfra Kubernetes Engine validation commands or tests that are safe for the stated environment.
...
- Arcfra Kubernetes Engine identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Arcfra Kubernetes Engine failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Arcfra Kubernetes Engine is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Arcfra Kubernetes Engine as the basis for migration or production guidance.
