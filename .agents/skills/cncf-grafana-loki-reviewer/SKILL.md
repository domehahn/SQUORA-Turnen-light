...
---
# Cncf Grafana Loki Reviewer

## Purpose

Review and operate Grafana Loki using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Grafana Loki code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Grafana Loki requires evidence-backed assessment.
...
- First-party source: https://github.com/grafana/loki
...
- [ ] Trace Grafana Loki inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Grafana Loki defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Grafana Loki least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Grafana Loki resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Grafana Loki release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Grafana Loki dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Grafana Loki identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Grafana Loki change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Grafana Loki documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Grafana Loki architecture and dependency summary with trust boundaries and persistent state.
- Include Grafana Loki validation commands or tests that are safe for the stated environment.
...
- Grafana Loki identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Grafana Loki failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Grafana Loki is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Grafana Loki as the basis for migration or production guidance.
