---
name: "cncf-chef-inspec-reviewer"
description: "Review Chef InSpec using its official documentation and repository in the Provisioning / Security & Compliance category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Chef Inspec Reviewer

## Purpose

Review and operate Chef InSpec using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Chef InSpec code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Chef InSpec requires evidence-backed assessment.
...
- First-party source: https://github.com/inspec/inspec
...
- [ ] Trace Chef InSpec inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Chef InSpec defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Chef InSpec least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Chef InSpec resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Chef InSpec release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Chef InSpec dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Chef InSpec identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Chef InSpec change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Chef InSpec documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Chef InSpec architecture and dependency summary with trust boundaries and persistent state.
- Include Chef InSpec validation commands or tests that are safe for the stated environment.
...
- Chef InSpec identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Chef InSpec failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Chef InSpec is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Chef InSpec as the basis for migration or production guidance.
