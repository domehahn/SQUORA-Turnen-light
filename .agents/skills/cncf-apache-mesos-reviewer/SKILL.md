---
name: "cncf-apache-mesos-reviewer"
description: "Review Apache Mesos using its official documentation and repository in the Orchestration & Management / Scheduling & Orchestration category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Apache Mesos Reviewer

## Purpose

Review and operate Apache Mesos using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Apache Mesos code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Apache Mesos requires evidence-backed assessment.
...
- First-party source: https://github.com/apache/mesos
...
- [ ] Trace Apache Mesos inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Apache Mesos defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Apache Mesos least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Apache Mesos resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Apache Mesos release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Apache Mesos dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Apache Mesos identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Apache Mesos change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Apache Mesos documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Apache Mesos architecture and dependency summary with trust boundaries and persistent state.
- Include Apache Mesos validation commands or tests that are safe for the stated environment.
...
- Apache Mesos identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Apache Mesos failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Apache Mesos is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Apache Mesos as the basis for migration or production guidance.
