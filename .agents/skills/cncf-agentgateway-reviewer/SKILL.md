---
name: "cncf-agentgateway-reviewer"
description: "Review Agentgateway using its official documentation and repository in the AI Native Infra / Gateway category. Next Generation Agentic Proxy for AI Agents and MCP servers"
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Agentgateway Reviewer

## Purpose

Review and operate Agentgateway using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Agentgateway code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Agentgateway requires evidence-backed assessment.
...
- First-party source: https://github.com/agentgateway/agentgateway
...
- [ ] Trace Agentgateway inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Agentgateway defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Agentgateway least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Agentgateway resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Agentgateway release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Agentgateway dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Agentgateway identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Agentgateway change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Agentgateway documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Agentgateway architecture and dependency summary with trust boundaries and persistent state.
- Include Agentgateway validation commands or tests that are safe for the stated environment.
...
- Agentgateway identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Agentgateway failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Agentgateway is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Agentgateway as the basis for migration or production guidance.
