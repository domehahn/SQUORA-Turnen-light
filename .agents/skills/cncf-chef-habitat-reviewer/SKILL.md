---
name: "cncf-chef-habitat-reviewer"
description: "Review Chef Habitat using its official documentation and repository in the App Definition and Development / Application Definition & Image Build category."
version: "1.0.0"
since: "2026-08-27"
...
---
# Cncf Chef Habitat Reviewer

## Purpose

Review and operate Chef Habitat using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- Chef Habitat code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving Chef Habitat requires evidence-backed assessment.
...
- First-party source: https://github.com/habitat-sh/habitat
...
- [ ] Trace Chef Habitat inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check Chef Habitat defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check Chef Habitat least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check Chef Habitat resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check Chef Habitat release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check Chef Habitat dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If Chef Habitat identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a Chef Habitat change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- Chef Habitat documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a Chef Habitat architecture and dependency summary with trust boundaries and persistent state.
- Include Chef Habitat validation commands or tests that are safe for the stated environment.
...
- Chef Habitat identity, version, configuration, dependencies, and deployment assumptions are explicit.
- Chef Habitat failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that Chef Habitat is secure, supported, compatible, or suitable.
- Using an unpinned latest version of Chef Habitat as the basis for migration or production guidance.
