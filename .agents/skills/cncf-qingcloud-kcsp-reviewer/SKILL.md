...
---
# Cncf Qingcloud Kcsp Reviewer

## Purpose

Review and operate QingCloud (KCSP) using current official documentation, repository evidence, and its CNCF Landscape classification without treating Landscape inclusion as endorsement.
...
- QingCloud (KCSP) code, configuration, APIs, deployment, integrations, dependencies, upgrades, or operations are in scope.
- A technology choice or migration involving QingCloud (KCSP) requires evidence-backed assessment.
...
- First-party source: https://www.qingcloud.com/products/kubesphereqke/
...
- [ ] Trace QingCloud (KCSP) inputs, outputs, identities, trust boundaries, external dependencies, and persistent state.
- [ ] Check QingCloud (KCSP) defaults, configuration precedence, environment separation, and drift from reviewed source.
- [ ] Check QingCloud (KCSP) least privilege, credential rotation, audit events, policy enforcement, and break-glass behavior.
- [ ] Check QingCloud (KCSP) resource ownership, cleanup, quotas, rate limits, timeouts, retries, and backpressure.
- [ ] Check QingCloud (KCSP) release notes, supported upgrade paths, schema or API compatibility, and rollback constraints.
- [ ] Check QingCloud (KCSP) dashboards, alerts, health signals, incident procedures, backup evidence, and recovery exercises.
...
- If QingCloud (KCSP) identity, version, edition, or deployment model cannot be established, report the uncertainty before recommending a change.
- If a QingCloud (KCSP) change can affect availability, security boundaries, persistent data, or external consumers, require staged validation and explicit rollback criteria.
...
- QingCloud (KCSP) documentation, ownership, maintenance, licensing, provenance, or evidence gap.
...
- Include a QingCloud (KCSP) architecture and dependency summary with trust boundaries and persistent state.
- Include QingCloud (KCSP) validation commands or tests that are safe for the stated environment.
...
- QingCloud (KCSP) identity, version, configuration, dependencies, and deployment assumptions are explicit.
- QingCloud (KCSP) failure, upgrade, rollback, observability, and recovery paths have proportionate evidence.
...
- Assuming Landscape inclusion or popularity proves that QingCloud (KCSP) is secure, supported, compatible, or suitable.
- Using an unpinned latest version of QingCloud (KCSP) as the basis for migration or production guidance.
